...
<tr>
  <td>
    <checkbox-sk ?checked=${ele._checked.has(autoroller.name)} @click=${ele._clickHandler} id=${autoroller.name}></checkbox-sk>
  </td>
  <td>
...
<div class="autorollers">
  <radio-sk label="Caution" name=tree_state @change=${() => ele._radioHandler("Caution")}></radio-sk>
  <radio-sk label="Closed" name=tree_state @change=${() => ele._radioHandler("Closed")}></radio-sk>
<table>
  ${autorollerRow(ele)}
</table>
</div>
...
define('list-autorollers-sk', class extends ElementSk {
  constructor() {
    ...
    this._selectedTreeStatus = new Set();
  }
  ...
  _getTreeStatusFromAutorollers() {
    let treeStatus = '';
    $('radio-sk', this).forEach((radio) => {
      if (radio.checked) {
       treeStatus = radio.label;
      }
    });
    let suffix = Array.from(this._checkedAutorollers).join(',');
    if (suffix) {
      let roller = "roller";
      if (this._checkedAutorollers.size > 1) {
        roller = "rollers";
      }
      treeStatus = treeStatus + ": Waiting for " + suffix + " " + roller + " to turn green"
    }
    return treeStatus;
  }
