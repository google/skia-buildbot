...
type Status struct {
	...
	// New field. One of open/closed/caution.
...
func AddStatus(message, username, generalState, rollers string) error {
	...
	if _, err := DS.RunInTransaction(context.Background(), func(tx *datastore.Transaction) error {
		var err error
