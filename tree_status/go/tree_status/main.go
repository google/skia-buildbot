...
func (srv *Server) addStatusHandler(w http.ResponseWriter, r *http.Request) {
	...
	if (containsOpenState && containsCautionState) ||
		...
		httputils.ReportError(w, nil, fmt.Sprintf("Cannot specify two keywords from (%s, %s, %s) in a status message.", OPEN_STATE, CAUTION_STATE, CLOSED_STATE), http.StatusInternalServerError)
		return
	} else if !(containsOpenState || containsCautionState || containsClosedState) {
		httputils.ReportError(w, nil, fmt.Sprintf("Must specify either (%s, %s, %s) somewhere in the status message.", OPEN_STATE, CAUTION_STATE, CLOSED_STATE), http.StatusInternalServerError)
		return
	}
	...
	fmt.Println(s)
	if err := AddStatus(message, user); err != nil {
...
func main() {
	if err := DatastoreInit(*project, *namespace, *baseapp.Local); err != nil {
		sklog.Fatal(err)
	}
	statuses, err := GetStatuses(25)
	fmt.Println(err)
