...
func AutorollersInit(ctx context.Context, ts oauth2.TokenSource) error {
	...
	// Below is initial call. In start watching you will be doing more.
	AutorollStatuses = []autoRollStatus{}
	...
	go func() {
		for {
			...
			case roller := <-RollersToWatch:
				fmt.Printf("Need to watch %s\n", roller)
				fmt.Println("Starting goroutine to poll every 10 seconds")
			...
			fmt.Println("OUT OF THE LOOP, WHAT DOES THIS MEAN NOW???")
		}
	}()
...
func StartWatchingAutorollers(rollers string) {
	fmt.Println("STARTING TO WATCH AUTOROLLERS")
	fmt.Println(rollers)
	fmt.Println(RollersToWatch)
