...
import (
	...
	bt_gitstore_old "go.skia.org/infra/go/gitstore_deprecated/bt_gitstore"
...
var (
	...
	bTable          = flag.String("b_table", "git-repos2", "BigTable table.")
...
func compareBranch(ctx context.Context, a gitstore_deprecated.GitStore, b gitstore.GitStore, branch string, endIdx int) error {
	// Compare IndexCommits in chunks.
	var egroup errgroup.Group
	_ = util.ChunkIter(endIdx+1, 5000, func(start, end int) error {
		egroup.Go(func() error {
			sklog.Debugf("Check %s %d-%d of %d", branch, start, end, endIdx)
...
func convertBranchPointers(orig map[string]*gitstore_deprecated.BranchPointer) map[string]*gitstore.BranchPointer {
	rv := make(map[string]*gitstore.BranchPointer, len(orig))
	for k, v := range orig {
		rv[k] = &gitstore.BranchPointer{
			Head:  v.Head,
			Index: v.Index,
		}
	}
...
func compareRepos(ctx context.Context, a gitstore_deprecated.GitStore, b gitstore.GitStore) error {
	// Compare branch heads.
	aBranchesOrig, err := a.GetBranches(ctx)
	if err != nil {
		return err
	}
	aBranches := convertBranchPointers(aBranchesOrig)
	bBranches, err := b.GetBranches(ctx)
	if err != nil {
		return err
	}
	// TODO(borenet): Why is ALL_BRANCHES missing from bBranches?
	delete(aBranches, gitstore_deprecated.ALL_BRANCHES)
...
func main() {
	...
	aCfg := &bt_gitstore_old.BTConfig{
		ProjectID:  *aProject,
		InstanceID: *aInstance,
		TableID:    *aTable,
	}
	a, err := bt_gitstore_old.New(ctx, aCfg, *aRepo)
	if err != nil {
		sklog.Fatal(err)
	}
