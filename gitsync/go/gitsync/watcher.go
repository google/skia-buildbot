...
func (r *repoImpl) Update(ctx context.Context) error {
	...
	for _, branch := range branches {
		...
		if oldBranch != nil {
			...
			commits, err := r.gitiles.Log(ctx, oldBranch.Head, branch.Head)
			if err != nil {
...
func (r *repoImpl) Details(ctx context.Context, hash string) (*vcsinfo.LongCommit, error) {
	...
	// we've seen before.
	addedCommits := 0
	if err := r.gitiles.LogFnBatch(ctx, hash, func(ctx context.Context, commits []*vcsinfo.LongCommit) error {
		stop := false
