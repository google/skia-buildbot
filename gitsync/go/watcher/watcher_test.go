...
func (u *gitsyncRefresher) checkIngestion(ctx context.Context) {
	...
	for {
		...
		sklog.Errorf("Waiting for GitStore to be up to date.")
	}

	// Dump the repo graph.
	out, err := u.repo.Git(ctx, "log", "--graph", "--all", "--decorate=short", "--format=%h %D")
	assert.NoError(u.t, err)
	sklog.Errorf("\n%s", out)
	...
	for name := range expectBranches {
		...
		assert.NoError(u.t, err)
		sklog.Errorf("Checking index commits for %s", name)
		for _, ic := range expectIndexCommits {
			sklog.Errorf("  Expect %d %s", ic.Index, ic.Hash[:7])
		}
		for _, ic := range gotIndexCommits {
			sklog.Errorf("  Actual %d %s", ic.Index, ic.Hash[:7])
		}
