...
func TestIngestCommits(t *testing.T) {
	...
	assert.NoError(t, ri.ingestCommits(ctx, ri.gitstore, func(ctx context.Context, ch chan<- []*vcsinfo.LongCommit) error {
	...
	assert.Equal(t, err, ri.ingestCommits(ctx, ri.gitstore, func(ctx context.Context, ch chan<- []*vcsinfo.LongCommit) error {
	...
	err = ri.ingestCommits(ctx, ri.gitstore, func(ctx context.Context, ch chan<- []*vcsinfo.LongCommit) error {
		for i := 5; i < 10; i++ {
	...
	assert.True(t, strings.Contains(err.Error(), "GitStore ingestion failed with: commit ingestion failed"))
...
func (u *gitsyncRefresher) Refresh(commits ...*vcsinfo.LongCommit) {
	...
	for _, b := range branches {
		if b.Head != u.oldBranches[b.Name] {
			u.gitiles.MockLog(ctx, false, b.Head)
		}
...
func (u *gitsyncRefresher) checkIngestion(ctx context.Context) {
	...
	out, err := u.repo.Git(ctx, "log", "--graph", "--format=%h", "--all", "--decorate")
	assert.NoError(u.t, err)
	sklog.Errorf("\n%s", out)

	// Assert that the IndexCommits are correct for each branch.
	for _, branch := range expectBranches {
		sklog.Errorf("Checking indexcommits for %s", branch.Name)
		...
		for i := len(branchCommits) - 1; i >= 0; i-- {
			...
			sklog.Errorf("Expect: %s", c.Hash)
		}
		gotIndexCommits, err := u.gs.RangeN(ctx, 0, branchPtr.Index+1, branch.Name)
		assert.NoError(u.t, err)
		for _, c := range gotIndexCommits {
			sklog.Errorf("Actual: %s", c.Hash)
		}
...
func setupGitsync(t *testing.T) (context.Context, *git_testutils.GitBuilder, *repograph.Graph, repograph_shared_tests.RepoImplRefresher, func()) {
	...
	sklog.Error("new repograph for test")
	graph, err := repograph.NewWithRepoImpl(ctx, ri)
	assert.NoError(t, err)
	graph.EnableBranchTracking()
