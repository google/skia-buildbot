...
func (u *gitsyncRefresher) checkIngestion(ctx context.Context) {
	...
	for _, c := range u.graph.GetAll() {
		deepequal_testutils.AssertDeepEqual(u.t, c.LongCommit, commits[c.Hash])
	}
	...
	for name := range expectBranches {
		...
		require.NoError(u.t, err)
		deepequal_testutils.AssertDeepEqual(u.t, expectIndexCommits, gotIndexCommits)
...
func TestMissingOldBranchHeadFallback(t *testing.T) {
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, map[string]*gitstore.BranchPointer{
		"master": {
			Head:  deleted,
			Index: 1,
		},
