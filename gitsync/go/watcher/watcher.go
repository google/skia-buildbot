...
func NewRepoWatcher(ctx context.Context, conf *bt_gitstore.BTConfig, repoURL, gitcookiesPath string) (*RepoWatcher, error) {
	...
	sklog.Errorf("NewWithRepoImpl")
	repo, err := repograph.NewWithRepoImpl(ctx, ri)
	if err != nil {
		return nil, err
	}
	sklog.Errorf("Enable branch tracking")
...
func (r *repoImpl) initialIngestion(ctx context.Context) error {
	...
	sklog.Infof("Putting %d commits into GitStore.", len(r.commits))
	putCommits := make([]*vcsinfo.LongCommit, 0, len(r.commits))
...
func (r *repoImpl) UpdateCallback(ctx context.Context, added, removed []*vcsinfo.LongCommit, graph *repograph.Graph) error {
	sklog.Infof("New graph has %d commits", graph.Len())
	for _, c := range added {
		sklog.Infof("%s\n%+v", c.Hash, c)
	}
