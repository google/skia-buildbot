...
func (r *repoImpl) UpdateCallback(ctx context.Context, added, removed []*vcsinfo.LongCommit, graph *repograph.Graph) error {
	sklog.Errorf("UpdateCallback")
	defer sklog.Errorf("UpdateCallback done")
	...
	for _, c := range modified {
		sklog.Errorf("Modified %s (%+v)", c.Hash, c.Branches)
		modifiedMap[c.Hash] = c
	}
	// Don't include commits in the 'removed' list.
	for _, c := range removed {
		sklog.Errorf("Removed %s (%+v)", c.Hash, c.Branches)
	...
	sklog.Errorf("Putting %d new/modified commits into GitStore.", len(putCommits))
	...
	for _, b := range branchHeads {
		sklog.Errorf("Have %s @ %s", b.Name, b.Head)
