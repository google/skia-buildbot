...
func (r *RepoWatcher) updateFn() {
	...
	if err != nil {
		sklog.Errorf("Failed to get branches from Git repo: %s", err)
		return
	}
	...
	for _, newBranch := range branches {
		...
		if ok {
			...
			// See if the new branch head is a decendent of the old branch head.
			anc, err := r.repo.IsAncestor(ctx, foundBranch.Head, newBranch.Head)
			if err != nil {
				sklog.Errorf("Error checking if %s is an ancestor of %s: %s", foundBranch.Head, newBranch.Head, err)
				return
			}

			// Either retrieve all hashes or only the ones between the old and new head.
			...
			if err != nil {
				sklog.Errorf("Error retrieving hashes with the argument %q: %s", revListStr, err)
				return
			}
