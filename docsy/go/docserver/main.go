...
import (
	...
	"go.skia.org/infra/docsy/go/codereview/gerrit"
	"go.skia.org/infra/docsy/go/docset"
...
// flags
var (
	docPath   = flag.String("doc_path", "site", "The relative directory where the documents are located.")
...
func new() (*server, error) {
	...
	docset, err := docset.New(context.Background(), *workDir, *docPath, *docsyDir, *docRepo, codeReview, docsy)
	if err != nil {
		sklog.Fatal(err)
	}
	docset.Start()
...
func (s *server) mainHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httputils.ReportError(w, err, "Failed to load.", http.StatusInternalServerError)
		return
	}
