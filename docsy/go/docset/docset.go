...
func NewDocsy(hugoExe string, docsyDir string) *docsy {
	return &docsy{
		hugoExe:  hugoExe,
		docsyDir: docsyDir,
	}
...
func (d *docsy) Render(src, dst string) error {
	sklog.Fatal("TBD")
	return nil
}
...
type CodeReview interface {
	...
	ListModifiedFiles(ctx context.Context, issue Issue) ([]ListModifiedFilesResult, error)

	GetFile(ctx context.Context, issue Issue, filename string) (io.ReadCloser, error)
}
...
func (cr *gerritCodeReview) ListModifiedFiles(ctx context.Context, issue Issue) ([]ListModifiedFilesResult, error) {
	// Use this to get all changed files as of the last revision.
	// https://skia-review.googlesource.com/changes/386796/detail?o=CURRENT_REVISION&o=ALL_FILES
	ci, err := cr.gc.GetChangeWithFiles(ctx, string(issue))
	if err != nil {
		return nil, skerr.Wrap(err)
	}
	ret := []ListModifiedFilesResult{}
	// There will only be one entry in Revisions.
	for _, rev := range ci.Revisions {
		for filename, fileinfo := range rev.Files {
...
func (d *docSet) getCacheEntry(issue Issue) *entry {
	...
	if entry, ok := d.cache[issue]; ok {
		// TODO - If the cache is too old then ignore the cache entry and check
		// if the issue has been updated.

		// If issue is closed then delete from cache.
		return entry
	}
	return nil
}
...
func (d *docSet) FileSystem(ctx context.Context, issue Issue) (http.FileSystem, error) {
	if e := d.getCacheEntry(issue); e != nil {
		return e.fs, nil
	}
	...
	// TODO We hold a single lock at this point. A better solution would be to
	// move the mutex into the cache entry and then only hold the mutex
	// per-issue.
	...
	if issue == d.codeReview.MainIssue() {
		_, err := gitinfo.CloneOrUpdate(ctx, d.repoURL, filepath.Join(d.workDir, contentSubDirectory, string(issue)), false)
		if err != nil {
			return nil, skerr.Wrap(err)
		}
	} else {
		...
		copyFilesAsLinks(mainSrcDir, srcDir)

		// Then download the patched files from the code review system.
		files, err := d.codeReview.ListModifiedFiles(ctx, issue)
		if err != nil {
			return nil, skerr.Wrap(err)
		}
		for _, fileinfo := range files {
			if fileinfo.Deleted {
				os.Remove(filepath.Join(dstDir, fileinfo.Filename))
				continue
			}

			r, err := d.codeReview.GetFile(ctx, issue, fileinfo.Filename)
			if err != nil {
				return nil, skerr.Wrap(err)
			}
	...
	if err := d.docsy.Render(srcDir, dstDir); err != nil {
		return nil, skerr.Wrap(err)
	}
