...
//
// repoURL is he URL of the repo passed to 'git clone'.
//
	...
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		...
		if _, err := os.Stat(dstFile); !os.IsNotExist(err) {
			if err := os.Remove(dstFile); err != nil {
				sklog.Errorf("Failed to remove %q: %s", dstFile, err)
			}
		}
...
func (d *docSet) singleStep(ctx context.Context) {
	for issue := range d.cache {
		if issue == codereview.MainIssue {
			continue
		}
		_, isClosed, err := d.codeReview.GetPatchsetInfo(ctx, issue)
		if err != nil {
