...
type CodeReview interface {
	...
	// issue at the most recent patchset.
	ListModifiedFiles(ctx context.Context, issue Issue) ([]ListModifiedFilesResult, error)

	// GetFiles returns an io.ReadCloser with the contents of the given file at
	// the latest patchset of the given issue.
	GetFile(ctx context.Context, filename string, ref string) ([]byte, error)

	// GetPatchset returns the most recent patchset Ref of the given issue and
...
// GetPatchset implements CodeReview.
...
func (d *docSet) FileSystem(ctx context.Context, issue Issue) (http.FileSystem, error) {
	...
	if e == nil || time.Now().After(e.lastPatchsetCheck.Add(refreshDuration)) {
		patchsetRef := ""
		isClosed := false
		...
		if patchsetRef == e.patchsetRef {
			// Not udpated, so just update the timestamp.
			e.lastPatchsetCheck = time.Now()
