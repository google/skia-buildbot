...
//
// The DocSet will run a background
type DocSet interface {
	// FileSystem returns a FileSystem of the rendered contents of the
	// documentation. Pass in the NoIssue and NoPatchset to get the main branch.
	FileSystem(issue, patchset int64) (http.FileSystem, error)
...
type docSet struct {
	...
	// The path under the repo where the docs are stored, e.g. "site" for Skia.
	docPath string
	...
	// from HEAD at [NoIssue, NoPatchset].
	cache map[issuePatchset]http.FileSystem
}

func New(local bool, workDir string, gerritURL string, docPath string) (*docSet, error) {
	...
	if _, err := ret.FileSystem(NoIssue, NoPatchset); err != nil {
		return nil, skerr.Wrap(err)
	}
...
func (d *docSet) FileSystem(issue, patchset int64) (http.FileSystem, error) {
	return nil, nil
}
