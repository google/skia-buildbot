...
func main() {
	...
	for _, k := range keys {
		fmt.Fprintf(f, "<h1 id='%s'>Schema: %s</h1>", k[1:], k)
		dumpSchema(1, f, allSchemas[k])
	}
	fmt.Fprintf(f, "  </body>\n</html>\n")
}

func dumpExactly(depth int, w io.Writer, s *JSONSchema) {
	fmt.Fprint(w, "<section>\n")
	if s.Ref != "" {
		fmt.Fprintf(w, "<a href='%s'>%s</a>\n", s.Ref, s.Ref)
	} else {
		fmt.Fprintf(w, "<h%d>Title: %s - %s</h%d>\n", depth, s.Title, s.Description, depth)
		fmt.Fprintf(w, "<div><b>Type:</b> %s </div>\n", s.Type)
		...
		if len(s.Properties) > 0 {
			...
			fmt.Fprint(w, "<section>\n")
			fmt.Fprintf(w, "<h%d>Properties</h%d>\n", depth, depth)
			for _, k := range props {
				p := s.Properties[k]
				fmt.Fprintf(w, "<h%d>%s</h%d>\n", depth+1, k, depth+1)
				dumpSchema(depth+1, w, p)
			}
			fmt.Fprint(w, "</section>\n")
		}
		dumpSchema(depth+1, w, s.Items)
	}
	fmt.Fprint(w, "</section>\n")
}

func dumpOneOf(depth int, w io.Writer, s []*JSONSchema) {
	fmt.Fprint(w, "<section>\n")
	fmt.Fprintf(w, "<h%d>One Of</h%d>\n", depth, depth)
	for _, schema := range s {
		dumpExactly(depth+1, w, schema)
	}
	fmt.Fprint(w, "</section>\n")
}

func dumpSchema(depth int, w io.Writer, schema *Items) {
	if schema == nil {
		return
	}
	if schema.Plurality == "exactly" {
		dumpExactly(depth+1, w, schema.Schemas[0])
	} else if schema.Plurality == "oneOf" {
		dumpOneOf(depth+1, w, schema.Schemas)
	} else {
		// Do nothing.
	}
}
