...
def ShouldDoUploadStep(step):
  """ We only run steps which upload results on scheduled builds, so that forced
  builds of arbitrary revisions don't clobber our uploaded results, unless
  the 'force_upload' property has been manually set using the force build web
  form. """
  try:
    return step.getProperty('scheduler') != None
  except:
    pass
  try:
    return step.getProperty('force_upload') != None
  except:
    return False
