...
func alertManagerHandler(w http.ResponseWriter, r *http.Request) {
	...
	request := alertmanager.AlertManagerRequest{}
	...
	body, err := alertmanager.EmailBody(request)
	if err != nil {
	...
	subject := fmt.Sprintf("Alert: %s at %s", strings.Join(alertnames, " "), ts.Format("11:06 MST (7 Mar 2015)"))
	if err := emailAuth.Send("alertserver@skia.org", to, subject, body); err != nil {
