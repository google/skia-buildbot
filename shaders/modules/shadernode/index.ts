 ...
 * uniforms".
 */
export const numPredefinedUniforms =
  predefinedUniforms.match(/^uniform/gm)!.length - numPredefinedShaderUniforms;
...
export class ShaderNode {
  ...
  async setCurrentImageURL(val: string): Promise<void> {
    ...
    try {
      ...
      const _ = errorMessage(
        `Failed to load image: ${this.currentImageURL}. Falling back to an empty image.`
      );
   ...
   * */
  get currentUserUniformValues(): number[] {
    return this._currentUserUniformValues;
  }
  ...
  compile(): void {
    ...
    this.effect?.delete();
    this.effect = this.canvasKit!.RuntimeEffect.Make(
      `${predefinedAndChildShaderUniformDeclarations}${this.runningCode}`,
      (err: string) => {
        ...
        err = err.replace(
          shaderCompilerErrorRegex,
          (_match: unknown, firstRegexCaptureValue: string): string => {
  ...
  getShader(predefinedUniformsValues: number[]): Shader | null {
    ...
    const uniformsFloat32Array: Float32Array =
      this.uniformsMallocObj!.toTypedArray() as Float32Array;
  ...
  private buildUniformsFromEffect() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.uniforms.push({
        ...this.effect.getUniform(i),
        name: this.effect.getUniformName(i),
      });
    }
  }
