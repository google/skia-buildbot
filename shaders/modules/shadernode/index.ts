...
export class ShaderNode {
    ...
    compile(): void {
      ...
      if (!this.uniformsMallocObj) {
        this.uniformsMallocObj = this.canvasKit!.Malloc(Float32Array, uniformFloatCount);
      } else if (this.uniformsMallocObj.length !== uniformFloatCount) {
        this.canvasKit!.Free(this.uniformsMallocObj);
        this.uniformsMallocObj = this.canvasKit!.Malloc(Float32Array, uniformFloatCount);
      }
      this._numPredefinedUniformValues = this.calcNumPredefinedUniformValues();

      // Fix up currentUserUniformValues if it's the wrong length.
      const userUniformValuesLength = uniformFloatCount - this._numPredefinedUniformValues;
    ...
    getUniformCount(): number {
      // TODO(jcgregorio) Calc this once on compile and returned the cached value.
      if (!this.effect) {
        return 0;
      }
    ...
    private calcNumPredefinedUniformValues(): number {
      let ret = 0;
      if (!this.effect) {
        return 0;
      }
