...
export class ShaderNode {
    ...
    private _uniformFloatCount: number = 0;
    ...
    compile(): void {
      ...
      this._uniformFloatCount = this.effect!.getUniformFloatCount();

      // Fix up currentUserUniformValues if it's the wrong length.
      const userUniformValuesLength = this._uniformFloatCount - this._numPredefinedUniformValues;
    ...
    getUniformFloatCount(): number {
      return this._uniformFloatCount;
    }
    ...
    private buildUniformsFromEffect() {
      ...
      for (let i = 0; i < count; i++) {
        this._uniforms.push({ ...this.effect!.getUniform(i), name: this.effect!.getUniformName(i) });
      }
    }
    ...
    private mallocUniformsMallocObj(): void {
      ...
      if (!this.uniformsMallocObj) {
        this.uniformsMallocObj = this.canvasKit!.Malloc(Float32Array, this._uniformFloatCount);
      } else if (this.uniformsMallocObj.length !== this._uniformFloatCount) {
