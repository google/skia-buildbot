...
export class ShaderNode {
    ...
    private inputImageShader: InputImage;
    ...
    constructor(canvasKit: CanvasKit) {
      ...
      this.inputImageShader = this.inputImageShaderFromCanvasImageSource(new Image(DEFAULT_SIZE, DEFAULT_SIZE));
    ...
    setScrap(scrapBody: ScrapBody, imageLoadedCallback: callback | null = null): void {
      ...
      if (this.body.SKSLMetaData?.ImageURL) {
        this.promiseOnImageLoaded(this.body.SKSLMetaData.ImageURL).then((imageElement) => {
          this.inputImageShader = this.inputImageShaderFromCanvasImageSource(imageElement);
          if (imageLoadedCallback) {
            imageLoadedCallback();
          }
        }).catch(errorMessage);
      } else if (imageLoadedCallback) {
        imageLoadedCallback();
      }
      this.compile();
    }
    ...
    async saveScrap(): Promise<string> {
      const body: ScrapBody = {
        ...
        SKSLMetaData: {
          ...
          ImageURL: '', // Remember to remove ImageURLs that aren't relative or that don't start with http[s]://, e.g. don't store blob:// or file:// urls.
          Children: [],
        },
      };
    ...
    private inputImageShaderFromCanvasImageSource(imageElement: HTMLImageElement): InputImage {
