...
export class ShaderNode {
    ...
    saveScrap(): void {
      ...
      this.children.forEach((childNode) => { childNode.saveScrap(); });

      // Then copy the scrapIDs of the children up into this node.
    ...
    // Runs the editedCode for this node and all its children.
    runEditedCode(): void {
      ...
      this.children.forEach((childNode) => childNode.runEditedCode());
    }

    // Returns true if this node or any child nodes need to be run.
    needsRun(): boolean {
      return (this.editedCode !== this.runningCode) || this.children.some((childNode) => childNode.needsRun());
    }

    // Returns true if this node or any child node needs to be saved.
    needsSave(): boolean {
      return (this.editedCode !== this.body!.Body) || !this.userUniformValuesHaveBeenEdited() || this.childShaderNamesHaveChanged() || this.children.some((childNode) => childNode.needsSave());
    }
