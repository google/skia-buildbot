...
export class ShadersAppSk extends ElementSk {
  ...
  private rafID: number = NOT_RUNNING;

  // Records the code that we started with, either at startup, or after we've saved.
  private startingCode = defaultShader;

  // The current value of the editor.
  private currentCode = defaultShader;
  ...
  private static template = (ele: ShadersAppSk) => html`
    ...
    <main>
      ...
      <div>
        <button
          ?hidden=${ele.currentCode === ele.startingCode}
          @click=${ele.saveClick}
        >
          Save
        </button>
  ...
  private init(shaderCode: string) {
    ...
    if (this.rafID != NOT_RUNNING) {
      cancelAnimationFrame(this.rafID);
      this.rafID = NOT_RUNNING;
    }
    this.startTime = Date.now();
    this.startingCode = shaderCode;
  ...
  private async saveClick() {
    ...
    try {
      ...
      this.startingCode = this.currentCode;
      this.stateChanged!();
      this._render();
    } catch (error) {
      errorMessage(`${error}`);
  ...
  private codeChange() {
    this.currentCode = this.codeMirror!.getValue();
    this._render();
  }
