...
export class ShadersAppSk extends ElementSk {
  ...
  private monitorIfDevicePixelRatioChanges() {
    ...
    matchMedia(mqString).addEventListener('change', () => this.compileCode());
  }
  ...
  private totalPredefinedUniformValues(): number {
    ...
    for (let i = 0; i < numPredefinedUniforms; i++) {
      const u = this.shaderNode.getUniform(i);
  ...
  private getCurrentUserUniformValues(uniforms: number[]): number[] {
    ...
    if (this.shaderNode) {
      ...
      for (let i = this.totalPredefinedUniformValues(); i < this.shaderNode.getUniformFloatCount(); i++) {
  ...
  private drawFrame() {
    ...
    const predefinedUniformValues = uniformsArray.slice(0, this.totalPredefinedUniformValues());
