...
export class ShadersAppSk extends ElementSk {
    ...
    ret.push(html`
      <p
        ...
        @click=${() => ele.childShaderClick(node)}>
          ...
          <button class=addButton title="Append a new child shader." @click=${() => ele.appendChildShader(node)}><add-icon-sk></add-icon-sk></button>
      </p>`);
  ...
  private async appendChildShader(node: ShaderNode) {
    try {
