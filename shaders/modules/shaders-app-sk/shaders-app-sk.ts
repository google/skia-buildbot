...
// Counts the number of uniforms defined in 'predefinedUniforms'.
...
export class ShadersAppSk extends ElementSk {
  ...
  private getUniformValuesFromControls(): Float32Array {
  ...
  private getCurrentUserUniformValues(uniforms: Float32Array): number[] {
    const uniformsArray: number[] = [];
    if (this.effect) {
      // First figure out how many uniform values we need to skip. That is, we
      // don't store or retrieve predefined uniform values.
      let totalPredefinedUniformValues = 0;
      for (let i = 0; i < numPredefinedUniforms; i++) {
        const u = this.effect.getUniform(i);
        totalPredefinedUniformValues += u.rows * u.columns;
      }
      // Copy only the non-predefined uniform values.
      for (let i = totalPredefinedUniformValues; i < this.effect.getUniformFloatCount(); i++) {
