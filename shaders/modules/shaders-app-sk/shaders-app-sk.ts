...
export class ShadersAppSk extends ElementSk {
    ...
    <main>
      ...
      <div>
        <details id=shaderinputs>
          ...
          <div id=imageSources>
            <figure>
              ${ele.rootShaderNode?.inputImageElement}
            ...
            <details id=image_edit>
              ...
              <div id=image_edit_dialog>
                ...
                <div>
                  <input type=url id=image_url placeholder="URL of image to use." .value="${ele.rootShaderNode?.getSafeImageURL() || ''}">
        ...
        <textarea style="display: ${ele.rootShaderNode?.children.length ? 'block' : 'none'}" rows=${ele.rootShaderNode?.children.length || 0} cols=75>${ele.rootShaderNode?.getChildShaderUniforms() || ''}</textarea>
        ...
        <div ?hidden=${!ele.rootShaderNode?.compileErrorMessage} id="compileErrors">
          <h3>Errors</h3>
          <pre>${ele.rootShaderNode?.compileErrorMessage}</pre>
        </div>
      </div>
  ...
  private run() {
    ...
    this.codeMirror!.setValue(this.rootShaderNode?.shaderCode || defaultShader);
    ...
    this.rootShaderNode!.compileErrorLineNumbers.forEach((lineNumber: number) => {
  ...
  private getUserUniformValuesFromControls(): number[] {
    const uniforms: number[] = new Array(this.rootShaderNode!.getUniformFloatCount()).fill(0);
    ...
    return uniforms.slice(this.rootShaderNode?.numPredefinedUniformValues || 0);
  }
  ...
  private uniformControlsChange() {
    this.rootShaderNode!.currentUserUniformValues = this.getUserUniformValuesFromControls();
    this._render();
  }

  private drawFrame() {
    this.kit!.setCurrentContext(this.canvasKitContext);
    const shader = this.rootShaderNode!.getShader(this.getPredefinedUniformValuesFromControls());
    if (!shader) {
      return;
    }
  ...
  private codeChange() {
    this.rootShaderNode!.shaderCode = this.codeMirror!.getValue();
    this._render();
  }

  private async appendChildShader() {
    try {
      await this.rootShaderNode?.appendNewChildShader();
      // eslint-disable-next-line no-unused-expressions
      this._render();
    } catch (error) {
      errorMessage(error);
  ...
  private setCurrentImageURL(url: string): void {
    const oldURL = this.rootShaderNode!.getCurrentImageURL();
    ...
    this.rootShaderNode!.setCurrentImageURL(url).then(() => this._render());
  }
}
