...
export class ShadersAppSk extends ElementSk {
  ...
  /** Do not display blob: based URLs in the URL input. */
  private static displayableImageURL = (ele: ShadersAppSk): string => {
    if (!ele.shaderNode) {
      return '';
    }
    const ret = ele.shaderNode!.getCurrentImageURL();
    if (ret.startsWith('blob:')) {
      return ele.shaderNode.getSavedImageURL();
    }
    return ret;
  }
          ...
          <div id=imageSources>
            ...
            <details id=image_edit>
              ...
              <div id=image_edit_dialog>
                ...
                <div>
                  <input type=url id=image_url placeholder="URL of image to use." .value="${ShadersAppSk.displayableImageURL(ele)}">
