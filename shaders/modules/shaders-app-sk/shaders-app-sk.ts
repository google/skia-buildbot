...
export class ShadersAppSk extends ElementSk {
  ...
  private getUserUniformValuesFromControls(): number[] {
    ...
    $('#uniformControls > *').slice(numPredefinedUniforms).forEach((control) => {
  ...
  private getPredefinedUniformValuesFromControls(): number[] {
    const uniforms: number[] = new Array(this.shaderNode!.getUniformFloatCount()).fill(0);
    $('#uniformControls > *').slice(0, numPredefinedUniforms).forEach((control) => {
      (control as unknown as UniformControl).applyUniformValues(uniforms);
    });
    return uniforms;
  }
  ...
  private drawFrame() {
    ...
    const preDefinedUiformsArray = this.getPredefinedUniformValuesFromControls();
    const shader = this.shaderNode!.getShader(preDefinedUiformsArray);
    if (!shader) {
