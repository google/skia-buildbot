...
export class ShadersAppSk extends ElementSk {
    ...
    <main>
      ...
      <div>
        <details id=shaderinputs>
          ...
          <div id=imageSources>
            <figure>
              <img id=iImage1 style="width: 32px; height: 32px;" loading="eager" crossorigin="anonymous" src="/dist/mandrill.png">
              <figcaption>iImage1</figcaption>
            </figure>
            <figure>
              <img id=iImage2 style="width: 32px; height: 32px;" loading="eager" crossorigin="anonymous" src="/dist/soccer.png">
  ...
  connectedCallback(): void {
    ...
    // TODO(jcgregorio) Offer a range of input images to use.

    // A Promise that resolves once the source image is loaded.
    const mandrillLoaded = new Promise<HTMLImageElement>((resolve, reject) => {
      const ele = $$<HTMLImageElement>('#iImage1', this)!;
      ele.addEventListener('load', () => resolve(ele));
      ele.addEventListener('error', (e) => reject(e));
    });

    const soccerLoaded = new Promise<HTMLImageElement>((resolve, reject) => {
      const ele = $$<HTMLImageElement>('#iImage2', this)!;
      ele.addEventListener('load', () => resolve(ele));
      ele.addEventListener('error', (e) => reject(e));
    });
    ...
    kitReady.then(async (ck: CanvasKit) => {
      ...
      // Wait until the mandrill is loaded and then convert it into a shader.
      try {
        const mandrillImgElement = await mandrillLoaded;
        this.inputImageShaders = [this.kit.MakeImageFromCanvasImageSource(mandrillImgElement).makeShaderOptions(this.kit.TileMode.Clamp, this.kit.TileMode.Clamp, this.kit.FilterQuality.Medium, this.kit.MipmapMode.None)];
      } catch (error) {
        errorMessage(error);
      }

      try {
        const soccerImgElement = await soccerLoaded;
        this.inputImageShaders.push(this.kit.MakeImageFromCanvasImageSource(soccerImgElement).makeShaderOptions(this.kit.TileMode.Clamp, this.kit.TileMode.Clamp, this.kit.FilterQuality.Medium, this.kit.MipmapMode.None));
      } catch (error) {
        errorMessage(error);
      }
