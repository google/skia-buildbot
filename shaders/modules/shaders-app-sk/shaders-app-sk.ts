...
import {
  defaultShader, numPredefinedUniformLines, numPredefinedUniforms, predefinedUniforms, ShaderNode,
} from '../shadernode';
...
export class ShadersAppSk extends ElementSk {
  ...
  private static template = (ele: ShadersAppSk) => html`
    ...
    <main>
      ...
      <div id=shaderControls>
        ...
        <button
          ?hidden=${!ele.shaderNode?.needsRun()}
          @click=${ele.runClick}
          class=action
        >
          Run
  ...
  private async loadShaderIfNecessary() {
    ...
    try {
      ...
      this._render();
      // Push the potentially newly loaded uniform values out to the controls.
      const uniformsArray = this.getUniformValuesFromControls();
      const userUniformsOffset = this.shaderNode!.totalPredefinedUniformValues();
      this.shaderNode!.currentUserUniformValues.forEach((value, index) => {
        uniformsArray[index + userUniformsOffset] = value;
      });
      this.setUniformValuesToControls(uniformsArray);
      this.run();
    } catch (error) {
      errorMessage(error, 0);
  ...
  private run() {
    ...
    this.shaderNode!.errorLineNumbers.forEach((lineNumber: number) => {
  ...
  private getUniformValuesFromControls(): number[] {
    ...
    const uniforms: number[] = new Array(this.shaderNode!.getUniformFloatCount());
  ...
  private getCurrentUserUniformValues(uniforms: number[]): number[] {
    ...
    if (this.shaderNode) {
      ...
      for (let i = this.shaderNode.totalPredefinedUniformValues(); i < this.shaderNode.getUniformFloatCount(); i++) {
  ...
  private drawFrame() {
    ...
    const predefinedUniformValues = uniformsArray.slice(0, this.shaderNode!.totalPredefinedUniformValues());
    const shader = this.shaderNode!.getShader(predefinedUniformValues);
    if (!shader) {
      return;
    }

    // Allow uniform controls to update, such as uniform-timer-sk.
    // TODO(jcgregorio) This might be overkill, maybe just allow controls to
    //   register for a 'raf' event if they need to update frequently?
    this._render();
