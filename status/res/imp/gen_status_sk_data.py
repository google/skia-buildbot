...
URL_TMPL = 'https://status.skia.org/json/skia/incremental?from=%d&to=%d'


beginning_of_time = datetime.datetime.utcfromtimestamp(1505030640)
start = datetime.datetime.utcfromtimestamp(1505324460)
num_mins = 10
...
def get_all():
  ...
  for _ in xrange(num_mins):
    next = prev + datetime.timedelta(0, 60)
    yield get(prev, next)
    prev = next
