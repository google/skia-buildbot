...
func newTasksPerCommitCache(ctx context.Context, workdir, recipesCfgFile string, repos repograph.Map, period time.Duration, bigtableInstance string, ts oauth2.TokenSource) (*tasksPerCommitCache, error) {
	...
	tcc, err := specs.NewTaskCfgCache(ctx, repos, depotTools, gitCache, 3, bigtableInstance, ts)
	if err != nil {
		return nil, err
	}
