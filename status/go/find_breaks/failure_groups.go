...
// maybeMerge combines two failures into a third failure or returns nil if the two
...
func (g *FailureGroup) maybeMerge(f *failure) {
	...
	// 2. The the slices of commits which possibly caused the failures
	//    must overlap. If the break might have occurred before the
	...
	if brokeIn.Empty() {
		if !g.brokeIn.Empty() || !f.brokeIn.Empty() {
			return
		}
	}
	...
	if !g.fixedIn.Empty() && !f.fixedIn.Empty() && fixedIn.Empty() {
		return
	}
