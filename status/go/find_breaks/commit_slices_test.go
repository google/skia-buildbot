...
func toString(c []*repograph.Commit) []string {
	rv := make([]string, 0, len(c))
	for _, commit := range c {
		rv = append(rv, commit.Hash)
	}
	return rv
}
...
func TestCommitSlices1(t *testing.T) {
	...
	repo, cleanup := setupHelper(t, func(gb *git_testutils.GitBuilder) {
		...
		gb.AddGen("file")
		a = gb.CommitMsgAt("a", ts)

		ts = ts.Add(2 * time.Minute)
		gb.AddGen("file")
		b = gb.CommitMsgAt("b", ts)

		ts = ts.Add(2 * time.Minute)
		gb.AddGen("file")
		c = gb.CommitMsgAt("c", ts)

		ts = ts.Add(2 * time.Minute)
		gb.AddGen("file")
		d = gb.CommitMsgAt("d", ts)

		ts = ts.Add(2 * time.Minute)
		gb.AddGen("file")
		e = gb.CommitMsgAt("e", ts)
	})
	defer cleanup()
	...
	testutils.AssertDeepEqual(t, []string{a, b, c, d, e}, toString(slices[0]))

	// Make sure the timestamp cutoffs work.
	s := slices[0]
	end := s[len(s)-1].Timestamp
	start := s[0].Timestamp
	slices = commitSlices(repo, start, end)
	assert.Equal(t, 1, len(slices))
	assert.Equal(t, 4, len(slices[0]))
	testutils.AssertDeepEqual(t, []string{a, b, c, d}, toString(slices[0]))
	...
	testutils.AssertDeepEqual(t, []string{b, c, d}, toString(slices[0]))
...
func TestCommitSlices2(t *testing.T) {
	...
	testutils.AssertDeepEqual(t, []string{a, b, d}, toString(slices[0]))
	testutils.AssertDeepEqual(t, []string{a, b, c}, toString(slices[1]))
}
...
func TestCommitSlices3(t *testing.T) {
	...
	assert.Equal(t, 3, len(slices[0]))
	assert.Equal(t, 3, len(slices[1]))
	testutils.AssertDeepEqual(t, []string{a, c, d}, toString(slices[0]))
	testutils.AssertDeepEqual(t, []string{b, c, d}, toString(slices[1]))
}
...
func TestCommitSlices4(t *testing.T) {
	...
	testutils.AssertDeepEqual(t, []string{a, b, c, e, f}, toString(slices[0]))
	testutils.AssertDeepEqual(t, []string{a, b, d, e, f}, toString(slices[1]))
}
