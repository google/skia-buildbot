...
func commit(hash string) *repograph.Commit {
	return &repograph.Commit{
		LongCommit: &vcsinfo.LongCommit{
...
func TestMakeSlice(t *testing.T) {
	test := func(sub, super []string, start, end int) {
		commits := make([]*repograph.Commit, 0, len(super))
		for _, c := range super {
			commits = append(commits, commit(c))
		}
		s := makeSlice(sub, commits)
	...
	// Subslice is not contiguous.
	test([]string{"b", "d"}, []string{"a", "b", "c", "d", "e"}, -1, -1)

	// Subslice extends outside of parent slice.
	test([]string{"a", "b"}, []string{"b", "c"}, -1, -1)
	test([]string{"b", "c"}, []string{"a", "b"}, -1, -1)
}
