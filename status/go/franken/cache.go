...
func (c *BTCache) getLastNCommits(r *repograph.Graph, n int) ([]*vcsinfo.LongCommit, error) {
	...
	for i := 0; i < n; i++ {
		p := commit.GetParents()
		if len(p) < 1 {
			n = i // Cut short if we've hit the beginning of history.
			break
		}
		commit = p[0]
	}
...
func (c *BTCache) GetLastN(repo string, n int, loggedIn bool) (*CommitsData, error) {
	...
	commitComments := c.cachedCommitComments[repo] // TODO(borenet): Copy?
...
func (c *BTCache) AddCommitComment(comment *buildbot.CommitComment) error {
	_, repo, _, err := c.repos.FindCommit(comment.Commit)
	if err != nil {
		return fmt.Errorf("Unable to add commit comment: %s", err)
	}
...
func (c *BTCache) DeleteCommitComment(commit string, id int64) error {
	_, repo, _, err := c.repos.FindCommit(commit)
	if err != nil {
		return fmt.Errorf("Unable to delete commit comment: %s", err)
	}
	ts := time.Unix(0, id)
