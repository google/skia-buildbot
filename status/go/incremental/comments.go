...
func (c *commentsCache) Update(w *window.Window) (map[string]RepoComments, error) {
	...
	for _, rc := range repoComments {
		entry := RepoComments{}
		if !reflect.DeepEqual(rc.CommitComments, c.prevComments[rc.Repo].CommitComments) {
			entry.CommitComments = commitComments(rc.CommitComments)
		}
		if !reflect.DeepEqual(rc.TaskComments, c.prevComments[rc.Repo].TaskComments) {
			entry.TaskComments = taskComments(rc.TaskComments)
		}
		if !reflect.DeepEqual(rc.TaskSpecComments, c.prevComments[rc.Repo].TaskSpecComments) {
