...
func newTasksPerCommitCache(ctx context.Context, workdir string, repoUrls []string, period time.Duration, bigtableInstance string, ts oauth2.TokenSource) (*tasksPerCommitCache, error) {
	...
	tcc, err := specs.NewTaskCfgCache(ctx, repos, depotTools.Dir(), gitCache, 3, bigtableInstance, ts)
	if err != nil {
		return nil, err
	}
