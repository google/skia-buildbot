...
func runServer(serverURL string) {
	...
	err = r.Walk(func(route *mux.Route, router *mux.Router, ancestors []*mux.Route) error {
		...
		if err == nil {
			sklog.Info("ROUTE:", pathTemplate)
		}
		pathRegexp, err := route.GetPathRegexp()
		if err == nil {
			sklog.Info("Path regexp:", pathRegexp)
		}
		queriesTemplates, err := route.GetQueriesTemplates()
		if err == nil {
			sklog.Info("Queries templates:", strings.Join(queriesTemplates, ","))
		}
		queriesRegexps, err := route.GetQueriesRegexp()
		if err == nil {
			sklog.Info("Queries regexps:", strings.Join(queriesRegexps, ","))
		}
		methods, err := route.GetMethods()
		if err == nil {
			sklog.Info("Methods:", strings.Join(methods, ","))
		}
		sklog.Info()
		return nil
	})
