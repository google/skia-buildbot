...
// repoUrlToName returns a sort repo nickname given a full repo URL.
...
func repoNameToUrl(repoName string) (string, error) {
	// Special case: no repo given; default to Skia repo.
	if repoName == "" {
		return common.REPO_SKIA, nil
	}
	// Special case: we like "infra" better than "buildbot".
	if repoName == "infra" {
...
func runServer(serverURL string) {
	...
	r.HandleFunc("/{repo}", statusHandler)
