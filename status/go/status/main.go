...
func commitsJsonHandler(w http.ResponseWriter, r *http.Request) {
	...
	builds, err := buildCache.GetBuildsForCommits(hashes, login.LoggedInAs(r) != "")
	if err != nil {
...
func buildsJsonHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Fetch the builds.
	builds, err := buildCache.GetBuildsFromDateRange(startTime, endTime, login.LoggedInAs(r) != "")
	if err != nil {
...
func buildProgressHandler(w http.ResponseWriter, r *http.Request) {
	...
	buildsAtNewCommit, err := buildCache.GetBuildsForCommit(hash, login.LoggedInAs(r) != "")
	if err != nil {
	...
	buildsAtOldCommit, err := buildCache.GetBuildsForCommit(oldCommit.Hash, login.LoggedInAs(r) != "")
	if err != nil {
		httputils.ReportError(w, r, err, fmt.Sprintf("Failed to get the number of finished builds."))
		return
	}
