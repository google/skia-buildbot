...
type BotConfig struct {
	...
	Bots                 map[string]bool `json:"gpu"` // maps bot id to boolean
	TaskAverageDurations []time.Duration `json:"task_times_ns"`
}
...
func (c *CapacityClient) QueryAll() error {
	...
	for _, task := range tasks {
		...
		if task.Finished.IsZero() || task.Finished == task.Started {
			continue
		}
	...
	for taskName, taskRuns := range durations {
		...
		config.TaskAverageDurations = append(config.TaskAverageDurations, avgDuration)
