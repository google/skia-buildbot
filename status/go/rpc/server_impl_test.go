...
func setup(t *testing.T) (context.Context, *status_mocks.CapacityClientInterface, *statusServerImpl) {
	unittest.SmallTest(t)
	mcc := &status_mocks.CapacityClientInterface{}
...
func TestGetBotUsage(t *testing.T) {
	ctx, mcc, server := setup(t)
	mcc.On("CapacityMetrics").Return(map[string]capacity.BotConfig{
		"keyIgnored0": {
			...
			Bots: map[string]bool{"bot1": true, "bot2": true, "bot3": true},
		},
		"keyIgnored1": {
			Dimensions: []string{
				"cpu:widget5",
	...
	mcc.AssertNumberOfCalls(t, "CapacityMetrics", 1)
