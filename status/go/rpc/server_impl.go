...
func (s *statusServerImpl) GetIncrementalCommits(ctx context.Context, req *GetIncrementalCommitsRequest) (*GetIncrementalCommitsResponse, error) {
...
func ConvertUpdate(u *incremental.Update, podID string) *GetIncrementalCommitsResponse {
	...
	rv.Metadata.StartOver = *u.StartOver
	...
	for _, c := range u.Commits {
		update.Commits = append(update.Commits, &LongCommit{Hash: c.Hash, Author: c.Author, Subject: c.Subject, Parents: c.Parents, Body: c.Body, Timestamp: timestamppb.New(c.Timestamp)})
	}
	update.SwarmingUrl = u.SwarmingUrl
	for _, t := range u.Tasks {
		update.Tasks = append(update.Tasks, &Task{Commits: t.Commits, Name: t.Name, Id: t.Id, Revision: t.Revision, Status: string(t.Status), SwarmingTaskId: t.SwarmingTaskId})
	}
	update.TaskSchedulerUrl = u.TaskSchedulerUrl
	for hash, srcComments := range u.CommitComments {
		for _, c := range srcComments {
			update.Comments = append(update.Comments, &Comment{Commit: hash, Id: c.Id, Repo: c.Repo, Revision: c.Revision, Timestamp: timestamppb.New(c.Timestamp), User: c.User, IgnoreFailure: c.IgnoreFailure, Message: c.Message, Deleted: *c.Deleted})
		}
	}
	for _, srcComments := range u.TaskSpecComments {
		for _, c := range srcComments {
			update.Comments = append(update.Comments, &Comment{Id: c.Id, Repo: c.Repo, TaskSpecName: c.Name, Timestamp: timestamppb.New(c.Timestamp), User: c.User, Flaky: c.Flaky, IgnoreFailure: c.IgnoreFailure, Message: c.Message, Deleted: *c.Deleted})
		}
	}
	for taskSpec, commitComments := range u.TaskComments {
		for hash, srcComments := range commitComments {
			for _, c := range srcComments {
				...
				update.Comments = append(update.Comments, &Comment{Commit: hash, Id: c.Id, Repo: c.Repo, Revision: c.Revision, TaskSpecName: c.Name, Timestamp: timestamppb.New(c.Timestamp), TaskId: c.TaskId, User: c.User, Message: c.Message, Deleted: *c.Deleted})
			}
		}
	}
