...
for (let i = 0; i < 30; i++) {
  ...
  commits.push(Object.assign(JSON.parse(JSON.stringify(commitTemplate)),
    { hash: hash, parents: [`abc${i + 1}`], body: body, subject: 'something', timestamp: (parseInt(commitTemplate.timestamp) - (100 * i)).toString(), author: randomAuthor() })
  );
  ...
  for (let spec of taskSpecs) {
    if (!seen(`${hash}/${spec}`)) {
      ...
      // Randomly make some tasks fail, mishap, or running/pending(but only for recent tasks, for realism).
