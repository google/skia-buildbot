/**
 * @module modules/commits-table-experimental-sk
...
class Data {
  // Outputs - Data to be used by the CommitsTableExperimentalSk class. Derived from calling the
  ...
  private mapRevertsAndRelands(commit: Commit) {
    ...
    const reverted = findRevertedCommit(this.commitsByHash, commit);
    if (reverted) {
    ...
    const relanded = findRelandedCommit(this.commitsByHash, commit);
    if (relanded) {
...
export class CommitsTableExperimentalSk extends ElementSk {
  ...
  private static template = (el: CommitsTableExperimentalSk) =>
  ...
  constructor() {
    super(CommitsTableExperimentalSk.template);
  }
...
define('commits-table-experimental-sk', CommitsTableExperimentalSk);
...
function findIssueAndReviewTool(commit: LongCommit): [string, string] {
  ...
  const gerritRE = /(.|[\r\n])*Reviewed-on:.*\/([0-9]*)/g;
  const gerritTokens = gerritRE.exec(commit.body);
  if (gerritTokens) {
...
function findRelandedCommit(commits: Map<string, Commit>, commit: Commit) {
  ...
  const revert = findRevertedCommit(commits, commit);
  if (revert) {
