...
import {
  incrementalResponse0, responseSingleCommitTask, responseMultiCommitTask,
  responseNoncontiguousCommitsTask, responseTasksToFilter
...
describe('commits-table-sk', () => {
  ...
  it('displays commits', async () => {
    ...
    expect(commitDivs.sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top)
      .map(el => el.id)).to.deep.equal(incrementalResponse0.update!.commits!.map(c => c.hash));
  });
  ...
  it('filters task specs', async () => {
    ...
    expect($('.task-spec', table).map(el => el.getAttribute('title')))
      .to.have.deep.members(['Always-Green-Spec', 'Always-Red-Spec',
        'Interesting-Spec', 'Only-Failed-On-Commented-Commit-Spec']);
    ...
    expect($('.task-spec', table).map(el => el.getAttribute('title')))
      .to.have.deep.members(['Always-Red-Spec', 'Interesting-Spec']);
