...
export class CommitsTableSk extends ElementSk {
  ...
  private setState(fromUrl: HintableObject) {
    ...
    // We want filter and repo aruments to always be reflected in the url, so we use empty string
    // as the default so our real defaults produce a delta.Don't let initial load or removal of
    // the params mangle our settings.
    if (state.filter) {
