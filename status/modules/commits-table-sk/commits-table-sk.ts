...
<div id=commitsTableContainer>
  ...
  iconsForTaskSpec(taskSpec: string): Array<TemplateResult> {
      ...
      categoryDetails.taskSpecsBySubCategory.forEach((taskSpecs: Array<string>, subcategoryName: string) => {
        ...
        taskSpecs.filter((ts) => this.includeTaskSpec(ts)).forEach((taskSpec: string) => {
          ...
          res.push(
            ...
            html`<div class="category task-spec" style=${this.gridLocation(TASKSPEC_START_ROW, taskSpecStartCol++)} title=${taskSpec}>${this.iconsForTaskSpec(taskSpec)}</div>`
          );
        });
  ...
  addTasks(): Array<TemplateResult> {
    ...
    for (const [i, commit] of this.data().commits.entries()) {
      ...
      res.push(
        html`<div class=commit style=${this.gridLocation(rowStart, 1)} title=${title}>${text}</div>`
      );
