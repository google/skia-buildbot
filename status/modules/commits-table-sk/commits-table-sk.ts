...
<div id=commitsTableContainer>
  ...
  includeTaskSpec(taskSpec: string): boolean {
    // TODO(westont): Don't stub this out.
    switch (this._filter) {
      case Filter.all: return true;
      case Filter.comments: return !!this.taskSpecComments()?.has(taskSpec);
      case Filter.nocomment: return !this.taskSpecComments()?.has(taskSpec);
      case Filter.failures: return true; // TODO could use data().tasksBySpec[foo].every(). but precomputing will be faster.
      case Filter.interesting: return true; // TODO
  ...
  addTaskHeaders(res: Array<TemplateResult>): Map<TaskSpec, number> {
    ...
    this.data().categories.forEach((categoryDetails: CategorySpec, categoryName: string) => {
      ...
      categoryDetails.taskSpecsBySubCategory.forEach((taskSpecs: Array<string>, subcategoryName: string) => {
        ...
        taskSpecs.filter((ts) => this.includeTaskSpec(ts)).forEach((taskSpec: string) => {
          ...
          res.push(
            ...
            html`<div class="category task-spec" style=${this.gridLocation(TASKSPEC_START_ROW, taskSpecStartCol++)} title=${taskSpec} @click=${() => this.openTaskSpecDialog(taskSpec)}></div>`
          );
        });
        if (taskSpecStartCol != subcategoryStartCol) {
          // We added at least one TaskSpec in this subcategory, so add a Subcategory header.
      ...
      if (subcategoryStartCol != categoryStartCol) {
        // We added at least one Subcategory in this category, so add a Category header.
  ...
  addTasks(): Array<TemplateResult> {
    ...
    for (const [i, commit] of this.data().commits.entries()) {
      ...
      // TODO(westont): This requires a re-render when author/desc toggles, test to see if this is costly, use cache directive to make it cheap, and maybe move it out of this element, if it has a lot of overhead. 
      res.push(
