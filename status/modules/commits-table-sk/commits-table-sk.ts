...
<div id=commitsTableContainer>
  ...
  computeTaskHeaders(): Array<TemplateResult> {
    ...
    this.data().tasksByCommit.forEach((tasksBySpec: Map<TaskSpec, Task>, commit: CommitHash) => {
      // TODO This is ineffecient and unsafe. fix it.
      const rowStart = this.data().commits.indexOf(this.data().commitsByHash.get(commit) as Commit) + 4;
      tasksBySpec.forEach((task: Task, name: TaskSpec) => {
        const colStart = taskSpecStartCols.get(name) as number;
        // DTDO deal with multiple commits by getting commit index of last commit from task, and expanding row end.
          res.push(
            html`<div class=category style=${this.gridLocation(rowStart, colStart)}>${task.id}</div>`
          );
      });
    });
    return res;
