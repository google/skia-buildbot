...
import { CommitsDataSk, CategorySpec, TaskSpec, TaskId, Commit } from '../commits-data-sk/commits-data-sk';
...
<div id=commitsTableContainer>
  ...
  commitIcons(commit: Commit): Array<TemplateResult> {
    ...
    if (reverted && (reverted.timestamp! > commit.timestamp!)) {
      res.push(html`<undo-icon-sk class="tiny icon-right"
    ...
    if (relanded && (relanded.timestamp! > commit.timestamp!)) {
      res.push(html`<redo-icon-sk class="tiny icon-right"
  ...
  highlightAssociatedCommit(hash: string, revert: boolean) {
    $$(`#${hash}`, this)?.classList
      .toggle(revert ? REVERT_HIGHLIGHT_CLASS : RELAND_HIGHLIGHT_CLASS)
  }
  ...
  addTaskHeaders(res: Array<TemplateResult>): Map<TaskSpec, number> {
    ...
    this.data().categories.forEach((categoryDetails: CategorySpec, categoryName: string) => {
      ...
      categoryDetails.taskSpecsBySubCategory.forEach(
        (taskSpecs: Array<string>, subcategoryName: string) => {
          ...
          taskSpecs.filter((ts) => this.includeTaskSpec(ts)).forEach((taskSpec: string) => {
            ...
            res.push(
              html`<div class="category task-spec"
              style=${this.gridLocation(TASKSPEC_START_ROW, taskSpecStartCol++)}
              title=${taskSpec}>${this.taskSpecIcons(taskSpec)}
            </div>`
            );
          });
          if (taskSpecStartCol != subcategoryStartCol) {
            ...
            res.push(
              ...
              style=${this.gridLocation(SUBCATEGORY_START_ROW, subcategoryStartCol,
                SUBCATEGORY_START_ROW + 1, subcategoryEndCol)}>
              ${subcategoryName}
            </div>`
            );
      ...
      if (subcategoryStartCol != categoryStartCol) {
        ...
        res.push(
          html`<div class=category
            style=${this.gridLocation(CATEGORY_START_ROW, categoryStartCol,
            CATEGORY_START_ROW + 1, categoryEndCol)}>
            ${categoryName}
          </div>`
        )
   ...
   * each styled with 'grid-area' to place them inside a css-grid element.
   */
  fillTableTemplate(): Array<TemplateResult> {
    ...
    for (const [i, commit] of this.data().commits.entries()) {
      ...
      res.push(
        html`<div class=commit style=${this.gridLocation(rowStart, 1)} id=${commit.hash}
          title=${title}>${text}${this.commitIcons(commit)}
        </div>`
      );
    ...
    const nextRowDiv = () => html`
      <div style=${this.gridLocation(row, 1, ++row, taskSpecStartCols.size + 2)}>
      </div>`;
    res.push(html`
      <div id=rowUnderlay>${Array(this.data().commits.length).fill(1).map(nextRowDiv)}
      </div>`);
    return res;
  }
  ...
  displayTaskRows(task: Task, latestCommitIndex: number) {
    ...
    for (let earlierCommitIndex = latestCommitIndex + 1;
