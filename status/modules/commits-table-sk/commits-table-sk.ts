...
class Data {
  ...
  update(repo: string, numCommits: number, lastLoaded?: Date) {
    ...
    return this.client
      ...
      .then((json: GetIncrementalCommitsResponse) => {
        ...
        this.extractData(json.update!, numCommits);
   ...
   */
  private extractData(update: IncrementalUpdate, maxCommits: number) {
    const newCommits = (update.commits || []) as Array<Commit>;
    var sliceIdx = maxCommits - newCommits.length;
...
export class CommitsTableSk extends ElementSk {
  ...
  private update() {
    ...
    this.data.update(this.repo, numCommits, this.lastLoaded).finally(() => {
      ...
      // If an additional update was requested, start it, otherwise schedule it update.
