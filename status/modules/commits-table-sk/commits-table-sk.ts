        ...
        tasksBySpec.forEach((task: Task, name: TaskSpec) => {
          ...
          if (!displayTask.addedToDom) {
            ...
            if (displayTaskRows.every(Boolean)) {
              ...
              res.push(
                html`<div class=${taskClasses(task, "grow")} style=${this.gridLocation(rowStart, colStart, rowStart + displayTaskRows.length)} title=${taskTitle(task)}>${task.id}</div>`
              );
            } else {
              ...
              res.push(
                html`<div class="multicommit-task grow" style=${this.gridLocation(rowStart, colStart, rowStart+displayTaskRows.length)}>
                  ${displayTaskRows.map((display, index) => {
                    ...
                    let ret: TemplateResult = html`<div class="hidden ${taskClasses(task)}" style=${this.gridLocation(currRow-rowStart+1, 1)}>${task.id}</div>`;
                    if (display) {
                      let brokenBorderClasses = this.getDashedBorderClasses(displayTaskRows, index);
                      ret = html`<div class=${taskClasses(task, ...brokenBorderClasses)} style=${this.gridLocation(currRow-rowStart+1, 1)}>${task.id}</div>`;
                    }
                    currRow++;
                    return ret;
                  })}
