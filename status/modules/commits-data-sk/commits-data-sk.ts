...
export class CommitsDataSk extends ElementSk {
  ...
  private client: StatusServiceClient = new StatusServiceClient(window.location.protocol + "//" + window.location.host, window.fetch.bind(window));
  ...
  connectedCallback() {
    ...
    this.client.getIncrementalCommits({
    ...
    .then((json: GetIncrementalCommitsResponse) => {
      ...
      // TODO(westont): Use StartOver to know to clear vs update.
      ...
      for (let comment of (update.comments || [])) {
        ...
        const commentsBySpec = getWithDefaultConstruction<string, Map<TaskSpec, Array<Comment>>>(this.comments, comment.commit, Map)
