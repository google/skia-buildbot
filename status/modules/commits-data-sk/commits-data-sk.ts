...
export class CommitsDataSk extends ElementSk {
  ...
  constructor() {
    super(() => html``);
    console.log('CONSTRUCTIG THE DATA THING');
  }
      ...
      .then((json: GetIncrementalCommitsResponse) => {
        ...
        for (let commit of this.commits) {
          ...
          for (const [taskSpec, task] of (commitTasks || [])) {
            ...
            details.hasFailure = details.hasFailure || (!commit.ignoreFailure && (task.status == TASK_STATUS_FAILURE || task.status == TASK_STATUS_MISHAP));
