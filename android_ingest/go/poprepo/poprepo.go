...
func (p *PopRepo) GetLast() (int64, int64, error) {
	...
	if err != nil {
		return "", 0, fmt.Errorf("BuildID is invalid in %q: %s", string(b), error)
	}
...
func (p *PopRepo) Add(buildid string, ts int64) error {
	...
	defer func() {
		...
		if err := p.checkout.Update(); err != nil {
			glog.Errorf("While rolling back failed Add(): Unable to update the checkout of %q at %q: %s", repoUrl, workdir, err)
		}
	}()
	...
	lastBuildID, lastTs, err := p.GetLast()
	if err != nil {
