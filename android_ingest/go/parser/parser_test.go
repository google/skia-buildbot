...
func TestConvertSecondBranch(t *testing.T) {
	testutils.SmallTest(t)
	// If our branch is listed as the second or later branch then it should become
	// part of the key.
	c := New(lookupMockGood{}, []string{"master", "google-marlin-marlin-O"})
	r := bytes.NewBufferString(INCOMING)
	benchData, err := c.Convert(r)
	assert.NoError(t, err)
	assert.Equal(t, "8dcc84f7dc8523dd90501a4feb1f632808337c34", benchData.Hash)
...
func TestConvert2(t *testing.T) {
	testutils.SmallTest(t)
	c := New(lookupMockGood{}, []string{"google-angler-angler-O", "some-other-branch"})
	r := bytes.NewBufferString(INCOMING2)
	benchData, err := c.Convert(r)
	assert.NoError(t, err)
	assert.Equal(t, "8dcc84f7dc8523dd90501a4feb1f632808337c34", benchData.Hash)
