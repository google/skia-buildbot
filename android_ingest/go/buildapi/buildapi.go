...
// GetMostRecentBuildID returns all buildIDs with Skia commits from latest build back to endBuildId.
//
// The value of endBuildId must not be zero.
func (a *API) GetMostRecentBuildID() (int64, int64, error) {

	// TODO Change this to just look up the most recent buildID.
	...
	if err != nil {
		return -1, -1, skerr.Wrapf(err, "Got an invalid buildid %q: %s ", build.BuildId)
	}
