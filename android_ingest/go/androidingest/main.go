...
func Init() {
	...
	ts, err := auth.NewDefaultTokenSource(*local, androidbuildinternal.AndroidbuildInternalScope, storage.ScopeReadWrite, "https://www.googleapis.com/auth/gerritcodereview")
	if err != nil {
	...
	if !*local {
		_, err := gitauth.New(ts, "/tmp/git-cookie", true, "skia-android-ingest@skia-public.iam.gserviceaccount.com")
		if err != nil {
			sklog.Fatal(err)
		}
	}
