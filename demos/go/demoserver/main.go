...
// the available subdirectories and other repo information. The repo files can be safely read by
// read locking the returned object.
func newSyncedDemos(ctx context.Context, repoURL, checkoutDir, demoPath string) *syncedDemos {
	...
	s.repoURL = repoURL
	s.demoPath = demoPath
	// Write initial version of metadata file.
	hash, err := s.repo.FullHash(ctx, "HEAD")
	if err != nil {
		sklog.Fatalf("Unable to get initial revision info: %s", err)
	}
	sklog.Infof("Repo cloned at %s", hash)
	if err = s.writeMetadata(ctx, hash); err != nil {
		sklog.Fatalf("Unable to write metadata: %s", err)
	} else {
		sklog.Info("WESTON the damn thing said it worked.")

	}
	go util.RepeatCtx(ctx, 3*time.Minute, s.Sync)
	return s
}
...
func (s *syncedDemos) writeMetadata(ctx context.Context, rev string) error {
	...
	metadataPath := filepath.Join(s.repo.Dir(), s.demoPath, "metadata.json")
	err = ioutil.WriteFile(metadataPath, obj, 0644)
	sklog.Infof("Did that thing %s,  %s, %s", metadataPath, err, obj)
	if err != nil {
		return skerr.Wrapf(err, "Unable to write json to '%s'.", metadataPath)
	}
	bytes, err := ioutil.ReadFile(metadataPath)
	sklog.Infof("Err: %s, Bytes: %s, the file is at %s", err, bytes, metadataPath)
	return nil
}

// Sync performs a repo Update under s.Lock().
//
// If the HEAD revision changes it rewrites the
// metadata.json file with updated information.
...
// demoHandler returns a demos.RLock() wrapped fileserver handler for dir.
...
func setupGit() error {
	...
	if !*local {
		if _, err := gitauth.New(ts, filepath.Join(os.TempDir(), "gitcookies"), true, ""); err != nil {
			return skerr.Wrapf(err, "Failed to create git cookie updater: %s")
		}
	}
	return nil
}
