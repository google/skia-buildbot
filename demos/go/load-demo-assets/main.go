...
// Loads all checked in demos from infra-internal to be bundled into the demoserver image for release.
...
var (
	...
	demosPath = flag.String("demos_dir", "scripts", "The top level directory in the repo that holds the demos.")
...
func main() {
	...
	files, err := repo.ListFilesRecursive(context.Background(), *demosPath)
	if err != nil {
		sklog.Fatal(err)
	}
	...
	for _, f := range files {
		err := repo.DownloadFile(context.Background(), filepath.Join(*demosPath, f), filepath.Join(*outDir, f))
		if err != nil {
			sklog.Fatal(err)
		}
	}
