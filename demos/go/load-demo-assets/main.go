...
type directoryBlame vcsinfo.LongCommit

/*type directoryBlame struct {
...
func main() {
	...
	//demoDirsMetadata := make(map[string]*directoryBlame)
	for _, f := range files {
		...
		if demoDirsMetadata[demoName] == nil {
			// When we see a new directory, fetch the recent author.
