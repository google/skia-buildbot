...
func New(ctx context.Context, ts oauth2.TokenSource, fsNamespace, fsProjectId string) (types.NpmDB, error) {
...
func (f *FirestoreDB) GetFromDB(ctx context.Context, key string) (*types.NpmAuditData, error) {
	docRef := f.client.Collection(npmAuditDataCol).Doc(key)
...
func (f *FirestoreDB) PutInDB(ctx context.Context, key, issueName string, created time.Time) error {
	...
	npmAuditCol := f.client.Collection(npmAuditDataCol)
