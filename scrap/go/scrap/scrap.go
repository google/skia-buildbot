...
// isValidType returns true if t is a valid Type.
...
// isValidLang returns true if l is a valid Lang.
func isValidLang(l Lang) bool {
	return ToLang(string(l)) != UnknownLang
}
...
// isValidName returns true if s is a valid name for a scrap.
...
func (s *scrapExchange) Expand(ctx context.Context, t Type, hashOrName string, lang Lang, w io.Writer) error {
	if !isValidLang(lang) {
		return skerr.Wrap(ErrInvalidLanguage)
	}
...
func (s *scrapExchange) ListNames(ctx context.Context, t Type) ([]string, error) {
	...
	if !isValidType(t) {
		return nil, skerr.Wrap(ErrInvalidScrapType)
	}
