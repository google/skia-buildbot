...
// getType returns the Type specified in the URL, or false if the type
// was invalid or not present.
//
// This function will also report the erro on the http.ResponseWriter.
...
func (a *Api) scrapGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	if err := json.NewEncoder(w).Encode(scrapBody); err != nil {
		sklog.Errorf("Failed to encode result: %s", err)
	}
}
