...
func TestAddHandlers_DoNotEnableProtectedEndpoints_ProtectedEndpointsReturn4xxStatusCodes(t *testing.T) {
	...
	testcase := []struct {
		...
		{
			method:       "DELETE",
			path:         fmt.Sprintf("/_/scraps/svg/%s", hash),
			expectedCode: http.StatusMethodNotAllowed,
		},
		{
			method:       "PUT",
			path:         fmt.Sprintf("/_/names/svg/%s", scrapName),
			expectedCode: http.StatusMethodNotAllowed,
		},
		{
			method:       "DELETE",
	...
	for _, pathAndMethod := range testcase {
		w := httptest.NewRecorder()
		r := httptest.NewRequest(pathAndMethod.method, pathAndMethod.path, nil)

		router.ServeHTTP(w, r)
		require.Equal(t, pathAndMethod.expectedCode, w.Code, pathAndMethod)
	}
}
...
func TestNameListHandler_HappyPath_Success(t *testing.T) {
	...
	body := map[string]scrap.Name{
		...
		"AnotherScrap": {
			Hash:        "0000011111",
			Description: "Another description of a different scrap.",
		},
	}
...
// TODO(jcgregorio) Test when you don't add in protected methods.
