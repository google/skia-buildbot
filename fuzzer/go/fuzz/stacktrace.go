...
func (st *StackTrace) LookUpFunctions(finder functionnamefinder.Finder) {
	for i, f := range st.Frames {
		// this awkward syntax is required to write to the actual StackTraceFrame, not the copy generated by range
		if finder != nil {
			st.Frames[i].FunctionName = finder.FunctionName(f.PackageName, f.FileName, f.LineNumber)
		} else {
			st.Frames[i].FunctionName = common.UNKNOWN_FUNCTION
		}
