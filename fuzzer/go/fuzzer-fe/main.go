...
// indexHandler displays the index page, which has no real templating. The client side JSON will
...
// side JSON will query for more information.
...
// rollHandler displays the roll page, which has no real templating. The client side JSON will
...
func detailsJSONHandler(w http.ResponseWriter, r *http.Request) {
	...
	if name != "" {
		var err error
		if reports, err = fuzzPool.FindFuzzDetailForFuzz(name); err != nil {
		...
		if reports, err = fuzzPool.FindFuzzDetails(category, architecture, file, function, int(line)); err != nil {
			httputils.ReportError(w, r, err, "There was a problem fulfilling the request.")
			return
		}
	}
