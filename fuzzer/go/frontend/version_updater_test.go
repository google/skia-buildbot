...
func TestUpdateVersionToFuzz(t *testing.T) {
	...
	m.On("SetFileContents", ctx, "skia_version/pending/"+testRevision, FILE_WRITE_OPTS, []byte(testRevision)).Return(nil).Once()
	f := "skia_version/pending/working_skia-fuzzer-be-1"
	m.On("SetFileContents", ctx, f, FILE_WRITE_OPTS, []byte(f)).Return(nil).Once()
