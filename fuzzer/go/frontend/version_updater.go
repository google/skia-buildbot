...
func UpdateVersionToFuzz(storageClient storage.FuzzerGCSClient, backendWorkers []string, version string) error {
	...
	if err := storageClient.SetFileContents(context.Background(), newVersionFile, FILE_WRITE_OPTS, []byte(version)); err != nil {
		return fmt.Errorf("Could not set pending version: %s", err)
	}
	for bw := range backendWorkers {
		workFile := fmt.Sprintf("skia_version/pending/working_%s", bw)
		if err := storageClient.SetFileContents(context.Background(), workFile, FILE_WRITE_OPTS, []byte(workFile)); err != nil {
