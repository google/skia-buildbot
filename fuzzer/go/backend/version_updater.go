...
func (v *VersionUpdater) reportWorkDone(oldRevision, newRevision string) error {
	...
	count := 0
	v.storageClient.AllFilesInDirectory(context.Background(), "skia_version/pending/working_", func(item *storage.ObjectAttrs) {
		count++
	})
	...
	if count == 0 {
		...
		if err := v.storageClient.SetFileContents(context.Background(), newVersionFile, FILE_WRITE_OPTS, []byte(newRevision)); err != nil {
		...
		if err := v.storageClient.SetFileContents(context.Background(), oldVersionFile, FILE_WRITE_OPTS, []byte(oldRevision)); err != nil {
...
func uploadFuzzNames(sc fstorage.FuzzerGCSClient, oldRevision, category string, bad, grey []string) {
	uploadString := func(fileName, contents string) error {
		...
		if err := sc.SetFileContents(context.Background(), name, FILE_WRITE_OPTS, []byte(contents)); err != nil {
