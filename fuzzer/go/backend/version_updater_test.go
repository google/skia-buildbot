...
func TestReportWorkDoneLastStanding(t *testing.T) {
	...
	mg.On("SetFileContents", ctx, "skia_version/current/newRevision", FILE_WRITE_OPTS, []byte("newRevision")).Return(nil).Once()
	mg.On("SetFileContents", ctx, "skia_version/old/oldRevision", FILE_WRITE_OPTS, []byte("oldRevision")).Return(nil).Once()
