...
func revisionHelper(storageClient fstorage.FuzzerGCSClient, prefix string) (string, time.Time, error) {
	...
	rev := ""
	ts := time.Time{}
	storageClient.AllFilesInDirectory(context.Background(), prefix, func(item *storage.ObjectAttrs) {
...
func AtRevision(revision, path string, v config.VersionSetter, clean bool) error {
	if lc, err := buildskia.GNAtRevision("master", revision, path, config.Common.DepotToolsPath, clean, false); err != nil {
		return fmt.Errorf("Could not buildskia.GNAtRevision for skia revision %s: %s", revision, err)
	} else {
		v.SetSkiaVersion(lc)
		return nil
	}
