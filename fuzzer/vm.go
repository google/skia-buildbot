...
func FuzzerBase(name, ipAddress string) *gce.Instance {
...
func FrontEnd() *gce.Instance {
	vm := FuzzerBase("skia-fuzzer-fe", "104.154.112.170")
	vm.MachineType = gce.MACHINE_TYPE_STANDARD_8
	return vm
}

func BackEnd(num int, ip string) *gce.Instance {
	vm := FuzzerBase(fmt.Sprintf("skia-fuzzer-be-%d", num), ip)
...
func main() {
	server.Main(gce.ZONE_DEFAULT, map[string]*gce.Instance{
		"fe":   FrontEnd(),
		"be-1": BackEnd(1, "104.154.112.171"),
		"be-2": BackEnd(2, "104.154.112.172"),
		"be-3": BackEnd(3, "104.154.112.173"),
	})
}
