...
func main() {
	...
	registerPodWithSwitchboard(ctx, hostname, switchboardInstance)
}

// registerPodWithSwitchboard registers the pod with switchboard and does not
// return unless the given context is cancelled.
func registerPodWithSwitchboard(ctx context.Context, hostname string, switchboardInstance switchboard.Switchboard) {
	...
	case <-ctx.Done():
		return
	case <-time.Tick(switchboard.PodKeepAliveDuration):
