...
func main() {
	...
	if err := registerPodWithSwitchboard(ctx, hostname, switchboardInstance, switchboard.PodKeepAliveDuration, switchboard.PodMaxConsecutiveKeepAliveErrors); err != nil {
		sklog.Fatal(err)
	}
}

// registerPodWithSwitchboard registers the pod with switchboard and does not
...
func registerPodWithSwitchboard(ctx context.Context, hostname string, switchboardInstance switchboard.Switchboard, keepAliveDuration time.Duration, maxConsecutiveKeepAliveErrors int) error {
	...
	// Do a defer that removes the pod from switchboard here because we might
	// exit because of too many consecutive KeepAlivePod failures.
	defer func() {
		if err := switchboardInstance.RemovePod(ctx, hostname); err != nil {
			sklog.Error(err)
		}
