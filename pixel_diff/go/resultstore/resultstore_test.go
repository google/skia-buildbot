...
func TestGetAll(t *testing.T) {
	...
	// If the run doesn't exist in the database, GetAll should return an error.
	_, err = resultStore.GetAll(TEST_NOEXIST_RUN_ID)
	assert.Error(t, err)
}
...
func TestContainsRun(t *testing.T) {
	...
	assert.True(t, resultStore.ContainsRun(TEST_RUN_ID))
	assert.False(t, resultStore.ContainsRun(TEST_NOEXIST_RUN_ID))
}
