...
type ResultStore interface {
	// Get returns a ResultRec from the ResultStore using a runID and url.
	Get(runID, url string) (*ResultRec, error)
	// Add adds a ResultRec to the ResultStore using a runID and url.
	Add(runID, url string, rec *ResultRec) error
	// RemoveRun removes all the data associated with a run from the ResultStore.
...
func (b *BoltResultStore) Get(runID, url string) (*ResultRec, error) {
	...
	viewFn := func(tx *bolt.Tx) error {
		// Retrieve bucket using the runID.
		b := tx.Bucket([]byte(runID))
		if b == nil {
			return fmt.Errorf("Bucket %s does not exist", runID)
		}

		// Get the serialized ResultRec and decode it.
		bytes := b.Get([]byte(url))
		if bytes == nil {
			return fmt.Errorf("Key %s does not exist", url)
		}
