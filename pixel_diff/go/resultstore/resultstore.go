...
type ResultStore interface {
	...
	// ContainsRun indicates if there is data for the runID in the ResultStore.
	ContainsRun(runID string) bool
}
...
// ContainsRun returns true if the boltDB instance contains a bucket for the
// specified RunID.
func (b *BoltResultStore) ContainsRun(runID string) bool {
	var containsRun bool
	viewFn := func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(runID))
		containsRun = b != nil
		return nil
