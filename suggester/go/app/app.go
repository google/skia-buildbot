...
type App struct {
	...
	// The flaky bots and the time range they are active in.
	// TODO Need a way to prepopulate flaky when the app runs for the first time.
	flaky map[string][]*store.TimeRange
}
...
func NewApp(ctx context.Context, git *git.Checkout, httpClient *http.Client, swarmApi swarming.ApiClient, since time.Duration, gitRepoURL string) *App {
	return &App{
		totats:           map[string]map[string]int{},
		ctx:              ctx,
		git:              git,
...
func (a *App) onestep() error {
	...
	for _, r := range resp {
		...
		if tags["sk_issue_server"] != "" {
			...
			files, err := a.git.Git(ctx, "show", "--pretty=", "--name-only", tags["sk_revision"])
			if err != nil {
