...
func NewApp(ctx context.Context, git *git.Checkout, httpClient *http.Client, taskListProvider TaskListProvider, since time.Duration, gitRepoURL string, flakyProvider flaky.FlakyProvider) (*App, error) {
...
func (a *App) onestep() error {
	...
	if err := store.UpdateFlakyBots(a.flakyProvider); err != nil {
		return fmt.Errorf("Failed to update flaky during onestep: %s", err)
	}

	flaky, err := store.ReadFlaky(a.since, time.Now())
	if err != nil {
		return err
	}
	...
	for _, r := range resp {
		...
		if tags["sk_issue_server"] != "" {
			...
			for k, _ := range files {
				...
				if !flaky.WasFlaky(tags["sk_name"], startTime) {
