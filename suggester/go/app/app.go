...
type App struct {
	...
	flaky         flaky.Flaky
	flakyProvider flaky.FlakyProvider
}

func NewApp(ctx context.Context, git *git.Checkout, httpClient *http.Client, swarmApi swarming.ApiClient, since time.Duration, gitRepoURL string, flakyProvider flaky.FlakyProvider) (*App, error) {
...
func (a *App) updateFlaky() error {
	flakes, err := a.flakyProvider()
	if err != nil {
		return fmt.Errorf("Failed to update flaky bots during onestep: %s", err)
	}
	// First, list all bots that are Open.
	open, err := store.AllOpenFlakyBots()
	if err != nil {
		return fmt.Errorf("Failed to update the flaky bots time ranges: %s", err)
	}
	stillOpen := map[string]bool{}
	now := time.Now()
	for botname, begin := range flakes {
		stillOpen[botname] = true
		store.CreateOrUpdateFlaky(botname, begin, now, true)
	}
	// Close all bots that were Open but aren't in "flakes".
	for botname, _ := range open {
		if !stillOpen[botname] {
			store.CloseFlaky(botname)
		}
	}

	updatedFlaky, err := store.ReadFlaky(a.since, time.Now())
	if err != nil {
		return fmt.Errorf("Unable to read flakes from datastore: %s", err)
	}
	a.mutex.Lock()
	defer a.mutex.Unlock()
	a.flaky = updatedFlaky
	return nil
}
