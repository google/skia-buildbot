...
// Returns true if a bot should be ignored.
...
func (sf *StoredFailure) Name() string {
	return fmt.Sprintf("%x", md5.New().Sum([]byte(sf.BotName+sf.Hash+sf.Issue+sf.Patch)))
}

// Store failures in the Cloud Datastore.
...
func New(filter BadBot, provider TaskListProvider, client *datastore.Client, git *git.Checkout, httpClient *http.Client, gitRepoURL string) *FailureStore {
	return &FailureStore{
		filter:     filter,
		provider:   provider,
		client:     client,
...
func (f *FailureStore) Update(since time.Duration) error {
	...
	for _, r := range resp {
		...
		if tags["sk_issue_server"] != "" {
			...
			if err != nil {
				fmt.Println("Failed to get commit file list: %s", err)
		...
		key.Name = sf.Name()
		if _, err := f.client.Put(f.ctx, key, sf); err != nil {
...
func (f *FailureStore) List(begin, end time.Time) ([]*StoredFailure, error) {
	ret := []*StoredFailure{}
	//q := ds.NewQuery(dsconst.FAILURES).Filter("TS >", begin).Filter("TS <=", end)
	q := ds.NewQuery(dsconst.FAILURES)
	it := f.client.Run(f.ctx, q)
	row := &StoredFailure{}
	for {
		_, err := it.Next(row)
