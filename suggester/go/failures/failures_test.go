	...
	return []*swarmingv1.SwarmingRpcsTaskRequestMetadata{
		&swarmingv1.SwarmingRpcsTaskRequestMetadata{
			TaskResult: &swarmingv1.SwarmingRpcsTaskResult{
				...
				StartedTs: now.Add(-10 * time.Minute).Format(time.RFC3339Nano),
			},
		},
		&swarmingv1.SwarmingRpcsTaskRequestMetadata{
			TaskResult: &swarmingv1.SwarmingRpcsTaskResult{
				...
				StartedTs: now.Add(-1 * time.Minute).Format(time.RFC3339Nano),
			},
		},
	}, nil
}
...
func TestStore(t *testing.T) {
	...
	fs := New(badbot, taskListProvider, ds.DS, git, http.DefaultClient, "https://skia.googlesource.com/skia.git")
