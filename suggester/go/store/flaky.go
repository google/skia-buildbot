...
func CloseFlaky(botname string) error {
	// Look for an existing timeRangeStored that is open. If none-found
	// then create a new entry (if !open).
	defer metrics2.FuncTimer().Stop()
...
func AllOpenFlakyBots() (map[string]bool, error) {
	ret := map[string]bool{}
	ctx := context.Background()
...
func UpdateFlakyBots(flakyProvider flaky.FlakyProvider) error {
	flakes, err := flakyProvider()
	if err != nil {
		return err
	}
	// First, list all bots that are Open.
	open, err := AllOpenFlakyBots()
	if err != nil {
		return err
	}
	...
	for botname, begin := range flakes {
		...
		CreateOrUpdateFlaky(botname, begin, now, true)
	}
...
func ReadFlaky(since time.Duration, now time.Time) (flaky.Flaky, error) {
	now = now.UTC()
	defer metrics2.FuncTimer().Stop()
	ret := flaky.Flaky{}
	ctx := context.Background()
	...
	for {
		...
		if _, ok := ret[row.BotName]; !ok {
			ret[row.BotName] = []*flaky.TimeRange{tr}
		} else {
