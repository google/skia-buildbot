...
func NewFlakyBuilder(provider FlakyProvider, client *datastore.Client) *FlakyBuilder {
	return &FlakyBuilder{
		provider: provider,
		client:   client,
	}
...
func (fb *FlakyBuilder) createOrUpdateFlaky(botname string, begin, end time.Time, open bool) error {
	...
	keys, err := fb.client.GetAll(ctx, query, &slice_stored)
	if len(slice_stored) == 1 {
		stored := slice_stored[0]
		stored.Begin = begin
		stored.End = end
		stored.Open = open
		_, err = fb.client.Put(ctx, keys[0], stored)
		if err != nil {
		...
		_, err = fb.client.Put(ctx, ds.NewKey(dsconst.FLAKY_RANGES), stored)
		if err != nil {
