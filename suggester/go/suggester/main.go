...
import (
	...
	"go.skia.org/infra/suggester/go/app"
	"go.skia.org/infra/suggester/go/failures"
	"go.skia.org/infra/suggester/go/flaky"
	"go.skia.org/infra/suggester/go/statusprovider"
	"go.skia.org/infra/suggester/go/tasklistprovider"
)
...
func main() {
	...
	fb := flaky.NewFlakyBuilder(flaky.FlakyProvider(statusProvider.Get), ds.DS)
