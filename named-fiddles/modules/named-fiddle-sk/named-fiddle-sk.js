...
const template = (ele) => html`<span class=name><a href='https://fiddle.skia.org/c/${ele._state.hash}'>${ele._state.name}</a></span> <span class=status>${status(ele)}</span> <button on-click=${(e) => ele._editClick(e)}>Edit</button> <button  on-click=${(e) => ele._deleteClick(e)}>Delete</button>`;
