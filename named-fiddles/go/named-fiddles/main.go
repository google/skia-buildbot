...
func getIncludes(directoryPath string, reader io.Reader) ([]string, error) {
	...
	for scanner.Scan() {
		if match := re.FindStringSubmatch(scanner.Text()); len(match) > 1 {
			values = append(values, filepath.Join(directoryPath, match[1]))
		}
	}
...
func (srv *Server) exampleStep() {
	...
	examplesPath := filepath.Join(*repoDir, "tools", "fiddle", "all_examples.cpp")
	examplesFile, err := os.Open(filepath.Clean(examplesPath))
	if err != nil {
		sklog.Errorf("Failed to open all_examples.cpp: %q.", err)
		return
	}

	paths, err := getIncludes(filepath.Dir(examplesPath), examplesFile)
	if err != nil {
