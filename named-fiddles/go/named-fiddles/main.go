...
//
// TODO(jcgregorio) Break out as its own library.
...
func (srv *Server) mainHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := srv.templates.ExecuteTemplate(w, "index.html", map[string]string{
		// base64 encode to avoid golang templating escaping.
...
func (srv *Server) updateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" {
		httputils.ReportError(w, r, nil, "One of Name and NewName must be non-empty.")
		return
	}
...
func main() {
	...
	CSRF := csrf.Protect(srv.salt, csrf.Secure(!*local))
	h = CSRF(h)
