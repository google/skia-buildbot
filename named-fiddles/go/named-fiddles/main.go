...
func (srv *Server) updateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		httputils.ReportError(w, err, "Error decoding JSON.")
		return
	}
	req.User = srv.user(r)
	...
	if req.Hash == "" {
		httputils.ReportError(w, nil, "Invalid request, Hash must be non-empty.")
		return
	}
	if err := srv.store.Exists(req.Hash); err != nil {
		httputils.ReportError(w, err, "Hash is not a valid fiddle.")
		return
	}
	// Name == ""     => Create
	// NewName != ""  => Rename
	...
	if name == "" {
		httputils.ReportError(w, nil, "Name must not be empty.")
		return
	}
	if !srv.store.ValidName(name) {
		httputils.ReportError(w, nil, "Invalid characaters found in name.")
		return
	}
	if err := srv.store.WriteName(name, req.Hash, req.User, req.Status); err != nil {
		httputils.ReportError(w, err, "Failed update.")
		return
	}
	if req.Name != "" && req.NewName != "" {
		// This is a rename so delete old Name.
		if err := srv.store.DeleteName(req.Name); err != nil {
			httputils.ReportError(w, err, "Failed delete on rename.")
			return
		}
	}
...
func (srv *Server) namedHandler(w http.ResponseWriter, r *http.Request) {
	named, err := srv.store.ListAllNames()
	if err != nil {
		httputils.ReportError(w, err, "Failed to read named fiddles.")
		return
	}
