...
func InitMonorail(ctx context.Context, serviceAccountFilePath, project, query string) (BugFramework, error) {
...
func (m *Monorail) searchIssuesWithPagination() ([]MonorailIssue, error) {
	...
	nextPageToken := ""
	for {
		query := fmt.Sprintf(`{"projects": ["projects/%s"], "query": "%s", "page_token": "%s"}`, m.project, m.query, nextPageToken)
		// query := fmt.Sprintf(`{"projects": ["projects/%s"], "query": "%s", "page_size": %d}`, m.project, m.query, MonorailMaxResults)
...
// TODO(rmistry): Figure out a way to query for untriaged otherwise the counts will not work.
// The open parameter is not used for Monorail searches. Specify this in your query with 'is:open'.
func (m *Monorail) Search(ctx context.Context, _, unassigned bool) ([]*Issue, error) {
	// // USE MonorailMaxResults instead of 3!
	// query := fmt.Sprintf(`{"projects": ["projects/%s"], "query": "%s", "page_size": 3}`, m.project, m.query)
	// // query := fmt.Sprintf(`{"projects": ["projects/%s"], "query": "%s", "page_size": %d}`, m.project, m.query, MonorailMaxResults)
	// b, err := m.makeJSONCall([]byte(query), "Issues", "SearchIssues")
	// if err != nil {
	// 	return nil, skerr.Wrapf(err, "Issues.SearchIssues JSON API call failed")
	...
	for _, mi := range monorailIssues {
		...
		if mi.Owner.User != "" {
			if unassigned {
				// This issue has an owner but we want only unassigned issues.
				continue
			}
			...
			if email, ok := userToEmailCache[mi.Owner.User]; ok {
				...
				fmt.Println("DID NOT FIND IN CACHE!")
		...
		priority := ""
		if priorityData, ok := MonorailProjectToPriorityData[m.project]; ok {
