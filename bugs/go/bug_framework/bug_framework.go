...
func (m *Monorail) Search(ctx context.Context, open, unassigned bool) ([]*Issue, error) {
	searchIssuesPath := MonorailApiBase + "monorail.v3.Issues/SearchIssues"
	// Where is the documentation for this..

	// bodyJSON := []byte(`{"projects": ["projects/skia"], "query": "owner:rmistry@google.com status:Started"}`)
	bodyJSON := []byte(`{"projects": ["projects/skia"], "query": "id:873"}`)
	req, err := http.NewRequest("POST", searchIssuesPath, bytes.NewBuffer(bodyJSON))
	if err != nil {
