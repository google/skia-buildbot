...
However, the Gazelle API uses the word "rule" to refer to what we normally call targets. This
Gazelle extension uses said word in the same fashion as the Gazelle API to avoid confusion.
...
`language.Language` interface:
 ...
 1. Index the imports that existing Bazel rules may provide.
 2. Generate or update rules in each target directory.
 3. Resolve the dependencies of any generated or updated rules (i.e. populate their `deps`
    arguments).
...
struct with the following contents:
 ...
 - A list of empty rules, that is, existing rules (defined in the directory's `BUILD.bazel` file)
   that no longer can be built, e.g. because they source files have been deleted (field
...
The above imports must be parsed from the sources of each rule. Gazelle extensions may use for this
purpose a parser for the programming language of the source files, regular expressions, or any other
suitable technique.

As mentioned in step 1, Gazelle will call the `Imports` method with each rule returned by
`GenerateRules` in order th keep the `resolve.RuleIndex` up-to-date.

Note that in this example, none of the generated rules have a `deps` argument. Step 3 populates said
argument.
...
## How this particular Gazelle extension works

While Gazelle extensions typically rely on the `resolve.RuleIndex` to resolve dependencies, this
particular Gazelle extension uses a custom index due to limitations with the `resolve.RuleIndex`
struct. Method `Imports` populates the custom index, and method `Resolve` queries it. Save for this
difference, this Gazelle extension behaves as described in the high level overview.
