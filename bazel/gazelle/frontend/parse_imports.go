...
var tsImportRegexps = []*regexp.Regexp{
	...
	regexp.MustCompile(`^\s*import\s*{?([[:alnum:]]|\s|\*)*}?\s*from\s*'(?P<path>.*)'`), // Single quotes.
	regexp.MustCompile(`^\s*import\s*{?([[:alnum:]]|\s|\*)*}?\s*from\s*"(?P<path>.*)"`), // Double quotes.
...
// parseTSImports takes the contents of a Sass source file and extracts the verbatim paths of any
// imported modules.
