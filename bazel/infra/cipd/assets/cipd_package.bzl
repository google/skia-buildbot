...
ExecutableInfo = provider("Passes a path to a tool", fields = ["path"])
...
def _impl2(ctx):
    ...
    #return [DefaultInfo(
    #    executable = ctx.actions.declare_file("clang_linux/bin/clang")
    #)]
    return [ExecutableInfo(path = ctx.outputs.outputs[0])]


cipd_package_executable = rule(
