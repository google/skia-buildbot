...
describe('example test', () => {
  ...
  it('should get a 404 error at /', async () => {
    expect(await getStatusCodeAndPageText(`${baseUrl}/`)).to.deep.equal([
      404,
      '404 page not found',
    ]);
  });
  ...
  it('should get a 400 error at /echo', async () => {
    expect(await getStatusCodeAndPageText(`${baseUrl}/echo`)).to.deep.equal([
      400,
      'Error: Query parameter "msg" is empty or missing.',
    ]);
  });
