...
def test_on_env(name, test, env, timeout_secs = 10):
    ...
    - The $ENV_DIR directory can be used to communicate between the environment and test binaries.
      A typical use case is for the environment to create an $ENV_DIR/port file containing the TCP
    ...
    integration tests); integration tests for a command-line tools that talk to RPC servers, etc.

    Args:
      name: Name of the rule.
      test: Target for the test (can be any *_test target).
