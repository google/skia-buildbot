...
func main() {
	...
	if start {
		...
		for _, e := range emulators.AllEmulators {
			fmt.Println(fmt.Sprintf("export %s=%s", emulators.GetEmulatorHostEnvVar(e), emulators.GetEmulatorHost(e)))
		}
	} else {
		fmt.Println("Emulators stopped. Unset environment variables as follows:")
		for _, e := range emulators.AllEmulators {
			fmt.Println(fmt.Sprintf("export %s=", emulators.GetEmulatorHostEnvVar(e)))
		}
	}
}
