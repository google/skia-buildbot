...
var (
	...
	now     = flag.Int("now", time.Now().Unix(), "Current timestamp; use to make this script reproducible.")
...
func main() {
	...
	if err := util.WithWriteFile(*botsCfg, func(w io.Writer) error {
		_, err := w.Write([]byte(botCfgData))
		return err
	}); err != nil {
		sklog.Fatal(err)
	}
