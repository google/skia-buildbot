...
# This is a linux amd64 version of a credential helper recommended for use on the cloud.
# See https://cloud.google.com/container-registry/docs/advanced-authentication
AUTH_HELPER_URL = 'https://storage.googleapis.com/skia-backups/docker-credential-gcr/1.5.0/docker-credential-gcr'
...
# Stop the daemon and tell it to put the docker images in /mnt/pd0/docker, which has a bigger disk.
...
# Download the Docker authenticator helper, which will authenticate our calls to gcr.io properly
subprocess.check_call(['rm', '-f', '/home/chrome-bot/.docker/config.json'])
subprocess.check_call(['wget', '-O', '/tmp/docker-credential-gcr', AUTH_HELPER_URL])
