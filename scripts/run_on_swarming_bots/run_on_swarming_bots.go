	...
	*workdir, err = filepath.Abs(*workdir)
	...
	for _, bot := range bots {
		...
		go func(id string) {
			...
			if _, err := swarming.TriggerSwarmingTasks(m, dims, tags, 100, 120*time.Minute, 120*time.Minute, 120*time.Minute, false); err != nil {
				glog.Fatal(err)
			}
		}(bot.BotId)
	}
	wg.Wait()
	tasksLink := fmt.Sprintf("https://chromium-swarm.appspot.com/tasklist?f=task_tag=group:%s", group)
