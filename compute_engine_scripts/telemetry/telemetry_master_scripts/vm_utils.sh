...
function get_current_work_on_slave() {
  ...
  current_work=`ssh -o UserKnownHostsFile=/dev/null -o CheckHostIP=no \
    -o StrictHostKeyChecking=no -i /usr/local/google/home/rmistry/.ssh/google_compute_engine \
    -A -p 22 default@108.170.222.$slave -- "ls /home/default/storage/current_work"`
  echo $current_work
}
