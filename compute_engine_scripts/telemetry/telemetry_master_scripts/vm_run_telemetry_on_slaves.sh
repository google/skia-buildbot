...
for SLAVE_NUM in $(seq 1 $NUM_SLAVES); do
  ...
  ssh -f -X -o UserKnownHostsFile=/dev/null -o CheckHostIP=no \
    ...
    -A -p 22 default@108.170.222.$SLAVE_NUM -- "source .bashrc; cd skia-repo/buildbot/compute_engine_scripts/telemetry/telemetry_slave_scripts; svn update; $CMD > /tmp/${TELEMETRY_BENCHMARK}_output.txt 2>&1"
done
