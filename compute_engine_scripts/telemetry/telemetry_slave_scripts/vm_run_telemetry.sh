...
for page_set in /home/default/storage/page_sets/*.json; do
  if [[ -f $page_set ]]; then
    ...
    DISPLAY=:0 timeout 300 tools/perf/run_measurement --extra-browser-args=--disable-setuid-sandbox --browser=system $TELEMETRY_BENCHMARK $page_set $EXTRA_ARGS -o /tmp/${RUN_ID}.${page_set_basename}
    if [ $? -eq 124 ]; then
