...
function install_packages {
  ...
  sudo debconf-set-selections <<< \
    'mysql-server mysql-server/root_password_again password tmp_pass' && \
  ...
  gcloud compute --project $PROJECT_ID ssh --zone $ZONE ${PROJECT_USER}@$INSTANCE_NAME \
    --command "sudo apt-get -y --purge remove apache2*" \
    || FAILED="$FAILED RemoveApache2"
  gcloud compute --project $PROJECT_ID ssh --zone $ZONE ${PROJECT_USER}@$INSTANCE_NAME \
    --command "sudo sh -c \"echo '* - nofile 500000' >> /etc/security/limits.conf\" " \
    || FAILED="$FAILED FixUlimit"
  echo
}
...
function download_file {
  ...
  gcloud compute --project $PROJECT_ID ssh --zone $ZONE chrome-bot@$INSTANCE_NAME \
    --command "gsutil cp gs://skia-buildbots/artifacts/$1 ~" \
...
function setup_ct_swarming_bot {
  ...
  URL="https://skia.googlesource.com/buildbot/+/master/ct/tools/setup_ct_machine.sh?format=TEXT"
  gcloud compute --project $PROJECT_ID ssh --zone $ZONE ${PROJECT_USER}@$INSTANCE_NAME \
    --command "curl -sSf '$URL' | base64 --decode > '/tmp/ct_setup.sh' && bash /tmp/ct_setup.sh" \
