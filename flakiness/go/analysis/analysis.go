...
// flakes, and returns a map[string]map[string]*Results instance whose keys
// are repository URLs at the top level and task spec names at the next.
...
func DefinitelyFlaky(tasks []*db.Task) []*Flake {
	...
	for _, t := range tasks {
		...
		if t.RetryOf != "" && t.Success() {
			...
			if !ok {
				sklog.Errorf("Failed to retrieve task %q: not in range", t.RetryOf)
				continue
			}
...
func MaybeFlaky(tasks []*db.Task, start, end time.Time, repo *repograph.Graph) []*Flake {
	...
	for _, seq := range sequences {
		...
		for _, commit := range seq[0].Commits {
			...
			if revertedCommit != "" {
				revertedCommits = append(revertedCommits, revertedCommit)
			}
		}
		for _, revertedCommit := range revertedCommits {
			for _, commit := range seq[1].Commits {
				if revertedCommit == commit {
					wasRevert = true
					break
				}
			}
