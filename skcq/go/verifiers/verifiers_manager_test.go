...
func testGetVerifier(t *testing.T, isCQ, isDryRun bool, submittedTogetherChange *gerrit.ChangeInfo, expectedVerifiers []string) {
	...
	if submittedTogetherChange == nil {
		cr.On("GetCommitMessage", testutils.AnyContext, ci).Return(commitMsg, nil).Once()
		...
		cr.On("GetCommitMessage", testutils.AnyContext, ci).Return(commitMsg, nil).Once()
		cr.On("GetCommitMessage", testutils.AnyContext, submittedTogetherChange).Return(commitMsg, nil).Once()
	...
	verifiers, actualSubmittedTogether, err := vm.GetVerifiers(context.Background(), mockClient.Client(), cfg, cr, ci, false, cfgReader)
	require.Nil(t, err)
