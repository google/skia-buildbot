...
func (stv *SubmittedTogetherVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	for ts, tsVerifiers := range stv.togetherChangesToVerifiers {
		...
		if len(rm) > 0 {
			return types.VerifierFailureState, fmt.Sprintf("%s Submitted together change https://skia-review.googlesource.com/c/%d has failed verifiers: %s", stv.Name(), ts.Issue, strings.Join(rm, "\n")), nil
		} else if len(wm) > 0 {
			return types.VerifierWaitingState, fmt.Sprintf("%s Submitted together change %d is waiting for verifiers: %s", stv.Name(), ts.Issue, strings.Join(wm, "\n")), nil
		} else {
			successMsgsFromVerfiers = append(successMsgsFromVerfiers, sm...)
		}
	}
	return types.VerifierSuccessState, fmt.Sprintf("%s Successfully ran all verifiers of submitted together change: %s", stv.Name(), strings.Join(successMsgsFromVerfiers, "\n")), nil
}
