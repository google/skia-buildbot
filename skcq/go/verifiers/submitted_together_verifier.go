...
func NewSubmittedTogetherVerifier(ctx context.Context, togetherChanges []*gerrit.ChangeInfo, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, gitilesRepo *gitiles.Repo) (Verifier, error) {
	togetherChangesToVerifiers := map[*gerrit.ChangeInfo][]Verifier{}
	for _, tc := range togetherChanges {
		tcVerifiers, err := GetVerifiers(ctx, httpClient, cfg, cr, tc, true /* isSubmittedTogetherChange */, gitilesRepo)
		if err != nil {
...
func (stv *SubmittedTogetherVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	for ts, tsVerifiers := range stv.togetherChangesToVerifiers {
		sm, wm, rm := RunVerifiers(ctx, ts, tsVerifiers)
		if len(rm) > 0 {
