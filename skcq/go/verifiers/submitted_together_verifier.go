...
func NewSubmittedTogetherVerifier(ctx context.Context, togetherChanges []*gerrit.ChangeInfo, httpClient *http.Client, skCQCfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, gitilesRepo *gitiles.Repo, configReader *config.GitilesConfigReader, footersMap map[string]string) (Verifier, error) {
...
func (stv *SubmittedTogetherVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if len(stv.togetherChangesToVerifiers) > 0 {
		noDepChecks := footers.GetBoolVal(stv.footersMap, footers.NoDependencyChecks, ci.Issue)
		if noDepChecks {
			return types.VerifierFailureState, fmt.Sprintf("Failing because \"%s: %t\" has been specified and there are %d changes that will be submitted together", footers.NoDependencyChecks, noDepChecks, len(stv.togetherChangesToVerifiers)), nil
		}
	}
