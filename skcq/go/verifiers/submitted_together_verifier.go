...
func NewSubmittedTogetherVerifier(ctx context.Context, togetherChanges []*gerrit.ChangeInfo, httpClient *http.Client, skCQCfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, configReader *config.GitilesConfigReader, footersMap map[string]string) (Verifier, error) {
	togetherChangesToVerifiers := map[*gerrit.ChangeInfo][]Verifier{}
	for _, tc := range togetherChanges {
		tcVerifiers, _, err := GetVerifiers(ctx, httpClient, skCQCfg, cr, tc, true /* isSubmittedTogetherChange */, configReader)
		if err != nil {
...
func (stv *SubmittedTogetherVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if len(stv.togetherChangesToVerifiers) > 0 {
		noDepChecks := footers.GetBoolVal(stv.footersMap, footers.NoDependencyChecksFooter, ci.Issue)
		if noDepChecks {
