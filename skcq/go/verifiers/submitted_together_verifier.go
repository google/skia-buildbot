...
func NewSubmittedTogetherVerifier(ctx context.Context, vm *SkCQVerifiersManager, togetherChanges []*gerrit.ChangeInfo, httpClient *http.Client, skCQCfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, configReader *config.GitilesConfigReader, footersMap map[string]string) (interfaces.Verifier, error) {
	togetherChangesToVerifiers := map[*gerrit.ChangeInfo][]interfaces.Verifier{}
