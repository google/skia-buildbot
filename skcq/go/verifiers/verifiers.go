...
func GetVerifiers(ctx context.Context, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, isSubmittedTogetherChange bool) ([]Verifier, error) {
	fmt.Printf("\n\nDEBUGGING IN GET VERIFIERS %d: %+v", ci.Issue, cfg)
	...
	if isSubmittedTogetherChange || cr.IsCQ(ctx, ci) {
		if !isSubmittedTogetherChange {
			...
			// TODO(rmistry): Rename to cqTriggererVerifier?
...
func (av *ApprovedVerifier) Verify(ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	fmt.Println("DEBUGGING in VERIFY IN [ApprovedVerifier]")
	fmt.Println(committersWhoApproved)
...
func NewSubmittedTogetherVerifier(ctx context.Context, togetherChanges []*gerrit.ChangeInfo, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo) (Verifier, error) {
	...
	for _, tc := range togetherChanges {
		tcVerifiers, err := GetVerifiers(ctx, httpClient, cfg, cr, tc, true /* isSubmittedTogetherChange */)
		if err != nil {
...
func (stv *SubmittedTogetherVerifier) Verify(ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	for ts, tsVerifiers := range stv.togetherChangesToVerifiers {
		...
		if len(rm) > 0 {
			return FailureState, fmt.Sprintf("Submitted together change https://skia-review.googlesource.com/c/%d has failed verifiers: %s", ts.Issue, strings.Join(rm, "\n")), nil
		} else if len(wm) > 0 {
