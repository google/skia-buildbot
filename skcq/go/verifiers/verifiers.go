...
func GetVerifiers(ctx context.Context, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, isSubmittedTogetherChange bool, gitilesRepo *gitiles.Repo, configReader *config.GitilesConfigReader) ([]Verifier, []string, error) {
	...
	if isSubmittedTogetherChange || cr.IsCQ(ctx, ci) {
		...
		if !isSubmittedTogetherChange {
			...
			if len(togetherChanges) > 0 {
				togetherChangesVerifier, err := NewSubmittedTogetherVerifier(ctx, togetherChanges, httpClient, cfg, cr, ci, gitilesRepo, configReader, footersMap)
				if err != nil {
	...
	tryJobsVerifier, err := NewTryJobsVerifier(httpClient, cr, tasksCfg, footersMap)
	if err != nil {
