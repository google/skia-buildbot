...
func (av *ApprovedVerifier) Verify(ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	// // Make sure this CL has been approved by a commiter.
	// if val, ok := ci.Labels[gerrit.LabelCodeReview]; ok {
	// 	// Find approver who is also a committer.
	// 	committersWhoApproved := []string{}
	// 	for _, ld := range val.All {
	// 		if ld.Value == gerrit.LabelCodeReviewApprove {
	// 			if av.committerAllowed.Member(ld.Email) {
	// 				committersWhoApproved = append(committersWhoApproved, ld.Email)
	// 			}
	// 		}
	// 	}
