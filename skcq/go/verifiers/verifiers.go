...
func (cv *CommitterListVerifier) Verify(ci *gerrit.ChangeInfo) (waitMsg string, rejectMsg string, err error) {
	if cv.committerAllowed.Member(ci.Owner.Email) {
		return "", "", nil
	} else {
		return "", fmt.Sprintf("%s is not a member of the cria group %s", ci.Owner.Email, cv.criaGroupName), nil
	}
}
...
func NewDryRunAccessListVerifier(httpClient *http.Client, criaGroup string) (Verifier, error) {
	...
	fmt.Println(dryRunAllowed.Emails())
	if err != nil {
		return nil, skerr.Fmt("Could not create an allowed from %s: %s", criaGroup, err)
	}
...
func (cv *DryRunAccessListVerifier) Verify(ci *gerrit.ChangeInfo) (waitMsg string, rejectMsg string, err error) {
...
func (tv *TreeStatusVerifier) Verify(ci *gerrit.ChangeInfo) (waitMsg string, rejectMsg string, err error) {
	...
	if treeStatus.GeneralState == types.OpenState {
		...
		return fmt.Sprintf("Waiting for tree to be open. Tree is currently in %s state: \"%s\"", treeStatus.GeneralState, treeStatus.Message), "", nil
	}
}
