...
func (dv *DryRunAccessListVerifier) Verify(ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	if len(allowedDryRunVoters) > 0 {
		return SuccessState, fmt.Sprintf("%s CQ+1 voted by allowed dry-run voters %s", dv.Name(), strings.Join(allowedDryRunVoters, ",")), nil
	}
	return FailureState, fmt.Sprintf("%s CQ+1 requires a vote by an allowed dry-run voter", dv.Name()), nil
}

func (dv *DryRunAccessListVerifier) Cleanup(ci *gerrit.ChangeInfo) {
	return
}
