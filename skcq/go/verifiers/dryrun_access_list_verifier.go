...
func (dv *DryRunAccessListVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if len(allowedDryRunVoters) > 0 {
		return types.VerifierSuccessState, fmt.Sprintf("%s CQ+1 voted by allowed dry-run voters %s", dv.Name(), strings.Join(allowedDryRunVoters, ",")), nil
	}
	return types.VerifierFailureState, fmt.Sprintf("%s CQ+1 requires a vote by an allowed dry-run voter", dv.Name()), nil
}
