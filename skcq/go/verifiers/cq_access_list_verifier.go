...
func (cv *CQAccessListVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if len(committersWhoCQed) > 0 {
		return types.VerifierSuccessState, fmt.Sprintf("%s CQ+2 voted by committers %s", cv.Name(), strings.Join(committersWhoCQed, ",")), nil
	}
	return types.VerifierFailureState, fmt.Sprintf("%s CQ+2 requires a vote from a committer", cv.Name()), nil
}
