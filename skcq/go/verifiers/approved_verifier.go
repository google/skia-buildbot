...
func (av *ApprovedVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if len(committersWhoApproved) > 0 {
		return types.VerifierSuccessState, fmt.Sprintf("%s Approved by committers %s", av.Name(), strings.Join(committersWhoApproved, ",")), nil
	}
	...
	return types.VerifierFailureState, fmt.Sprintf("%s This CL requires approval from a committer", av.Name()), nil
}
