...
func GetSkCQVerifiersManager(throttlerManager types.ThrottlerManager) types.VerifiersManager {
...
func (vm *SkCQVerifiersManager) GetVerifiers(ctx context.Context, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, isSubmittedTogetherChange bool, configReader config.ConfigReader) ([]types.Verifier, []string, error) {
	...
	commitMsg, err := cr.GetCommitMessage(ctx, ci.Issue)
	if err != nil {
