...
func (vm *SkCQVerifiersManager) GetVerifiers(ctx context.Context, httpClient *http.Client, cfg *config.SkCQCfg, cr codereview.CodeReview, ci *gerrit.ChangeInfo, isSubmittedTogetherChange bool, configReader *config.GitilesConfigReader) ([]interfaces.Verifier, []string, error) {
	// Instantiate the 2 slices that will be returned.
	clVerifiers := []interfaces.Verifier{}
