...
func (tv *TryJobsVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if includeTryjobsFooter != "" {
		...
		for bucket, tryJobs := range includeTryJobsMap {
			...
			if len(projectAndBucket) != 2 {
				sklog.Errorf("[%d] Unsupported bucket value in \"%s\" for %+v", ci.Issue, projectAndBucket, includeTryJobsMap)
				continue
			}
			p := projectAndBucket[0]
			if p != BuildBucketDefaultSkiaProject {
				sklog.Errorf("[%d] Could not recognize bb project %s for %+v", ci.Issue, p, includeTryJobsMap)
				continue
			}
			b := projectAndBucket[1]
			if b != bbBucket {
				sklog.Errorf("[%d] Specified bucket %s is different than expected bucket %s", ci.Issue, b, bbBucket)
				continue
			}
			for _, t := range tryJobs {
				...
				if _, ok := cqTryjobsToConfigs[t]; !ok {
					...
					sklog.Infof("[%d] Added tryjob %s because it was specified in %d", ci.Issue, t, footers.IncludeTryjobsFooter)
	...
	for cqJobName, cqCfg := range cqTryjobsToConfigs {
			...
			for _, locationRegex := range cqCfg.LocationRegexes {
				...
				for _, cf := range changedFiles {
					if r.MatchString(cf) {
						...
						sklog.Infof("[%d] Adding CQ job %s because it did not matches the location regex: %s", ci.Issue, cqJobName, locationRegexMatch)
						break regexesLoop
					}
				}
			}
		...
		if build, ok := nameToBuilderOnChange[cqJobName]; ok {
			...
			if !rerunTryJobs && time.Now().Unix()-build.GetCreateTime().GetSeconds() >= TryJobStaleTimeoutSecs {
	...
	if len(waitingTryJobs) > 0 {
		return types.VerifierWaitingState, fmt.Sprintf("%s Waiting for: %s", tv.Name(), strings.Join(waitingTryJobs, ",")), nil
	} else {
		...
		return types.VerifierSuccessState, "CQ Try jobs were successful", nil
	}
}
...
func (tv *TryJobsVerifier) Cleanup(ctx context.Context, ci *gerrit.ChangeInfo) {
	...
	// Use DoNotCancelTryjobsFooter here?
