...
func (tv *TryJobsVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if rerunTryJobs {
		sklog.Infof("[%d] \"%s: %t\" has been specified. Try jobs that ran < 24hrs will not be reused. All successful try jobs that completed before this cq attempt will not be counted.", ci.Issue, footers.RerunTryjobsFooter, rerunTryJobs)
	}
