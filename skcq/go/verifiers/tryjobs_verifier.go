...
func (tv *TryJobsVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo) (state VerifierState, reason string, err error) {
	...
	gerritURL := tv.cr.Url(0)
	tryJobsOnChange, err := tv.bb2.GetTrybotsForCL(ctx, ci.Issue, latestPatchsetID, gerritURL)
	if err != nil {
	...
	for cqJobName, cqCfg := range tv.tasksCfg.CommitQueue {
		...
		if len(cqCfg.LocationRegexes) > 0 {
			changedFiles, err := tv.cr.GetFileNames(ctx, ci.Issue, latestPatchsetID)
			if err != nil {
	...
	if len(triggerTryJobs) > 0 {
		if err := tv.bb2.ScheduleBuilds(ctx, triggerTryJobs, ci.Issue, latestPatchsetID, gerritURL); err != nil {
