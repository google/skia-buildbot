...
func NewTryJobsVerifier(httpClient *http.Client, cr codereview.CodeReview, tasksCfg *specs.TasksCfg, footersMap map[string]string) (Verifier, error) {
...
func (tv *TryJobsVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	// See if non-stale try jobs should still be retriggered.
	...
	for cqJobName, cqCfg := range tv.tasksCfg.CommitQueue {
		...
		if build, ok := nameToBuilderOnChange[cqJobName]; ok {
			...
			if !rerunTryJobs && time.Now().Unix()-build.GetCreateTime().GetSeconds() >= TryJobStaleTimeoutSecs {
				...
				fmt.Println("IN HERE FOR STALE STUFF")
				fmt.Println(rerunTryJobs)
