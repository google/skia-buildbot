...
func (tv *TryJobsVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	rerunTryJobs := footers.GetBoolVal(tv.footersMap, footers.RerunTryJobs, ci.Issue)
	if rerunTryJobs {
		sklog.Infof("[%d] \"%s: %t\" has been specified. Try jobs that ran > 24hrs will not be reused.", ci.Issue, footers.RerunTryJobs, rerunTryJobs)
	}
