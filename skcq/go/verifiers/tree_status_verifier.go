...
func (tv *TreeStatusVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if treeStatus.GeneralState == tree_status_types.OpenState {
		return types.VerifierSuccessState, fmt.Sprintf("%s Tree is open: \"%s\"", tv.Name(), treeStatus.Message), nil
	} else {
		return types.VerifierWaitingState, fmt.Sprintf("%s Waiting for tree to be open. Tree is currently in %s state: \"%s\"", tv.Name(), treeStatus.GeneralState, treeStatus.Message), nil
	}
}
