...
func NewTreeStatusVerifier(httpClient *http.Client, treeStatusURL string, footersMap map[string]string) (Verifier, error) {
...
func (tv *TreeStatusVerifier) Verify(ctx context.Context, ci *gerrit.ChangeInfo, startTime int64) (state types.VerifierState, reason string, err error) {
	...
	if val, ok := tv.footersMap[string(footers.NoTreeChecksFooter)]; ok {
		b, err := strconv.ParseBool(val)
		if err != nil {
			sklog.Errorf("Could not parse bool out of %s for %s", val, footers.NoTreeChecksFooter)
		} else {
			if b {
				return types.VerifierSuccessState, fmt.Sprintf("Tree check is skipped because %s: %t has been specified", footers.NoTreeChecksFooter, b), nil
			}
		}
	}
