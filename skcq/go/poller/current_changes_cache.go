...
// It returns the cqStartTime.
func AddToChangesCache(ctx context.Context, changePatchset, changeSubject, changeOwner string, dbClient *db.FirestoreDB, dryRun, internal bool, changeID int64) int64 {
	cqStartTime := time.Now().Unix()
	// Add to the changes cache if it is already not there.
	cqRecord, ok := CurrentChangesCache[changePatchset]
	if !ok {
		...
		CurrentChangesCache[changePatchset] = cqRecord
...
func RemoveFromChangesCache(ctx context.Context, changeID string, runCleanup bool, dbClient *db.FirestoreDB) {
	if _, ok := CurrentChangesCache[changeID]; ok {
		if runCleanup {
			// cq Record
