...
func Start(ctx context.Context, pollInterval time.Duration, cr codereview.CodeReview, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth, publicFEInstanceURL, corpFEInstanceURL string) error {
	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			if newCQRun {
				...
				comment := fmt.Sprintf("SkCQ is trying the patch.\n\nFollow status at: %s/%d/%d", feURL, ci.Issue, cr.GetLatestPatchSetID(ci))
				if cr.IsDryRun(ctx, ci) {
					comment = fmt.Sprintf("Dry run: %s", comment)
				} else if len(togetherChanges) > 0 {
					// comment = fmt.Sprintf("\n\n")
				}
				cr.AddComment(ctx, ci, comment)
			}
			// HER EHERE.
					...
					if ci.WorkInProgress {
						...
						for _, r := range ci.Reviewers.Reviewer {
							...
							if r.AccountID != ci.Owner.AccountID {
								fmt.Println("THIS REVIWEER IS NOT AN OWNER")
								fmt.Println(r.Email)
