...
// and runs them.
func Start(ctx context.Context, pollInterval time.Duration, cr codereview.CodeReview, currentChangesCache caches.CurrentChangesCache, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth, publicFEInstanceURL, corpFEInstanceURL string, reposAllowList, reposBlockList []string) error {
	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, incompleteCI := range cls {
			processCL(ctx, incompleteCI, clsInThisRound, cr, currentChangesCache, httpClient, dbClient, canModifyCfgsOnTheFly, publicFEInstanceURL, corpFEInstanceURL, reposAllowList, reposBlockList)
		}

		// Find CLs that were processed in the last cycle but not the current one.
		// These CLs might still be running, run cleanup on their verifiers.
		for changeEquivalentPatchset, cqRecord := range currentChangesCache.Get() {
			if _, ok := clsInThisRound[changeEquivalentPatchset]; !ok {
				...
				if err := currentChangesCache.Remove(ctx, changeEquivalentPatchset); err != nil {
					sklog.Errorf("[%d] could not update the currentChangesCache: %s", cqRecord.ChangeID, err)
				}

				// Update the attempt as being abandoned so that the UI accurately reflects what happened to
				// the change.
				if err := dbClient.UpdateChangeAttemptAsAbandoned(ctx, cqRecord.ChangeID, cqRecord.EquivalentPatchsetID, cqRecord.Internal, cqRecord.StartTime); err != nil {
				...
				// Get all verifiers.
				vm := &verifiers.SkCQVerifiersManager{}
				verifiers, _, err := vm.GetVerifiers(ctx, httpClient, skCQCfg, cr, ci, false, configReader)
				if err != nil {
					sklog.Errorf("Could not get verifiers to cleanup for %d: %s", ci.Issue, err)
					continue
				}
				...
				if err != nil {
					sklog.Errorf("Could not parse patchsetID from %s: %s", tokens[1], err)
					continue
				}
...
func processCL(ctx context.Context, incompleteCI *gerrit.ChangeInfo, clsInThisRound map[string]bool, cr codereview.CodeReview, currentChangesCache caches.CurrentChangesCache, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth, publicFEInstanceURL, corpFEInstanceURL string, reposAllowList, reposBlockList []string) {
	...
	// Gather all verifiers that will be used.
	vm := &verifiers.SkCQVerifiersManager{}
	clVerifiers, togetherChanges, err := vm.GetVerifiers(ctx, httpClient, skCQCfg, cr, ci, false /* isSubmittedTogetherChange */, configReader)
	if err != nil {
