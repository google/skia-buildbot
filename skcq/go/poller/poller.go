	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			if cr.IsCQ(ctx, ci) {
				...
				// THIS SHOULD NOT BE THE ONWER. THIS SHOULD BE THE PERSON WHO VOTED ON THE CQ+2.
				...
				approvedVerifier, err := verifiers.NewApprovedVerifier(httpClient, cfg.CommitterList)
				if err != nil {
					sklog.Errorf("Error when creating ApprovedVerifier: %s", err)
					cr.RemoveFromCQ(ctx, ci, "Error when creating ApprovedVerifier. Removing from CQ. Please ask Infra Gardener to investigate.")
					continue
				}
			...
			if cr.IsDryRun(ctx, ci) {
				...
				// THIS SHOULD NOT BE THE OWNER. THIS SHOULD BE THE PERSON WHO VOTED ON THE DRY RUN (CQ+1)!!!!!
			...
			if cfg.TreeStatusURL != "" {
				...
				if len(rejectMsgsFromVerifiers) > 0 {
					...
					if cr.IsDryRun(ctx, ci) {
						...
						// FIGURE OUT HOW TO GO THROUGH THE TOGheTE CHANGES AND CHECK THINGS!!!
