	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for changeEquivalentPatchset, cqRecord := range currentChangesCache.Get() {
			if _, ok := clsInThisRound[changeEquivalentPatchset]; !ok {
				...
				if err := cleanupCL(ctx, changeEquivalentPatchset, currentChangesCache, dbClient, cqRecord, ci, configReader, cr, httpClient); err != nil {
...
func cleanupCL(ctx context.Context, changeEquivalentPatchset string, currentChangesCache caches.CurrentChangesCache, dbClient db.DB, cqRecord *types.CurrentlyProcessingChange, ci *gerrit.ChangeInfo, configReader config.ConfigReader, cr codereview.CodeReview, httpClient *http.Client) error {
	...
	// the change.
	if err := dbClient.UpdateChangeAttemptAsAbandoned(ctx, cqRecord.ChangeID, cqRecord.LatestPatchsetID, cqRecord.Internal, cqRecord.StartTs); err != nil {
