...
func Start(ctx context.Context, pollInterval time.Duration, cr codereview.CodeReview, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth) error {
	...
	currentChangesData, err := dbClient.GetCurrentChanges(ctx)
	if err != nil {
		return skerr.Fmt("Could not get current changes: %s", err)
	}
	fmt.Println("CURRNE TCHNAGES DATA?")
	fmt.Println(currentChangesData)
	if currentChangesData == nil {
		sklog.Info("This is the first time this instance is coming up. Creating a new cache of changesToCQRecords")
		ChangeAndPatchsetToCQRecord = map[string]*CQRecord{}
	} else {
		// rv := new(map[string]*CQRecord)
		sklog.Info("Loading the changesToCQRecords cache from the DB")
		if err := gob.NewDecoder(bytes.NewReader(currentChangesData.Data)).Decode(&ChangeAndPatchsetToCQRecord); err != nil {
			return skerr.Fmt("Could not decode the changes cache: %s", err)
		}
		// ChangeAndPatchsetToCQRecord = rv
	}
	fmt.Println("WE GOT THEDSE CHANGES: ")
	fmt.Printf("%+v", currentChangesData)
	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			addToChangesCache(ctx, changePatchset, ci, clVerifiers, dbClient, cqStartTime)

			// Now run the verifiers.
			successMsgsFromVerifiers, waitMsgsFromVerifiers, rejectMsgsFromVerifiers := verifiers.RunVerifiers(ctx, ci, clVerifiers, cqStartTime)
...
func addToChangesCache(ctx context.Context, changePatchset string, ci *gerrit.ChangeInfo, changeVerifiers []verifiers.Verifier, dbClient *db.FirestoreDB, cqStartTime int64) {
	// Update the cache if it is not already in there.
...
func removeFromChangesCache(ctx context.Context, changePatchset string, runCleanup bool, dbClient *db.FirestoreDB) {
...
func updateDB(ctx context.Context, dbClient *db.FirestoreDB) {
	...
	if err := gob.NewEncoder(buf).Encode(ChangeAndPatchsetToCQRecord); err != nil {
