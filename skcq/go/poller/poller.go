	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for changePatchset, _ := range CurrentChangesCache {
			...
			if _, ok := clsInThisRound[changePatchset]; !ok {
				...
				// TODO(rmistry): SEE BELOW for when new patchsets are uploaded.
				// Update the change in dbClient, we need to mark it as being done.
				// Say because a CODE_CHANGE patchset was uploaded.

				// NEED CHANGE ID!!!!
				// Mark the change attempt has done with an error message from somewhere!!!
				// dbClient.GetChangeAttempt(ctx, changeID, internal)
			}
		}
