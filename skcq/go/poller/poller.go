	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			for _, v := range clVerifiers {
				waitMsg, rejectMsg, err := v.Verify(ci)
				fmt.Println("DEBUGGING")
				fmt.Println(v.Name())
				fmt.Println("X" + waitMsg + "X")
				fmt.Println("X" + rejectMsg + "X")
				fmt.Println(err)
				if err != nil {
					waitMsg = fmt.Sprintf("%s: Hopefully a transient error: %s", v.Name(), err)
					waitMsgsFromVerfiers = append(waitMsgsFromVerfiers, waitMsg)
				} else if waitMsg != "" {
					fmt.Println("VERIFIER WAIT MSG")
					waitMsg = fmt.Sprintf("%s: %s", v.Name(), waitMsg)
					waitMsgsFromVerfiers = append(waitMsgsFromVerfiers, waitMsg)
				} else if rejectMsg != "" {
					// The verifier failed.
					fmt.Println("VERIFIER FAILED")
					rejectMsg = fmt.Sprintf("%s: %s", v.Name(), rejectMsg)
					rejectMsgsFromVerifiers = append(rejectMsgsFromVerifiers, rejectMsg)
				} else {
					fmt.Println("VERIFIER SUCCESS")
					// Succesful. Now what? nothing.
				}
			}
