...
func Start(ctx context.Context, pollInterval time.Duration, cr codereview.CodeReview, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth, publicFEInstanceURL, corpFEInstanceURL string, reposAllowList, reposBlockList []string) error {
	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, incompleteCI := range cls {
			...
			// Make sure things have not changed about this issue???
			...
			cqStartTime, newCQRun := caches.AddToChangesCache(ctx, changeEquivalentPatchset, ci.Subject, ci.Owner.Email, ci.Project, ci.Branch, dbClient, cr.IsDryRun(ctx, ci), skCQCfg.Internal, ci.Issue, cr.GetLatestPatchSetID(ci))
			cqEndTime := int64(0)
			...
			if len(rejectMsgsFromVerifiers) > 0 {
				...
				caches.RemoveFromChangesCache(ctx, changeEquivalentPatchset, dbClient)
		...
		for changeEquivalentPatchset, cqRecord := range caches.GetCurrentChangesCache() {
			...
			if _, ok := clsInThisRound[changeEquivalentPatchset]; !ok {
				...
				vm := &verifiers.SkCQVerifiersManager{}
				verifiers, _, err := vm.GetVerifiers(ctx, httpClient, skCQCfg, cr, ci, false /* isSubmittedTogetherChange */, configReader)
				if err != nil {
