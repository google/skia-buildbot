...
func Start(ctx context.Context, pollInterval time.Duration, cr codereview.CodeReview, httpClient *http.Client, dbClient *db.FirestoreDB, canModifyCfgsOnTheFly *allowed.AllowedFromChromeInfraAuth) error {
	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			cqStartTime := AddToChangesCache(ctx, changePatchset, ci.Subject, ci.Owner.Email, dbClient, cr.IsDryRun(ctx, ci), skCQCfg.Internal, ci.Issue)
			cqEndTime := int64(0)
		...
		for changePatchset, cqRecord := range CurrentChangesCache {
			...
			if _, ok := clsInThisRound[changePatchset]; !ok {
				...
				dbClient.UpdateChangeAttemptAsAbandoned(ctx, strconv.FormatInt(cqRecord.ID, 10), cqRecord.Internal, cqRecord.StartTime)
				if err := dbClient.UpdateChangeAttemptAsAbandoned(ctx, strconv.FormatInt(cqRecord.ID, 10), cqRecord.Internal, cqRecord.StartTime); err != nil {
					sklog.Errorf("Could not mark change %s as abandoned", changePatchset)
				}
			}
		}
