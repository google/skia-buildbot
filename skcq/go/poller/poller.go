	...
	cleanup.Repeat(pollInterval, func(ctx context.Context) {
		...
		for _, ci := range cls {
			...
			if cachedCfg, ok := ProjectsBranchesConfigCache[repoBranch]; ok {
				...
				sklog.Info("Caching SkCQ cfg for %s: %+v", repoBranch, cfg)
			...
			clVerifiers, err := verifiers.GetVerifiers(ctx, httpClient, cfg, cr, ci, false /* isSubmittedTogetherChange */)
			if err != nil {
			...
			sklog.Infof("[%d] uses verifiers: %s", strings.Join(verifierNames, ", "))
