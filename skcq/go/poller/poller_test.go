...
func TestCleanupCL(t *testing.T) {
	...
	// Mock db.
	dbClient := &db_mocks.DB{}
	dbClient.On("UpdateChangeAttemptAsAbandoned", testutils.AnyContext, changeID, latestPatchsetID, false, startTime).Return(nil).Once()
	...
	v1.On("Cleanup", testutils.AnyContext, ci, int64(5))
	v2 := &types_mocks.Verifier{}
	v2.On("Cleanup", testutils.AnyContext, ci, int64(5))

	// Mock verifier manager.
	vm := &types_mocks.VerifiersManager{}
	vm.On("GetVerifiers", testutils.AnyContext, httpClient, skcfg, cr, ci, false, cfgReader).Return([]types.Verifier{v1, v2}, []string{}, nil).Once()

	cleanupCL(context.Background(), changePatchsetID, cc, dbClient)
}
