...
func TestThrottler(t *testing.T) {
	...
	require.False(t, Throttle(testRepo1, time.Unix(epochTestTime, 0).UTC()))
	require.False(t, Throttle(testRepo2, time.Unix(epochTestTime, 0).UTC()))

	// Add 1 commit to repo1 made 50s ago.
	UpdateThrottler(testRepo1, time.Unix(epochTestTime-50, 0).UTC(), throttlerCfgRepo1)
	// repo1 should have no throttling.
	require.False(t, Throttle(testRepo1, time.Unix(epochTestTime, 0).UTC()))

	// Add 2nd commit to repo1 made 20s ago.
	UpdateThrottler(testRepo1, time.Unix(epochTestTime-20, 0).UTC(), throttlerCfgRepo1)
	// repo1 should be throttled because there have been 2 commits in the last 100s.
	require.True(t, Throttle(testRepo1, time.Unix(epochTestTime, 0).UTC()))
