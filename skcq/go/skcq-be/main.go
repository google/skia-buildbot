...
var (
	...
	// Useful for debugging k8s pod.
	hang = flag.Bool("hang", false, "Hangs the backend.")
)
...
func main() {
	...
	ts, err := auth.NewDefaultTokenSource(*baseapp.Local, datastore.ScopeDatastore, auth.SCOPE_USERINFO_EMAIL, auth.SCOPE_GERRIT, auth.SCOPE_READ_WRITE, datastore.ScopeDatastore, compute.CloudPlatformScope, datastore.ScopeDatastore)
	if err != nil {
	...
	// Instantiate the current changes cache so that if the server was restarted
	...
	// TODO(rmistry): Have to pass this in everywhere.
	cfgModifyAllowed, err := allowed.NewAllowedFromChromeInfraAuth(criaClient, *canModifyCfgsOnTheFly)
	if err != nil {
		sklog.Fatal(err)
	}
	if err := poller.Start(ctx, *pollInterval, g, currentChangesCache, httpClient, dbClient, cfgModifyAllowed, *publicFEInstanceURL, *corpFEInstanceURL, *reposAllowList, *reposBlockList); err != nil {
