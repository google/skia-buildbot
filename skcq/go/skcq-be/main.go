...
func main() {
	...
	// cfgModifyAllowed, err := allowed.NewAllowedFromChromeInfraAuth(httpClient, "project-skia-tryjob-access")
	cfgModifyAllowed, err := allowed.NewAllowedFromChromeInfraAuth(httpClient, *canModifyCfgsOnTheFly)
	if err != nil {
		sklog.Fatal(err)
	}
	if err := poller.Start(ctx, *pollInterval, g, httpClient, cfgModifyAllowed); err != nil {
