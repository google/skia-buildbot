...
func (srv *Server) AddHandlers(r *mux.Router) {
	...
	r.HandleFunc("/verifiers_detail/{id:[0-9]+}", srv.verifiersDetailHandler).Methods("POST")
...
func (srv *Server) getChangeAttemptsHandler(w http.ResponseWriter, r *http.Request) {
	...
	changeAttempts, err := srv.dbClient.GetChangeAttempts(r.Context(), changeAttemptsRequest.ChangeID, *internal)
	if err != nil {
...
func (srv *Server) getCurrentChangesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to get current chnages", http.StatusInternalServerError)
		return
	}

	respChanges := []*types.CQRecord{}
