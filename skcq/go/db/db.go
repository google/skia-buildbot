...
type DB interface {
	...
	// PutCurrentChanges returns the changes that were processed by the last iteration.
	...
	// change+patchset.
	GetChangeAttempts(ctx context.Context, changeID, patchsetID int64, internal bool) (*types.ChangeAttempts, error)

	// UpdateChangeAttemptAsAbandoned marks the specified change attempt as
	// abandoned.
	UpdateChangeAttemptAsAbandoned(ctx context.Context, changeID, patchsetID int64, internal bool, patchStart int64) error

	// PutChangeAttempts adds the specified change attempt to the DB.
	PutChangeAttempt(ctx context.Context, newChangeAttempt *types.ChangeAttempt, internal bool) error
}
