...
func (f *FirestoreDB) GetChangeAttempts(ctx context.Context, changeID string, internal bool) (*types.ChangeAttempts, error) {
...
func (f *FirestoreDB) UpdateChangeAttemptAsAbandoned(ctx context.Context, changeID string, internal bool, patchStart int64) error {
	changeAttempts, err := f.GetChangeAttempts(ctx, changeID, internal)
	if err != nil {
		return skerr.Fmt("Error getting change attempts of %s from DB: %s", changeID, err)
	}
	if changeAttempts == nil || len(changeAttempts.Attempts) == 0 {
		return nil
	}
	for _, ca := range changeAttempts.Attempts {
		if ca.PatchStart == patchStart {
			...
			ca.CQAbandoned = true
			col := f.client.Collection(getChangesColName(internal))
			if _, err := f.client.Set(ctx, col.Doc(changeID), changeAttempts, defaultAttempts, putSingleTimeout); err != nil {
				return skerr.Fmt("Could not set ChangeAttempts: %s", err)
			}
			return nil
		}
	}
	return skerr.Fmt("Could not find ChangeAttempt with ID %s and patch start of %d", changeID, patchStart)
}

func (f *FirestoreDB) PutChangeAttempt(ctx context.Context, changeAttempt *types.ChangeAttempt, internal bool) error {
	changeAttempts, err := f.GetChangeAttempts(ctx, changeAttempt.ID, internal)
	if err != nil {
