...
func (f *FirestoreDB) PutChangeAttempt(ctx context.Context, changeAttempt *types.ChangeAttempt, internal bool) error {
	changeAttempts, err := f.GetChangeAttempts(ctx, changeAttempt.ChangeID, changeAttempt.PatchsetID, internal)
	if err != nil {
		return skerr.Fmt("Error getting change attempts of %d/%d from DB: %s", changeAttempt.ChangeID, changeAttempt.PatchsetID, err)
	}
	...
	if changeAttempts == nil || len(changeAttempts.Attempts) == 0 {
		...
		exists := false
		for index, ca := range changeAttempts.Attempts {
			if ca.PatchStart == changeAttempt.PatchStart {
				exists = true
				// If it exists then we have to replace the change Attempt.
				fmt.Println("REPLACE IT!!!!!!!")
				changeAttempts.Attempts[index] = changeAttempt
				break
			}
		}
		if !exists {
	...
	// Commit to DB.
	col := f.client.Collection(getChangesColName(internal))
	docName := getChangeAttemptsDocName(changeAttempt.ChangeID, changeAttempt.PatchsetID)
