...
func NewGitilesConfigReader(ctx context.Context, gitilesRepo *gitiles.Repo, repo, branch, gerritChangeRef, tasksJSONPath string) {
...
func GetTasksCfg(ctx context.Context, gitilesRepo *gitiles.Repo, repo, branch, gerritChangeRef, tasksJSONPath string) (*specs.TasksCfg, error) {
	// If list of changed files includes tasksJSON then read from gerritChangeRef, ELSE READ from branch!
	contents, err := gitilesRepo.ReadFileAtRef(ctx, tasksJSONPath, ref)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("%s not found for %s/%s", tasksJSONPath, repo, ref)
		}
