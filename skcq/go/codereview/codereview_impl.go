...
func (gc *gerritCodeReview) AddComment(ctx context.Context, ci *gerrit.ChangeInfo, comment string, notify NotifyOption) error {
	...
	addCommentFunc := func() error {
		return gc.gerritClient.SetReview(ctx, ci, comment, map[string]int{}, []string{}, notifyOption, notifyDetails, AutogeneratedCommentTag, 0, nil)
	}
...
func (gc *gerritCodeReview) RemoveFromCQ(ctx context.Context, ci *gerrit.ChangeInfo, reason string) {
	...
	if err := gc.AddComment(ctx, ci, reason, NotifyOwnerTriggerers); err != nil {
