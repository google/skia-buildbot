...
// Interface to abstract out communications with the gerrit client for SkCQ callers.
type CodeReview interface {
	// Returns change infos that are incomplete. Call GetIssueProperties to populate them.
	...
	AddComment(ctx context.Context, ci *gerrit.ChangeInfo, comment string, notify gerrit.NotifyOption)
...
func (gc *gerritCodeReview) AddComment(ctx context.Context, ci *gerrit.ChangeInfo, comment string, notify gerrit.NotifyOption) {
	if err := gc.gerritClient.SetReview(ctx, ci, comment, map[string]int{}, []string{}, notify, "autogenerated:skcq", 0); err != nil {
...
func (gc *gerritCodeReview) RemoveFromCQ(ctx context.Context, ci *gerrit.ChangeInfo, reason string) {
	...
	gc.AddComment(ctx, ci, reason, gerrit.NotifyOwner)
	return
}
