...
`;
  ...
  private displayChanges(): TemplateResult[]|TemplateResult {
    return this.changes.map((change) => html`
      <tr>
        <td><a href="http://skia-review.googlesource.com/c/${change?.change_id}/${change?.equivalent_patchset_id}" target=_blank>${change?.change_id}/${change?.equivalent_patchset_id}</a></td>
        ...
        <td><a href="/verifiers_detail/${change?.change_id}/${change?.equivalent_patchset_id}">Verfiers Details<a></td>
