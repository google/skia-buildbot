...
`;
  ...
  private displayAttempts(): TemplateResult[] {
    ...
    return this.changeAttempts!.attempts.reverse().map((attempt, index) => html`
      <h4>Attempt #${this.changeAttempts!.attempts!.length - index} ${this.getDryRunText(attempt!)}</h4>
      Start Time: ${new Date(attempt!.created * 1000).toLocaleString()}<br/>
      ${this.getTotalDurationText(attempt!)}
  ...
  private displayVerifiers(attempt: ChangeAttempt): TemplateResult[] {
    if (!attempt.verifiers_status || !attempt.verifiers_status.length) {
      return [];
    }
    return attempt.verifiers_status.map((verifier) => html`
      <br/>
      ${verifier?.name} (State: ${verifier?.state})<br/>
        ${this.getVerificationDurationText(verifier!)}
        ${verifier?.reason}
      <br/>
    `);
  }

  private getVerificationDurationText(verifier: VerifierStatus): TemplateResult {
    let endTime = Date.now();
    let additionalInfo = '(still running)';
    if (verifier.stop) {
      endTime = verifier.stop * 1000;
      additionalInfo = '';
    }
    return html`Total duration: ${diffDate(verifier?.start as number * 1000, endTime)} ${additionalInfo}<br/>`;
  }
  ...
  private displaySubmittableChanges(attempt: ChangeAttempt): TemplateResult {
    if (!attempt.dry_run && attempt.submittable_changes && attempt.submittable_changes.length > 0) {
      return html`Changes submitted together:
      <ul>
  ...
  private getTotalDurationText(attempt: ChangeAttempt): TemplateResult {
    ...
    return html`Total duration: ${diffDate(attempt?.created as number * 1000, endTime)} ${additionalInfo}<br/>`;
  }

  private getCommittedText(attempt: ChangeAttempt): TemplateResult {
    if (attempt.committed) {
      const d = new Date(attempt.committed * 1000);
      return html`Patch committed at ${d}<br/>`;
    }
    return html``;
  }
