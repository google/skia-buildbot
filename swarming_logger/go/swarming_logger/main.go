...
func splitOutput(taskId, output string) ([]*logging.LogEntry, []int, error) {
	...
	size, err := entrySize(&baseEntry)
	if err != nil {
		return nil, nil, err
	}
	maxLineLength := MAX_BYTES_ENTRY - size
	...
	for i, line := range strs {
		...
		sizes = append(sizes, size+len(line)) // Roughly, anyway.
	}
	return rv, sizes, nil
}
...
func (tl *taskLogger) HandleSwarmingPubSub(taskId string) bool {
	...
	entries, sizes, err := splitOutput(taskId, output.Output)
	if err != nil {
