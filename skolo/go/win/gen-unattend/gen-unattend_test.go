...
func TestParseExampleYaml(t *testing.T) {
	...
	parseYamlOrDie(filepath.Join(filepath.Dir(filename), "example-devices.yaml"), "", &devices)
	...
	parseYamlOrDie(filepath.Join(filepath.Dir(filename), "example-vars.yaml"), "", &globalVars)
