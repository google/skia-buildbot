...
func TestParseExampleYaml(t *testing.T) {
	...
	yamlutil.MustParseYamlFile(filepath.Join(filepath.Dir(filename), "example-devices.yaml"), "", &devices)
	...
	yamlutil.MustParseYamlFile(filepath.Join(filepath.Dir(filename), "example-vars.yaml"), "", &globalVars)
