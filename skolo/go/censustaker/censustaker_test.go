...
func TestMakeList_Success(t *testing.T) {
	...
	out, err := makeList(ctx, fakeAddress, fakeUser, fakePassword)
	require.NoError(t, err)

	// The following machines have a mac address that appears in both the arpOutput as well as
	// the edge switch output.
	assert.Equal(t, `       Hostname | Port
=======================
   skia-rpi-007 | 7
   skia-rpi-011 | 11
   skia-rpi-014 | 14
...
skia-rpi-052             ether   b8:27:eb:4f:5f:60   C                     eno1
...
1        B8:27:EB:00:B5:69   0/20                   20       Learned
...
1        B8:27:EB:10:BA:B5   0/45                   45       Learned
