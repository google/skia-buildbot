...
func makeList(ctx context.Context, address, user, password string, hostnameMatcher *regexp.Regexp) (powercycle.EdgeSwitchConfig, error) {
...
func main() {
	...
	out, err := makeList(ctx, *switchAddress, *switchUser, *switchPassword, regexp.MustCompile(*hostnameRegex))
	if err != nil {
		fmt.Printf("Error: %s\n", err)
		os.Exit(1)
	}
