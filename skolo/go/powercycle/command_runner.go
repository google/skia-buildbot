...
// standardInRunner implements the CommandRunner interface by sending commands through standard in
...
func (s *standardInRunner) ExecCmds(ctx context.Context, cmds ...string) (string, error) {
	...
	// start the command before sending to stdin just in case we try to send more data to standard in
	// than it can take (~4k).
