...
import (
	...
	"go.skia.org/infra/skolo/go/meta_util"
...
func main() {
	...
	tokens := make(map[string]*meta_util.ServiceAccountToken, len(serviceAccounts))
	...
	for _, acct := range serviceAccounts {
		...
		for _, ipAddr := range ipAddrs {
			...
			if _, ok := tokens[tokenFile]; !ok {
				tok, err := meta_util.NewServiceAccountToken(tokenFile, false)
				if err != nil {
					sklog.Fatal(err)
				}
	...
	tokenMapping := make(map[string]meta_util.TokenProvider, len(tokenIpMapping))
	for ipAddr, tokenFile := range tokenIpMapping {
		tokenMapping[ipAddr] = tokens[tokenFile]
	}

	r := mux.NewRouter()
	meta_util.SetupServer(r, pm, im, tokenMapping)
