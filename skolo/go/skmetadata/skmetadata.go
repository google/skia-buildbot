...
func makeInstanceMetadataHandler(im InstanceMetadata) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			httputils.ReportError(w, nil, "Metadata key is required.")
		}
		...
		if _, err := w.Write([]byte(val)); err != nil {
			httputils.ReportError(w, nil, "Failed to write response.")
			return
		}
	}
}
...
func SetupServer(r *mux.Router, pm ProjectMetadata, im InstanceMetadata, tokenMapping map[string]*ServiceAccountToken) {
	...
	r.HandleFunc(metadata.TOKEN_PATH, func(w http.ResponseWriter, r *http.Request) {
		...
		if t, ok := tokenMapping[ipAddr]; ok {
			...
			httputils.ReportError(w, fmt.Errorf("Unknown IP address %s and no default token provided.", ipAddr), "Failed to retrieve token.")
			return
		}

		t, err := tok.Get()
		if err != nil {
			httputils.ReportError(w, err, "Failed to obtain key.")
			return
		}
