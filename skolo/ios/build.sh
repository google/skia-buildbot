...
git fetch https://github.com/libimobiledevice/libimobiledevice.git \
  e1f4a869f58f5378c9a107e97945a6003d4d9ae5 && git cherry-pick FETCH_HEAD
./autogen.sh --prefix=$PREFIX --without-cython && make && make install && cd ..

cd ifuse && ./autogen.sh --prefix=$PREFIX && make && make install && cd ..
cd ideviceinstaller && ./autogen.sh --prefix=$PREFIX && make && make install && cd ..
cd usbmuxd && ./autogen.sh --prefix=$PREFIX \
  --with-udevrulesdir=$PREFIX/udev-rules \
  --with-systemdsystemunitdir=$PREFIX/systemd && make && make install && cd ..
