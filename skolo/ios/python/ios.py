...
DEV_IMG_DIR = "./DeviceSupport/"

# Directory where the provisioning profile is.
...
class IOSDevice(object):
    ...
    def _setup_device(self):
        ...
        if kv["ImagePresent"] != "true":
          ...
          devImage, devImageSig = self._get_dev_image()
          kv = _get_kv_pairs(_run_cmd("ideviceimagemounter -u %s %s %s" % (self._id, devImage, devImageSig)))
          assert(kv["Status"] == "Complete")

        # Install the provisioning profile.
        provisioning_profile = self._get_provisioning_profile()
        _run_cmd("ideviceprovision -u %s remove-all" % self._id)
        _run_cmd("ideviceprovision -u %s install %s" % (self._id, provisioning_profile))

    def _get_dev_image(self):
        devImage = " ./DeviceSupport/9.2/DeveloperDiskImage.dmg"
        devImageSig = "./DeviceSupport/9.2/DeveloperDiskImage.dmg.signature"
        return devImage, devImageSig

    def _get_provisioning_profile(self):
        return "./Development.mobileprovision"
