    ...
    if (target && target.parentElement === this) {
      for (let i = 0; i < this.children.length; i++) {
        ...
        if (this.children[i] === target) {
          let idx = binarySearch(this._selection, i);
          if (idx < 0) {
            // Insert at index (-1 - idx), which is how to convert the
            // return value from binarySearch to "index item should go".
            this._selection.splice(-1 - idx, 0, i);
          } else {
            // remove it
