...
# As of 2023-12-01, we started migrating to Bazel 6.0.0, which removed the "managed_directories"
# feature (https://github.com/bazelbuild/bazel/issues/15463). Before this change, the npm_install
# rule used to manage the node_modules directory (e.g. it recreated it upon package.json or
# package-lock.json changes).
#
# Today, the node_modules directory will still be generated by Bazel, but only when running a
# "bazel build" command right after cleaning the cache with "bazel clean --expunge", or when the
# below npm_install rule changes, e.g. by changing an argument. Unfortunately running "npm install"
# or "npm ci" yields errors, which we might be able to fix by updating some NPM packages. lovisolo@
# is working on a solution to bring this to a more reasonable state.
#
