...
func New(ctx context.Context, git *git.Checkout, httpClient *http.Client, taskListProvider failures.TaskListProvider, period time.Duration, modelPeriod time.Duration, gitRepoURL string, flakyBuilder *flaky.FlakyBuilder) (*App, error) {
	a := &App{
		ctx:              ctx,
