...
func New(git *git.Checkout, httpClient *http.Client, taskListProvider failures.TaskListProvider, period time.Duration, modelPeriod time.Duration, gitRepoURL string, flakyBuilder *flaky.FlakyBuilder, gitRepoDir string) (*App, error) {
	a := &App{
...
func (a *App) onestep() error {
	...
	if err := a.failureStore.Update(ctx, a.period); err != nil {
		return err
	}
