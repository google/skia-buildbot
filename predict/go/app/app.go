...
func New(git *git.Checkout, httpClient *http.Client, taskListProvider failures.TaskListProvider, period time.Duration, modelPeriod time.Duration, gitRepoURL string, flakyBuilder *flaky.FlakyBuilder, gitRepoDir string, maxBotDuration time.Duration) (*App, error) {
	a := &App{
...
func (a *App) onestep() error {
	...
	fr, err := a.flakyBuilder.Build(ctx, a.period, time.Now())
	if err != nil {
		return err
	}
	a.mutex.Lock()
	...
	if err := a.failureStore.Update(ctx, a.period+a.maxBotDuration); err != nil {
		return err
	}
