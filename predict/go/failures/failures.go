...
func (f *FailureStore) Failures(begin, end time.Time) (Failures, error) {
	st, err := f.List(begin, end)
	if err != nil {
...
func (f Failures) predictOne(filename string) BotCounts {
	...
	for strings.Contains(filename, "/") {
		sklog.Infof("predictOne: %s", filename)
...
func (f Failures) Predict(filenames []string) []*Summary {
	...
	for _, filename := range filenames {
		sklog.Infof("Predicting: %s", filename)
		for bot, count := range f.predictOne(filename) {
			sklog.Infof("Predicted: %s %#v", bot, count)
	...
	sklog.Infof("Before sort: %#v", ordered)
	sklog.Infof("Sorting")
