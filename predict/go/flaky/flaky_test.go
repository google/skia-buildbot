...
func TestFlakyReadWrite(t *testing.T) {
	...
	err := fb.createOrUpdateFlaky(ctx, BOTNAME, begin, now.Add(-3*time.Hour), true)
	...
	err = fb.createOrUpdateFlaky(ctx, BOTNAME, begin, now.Add(-2*time.Hour), true)
	assert.NoError(t, err)
	...
	// Close the range.
	err = fb.createOrUpdateFlaky(ctx, BOTNAME, begin, now.Add(-2*time.Hour), false)
	assert.NoError(t, err)
	...
	err = fb.createOrUpdateFlaky(ctx, BOTNAME, now.Add(-1*time.Hour), now.Add(-2*time.Minute), true)
	assert.NoError(t, err)
