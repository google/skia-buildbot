...
func (fb *FlakyBuilder) createOrUpdateFlaky(ctx context.Context, botname string, begin, end time.Time, open bool) error {
	begin = begin.UTC()
	end = end.UTC()
...
func (fb *FlakyBuilder) allOpenFlakyBots(ctx context.Context) (map[string]bool, error) {
	ret := map[string]bool{}
...
func (fb *FlakyBuilder) Update(ctx context.Context) error {
	...
	for botname, begin := range flakes {
		...
		err := fb.createOrUpdateFlaky(ctx, botname, begin, now, true)
		if err != nil {
