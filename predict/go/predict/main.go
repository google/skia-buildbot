...
var (
	...
	modelPeriod  = flag.Duration("period", 12*7*24*time.Hour, "The model should be built over all data over the last time period.")
	namespace    = flag.String("namespace", "", "The Cloud Datastore namespace, such as 'perf'.")
	period       = flag.Duration("period", 24*time.Hour, "How often to rebuild the prediction model.")
...
func main() {
	...
	state, err = app.New(ctx, git, httpClient, provider, *period, "https://skia.googlesource.com/skia.git", fb)
	if err != nil {
