...
func predictHandler(w http.ResponseWriter, r *http.Request) {
	...
	issue, err := strconv.ParseInt(r.FormValue("change"), 10, 64)
	if err != nil {
		httputils.ReportError(w, r, err, "Failed to parse change query parameter.")
		return
	}
	files, err := gerritClient.Files(issue, r.FormValue("revision"))
	if err != nil {
...
func main() {
	...
	statusProvider, err := statusprovider.New([]string{common.REPO_SKIA})
	if err != nil {
		sklog.Fatal(err)
	}
	fb := flaky.NewFlakyBuilder(flaky.FlakyProvider(statusProvider.Get))
	state, err = app.New(ctx, git, httpClient, taskListProvider, *period, *modelPeriod, "https://skia.googlesource.com/skia.git", fb, *gitRepoDir)
	if err != nil {
