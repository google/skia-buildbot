...
func simpleTx(ctx context.Context, format, dotCode string) (string, error) {
	return fmt.Sprintf("%s", dotCode), nil
}
...
func Test_server_transformHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			statusCode: 404,
			body:       fmt.Sprintf(reportErrorDotTemplate, "Failed to find requester URL in source document."), // The error will be in svg format.
		},
		{
			...
			statusCode: 404,
			body:       fmt.Sprintf(reportErrorDotTemplate, "Unknown format."),
		},
		{
			...
			statusCode: 404,
			body:       fmt.Sprintf(reportErrorDotTemplate, "Missing Referer header."), // The error will be in svg format.
		},
		{
			...
			statusCode: 404,
			body:       fmt.Sprintf(reportErrorDotTemplate, "Failed to get 200 fetching referring page."), // The error will be in svg format.
		},
	}
