...
func startPoller(githubClient *github.GitHub, gerritClient *gerrit.Gerrit, dbClient *db.FirestoreDB) {
	cleanup.Repeat(*pollInterval, func(ctx context.Context) {
		...
		for _, pr := range pullRequests {
			...
			// TODO(rmistry): Move back down if you do not really use it over here. 
...
`, pr.Head.GetSHA(), gerritURL)
				...
				// Add the author to the CC list because otherwise they will not get emails.

				// In the future maybe add the sheriff to the CC list automatically.
			} else {
...
func main() {
	...
	startPoller(githubSkiaBuildbotClient, gerritClient, dbClient)
	...
	// 	sklog.Fatalf("Could not instantiate github client: %s", err)
	// }
	// startPoller(githubSkiaClient, gerritClient, dbClient)
