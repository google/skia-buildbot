...
func main() {
	...
	cleanup.Repeat(*pollInterval, func(ctx context.Context) {
		...
		for _, pr := range pullRequests {
			...
			// First check to see if we have already completely processed this PR.
			// prData, err := dbClient.GetFromDB(ctx, repo, prNumber)
			// if err != nil {
			// 	sklog.Errorf("Error getting %s/%d from DB: %s", repo, prNumber, err)
			// 	continue
			// }
			// if prData.Commented && (prData.Abandoned || prData.Merged) {
			// 	sklog.Infof("We have ")
			// 	continue
			// }
			...
			if !isCLASigned {
				sklog.Infof("CLA has not been signed yet for %s/%d", repo, prNumber)
				continue
			}
			if gerritURL == "" {
				sklog.Infof("Copybara importer has not run yet on %s/%d", repo, prNumber)
				continue
			}
...
`, pr.Head.GetSHA(), gerritURL)
				...
				sklog.Infof("Added comment to %s/%d", repo, prNumber)
			} else {
				sklog.Infof("Already added comment to %s/%d previously", repo, prNumber)
			}

			// Call the Gerrit API to see if the Gerrit change is not open anymore.
			// gerritClient.GetChange(ctx, gerritURL)
			// Status == CHANGE_STATUS_ABANDONED OR CHANGE_STATUS_MERGED
			...
			fmt.Println("Gerrit change stuff:")
			fmt.Println(gerritChangeInfo.Id)
			fmt.Println(gerritChangeInfo.Owner)
			fmt.Println(gerritChangeInfo.Status)
			fmt.Println(gerritChangeInfo.Messages)
