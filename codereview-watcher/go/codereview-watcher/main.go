...
func startPoller(githubClient *github.GitHub, gerritClient *gerrit.Gerrit, dbClient *db.FirestoreDB) {
	cleanup.Repeat(*pollInterval, func(ctx context.Context) {
		sklog.Info("--------- new round of polling ---------")
...
Please visit %s to see it. Please CC yourself to the Gerrit change.
...
`, pr.Head.GetSHA(), gerritURL)
				...
				// In the future maybe add the sheriff to the Reviewer/CC list automatically here.
			} else {
