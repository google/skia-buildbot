...
// WIll need one for updating - for abandoned and the other thingy as well!
func (f *FirestoreDB) UpdateDB(ctx context.Context, prNumber int, merged, abandoned bool) error {
	return nil
}

// PutInDB puts the specified pull request data into the DB.
func (f *FirestoreDB) PutInDB(ctx context.Context, prNumber int, repo, prURL string, commented bool) error {
	f.mtx.Lock()
	defer f.mtx.Unlock()
	now := time.Now()
	qd := &PullRequestData{
	...
	pullRequestCol := f.client.Collection("PullRequestData")
	docID := fmt.Sprintf("%s_%d", repo, prNumber)
