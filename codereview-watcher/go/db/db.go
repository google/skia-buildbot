...
func (f *FirestoreDB) GetFromDB(ctx context.Context, repo string, prNumber int) (*firestore_api.DocumentSnapshot, error) {
	f.mtx.RLock()
	defer f.mtx.RUnlock()
	docID := f.getPullRequestDocID(repo, prNumber)
	docRef := f.client.Collection("PullRequestData").Doc(docID)
