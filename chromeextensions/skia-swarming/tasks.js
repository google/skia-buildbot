...
function drawExtraTableEntries(jobID, gerritIssue, gerritPatchSet, sourceRepo, sourceRevision, retryOf, parentTaskId, thisTaskId) {
  ...
  if (jobID) {
    overwriteIfDifferent(elem, `<td>Associated Job</td><td colspan="2"><a rel="noopener" target="_blank" href="https://task-scheduler.skia.org/job/20161205T172828.520126401Z_000000000000d128">View at task-scheduler.skia.org</a></td>`);
  } else {
    elem.innerHTML = "";
  }
  ...
  if (gerritIssue && gerritPatchSet) {
    overwriteIfDifferent(elem, `<td>Associated CL</td><td colspan="2"><a rel="noopener" target="_blank" href="https://skia-review.googlesource.com/c/${gerritIssue}/${gerritPatchSet}">View at review.skia.org</a></td>`);
  } else {
    elem.innerHTML = "";
  }
  ...
  if (sourceRepo && sourceRevision) {
    ...
    overwriteIfDifferent(elem, `<td>${key}</td><td colspan="2"><a rel="noopener" target="_blank" href="${link}">View Commit</a></td>`);
  } else {
    elem.innerHTML = "";
  }
  ...
  if (retryOf) {
    overwriteIfDifferent(elem, `<td>Retry of Task</td><td colspan="2"><a rel="noopener" target="_blank" href="https://chromium-swarm.appspot.com/tasklist?c=name&amp;c=state&amp;c=created_ts&amp;c=user&amp;f=sk_id%3A${retryOf}&amp;l=10&amp;s=created_ts%3Adesc">Find in Task List</a></td>`);
  } else {
    elem.innerHTML = "";
  }
  ...
  if (parentTaskId) {
    overwriteIfDifferent(elem, `<td>Parent Task</td><td colspan="2"><a rel="noopener" target="_blank" href="https://chromium-swarm.appspot.com/tasklist?c=name&amp;c=state&amp;c=created_ts&amp;c=user&amp;f=sk_id%3A${parentTaskId}&amp;l=10&amp;s=created_ts%3Adesc">Find in Task List</a></td>`);
  } else {
    elem.innerHTML = "";
  }
...
window.addEventListener("WebComponentsReady", function(e) {
  ...
  window.setInterval(function(){
    ...
    // a map of all properties that begins with sk_ or source_
    var skData = {};
