...
func (p *Processor) process(start, end time.Time) error {
	sklog.Infof("process(%s, %s)", start, end)
	if err := p.ProcessFn(start, end); err != nil {
		return err
	}
	p.lastSuccess = end
	return util.WriteGobFile(p.File, &p.lastSuccess)
}
...
func (p *Processor) run() error {
	now := time.Now()
	if now.Sub(p.lastSuccess) > p.Window {
		// We're not caught up yet. Start at the beginning of time, or
		// the last successful update time.
		return util.IterTimeChunks(p.lastSuccess, now, p.ChunkSize, p.process)
	}
	return p.process(now.Add(-p.Window), now)
}
...
func (p *Processor) Start(ctx context.Context) error {
	...
	p.lastSuccess = p.BeginningOfTime
	if err := util.ReadGobFile(p.File, &p.lastSuccess); err != nil {
		return err
	}
	sklog.Infof("Starting at: %s", p.lastSuccess)
