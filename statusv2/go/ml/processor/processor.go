...
func (p *Processor) run() error {
	now := time.Now()
	if now.Sub(p.processedUpTo) > p.Window {
		// We're not caught up yet. Start at the beginning of time, or
		// the last successful update time.
		return util.IterTimeChunks(p.processedUpTo, now, p.ChunkSize, p.process)
	}
...
func (p *Processor) Start(ctx context.Context) error {
	...
	if err := util.ReadGobFile(p.file, &p.processedUpTo); err != nil {
		return err
	}
