...
func regexFailure(expr string) failureFinder {
	...
	if re.NumSubexp() != 1 {
		sklog.Fatal("Regular expression has != 1 capturing group:\n%s", expr)
	}
...
func (fp *FailureParser) Tick() error {
	...
	if fp.ranOnce {
		...
		// We're not actually loading all of the tasks from the
		// beginning of time; this is limited by the cache window.
