...
// mockResolver enables mocking calls to a CommitResolver.
type mockResolver struct {
	calls [][3]string
}

func (r *mockResolver) Mock(repoURL, ref, returnVal string) {
	r.calls = append(r.calls, [3]string{repoURL, ref, returnVal})
}

// Resolve is a CommitResolver which uses the stored mock calls.
func (r *mockResolver) Resolve(ctx context.Context, repoURL, ref string) (string, error) {
	if len(r.calls) < 1 {
		return "", skerr.Fmt("no mocked calls")
	}
	expectRepoURL := r.calls[0][0]
	expectRef := r.calls[0][1]
	returnVal := r.calls[0][2]
	if repoURL != expectRepoURL || ref != expectRef {
		return "", skerr.Fmt("have mocked call for (%s, %s) but got (%s, %s)", expectRepoURL, expectRef, repoURL, ref)
	}
	r.calls = r.calls[1:]
	return returnVal, nil
}

func setup(t *testing.T) (context.Context, *StageManager, vfs.FS, *mockResolver, *docker_mocks.Client, func()) {
	...
	sm := NewStageManager(ctx, fs, dc, resolver.Resolve)
	return ctx, sm, fs, resolver, dc, func() {
...
func TestStageManager_SetStage_ByDigest(t *testing.T) {
	ctx, sm, fs, _, dc, cleanup := setup(t)
	defer cleanup()
...
func TestStageManager_SetStage_ByGitHash(t *testing.T) {
	ctx, sm, fs, resolver, dc, cleanup := setup(t)
	defer cleanup()

	resolver.Mock("https://my-repo.git", "6e2fb7dbe8533d1497dae1d08b2f855a5b3bd2e8", "6e2fb7dbe8533d1497dae1d08b2f855a5b3bd2e8")
...
func TestStageManager_SetStage_ByShortGitHash(t *testing.T) {
	ctx, sm, fs, resolver, dc, cleanup := setup(t)
	defer cleanup()

	resolver.Mock("https://my-repo.git", "6e2fb7d", "6e2fb7dbe8533d1497dae1d08b2f855a5b3bd2e8")
	dc.On("GetManifest", testutils.AnyContext, "gcr.io", "skia-public/status", "git-6e2fb7dbe8533d1497dae1d08b2f855a5b3bd2e8").Return(&docker.Manifest{
		Digest: "sha256:4a75315fabbfe385da4cdebc9d0db6a742ef8d41319fe3100dd435a6e4bc58c2",
	}, nil)
	require.NoError(t, sm.SetStage(ctx, fakeImage, "prod", "6e2fb7d"))
	assertFileContents(t, ctx, fs, StageFilePath, `{
  "default_git_repo": "https://default-repo.git",
  "images": {
    "gcr.io/skia-public/status": {
