...
func populateRunningTask(newState, botId string, k *datastore.Key, t *Task) error {
	...
	emailThreadId, err := SendStartEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, t.TaskIdForIsolates)
	if err != nil {
		return fmt.Errorf("Error sending start email: %s", err)
	}
	t.EmailThreadId = emailThreadId
...
func taskExpiringSoon(k *datastore.Key, t *Task) error {
	if err := SendWarningEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, t.EmailThreadId); err != nil {
...
func checkForUnexpectedStates(newState string, failure bool, k *datastore.Key, t *Task) error {
	...
	for _, unexpectedState := range unexpectedStates {
		if newState == unexpectedState {
			...
			if err := SendFailureEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, t.SwarmingTaskState, t.EmailThreadId); err != nil {
