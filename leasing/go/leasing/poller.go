...
func populateRunningTask(newState, botId string, k *datastore.Key, t *Task) error {
	...
	threadingData, err := SendStartEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, t.TaskIdForIsolates)
	if err != nil {
		return fmt.Errorf("Error sending start email: %s", err)
	}
	// Store threadingData in datastore for threading followup emails.
	t.ThreadingData = *threadingData
...
func taskExpiringSoon(k *datastore.Key, t *Task) error {
	if err := SendWarningEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, &t.ThreadingData); err != nil {
...
func checkForUnexpectedStates(newState string, failure bool, k *datastore.Key, t *Task) error {
	...
	for _, unexpectedState := range unexpectedStates {
		if newState == unexpectedState {
			...
			if err := SendFailureEmail(t.Requester, t.SwarmingServer, t.SwarmingTaskId, t.SwarmingBotId, t.SwarmingTaskState, &t.ThreadingData); err != nil {
