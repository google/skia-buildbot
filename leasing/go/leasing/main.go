...
func swarmingTaskStateChanged(previousState, newState string, t *Task) error {
	...
	if previousState == swarming.TASK_STATE_PENDING {
...
// Complete this with the below function!
func checkLeaseTasksStarted() error {

	pendingTasks := []*Task{}
	q := ds.NewQuery(TASK).EventualConsistency().Filter("SwarmingBotId =", "").Filter("Done =", false)
	...
	for {
		...
		fmt.Println("FOUND THIS IN CHECK LEASE TASKS STARTED:")
		fmt.Println(k.ID)
		// Call the API using the swarmingTask
		swarmingTask, err := swarmingClient.GetTask(t.SwarmingTaskId, false)
		if err != nil {
			return fmt.Errorf("Failed to retrieve swarming task %s: %s", t.SwarmingTaskId, err)
		}
		fmt.Println("THIS IS THE STATE")
		fmt.Println(swarmingTask.State)
		if swarmingTask.State != swarming.TASK_STATE_PENDING {
			if t.SwarmingTaskState == swarming.TASK_STATE_PENDING {
				// If the previous state was pending then we should now be able to populate the task struct.
				// Update the state and add the bot name
				t.SwarmingTaskState = swarmingTask.State
				t.SwarmingBotId = swarmingTask.BotId
			...
			if swarmingTask.State == swarming.TASK_STATE_RUNNING && !t.WarningSent {
				// Check to see if a 15 min warning email should be sent.
				// TODO(Rmistry): Will need a warned flag that is reset when things are extended!
				if t.LeaseEndTime.Before(time.Now().Add(time.Minute * 15)) {
					// Inform the requester that there are only about 15 mins to go.
			...
			for _, unexpectedState := range unexpectedStates {
				if swarmingTask.State == unexpectedState {
					// Update the state.
					t.SwarmingTaskState = swarmingTask.State
			...
			// If the task state is TASK_STATE_RUNNING, then everything looks good.
			// We should check for 15 mins thingy if it is running. to make sure it is not going to expire soonish
		...
		//	sklog.Infof("Task %d requested by %s has expired", k.ID, t.Requester)
		//	t.Done = true
		//}
	}
	...
	//if err:=SendStartEmail(task.Requester, task.SwarmingServer, task.SwarmingId, task.SwarmingBotId string); err != nil {
	//	httputils.ReportError(w, r, err, "Error sending start email")
	//	return
	//}
}
...
// TODO(rmistry): Also check for the swarming tasks of the running stuff to see if is stops running or not.
...
func addTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(rmistry): Temp
	task.Requester = "rmistry@google.com"
	// Add the created time and calculate the lease end time.
	task.Created = time.Now()
	// This needs to be done only afte rit is picked up!
	// task.LeaseEndTime = time.Now().Add(time.Hour * time.Duration(durationHrs))
	...
	fmt.Println(task.SwarmingBotId)
	...
	// HERE HER
	extraArgs := []string{
		"--task-id", strconv.Itoa(int(datastoreKey.ID)),
		"--os-type", task.OsType,
	...
	// https://chromium-swarm.appspot.com/task?id=39719a58ab46f610
	// adsf
	task.SwarmingServer = "chromium-swarm.appspot.com" // TODO(rmistry): Figure this out from the Pool.
	task.SwarmingTaskId = resp.TaskId
	task.SwarmingTaskState = swarming.TASK_STATE_PENDING
	if _, err := UpdateTask(datastoreKey, task); err != nil {
		httputils.ReportError(w, r, err, "Error updating task in datastore")
		return
	}
}
...
func main() {
	...
	go func() {
		for range time.Tick(*pollInterval) {
			...
			//}
			if err := checkLeaseTasksStarted(); err != nil {
				sklog.Errorf("Error when checking for expired tasks: %v", err)
			}
		}
	}()
