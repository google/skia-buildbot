...
func (srv *Server) addTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	isolateHash, err := GetIsolateHash(ctx, task.SwarmingPool, swarmingProps.InputsRef.Isolated)
	if err != nil {
	...
	// TriggerSwarmingTask(pool, requester, datastoreId, osType, deviceType, botId, serverURL, isolateHash, relativeCwd string, cipdInput *swarming_api.SwarmingRpcsCipdInput, cmd []string) (string, error) {
