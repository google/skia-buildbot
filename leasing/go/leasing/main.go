...
func myLeasesHandler(w http.ResponseWriter, r *http.Request) {
	...
	tasks, err := getLeasingTasks(login.LoggedInAs(r))
	if err != nil {
		httputils.ReportError(w, r, err, "Failed to expand template")
		return
	}
...
func extendTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	maxPossibleLeaseEndTime := t.Created.Add(time.Hour * time.Duration(MAX_LEASE_DURATION))
	if newLeaseEndTime.After(maxPossibleLeaseEndTime) {
		httputils.ReportError(w, r, nil, fmt.Sprintf("Can not extend lease beyond %d hours of the task creation time", MAX_LEASE_DURATION))
		return
	}
