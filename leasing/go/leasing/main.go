...
func checkForExpiredTasks() error {
	...
	tasks := []*Task{}
	q := ds.NewQuery(TASK).EventualConsistency().Order("-Created")
	// .Order("-TS")
	fmt.Println(q)
	...
	for k, t := range keyToExpiredTask {
		...
		sklog.Infof("Marked as expired task %v in the datastore with key %s", t, k.ID)
	}

	return nil
}

func GetTask(taskID string) (*datastore.Key, *Task, error) {
	i, err := strconv.ParseInt(taskID, 10, 64)
	if err != nil {
		return nil, nil, fmt.Errorf("Invalid task parameter: %v", err)
	}
	key := ds.NewKey(TASK)
	key.ID = i
