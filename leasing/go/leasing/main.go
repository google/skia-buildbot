...
// TODO(rmistry): Will need a goroutine to loop over all not completed runs to know when they are finished.
...
func statusHandler(w http.ResponseWriter, r *http.Request) {
	...
	datastoreKey := r.FormValue("task")
	if datastoreKey == "" {
		httputils.ReportError(w, r, nil, "Missing task parameter")
		return
	}

	i, err := strconv.ParseInt(datastoreKey, 10, 64)
	if err != nil {
		httputils.ReportError(w, r, err, "Invalid task parameter")
		return
	}
	key := ds.NewKey("Task") // TODO(rmistry): Make constant
	key.ID = i

	task := &Task{}
	if err := ds.DS.Get(context.TODO(), key, task); err != nil {
		httputils.ReportError(w, r, err, "Error retrieving task from Datastore")
		return
	}
...
func runServer(serverURL string) {
	...
	r.HandleFunc("/_/get_task_status", statusHandler) // Should this be POST? rmistry
