...
const (
	...
	GET_SUPPORTED_POOLS_POST_URI = "/_/get_supported_pools"
...
func (srv *Server) AddHandlers(r *mux.Router) {
	...
	r.HandleFunc(GET_TASK_STATUS_URI, srv.statusHandler).Methods("GET")

	// All endpoints that require authentication should be added to this router.
	appRouter := mux.NewRouter()
	appRouter.HandleFunc("/", srv.indexHandler)
	appRouter.HandleFunc(MY_LEASES_URI, srv.myLeasesHandler)
	appRouter.HandleFunc(ALL_LEASES_URI, srv.allLeasesHandler)
	appRouter.HandleFunc(POOL_DETAILS_POST_URI, srv.poolDetailsHandler).Methods("POST")
	appRouter.HandleFunc(GET_SUPPORTED_POOLS_POST_URI, srv.supportedPoolsHandler).Methods("POST")
	appRouter.HandleFunc(GET_LEASES_POST_URI, srv.getLeasesHandler).Methods("POST")
	appRouter.HandleFunc(ADD_TASK_POST_URI, srv.addTaskHandler).Methods("POST")
	appRouter.HandleFunc(EXTEND_TASK_POST_URI, srv.extendTaskHandler).Methods("POST")
	appRouter.HandleFunc(EXPIRE_TASK_POST_URI, srv.expireTaskHandler).Methods("POST")
