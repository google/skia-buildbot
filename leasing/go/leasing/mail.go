...
func SendStartEmail(taskOwner, swarmingServer, swarmingId, swarmingBot string) error {
	...
	bodyTemplate := `
		...
		Thanks!
	`
	body := fmt.Sprintf(bodyTemplate, GetSwarmingTaskLink(swarmingServer, swarmingId), swarmingBot)
...
func SendWarningEmail(taskOwner, swarmingServer, swarmingId string) error {
	...
	bodyTemplate := `
		...
		Thanks!
	`
	body := fmt.Sprintf(bodyTemplate, GetSwarmingTaskLink(swarmingServer, swarmingId))
	if err := gmail.Send(LEASING_EMAIL_DISPLAY_NAME, getRecipients(taskOwner), subject, body); err != nil {
		return fmt.Errorf("Could not send email: %s", err)
	}
	return nil
...
func SendFailureEmail(taskOwner, swarmingServer, swarmingId, swarmingTaskState string) error {
	...
	bodyTemplate := `
		...
		<br/><br/>
		You can reschedule another leasing task <a href="https://leasing.skia.org/">here</a>.
		<br/>
		Contact the CC'ed trooper if you have any questions.
		<br/><br/>
		Thanks!
	`
	body := fmt.Sprintf(bodyTemplate, GetSwarmingTaskLink(swarmingServer, swarmingId), swarmingTaskState)
	if err := gmail.Send(LEASING_EMAIL_DISPLAY_NAME, getRecipients(taskOwner), subject, body); err != nil {
		return fmt.Errorf("Could not send email: %s", err)
	}
	return nil
...
func SendCompletionEmail(taskOwner, swarmingServer, swarmingId string) error {
	...
	bodyTemplate := `
		...
		<br/><br/>
		If needed, you can reschedule more leasing tasks <a href="https://leasing.skia.org/">here</a>.
		<br/><br/>
		Thanks!
	`
	body := fmt.Sprintf(bodyTemplate, GetSwarmingTaskLink(swarmingServer, swarmingId))
