...
def main():
  ...
  if args.skiaserve_gs_path:
    ...
    skiaserve_device_location = '/data/tmp-debugger/skiaserve'
    # Make sure we are running as adb root.
    subprocess.check_call(['adb', 'root'])
    # Setup directory structure on the device.
    print 'Creating %s on device' % os.path.dirname(skiaserve_device_location)
    subprocess.check_call(['adb', 'shell', 'mkdir', '-p',
                           os.path.dirname(skiaserve_device_location)])
    ...
    try:
      ...
      subprocess.check_call(['adb', 'push', tmpFile, skiaserve_device_location])
    finally:
      os.remove(tmpFile)
    # Turn on the debugger and setup adb port forwarding from host to device.
    print 'Bringing up the debugger'
    subprocess.check_call(['adb', 'shell', 'chmod', '+x', skiaserve_device_location])
  ...
  if args.skiaserve_gs_path:
    ...
    proc.terminate()
    # Cleanup the tmp dir.
    subprocess.check_call(['adb', 'shell', 'rm', '-r',
                           os.path.dirname(skiaserve_device_location)])
