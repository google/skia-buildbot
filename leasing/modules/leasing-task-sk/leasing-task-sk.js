...
define('leasing-task-sk', class extends ElementSk {
  ...
  _onExtend() {
    const confirmed = confirm("Proceed with extending leasing task?")
    if (!confirmed) {
      return;
    }
    const detail = {
      'task' : this._task.datastoreId,
      'duration': parseInt($$('#duration', this).value),
    }
    doImpl('/_/extend_leasing_task', detail, (json) => {
      window.location.href = '/my_leases';
    });
  }

  _onExpire() {
    const confirmed = confirm("Proceed with expiring leasing task?")
    if (!confirmed) {
      return;
    }
    const detail = {
    ...
    doImpl('/_/expire_leasing_task', detail, (json) => {
      window.location.href = '/my_leases';
    });
  }
