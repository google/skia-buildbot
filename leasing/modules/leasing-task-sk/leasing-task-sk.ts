...
export class LeasingTaskSk extends ElementSk {
  ...
  <table class="tasktable" cellpadding="5" border="1">
    ...
    <tr class="headers">
       <td colspan=2>
         Task - ${ele._leasingTask.description} - ${ele._leasingTask.requester} - ${displayTaskStatus(ele._leasingTask)}
       </td>
    </tr>

    <tr>
      <td>
        Created: ${formatTimestamp(ele._leasingTask.created)}
      </td>
      <td>
        Lease Start Time: ${displayLeaseStartTime(ele._leasingTask)}
        <br/>
        Lease End Time: ${displayLeaseEndTime(ele._leasingTask)}
      </td>
    </tr>

    <tr>
      <td>
        Pool: ${ele._leasingTask.pool}
        ${displayDimensions(ele._leasingTask)}
        ${displayBotId(ele._leasingTask)}
        ${displaySwarmingTaskForIsolates(ele._leasingTask)}
      </td>
      <td>
        Task Log:${displaySwarmingTask(ele._leasingTask)}
        <br/>
        Task Status: ${ele._leasingTask.swarmingTaskState}
      </td>
    </tr>
...
`;
  ...
  private onExtend(): void {
    ...
    const detail: ExtendTaskRequest = {
      task: this._leasingTask.datastoreId,
  ...
  get task(): Task { return this._leasingTask; }

  set task(val: Task) {
    this._leasingTask = val;
    this._render();
  }
}
