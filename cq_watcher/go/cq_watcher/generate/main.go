...
func main() {
	...
	cqWatcherDir := path.Join(root, "cq_watcher", "go", "cq_watcher")
	dst := path.Join(cqWatcherDir, path.Base(PROTO_FILE_PATH))
	...
	if output, err := exec.RunCwd(cqWatcherDir, "protoc", "--go_out=plugins=grpc:.", dst, "--proto_path", cqWatcherDir); err != nil {
		glog.Fatalf("Error: %s\n\n%s", err, output)
	}
}
