...
const alarmVisibility = (ele, installed) => {
  if (!ele.packageLookup[installed]) {
    return 'invisible'
  } else {
    return this.packageLookup[installed].Latest ? 'invisible' : '';
  }
};
const dirtyVisibility = (ele, installed) => {
  if (!ele.packageLookup[installed]) {
    ...
    return this.packageLookup[installed].Dirty ? 'invisible' : '';
  }
};
...
const servicesOf = (installed) => {
  let p = this.packageLookup[installed];
  if (!p) {
    return [];
  }
  return p.Services;
...
const listServices(ele, server, installed) => servicesOf(installed).map(service => {
  return html`<systemd-unit-status-sk machine$="${server.Name}" value=${service} ></systemd-unit-status-sk>`;
}
...
const listApplications = (ele, server) => server.Installed.map(installed => html`
  ...
  <span class=appName><a href$="https://github.com/google/skia-buildbot/compare/${fullHash(installed)}...HEAD">${short(installed)}</a></span>
  <icon-warning-sk class$="${alarmVisibility(ele, installed)}" title="Out of date."></icon-warning-sk>
  <icon-alarm-sk class$="${dirtyVisibility(installed)}" title="Uncommited changes when the package was built."></icon-alarm-sk>
  <a href$="${logsFullURI(server.Name, installed)}">logs</a>
  <div>
