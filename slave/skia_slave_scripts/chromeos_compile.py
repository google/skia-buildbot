...
class Compile(BuildStep):
  def _Run(self):
    ...
    # TODO(borenet): We can't yet use the new build script due to GS permissions
    # so we just use the generic Linux build for now.
    os.environ['GYP_DEFINES'] = self._args['gyp_defines'] + ' skia_gpu=0'
    print 'GYP_DEFINES="%s"' % os.environ['GYP_DEFINES']
    cmd = ['make', self._args['target'], 'BUILDTYPE=%s' % self._configuration]
