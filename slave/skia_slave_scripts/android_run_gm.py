...
def _PreGM(serial, dir_name):
  ...
  try:
    misc.RunADB(serial, 'shell rm -r %s%s' % (DEVICE_PATH, dir_name))
  except:
    pass
  misc.RunADB(serial, 'shell mkdir -p %s%s' % (DEVICE_PATH, dir_name))
...
def _PostGM(serial, dir_name):
  ...
  misc.RunADB(serial, 'pull %s%s %s' % (DEVICE_PATH, dir_name, gm_actual_dir))
  misc.RunADB(serial, 'shell rm -r %s%s' % (DEVICE_PATH, dir_name))
...
def main(argv):
  ...
  misc.ConfirmOptionsSet({'--device': options.device, '--args': options.args})
  ...
  misc.Run(serial=serial, binary_name=BINARY_NAME, arguments=arguments)
