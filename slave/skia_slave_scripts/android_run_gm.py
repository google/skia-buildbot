...
class AndroidRunGM(AndroidBuildStep, RunGM):
  def _PullImages(self, serial):
    ...
    misc.RunADB(serial, ['pull', '%s/%s' % (self._adb_dirs.GMDir(),
                                            self._gm_image_subdir),
                         self._gm_actual_dir])
    misc.RunADB(serial, ['shell', 'rm', '-r', '%s/%s' % (self._adb_dirs.GMDir(),
                                                         self._gm_image_subdir)
                         ])
