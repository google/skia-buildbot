...
describe('Triage Smoke Test', async () => {
  let browser: Browser;
  let page: Page;
  ...
  it('should load the triage page', async () => {
    await page.goto(`${PERF_BASE_URL}/triage`, { waitUntil: 'networkidle0' });
    try {
      await page.waitForSelector('triage-page-sk', { timeout: 10000 });
    } catch (e) {
      await takeScreenshot(page, 'perf', 'triage-smoke-failure');
      throw e;
    }
  }).timeout(15000);
});
