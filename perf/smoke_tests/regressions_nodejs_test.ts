...
describe('Regressions Smoke Test', async () => {
  let browser: Browser;
  let page: Page;
  ...
  it('should load the regressions page', async () => {
    await page.goto(`${PERF_BASE_URL}/regressions`, { waitUntil: 'networkidle0' });
    try {
      await page.waitForSelector('regressions-page-sk', { timeout: 10000 });
    } catch (e) {
      await takeScreenshot(page, 'perf', 'regressions-smoke-failure');
      throw e;
    }
  }).timeout(15000);
});
