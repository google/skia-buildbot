...
import { messageToErrorString, startRequest } from './progress';
...
describe('messageToErrorString', () => {
  it('converts an empty messages into an error string', () => {
    assert.equal('(no error message available)', messageToErrorString([]));
  });

  it('uses the Error value if available', () => {
    assert.equal('Error: This is my error', messageToErrorString([{ key: 'Error', value: 'This is my error' }]));
  });

  it('falls back to using all the key/value pairs if Error is not present', () => {
    assert.equal('Step: 1/2 Status: Querying database', messageToErrorString([{ key: 'Step', value: '1/2' }, { key: 'Status', value: 'Querying database' }]));
  });
});
