...
const _lowCell = (ele, rowIndex, col, colIndex) => {
  if (col && col.low) {
    return html`<triage-status-sk .alert=${ele._alertAt(colIndex)} .cluster_type=low .full_summary=${_full_summary(col.frame, col.low)} .triage=${col.low_status}></triage-status-sk> `;
  } else {
...
const _highCell = (ele, rowIndex, col, colIndex) => {
  if (col && col.high) {
    return html`<triage-status-sk .alert=${ele._alertAt(colIndex)} .cluster_type=high .full_summary=${_full_summary(col.frame, col.high)} .triage=${col.high_status}></triage-status-sk> `;
  } else {
    return html`<a class=dot title='No clusters found.' href='/g/c/${ele._hashFrom(rowIndex)}?query=${ele._encQueryFrom(colIndex)}'>∅</a> `;
  }
}
...
const template = (ele) => html`
  <header>
    ...
    <details @toggle=${(e) => ele._statusOpen = e.target.open}>
      <summary>
        <h2>Status</h2>
      </summary>
      <div>
  ...
  <dialog id=cluster_summary_dialog>
    <cluster-summary2-sk
...
window.customElements.define('triage-page-sk', class extends ElementSk {
  constructor() {
    ...
    // If the status detail is open then we need to poll the
    // currentClusterinStatus.
  ...
  connectedCallback() {
    ...
    this._render();
    this._dialog = this.querySelector('#cluster_summary_dialog');
  ...
  _filterChange(e) {
    console.log(e);
    this._state.filter = e.target.value;
  }
  ...
  _poll() {
    if (this._statusOpen) {
      ...
      window.setTimeout(() => this._poll(), 1000);
    }
  }
  ...
  _updateRange() {
    ...
    var body = {
      ...
      subset: this._state.subset,
      alert_filter: this._state.filter,
    };
