...
define('plot-simple-sk', class extends ElementSk {
  ...
  _recalcPaths() {
    ...
    xAxisPath.moveTo(this._summary.rect.x, this._summary.rect.y + this._summary.rect.height);
    xAxisPath.lineTo(this._summary.rect.x + this._summary.rect.width, this._summary.rect.y + this._summary.rect.height);
    this._summary.axis.labels = [];
    ticks(this._labels).forEach(tick => {
      const label = {
        x: this._summary.range.x(tick.x),
        y: this._summary.rect.y + this._summary.rect.height + MARGIN/2 ,
        text: tick.text,
      };
      this._summary.axis.labels.push(label);
      xAxisPath.moveTo(label.x, this._summary.rect.y + this._summary.rect.height);
      xAxisPath.lineTo(label.x, this._summary.rect.y + this._summary.rect.height + MARGIN/2);
    });
