...
define('plot-simple-sk', class extends ElementSk {
  constructor() {
    ...
    // for details on tasks vs microtasks. The actual value is a window.setTimer
    // timerId or zero if no task is scheduled.
    this._recalcSearchTask = 0;

    // A task to do the actual re-draw work of a zoom.
    //
    // See https://jakearchibald.com/2015/tasks-microtasks-queues-and-schedules/
    // for details on tasks vs microtasks. The actual value is a window.setTimer
    // timerId or zero if no task is scheduled.
  ...
  */
  ...
  _recalcSearchImpl() {
    if (this._zoomTask) {
      // If we are still handling a zoom action then try again later.
      this._recalcSearchTask = window.setTimeout(() => this._recalcSearchImpl());
      return;
    }
    const domain = this._detail.range.x.domain();
