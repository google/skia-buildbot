...
define('plot-simple-sk', class extends ElementSk {
  ...
  _updateScaledMeasurements() {
    ...
    this.TICK_LENGTH = this.MARGIN / 4; // px
  ...
  */
  ...
  _recalcYAxis(area) {
    ...
    const cx = Math.floor(this._detail.rect.x) + 0.5;
    yAxisPath.moveTo(cx, this._detail.rect.y);
    yAxisPath.lineTo(cx, this._detail.rect.y + this._detail.rect.height);
    area.yaxis.labels = [];
    area.range.y.ticks(NUM_Y_TICKS).forEach((t) => {
      ...
      yAxisPath.moveTo(cx, label.y);
      yAxisPath.lineTo(cx - this.TICK_LENGTH, label.y);
    });
  ...
  _recalcXAxis(area, labels, labelOffset) {
    ...
    const cy = Math.floor(area.rect.y) + 0.5;
    xAxisPath.moveTo(area.rect.x + 0.5, cy);
    xAxisPath.lineTo(area.rect.x + 0.5 + area.rect.width, cy);
    area.axis.labels = [];
  ...
  _drawOverlayCanvas() {
    ...
    { // Block to scope save/restore.
      ...
      if (!this._inZoomDrag) {
        ...
        ctx.beginPath();
        const cx = Math.floor(this._crosshair.x) + 0.5;
        const cy = Math.floor(this._crosshair.y) + 0.5;
        ctx.moveTo(this._detail.rect.x, cy);
        ctx.lineTo(this._detail.rect.x + this._detail.rect.width, cy);
        ctx.moveTo(cx, this._detail.rect.y);
        ctx.lineTo(cx, this._detail.rect.y + this._detail.rect.height);
        ctx.stroke();
