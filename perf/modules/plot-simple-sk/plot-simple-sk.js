...
define('plot-simple-sk', class extends ElementSk {
  ...
  */
  ...
  _recalcYAxis(area) {
    ...
    const x = Math.floor(this._detail.rect.x);
    yAxisPath.moveTo(x - 0.5, this._detail.rect.y);
    yAxisPath.lineTo(x - 0.5, this._detail.rect.y + this._detail.rect.height);
    area.yaxis.labels = [];
    area.range.y.ticks(NUM_Y_TICKS).forEach((t) => {
      ...
      yAxisPath.moveTo(x, label.y + 0.5);
      yAxisPath.lineTo(x - this.TICK_LENGTH, label.y + 0.5);
    });
