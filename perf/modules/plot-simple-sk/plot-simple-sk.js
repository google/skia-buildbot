...
define('plot-simple-sk', class extends ElementSk {
  constructor() {
    ...
    this._xDetailRange = d3Scale.scaleLinear();
    this._yDetailRange = d3Scale.scaleLinear();
    this._xSummaryRange = d3Scale.scaleLinear();
    this._ySummaryRange = d3Scale.scaleLinear();
  ...
  connectedCallback() {
    ...
    this.addEventListener("mousedown", e => {
      ...
      if (inRect(this._summaryRect, { x: x, y: y })) {
        const zx = this._xSummaryRange.invert(x);
    ...
    this.addEventListener("click", e => {
      ...
      if (!inRect(this._detailRect, { x: x, y: y })) {
        return
      }
  ...
  _raf() {
    ...
    if (this._inZoomDrag == false) {
      ...
      if (x < this._summaryRect.x) {
        x = this._summaryRect.x;
      } else if (x > this._summaryRect.x + this._summaryRect.width) {
        x = this._summaryRect.x + this._summaryRect.width;
      }
      // x in source coordinates.
      const sx = this._xSummaryRange.invert(x);
