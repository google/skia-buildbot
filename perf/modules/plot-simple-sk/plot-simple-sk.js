...
define('plot-simple-sk', class extends ElementSk {
  ...
  _raf() {
    if (this._mouseMoveRaw != null) {
      ...
      // x,y on canvas in CSS pixels.
      const x = this._mouseMoveRaw.x - clientRect.left;
      const y = this._mouseMoveRaw.y - clientRect.top;
      // x,y on detail.
      const dx = x*this._scale - this._detailRect.x;
      const dy = y*this._scale - this._detailRect.y;
      // x,y in souce coordinates.
      const sx = this._xDetailRange.invert(dx);
      const sy = this._yDetailRange.invert(dy);
      if (this._pointSearch) {
        const closest = this._pointSearch.nearest({ x: sx, y: sy }, 1)[0][0];
        console.log(this._mouseMoveRaw.x, this._mouseMoveRaw.y, x, y, sx, sy, closest);
        console.log(this._xDetailRange(closest.x), this._yDetailRange(closest.y));
      }
      this._mouseMoveRaw = null;
    }
    window.requestAnimationFrame(this._raf.bind(this));
