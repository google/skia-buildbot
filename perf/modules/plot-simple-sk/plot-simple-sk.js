...
define('plot-simple-sk', class extends ElementSk {
  ...
  connectedCallback() {
    ...
    this.addEventListener("mousedown", e => {
      ...
      // x,y on canvas.
      const x = (e.clientX - clientRect.left) * this._scale;
      const y = (e.clientY - clientRect.top) * this._scale;
      if (inRect(this._summary.rect, { x: x, y: y })) {
        const zx = this._summary.range.x.invert(x);
    ...
    this.addEventListener("click", e => {
      ...
      if (!inRect(this._detail.rect, { x: x, y: y })) {
        return
      }
      const sx = this._detail.range.x.invert(x);
      const sy = this._detail.range.y.invert(y);
  ...
  _raf() {
    ...
    if (this._inZoomDrag == false) {
      ...
      const clientRect = this._ctx.canvas.getBoundingClientRect();
      // x,y on canvas.
      let x = (this._mouseMoveRaw.x - clientRect.left) * this._scale;

      // Clamp x to the summary rect;
      if (x < this._summary.rect.x) {
        x = this._summary.rect.x;
      } else if (x > this._summary.rect.x + this._summary.rect.width) {
        x = this._summary.rect.x + this._summary.rect.width;
      }
      // x in source coordinates.
      const sx = this._summary.range.x.invert(x);
  ...
  _drawOverlay() {
    ...
    this._overlayCtx.save();
    {
      ...
      this._overlayCtx.rect(this._summary.rect.x, this._summary.rect.y, this._summary.rect.width, this._summary.rect.height);
      ...
      if (this._zoom !== null) {
        ...
        this._overlayCtx.beginPath();
        this._overlayCtx.moveTo(leftx, this._summary.rect.y);
        this._overlayCtx.lineTo(leftx, this._summary.rect.y + this._summary.rect.height);

        // Draw right bar.
        const rightx = this._summary.range.x(this._zoom[1]);
        this._overlayCtx.moveTo(rightx, this._summary.rect.y);
        this._overlayCtx.lineTo(rightx, this._summary.rect.y + this._summary.rect.height);
        this._overlayCtx.stroke();

        // Draw gray boxes.
        this._overlayCtx.fillStyle = "#0003"
        this._overlayCtx.rect(this._summary.rect.x, this._summary.rect.y, leftx - this._summary.rect.x, this._summary.rect.height);
        this._overlayCtx.rect(rightx, this._summary.rect.y, this._summary.rect.x + this._summary.rect.width - rightx, this._summary.rect.height);
    ...
    this._overlayCtx.save();
    this._overlayCtx.beginPath();
    this._overlayCtx.rect(this._detail.rect.x, this._detail.rect.y, this._detail.rect.width, this._detail.rect.height);
    this._overlayCtx.clip();
    ...
    if (this._xbar !== -1) {
      ...
      this._overlayCtx.beginPath();
      this._overlayCtx.moveTo(bx, this._detail.rect.y);
      this._overlayCtx.lineTo(bx, this._detail.rect.y + this._detail.rect.height);
      this._overlayCtx.stroke();
    }
    ...
    this._bands.forEach(band => {
      this._overlayCtx.lineWidth = 3;
      this._overlayCtx.strokeStyle = "#888";
      this._overlayCtx.setLineDash([5, 5]);
      const bx = this._detail.range.x(band);
      this._overlayCtx.beginPath();
      this._overlayCtx.moveTo(bx, this._detail.rect.y);
      this._overlayCtx.lineTo(bx, this._detail.rect.y + this._detail.rect.height);
      this._overlayCtx.stroke();
      this._overlayCtx.setLineDash([]);
    });
    ...
    if (!this._inZoomDrag) {
      ...
      this._overlayCtx.beginPath();
      this._overlayCtx.moveTo(this._detail.rect.x, this._crosshair.y);
      this._overlayCtx.lineTo(this._detail.rect.x + this._detail.rect.width, this._crosshair.y);

      this._overlayCtx.moveTo(this._crosshair.x, this._detail.rect.y);
      this._overlayCtx.lineTo(this._crosshair.x, this._detail.rect.y + this._detail.rect.height);
      this._overlayCtx.stroke();
  ...
  _plot() {
    ...
    this._ctx.save();
    this._ctx.beginPath();
    this._ctx.rect(this._detail.rect.x, this._detail.rect.y, this._detail.rect.width, this._detail.rect.height);
    this._ctx.clip();
    this._lineData.forEach(line => {
    ...
    this._ctx.save();
    this._ctx.beginPath();
    this._ctx.rect(this._summary.rect.x, this._summary.rect.y, this._summary.rect.width, this._summary.rect.height);
    this._ctx.clip();
