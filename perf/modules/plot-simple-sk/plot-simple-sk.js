...
define('plot-simple-sk', class extends ElementSk {
  ...
  */
  ...
  _recalcYAxis(area) {
    ...
    const thin_x = Math.floor(this._detail.rect.x) + 0.5;
    ...
    area.range.y.ticks(NUM_Y_TICKS).forEach((t) => {
      ...
      yAxisPath.lineTo(thin_x - this.TICK_LENGTH, label.y);
    });
  ...
  _recalcXAxis(area, labels, labelOffset) {
    ...
    const thin_y = Math.floor(area.rect.y) + 0.5;
  ...
  _drawOverlayCanvas() {
    ...
    { // Block to scope save/restore.
      ...
      if (!this._inZoomDrag) {
        ...
        ctx.beginPath();
        const this_x = Math.floor(this._crosshair.x) + 0.5;
        const this_y = Math.floor(this._crosshair.y) + 0.5;
        ctx.moveTo(this._detail.rect.x, this_y);
        ctx.lineTo(this._detail.rect.x + this._detail.rect.width, this_y);
        ctx.moveTo(this_x, this._detail.rect.y);
        ctx.lineTo(this_x, this._detail.rect.y + this._detail.rect.height);
        ctx.stroke();
