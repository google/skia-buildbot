...
const AXIS_FONT_SIZE = 14; // px

const AXIS_FONT = `${AXIS_FONT_SIZE}px Roboto,Helvetica,Arial,Bitstream Vera Sans,sans-serif`;
...
define('plot-simple-sk', class extends ElementSk {
  ...
  // e is a mouse event or an object that has the coords stored in clientX and
  // clientY.
  _eventToCanvasPt(e) {
  ...
  _recalcPaths() {
    ...
    xAxisPath.moveTo(this._summary.rect.x, this._summary.rect.y);
    xAxisPath.lineTo(this._summary.rect.x + this._summary.rect.width, this._summary.rect.y);
    this._summary.axis.labels = [];
    ticks(this._labels).forEach(tick => {
      const label = {
        x: this._summary.range.x(tick.x),
        y: this._summary.rect.y - MARGIN / 2,
        text: tick.text,
      };
      this._summary.axis.labels.push(label);
      xAxisPath.moveTo(label.x, this._summary.rect.y );
      xAxisPath.lineTo(label.x, this._summary.rect.y  - MARGIN / 2);
    });
    this._summary.axis.path = xAxisPath;

    // Build detail tick marks.
    const detailDomain = this._detail.range.x.domain();
    const xOffset = Math.ceil(detailDomain[0]);
    const detailLabels = this._labels.slice(Math.ceil(detailDomain[0]), Math.floor(detailDomain[1]));
    xAxisPath = new Path2D();
    xAxisPath.moveTo(this._detail.rect.x, this._detail.rect.y);
    xAxisPath.lineTo(this._detail.rect.x + this._detail.rect.width, this._detail.rect.y);
    this._detail.axis.labels = [];
    ticks(detailLabels).forEach(tick => {
      const label = {
        x: this._detail.range.x(tick.x + xOffset),
        y: this._detail.rect.y - MARGIN / 2,
        text: tick.text,
      };
      this._detail.axis.labels.push(label);
      xAxisPath.moveTo(label.x, this._detail.rect.y );
      xAxisPath.lineTo(label.x, this._detail.rect.y  - MARGIN / 2);
    });
    this._detail.axis.path = xAxisPath;
  }
