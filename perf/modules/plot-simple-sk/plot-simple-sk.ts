...
function inRect(pt: Point, rect: Rect) {
  return (
    pt.x >= rect.x &&
...
interface TracePaths {
  _linePath: Path2D | null;
  _dotsPath: Path2D | null;
}
...
export class PlotSimpleSk extends ElementSk {
  ...
  private SUMMARY_HIGHLIGHT_LINE_WIDTH!: number;

  private DETAIL_RADIUS!: number;

  private SUMMARY_RADIUS!: number; // The radius of points in the summary area.

  private MARGIN!: number; // The margin around the details and summary areas.
  ...
  private LABEL_FONT!: string;

  private ZOOM_BAR_LINE_WIDTH!: number; // px

  private HOVER_LINE_WIDTH!: number; // px

  private LABEL_COLOR!: string;

  private LABEL_BACKGROUND!: string;

  private CROSSHAIR_COLOR!: string;

  private BAND_COLOR!: string;
