...
export class PlotSimpleSk extends ElementSk {
  ...
  connectedCallback(): void {
    ...
    this.addEventListener('wheel', (e: WheelEvent) => {
      if (this.detailsZoomRange) {
        e.stopPropagation();
        e.preventDefault();
        console.log(e.deltaY); // positive is down, negative is up.
        this.detailsZoomRange = null;
        this._zoomImpl();
      }
    });
  ...
  private doDetailsZoom() {
    ...
    if (Math.abs(this.zoomRect.width) < MIN_MOUSE_MOVE_FOR_ZOOM || Math.abs(this.zoomRect.height) < MIN_MOUSE_MOVE_FOR_ZOOM) {
      return;
    }
    ...
    this._inZoomDrag = 'no-zoom';
    // Calculate a new x and y extents and update this._details.domains.
    // The redraw everything.
    this._zoomImpl();
  }
