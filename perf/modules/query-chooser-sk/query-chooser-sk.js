 ...
 * @attr {string} key_order - The current query formatted as a URL formatted query string.
 *
 */
...
const template = (ele) => html`
  ...
  <query-summary-sk id=summary selection=${ele._current_query}></query-summary-sk>
  <div id="dialog">
...
window.customElements.define('query-chooser-sk', class extends ElementSk {
  ...
  _queryChange(e) {
    this._current_query = e.detail.q;
    this._render();
  }
  ...
  /** @prop key_order {string} Mirrors the key_order attribue. */
  get key_order() { return this.getAttribute('key_order'); }
  set key_order(val) { this.setAttribute('key_order', val); }
