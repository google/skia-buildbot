...
window.customElements.define('perf-scaffold-sk', class extends ElementSk {
  ...
  connectedCallback() {
    ...
    if (!this._main) {
      ...
      Array.prototype.slice.call(this.children).forEach((ele) => div.appendChild(ele))
      this._render();
      this._main = this.querySelector('main');
      Array.prototype.slice.call(div.children).forEach((ele) => this._main.appendChild(ele));
    }
  }
});
