...
const _request_type = (ele) => {
 if (ele._state.request_type === 0) {
   return html`
     ...
     <label>
       Begin:
       <elix-date-combo-box @date-changed=${ele._beginChange} date=${ele._toDate(ele._state.begin)}></elix-date-combo-box>
     </label>
     `;
 } else {
   return html`
...
const template = (ele) => html`
  <dialog-sk>
    ...
    <div>
      <label>
        End:
        <elix-date-combo-box @date-changed=${ele._endChange} date=${ele._toDate(ele._state.end)}></elix-date-combo-box>
      </label>
    </div>
    <div id=controls>
