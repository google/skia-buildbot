 ...
 * date.
 *
 * @evt {string} force_request_type - A value of 'dense' or 'range' will force the corresponding
...
const template = (ele) => html`
  <dialog-sk>
    <h2>Graph Domain</h2>
    <radiogroup>
      <label><input type=radio @change=${ele._typeRange} .checked=${ele._state.request_type === 0}>Date Range </label>
      <label><input type=radio @change=${ele._typeDense} .checked=${ele._state.request_type === 1}>Dense      </label>
    </radiogroup>
    <div class=ranges>
      ${_request_type(ele)}
      <label>
