...
describe('graph-title-sk', () => {
  ...
  it('shows short title and "Show Full Title" button when many entries', () => {
  ...
  it('shows full title when clicking "Show Full Title"', () => {
    const entries = new Map<string, string>();
    for (let i = 0; i < 10; i++) {
      entries.set(`key${i}`, `value${i}`);
    }
  ...
  it('can toggle back to short titles', () => {
    const entries = new Map<string, string>();
    for (let i = 0; i < 10; i++) {
      entries.set(`key${i}`, `value${i}`);
    }
