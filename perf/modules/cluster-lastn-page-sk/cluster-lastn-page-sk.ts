...
import { UIDomain, ParamSet } from '../json';
  ...
  ele._regressions.map(
    (reg) => html`
      <tr>
        ...
        ${_low(ele, reg)} ${_high(ele, reg)} ${_filler(ele)}
      </tr>
    `
  );

const _configTitle = (ele) =>
  html`
  ...
  private static template = (ele: ClusterLastNPageSk) => html`
    ...
    <div class="controls">
      <label>
        ...
        <button @click=${ele._alertEdit}>${_configTitle(ele)}</button>
      </label>
      <label>
        <h2>Time Range</h2>
        <domain-picker-sk
