...
// displayed commit.
const RANGE_CHANGE_ON_ZOOM_PERCENT = 0.25;
...
function calculateRangeChange(
  ...
  if (exceedsLeftEdge && exceedsRightEdge) {
    ...
    return {
      rangeChange: true,
      newOffsets: [offsets[0] - offsetDelta, offsets[1] + offsetDelta],
    };
  } else if (exceedsLeftEdge) {
    // shift left
    return {
      rangeChange: true,
      newOffsets: [offsets[0] - offsetDelta, offsets[1]],
    };
...
export class ExploreSk extends ElementSk {
  ...
  private zoomOrRangeChange(zoom: [number, number]) {
    ...
    if (result.rangeChange) {
      ...
      this.state.begin = result.newOffsets![0];
      this.state.end = result.newOffsets![1];
  ...
  private rangeChangeImpl() {
    ...
    this.requestFrame(body, (json) => {
      ...
      // If we did an offset based frame request then we need to re-interpret that
      // back to a time based query.
      if (this.state.requestType === 2) {
        this.state.requestType = 0;
        this.state.begin = this._dataframe.header![0]!.timestamp;
        this.state.end = this._dataframe.header![
          this._dataframe.header!.length - 1
        ]!.timestamp;
