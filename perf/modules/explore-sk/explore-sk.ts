...
export class ExploreSk extends ElementSk {
  ...
  <div id=bottomButtons>
    <div id=shiftButtons ?hide_if_no_data=${!ele.hasData()}>
      <button @click=${ele.shiftLeft} title='Move ${
    ele._numShift
  } commits in the past.'>&lt;&lt; ${ele._numShift}</button>
      <button @click=${ele.shiftBoth} title='Expand the display ${
    ele._numShift
  } commits in both directions.'>&lt;&lt; ${+ele._numShift} &gt;&gt;</button>
      <button @click=${ele.shiftRight} title='Move ${
    ele._numShift
  } commits in the future.'>${+ele._numShift} &gt;&gt;</button>
    </div>
    <div id=calcButtons ?hide_if_no_data=${!ele.hasData()}>
      <button @click=${
        ele.norm
