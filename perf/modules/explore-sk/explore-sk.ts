...
class State {
  ...
  num_commits: number;
  request_type: RequestType; // TODO(jcgregorio) Use constants in domain-picker-sk.
  ...
  constructor() {
    ...
    this.num_commits = 50;
    this.request_type = 1; // TODO(jcgregorio) Use constants in domain-picker-sk.
  }
}
...
export class ExploreSk extends ElementSk {
  ...
  <div id=buttons>
    ...
    <div id=traceButtons ?hide_if_no_data=${!ele._hasData()}>
      <button @click=${() =>
        ele._removeAll(
          false
        )} title='Remove all the traces.'>Remove All</button>
      <button @click=${
        ele._removeHighlighted
      } title='Remove all the highlighted traces.'>Remove Highlighted</button>
      <button @click=${
        ele._highlightedOnly
      } title='Remove all but the highlighted traces.'>Highlighted Only</button>
      <span title='Number of commits skipped between each point displayed.' ?hidden=${ele._isZero(
        ele._dataframe.skip
      )} id=skip>${ele._dataframe.skip}</span>
      <checkbox-sk name=zero @change=${ele._zeroChangeHandler} ?checked=${
    ele.state.show_zero
  } label='Zero' title='Toggle the presence of the zero line.'>Zero</checkbox-sk>
      <checkbox-sk name=auto @change=${ele._autoRefreshHandler} ?checked=${
    ele.state.auto_refresh
  } label='Auto-refresh'   title='Auto-refresh the data displayed in the graph.'>Auto-Refresh</checkbox-sk>
      <button @click=${ele._zoomToRange} ?disabled=${
    ele._zoomRange === null
  ...
  <div id=bottomButtons>
    <div id=shiftButtons ?hide_if_no_data=${!ele._hasData()}>
      <button @click=${ele._shiftLeft} title='Move ${
    ele._numShift
  } commits in the past.'>&lt;&lt; ${ele._numShift}</button>
      <button @click=${ele._shiftBoth} title='Expand the display ${
    ele._numShift
  } commits in both directions.'>&lt;&lt; ${+ele._numShift} &gt;&gt;</button>
      <button @click=${ele._shiftRight} title='Move ${
    ele._numShift
  } commits in the future.'>${+ele._numShift} &gt;&gt;</button>
    </div>
    <div id=calcButtons ?hide_if_no_data=${!ele._hasData()}>
      <button @click=${
        ele._normalize
      } title='Apply norm() to all the traces.'>Normalize</button>
      <button @click=${
        ele._scale_by_avg
      } title='Apply scale_by_avg() to all the traces.'>Scale By Avg</button>
      <button @click=${
        ele._csv
      } title='Download all displayed data as a CSV file.'>CSV</button>
    ...
    <div id=tabs ?hide_if_no_data=${!ele._hasData()}>
      ...
      <tabs-panel-sk>
        <div>
          ...
          <paramset-sk id=paramset clickable_values @paramset-key-value-click=${
            ele._paramsetKeyValueClick
          }></paramset-sk>
        </div>
        <div id=details>
          <paramset-sk id=simple_paramset clickable_values @paramset-key-value-click=${
            ele._paramsetKeyValueClick
          }></paramset-sk>
          <div>
  ...
  private _commits: CommitDetailPanelSk | null = null;
  private _commitsTab: HTMLButtonElement | null = null;
  private _detailTab: TabsSk | null = null;
  private _formula: HTMLTextAreaElement | null = null;
  private _jsonsource: JSONSourceSk | null = null;
  private _paramset: ParamSetSk | null = null;
  private _percent: HTMLSpanElement | null = null;
  private _plot: PlotSimpleSk | null = null;
  private _query: QuerySk | null = null;
  private _query_count: QueryCountSk | null = null;
  private _range: DomainPickerSk | null = null;
  private _simple_paramset: ParamSetSk | null = null;
  private _spinner: SpinnerSk | null = null;
  private _summary: ParamSetSk | null = null;
  private _trace_id: HTMLSpanElement | null = null;
  private _csv_download: HTMLAnchorElement | null = null;
  private _queryDialog: HTMLDialogElement | null = null;
   ...
   */
  _rationalizeZoom(zoom: [number, number]) {
  ...
  _queryChangeDelayedHandler(e: CustomEvent<QuerySkQueryChangeEventDetail>) {
    this._query_count!.current_query = e.detail.q;
  }

  // Reflect the current query to the query summary.
  _queryChangeHandler(e: CustomEvent<QuerySkQueryChangeEventDetail>) {
  ...
  _traceSelected(e: CustomEvent<PlotSimpleSkTraceEventDetails>) {
   ...
   */
  _rationalizeTimeRange(state: State): State {
  ...
  _paramsetKeyValueClick(e: CustomEvent<ParamSetSkClickEventDetail>) {
    const keys: string[] = [];
  ...
  _shiftImpl(beginOffset: number, endOffset: number) {
    const body = {
  ...
  _requestFrameBodyFullFromState(): FrameRequest {
    return {
      begin: this.state.begin,
      end: this.state.end,
      num_commits: this.state.num_commits,
  ...
  _zeroChangeHandler(e: MouseEvent) {
    this.state.show_zero = (e.target! as HTMLInputElement).checked;
  ...
  _autoRefreshHandler(e: MouseEvent) {
    this.state.auto_refresh = (e.target! as HTMLInputElement).checked;
    this._stateHasChanged();
    this._autoRefreshChanged();
  }
   ...
   */
  _addTraces(json: FrameResponse, tab: boolean) {
   ...
   */
  _removeAll(skipHistory: boolean) {
    this.state.formulas = [];
    this.state.queries = [];
    this.state.keys = '';
    this._plot!.removeAll();
   ...
   */
  _addCalculated(replace: boolean) {
    this._queryDialog!.close();
  ...
  _requestFrame(body: FrameRequest, cb: RequestFrameCallback) {
  ...
  _checkFrameRequestStatus(cb: RequestFrameCallback) {
