...
const template = (ele) => html`
    ...
    <div id=buttons>
      ...
      <checkbox-sk name=zero @click=${ele._zeroChanged} ?checked=${ele.state.show_zero} label="Zero" title="Toggle the presence of the zero line.">Zero</checkbox-sk>
      <checkbox-sk name=auto @click=${ele._autoRefreshChanged} ?checked=${ele.state.auto_refresh} label="Auto-refresh" title="Auto-refresh the data displayed in the graph.">Auto-Refresh</checkbox-sk>
    ...
    <div id=tabs>
      <tabs-sk id=detailTab>
        ...
        <tabs-panel-sk id=queryTab>
          <div class="layout horizontal">
            ...
            <div class="layout vertical" id=selections>
              ...
              <div>
                <query-count-sk url='/_/count/'></query-count-sk>
              </div>
        ...
        <tabs-panel-sk>
          <input title="Trace ID" readonly type=text id=trace_id></input>
          <paramset-sk id=paramset class=hidden clickable_values></paramset-sk>
        </tabs-panel-sk>
        <tabs-panel-sk id=details class=hidden>
...
window.customElements.define('explore-sk', class extends ElementSk {
  ...
  connectedCallback() {
    ...
    this._query.addEventListener("query-change", (e) => {
    ...
    this._query.addEventListener("query-change-delayed", (e) => {
      this._query_count.current_query = e.detail.q;
    });

    // Highlight trace when a paramset value is clicked.
    this._paramset.addEventListener("paramset-key-value-click", (e) =>  this._paramsetKeyValueClick(e));
    this._simple_paramset.addEventListener("paramset-key-value-click", (e) => this._paramsetKeyValueClick(e));

    // Reflect the focused trace in the paramset.
    this._plot.addEventListener("trace_focused", (e) => {
      this._paramset.highlight = sk.key.toObject(e.detail.id);
      this._trace_id.value = e.detail.id;
    });

    // User has zoomed in on the graph.
    this._plot.addEventListener("zoom", (e) => {
    ...
    this._plot.addEventListener("trace_selected", (e) => {
      ...
      const params = toObject(e.detail.id);
      const paramset = {}
  ...
  */
  ...
  _matches(key, paramName, paramValue) {
  ...
  _paramsetKeyValueClick(e) {
    ...
    Object.keys(this._lines).forEach((key) => {
      if (this._matches(key, e.detail.key, e.detail.value)) {
        keys.push(key);
      }
    });
