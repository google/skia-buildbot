...
import { ParamSet, Alert, Direction, StepDetection } from '../json';
...
// TODO(jcgregorio) Fix alerts.Direction to be a string.
const BOTH = 0;
const UP = 1;
const DOWN = 2;
...
export class AlertConfigSk extends ElementSk {
  private static template = (ele: AlertConfigSk) => html`
    ...
    <select-sk
      ...
      @selection-changed=${(e) =>
        (ele._config.state = e.target.children[e.detail.selection].getAttribute(
          'value'
        ))}
    >
      <div
  ...
  private _key_order: string[] | null = null;
  private _bugSpinner: SpinnerSk | null = null;
  private _alertSpinner: SpinnerSk | null = null;
