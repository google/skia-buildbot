...
<algo-select-sk id=grouping algo=${ele._config.algo} @algo-change=${(e) => ele._config.algo = e.detail.algo}></algo-select-sk>
  ...
  <label for=step>Choose the algorithm used to determine if a regression has occurred.
    The actual value is set in <em>Threshhold</em>.
  </label>
  <select-sk id=step @selection-changed=${(e) => ele._config.step = e.target.children[e.detail.selection].getAttribute('value')}>
    ...
    <div value=absolute ?selected=${ele._config.step === 'absolute'}>A change in absolute magnitude. Threshhold is the minimum difference to trigger an alert.</div>
    <div value=percent ?selected=${ele._config.step === 'percent'}>A change by percent. Threshhold is a value in [0.0, 1.0] and the minimum difference to trigger an alert.</div>
    <div value=cohen ?selected=${ele._config.step === 'cohen'}>Use Cohen's d method to detect a change. Threshhold is the standard deviations that the mean must move to trigger an alert.</div>
  </select-sk>
  <h4>Threshold</h4>
