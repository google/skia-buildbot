...
const template = (ele) => html`
  ...
  <h3>Status</h3>
  <select-sk .selection=${ele._state()} @selection-changed=${(e) => ele.config.state=e.target.children[e.detail.selection].getAttribute('value')}>
...
window.customElements.define('alert-config-sk', class extends ElementSk {
  ...
  _state() {
    return STATUS_MAP[this.config.state];
  }
