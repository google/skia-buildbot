...
<algo-select-sk id=grouping algo=${ele._config.algo} @algo-change=${(e) => ele._config.algo = e.detail.algo}></algo-select-sk>
  ...
  <label for=step>Choose the algorithm used to determine if a regression has occurred.
    The actual value is set in Threshhold.
  </label>
  <select-sk id=step @selection-changed=${(e) => ele._config.step = e.target.children[e.detail.selection].getAttribute('value')}>
    ...
    <div value=absolute ?selected=${ele._config.step === 'absolute'}>A change in absolute magnitude.</div>
    <div value=percent ?selected=${ele._config.step === 'percent'}>A change by percent (0.0 &rightarrow; 1.0).</div>
    <div value=cohen ?selected=${ele._config.step === 'cohen'}>Use Cohen's d method to detect a change.</div>
  </select-sk>
  <h4>Threshold</h4>
  <label for=threshold>The threshold for Step Detection to trigger an alert. The meaning of the value and meaningful range depends on the algorithm chosen for Step Detection. </label>
