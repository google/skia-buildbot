...
// Only show the first and last N individual traces, because showing them all
// could be problematic since there could be >100k.
const numIndividualTracesHeadTail = 10;
...
private static individualResults = (ele: TrybotPageSk): TemplateResult[] | null => {
  ...
  ele.results.results!.forEach((r, i) => {
    ...
    // can be huge.
    if (i > numIndividualTracesHeadTail && i < ele.results!.results!.length - numIndividualTracesHeadTail) {
      return;
    }
