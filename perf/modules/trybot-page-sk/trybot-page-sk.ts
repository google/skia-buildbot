  ...
  private static template = (ele: TrybotPageSk) => html`
    ...
    >
      ...
      <tabs-panel-sk>
        <div>
          <table>
            <tr><th>Index</th><th>StdDev Ratio</th> <th>Params</th></tr>
  ...
  ele.results.results!.forEach((r, i) => {
    ...
    Object.entries(r.params).forEach((keyValue) => {
      ...
      if (value !== lastParams[key]) {
        keyValueDelta.push(html`<span class=changed>${key}=${value}</span> `);
      } else {
        keyValueDelta.push(html`${key}=${value} `);
      }
    });
    ret.push(html`<tr><td>${i}</td> <td>${r.stddevRatio}</td> <td>${keyValueDelta} </td></tr>`);
    lastParams = r.params;
  });
  return ret;
}
