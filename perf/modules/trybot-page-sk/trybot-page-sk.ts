...
export class TrybotPageSk extends ElementSk {
  ...
  private static template = (ele: TrybotPageSk) => html`
    ...
    >
      ...
      <tabs-panel-sk>
        <div>
          <table>
            <tr><th>Index</th><th>Param</th><th>StdDev Ratio</th></tr>
  ...
  private static individualResults = (ele: TrybotPageSk): TemplateResult[] | null => {
    ...
    // can be huge.
      ...
      ret.push(html`<tr><td class=link @click=${() => ele.plotIndividualTrace(i)}>${i + 1}</td> <td>${r.stddevRatio}</td> ${keyValueDelta}</tr>`);
      lastParams = r.params;
    });
    return ret;
  }
  ...
  private static byParamsResults = (ele: TrybotPageSk): TemplateResult[] | null => {
    ...
    // Only display the head and tail of the byParams results since the list can
    // be huge.
      if (i > numHeadTail && i < ele.byParams.length - numHeadTail) {
        return;
      }
      ret.push(html`<tr><td>${i + 1}</td> <td>${b.keyValue}</td> ${b.aveStdDevRatio}</tr>`);
    });
    return ret;
  }
  ...
  async connectedCallback(): Promise<void> {
    ...
    this.plot = this.querySelector('plot-simple-sk');
  ...
  private plotIndividualTrace(i: number) {
    const result = this.results!.results![i];
    const params = result.params;
    const keys = Object.keys(params).sort();
    const traceName = `,${keys.map((key) => `${key}=${params[key]}`).join(',')},`;


    const lines: {[key: string]: number[]|null} = {};
    lines[traceName] = result.values;
    const labels = this.results!.header!.map((colHeader) => new Date(colHeader!.timestamp * 1000));
    this.plot!.addLines(lines, labels!);
