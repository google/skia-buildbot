...
import {
  ...
  Commit,
  TryBotRequest,
  TryBotResponse, Params, TryBotResult,
} from '../json';
...
export class TrybotPageSk extends ElementSk {
  ...
  private static template = (ele: TrybotPageSk) => html`
    ...
    >
      ...
      <tabs-panel-sk>
        <div>
          <table>
            <tr><th>Index</th><th>StdDev Ratio</th> <th>Plot</th> ${TrybotPageSk.paramKeysAsHeaders(ele)} </tr>
        ...
        <div>
          <table>
            <tr><th>Index</th> <th>Plot</th> <th>Param</th> <th>StdDev Ratio</th> <th>N</th></tr>
  ...
  private static byParamsResults = (ele: TrybotPageSk): TemplateResult[] | null => {
    ...
    // Only display the head and tail of the byParams results.
      ...
      ret.push(html`<tr><td>${i + 1}</td> <td class=link @click=${(e: MouseEvent) => ele.plotByParamsTraces(e, i)}><timeline-icon-sk></timeline-icon-sk></td> <td>${b.keyValue}</td> <td>${b.aveStdDevRatio}</td> <td>${b.n}</td></tr>`);
    });
    return ret;
  }
  ...
  // TODO(jcgregorio) Break out into structured key module.
  private makeKey(params: Params): string {
  ...
  private plotIndividualTrace(e: MouseEvent, i: number) {
    ...
    lines[this.makeKey(params)] = result.values;
  ...
  private plotByParamsTraces(e: MouseEvent, i: number) {
    ...
    // Sort from largest to smallest, or smallest to largest depending on sign of
    matches.sort((a, b) => b.stddevRatio - a.stddevRatio);
    ...
    const lines: {[key: string]: number[]|null} = {};
    matches.forEach((r) => {
      lines[this.makeKey(r.params)] = r.values;
    });
