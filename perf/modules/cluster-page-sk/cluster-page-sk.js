...
const template = (ele) => html`
  ...
  <div class=day-range-with-spinner>
    <day-range-sk id=range @day-range-change=${ele._rangeChange} begin=${ele._state.begin} end=${ele._state.end}></day-range-sk>
    <spinner-sk ?active=${ele._updating_commits}></spinner-sk>
  </div>
  <h3>Commit</h3>
  <div>
    <commit-detail-picker-sk @commit-selected=${ele._commitSelected} .selected=${ele._selected_commit_index} .details=${ele._cids} id=commit></commit-picker-sk>
  </div>

  <h2>Algorithm</h2>
  <algo-select-sk @algo-change=${_algoChange} algo=${ele._state.algo}></algo-select-sk>

  <h2>Query</h2>
  <div class=query-action>
    <query-sk
      @query-change=${_queryChange}
      .key_order=${sk.perf.key_order}
      .paramset=${ele._paramset}
      current_query=${ele._state.q}
      ></query-sk>
    <div id=selections>
      <h3>Selections</h3>
      <query-summary-sk id=summary selection=${ele._state.q}></query-summary-sk>
      <div>
        Matches: <query-count-sk url='/_/count/' current_query=${ele._state.q}></query-count-sk>
      </div>
      <button @click=${ele._start} class=action id=start ?disabled=${!!ele._requestId}>Run</button>
      <div>
  ...
  <details>
    ...
    <div id=inputs>
      <label>
        ...
        <input type=number min=0 max=100 .value=${ele._state.k} @input=${(e) => { ele._state.k = e.target.value }}>
      </label>
      <label>
        Number of commits to include on either side.
        <input type=number min=1 max=25  .value=${ele._state.radius}>
      </label>
      <label>
        Clusters are interesting if regression score &gt;= this.
        <input type=number min=0 max=500 .value=${ele._state.interesting}>
      </label>
      <checkbox-sk
...
window.customElements.define('cluster-page-sk', class extends ElementSk {
  constructor() {
    ...
    // The id of the current cluster request. Will be the empty string if there is no pending request.
    this._requestId = '';
  ...
  _checkClusterRequestStatus(cb) {
    fetch(`/_/cluster/status/${this._request}`).then(jsonOrThrow).then((json) => {
