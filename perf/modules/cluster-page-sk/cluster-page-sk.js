  ...
  <details>
    ...
    <div id=inputs>
      <label>
        ...
        <input type=number min=0 max=100 .value=${ele._state.k} @input=${ele._kChanged}>
      </label>
      <label>
        Number of commits to include on either side.
        <input type=number min=1 max=25  .value=${ele._state.radius} @input=${ele._radiusChanged}>
      </label>
      <label>
        Clusters are interesting if regression score &gt;= this.
        <input type=number min=0 max=500 .value=${ele._state.interesting} @input=${ele._interestingChanged}>
      </label>
      <checkbox-sk
        ?checked=${ele._state.sparse}
        @input=${ele._sparseChanged}
