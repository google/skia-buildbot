...
There should be a `connect-*-cockroachdb.sh` script for each instance in this
directory. Run that script and you will be dropped into an SQL shell as root to
send commands to the server. You can, for example, create a new database here:
...
`admin-*-cockroachdb.sh` script, which will set up a port-forward to the admin
