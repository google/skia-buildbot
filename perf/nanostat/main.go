...
var (
	...
	flagSet = flag.NewFlagSet("nanostat", flag.ExitOnError)

	flagAlpha = flagSet.Float64("alpha", 0.05, "Consider a change significant if p < α.")
	flagSort  = flagSet.String("sort", "delta", "Sort by `order`: [-]delta, [-]name")
	flagIQRR  = flagSet.Bool("iqrr", false, "If true then remove outliers using the Interquartile Rule")
...
func main() {
	...
	// Ignore the output since the flagSet is ExitOnError.
	...
	if len(result.Rows) > 0 {
		...
		if !config.All {
			fmt.Println("No significant deltas found. Add --all to see non-significant results.")
		}
	}
}
