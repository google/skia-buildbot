...
UPSERT INTO
  TraceNames (trace_name, trace_id, params)
VALUES
...
FROM
  TraceNames
WHERE
  ARRAY['arch=x86'] <@ params;

-- Retrieve matching values. Note that sqlite querys are limited to 1MB,
-- so we might need to break up the trace_ids if the query is too long.
SELECT
  trace_id, commit_number, val
FROM
  TraceValues2
WHERE
  commit_number>=0 
  AND commit_number<255
  AND trace_id IN ('fe385b159ff55dca481069805e5ff050', '277262a9236d571883d47dab102070bc');

-- Build traces using a JOIN.
SELECT
  TraceNames.trace_name, TraceValues2.commit_number, TraceValues2.val
FROM
  TraceNames
INNER JOIN
  TraceValues2
...
WHERE
  ...
  AND
  TraceValues2.commit_number>=0 AND TraceValues2.commit_number<255;
