...
VALUES
    (
        0,
        '586101c79b0490b50623e76c71a5fd67d8d92b08',
        1158764756,
        'unknown@example.com',
    ...
    (
        1,
        '0f87cd842dd46205d5252c35da6d2c869f3d2e98',
        1158767262,
        'unknown@example.com',
    ...
    (
        2,
        '48ede9b432a3c3d62835a1400a9ed347b4a93024',
        1163013888,
        'unknown@example.org',
        'Add LICENSE'
    );
...
FROM
    Commits
WHERE
    git_hash = '0f87cd842dd46205d5252c35da6d2c869f3d2e98';
...
INSERT INTO
    TraceIDs (trace_name, trace_id)
VALUES
    (',arch=x86,config=8888,', 1),
    (',arch=x86,config=565,', 2),
    (',arch=arm,config=8888,', 3),
    (',arch=arm,config=565,', 4) ON CONFLICT DO NOTHING;
...
WHERE
    TraceIDs.trace_name = ',arch=x86,config=8888,'
    OR TraceIDs.trace_name = ',arch=x86,config=565,';
...
WHERE
    TraceValues.trace_id IN (
        ...
        FROM
            Postings
        WHERE
            key_value IN ('arch=x86', 'arch=arm')
            AND tile_number = 0
    )
