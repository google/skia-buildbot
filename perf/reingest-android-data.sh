...
# Alter the date argument to the gcloud storage ls command to determine which part of
# the log to re-transmit.

gcloud storage ls "gs://skia-perf/android-master-ingest/tx_log/2021/02/17/**" | xargs -L 1 -P 10 -I {} sh -c "echo {}; gcloud storage cp {} - | curl --include  --data @- https://android-metric-ingest.skia.org/upload"