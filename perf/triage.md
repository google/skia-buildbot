...
The UI will look roughly like this:
  ...
  * Each query/commit/(low|high) cell has:
    * Status:
      * Nfs - insufficient data to do clustering around that commit.
...
The data stored for the triage page analysis will be:
...
The alerting analysis will run the following analysis continuously:
    ...
    For each commit:
      For each query:
        * Do clustering.
        * Save results into datastore for regressions that show up
          for that commit.
