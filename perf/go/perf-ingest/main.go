...
func main() {
	...
	subName := fmt.Sprintf("%s-%s", cfg.DataStoreConfig.Topic, "prod")
	if *local {
		// When running locally create a new topic for every host.
		subName = fmt.Sprintf("%s-%s", cfg.DataStoreConfig.Topic, hostname)
	}
	...
	if !ok {
		sub, err = pubSubClient.CreateSubscription(ctx, subName, pubsub.SubscriptionConfig{
			Topic: pubSubClient.Topic(cfg.DataStoreConfig.Topic),
		})
		if err != nil {
