...
func newPerfProcessor(ctx context.Context, vcs vcsinfo.VCS, cfg *config.IngesterConfig, ts oauth2.TokenSource) (*btPerfProcessor, error) {
	store, err := btts.NewBigTableTraceStoreFromConfig(ctx, cfg, ts)
	if err != nil {
		return nil, err
	}
...
func (p *btPerfProcessor) Process(ctx context.Context, name string, r io.Reader, timestamp time.Time) error {
	...
	if err != nil {
		...
		index, err = p.vcs.IndexOf(ctx, benchData.Hash)
		if err != nil {
			return fmt.Errorf("Could not ingest, hash not found even after pulling %q: %s", benchData.Hash, err)
		}
	}
	...
	go func() {
		for {
			err := sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
				...
				obj := gcsClient.Bucket(event.Bucket).Object(event.ObjectName)
