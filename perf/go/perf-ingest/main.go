...
func (p *btPerfProcessor) Process(ctx context.Context, resultsFile ingestion.ResultFileLocation, timestamp time.Time) error {
	...
	return p.store.WriteTraces(int32(index), encoded, resultsFile.Name(), timestamp)
}
...
func main() {
	...
	cfg, ok := config.PERF_INGESTION_CONFIGS[*configName]
	if !ok {
	...
	processor, err := newPerfProcessor(ctx, vcs, cfg.TileSize, cfg.Project, cfg.Instance, cfg.Table, ts)
	if err != nil {
		sklog.Fatal(err)
	}
	...
	go func() {
		for {
			err := sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
				msg.Ack()
				var event gcs.StorageEvent
				if err := json.Unmarshal(msg.Data, &event); err != nil {
					sklog.Error(err)
					return
				}
				attrs, err := gcsClient.Bucket(event.Bucket).Object(event.ObjectName).Attrs(ctx)
				if err != nil {
					sklog.Error(err)
					return
				}
				resultFileLocation := ingestion.NewGCSResultFileLocation(attrs, "unused", gcsClient)
				if err := processor.Process(ctx, resultFileLocation, attrs.Created); err != nil {
