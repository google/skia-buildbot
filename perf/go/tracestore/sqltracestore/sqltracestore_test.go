...
func TestGetOrderedParamSet_CacheEntriesAreWrittenForParamSets(t *testing.T) {
	...
	assert.True(t, s.cache.Contains(cacheKeyForParamSets(tileNumber, "arch", "x86")))
	assert.True(t, s.cache.Contains(cacheKeyForParamSets(tileNumber, "config", "565")))
	assert.True(t, s.cache.Contains(cacheKeyForParamSets(tileNumber, "config", "8888")))
}
...
func TestGetOrderedParamSet_ParamSetCacheIsClearedAfterTTL(t *testing.T) {
	...
	// Restore timeNow when we are done.
	defer func() {
		timeNow = time.Now
	}()
	ops, err = s.GetOrderedParamSet(ctx, tileNumber)
