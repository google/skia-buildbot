...
func testNew(t *testing.T, s *SQLTraceStore) {
	err := s.WriteTraces(types.CommitNumber(0), []paramtools.Params{
...
// subTestsEmptyStore are all the tests we have for *SQLTraceStore that start
// from an empty Store.
...
// subTestsPopulatedStore are all the tests we have for *SQLTraceStore that
// start from a populated Store.
