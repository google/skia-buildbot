...
func TestGetLsatNSources_MoreCommitsMatchThanAreAskedFor_Success(t *testing.T) {
	...
	require.NoError(t, err)
	expected := []tracestore.SourceAndCommitNumber{
		{
			Source:       "gs://perf-bucket/2020/02/08/13/testdata.json",
			CommitNumber: 8,
		},
		{
			Source:       "gs://perf-bucket/2020/02/08/12/testdata.json",
			CommitNumber: 2,
		},
	}
...
func TestGetLsatNSources_LessCommitsMatchThanAreAskedFor_Success(t *testing.T) {
	...
	expected := []tracestore.SourceAndCommitNumber{
		...
		{
			Source:       "gs://perf-bucket/2020/02/08/11/testdata.json",
			CommitNumber: 1,
		},
	}
	require.Equal(t, expected, sources)
