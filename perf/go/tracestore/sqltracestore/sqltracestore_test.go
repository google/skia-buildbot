...
func TestGetOrderedParamSet(t *testing.T) {
	...
	assert.False(t, s.paramSetCache.Contains(tileNumber))
	...
	assert.True(t, s.paramSetCache.Contains(tileNumber))
}
...
func TestGetOrderedParamSet_ParamSetCacheIsClearedAfterTTL(t *testing.T) {
	...
	ops, err = s.GetOrderedParamSet(ctx, tileNumber, time.Now().Add(paramSetCacheTTL*2))
	assert.NoError(t, err)
