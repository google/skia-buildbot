...
const paramSetCacheSize = 100

const paramSetCacheTTL = 5 * time.Minute

type paramSetCacheEntry struct {
	added    time.Time
	paramset *paramtools.OrderedParamSet
}
...
type SQLTraceStore struct {
	...
	// paramSetCache is a cache for OrderedParamSets that have a TTL.
	// The cache maps tileNumber -> paramSetCacheEntry.
	paramSetCache *lru.Cache
...
func New(db *pgxpool.Pool, datastoreConfig config.DataStoreConfig) (*SQLTraceStore, error) {
	...
	paramSetCache, err := lru.New(paramSetCacheSize)
	if err != nil {
		return nil, skerr.Wrap(err)
	}
...
func (s *SQLTraceStore) GetOrderedParamSet(ctx context.Context, tileNumber types.TileNumber) (*paramtools.OrderedParamSet, error) {
	...
	iEntry, ok := s.paramSetCache.Get(tileNumber)
	if ok {
		entry := iEntry.(paramSetCacheEntry)
		if entry.added.Add(paramSetCacheTTL).After(time.Now()) {
			return entry.paramset, nil
		}
		_ = s.paramSetCache.Remove(tileNumber)
	}
	...
	_ = s.paramSetCache.Add(tileNumber, paramSetCacheEntry{
		added:    time.Now(),
		paramset: ret,
	})
...
func cacheKeyForTile(traceID traceIDForSQL, tileNumber types.TileNumber) string {
...
func (s *SQLTraceStore) WriteTraces(commitNumber types.CommitNumber, params []paramtools.Params, values []float32, _ paramtools.ParamSet, source string, _ time.Time) error {
	...
	for i, p := range params {
		...
		if !s.cache.Exists(cacheKeyForTile(traceID, tileNumber)) {
	...
	if len(tilesTemplateContext) > 0 {
		...
		for _, entry := range tilesTemplateContext {
			s.cache.Add(cacheKeyForTile(entry.MD5HexTraceID, tileNumber), "1")
		}
	}
