...
var errTraceNameNotFoundInCache = errors.New("trace name not found in cache")

func (s *SQLTraceStore) getTraceIDFromCache(hashedTraceName string) (traceIDFromSQL, error) {
	if traceIDString, ok := s.cache.Get(hashedTraceName); ok {
		traceIDInt64, err := strconv.ParseInt(traceIDString, 10, 64)
		if err != nil {
			return badTraceIDFromSQL, err
		}
		return traceIDFromSQL(traceIDInt64), nil
	}
	return badTraceIDFromSQL, errTraceNameNotFoundInCache
}
...
func (s *SQLTraceStore) writeTraceIDAndPostings(traceNameAsParams paramtools.Params, tileNumber types.TileNumber) (traceIDFromSQL, error) {
	...
	if traceID, err = s.getTraceIDFromCache(hashedTraceName); err == nil {
