...
var statements = map[statement]string{
	...
	readLastNTraces: `
        SELECT
            t.trace_id,
            ARRAY(
                SELECT val FROM TraceValues
                WHERE trace_id = t.trace_id AND commit_number <= $2
                ORDER BY commit_number DESC
                LIMIT $3
            ) as values,
            ARRAY(
                SELECT commit_number FROM TraceValues
                WHERE trace_id = t.trace_id AND commit_number <= $2
                ORDER BY commit_number DESC
                LIMIT $3
            ) as commits
	...
	readTraces: `
        SELECT
            TraceValues.trace_id, TraceValues.commit_number, TraceValues.val, TraceValues.source_file_id
        FROM
            TraceValues
        JOIN
            unnest($3::bytea[]) as t(trace_id) ON TraceValues.trace_id = t.trace_id
        WHERE
            TraceValues.commit_number >= $1
            AND TraceValues.commit_number <= $2`,
	readTracesNoSourceFile: `
        SELECT
            TraceValues.trace_id, TraceValues.commit_number, TraceValues.val
        FROM
            TraceValues
        JOIN
            unnest($3::bytea[]) as t(trace_id) ON TraceValues.trace_id = t.trace_id
...
// QueryLastNPoints implements the tracestore.TraceStore interface.
func (s *SQLTraceStore) QueryLastNPoints(ctx context.Context, traceNames []string, n int, endCommit types.CommitNumber) (map[string]types.Trace, map[string][]types.CommitNumber, error) {
	ctx, span := trace.StartSpan(ctx, "sqltracestore.QueryLastNPoints")
	defer span.End()
	defer timer.New("QueryLastNPoints").Stop()

	traceIdToNameMap, traceIDsForQuery := s.translateTraceNamesToIds(traceNames)
	if len(traceIDsForQuery) == 0 {
		return map[string]types.Trace{}, map[string][]types.CommitNumber{}, nil
	}

	// Results, protected by mutex.
	var mutex sync.Mutex
	resTraces := map[string]types.Trace{}
	resCommits := map[string][]types.CommitNumber{}
	...
	err := util.ChunkIterParallelPool(ctx, len(traceIDsForQuery), customQueryTracesChunkSize, s.queryTracesPoolSize, func(ctx context.Context, startIdx, endIdx int) error {
		...
		// Execute the query.
		queryCtx, querySpan := trace.StartSpan(ctx, "sqltracestore.QueryLastNPoints.Chunk.ExecuteSQLQuery")

		rows, err := s.db.Query(queryCtx, s.statements[readLastNTraces], chunk, endCommit, n)
		querySpan.End()
		if err != nil {
			return skerr.Wrapf(err, "SQL: %q", s.statements[readLastNTraces])
		}
		defer rows.Close()
...
func readRowsFromReadLastNTraces(rows pgx.Rows, traceIdToNameMap map[types.TraceIDForSQLInBytes]string) (map[string]types.Trace, map[string][]types.CommitNumber, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&traceIDInBytes, &vals, &commits); err != nil {
			return nil, nil, skerr.Wrap(err)
		}
		...
		// Reverse to chronological order - we used order by desc in the sql.
		slices.Reverse(vals)
		slices.Reverse(commits)

		localTraces[traceName] = vals

		commitNumbers := make([]types.CommitNumber, len(commits))
		for i, c := range commits {
			commitNumbers[i] = types.CommitNumber(c)
		}
		localCommits[traceName] = commitNumbers
	}
	if err := rows.Err(); err != nil {
		return nil, nil, skerr.Wrap(err)
	}
	return localTraces, localCommits, nil
}

// translateTraceNamesToIds, given a list of traceNames,
// filters out corp-only traces in a public setting,
// and returns a list of traceIDs, plus a mapping traceID->traceName
func (s *SQLTraceStore) translateTraceNamesToIds(traceNames []string) (map[types.TraceIDForSQLInBytes]string, [][]byte) {
	traceNameMap := map[types.TraceIDForSQLInBytes]string{}
	traceIDsForQuery := [][]byte{}
	...
	for _, traceName := range traceNames {
		...
		traceIDsForQuery = append(traceIDsForQuery, traceIDBytes[:])
	}
	return traceNameMap, traceIDsForQuery
}
...
func (s *SQLTraceStore) readTracesByChannelForCommitRange(ctx context.Context, traceNamesChan <-chan string, beginCommit types.CommitNumber, endCommit types.CommitNumber) (types.TraceSet, []provider.Commit, map[string]*types.TraceSourceInfo, error) {
	...
	if len(commits) == 0 {
		// Empty the traceNames channel to avoid leaking background producer goroutines.
		for range traceNamesChan {
		}
		// Return empty results without error so the caller knows there is no history in this range.
		return types.TraceSet{}, commits, map[string]*types.TraceSourceInfo{}, nil
	}
	...
	for _, name := range traceNames {
		...
		sourceFileMap[name] = types.NewTraceSourceInfo()
	}

	// Pre-calculate commitToIndexMap once for all parallel workers
	commitToIndexMap := make(map[types.CommitNumber]int, len(commits))
	for i, commit := range commits {
		commitToIndexMap[commit.CommitNumber] = i
	}

	// Sort and deduplicate trace IDs to improve database index scan and caching performance.
	slices.SortFunc(traceIDsForQuery, bytes.Compare)
	traceIDsForQuery = slices.CompactFunc(traceIDsForQuery, bytes.Equal)
	...
	err = util.ChunkIterParallelPool(ctx, len(traceIDsForQuery), s.queryTracesChunkSize, s.queryTracesPoolSize, func(ctx context.Context, startIdx, endIdx int) error {
		chunk := traceIDsForQuery[startIdx:endIdx]
		if err := s.readTracesChunk(ctx, beginCommit, endCommit, commits, commitToIndexMap, chunk, traceNameMap, ret, sourceFileMap); err != nil {
...
func (s *SQLTraceStore) readTracesChunk(ctx context.Context, beginCommit types.CommitNumber, endCommit types.CommitNumber, commits []provider.Commit, commitToIndexMap map[types.CommitNumber]int, chunk [][]byte, traceNameMap map[types.TraceIDForSQLInBytes]string, ret types.TraceSet, sourceFileMap map[string]*types.TraceSourceInfo) error {
	if len(chunk) == 0 {
		return nil
	}
	...
	skipSourceInfo := perfgit.SkipMetadataFromContext(ctx)

	stmtToRun := s.statements[readTraces]
	if skipSourceInfo {
		stmtToRun = s.statements[readTracesNoSourceFile]
	}
	...
	rows, err := s.db.Query(queryCtx, stmtToRun, int64(beginCommit), int64(endCommit), chunk)
	querySpan.End()
	if err != nil {
		return skerr.Wrapf(err, "SQL: %q", stmtToRun)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if skipSourceInfo {
			if err := rows.Scan(&traceIDInBytes, &commitNum, &val); err != nil {
				return skerr.Wrap(err)
			}
		} else {
			if err := rows.Scan(&traceIDInBytes, &commitNum, &val, &sourceFileID); err != nil {
				return skerr.Wrap(err)
			}
		}
		...
		traceSlice := ret[traceName]
		traceSource := sourceFileMap[traceName]

		cNum := types.CommitNumber(commitNum)
		idx, ok := commitToIndexMap[cNum]
		if ok {
			if traceSlice != nil {
				traceSlice[idx] = val
			}
			if traceSource != nil && !skipSourceInfo {
				traceSource.Add(cNum, sourceFileID)
			}
		}
	}
	if err := rows.Err(); err != nil {
...
func (s *SQLTraceStore) commitSliceFromCommitNumberRange(ctx context.Context, begin, end types.CommitNumber) ([]provider.Commit, error) {
	...
	stmt := s.statements[getCommitsFromCommitNumberRange]
	skipMetadata := perfgit.SkipMetadataFromContext(ctx)
	if skipMetadata {
		stmt = s.statements[getCommitNumbersFromCommitNumberRange]
	}

	rows, err := s.db.Query(ctx, stmt, begin, end)
	if err != nil {
...
func (s *SQLTraceStore) readTracesForCommitRangesChunk(ctx context.Context, chunkTraceIDs [][]byte, chunkBeginCommits, chunkEndCommits []int64, traceIDToNameMap map[types.TraceIDForSQLInBytes]string, mutex *sync.Mutex, resTraces map[string]types.Trace, resCommits map[string][]types.CommitNumber) error {
	...
	for rows.Next() {
		...
		cNum := types.CommitNumber(commitNumber)
		localTraces[traceName] = append(localTraces[traceName], float32(val))
		localCommits[traceName] = append(localCommits[traceName], cNum)
	}
	if err := rows.Err(); err != nil {
		return skerr.Wrap(err)
	}
