...
type SQLTraceStore struct {
	...
	cacheLenMetrics                 metrics2.Int64Metric
...
func New(db *pgxpool.Pool, datastoreConfig config.DataStoreConfig) (*SQLTraceStore, error) {
	...
	ret := &SQLTraceStore{
		...
		cacheLenMetrics:         metrics2.GetInt64Metric("perfserver_sqltracestore_cache_len"),
	...
	go func() {
		for range time.Tick(cacheMetricsRefreshDuration) {
			ret.cacheLenMetrics.Update(int64(ret.cache.Len()))
