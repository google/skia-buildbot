...
const badTraceIDFromSQL traceIDForSQL = ""
...
var templates = map[statement]string{
	replaceTraceValues: `UPSERT INTO
...
// queryParameter represents a single part of a query, it is used in queryTracesContext.
...
// called.
func New(db *sql.DB, datastoreConfig config.DataStoreConfig) (*SQLTraceStore, error) {
	preparedStatements := map[statement]*sql.Stmt{}
...
func (s *SQLTraceStore) paramSetForTile(tileNumber types.TileNumber) (paramtools.ParamSet, error) {
	...
	rows, err := s.preparedStatements[paramSetForTile].QueryContext(context.TODO(), beginCommit, endCommit)
	if err != nil {
...
func (s *SQLTraceStore) GetSource(ctx context.Context, commitNumber types.CommitNumber, traceName string) (string, error) {
	...
	var b bytes.Buffer
	if err := s.unpreparedStatements[getSource].ExecContext(ctx, &b, sourceContext); err != nil {
		return "", skerr.Wrapf(err, "failed to expand get source template")
	}
	sql := b.String()

	if err := s.db.QueryRowContext(ctx, sql).Scan(&filename); err != nil {
...
func (s *SQLTraceStore) QueryTracesByIndex(ctx context.Context, tileNumber types.TileNumber, q *query.Query) (types.TraceSet, error) {
	...
	var b bytes.Buffer
	if err := s.unpreparedStatements[queryTraces].ExecContext(ctx, &b, context); err != nil {
...
func (s *SQLTraceStore) QueryTracesIDOnlyByIndex(ctx context.Context, tileNumber types.TileNumber, q *query.Query) (<-chan paramtools.Params, error) {
	...
	// Expand the template for the SQL.
	var b bytes.Buffer
	if err := s.unpreparedStatements[queryTracesIDOnly].ExecContext(ctx, &b, context); err != nil {
		return nil, skerr.Wrapf(err, "failed to expand trace names template")
	}

	// Execute the query.
	rows, err := s.db.QueryContext(ctx, b.String())
	if err != nil {
		return nil, skerr.Wrap(err)
	}
...
func (s *SQLTraceStore) ReadTraces(tileNumber types.TileNumber, keys []string) (types.TraceSet, error) {
	...
	// Expand the template for the SQL.
	var b bytes.Buffer
	if err := s.unpreparedStatements[readTraces].ExecContext(context.TODO(), &b, readTracesContext); err != nil {
		return nil, skerr.Wrapf(err, "failed to expand read traces template")
	}

	// Execute the query.
	rows, err := s.db.QueryContext(context.TODO(), b.String())
	if err != nil {
		return nil, skerr.Wrap(err)
	}
...
func (s *SQLTraceStore) WriteTraces(commitNumber types.CommitNumber, params []paramtools.Params, values []float32, _ paramtools.ParamSet, source string, _ time.Time) error {
	...
	if err := s.unpreparedStatements[replaceTraceNames].ExecContext(context.TODO(), &b, namesTemplateContext); err != nil {
		return skerr.Wrapf(err, "failed to expand trace names template")
	}
	sql := b.String()
	if _, err := s.db.ExecContext(context.TODO(), sql); err != nil {
		return skerr.Wrapf(err, "Executing: %q", b.String())
	}

	b.Reset()
	if err := s.unpreparedStatements[replaceTraceValues].ExecContext(context.TODO(), &b, valuesTemplateContext); err != nil {
