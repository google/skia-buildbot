...
func (s *SQLTraceStore) QueryTracesByIndex(ctx context.Context, tileNumber types.TileNumber, q *query.Query) (types.TraceSet, error) {
	...
	// This is safe because the keys and values all come from Postings,
	// which are validated before writing. I.e. the output of query.Plan
	// is built only from things from ops, which comes from
	// Postings, which is validated before writing.
...
WHERE
	...
	for i, key := range keys {
		if !query.ValidateKey(key) {
			sklog.Warningf("Invalid key stored in shortcut: %q", key)
			continue
		}
		singleQuoted := "'" + key + "'"
