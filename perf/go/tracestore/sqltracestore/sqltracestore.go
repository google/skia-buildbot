...
// replaceTraceValuesContext is used in the replaceTraceValues template.
...
// replaceTraceNamesContext represents the values used in the replaceTraceNames template.
...
func (s *SQLTraceStore) GetSource(ctx context.Context, commitNumber types.CommitNumber, traceName string) (string, error) {
	...
	if err := s.preparedStatements[getSource].QueryRowContext(ctx, traceID, commitNumber).Scan(&filename); err != nil {
...
func (s *SQLTraceStore) QueryTracesIDOnlyByIndex(ctx context.Context, tileNumber types.TileNumber, q *query.Query) (<-chan paramtools.Params, error) {
	...
	s.unpreparedStatements[queryTraces].Execute()
...
func (s *SQLTraceStore) WriteTraces(commitNumber types.CommitNumber, params []paramtools.Params, values []float32, _ paramtools.ParamSet, source string, _ time.Time) error {
	...
	var b bytes.Buffer
	if err := s.unpreparedStatements[replaceTraceNames].Execute(&b, namesTemplateContext); err != nil {
		return skerr.Wrapf(err, "failed to expand trace names template")
	}
	if _, err := s.db.ExecContext(context.TODO(), b.String()); err != nil {
		return skerr.Wrap(err)
	}

	if err := s.unpreparedStatements[replaceTraceValues].Execute(&b, valuesTemplateContext); err != nil {
