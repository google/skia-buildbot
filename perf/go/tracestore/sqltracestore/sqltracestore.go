...
// they are md5 hashes of a trace name, e.g. []byte{0x00, 0x11, ...}.
type traceIDForSQL []byte

var badTraceIDFromSQL traceIDForSQL = []byte{}

// Calculates the traceIDForSQL for the given trace name, e.g. []byte{0x00, 0x11, ...}.
func traceIDForSQLFromTraceName(traceName string) traceIDForSQL {
	b := md5.Sum([]byte(traceName))
	return b[:]
}

// hexEncodedTraceIDForSQLFromTraceName encodes the trace_name as a hex encoded
// md5 hash of a trace name, e.g. "\x00112233...". Note the \x prefix which
// tells CockroachDB that this is hex encoded.
func hexEncodedTraceIDForSQLFromTraceName(traceName string) string {
	return `\x` + hex.EncodeToString(traceIDForSQLFromTraceName(traceName))
}
...
func (s *SQLTraceStore) GetSource(ctx context.Context, commitNumber types.CommitNumber, traceName string) (string, error) {
	var filename string
	traceID := hexEncodedTraceIDForSQLFromTraceName(traceName)
...
func (s *SQLTraceStore) ReadTraces(tileNumber types.TileNumber, keys []string) (types.TraceSet, error) {
	...
	for _, traceName := range keys {
		readTracesContext.TraceIDs = append(readTracesContext.TraceIDs, hexEncodedTraceIDForSQLFromTraceName(traceName))
	}
	// Expand the template for the SQL.
	var b bytes.Buffer
...
func (s *SQLTraceStore) sendBatch(ctx context.Context, batch *pgx.Batch) error {
	batchResults := s.db.SendBatch(ctx, batch)

	defer util.Close(batchResults)
	for i := 0; i < batch.Len(); i++ {
		_, err := batchResults.Exec()
		if err != nil {
			return skerr.Wrapf(err, "Batch insert failed.")
		}
	}
	return nil
}
...
func (s *SQLTraceStore) WriteTraces(commitNumber types.CommitNumber, params []paramtools.Params, values []float32, _ paramtools.ParamSet, source string, _ time.Time) error {
	...
	batch := &pgx.Batch{}
	traceNames := make([]string, 0, len(values))
	for i, p := range params {
		...
		batch.Queue(statements[replaceTraceValues], traceID, commitNumber, values[i], sourceID)
		if !s.cache.Exists(string(traceID)) {
			traceNames = append(traceNames, traceName)
			batch.Queue(statements[replaceTraceNames], traceID, p)
		}
		if batch.Len() >= 100 {
