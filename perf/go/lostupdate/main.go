...
func (b *BigTableTraceStore) QueryTraces(tileKey TileKey, q *regexp.Regexp) (map[string][]float32, error) {
	...
	err := b.table.ReadRows(b.ctx, bigtable.PrefixRange(tileKey.TraceRowPrefix()), func(row bigtable.Row) bool {
		sklog.Infof("%v", row[VALUES_FAMILY])
...
func (b *BigTableTraceStore) GetSource(index int32, traceId string) (string, error) {
	tileKey := TileKeyFromOffset(index / b.tileSize)
	row, err := b.table.ReadRow(b.ctx, tileKey.TraceRowName(traceId), bigtable.RowFilter(
		bigtable.ChainFilters(
			bigtable.LatestNFilter(1),
	...
	return string(row[SOURCES_FAMILY][0].Value), nil
}
...
func main() {
	...
	source, err := btts.GetSource(3, key)
	sklog.Infof("Source: %q %s", source, err)

	source, err = btts.GetSource(4, key)
	sklog.Infof("Source: %q %s", source, err)
}
