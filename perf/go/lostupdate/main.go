...
func (b *BigTableTraceStore) clearOPS(tileKey TileKey) error {
	...
	err = b.table.Apply(b.ctx, tileKey.String(), updateMutation)
	if err == nil {
...
func (b *BigTableTraceStore) getOPS(tileKey TileKey) (*OpsCacheEntry, error) {
	b.mutex.RLock()
	entry, ok := b.opsCache[tileKey.String()]
	b.mutex.RUnlock()
	if ok {
		return entry, nil
	}
	b.mutex.Lock()
	defer b.mutex.Unlock()
	row, err := b.table.ReadRow(b.ctx, tileKey.String(), bigtable.RowFilter(bigtable.LatestNFilter(1)))
	if err != nil {
...
func (b *BigTableTraceStore) UpdateOrderedParamSet(tileKey TileKey, p paramtools.ParamSet) (*paramtools.OrderedParamSet, error) {
	...
	for {
		...
		if err := b.table.Apply(b.ctx, tileKey.String(), condUpdate, bigtable.GetCondMutationResult(&condTrue)); err != nil {
...
func main() {
	...
	fmt.Printf("Latest: %d ", latest.Offset())
}
