...
type BigTableTraceStore struct {
	...
	opsCache map[int32]*paramtools.OrderedParamSet // map[tile] -> ops.
}

func NewBigTableTraceStore(tileSize int32, prefix, project, instance string, ts oauth2.TokenSource) (*BigTableTraceStore, error) {
	...
	return &BigTableTraceStore{
		...
		opsCache: map[int32]*paramtools.OrderedParamSet{},
	}, nil
}

func (b *BigTableTraceStore) GetOrderedParamSet(tileKey TileKey) (*paramtools.OrderedParamSet, error) {
	if ops, ok := b.opsCache[tileKey.Offset()]; ok {
		return ops, nil
	}
	ctx := context.Background()
	row, err := b.ops.ReadRow(ctx, tileKey.String(), bigtable.RowFilter(bigtable.LatestNFilter(1)))
	if err != nil {
	...
	/*
		for _, col := range row[OPS_FAMILY] {
			col.Column
		}
	*/
	return nil, nil
...
func main() {
	...
	_, err = NewBigTableTraceStore(1024, "skia", "skia-public", "perf-skia", ts)
	if err != nil {
