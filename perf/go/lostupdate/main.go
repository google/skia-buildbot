...
func main() {
	...
	store, err := btts.NewBigTableTraceStore(ctx, 50, "skia", "skia-public", "perf-bt", ts)
	if err != nil {
	...
	q, err := query.New(url.Values{"name": []string{"A_large_blank_world_map_with_oceans_marked_in_blue.svg"}})
	if err != nil {
		sklog.Fatal(err)
	}
	r, err := q.Regexp(op)
