...
func (b *BigTableTraceStore) GetOrderedParamSet(tileKey TileKey) (*paramtools.OrderedParamSet, error) {
	...
	if len(row) == 0 {
		return paramtools.NewOrderedParamSet(), nil
	}
...
func main() {
	...
	bops, err := btts.GetOrderedParamSet(tileKey)
	if err != nil {
		sklog.Fatal(err)
	}
	buf, err = bops.Encode()
	...
	bopsHash := fmt.Sprintf("%x", md5.Sum(buf))
	if bopsHash != hash {
		sklog.Fatalf("Hash mismatch: want %q, got %q", hash, bopsHash)
	}
