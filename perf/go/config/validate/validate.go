...
func Validate(i config.InstanceConfig) error {
	...
	if i.IssueTrackerConfig.NotificationType == types.IssueNotify {
		if i.IssueTrackerConfig.IssueTrackerAPIKeySecretProject == "" {
			return skerr.Fmt("issue_tracker_api_key_secret_project must be supplied when `notifications` is set to %q", i.IssueTrackerConfig.NotificationType)
		}
		if i.IssueTrackerConfig.IssueTrackerAPIKeySecretName == "" {
			return skerr.Fmt("issue_tracker_api_key_secret_name must be supplied when `notifications` is set to %q", i.IssueTrackerConfig.NotificationType)
		}
	}
