...
func (api triageApi) ListIssues(w http.ResponseWriter, r *http.Request) {
	...
	var issueTitleListRequest perf_issuetracker.ListIssuesRequest
	if err := json.NewDecoder(r.Body).Decode(&issueTitleListRequest); err != nil {
		httputils.ReportError(w, err, "Failed to decode JSON on bug title request.", http.StatusInternalServerError)
		return
	}
	sklog.Debugf("[SkiaTriage] ListIssues request received from frontend: %s", issueTitleListRequest)
	...
	resp, err := api.issueTracker.ListIssues(ctx, issueTitleListRequest)
