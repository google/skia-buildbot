...
// PerfClient is the interface around getting data into Perf's ingestion.
...
// PerfClientImpl implements the PerfClient interface
type PerfClientImpl struct {
	storageClient gcs.GCSClient
	basePath      string
}

// New creates a new PerfClient around the given GCSClient. It will use the
// passed-in basePath as the folder name in the bucket that GCSClient is
// configured for.
func New(basePath string, s gcs.GCSClient) *PerfClientImpl {
	return &PerfClientImpl{
		storageClient: s,
		basePath:      basePath,
	}
}

// See the PerfClient interface for more information
func (pb *PerfClientImpl) PushToPerf(now time.Time, folderName, filePrefix string, data ingestcommon.BenchData) error {
