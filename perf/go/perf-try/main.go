...
func main() {
	...
	for traceID, values := range ts {
		...
		median, lower, upper, err := vec32.TwoSidedStdDev(values[:valuesLength-1])
		if err != nil {
			continue
		}
		...
		if currentValue < median {
			stddevRatio = (median - currentValue) / lower

			if stddevRatio > maxPercentDiff {
				fmt.Printf("values: %v, median: %g, lower: %g, stddevRatio: %g,\n", values, median, lower, stddevRatio)
				stddevRatio = maxPercentDiff
			}
			stddevRatio *= -1
		} else {
			stddevRatio = (currentValue - median) / upper
			if stddevRatio > maxPercentDiff {
				stddevRatio = maxPercentDiff
			}
		}
		...
		if math.IsInf(float64(stddevRatio), 0) {
			stddevRatio = maxPercentDiff
		}
