...
func main() {
	...
	// Do analysis by key=value

	totals := map[string]float32{}
	for _, s := range samples {
		tryValue := s.values[0]
		currentValue := s.values[1]
		if tryValue == vec32.MissingDataSentinel || currentValue == vec32.MissingDataSentinel {
			continue
		}
		percentChange := (tryValue - currentValue) / currentValue
		if math.IsNaN(float64(percentChange)) {
			continue
		}
	...
	for i, top := range sortableTotals[:5] {
		fmt.Printf("%4d %40s %g\n", i+1, top.keyValue, top.total)
	}
	fmt.Println("...")
	for i, top := range sortableTotals[len(sortableTotals)-5:] {
		fmt.Printf("%4d %40s %g\n", len(sortableTotals)-5+i+1, top.keyValue, top.total)
	}
