...
func (t TileKey) TraceRowPrefix(shard int32) string {
	fmt.Printf("TraceRowPrefix: %q\n", fmt.Sprintf("%d:%07d:", shard, t))
...
func (t TileKey) TraceRowName(traceId string, shards int32) string {
	fmt.Printf("TraceRowName: %q\n", fmt.Sprintf("%d:%07d:%s", crc32.ChecksumIEEE([]byte(traceId))%uint32(shards), t, traceId))
