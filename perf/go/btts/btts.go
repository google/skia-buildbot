...
type BigTableTraceStore struct {
	...
	// TODO turn off cache when using from Perf.
	cacheOps bool
	mutex    sync.RWMutex
	opsCache map[string]*OpsCacheEntry // map[tile] -> ops.
}

func NewBigTableTraceStoreFromConfig(ctx context.Context, cfg *config.IngesterConfig, ts oauth2.TokenSource, cacheOps bool) (*BigTableTraceStore, error) {
	if cfg.TileSize <= 0 {
