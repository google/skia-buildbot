...
func (b *BigTableTraceStore) WriteTraces(index int32, values map[string]float32, source, contentHash string) error {
	...
	// Write the hash of the file contents also, which is us implementing IngestionStore.
	sklog.Infof("Content Hash: %q", contentHash)
	mut = bigtable.NewMutation()
	mut.Set(HASHES_FAMILY, HASHES_SOURCE, now, []byte(source))
	b.table.Apply(b.ctx, fmt.Sprintf("&%s", contentHash), mut)
	return nil
}
