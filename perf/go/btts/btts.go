...
func (b *BigTableTraceStore) QueryTracesIDOnlyByIndex(ctx context.Context, tileKey TileKey, q *query.Query) (<-chan paramtools.Params, error) {
	...
	var out <-chan string
	out, err = ExecutePlan(tctx, encodedPlan, b.getTable(), tileKey)
	if err != nil {
		cancel()
		close(outParams)
...
func (b *BigTableTraceStore) QueryTracesByIndex(ctx context.Context, tileKey TileKey, q *query.Query) (types.TraceSet, error) {
	...
	// We start with a fresh context, because we might get handed a http request
	// context which will eventually end and might accidentally kill this request.
	tctx, cancel := context.WithTimeout(context.Background(), TIMEOUT)
	defer cancel()
