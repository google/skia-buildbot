...
// For each tile in the given range of indices, construct a map[int32]int32 that
// maps the offset within that table to its offset within the final Trace.
func (b *BigTableTraceStore) buildMapper(beginIndex, endIndex int32) map[tileKey]map[int32]int32 {
	mapper := map[tileKey]map[int32]int32{}
	for srcIndex := beginIndex; i < endIndex; i++ {
		dstIndex := srcIndex - beginIndex
		tileKey := b.TileKey(srcIndex)
		if m, ok := mapper[tileKey]; !ok {
			mapper[tileKey] = map[int32]int32{srcIndex % b.tileSize: dstIndex}
		} else {
			m[srcIndex%b.tileSize] = dstIndex
		}
	}
	return mapper
}
