...
const (
	...
	// MAX_INDEX_QUERY_RESULTS is the most results we can safely return from a
	// ByIndex query, all others should fall back to table scans.
	MAX_INDEX_QUERY_RESULTS = 10000
)
...
func (b *BigTableTraceStore) QueryTracesByIndex(ctx context.Context, tileKey TileKey, q *query.Query) (types.TraceSet, error) {
	...
	if len(ss.Keys()) > MAX_INDEX_QUERY_RESULTS {
		return nil, fmt.Errorf("Too many rows to query by index, falling back to rowscan: ")
	}
	...
	for {
		...
		numRequests++
		// This should be a worker pool since a bad query may launch 10k Go routines.
		g.Go(func() error {
	...
	sklog.Infof("%d Launched %d parallel requests for query %q", gid, numRequests, q.String())
	if err := g.Wait(); err != nil {
		// TODO(jcgregorio) This can happen if we write too much data into an index row. Maybe index rows need to be sharded?
		return nil, fmt.Errorf("Failed to query : %q", err)
	}
