...
func ExecutePlan(ctx context.Context, plan paramtools.ParamSet, table *bigtable.Table, tileKey TileKey) (<-chan string, error) {
	...
	for key, values := range plan {
		...
		for i, value := range values {
			unionChannels[i] = ParamIndex(ctx, table, tileKey, key, value)
		}
