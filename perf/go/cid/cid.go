...
import (
	...
	perfgit "go.skia.org/infra/perf/go/perf"
...
func New(ctx context.Context, git *perfgit.Git, gitRepoURL string, instanceConfig *config.InstanceConfig) *CommitIDLookup {
	cidl := &CommitIDLookup{
		git:            git,
...
func (c *CommitIDLookup) Lookup(ctx context.Context, cids []*CommitID) ([]*CommitDetail, error) {
	...
	for i, cid := range cids {
		...
		ret[i] = &CommitDetail{
			...
			URL:       urlFromParts(c.gitRepoURL, commit.GitHash, commit.Subject, c.instanceConfig.GitRepoConfig.DebouceCommitURL),
			Hash:      commit.GitHash,
