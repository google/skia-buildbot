...
//
// TODO(jcgregorio) Collapse this into just a types.CommitNumber.
...
func New(ctx context.Context, git *perfgit.Git, gitRepoURL string) *CommitIDLookup {
	cidl := &CommitIDLookup{
		git:        git,
...
func (c *CommitIDLookup) Lookup(ctx context.Context, cids []*CommitID) ([]*CommitDetail, error) {
	...
	for i, cid := range cids {
		...
		ret[i] = &CommitDetail{
			...
			URL:       urlFromParts(c.gitRepoURL, commit.GitHash, commit.Subject, config.Config.GitRepoConfig.DebouceCommitURL),
			Hash:      commit.GitHash,
