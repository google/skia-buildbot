...
// TrybotReference provides a mock function with given fields: local, store, instanceConfig, trybotFilename, outputFilename
func (_m *Application) TrybotReference(local bool, store tracestore.TraceStore, instanceConfig *config.InstanceConfig, trybotFilename string, outputFilename string) error {
	ret := _m.Called(local, store, instanceConfig, trybotFilename, outputFilename)

	var r0 error
	if rf, ok := ret.Get(0).(func(bool, tracestore.TraceStore, *config.InstanceConfig, string, string) error); ok {
		r0 = rf(local, store, instanceConfig, trybotFilename, outputFilename)
	} else {
		r0 = ret.Error(0)
	}
