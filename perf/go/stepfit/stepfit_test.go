...
func TestStepFit(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			value:       []float32{1, 1, 2, 2},
			interesting: 2.0 / 3.0,
			expected:    &StepFit{TurningPoint: 2, StepSize: -2.0 / 3.0, Status: HIGH, Regression: -2.0 / 3.0},
		...
		},
		{
			value:       []float32{1, 1, 1.5, 1.5},
			interesting: 1.0,
			expected:    &StepFit{TurningPoint: 2, StepSize: -0.4, Status: UNINTERESTING, Regression: -0.4},
		...
		},
		{
			value:       []float32{1, 1, 2, 2},
			interesting: 0.2,
			expected:    &StepFit{TurningPoint: 2, StepSize: -100000, Status: HIGH, Regression: -100000},
			message:     "Cohen - step - zero std dev",
			stepDet:     types.COHEN_STEP,
		},
		{
			value:       []float32{1, 2, 2, 3},
			interesting: 0.2,
			expected:    &StepFit{TurningPoint: 2, StepSize: -2, Status: HIGH, Regression: -2},
			message:     "Cohen - step - small std dev",
			stepDet:     types.COHEN_STEP,
		},
		{
			value:       []float32{1, 2, 3, 3, 4},
			interesting: 0.2,
			expected:    &StepFit{TurningPoint: 2, StepSize: -3.4034116, Status: HIGH, Regression: -3.4034116},
			message:     "Cohen - step - odd - big std dev",
			stepDet:     types.COHEN_STEP,
		},
		{
			value:       []float32{1, 1.01, 1, 1.02, 1.01, 1.02},
			interesting: 0.2,
			expected:    &StepFit{TurningPoint: 3, StepSize: -2.309401, Status: HIGH, Regression: -2.309401},
