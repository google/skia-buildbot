...
func testCommitSliceFromTimeRange_Success(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()

	commits, err := g.CommitSliceFromTimeRange(ctx, startTime.Add(1*time.Minute), startTime.Add(3*time.Minute))
	require.NoError(t, err)
...
func testCommitSliceFromTimeRange_ZeroWidthRangeReturnsZeroResults(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()

	commits, err := g.CommitSliceFromTimeRange(ctx, startTime.Add(1*time.Minute), startTime.Add(1*time.Minute))
	require.NoError(t, err)
	assert.Len(t, commits, 0)
}

func testCommitSliceFromTimeRange_NegativeWidthRangeReturnsZeroResults(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()

	commits, err := g.CommitSliceFromTimeRange(ctx, startTime.Add(2*time.Minute), startTime.Add(1*time.Minute))
	require.NoError(t, err)
	assert.Len(t, commits, 0)
}

func testCommitSliceFromCommitNumberRange_Success(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()
...
func testCommitSliceFromCommitNumberRange_ZeroWidthReturnsOneResult(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()
...
func testCommitSliceFromCommitNumberRange_NegativeWidthReturnsZeroResults(t *testing.T, ctx context.Context, g *Git, gb *testutils.GitBuilder, hashes []string, cleanup cleanupFunc) {
	unittest.LargeTest(t)
	defer cleanup()
