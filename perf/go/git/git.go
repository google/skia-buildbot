...
func New(ctx context.Context, local bool, db *sql.DB, dialect perfsql.Dialect, instanceConfig *config.InstanceConfig) (*Git, error) {
	...
	// to the database.
	if err := ret.singleUpdateStep(ctx); err != nil {
...
func parseGitRevLogStream(r io.ReadCloser, f parseGitRevLogStreamProcessSingleCommit) error {
	...
	for scanner.Scan() {
		...
		if err := f(Commit{types.BadCommitNumber, gitHash, ts, author, subject}); err != nil {
...
func pull(ctx context.Context, gitFullPath, dir string) error {
	...
	if err := cmd.Run(); err != nil {
		exerr := err.(*exec.ExitError)
		sklog.Errorf("Failed to pull repo %q with git %q: %s - %s", dir, gitFullPath, err, exerr.Stderr)
		return skerr.Wrap(err)
	}
	return nil
}

// singleUpdateStep does a git pull and then finds all the new commits
// added to the repo since our last singleUpdateStep.
//
...
func (g *Git) singleUpdateStep(ctx context.Context) error {
...
func (g *Git) GitHashFromCommitNumber(ctx context.Context, commitNumber types.CommitNumber) (string, error) {
	...
	if err := g.preparedStatements[getHashFromCommitNumber].QueryRowContext(ctx, commitNumber).Scan(&ret); err != nil {
		return "", skerr.Wrapf(err, "Failed to find git hash for commit number: %d", commitNumber)
	}
	return ret, nil
}
...
func (g *Git) CommitNumbersWhenFileChangesInCommitNumberRange(ctx context.Context, begin, end types.CommitNumber, filename string) ([]types.CommitNumber, error) {
	beginHash, err := g.GitHashFromCommitNumber(ctx, begin)
	if err != nil {
	...
	cmd := exec.CommandContext(ctx, g.gitFullPath, "log", beginHash+".."+endHash, "--reverse", "--format=format:%H", "--", filename)
	sklog.Error(cmd.String())
	cmd.Dir = g.instanceConfig.GitRepoConfig.Dir
	...
	if err := cmd.Wait(); err != nil {
		exerr := err.(*exec.ExitError)
		return nil, skerr.Wrapf(err, "Failed to get logs: %s - %s", err, exerr.Stderr)
	}
