...
import (
	...
	"go.skia.org/infra/perf/go/alerts/alertstore/alertstoreds"
...
// NewAlertStoreFromConfig creates a new alertstore.AlertStore from the InstanceConfig.
//
// If local is true then we aren't running in production.
func NewAlertStoreFromConfig(local bool, cfg *config.InstanceConfig) (alertstore.AlertStore, error) {
	if local {
		// Should we forcibly change the namespace?
	}
	return alertstoreds.New(), nil
}
