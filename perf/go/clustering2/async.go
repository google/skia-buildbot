...
type dataFrameFromQuery func(cids []*cid.CommitID) (*dataframe.DataFrame, error)
...
func calcCids(request *ClusterRequest, v vcsinfo.VCS, dff dataFrameFromQuery) ([]*cid.CommitID, error) {
	cids := []*cid.CommitID{}
	if request.Sparse {
		// [begin, end)
...
func (p *ClusterRequestProcess) Run() {
	...
	dff := func(cids []*cid.CommitID) (*dataframe.DataFrame, error) {
		return dataframe.NewFromCommitIDsAndQuery(cids, p.cidl, ptracestore.Default, q, nil)
	}

	p.cids, err = calcCids(p.request, p.git, dff)
	if err != nil {
