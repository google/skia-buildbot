	...
	if algo == KMeans {
		...
		for key, trace := range df.TraceSet {
			...
			if sf.Status == stepfit.LOW {
				if ret.Clusters[0].StepFit == nil {
					ret.Clusters[0].StepFit = sf
					ret.Clusters[0].StepPoint = df.Header[sf.TurningPoint]
					ret.Clusters[0].Centroid = trace
				}
				ret.Clusters[0].Num++
				ret.Clusters[0].Keys = append(ret.Clusters[0].Keys, key)
			} else if sf.Status == stepfit.HIGH {
				if ret.Clusters[1].StepFit == nil {
					ret.Clusters[1].StepFit = sf
					ret.Clusters[1].StepPoint = df.Header[sf.TurningPoint]
					ret.Clusters[1].Centroid = trace
				}
				ret.Clusters[1].Num++
				ret.Clusters[1].Keys = append(ret.Clusters[0].Keys, key)
			}
		}
		ret.Clusters[0].ParamSummaries = getParamSummariesForKeys(ret.Clusters[0].Keys)
		ret.Clusters[1].ParamSummaries = getParamSummariesForKeys(ret.Clusters[1].Keys)
		return ret, nil
	} else {
