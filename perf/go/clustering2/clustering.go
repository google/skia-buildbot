...
func FilterTail(trace []float32) float32 {
	...
	// TODO(liyuqian) make these configurable
	const QUANTILE = 0.1
	const SLACK = 1.1

	n := len(sortedTrace)
	p := int(math.Floor(float64(n - 1) * QUANTILE))
	lowerBound := sortedTrace[p]
	upperBound := sortedTrace[n - 1 - p]

	tail64 := float64(tail)
	if tail64 > upperBound * SLACK || tail64 < lowerBound * SLACK {
		return tail
	} else {
		return 0
	}
		...
		for key, trace := range df.TraceSet {
			...
			if isLow {
				...
				if high.StepFit.Status == "" {
					high.StepFit = sf
					low.StepFit.Status = stepfit.HIGH // for TAIL_ALGO
