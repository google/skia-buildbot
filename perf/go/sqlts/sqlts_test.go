...
func newTestDBs(t *testing.T) ([]*SQLTraceStore, cleanup) {
	ret := []*SQLTraceStore{}
	...
	s, err := New(db, SQLiteDialect, 8)
	require.NoError(t, err)

	ret = append(ret, s)

	// Create an Postgres test database.
	...
	s, err = New(db, PostgresDialect, 8)
	require.NoError(t, err)
...
func newPopulatedTestDBs(t *testing.T) ([]*SQLTraceStore, cleanup) {
	traceStores, cleanup := newTestDBs(t)

	for _, s := range traceStores {
...
func TestReadTraces(t *testing.T) {
	traceStores, cleanup := newPopulatedTestDBs(t)
	defer cleanup()
