...
func newCockroachTestDB(t *testing.T) (*SQLTraceStore, cleanup) {
 ...
 `)
	...
	err = migrations.Up("../../migrations/cockroachdb", "cockroach://root@localhost:26257/perftest?sslmode=disable")
	assert.NoError(t, err)
	...
	return s, func() {
		err := migrations.Down("../../migrations/cockroachdb", "cockroach://root@localhost:26257/perftest?sslmode=disable")
		assert.NoError(t, err)
...
func TestSQLite(t *testing.T) {
	...
	for name, f := range subTestsEmptyStore {
		t.Run(name, func(t *testing.T) {
			s, cleanup := newSQLiteTestDB(t)
			defer cleanup()
			f(t, s)
		})
	}

	for name, f := range subTestsPopulatedStore {
		t.Run(name, func(t *testing.T) {
			s, cleanup := newSQLiteTestDB(t)
			populatedTestDB(t, s)
			defer cleanup()
...
func TestCockroachDB(t *testing.T) {
	...
	for name, f := range subTestsEmptyStore {
		t.Run(name, func(t *testing.T) {
			s, cleanup := newCockroachTestDB(t)
			defer cleanup()
			f(t, s)
		})
	}
