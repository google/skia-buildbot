...
var statementsByDialect = map[dialect]statements{
	sqliteDialect: statements{
		...
		insertIntoTraceIDs: `INSERT INTO TraceIDs (trace_name) VALUES (?)`,
		getTraceID:         `SELECT trace_id FROM TraceIDs WHERE trace_name=?`,

		insertIntoPostings: `INSERT INTO Postings (tile_number, key_value, trace_id) VALUES (?, ?, ?)`,

		replaceTraceValues: `INSERT OR REPLACE INTO TraceValues (tile_number, trace_id, commit_number, val, source_file_id) VALUES( ?, ?, ?, ?, ?)`,

		replacePostings: `INSERT OR REPLACE INTO Postings (tile_number, key_value, trace_id)`,
	},
}
...
func (s *SQLTraceStore) updateSourceFile(source string) (int64, error) {
	...
	// Would it be faster to do a get first and then only replace if not found?
	ret := int64(-1)
	res, err := s.insertIntoSourceFiles.Exec(source)
	if err != nil {
		// TODO(jcgregorio) This is sqlite3 specific, need a dialect agnostic solution.
...
func (s *SQLTraceStore) updateTraceID(p paramtools.Params) (int64, error) {
	...
	if err != nil {
		...
		if e, ok := err.(sqlite3.Error); ok {
			...
			err = s.getSourceFileID.QueryRow(traceID).Scan(&ret)
			return ret, err
		}
		return ret, err
	}
