...
func (s *SQLTraceStore) ReadTraces(tileNumber types.TileNumber, keys []string) (map[string][]float32, error) {
	...
	stmt := `SELECT trace_id, trace_name FROM TraceIDS WHERE `
	whereClauses := make([]string, len(keys))
	for i, key := range keys {
		whereClauses[i] = fmt.Sprintf(`trace_name=%q`, key)
	}
	rows, err := s.db.Query(stmt + strings.Join(whereClauses, " OR "))
	if err != nil {
		return nil, err
	}
	mapIDToName := map[int64]string{}
	traceIDs := make([]int64, len(keys))
	i := 0
	for rows.Next() {
		var traceID int64
		var traceName string
		if err := rows.Scan(&traceID, &traceName); err != nil {
			return nil, err
		}
		mapIDToName[traceID] = traceName
		traceIDs[i] = traceID
		i++
	}
	...
	sort.Sort(int64Slice(traceIDs))
	stmt = fmt.Sprintf(`SELECT trace_id, commit_number, val FROM TraceValues WHERE tile_number=%d AND (`, tileNumber)
	for i, id := range traceIDs {
		whereClauses[i] = fmt.Sprintf(`trace_id=%d`, id)
	}

	fullQuery := stmt + strings.Join(whereClauses, " OR ") + ")"
	fmt.Printf(fullQuery)
	rows, err = s.db.Query(fullQuery)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		var val float64
		if err := rows.Scan(&traceID, &commitNumber, &val); err != nil {
			return nil, err
		}
		ret[mapIDToName[traceID]][commitNumber%int64(s.tileSize)] = float32(val)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}

	// Load all the values for each traceID.
	return ret, nil
}
