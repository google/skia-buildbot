...
var statementsByDialect = map[Dialect]statements{
	SQLiteDialect: {
		...
		countIndices:          `SELECT COUNT(*) FROM Postings WHERE tile_number=?`,
		getLatestTile:         `SELECT tile_number FROM Postings ORDER BY tile_number DESC LIMIT 1`,
		paramSetForTile:       `SELECT DISTINCT key_value FROM Postings WHERE tile_number=?`,
		getSource: `SELECT SourceFiles.source_file FROM TraceIDs
		...
		traceCount: `SELECT COUNT(DISTINCT trace_id) FROM TraceValues
		WHERE
		  commit_number >= ?
		  AND commit_number <= ?`,
	},
