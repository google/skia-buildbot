...
const intersectChannelSize = 1000
...
func (s *SQLTraceStore) paramSetForTile(tileNumber types.TileNumber) (paramtools.ParamSet, error) {
	// Should we have a short-lived cache for this?
	// Or should the caching happen at a higher level after we've built a query plan?
...
func (s *SQLTraceStore) QueryTracesByIndex(ctx context.Context, tileNumber types.TileNumber, q *query.Query) (types.TraceSet, error) {
	...
	intersectCh := make([]chan int64, len(plan))
	for i := range intersectCh {
		intersectCh[i] = make(chan int64, intersectChannelSize)
	}

	count := 0
	var g errgroup.Group
	// Find the list of trace names that match the query in this tile.
	for key, values := range plan {
		key := key
		values := values
		index := count
		// Create quuery over Postings that gets all trace_ids.
		g.Go(func() error {
			stmt := fmt.Sprintf(`SELECT trace_id FROM Postings WHERE tile_number=%d AND ( `, tileNumber)
			whereClauses := make([]string, len(values))
			for i, value := range values {
				whereClauses[i] = fmt.Sprintf("%s=%q", key, value)
			}
			fullStatement := stmt + strings.Join(whereClauses, " OR ") + ")"
	...
	out := engine.NewIntersect(ctx, intersectCh)

	traceIds := []int64{}
	for traceID := range out {
		traceIds = append(traceIds, traceID)
	}

	for _, ch := range intersectCh {
		close(ch)
	}
	...
	// Intersect all the trace_ids.

	// Convert trace_ids to keys.

	// Pass the rest of the work off to ReadTraces.
	traces, err := s.ReadTraces(tileNumber, keys)
	return types.TraceSet(traces), err
}
...
func (s *SQLTraceStore) ReadTraces(tileNumber types.TileNumber, keys []string) (map[string][]float32, error) {
	...
	stmt := fmt.Sprintf(`SELECT TraceIDs.trace_name, TraceValues.commit_number, TraceValues.val FROM TraceIDs
	INNER JOIN TraceValues ON TraceValues.trace_id = TraceIDs.trace_id
	WHERE TraceValues.commit_number>=%d AND TraceValues.commit_number<=%d AND (`, beginCommit, endCommit)
	fmt.Println(beginCommit, endCommit)
...
func (s *SQLTraceStore) updateTraceValues(traceID int64, commitNumber types.CommitNumber, x float32, sourceID int64) error {
	_, err := s.replaceTraceValues.Exec(traceID, commitNumber, x, sourceID)
	return err
}
