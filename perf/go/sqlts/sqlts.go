...
const (
	...
	// PostgresDialect covers both Postgres and CockroachDB.
	PostgresDialect
)
...
const (
	// TODO(jcgregorio) createTables should eventually be migrated out of this
	// module since we will also have tables for Shortcuts, Alerts, and
	// Regressions.
	createTables = iota
...
// New returns a new *SQLTraceStore that implements types.TraceStore on
// top of SQLite.
//
// The filename is the name of the sqlite3 database, which will be created if
// not present.
func New(db *sql.DB, dialect Dialect, tileSize int32) (*SQLTraceStore, error) {
...
// Confirm that *SQLTraceStore fulfills the types.TraceStore interface.
