...
// _new builds a DataFrame for the given columns and populates it with traces that match the given query.
//
// The progress callback is triggered once for every tile.
func (b *builder) _new(colHeaders []*dataframe.ColumnHeader, indices []int32, q *query.Query, progress types.Progress, skip int) (*dataframe.DataFrame, error) {
...
func (b *builder) NewN(progress types.Progress, n int) (*dataframe.DataFrame, error) {
	...
	return b._new(colHeaders, indices, q, progress, skip)
}
