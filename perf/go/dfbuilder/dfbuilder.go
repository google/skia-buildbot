...
func (b *builder) _new(colHeaders []*ColumnHeader, indices []int32, q *query.Query, progress ptracestore.Progress) (*dataframe.DataFrame, error) {
	defer timer.New("btts_new").Stop()
	mapper := buildTraceMapper(indices, b.store)
	var wg sync.WaitGroup
	// Replace with errgroup.
	for tileKey, traceMap := range mapper {
		wg.Add(1)
		go func(tileKey btts.TileKey, traceMap map[int32]int32) {
			traces, err := b.store.QueryTraces(tileKey, q)
			defer wg.Done()
		}(tileKey, traceMap)
	}
...
func (b *builder) NewFromCommitIDsAndQuery(ctx context.Context, cids []*cid.CommitID, cidl *cid.CommitIDLookup, q *query.Query, progress ptracestore.Progress) (*dataframe.DataFrame, error) {
	...
	b._new(colHeaders, indices, q, progress)
	return nil, nil
}
