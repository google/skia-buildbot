// Package dirsource implements the file.Source interface.
package dirsource
...
func (d *DirSource) Start(ctx context.Context) (<-chan file.File, error) {
	if d.started {
	...
	go func() {
		...
		if err != nil {
			sklog.Errorf("Error walking the path %q: %s", d.dir, skerr.Wrap(err))
		}
	}()
