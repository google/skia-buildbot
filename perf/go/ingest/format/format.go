...
// FileFormat is the struct for decoding ingestion files for all cases
...
func Parse(r io.Reader) (FileFormat, error) {
	var fileFormat FileFormat
	if err := json.NewDecoder(r).Decode(&fileFormat); err != nil {
		return FileFormat{}, skerr.Wrap(err)
	}
	if fileFormat.Version != fileFormatVersion {
		return FileFormat{}, ErrFileWrongVersion
	}
