...
// getParamsAndValuesFromFormat returns two parallel slices, each slice contains
// the params and then the float for a single value of a trace.
func getParamsAndValuesFromFormat(f format.Format) ([]paramtools.Params, []float64) {
...
func (p *Parser) Parse(file file.File) ([]paramtools.Params, []float64, string, error) {
	...
	// Read the whole contentx into bytes.Reader since we may take more than one
	// pass at the data.
