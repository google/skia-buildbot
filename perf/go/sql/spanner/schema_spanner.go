...
const Schema = `CREATE SEQUENCE Alerts_seq bit_reversed_positive;
CREATE SEQUENCE SourceFiles_seq bit_reversed_positive;
CREATE TABLE IF NOT EXISTS Alerts (
  id INT PRIMARY KEY DEFAULT nextval('Alerts_seq'),
  alert TEXT,
...
CREATE TABLE IF NOT EXISTS SourceFiles (
  source_file_id INT PRIMARY KEY DEFAULT nextval('SourceFiles_seq'),
...
CREATE INDEX by_revision on Culprits (revision, host, project, ref);
CREATE INDEX by_user_id on Favorites (user_id);
CREATE INDEX by_tile_number on ParamSets (tile_number DESC);
CREATE INDEX by_trace_id on Postings (tile_number, trace_id, key_value);
CREATE INDEX by_key_value on Postings (tile_number, key_value);
CREATE INDEX by_alert_id on Regressions2 (alert_id);
CREATE INDEX by_commit_alert on Regressions2 (commit_number, alert_id);
