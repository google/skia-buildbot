...
// NewCockroachDBForTests creates a new temporary sqlite3 database with all
// migrations applied for testing. It also returns a function to call to clean
// up the database after the tests have completed.
//
...
func NewCockroachDBForTests(t *testing.T, databaseName string) (*sql.DB, Cleanup) {
	...
	migrationsConnection := fmt.Sprintf("cockroach://root@%s/%s?sslmode=disable", GetCockroachDBEmulatorHost(), databaseName)
	...
	// uses.
	connectionString := fmt.Sprintf("postgresql://root@%s/%s?sslmode=disable", GetCockroachDBEmulatorHost(), databaseName)
