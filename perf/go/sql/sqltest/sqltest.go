...
func NewCockroachDBForTests(t *testing.T, databaseName string, applyMigrations ApplyMigrationsOption) (*pgx.Conn, Cleanup) {
	...
	cfg, err := pgx.ParseConnectionString(connectionString)
	require.NoError(t, err)
