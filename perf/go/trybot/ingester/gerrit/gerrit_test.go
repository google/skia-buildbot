...
func parserForTest(t *testing.T, filename string) (*parser.Parser, file.File) {
	...
	ret := parser.New(instanceConfig)

	// Load the sample data file as BenchData.
	r := testutils.MustGetReader(filename)
...
func TestGerrit_InvalidTryBotFile_DoesNotProduceTryFile(t *testing.T) {
	...
	parser, invalidFile := parserForTest(t, "invalid.json")
	ingester := New(parser)
...
func TestGerrit_InvalidPatchNumber_DoesNotProduceTryFile(t *testing.T) {
	...
	parser, invalidFile := parserForTest(t, "invalid_patch_number.json")
	ingester := New(parser)
	ingester.parseCounter.Reset()
	ingester.parseFailCounter.Reset()
	fileCh := make(chan file.File)
...
func TestGerrit_ValidTryBotFile_Success(t *testing.T) {
	...
	parser, invalidFile := parserForTest(t, filename)
	ingester := New(parser)
	ingester.parseCounter.Reset()
	ingester.parseFailCounter.Reset()
	fileCh := make(chan file.File)
