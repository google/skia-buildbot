...
func TestLoad_HappyPath_Success(t *testing.T) {
	...
	sourceLoader := New(traceStore, gcsclient, ingestParser())
...
func TestLoad_AskForUnknownTraceID_ReturnsEmptySlice(t *testing.T) {
	...
	sourceLoader := New(traceStore, gcsclient, ingestParser())
	sourceFileNames, err := sourceLoader.Load(ctx, []string{unknownTraceID}, 1)
	require.NoError(t, err)
...
func TestLoad_GetLatestTileFail_Failure(t *testing.T) {
	...
	sourceLoader := New(traceStore, gcsclient, ingestParser())
	_, err := sourceLoader.Load(ctx, []string{traceID1}, 1)
