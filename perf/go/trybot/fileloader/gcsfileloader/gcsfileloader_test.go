...
func TestGetSamples_Success(t *testing.T) {
	...
	expected := parser.SamplesSet{
		",config=565,test=mytest,": parser.Samples{
			Params: paramtools.Params{"config": "565", "test": "mytest"},
			Values: []float64{0.199286937713623, 0.1990699768066406, 0.1992120742797852},
		},
		",config=8888,test=mytest,": parser.Samples{
			Params: paramtools.Params{"config": "8888", "test": "mytest"},
...
func TestGetSamples_FileReaderFails_Failure(t *testing.T) {
	unittest.SmallTest(t)
	ctx := context.Background()
	const sourceFilePath = "path/file.json"
	const sourceFileName = "gs://skia-perf/" + sourceFilePath
	gcsclient := &test_gcsclient.GCSClient{}
	gcsclient.On("FileReader", testutils.AnyContext, sourceFilePath).Return(nil, fmt.Errorf("soemthing broke"))

	g := New(gcsclient, ingestParser())
	_, err := g.GetSamples(ctx, sourceFileName)
	require.Contains(t, err.Error(), "Failed to load from storage")
}

func TestGetSamples_InvalidFilenameURL_Failure(t *testing.T) {
	unittest.SmallTest(t)
	ctx := context.Background()
	gcsclient := &test_gcsclient.GCSClient{}
	g := New(gcsclient, ingestParser())
	_, err := g.GetSamples(ctx, "::: not a valid url :::")
	require.Contains(t, err.Error(), "Failed to parse filename")
}

func TestGetSamples_InvalidJSON_Failure(t *testing.T) {
	unittest.SmallTest(t)
	ctx := context.Background()
