...
func (s *Store) store(cid *cid.CommitDetail, r *Regressions) error {
	...
	_, err = db.DB.Exec("INSERT INTO regression (cid, timestamp, triaged, body) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE  cid=?, timestamp=?, triaged=?, body=?",
