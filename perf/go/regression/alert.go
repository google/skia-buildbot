...
func RegressionsForAlert(
	...
	regressionStateCallback RegressionStateCallback,
) {
	...
	for step, q := range queries {
		if regressionStateCallback != nil {
			regressionStateCallback(fmt.Sprintf("Step %d/%d\nQuery: %q", step+1, len(queries), q))
		}
		...
		_, err := Run(ctx, req, perfGit, cidl, dfBuilder, shortcutStore, clusterResponseProcessor, regressionStateCallback)
		if err != nil {
