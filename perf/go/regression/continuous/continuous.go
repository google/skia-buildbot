...
func (c *Continuous) Run(ctx context.Context) {
	...
	for cnp := range c.buildConfigAndParamsetChannel() {
		...
		for _, cfg := range cnp.configs {
			...
			req.Domain = domain
			req.Query = cfg.Query
			if err := regression.NewRunningProcess(ctx, req, clusterResponseProcessor, c.perfGit, c.shortcutStore, c.dfBuilder); err != nil {
				sklog.Errorf("Failed to start detection request to detector: %s", err)
				continue
			}
