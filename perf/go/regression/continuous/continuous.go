...
func matchingConfigsFromTraceIDs(traceIDs []string, configs []*alerts.Alert) []*alerts.Alert {
	...
	for _, config := range configs {
		...
		// Should we deterine if the Alert is a GroupBy, and then see if those
		// Group By params appear in the traceids, and then produce configs
		// with sub-queries that match the traces and the GroupBy.
