...
func (c *Continuous) Run(ctx context.Context) {
	...
	for range time.Tick(time.Second) {
		...
		// XXX Here is our first problem, this will change with LastN.
		...
		for _, commit := range indexCommits {
			...
			// XXX details isn't need by the core clustering algo.
			...
			for _, cfg := range configs {
				...
				// Handle GroupBy, each query would produce a different NewNFrom*
				...
				for _, q := range queries {
					...
					// Here either loop over the indexCommits or just drop the old algo and go with LastN all the time.
					...
					for _, cl := range resp.Summary.Clusters {
						// commit.Index is actually the midpoint header of the resp.Frame.
						if cl.StepPoint.Offset == int64(commit.Index) {
							if cl.StepFit.Status == stepfit.LOW && len(cl.Keys) >= cfg.MinimumNum && (cfg.Direction == alerts.DOWN || cfg.Direction == alerts.BOTH) {
								...
								// Compute detailsl from commit.Index here.
