...
func (c *Continuous) buildConfigAndParamsetChannel() <-chan configsAndParamSet {
	...
	if c.eventDriven {
		...
		if err != nil {
			sklog.Errorf("Failed to create pubsub subscription, not doing event driven regression detection", err)
		} else {
			...
			go func() {
				for {
					...
					err := sub.Receive(context.Background(), func(ctx context.Context, msg *pubsub.Message) {
						...
						// TODO(jcgregorio) Send keys in event, not Params.
						...
						for _, config := range configs {
							// TODO(jcgregorio) Add query.NewFromString().
