...
// RegressionStateCallback if a func that's called to return information on a currently running process.
type RegressionStateCallback func(message string)
...
func (c *Continuous) setCurrentStep(message string) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
