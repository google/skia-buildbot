...
func singleAlert(ctx context.Context, cfg *alerts.Config, ps paramtools.ParamSet, clusterResponseProcessor func([]*ClusterResponse, *alerts.Config, string), numContinuous int, git *gitinfo.GitInfo, cidl *cid.CommitIDLookup, dfBuilder dataframe.DataFrameBuilder) {
...
func (c *Continuous) Run(ctx context.Context) {
	...
	for range time.Tick(time.Second) {
		...
		for _, cfg := range configs {
			...
			singleAlert(ctx, cfg, c.paramsProvider(), clusterResponseProcessor, c.numCommits, c.git, c.cidl, c.dfBuilder)
		}
