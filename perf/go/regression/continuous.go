	...
	for range time.Tick(time.Second) {
		...
		for _, cfg := range configs {
			...
			if cfg.GroupBy != "" {
				...
				df, err := c.dfBuilder.NewNFromQuery(context.Background(), time.Now(), q, 20, nil)
				if err != nil {
					sklog.Warningf("Alert failed smoketest: %q Failed while trying generic query: %s", cfg.DisplayName, err)
					continue
				}
				if len(df.TraceSet) == 0 {
					sklog.Warningf("Alert failed smoketest: %q Failed to get any traces for generic query: %s", cfg.DisplayName, err)
					continue
				}
