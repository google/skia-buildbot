...
func (c *Continuous) buildConfigAndParamsetChannel() <-chan configsAndParamSet {
	...
	if c.eventDriven {
		...
		if err != nil {
			sklog.Errorf("Failed to create pubsub subscription, not doing even driven regression detection", err)
		}
		...
		go func() {
			for {
				...
				err := sub.Receive(context.Background(), func(ctx context.Context, msg *pubsub.Message) {
					...
					keys := make([]string, 0, len(ie.Params))
					for _, p := range ie.Params {
					...
					returnedConfigs := []*alerts.Config{}
					for _, config := range configs {
						parsed, err := url.Parse(config.Query)
						if err != nil {
							continue
						}
						q, err := query.New(parsed.Query())
						if err != nil {
							continue
						}
						for _, key := range keys {
							if q.Matches(key) {
								returnedConfigs = append(returnedConfigs, config)
								break
							}
						}
					}

					if len(returnedConfigs) > 0 {
