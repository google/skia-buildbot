...
func (v vcsmock) Update(ctx context.Context, pull bool, allBranches bool) error { return nil }
func (v vcsmock) From(start time.Time) []string                                 { return nil }
func (v vcsmock) Details(ctx context.Context, hash string, includeBranchInfo bool) (*vcsinfo.LongCommit, error) {
	return nil, nil
}
func (v vcsmock) DetailsMulti(ctx context.Context, hashes []string, includeBranchInfo bool) ([]*vcsinfo.LongCommit, error) {
	return nil, nil
}
func (v vcsmock) LastNIndex(N int) []*vcsinfo.IndexCommit { return nil }

// Range is the only mock func implemented because that's the only
func (v vcsmock) Range(begin time.Time, end time.Time) []*vcsinfo.IndexCommit {
...
func (v vcsmock) IndexOf(ctx context.Context, hash string) (int, error)           { return -1, nil }
func (v vcsmock) ByIndex(ctx context.Context, N int) (*vcsinfo.LongCommit, error) { return nil, nil }
func (v vcsmock) GetFile(ctx context.Context, fileName string, commitHash string) (string, error) {
	return "", nil
}
...
func ReadModifyWrite_StartWithEmptyAndFailOnMustExist(t *testing.T, store *SQLRegressionStore) {
	ctx := context.Background()
	err := store.readModifyWrite(ctx, 1, "1", true, func(r *regression.Regression) {
		r.HighStatus.Status = regression.POSITIVE
		r.HighStatus.Message = "my bad"
	})
	require.Error(t, err)
