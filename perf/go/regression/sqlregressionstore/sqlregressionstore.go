...
func (s *SQLRegressionStore) write(ctx context.Context, commitNumber types.CommitNumber, alertIDString string, r *regression.Regression, lookup regression.DetailLookup) error {
	...
	s.preparedStatements[write].ExecContext(ctx, commitNumber, alertID, string(b))
	return nil
}
