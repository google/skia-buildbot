...
func TestStore(t *testing.T) {
	...
	// Set expectations.
	mock.ExpectExec("INSERT INTO regression").WithArgs(c.ID(), c.Timestamp, r.Triaged(), body).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestLoad(t *testing.T) {
	...
	reg, err := st.load(&c)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(reg.ByQuery))
	assert.False(t, reg.Triaged())
