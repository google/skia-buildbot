...
func ProcessRegressions(ctx context.Context,
	...
	for index, req := range allRequests {
		...
		if err != nil {
			req.Progress.Message("Info", fmt.Sprintf("Failed to create iterator for query: %q: %s", req.Query(), err))
			continue
		}
...
//
// TODO - Get rid of the bool, switch to enum.
func allRequestsFromBaseRequest(req *RegressionDetectionRequest, ps paramtools.ReadOnlyParamSet, expandBaseRequest bool) []*RegressionDetectionRequest {
	ret := []*RegressionDetectionRequest{}

	if req.Alert.GroupBy == "" || !expandBaseRequest {
		ret = append(ret, req)
	} else {
