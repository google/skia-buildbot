...
func (s *singleIterator) Value(ctx context.Context) (*dataframe.DataFrame, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Invalid range of commits.", err)
	}
	return df, err
}

func NewSingleDataFrameIterator(q *query.Query, begin int, end int, progress types.Progress, cidl *cid.CommitIDLookup, v vcsinfo.VCS, request *ClusterRequest, dfBuilder dataframe.DataFrameBuilder) *singleIterator {
