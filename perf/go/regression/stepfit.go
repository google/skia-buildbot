...
func StepFit(ctx context.Context, df *dataframe.DataFrame, k int, stddevThreshold float32, progress clustering2.Progress, interesting float32, stepDetection types.StepDetection, rule *alerts.AnomalyDetectionRule) (*clustering2.ClusterSummaries, error) {
	...
	for key, trace := range df.TraceSet {
		count++
		sf := stepfit.EvaluateRule(trace, stddevThreshold, rule)
