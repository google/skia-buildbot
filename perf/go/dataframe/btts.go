...
// the return values.
func fromIndexCommit(resp []*vcsinfo.IndexCommit, skip int) ([]*ColumnHeader, []int32, int) {
...
func lastNCommits(vcs vcsinfo.VCS, n int) ([]*ColumnHeader, []int32, int) {
...
func fromTimeRange(vcs vcsinfo.VCS, begin, end time.Time, downsample bool) ([]*ColumnHeader, []int32, int) {
...
func buildTraceMapper(commitIDs []*cid.CommitID, store btts.BigTableTraceStore) TileMap {
	ret := TileMap{}
	for targetIndex, commitID := range commitIDs {
		tileKey := store.TileKey(int32(commitID.Offset))
		if tm, ok := ret[tileKey]; !ok {
			ret[tileKey] = map[int32]int32{
				store.OffsetFromIndex(int32(commitID.Offset)): int32(targetIndex),
			}
		} else {
			tm[store.OffsetFromIndex(int32(commitID.Offset))] = int32(targetIndex)
		}
	}
	return ret
}
