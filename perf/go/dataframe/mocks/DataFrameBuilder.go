...
// GetTraceStore provides a mock function with no fields
func (_m *DataFrameBuilder) GetTraceStore() tracestore.TraceStore {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for GetTraceStore")
	}

	var r0 tracestore.TraceStore
	if rf, ok := ret.Get(0).(func() tracestore.TraceStore); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(tracestore.TraceStore)
		}
	}
