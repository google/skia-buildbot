...
func getSkps(ctx context.Context, headers []*ColumnHeader, perfGit *perfgit.Git) ([]int, error) {
	...
	// TODO(jcgregorio) Make "infra/bots/assets/skp/VERSION" into a config option.
	commitNumbers, err := perfGit.CommitNumbersWhenFileChangesInCommitNumberRange(ctx, begin, end, "infra/bots/assets/skp/VERSION")
	if err != nil {
		return nil, skerr.Wrapf(err, "Failed to find skp changes for range: %d-%d", begin, end)
	}
	ret := make([]int, len(commitNumbers), len(commitNumbers))
