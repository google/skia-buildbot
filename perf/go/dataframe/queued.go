...
func (fr *RunningFrameRequests) Status(id string) (ProcessState, string, float32, error) {
	...
	if p, ok := fr.inProcess[id]; !ok {
		...
		var percentComplete float32 = 0.0
		if p.totalSteps > 0 {
			percentComplete = float32(float32(p.step) / float32(p.totalSteps))
		}
		return p.state, p.message, percentComplete, nil
	}
}
...
func (p *FrameRequestProcess) inc(err error, message string) {
	p.mutex.Lock()
	defer p.mutex.Unlock()
	glog.Errorf("FrameRequest failed: %#v %s: %s", *(p.request), message, err)
	p.message = message
