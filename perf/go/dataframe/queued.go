...
func (p *FrameRequestProcess) Run() {
	...
	for _, formula := range p.request.Formulas {
		...
		if err != nil {
			p.reportError(w, r, err, "Failed to complete query.")
			return
		}
		dfAppend(df, newDF)
	}
	...
	if len(df.Header) == 0 {
		df = NewHeaderOnly(git, begin, end)
	}

	resp, err := p.responseFromDataFrame(df)
	if err != nil {
		p.reportError(w, r, err, "Failed to get ticks or skps.")
		return
	}
	p.response = resp
}
...
func (p *FrameRequestProcess) getCommitTimesForFile(begin, end string, filename string) []int64 {
	ret := []int64{}

	// Now query for all the changes to the skp version over the given range of commits.
	log, err := p.git.LogFine(begin+"^", end, "--format=format:%ct", "--", filename)
	if err != nil {
...
// the ColumnHeaders.
func (p *FrameRequestProcess) getSkps(headers []*ColumnHeader) ([]int, error) {
	...
	// Now query for all the changes to the skp version over the given range of commits.
	ts := p.getCommitTimesForFile(begin, end, "infra/bots/assets/skp/VERSION")
	// Add in the changes to the old skp version over the given range of commits.
	ts = append(ts, p.getCommitTimesForFile(begin, end, "SKP_VERSION")...)
...
func (p *FrameRequestProcess) responseFromDataFrame(df *DataFrame) (*FrameResponse, error) {
...
func (p *FrameRequestProcess) doSearch(queryStr string, begin, end time.Time) (*DataFrame, error) {
	...
	return NewFromQueryAndRange(git, ptracestore.Default, begin, end, q)
}
...
func (p *FrameRequestProcess) doCalc(formula string, begin, end time.Time) (*DataFrame, error) {
	...
	rowsFromQuery := func(s string) (calc.Rows, error) {
		...
		df, err = NewFromQueryAndRange(git, ptracestore.Default, begin, end, q)
		if err != nil {
			return nil, err
		}
