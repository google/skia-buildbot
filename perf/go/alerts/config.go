...
type Alert struct {
	...
	// This is here to support the legacy format of Alerts where State was an
	// integer enum, with 0 = ACTIVE, and 1 = DELETED. This is only needed for
	// Cloud Datastore, not SQL backed stores.
	State int
	...
	StepUpOnly    bool        `json:"step_up_only"     datastore:",noindex"`

	// This is here to support the legacy format of Alerts where Direction was
	// an integer enum, with 0 = BOTH, 1 = UP, and 2 = DOWN. This is only needed
	// for Cloud Datastore, not SQL backed stores.
	Direction int

	// If true then only steps up will trigger an alert. [Deprecated, use DirectionAsString.]
