...
type Alert struct {
	...
	StepUpOnly    bool        `json:"step_up_only"     datastore:",noindex"` // [Deprecated, use DirectionAsString.]
	...
	// for Cloud Datastore, not SQL backed stores. This can be deleted after
	// migrating away from Cloud Datastore.
	Direction int `json:"-"`

	// If true then only steps up will trigger an alert.
