...
// called.
func New(db *pgx.Conn, dialect perfsql.Dialect) (*SQLAlertStore, error) {
...
func (s *SQLAlertStore) Save(ctx context.Context, cfg *alerts.Alert) error {
	...
	if cfg.ID == alerts.BadAlertID {
		...
		if _, err := s.db.ExecEx(ctx, statements[insertAlert], nil, string(b), now); err != nil {
			return skerr.Wrapf(err, "Failed to insert alert")
		}
	} else {
		if _, err := s.db.ExecEx(ctx, statements[updateAlert], nil, cfg.ID, string(b), cfg.StateToInt(), now); err != nil {
...
func (s *SQLAlertStore) Delete(ctx context.Context, id int) error {
	now := time.Now().Unix()
	if _, err := s.db.ExecEx(ctx, statements[deleteAlert], nil, now, id); err != nil {
...
func (s *SQLAlertStore) List(ctx context.Context, includeDeleted bool) ([]*alerts.Alert, error) {
	...
	rows, err := s.db.QueryEx(ctx, statements[stmt], nil)
	if err != nil {
		return nil, err
	}
	ret := []*alerts.Alert{}
