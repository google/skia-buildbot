...
// Alert can be either an email, or an identifier for where to send chat
// messages.
type Config struct {
	ID             int                     `json:"id"`
	...
	Algo           clustering2.ClusterAlgo `json:"algo"`
	State          AlertConfigState        `json:"state"`
}
...
// TODO Make sure each action is logged in the activity log.
...
func (s *Store) List(includeDeleted bool) ([]*Config, error) {
	...
	for rows.Next() {
		...
		cfg.ID = id
		cfg.State = AlertConfigState(state)
		ret = append(ret, cfg)
	}
