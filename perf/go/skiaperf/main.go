...
func newCalcHandler(w http.ResponseWriter, r *http.Request) {
	...
	rowsFromQuery := func(q string) (calc.Rows, error) {
		urlValues, err := url.ParseQuery(cr.Query)
		if err != nil {
			return nil, err
		}
		q, err := query.New(urlValues)
	...
	// TODO(jcgregorio) Do we wipe out the df.ParamSet since this is a calculated response?
