...
//
// TODO(jcgregorio) Would it be faster to have ops and traceMap be pointers?
type request struct {
...
// mergeWorker merges the data in requests into a traceSet
// and paramSet.
	...
	for i := 0; i < NUM_WORKERS; i++ {
		go func() {
			...
			for req := range t.hashStageCh {
				...
				req.key, err = query.MakeKey(req.params)
				if err != nil {
				...
				index := binary.BigEndian.Uint64(md5.New().Sum([]byte(req.key))) % NUM_WORKERS
...
// traces are keyed by encoded key and the traces are just a singe tile length.
