...
//     Commit: 51643
//     Details: "Filtered Traces: Num Before: 95 Num After: 92 Detla: 3"
//
//
package progress
...
func TestProgress_AddMessage_MessageAppearsInJSON(t *testing.T) {
	...
	assert.Equal(t, "{\"status\":\"Running\",\"messages\":[{\"key\":\"foo\",\"value\":\"bar\"}]}\n", buf.String())
}
...
func TestProgress_AddTwoMessages_MessagesAppearInJSONInTheOrderTheyWereAdded(t *testing.T) {
	...
	var buf bytes.Buffer
	require.NoError(t, p.JSON(&buf))
	assert.Equal(t, "{\"status\":\"Running\",\"messages\":[{\"key\":\"foo\",\"value\":\"bar\"},{\"key\":\"fizz\",\"value\":\"buzz\"}]}\n", buf.String())
}
