...
VALUES
  ("gs://perf-bucker/2020/02/08/11/testdata.json"),
  ("gs://perf-bucker/2020/02/08/12/testdata.json"),
  ("gs://perf-bucker/2020/02/08/13/testdata.json"),
  ("gs://perf-bucker/2020/02/08/14/testdata.json");
...
VALUES
 ...
 SELECT trace_id, trace_key FROM TraceKeys;
...
SELECT tile_key, key_value, trace_id FROM Postings
WHERE tile_key=0 AND key_value="arch=x86"
ORDER BY trace_id;
...
SELECT tile_key, trace_id, commit_number, val FROM TraceValues
WHERE tile_key=0 AND (trace_id=1 OR trace_id=2);
