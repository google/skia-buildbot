...
func releaseBranch(ctx context.Context, newBranch string, reviewers []string, allowEmptyCLs bool) error {
	...
	// Update the current milestone from the branch name.
	m := chromeBranchMilestoneRegex.FindStringSubmatch(newBranch)
	if len(m) != 2 {
		return skerr.Fmt("%q is not a recognized branch name format for Chrome; wanted %q", newBranch, chromeBranchTmpl)
	}
	...
	// Filter out unsupported CQ jobs on the new branch.
	fmt.Printf("Creating CL to filter out unsupported CQ try jobs on %s...\n", newBranch)
	updateTryjobCI, err := updateTryjobs(ctx, g, repo, newBranch, reviewers, allowEmptyCLs)
	if err != nil {
		return skerr.Wrap(err)
	}

	// Update the supported branch list.
	_, activeMilestones, err := cbc.Get(ctx)
	if err != nil {
		return skerr.Wrap(err)
	}
	supportedRefs := getSupportedRefs(currentChromeMilestone, numSupportedChromeBranches, activeMilestones)
	fmt.Println("Creating CL to update supported branches...")
	if err := updateSupportedBranches(ctx, g, repo, supportedRefs, reviewers); err != nil {
		return skerr.Wrap(err)
	}

	// Remove the CQ configs for every no-longer-supported branch.
	isSupportedRef := make(map[string]bool, len(supportedRefs))
	for _, ref := range supportedRefs {
		isSupportedRef[ref] = true
	}
	allBranches, err := repo.Branches(ctx)
	if err != nil {
		return skerr.Wrap(err)
	}
	for _, b := range allBranches {
		if !chromeBranchMilestoneRegex.MatchString(b.Name) {
			continue
		}
		ref := git.FullyQualifiedBranchName(b.Name)
		if !isSupportedRef[ref] {
			// This branch is no longer supported. Check if CQ is enabled on it.
			if _, err := repo.ReadFileAtRef(ctx, cqJSONFile, ref); err == nil {
				fmt.Printf("Creating CL to remove CQ on %s\n", b.Name)
				if err := removeCQ(ctx, g, repo, ref, reviewers); err != nil {
					return skerr.Wrap(err)
				}
			}
		}
...
func updateMilestone(ctx context.Context, g gerrit.GerritInterface, repo *gitiles.Repo, currentMilestone int, reviewers []string) error {
	...
	fmt.Printf("Creating CL to update milestone to %d...\n", newMilestone)
...
// by |baseChangeID|.
func mergeReleaseNotes(ctx context.Context, g gerrit.GerritInterface, repo gitiles.GitilesRepo, baseChangeID string, currentMilestone int, newBranch string, reviewers []string) (*gerrit.ChangeInfo, error) {
...
func updateSupportedBranches(ctx context.Context, g gerrit.GerritInterface, repo *gitiles.Repo, supportedRefs []string, reviewers []string) error {
	// Setup.
	...
	// Fetch and decode the supported-branches.json file.
	...
	// Copy over all non-Chrome branches and Chrome branches that are still supported.
	isSupportedRef := make(map[string]bool, len(supportedRefs))
	for _, b := range supportedRefs {
		isSupportedRef[b] = true
	}
	newSupportedBranches := make(map[string]*supported_branches.SupportedBranch, len(sbc.Branches))
	for _, sb := range sbc.Branches {
		if !chromeBranchMilestoneRegex.MatchString(sb.Ref) || isSupportedRef[sb.Ref] {
			newSupportedBranches[sb.Ref] = sb
		}
	}
	for _, b := range supportedRefs {
		if _, ok := newSupportedBranches[b]; !ok {
			newSupportedBranches[b] = &supported_branches.SupportedBranch{
				Ref:   b,
				Owner: owner,
			}
		}
	...
	// Determine which branches were added or removed.
	oldSupportedBranches := make(map[string]bool, len(sbc.Branches))
	for _, oldBranch := range sbc.Branches {
		oldSupportedBranches[oldBranch.Ref] = true
	}
	var addedBranches, removedBranches []string
	for _, b := range sbc.Branches {
		if _, ok := newSupportedBranches[b.Ref]; !ok {
			removedBranches = append(removedBranches, b.Ref)
		}
	}
	for b := range newSupportedBranches {
		if !oldSupportedBranches[b] {
			addedBranches = append(addedBranches, b)
		}
	}

	// If we didn't change anything, don't upload a CL.
	if len(addedBranches) == 0 && len(removedBranches) == 0 {
		return nil
	}

	// Encode the updated config.
	newSupportedBranchesSlice := make([]*supported_branches.SupportedBranch, 0, len(newSupportedBranches))
	for _, b := range newSupportedBranches {
		newSupportedBranchesSlice = append(newSupportedBranchesSlice, b)
	}
	sbc.Branches = newSupportedBranchesSlice
	buf := bytes.Buffer{}
	...
	// Create the Gerrit CL.
	commitMsg := "supported-branches.json: "
	if len(addedBranches) > 0 {
		commitMsg += fmt.Sprintf("add %s", strings.Join(addedBranches, ", "))
		if len(removedBranches) > 0 {
			commitMsg += ", "
		}
	}
	if len(removedBranches) > 0 {
		commitMsg += fmt.Sprintf("remove %s", strings.Join(removedBranches, ", "))
	}
...
func removeCQ(ctx context.Context, g gerrit.GerritInterface, repo *gitiles.Repo, ref string, reviewers []string) error {
	// Setup.
	...
	commitMsg := fmt.Sprintf("Remove CQ for unsupported branch %s", ref)
	repoSplit := strings.Split(repo.URL(), "/")
	project := strings.TrimSuffix(repoSplit[len(repoSplit)-1], ".git")
	changes := map[string]string{
		cqJSONFile: "",
...
func getSupportedRefs(currentChromeMilestone, numSupportedChromeBranches int, activeMilestones []*chrome_branch.Branch) []string {
	supportedMilestones := make(map[int]bool)

	// Add the last N milestones.
	for i := range numSupportedChromeBranches {
		m := currentChromeMilestone - i
		if m > 0 {
			supportedMilestones[m] = true
		}
	}

	// Add milestones from activeMilestones.
	for _, am := range activeMilestones {
		supportedMilestones[am.Milestone] = true
	}

	// Convert to slice of branch names ("chrome/m%d").
	var branches []string
	for m := range supportedMilestones {
		branches = append(branches, git.FullyQualifiedBranchName(fmt.Sprintf(chromeBranchTmpl, strconv.Itoa(m))))
	}
