...
func extract(ctx context.Context, d *docker.Docker, tmp string, a *config.Artifact) (string, error) {
	var rv string
	return rv, td.Do(ctx, td.Props(fmt.Sprintf("Extract %s", a.Image)), func(ctx context.Context) error {
		extracted, err := ioutil.TempDir(tmp, "")
		if err != nil {
			return err
		}
		volumes := []string{fmt.Sprintf("%s:/out", extracted)}
		cmd := []string{"cp", a.Source, "/out"}
		if err := d.Run(ctx, a.Image, cmd, volumes, nil); err != nil {
			return err
		}
		rv = extracted
		return nil
...
func main() {
	...
	if err := td.Do(ctx, td.Props("Setup").Infra(), func(ctx context.Context) error {
		...
		resolve := func(a string) (string, error) {
			...
			resolved, ok := artifactToSha256[a]
			if ok {
				return resolved, nil
			}
		...
		for _, a := range artifacts {
			d, err := extract(ctx, d, tmp, a)
			if err != nil {
				return err
			}
			volumes = append(volumes, fmt.Sprintf("%s:%s", d, a.Mount))
		}
		return nil
	}); err != nil {
		td.Fatal(ctx, err)
