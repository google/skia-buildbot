...
var (
	...
	manifest     = flag.String("output-manifest", "", "Write this file containing a mapping of image name to sha256 sum.")
...
func build(ctx context.Context, d *docker.Docker, tmp, target, image string) (string, error) {
	...
	return sha256, td.Do(ctx, td.Props(name), func(ctx context.Context) error {
		...
		args := []string{"--iidfile", iidFile, *buildDir, "--tag", image + ":latest"}
		if *buildArgs != nil {
		...
		if image != "" {
			sha256, err = d.Push(ctx, image+":latest@"+imageId)
			if err != nil {
				return err
			}
		}
