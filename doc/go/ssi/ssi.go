...
/*
 ...
 Such a tag looks like this:
	...
	This covers any use case where we want generate dynamic content and
	make the placement and configuration editable by the user.

	For example the tag implemented below would allow the user to write
...
func ProcessSSI(body []byte) ([]byte, error) {
	...
	if len(indices) > 0 {
		...
		// newly generated outpu in the right order.
		...
		for _, match := range indices {
			// Use start/end indices of the tag to keep track of the parts of the
		...
		// the original document and newly generate parts into one.
...
func (g *gceFolderListing) generateFolderListing(params map[string]string) ([]byte, error) {
	...
	sort.Slice(items, func(i, j int) bool { return items[i]["created"].(string) > items[j]["created"].(string) })
