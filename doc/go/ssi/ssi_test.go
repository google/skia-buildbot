...
func TestProcessSSI(t *testing.T) {
	...
	newDoc, err := ProcessSSI(testDoc)
	assert.NoError(t, err)
	assert.Equal(t, string(expDoc), string(newDoc))
}

func testProcessFN(params map[string]string) ([]byte, error) {
	for k, v := range params {
		return []byte(fmt.Sprintf("supercat:%s:%s", k, v)), nil
	}
	return nil, nil
}

func TestGCEListing(t *testing.T) {
	tokenSrc, err := auth.NewJWTServiceAccountTokenSource("", "/home/stephana/gospace/src/go.skia.org/infra/doc/go/ssi/service-account.json", storage.ScopeFullControl)
	assert.NoError(t, err)
	...
	Init(client)
	docs, err := ProcessSSI([]byte("<ssi:listgce path=skia-stephana-test/testing>"))
	assert.NoError(t, err)
