...
func TestProcess_FirstTimeSeeingMachine_MarkReady(t *testing.T) {
	...
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.Booted, TS: mockNow},
	})
}
...
func TestProcess_MachineStartedTask_MarkBusy(t *testing.T) {
	...
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.StartedTask, TS: mockNow},
	})
}
...
func TestProcess_MachineFinishedTaskSuccess_UpdateTask(t *testing.T) {
	...
	require.NoError(t, err)
	expectToFindBusyMachine(t, c, testMachine, testTask)
	expectToFindCompletedTaskWithState(t, c, testTask, task.Success)
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.FinishedTask, TS: mockNow},
	})
}
...
func TestProcess_MachineFinishedTaskFailure_UpdateTask(t *testing.T) {
	...
	require.NoError(t, err)
	expectToFindBusyMachine(t, c, testMachine, testTask)
	expectToFindCompletedTaskWithState(t, c, testTask, task.Failure)
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.FinishedTask, TS: mockNow},
	})
}
...
func TestProcess_MachineFinishedTaskTimedOut_UpdateTask(t *testing.T) {
	...
	err := m.process(ctx, machine.Description{
		...
		machine.EventAttribute:      string(machine.AbandonedTask),
		machine.TaskStatusAttribute: string(task.TimedOut),
	})
	require.NoError(t, err)
	expectToFindBusyMachine(t, c, testMachine, testTask)
	expectToFindAbandonedTaskWithState(t, c, testTask, task.TimedOut)
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.AbandonedTask, TS: mockNow},
	})
}
...
func TestProcess_MachineFinishedTaskCancelled_UpdateTask(t *testing.T) {
	...
	require.NoError(t, err)
	expectToFindBusyMachine(t, c, testMachine, testTask)
	expectToFindAbandonedTaskWithState(t, c, testTask, task.Cancelled)
	expectMachineEventsToEqual(t, c, testMachine, []fs_entries.MachineEvent{
		{Event: machine.AbandonedTask, TS: mockNow},
	})
}
...
func expectMachineEventsToEqual(t *testing.T, client *ifirestore.Client, machineID string, expected []fs_entries.MachineEvent) {
	q := client.Collection(fs_entries.MachinesCollection).Doc(machineID).Collection(fs_entries.MachineEventsCollection).OrderBy("ts", firestore.Desc)
	var actual []fs_entries.MachineEvent
	err := client.IterDocs(context.Background(), "", "", q, 1, 30*time.Second, func(ds *firestore.DocumentSnapshot) error {
		var me fs_entries.MachineEvent
		err := ds.DataTo(&me)
		actual = append(actual, me)
		return err
	})
	require.NoError(t, err)
...
func expectToFindRunningTask(t *testing.T, client *ifirestore.Client, taskID string) {
	...
	assert.Zero(t, ft.Completed)
	assert.Zero(t, ft.Abandoned)
}

func expectToFindCompletedTaskWithState(t *testing.T, client *ifirestore.Client, taskID string, s task.TaskStatus) {
	doc := client.Collection(fs_entries.TasksCollection).Doc(taskID)
	ds, err := doc.Get(context.Background())
	require.NoError(t, err)
	var ft fs_entries.Task
	...
	assert.Equal(t, mockNow, ft.Updated)
	assert.Equal(t, mockNow, ft.Completed)
	assert.Zero(t, ft.Abandoned)
}

func expectToFindAbandonedTaskWithState(t *testing.T, client *ifirestore.Client, taskID string, s task.TaskStatus) {
	doc := client.Collection(fs_entries.TasksCollection).Doc(taskID)
	ds, err := doc.Get(context.Background())
	require.NoError(t, err)
	var ft fs_entries.Task
	...
	assert.Equal(t, mockNow, ft.Updated)
	assert.Zero(t, ft.Completed)
	assert.Equal(t, mockNow, ft.Abandoned)
}
