...
func (m *Manager) scheduleMaintenanceTask(ctx context.Context, mt fs_entries.MaintenanceTask) error {
	doc := m.client.Collection(fs_entries.MaintenanceTasksCollection).NewDoc()
	if _, err := m.client.Create(ctx, doc, mt, maxFirestoreWriteAttempts, maxFirestoreOperationTime); err != nil {
