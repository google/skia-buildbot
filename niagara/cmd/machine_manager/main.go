...
const (
	...
	pool = "testing" // TODO(kjlubick) make this a flag
)

func main() {
	flag.Parse()
	ifirestore.EnsureNotEmulator()
	sklog.Info("hello machine manager")
	...
	// a token source.
	fsClient, err := ifirestore.NewClient(ctx, "skia-firestore", "niagara", pool, nil)
	if err != nil {
	...
	labels := map[string]string{
		"logSource": "machine_manager",
	}
	logger, err := sklog.NewCloudLogger(ctx, "skia-public", "machines", nil, labels)
	if err != nil {
		sklog.Fatalf("could not create logger: %s", err)
	}

	sklog.Infof("PubSub topic/subscription set up %v\n", sub)
	m := machine_manager.New(fsClient, logger.Logger())

	if err := m.StartMachineFirestoreQuery(ctx); err != nil {
		sklog.Fatalf("could not get initial machines: %s", err)
	}
...
func setupTopicSubscription(ctx context.Context, psClient *pubsub.Client, topicName, subscriberName string) (*pubsub.Subscription, error) {
	...
	// These settings were made up with the intent to process messages as quickly as possible.
	// If the queue of messages regularly exceeds the maximum, we should spin up more replicas
	// of Machine Manager.
