...
func (m *Manager) processMachineMessage(ctx context.Context, msg *pubsub.Message) {
	...
	mID := msg.Attributes[machine.IDAttribute]
	if mID == "" {
	...
	if event == machine.Booted {
		...
		if err := m.updateMachine(ctx, mID, machine.Ready, machine.Booted, state); err != nil {
			sklog.Errorf("Could not update machine entry %s", err)
		}
	} else if event == machine.StartedTask {
		// TODO(kjlubick) Check that the machine had previously finished a task or booted.
		if err := m.updateMachine(ctx, mID, machine.Busy, machine.StartedTask, state); err != nil {
			sklog.Errorf("Could not update machine entry %s", err)
		}
		now := time.Now()
	...
	} else if event == machine.FinishedTask {
		...
		// TODO(kjlubick) check the health of this machine.
		if err := m.updateMachine(ctx, mID, machine.Quarantined, machine.FinishedTask, state); err != nil {
			sklog.Errorf("Could not update machine entry %s", err)
		}
...
func (m *Manager) updateMachine(ctx context.Context, machineID string, newStatus machine.Status, currEvent machine.Event, state machine.State) error {
