...
func (m *machine) emulateMachine(ctx context.Context) error {
	if err := m.sendEvent(ctx, messages.Booted, m.getState()); err != nil {
...
func (m *machine) sendEvent(ctx context.Context, event messages.MachineEvent, s messages.MachineState) error {
	...
	pr := m.topic.Publish(ctx, &pubsub.Message{
		...
		Attributes: map[string]string{
			messages.MachineID: m.id,
			messages.Event:     string(event),
		},
	})
...
func (m *machine) getState() messages.MachineState {
	return messages.MachineState{
...
func (m *machine) runTask(ctx context.Context, id string, entry db.FirestoreTaskEntry) error {
	s := m.getState()
	s.CurrentTask = id
	if err := m.sendEvent(ctx, messages.StartedTask, s); err != nil {
		return skerr.Wrap(err)
	}
	...
	if err := m.sendEvent(ctx, messages.FinishedTask, m.getState()); err != nil {
		return skerr.Wrap(err)
	}
