...
func (m *emulatedMachine) emulateMachine(ctx context.Context) error {
	if err := m.sendEvent(ctx, machine.Booted, m.getState()); err != nil {
...
func (m *emulatedMachine) sendEvent(ctx context.Context, event machine.Event, s machine.Description) error {
	...
	pr := m.topic.Publish(ctx, &pubsub.Message{
		...
		Attributes: map[string]string{
			machine.EventAttribute: string(event),
		},
	})
...
func (m *emulatedMachine) getState() machine.Description {
...
func (m *emulatedMachine) runTask(ctx context.Context, id string, entry fs_entries.Task) error {
	s := m.getState()
	s.CurrentTask = id
	if err := m.sendEvent(ctx, machine.StartedTask, s); err != nil {
		return skerr.Wrap(err)
	}
	...
	s = m.getState()
	s.CurrentTask = id
	if err := m.sendEvent(ctx, machine.FinishedTask, s); err != nil {
		return skerr.Wrap(err)
	}
