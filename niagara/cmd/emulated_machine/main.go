...
import (
	...
	machine2 "go.skia.org/infra/niagara/go/machine"
...
func (m *machine) emulateMachine(ctx context.Context) error {
	if err := m.sendEvent(ctx, machine2.Booted, m.getState()); err != nil {
...
func (m *machine) sendEvent(ctx context.Context, event machine2.Event, s machine2.State) error {
	...
	pr := m.topic.Publish(ctx, &pubsub.Message{
		...
		Attributes: map[string]string{
			machine2.IDAttribute:    m.id,
			machine2.EventAttribute: string(event),
		},
	})
...
func (m *machine) getState() machine2.State {
	return machine2.State{
...
func (m *machine) runTask(ctx context.Context, id string, entry fs_entries.Task) error {
	s := m.getState()
	s.CurrentTask = id
	if err := m.sendEvent(ctx, machine2.StartedTask, s); err != nil {
		return skerr.Wrap(err)
	}
	...
	s = m.getState()
	s.CurrentTask = id
	if err := m.sendEvent(ctx, machine2.FinishedTask, s); err != nil {
		return skerr.Wrap(err)
	}
