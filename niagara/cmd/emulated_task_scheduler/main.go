...
func (t *taskScheduler) cycle(ctx context.Context) error {
	snap := t.client.Collection(fs_entries.MachinesCollection).Where(fs_entries.MachineStatusField, "==", machine.Ready).Snapshots(ctx)
	for {
...
func (t *taskScheduler) processReadyMachine(ctx context.Context, dc firestore.DocumentChange) {
	...
	newTask := fs_entries.Task{
		MachineAssigned: id,
		Command:         "docker run alpine /bin/sleep 5",
		Status:          task.New,
		Created:         time.Now(),
	}
	doc := t.client.Collection(fs_entries.TasksCollection).NewDoc()
	_, err := t.client.Create(ctx, doc, newTask, maxFirestoreWriteAttempts, maxFirestoreOperationTime)
	if err != nil {
	...
	_, err = t.client.Update(ctx, dc.Doc.Ref, maxFirestoreWriteAttempts, maxFirestoreOperationTime,
		[]firestore.Update{{Path: fs_entries.TaskStatusField, Value: machine.Assigned}})
	if err != nil {
