...
func (req *UploadRequest) validate(w http.ResponseWriter) error {
	...
	if req.Width < 1 || req.Width > 2028 {
		http.Error(w, "Width must be between 1 and 2028.", http.StatusBadRequest)
		return invalidRequestErr
	}
	if req.Height < 1 || req.Height > 2028 {
		http.Error(w, "Height must be between 1 and 2028.", http.StatusBadRequest)
		return invalidRequestErr
	}
	return nil
}
