...
// This resolves the Annotator (i.e. calling the function if necessary)
// and then returns [AnnotationMap, JSON] or [null, null] if there
// is no more annotation down this path.
function unwrap(annotator, key, json) {
    let newAnno = null;
    if (typeof annotator === 'function') {
      newAnno = annotator(json)[key]
    } else {
      newAnno = annotator[key];
    }
    if (newAnno) {
