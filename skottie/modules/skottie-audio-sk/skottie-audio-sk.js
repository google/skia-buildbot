...
const bpmListTemplate = (ele) => html`
  <ul class=bpm-options>
    ${ele._state.bmpList.map(((bpmOption) => bpmListOptionTemplate(bpmOption, ele._onBpmSelected)))}
  </ul>
`;
...
class SkottieAudioSk extends HTMLElement {
  constructor() {
    ...
    this._onBpmSelected = this._onBpmSelected.bind(this);
    this._onOfflineRenderComplete = this._onOfflineRenderComplete.bind(this);
    this._onAudioLoaded = this._onAudioLoaded.bind(this);
    this._onFileDataLoaded = this._onFileDataLoaded.bind(this);
    this._detectBPMs = this._detectBPMs.bind(this);
  }
  ...
  _detectBPMs(ev) {
    ...
    offlineContext.oncomplete = this._onOfflineRenderComplete;
  }
  ...
  _onFileChange(event) {
    ...
    reader.addEventListener('load', this._onFileDataLoaded, false);
    const arrayBufferReader = new FileReader();
    arrayBufferReader.readAsArrayBuffer(this._file);
    arrayBufferReader.addEventListener('load', this._detectBPMs, false);
    this._render();
  }
