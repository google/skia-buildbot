...
<header>
  <h2>Skottie Drive Previewer</h2><span><a href='https://skia.googlesource.com/skia/+/${SKIA_VERSION}'>${SKIA_VERSION.slice(0, 7)}</a></span>
</header>
<main>
  <!-- Create on player per id. -->
    <skottie-player-sk id=player></skottie-player-sk>
</main>
<footer>
...
window.customElements.define('skottie-drive-sk', class extends HTMLElement {
  ...
  updateSigninStatus(isSignedIn) {
    ...
    if (!isSignedIn) {
      ...
      this.loadFiles(['12M0hlsK-zYCrKU6TG-Bji5Kcr9hWoyJw']);
    }
  }

  loadFiles(ids) {
    // TODO(jcgregorio) We can actually get called with a list of file ids, so
    // handle that case.
