...
let annotations = {
  ...
  'assets': {
    '_annotation': 'Assets to be included'
  },'layers': {
    '_annotation': 'List of Composition Layers',
    '_is_array': true,
    'children': {
      'ty': leaf('Type of layer: Shape. Value 4 SHOULD BE DYNAMIC'),
      'ddd': leaf('3d layer flag.'),
...
function reannotate() {
  ...
  rows.forEach((row) => {
    ...
    if (key) {
      let anno = annotations[key.innerText];
      if (anno) {
        let parent = key.closest('tr');
        if (!$$('.annotation', parent)) {
          let newAnno = document.createElement('td');
          newAnno.innerHTML = `<div class="jsoneditor-value annotation">// ${anno._annotation}</div>`
