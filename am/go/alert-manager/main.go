...
var (
	...
	resolvedMax        = flag.Int64("resolved_max", 3, "The number of times an incident doesn't show up between healthz checks before it is archived.")
)
...
func New() (*Server, error) {
	...
	srv := &Server{
		salt:          salt,
		incidentStore: incident.NewStore(ds.DS, []string{"kubernetes_pod_name", "instance", "pod_template_hash"}, *resolvedMax),
		allow:         allow,
	}
	srv.loadTemplates()
		...
		for {
			err := sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
				...
				if m[alerts.TYPE] == alerts.TYPE_HEALTHZ {
					err := srv.incidentStore.Healthz(m[alerts.LOCATION])
					if err != nil {
						sklog.Errorf("Error processing HEALTHZ: %s", err)
					}
