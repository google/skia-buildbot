...
func (srv *Server) AddXSSProtectedHandlers(r *mux.Router) {
	// Served under the path /_/.

	// GETs
	r.HandleFunc("/emails", srv.emailsHandler).Methods("GET")
	r.HandleFunc("/incidents", srv.incidentHandler).Methods("GET")
	r.HandleFunc("/new_silence", srv.newSilenceHandler).Methods("GET")
	r.HandleFunc("/recent_incidents", srv.recentIncidentsHandler).Methods("GET")
	r.HandleFunc("/silences", srv.silencesHandler).Methods("GET")

	// POSTs
	r.HandleFunc("/add_note", srv.addNoteHandler).Methods("POST")
	r.HandleFunc("/add_silence_note", srv.addSilenceNoteHandler).Methods("POST")
	r.HandleFunc("/archive_silence", srv.archiveSilenceHandler).Methods("POST")
	r.HandleFunc("/assign", srv.assignHandler).Methods("POST")
	r.HandleFunc("/del_note", srv.delNoteHandler).Methods("POST")
	r.HandleFunc("/del_silence_note", srv.delSilenceNoteHandler).Methods("POST")
	r.HandleFunc("/reactivate_silence", srv.reactivateSilenceHandler).Methods("POST")
	r.HandleFunc("/save_silence", srv.saveSilenceHandler).Methods("POST")
	r.HandleFunc("/take", srv.takeHandler).Methods("POST")
	r.HandleFunc("/stats", srv.statsHandler).Methods("POST")
	r.HandleFunc("/incidents_in_range", srv.incidentsInRangeHandler).Methods("POST")
}
...
func main() {
	baseapp.Serve(New, APP_NAME, "am.skia.org", login.RestrictViewer, login.ForceAuthMiddleware(login.DEFAULT_REDIRECT_URL))
}
