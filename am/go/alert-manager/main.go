...
const (
	...
	DUMMY_ALERT_NAME = "DummyAlertsAreWorking"
)
...
func New() (baseapp.App, error) {
	...
	for _, location := range locations {
		...
		missingAlertsLivenesses[location] = metrics2.NewLiveness("recv_dummy_alert", map[string]string{"location": location})
	}
	...
	go func() {
		...
		for _ = range time.Tick(1 * time.Minute) {
				...
				if metric.Get() > 120 {
					...
					if !ok {
						alert = map[string]string{
							...
							"description":       fmt.Sprintf("No healthz in more than 2 minutes from alert-to-pubsub in %s. This probably means alert-to-pubsub is down in %s. No alerts will appear here until this is fixed!", location, location),
							"location":          location,
						}
			...
			for location, metric := range missingAlertsLivenesses {
				if metric.Get() > 120 {
					// Haven't seen a dummy alert from this location for more than two minutes.
					...
					if !ok {
						alert = map[string]string{
							...
							incident.CATEGORY:   "infra",
							incident.SEVERITY:   "critical",
							"description":       fmt.Sprintf("No alerts in more than 2 minutes from Prometheus in %s. This probably means either Prometheus or alert-to-pubsub is down in %s. No alerts will appear here until this is fixed!", location, location),
							"location":          location,
						}
	...
	go func() {
		for {
			err := sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
				...
				} else if m[incident.ALERT_NAME] == DUMMY_ALERT_NAME {
					sklog.Infof("dummy alert received: %q", m[alerts.LOCATION])
					if l, ok := missingAlertsLivenesses[m[alerts.LOCATION]]; ok {
						l.Reset()
					} else {
						sklog.Errorf("Unknown PubSub source location: %q", m[alerts.LOCATION])
