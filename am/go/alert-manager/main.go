...
func internalServer(app baseapp.App) {
	if srv, ok := app.(*Server); ok {
		...
		sklog.Fatal("Can't cast App to *Server.")
	}
}

func main() {
	baseapp.Serve(New, APP_NAME, "am.skia.org", internalServer, login.RestrictViewer, login.ForceAuthMiddleware(login.DEFAULT_REDIRECT_URL))
}
