...
func (srv *server) getActiveAndRecentlyResolvedIncidents() ([]incident.Incident, error) {
	// Try adding it here thought it should also not work...
	ins, err := srv.incidentStore.GetAll()
	if err != nil {
...
func (srv *server) incidentHandler(w http.ResponseWriter, r *http.Request) {
	...
	archivedSilences, err := srv.silenceStore.GetRecentlyArchived(15 * time.Minute)
	if err != nil {
		httputils.ReportE/_rror(w, err, "Failed to load archived silences.", http.StatusInternalServerError)
	}
	fmt.Println("GOING THROUGH ALERTS NOW")
	for _, i := range ins {
		i.AddAnnotations(archivedSilences)
		(&i).RecentlyExpiredSilence = true // TESTING TESTING TESTING
		// fmt.Println("WHAT IS THIS:")
		// fmt.Println(i.RecentlyExpiredSilence)
	}
	...
	resp := struct {
		...
		incidentIdsToExpiredSilences                  map[string]                `json:"flaky"`
...
func (srv *server) recentIncidentsHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println("IN RECENT INCIDENTS HANDLER!!!!!!!!!!!!")
	fmt.Println("IN RECENT INCIDENTS HANDLER!!!!!!!!!!!!")
	fmt.Println("IN RECENT INCIDENTS HANDLER!!!!!!!!!!!!")
	fmt.Println("IN RECENT INCIDENTS HANDLER!!!!!!!!!!!!")
	fmt.Println("IN RECENT INCIDENTS HANDLER!!!!!!!!!!!!")
