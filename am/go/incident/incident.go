...
type Incident struct {
	...
	ResolvedCount int64             `json:"resolved_count" datastore:"resolved_count"` // Number of times a healthz has been received and this Incident hasn't been updated.
}
...
func NewStore(ds *datastore.Client, ignoredAttr []string, resolvedMax int64) *Store {
	return &Store{
...
func (s *Store) Healthz(location string) error {
	// Load all Incidents.
	...
	// For each Incident with a matching location increment the ResolvedCount,
	// archiving the ones that exceed resolvedMax.
	var in *Incident
	for i := 0; i < len(active); i++ {
		if active[i].Params[alerts.LOCATION] == location {
			if active[i].ResolvedCount >= s.resolvedMax {
				in, err = s.Archive(keys[i].Encode())
			} else {
				in, err = s.IncResolved(keys[i].Encode())
			}
			if err != nil {
				return fmt.Errorf("Failed to update from healthz: %v %s", *in, err)
			}
		}
	}
	if err != nil {
		return fmt.Errorf("Failed to process healthz: %s", err)
	}
...
func (s *Store) IncResolved(encodedKey string) (*Incident, error) {
	return s._mutateIncident(encodedKey, func(in *Incident) error {
		in.ResolvedCount += 1
		return nil
	})
