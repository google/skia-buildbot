...
const DirtyCommittedImageRegex = ".+?-(?P<Owner>\\w+?)-\\w+?-dirty"
...
func getOwnerFromDirtyCommittedImage(committedImage string) string {
	re := regexp.MustCompile(DirtyCommittedImageRegex)
	matches := re.FindStringSubmatch(committedImage)
	if len(matches) == 0 {
		return ""
	}
	lastIndex := re.SubexpIndex("Owner")
	if lastIndex == -1 {
		return ""
	}
