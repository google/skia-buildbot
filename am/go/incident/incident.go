...
func (s *Store) AlertArrival(m map[string]string) error {
	...
	if err != nil {
		return fmt.Errorf("Could not create transaction: %s", err)
	}
	...
	for i := 0; i < TX_RETRIES; i++ {
		if _, err = s.ds.GetAll(ctx, q, &active); err != nil {
			break
		}
		if len(active) == 0 {
		...
		if _, err := tx.Put(key, active[0]); err != nil {
			break
		}
	...
	if err != nil {
		return fmt.Errorf("Failed to save incoming alert %v: %s", m, err)
	}
...
// Returns a list of all active and silenced Incidents.
func (s *Store) GetAll() ([]*Incident, []*Incident, error) {
	return nil, nil, nil
}
