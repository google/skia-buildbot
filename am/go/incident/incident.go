...
type Incident struct {
	...
	Params   string `json:"params" datastore:"params"`       // Params serializes as a StructuredKey.
...
func (s *Store) idForAlert(m map[string]string) (string, error) {
	...
	params, err := query.MakeKey(query.ForceValid(m))
	if err != nil {
		return "", fmt.Errorf("Failed to make into a structured key: %s", err)
	}
	return fmt.Sprintf("%x", md5.Sum([]byte(params))), nil
}
...
func (s *Store) Assign(encodedKey string, user string) (*Incident, error) {
	return s._mutateIncident(encodedKey, func(in *Incident) error {
		m, err := query.ParseKey(in.Params)
		if err != nil {
			return err
		}
		m[ASSIGNED_TO] = user
		p, err := query.MakeKey(query.ForceValid(m))
		if err != nil {
			return err
		}
		in.Params = p
