...
func getRegexesToOwners(abbr_owner_regex string) (map[string]string, error) {
	...
	for _, owner_to_regexes := range owners_to_regexes {
		...
		if len(tokens) != 2 {
			return nil, fmt.Errorf("%s is not a well-formed abbr_owner_regex", owner_to_regexes)
		}
		owner := tokens[0]
