...
`;
  ...
  matchingSilences(): TemplateResult[] {
    ...
    const ret = filteredSilences.slice(0, MAX_MATCHING_SILENCES_TO_DISPLAY).map((silence: Silence) => html`<silence-sk .state=${silence} collapsable collapsed></silence-sk>`);
    if (!ret.length) {
  ...
  history(): Promise<any> {
    ...
    return fetch(`/_/recent_incidents?id=${this.state.id}&key=${this.state.key}`, {
      ...
      return incidents.map((i: Incident) => html`<incident-sk .state=${i} minimized></incident-sk>`);
    }).catch(errorMessage);
  }
