...
function assignedTo(ele) {
  if (ele._state.params.assigned_to) {
    return ' - ' + ele._state.params.assigned_to
  } else {
    return ''
  }
}
...
const template = (ele) => html`
  <h2 class$=${classOnAssigned(ele)} on-click=${e => ele._toggle(e)}>${ele._state.params.alertname} ${assignedTo(ele)}</h2>
