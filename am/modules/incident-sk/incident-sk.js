...
function matchingSilences(ele) {
  return ele._silences.filter(silence => paramset.match(silence.param_set, ele._state.params)).map( silence =>
    // Make this a 'short' silence-sk?
    html`<h2>${displaySilence(silence)}</h2>`
  )
}
