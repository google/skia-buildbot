...
export function displaySilence(silence: SilenceSk): string {
  const ret: string[] = [];
  Object.keys(silence.silenceState.param_set!).forEach((key) => {
    if (key.startsWith('__')) {
      return;
    }
    ret.push(`${silence.silenceState.param_set![key]!.join(', ')}`);
  });
  let s = ret.join(' ');
  if (s.length > 33) {
    s = `${s.slice(0, 30)}...`;
 ...
 */
export function notes(ele: SilenceSk|IncidentSk): TemplateResult[] {
  if (!ele.silenceState.notes) {
    return [];
  }
  return ele.silenceState.notes.map((note: Note, index: number) => html`<section class=note>
...
export function expiresIn(silence: SilenceSk): string {
  if (silence.silenceState.active) {
    return diffDate((silence.silenceState.created + parseDuration(silence.silenceState.duration)) * 1000);
  }
  return '';
}
