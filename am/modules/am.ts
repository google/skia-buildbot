...
export function displaySilence(silence: SilenceSk): string {
  const ret: string[] = [];
  Object.keys(silence.silence_state.param_set!).forEach((key) => {
    if (key.startsWith('__')) {
      return;
    }
    ret.push(`${silence.silence_state.param_set![key]!.join(', ')}`);
  });
  let s = ret.join(' ');
  if (s.length > 33) {
    s = `${s.slice(0, 30)}...`;
...
export function expiresIn(silence: SilenceSk): string {
  if (silence.silence_state.active) {
    return diffDate((silence.silence_state.created + parseDuration(silence.silence_state.duration)) * 1000);
  }
  return '';
}
