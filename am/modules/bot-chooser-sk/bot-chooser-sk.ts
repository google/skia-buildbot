...
export class BotChooserSk extends HTMLElement {
  ...
  displayBotOptions(): TemplateResult[] {
  ...
  displayDialogContents(): TemplateResult {
    if (Object.keys(this.bots_to_incidents).length === 0) {
      return html`
        <h2>No active bot alerts found</h2>
        <br/>
        <div class=buttons>
          <button @click=${this._dismiss}>OK</button>
        </div>
      `;
    }
    return html`
        <h2>Bots with active alerts</h2>
        <select size=10 @input=${this._input}>
          ${this.displayBotOptions()}
        </select>
        <div class=buttons>
          <button @click=${this._dismiss}>Cancel</button>
          <button @click=${this._confirm}>OK</button>
        </div>
      `;
  }
   ...
   *
   */
  open(bots_to_incidents: Record<string, Incident[]>, bots_to_ignore: string[]): Promise<string> {
