...
function newSilence(ele) {
  return html`<silence-sk on-save-silence=${e => ele._saveSilence(e.detail.silence)} on-delete-silence-param=${e => ele._deleteSilenceParam(e.detail.key)} state=${ele._new_silence}></silence-sk>`;
}
...
<section class=silences>
  ${ele._silences.map(i => html`<h2>${i.key}</h2>`)}
</section>
<section class=edit>
  ${rightHandSide(ele)}
</section>
...
window.customElements.define('alert-manager-sk', class extends HTMLElement {
  ...
  _deleteSilenceParam(key) {
    delete this._new_silence.param_set[key];
    this._render();
  }
