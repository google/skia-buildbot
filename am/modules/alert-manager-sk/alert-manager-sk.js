...
function silenceHasNotes(silence) {
  return (silence.notes && silence.notes.length > 0) ? '' : 'invisible';
}
...
<section class=silences>
  ${ele._silences.map(i => html`<h2 class$=${classOfSilenceH2(ele, i)} on-click=${e => ele._silenceClick(i)}> <span>${displaySilence(i)}</span> <comment-icon-sk title='This silence has notes.' class$=${silenceHasNotes(i)}></comment-icon-sk> </h2>`)}
</section>
<section class=edit>
  ${rightHandSide(ele)}
</section>
...
window.customElements.define('alert-manager-sk', class extends HTMLElement {
  ...
  _poll() {
    ...
    Promise.all([incidents, recents, silences, emails]).then(() => { this._render() }).catch((msg) => {
      if (msg.resp) {
        msg.resp.text().then(errorMessage);
      } else {
        errorMessage(msg);
