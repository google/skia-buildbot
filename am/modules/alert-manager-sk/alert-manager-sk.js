...
define('alert-manager-sk', class extends HTMLElement {
  ...
  _botChooser(e) {
    ...
    console.log(this._current_silence);
    // _modifySilenceParam(silence)
    console.log("CALLING bot-chooser");
    console.log(this._bots_to_incidents);
    $$('#bot-chooser', this).open(this._bots_to_incidents).then((bot) => {
      console.log(this._current_silence.paramset);
      this._current_silence.param_set['bot'] = [`${this._current_silence.param_set['bot']}|${bot}`];
