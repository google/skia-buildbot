...
function editIncident(ele) {
  if (ele._selected) {
    return html`<incident-sk silences=${ele._silences} state=${ele._selected} on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}></incident-sk>`
  } else {
    return ``
  }
}
...
function editSilence(ele) {
  return html`<silence-sk
    ...
    on-add-silence-note=${(e) => ele._addSilenceNote(e)}
    on-del-silence-note=${(e) => ele._delSilenceNote(e)}
    ...
    <section class=incidents>
      ${ele._incidents.map(i => html`
        <h2 class$=${classOfH2(ele, i)} on-click=${e => ele._select(i)}>
          <span><checkbox-sk checked?=${ele._checked.has(i.key)} on-change=${e => ele._check_selected(e)} on-click=${e => ele._suppress(e)} id=${i.key}></checkbox-sk>
          ${displayIncident(i)}
          </span>
...
window.customElements.define('alert-manager-sk', class extends HTMLElement {
  ...
  _poll() {
    ...
    }).finally(() => window.setTimeout(() => this._poll(), 10000));
  }
  ...
  _rationalize() {
    this._incidents.forEach(incident => {
      let silenced = this._silences.reduce((isSilenced, silence) => (isSilenced || paramset.match(silence.param_set, incident.params)), false);
      incident.params.__silence_state = silenced ? 'silenced' : 'active';
    });
    let sortby = ['__silence_state', 'assigned_to', 'alertname'];
  ...
  _needsTriaging(incident) {
    if (incident.active && (incident.params.__silence_state != 'silenced') && !incident.params.assigned_to) {
      return true
    }
    return false
  }
  ...
  _render() {
    ...
    let numActive = this._incidents.reduce((n, incident) => n += this._needsTriaging(incident) ? 1 : 0, 0)
