...
function columns(cols, incident) {
  return cols.map(c => html`<td>${incident.params[c]}</td>`)
}
...
<main>
  <section on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}>
    <table>
      ${ele._incidents.map(i => html`<tr><td><incident-sk closed state=${i}></incident-sk></td>${columns(ele._columns, i)}</tr>`)}</td>
    </table>
  </section>
  ...
  <section on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}>
    <table>
      ${ele._recents.map(i => html`<incident-sk closed state=${i}></incident-sk>`)}
    </table>
  </section>
</main>
<footer>
