...
function columns(cols, incident) {
  return cols.map(c => html`<div>${incident.params[c]}</div>`)
}

function gridColumns(ele) {
  return 'grid-template-columns: 40%' + ' 1fr'.repeat(ele._columns.length);
}
...
<main>
  <section style$=${gridColumns(ele)} on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}>
    ${ele._incidents.map(i => html`<incident-sk closed state=${i}></incident-sk>${columns(ele._columns, i)}`)}
  </section>
  <email-chooser-sk id=chooser></email-chooser-sk>
  <spinner-sk id=busy></spinner-sk>
  <section style$=${gridColumns(ele)} on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}>
     ${ele._recents.map(i => html`<incident-sk closed state=${i}></incident-sk>${columns(ele._columns, i)}`)}
  </section>
</main>
<footer>
...
window.customElements.define('alert-manager-sk', class extends HTMLElement {
  ...
  _render() {
    let sortby = ["alertname"];
    sortby = sortby.concat(this._columns);
    this._incidents.sort((a,b) => {
      for (let i = 0; i < sortby.length; i++) {
        let key = sortby[i];
        let cmp = a.params[key].localeCompare(b.params[key]);
        if (cmp != 0) {
          return cmp;
        }
      }
