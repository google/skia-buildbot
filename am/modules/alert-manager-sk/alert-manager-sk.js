...
//      msg.resp.text().then(errorMessage);
  ...
  _rationalize() {
    this._silences.forEach(silence => {
      this._incidents.forEach(incident => {
        // Pick values for 'state' that sort correctly.
        incident.params.state = paramset.match(silence.param_set, incident.params) ? 'silenced' : 'active';
      });
    });
