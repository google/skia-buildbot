...
<main>
  <section on-add-note=${(e) => ele._addNote(e)} on-del-note=${(e) => ele._delNote(e)} on-take=${e => ele._take(e)} on-assign=${e => ele._assign(e)}>
   ${ele._incidents.map(i => html`<incident-sk closed state=${i}></incident-sk>`)}
  </section>
...
window.customElements.define('alert-manager-sk', class extends HTMLElement {
  ...
  parseParams(incidents) {
    for (let i = 0; i < incidents.length; i++) {
      let parsed = {};
      incidents[i].params.split(',').forEach(kvp => {
        let parts = kvp.split('=');
        parsed[parts[0]] = parts[1];
      })
      incidents[i].params = parsed;
    }
    return incidents
  }
  ...
  connectedCallback() {
    ...
    fetch('/_/incidents', {
      ...
      this._incidents = this.parseParams(json);
      this._render();
    ...
    fetch(url, {
      ...
      for (let i = 0; i < this._incidents.length; i++) {
        if (this._incidents[i].key == json.key) {
          this._incidents[i] = this.parseParams([json]);
          break;
        }
      }
      this._render();
