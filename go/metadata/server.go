...
func (t *ServiceAccountToken) Update() error {
	...
	if err := json.NewDecoder(bytes.NewBuffer(contents)).Decode(tok); err != nil {
		return err
	}
...
func makeInstanceMetadataHandler(md map[string]map[string]string) func(http.ResponseWriter, *http.Request) {
...
func makeProjectMetadataHandler(md map[string]string) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		key, ok := mux.Vars(r)["key"]
		if !ok {
			httputils.ReportError(w, r, nil, "Metadata key is required.")
...
// metadata server.
func SetupServer(r *mux.Router, projectMetadata map[string]string, instanceMetadata map[string]map[string]string, tok *ServiceAccountToken) {
	mdHandler(r, LEVEL_INSTANCE, makeInstanceMetadataHandler(instanceMetadata))
	mdHandler(r, LEVEL_PROJECT, makeProjectMetadataHandler(projectMetadata))
