...
// ProjectId returns the project id as reported by the metadata
// server, or "skolo" if no metadata server is present.
func ProjectId() (string, error) {
	id, err := getUrl(fmt.Sprintf(METADATA_URL_PREFIX, PROJECT_ID_PATH))
	if err == nil && id == "" {
		id = SKOLO_PROJECT_ID
	}
	return id, err
