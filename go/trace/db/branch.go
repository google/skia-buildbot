...
func NewBranchTileBuilder(db DB, git *gitinfo.GitInfo, rietveldReview *rietveld.Rietveld, gerritReview *gerrit.Gerrit, evt *eventbus.EventBus) BranchTileBuilder {
	return &tileBuilder{
		db:                 db,
		vcs:                git,
...
func (b *tileBuilder) getGerritIssue(source string) (*gerrit.ChangeInfo, error) {
	...
	if (len(splitPath) != 2) || (splitPath[0] != "c") {
		return nil, fmt.Errorf("Error parsing Gerrit URL. Path format should be /c/<id>. Got", u.Path)
	}
