...
const (
	...
	// indicating that periodic actions should be triggered. These files are
	// created by the systemd periodic-trigger-*.service.
	TRIGGER_DIRNAME = "periodic-trigger"

	// PERIODIC_TRIGGER_MEASUREMENT is the name of the liveness metric for
	// periodic triggers.
	PERIODIC_TRIGGER_MEASUREMENT = "periodic-trigger"
...
func NewTriggerer(workdir string) (*Triggerer, error) {
	wd := path.Join(workdir, TRIGGER_DIRNAME)
	metrics, err := newPeriodicTriggerMetrics(wd)
	if err != nil {
		return nil, err
	}
	return &Triggerer{
...
func (t *Triggerer) RunPeriodicTriggers() error {
	triggers, err := findAndParseTriggerFiles(t.workdir)
	if err != nil {
		return err
	}
	...
	for _, trigger := range triggers {
		...
		if len(errs) == 0 {
			if err := deleteTriggerFile(t.workdir, trigger); err != nil {
				errs = append(errs, err)
			}
	...
	if len(allErrs) > 0 {
		...
		return fmt.Errorf(rvMsg)
	}
	return nil
}

// Start running periodic triggers.
func Start(ctx context.Context, workdir string, triggers map[string][]func() error) error {
	...
	lv := metrics2.NewLiveness("last-successful-periodic-trigger-loop")
