...
type IndexedBucket struct {
	...
	// indices is the set of indices this store maintains.
...
// initBuckets makes sure all needed buckets exist and indices are in sync
...
func (ix *IndexedBucket) initBuckets() error {
	return ix.DB.Update(func(tx *bolt.Tx) error {
		...
		// Convert the requested indices to a StringSet.
		...
		if len(addIndices) > 0 {
			return ix.buildIndices(tx, addIndices)
		}

		// Write the meta data.
		metaRec := &metaData{Indices: ix.indices.Keys()}
...
func (ix *IndexedBucket) buildIndices(tx *bolt.Tx, indices util.StringSet) error {
	...
	for k, v := c.First(); k != nil; k, v = c.Next() {
		...
		if len(recBatch) >= BUILD_INDEX_BATCH_SIZE {
			...
			// Note this might prevent some instance of Record from being garbage collected
			// which is accpetable in this case.
			recBatch = recBatch[:]
		}
	}
