...
type Config struct {
	// DB is the boltdb instance to hold store the indexed bucket.
	DB *bolt.DB
...
// NewIndexedBucket returns a new instance of IndexedBucket. Since it embeds bolt.DB it is
// up to the caller to call Close() when the IndexedBucket instance is no longer needed.
...
func (ix *IndexedBucket) ReadTx(keys []string, tx *bolt.Tx) ([]Record, error) {
	return ix.readRecs(keys, tx)
}

// ReadIndex returns the record keys for the given index name and list of index valueix.
...
func (ix *IndexedBucket) WriteTx(inputRecs []Record, writeFn WriteFn, tx *bolt.Tx) error {
...
func (ix *IndexedBucket) DeleteTx(keys []string, tx *bolt.Tx) error {
