...
func (ix *IndexedBucket) indexLookup(bucket *bolt.Bucket, key string) []string {
	...
	for k, _ := cursor.Seek(searchPrefix); bytes.HasPrefix(k, searchPrefix); k, _ = cursor.Next() {
		...
		if keyPrefixLen < searchPrefixLen {
			// In case the searchPrefix matches a shorter key and part of the parent id.
			continue
		} else if keyPrefixLen > searchPrefixLen {
			// searchPrefix is shorter than the index value. We can stop the scan since
			// we are past all exact matches. This would be valid if we did a prefix scan.
			break
		}
		ret = append(ret, val)
	}
