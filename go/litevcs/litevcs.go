...
func (li *liteVCS) IndexOf(ctx context.Context, hash string) (int, error) {
	...
	result, err := li.gitStore.Get(ctx, []string{hash})
	if err != nil {
		return 0, err
	}
	...
	idxCommits, err := li.gitStore.RangeByTime(ctx, result[0].Timestamp, result[0].Timestamp.Add(time.Second), li.defaultBranch)
	if err != nil {
		return 0, err
	}

	for _, c := range idxCommits {
		if c.Hash == hash {
			return c.Index, nil
		}
	}
