...
type strTimestamped struct {
	key    string
	commit *vcsinfo.IndexCommit
}

type srTimestampedCommits struct {
	errs    []*multierror.Error
	results [][]*strTimestamped
	retSize int64
}

func newSRTimestampedCommits(shards uint32) *srTimestampedCommits {
	return &srTimestampedCommits{
		results: make([][]*strTimestamped, shards),
		errs:    make([]*multierror.Error, shards),
	}
}

func (s *srTimestampedCommits) Add(shard uint32, row bigtable.Row) error {
	strKey := keyFromUniqueTimestamp(row.Key())
	// sklog.Infof("Ysdldf: %s", row.Key())
	...
	s.results[shard] = append(s.results[shard], &strTimestamped{
		key: strKey,
		commit: &vcsinfo.IndexCommit{
			Hash:      hash,
			Timestamp: timeStamp.Time().UTC(),
...
func (s *srTimestampedCommits) Finish(shard uint32) {
	atomic.AddInt64(&s.retSize, int64(len(s.results)))
}

func (s *srTimestampedCommits) Sorted() []*vcsinfo.IndexCommit {
	tmp := make([]*strTimestamped, 0, s.retSize)
	for _, sr := range s.results {
		tmp = append(tmp, sr...)
	}
	sort.Slice(tmp, func(i, j int) bool { return tmp[i].key < tmp[j].key })

	ret := make([]*vcsinfo.IndexCommit, len(tmp))
	for idx, t := range tmp {
		ret[idx] = t.commit
	}
	return ret
...
func (r *rawNodesResult) Merge() [][]string {
