...
func (b *btGitStore) Put(ctx context.Context, commits []*vcsinfo.LongCommit) error {
	...
	for idx, commit := range commits {
		...
		tsRowName := b.rowName(typTimeStamp, uniqueTimestamp(commit.Timestamp, commit.Hash), "")
...
func (s *srIndexCommits) Add(shard int32, row bigtable.Row) error {
	...
	for _, col := range row[cfCommit] {
		if strings.TrimPrefix(col.Column, cfCommitPrefix) == colHash {
...
func (b *btGitStore) RangeN(ctx context.Context, startIndex, endIndex int, branch string) ([]*vcsinfo.IndexCommit, error) {
	...
	result := newSRIndexCommits(b.shards)
	err := b.iterShardedRange(ctx, startTS, endTS, typIndex, "", result)
	if err != nil {
		return nil, err
	}
	return result.Sorted(), nil
...
func (b *btGitStore) iterShardedRange(ctx context.Context, startKey, endKey, rowType string, colFilter, branch string, result shardedResults) error {
	...
	for shard := int32(0); shard < b.shards; shard++ {
		func(shard int32) {
			egroup.Go(func() error {
				...
				rStart := b.shardedRowName(shard, rowType, startKey, branch)
				rEnd := b.shardedRowName(shard, rowType, endKey, branch)
...
func (b *btGitStore) rowName(rowType string, key string, branch string) string {
	return b.shardedRowName(crc32.ChecksumIEEE([]byte(key))%uint32(b.shards), rowType, key, branch)
}

func (b *btGitStore) shardedRowName(shard int32, rowType, key string, branch string) string {
	return fmt.Sprintf("%d:%s:%s:%s%s", shard, b.repoID, branch, rowType, key)
}
...
func uniqueTimestamp(ts time.Time, hash string) string {
	return searchableTimestamp(ts) + ":" + hash
}
