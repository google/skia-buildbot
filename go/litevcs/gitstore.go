	...
	for _, branch := range branches {
		func(branch *git.Branch) {
			egroup.Go(func() error {
				return b.updateBranch(ctx, branch, repoInfoRowName, repoInfo)
			})
		}(branch)
	}
...
func (b *btGitStore) updateBranch(ctx context.Context, newBranch *git.Branch, repoInfoRowName string, repoInfo *tRepoInfo) error {
	startIdx := 0
	withinBranchIndex := 0

	currBranchInfo, ok := repoInfo.Branches[newBranch.Name]
	if ok {
		sklog.Infof("Found branch %s with commit and index: %s     %d", newBranch.Name, currBranchInfo.Head, currBranchInfo.Index)
	...
	if indices[0] < 0 {
		return skerr.Fmt("New HEAD commit %s for branch %s not found in repo %s", newBranch.Head, newBranch.Name, b.repoURL)
	}
	endIdx := indices[0] + 1
	...
	if err := b.writeCommits(branchLongCommits, indexCommits, newBranch.Name, true); err != nil {
		return err
	}

	// Write the new branch to the Repo
	idxHead := indexCommits[len(indexCommits)-1]
	idxBuf := make([]byte, 8)
	binary.BigEndian.PutUint64(idxBuf, uint64(idxHead.Index))
	pointer := idxHead.Hash + ":" + string(idxBuf)
	mut := bigtable.NewMutation()
	now := bigtable.Now()
	mut.Set(cfBranches, newBranch.Name, now, []byte(pointer))
	mut.DeleteTimestampRange(cfBranches, newBranch.Name, 0, now)

	sklog.Infof("B: %s   %s   %d", newBranch.Name, idxHead.Hash, idxHead.Index)
...
func (b *btGitStore) RangeN(ctx context.Context, startIndex, endIndex int, branch string) ([]*vcsinfo.IndexCommit, error) {
	...
	sklog.Infof("RangeN: %s -> %s", startIdx, endIdx)
...
func (r rowMap) GetStrMap(colFamName string) map[string]string {
	...
	for _, col := range r[colFamName] {
		...
		sklog.Infof("---> %s  -->   %s", trimmed, string(col.Value))
