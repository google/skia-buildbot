...
func TestMaster(t *testing.T) {
	...
	repoDir := skiaRepoDir
	ctx := context.TODO()
	targetBranches := util.NewStringSet([]string{"master"})
	...
	for idx, branchName := range branchNames {
		if !targetBranches[branchName] {
			continue
		}
		sklog.Infof("Processing branch: %s", branchName)
		indexCommits, err := gitStore.RangeN(ctx, 0, int(math.MaxInt32), branchName)
		assert.NoError(t, err)
		expectedHashes := branchCommits[idx]

		for idx, hash := range expectedHashes {
			found := indexCommits[idx].Hash
			if found != hash {
				sklog.Infof("Error hash:   %10d   %s       !=           %s ", idx, hash, found)
			}
		}
	}
}
...
func testGitStore(t *testing.T, repoURL, repoDir string, freshLoad bool) {
	...
	for branchIdx, branchName := range branchNames {
		...
		sklog.Infof("Found commit len: %d  %d   - expected commits len: %d", len(foundIndexCommits), len(foundLongCommits), len(expHashes))
		...
		sklog.Infof("Branch %s verified", branchName)
