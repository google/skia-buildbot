...
var grandfatheredTestifyPackages = []string{
...
func TestNoPackageImportsTestify(t *testing.T) {
	...
	for _, p := range unwantedTestifyPackages {
		...
		for _, i := range importers {
			if !strings.HasSuffix(i, "testutil") && !strings.HasSuffix(i, "testutils") && !util.In(i, testifyExceptions) && !util.In(i, grandfatheredTestifyPackages) {
