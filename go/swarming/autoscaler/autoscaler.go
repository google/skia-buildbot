...
/*
	...
	as, _ := NewAutoscaler("my-gce-project", "zone-1", "swarming-server.com", client, instances)
...
// NewAutoscaler returns an Autoscaler instance. The given http.Client must be
// authenticated for all AUTH_SCOPES. All instances must have the same
// dimensions.
...
func (a *Autoscaler) updateInstanceStatuses() error {
	...
	for _, instance := range instances {
		...
		group.Go(instance, func() error {
			...
			if err != nil {
				if strings.Contains(err.Error(), "statuscode 404") || strings.Contains(strings.ToLower(err.Error()), "not found") || strings.Contains(err.Error(), "response code 404") {
