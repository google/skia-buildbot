...
func Setup(t assert.TestingT) (*Autoscaler, *mockhttpclient.URLMock, []*gce.Instance) {
...
func mockGCEInstanceStatus(t assert.TestingT, urlMock *mockhttpclient.URLMock, name string, status string) {
	rv := &compute.Instance{
		Name: name,
...
func mockSwarmingStatus(t assert.TestingT, urlMock *mockhttpclient.URLMock, name string, online, busy bool, offlineType string) {
	...
	if !online {
		if offlineType == offlineDeleted {
			rv.Deleted = true
		} else if offlineType == offlineDead {
			rv.IsDead = true
		} else if offlineType == offlineError {
...
func MockStop(t assert.TestingT, urlMock *mockhttpclient.URLMock, name string) {
...
func MockStart(t assert.TestingT, urlMock *mockhttpclient.URLMock, name string) {
	rv := &compute.Operation{
