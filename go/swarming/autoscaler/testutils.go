...
func Setup(t testutils.TestingT) (*Autoscaler, *mockhttpclient.URLMock, []*gce.Instance) {
	urlMock := mockhttpclient.NewURLMock()
	instances := autoscaler.GetInstanceRange(1, 100, getInstance)
	MockAllOnline(t, urlMock, instances)
	// Do this twice because the GCE scaler updates itself when we
	// instantiate it, and we update ourself (and thus the GCE scaler) when
	// we are instantiated.
	MockAllGCEInstancesOnline(t, urlMock, instances)
...
func mockGCEInstanceStatus(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string, status string) {
	rv := &compute.Instance{
		Name: name,
	...
	url := fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/instances/%s?alt=json&prettyPrint=false", testProject, testZone, name)
...
func MockSwarmingStatuses(t testutils.TestingT, urlMock *mockhttpclient.URLMock, statuses map[string]*FakeSwarmingStatus) {
...
func MockOnline(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string) *FakeSwarmingStatus {
...
func MockOnlineAndBusy(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string) *FakeSwarmingStatus {
	mockGCEInstanceStatus(t, urlMock, name, "RUNNING")
	return &FakeSwarmingStatus{
		Online: true,
		Busy:   true,
...
func MockOffline(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string) *FakeSwarmingStatus {
...
// bots) are online.
func MockAllGCEInstancesOnline(t testutils.TestingT, urlMock *mockhttpclient.URLMock, instances []*gce.Instance) map[string]*FakeSwarmingStatus {
...
func MockAllOnline(t testutils.TestingT, urlMock *mockhttpclient.URLMock, instances []*gce.Instance) {
...
func MockAllOffline(t testutils.TestingT, urlMock *mockhttpclient.URLMock, instances []*gce.Instance) {
	status := make(map[string]*FakeSwarmingStatus, len(instances))
	for _, instance := range instances {
...
func MockStop(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string) {
	...
	js = testutils.MarshalJSON(t, rv)
	urlMock.MockOnce(fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/instances/%s/stop?alt=json&prettyPrint=false", testProject, testZone, name), mockhttpclient.MockPostDialogue("", nil, []byte(js)))
	urlMock.MockOnce(fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/operations/%s?alt=json&prettyPrint=false", testProject, testZone, rv.Name), mockhttpclient.MockGetDialogue([]byte(js)))
	mockGCEInstanceStatus(t, urlMock, name, "TERMINATED")
}

// Mock the API calls to start an instance.
func MockStart(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string) {
	rv := &compute.Operation{
	...
	js := testutils.MarshalJSON(t, rv)
	urlMock.MockOnce(fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/instances/%s/start?alt=json&prettyPrint=false", testProject, testZone, name), mockhttpclient.MockPostDialogue("", nil, []byte(js)))
	urlMock.MockOnce(fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/operations/%s?alt=json&prettyPrint=false", testProject, testZone, rv.Name), mockhttpclient.MockGetDialogue([]byte(js)))
	mockGCEInstanceStatus(t, urlMock, name, "RUNNING")
