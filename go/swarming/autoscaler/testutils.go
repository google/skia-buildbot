...
func mockGCEInstanceStatus(t testutils.TestingT, urlMock *mockhttpclient.URLMock, name string, status string) {
	...
	urlMock.MockOnce(fmt.Sprintf("https://www.googleapis.com/compute/beta/projects/%s/zones/%s/instances/%s?alt=json&prettyPrint=false", testProject, testZone, name), mockhttpclient.MockGetDialogue([]byte(js)))
}
