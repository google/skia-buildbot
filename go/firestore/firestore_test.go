...
func TestGetAllDescendants(t *testing.T) {
	...
	check(ca, []*firestore.DocumentRef{la})
	check(nc, []*firestore.DocumentRef{ch})
	check(topLevelDoc, []*firestore.DocumentRef{ca, la, ny, nyc, nc, ch})
	...
	assert.NoError(t, err)
	check(topLevelDoc, []*firestore.DocumentRef{ca, la, nc, ch})

	// Also test RecursiveDelete.
	assert.NoError(t, RecursiveDelete(c, ca, attempts, timeout))
	check(topLevelDoc, []*firestore.DocumentRef{nc, ch})
	assert.NoError(t, RecursiveDelete(c, topLevelDoc, attempts, timeout))
	check(topLevelDoc, []*firestore.DocumentRef{})
}
