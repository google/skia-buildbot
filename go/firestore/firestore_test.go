...
func TestIterDocs(t *testing.T) {
	...
	foundEntries := 0
	assert.NoError(t, c.IterDocs(q, attempts, timeout, func(doc *firestore.DocumentSnapshot) error {
		foundEntries++
		return nil
	}))
	...
	assert.NoError(t, c.IterDocs(q, attempts, timeout, appendEntry))
	...
	alreadyFailed := false
	found = make([]*testEntry, 0, total)
	err = c.IterDocs(q, attempts, timeout, func(doc *firestore.DocumentSnapshot) error {
	...
	assert.NoError(t, c.IterDocsInParallel(queries, attempts, timeout, func(idx int, doc *firestore.DocumentSnapshot) error {
		var e testEntry
		if err := doc.DataTo(&e); err != nil {
			return err
		}
