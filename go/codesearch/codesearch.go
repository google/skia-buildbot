...
func (cs CodeSearch) urlForQuery(q string, params url.Values) string {
	...
	return fmt.Sprintf("%s%s?search_request=b&%s&search_request=e", Origin, jsonQueryPath, encodedQuery)
}

// Query runs a search against the given search service.
//
// The query string should conform to
func (cs CodeSearch) Query(q string, params url.Values) (SearchResponse, error) {
	_ = cs.urlForQuery(q, params)
