...
// WithRetry4XX sets the backoff config to retry all non-2XX status codes. This
...
func (t *BackOffTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	notifyFunc := func(notifyErr error, wait time.Duration) {
		if notifyErr == retryableErr || notifyErr == permanentErr {
			...
			if resp != nil {
				panic("Expected retryableErr or permanentErr when resp is non-nil")
			}
		}
	}
