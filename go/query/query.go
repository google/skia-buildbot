...
func appendRegexForFilter(key, keyIndex string, ops *paramtools.OrderedParamSet, part queryParam, ret *[]string, filter func(string) bool) error {
		...
		for _, part := range q.params {
			...
			if partKey == key {
				...
				if part.isWildCard {
					...
					err = appendRegexForFilter(key, keyIndex, ops, part, &ret, func(value string) bool {
