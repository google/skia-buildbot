...
func TestScheduleBuilds(t *testing.T) {
	...
	builds, err := c.ScheduleBuilds(context.TODO(), []string{builderName}, map[string]map[string]string{builderName: map[string]string{tagName: tagValue}}, change, patchset, gerritHost, repo, bbProject, bbBucket)
	require.NoError(t, err)
