...
type BuildBucketInterface interface {
	...
	// GetTrybotsForEqualivalentPatchSets retrieves trybot results for the given
	// CL for patchsets till we get to the first code change patchset.
	// REMOVE THIS
	GetTrybotsForEquivalentPatchSets(ctx context.Context, issue int64, patchset []int64, gerritUrl string) ([]*buildbucketpb.Build, error)
	// ScheduleBuilds schedules the specified builds on the given CL.
	ScheduleBuilds(ctx context.Context, builds []string, buildsToExperimental map[string]bool, issue, patchset int64, gerritUrl, repo, bbProject, bbBucket string) ([]*buildbucketpb.Build, error)
...
// Needs tests.
func (c *Client) ScheduleBuilds(ctx context.Context, builds []string, buildsToExperimental map[string]bool, issue, patchset int64, gerritURL, repo, bbProject, bbBucket string) ([]*buildbucketpb.Build, error) {
	requests := []*buildbucketpb.BatchRequest_Request{}
	for _, b := range builds {
		experimental, ok := buildsToExperimental[b]
		if !ok {
			experimental = false
		}
			...
			Request: &buildbucketpb.BatchRequest_Request_ScheduleBuild{
				ScheduleBuild: &buildbucketpb.ScheduleBuildRequest{
					...
					Tags: []*buildbucketpb.StringPair{
						{
							Key:   "cq_experimental",
							Value: strconv.FormatBool(experimental),
						},
					},
...
// This does not work remove it.
// GetTrybotsForEqualivalentPatchSets implements the BuildBucketInterface.
func (c *Client) GetTrybotsForEquivalentPatchSets(ctx context.Context, issue int64, patchsets []int64, gerritUrl string) ([]*buildbucketpb.Build, error) {
	pred, err := common.GetTrybotsForMultiplePatchSetsPredicate(issue, patchsets, gerritUrl)
	if err != nil {
		return nil, err
	}
	return c.Search(ctx, pred)
