...
type BuildBucketInterface interface {
	...
	ScheduleBuilds(ctx context.Context, builds *buildbucketpb.BuilderID, change *buildbucketpb.GerritChange) (string, error)
}
...
// String should be a status here!
func (c *Client) ScheduleBuilds(ctx context.Context, builds []*buildbucketpb.BuilderID, change *buildbucketpb.GerritChange) (string, error) {

	fields := field_mask.FieldMask{
		Paths: []string{"id", "create_time", "created_by", "status", "tags"},
	}
	requests := []*buildbucketpb.BatchRequest_Request{}
	for _, b := range builds {

	}
	batchRequest := buildbucketpb.BatchRequest{
		Requests: requests,
	}
