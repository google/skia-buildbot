...
type BuildBucketInterface interface {
	...
	ScheduleBuilds(ctx context.Context, builds []string, issue, patchset int64, gerritUrl, repo, bbProject, bbBucket string) error
}
...
// Needs tests.
func (c *Client) ScheduleBuilds(ctx context.Context, builds []string, issue, patchset int64, gerritURL, repo, bbProject, bbBucket string) error {
	/*
		request := &buildbucketpb.ScheduleBuildRequest{
			Builder: &buildbucketpb.BuilderID{
				...
				// Bucket:  "skia.primary",
				Bucket:  "skia.testing",
				Builder: "Build-Debian9-Clang-x86_64-Debug",
			},
		...
		resp, err := c.bc.ScheduleBuild(ctx, request)
		if err != nil {
			return skerr.Fmt("Could not schedule build on buildbucket: %s", err)
		}
		// Verify that it was scheduled or there will be no way to tell anything.

		fmt.Println("LOOKING AT RESP")
		fmt.Println(resp)
		// fmt.Println("WAITING FOR 5 MINS incase problem is the context cancelling")
	...
	*/
	...
	if err != nil {
		return skerr.Fmt("Could not schedule builds on buildbucket: %s", err)
	}
	if len(resp.Responses) != len(builds) {
		return skerr.Fmt("Buildbucket gave %d responses for %d builders", len(resp.Responses), len(builds))
	}
	// fmt.Println(resp.)
	fmt.Println("DEBUG RESPONSES")
	for _, r := range resp.Responses {
		fmt.Println(r)
		// fmt.Println(r.String())
		// CHECK FOR ERROR AND RETURN BUILDS
		fmt.Println(r.GetError())
		fmt.Println(r.GetGetBuild())
		fmt.Println("------------")
	}
	// Make sure everything was successfully scheduled otherwise we will have a problem!
	// But then will have to make extra calls..
