...
func makeSVal(s string) *structpb.Value {
	return &structpb.Value{Kind: &structpb.Value_StringValue{s}}
}

func makeIVal(i int64) *structpb.Value {
	return &structpb.Value{Kind: &structpb.Value_NumberValue{float64(i)}}
}
		...
		Input: &buildbucketpb.Build_Input{
			Properties: &structpb.Struct{
				Fields: map[string]*structpb.Value{
					...
					//"patch_ref":            makeSVal("refs/changes/45/12345/1"),
					//"patch_repository_url": makeSVal("https://skia.googlesource.com/skia"),
