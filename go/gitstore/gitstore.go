...
func (b *btGitStore) PutBranches(ctx context.Context, branches map[string]string) error {
	...
	for branchName, head := range branches {
		// Assume we start out with a completely fresh branch (no Head and first index 0)
		oldHeadPtr := &BranchPointer{Head: "", Index: 0}
...
// Note: oldBranchPointer must never be nil. If there is no previous branch it must contain
//   Head == "" and Index == 0.
