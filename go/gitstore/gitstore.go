...
type GitStore interface {
	...
	// PutBranches updates the branches in the repository. It writes indices for the branches so they
	...
	// RangeN returns all commits in the half open index range [startIndex, endIndex). Thus not including endIndex.
...
type RepoInfo struct {
	// Numeric id of the repo. This is unique within all repos in a BT table.
	ID int64
...
func AllRepos(ctx context.Context, conf *BTConfig) (map[string]*RepoInfo, error) {
	// Create the client.
	client, err := bigtable.NewClient(context.TODO(), conf.ProjectID, conf.InstanceID)
	if err != nil {
...
// searchableTimestamp returns a timestamp as a string (in seconds) that can used as a key in BT.
func searchableTimestamp(ts time.Time) string {
	return fmt.Sprintf("%012d", util.MinInt64(999999999999, ts.Unix()))
}

// searchableIndex returns an index as a string that can be used as a key in BT.
func searchableIndex(index int) string {
	return fmt.Sprintf("%08d", util.MinInt(999999999, index))
}

// parseIndex parses an index previously generated with searchableIndex.
