...
func newClient(ctx context.Context, btProject, btInstance, btTable string, repoID int64, ts oauth2.TokenSource, createTopic bool) (*client, error) {
	c, err := pubsub.NewClient(ctx, btProject, option.WithTokenSource(ts))
	if err != nil {
		return nil, skerr.Wrapf(err, "failed to create PubSub client")
	}
	t := c.Topic(topicName(btInstance, btTable, repoID))
...
func NewPublisher(ctx context.Context, btProject, btInstance, btTable string, repoID int64, ts oauth2.TokenSource) (*Publisher, error) {
	client, err := newClient(ctx, btProject, btInstance, btTable, repoID, ts, true)
	if err != nil {
	...
	// TODO(borenet): Any reason not to do this?
	//cleanup.AtExit(func() {
	//	p.Wait()
	//})
	return p, nil
...
// callback function is responsible for calling Ack() or Nack() on the message.
func NewSubscriber(ctx context.Context, btProject, btInstance, btTable, subscriberID string, repoID int64, ts oauth2.TokenSource, callback func(*pubsub.Message, map[string]string)) error {
	c, err := newClient(ctx, btProject, btInstance, btTable, repoID, ts, false)
	if err != nil {
