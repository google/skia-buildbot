...
func TestPubSub(t *testing.T) {
	...
	p, err := NewPublisher(ctx, btProject, btInstance, btTable, repoID, nil)
	assert.NoError(t, err)
	ch := make(chan map[string]string)
	err = NewSubscriber(ctx, btProject, btInstance, btTable, subID, repoID, nil, func(msg *pubsub.Message, branches map[string]string) {
		ch <- branches
		msg.Ack()
	})
	assert.NoError(t, err)
	...
	msgs := []map[string]string{
		map[string]string{"a": "a1"},
		map[string]string{"a": "a2"},
		map[string]string{"b": "a1"},
		map[string]string{"b": "b1"},
		map[string]string{
			"a": "a4",
			"b": "b3",
		},
...
func TestAutoUpdateGraph(t *testing.T) {
	unittest.LargeTest(t)
	...
	wait, err := autoUpdateGraph(ctx, btProject, btInstance, btTable, subID, repoID, graph, nil, tickCh, func(ctx context.Context, g *repograph.Graph, ack, nack func()) error {
