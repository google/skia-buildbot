...
func (b *BigTableGitStore) Get(ctx context.Context, hashes []string) ([]*vcsinfo.LongCommit, error) {
	...
	err := util.ChunkIterParallel(ctx, len(rowNames), getBatchSize, func(eCtx context.Context, bStart, bEnd int) error {
		bRowNames := rowNames[bStart:bEnd]
		batchIdx := int64(bStart - 1)
		err := b.readRows(eCtx, bRowNames, func(row bigtable.Row) bool {
