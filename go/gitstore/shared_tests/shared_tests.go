...
func TestGitStore(t sktest.TestingT, gs gitstore.GitStore) {
	...
	require.Nil(t, lcs[0])
	require.Nil(t, lcs[1])
	deepequal_testutils.AssertDeepEqual(t, c0, lcs[2])
	ics, err = gs.RangeN(ctx, math.MinInt32, math.MaxInt32, gitstore.ALL_BRANCHES)
	require.NoError(t, err)
	require.Equal(t, 1, len(ics))
	deepequal_testutils.AssertDeepEqual(t, c0.IndexCommit(), ics[0])
	ics, err = gs.RangeByTime(ctx, time.Time{}, vcsinfo.MaxTime, gitstore.ALL_BRANCHES)
	require.NoError(t, err)
	require.Equal(t, 1, len(ics))
	deepequal_testutils.AssertDeepEqual(t, c0.IndexCommit(), ics[0])
	...
	deepequal_testutils.AssertDeepEqual(t, c1, lcs[0])
	ics, err = gs.RangeN(ctx, math.MinInt32, math.MaxInt32, gitstore.ALL_BRANCHES)
	require.NoError(t, err)
	...
	require.Equal(t, 2, len(ics))
	deepequal_testutils.AssertDeepEqual(t, c0.IndexCommit(), ics[0])
	deepequal_testutils.AssertDeepEqual(t, c1.IndexCommit(), ics[1])
	ics, err = gs.RangeByTime(ctx, vcsinfo.MinTime, vcsinfo.MaxTime, master)
	require.NoError(t, err)
	require.Equal(t, 2, len(ics))
	deepequal_testutils.AssertDeepEqual(t, c0.IndexCommit(), ics[0])
	...
	require.NoError(t, err)
	require.Equal(t, 1, len(lcs))
	deepequal_testutils.AssertDeepEqual(t, c2, lcs[0])
	...
	if ics[1].Hash == c1.Hash {
		deepequal_testutils.AssertDeepEqual(t, c1.IndexCommit(), ics[1])
		deepequal_testutils.AssertDeepEqual(t, c2.IndexCommit(), ics[2])
	} else {
		deepequal_testutils.AssertDeepEqual(t, c1.IndexCommit(), ics[2])
		deepequal_testutils.AssertDeepEqual(t, c2.IndexCommit(), ics[1])
	}
