...
func New(whitelist []string, projectNumber, backendServiceId string, h http.Handler) (http.Handler, error) {
...
func (i *iapHandler) IPinRange(remoteAddr string) error {
	ip, _, err := net.SplitHostPort(remoteAddr)
	if err != nil {
		return fmt.Errorf("Invalid IP:port format for %q: %s", remoteAddr, err)
	}

	userIP := net.ParseIP(ip)
	if userIP == nil {
		return fmt.Errorf("Invalid IP format for %q: %s", remoteAddr, err)
	}
	...
	return fmt.Errorf("IP not in acceptable range")
}
...
func (i *iapHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/" && jwtAssertion == "" {
		if err := i.IPinRange(r.RemoteAddr); err != nil {
			httputils.ReportError(w, r, fmt.Errorf("Healthcheck failed: %s", err), "Failed health check.")
			return
		} else {
	...
	if err == errInvalid {
		...
		token, err := parser.Parse(jwtAssertion, func(token *jwt.Token) (interface{}, error) {
			...
			if mapClaims["alg"] == nil || mapClaims["alg"].(string) != "ES256" {
