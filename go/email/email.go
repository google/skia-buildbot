...
func (a *GMail) SendWithMarkup(senderDisplayName string, to []string, subject, body, markup string, threadingData *ThreadingData) (string, error) {
	...
	// Get threadingData if specified.
	references := ""
	if threadingData != nil {
		references = threadingData.References
	}
	...
	if err := emailTemplateParsed.Execute(msgBytes, struct {
		...
		Subject    string
		References string
		InReplyTo  string /* Needed? just remove? */
		Body       template.HTML
	...
	//fmt.Printf("BEFORE These are the headers: %+v", req.Header())
	...
	//}
	//fmt.Printf("AFTER These are the headers: %+v", req.Header())
	m, err := req.Do()
	if err != nil {
...
func (a *GMail) GetThreadingData(messageId string) (*ThreadingData, error) {
	...
	references := ""
	inReplyTo := ""
	fmt.Println("THREAINDATA HEADERS")
	fmt.Println(m.Payload.Headers)
	for _, h := range m.Payload.Headers {
		if h.Name == "Message-Id" {
			references = h.Value
		} else if h.Name == "To" {
			inReplyTo = h.Value // REMOVE!
		}
	}
	if references == "" {
