...
func LinuxConfigs(vm *gce.Instance) *gce.Instance {
...
func LinuxSwarmingBot(num int, ipAddress string) *gce.Instance {
	return LinuxConfigs(Swarming20170523(fmt.Sprintf("skia-vm-%03d", num), ipAddress))
}

// Internal Linux GCE instances.
func InternalLinuxSwarmingBot(num int, ipAddress string) *gce.Instance {
	vm := LinuxConfigs(Swarming20170523(fmt.Sprintf("skia-i-vm-%03d", num), ipAddress))
...
func SkiaCTBot(num int, ipAddress string) *gce.Instance {
	vm := LinuxConfigs(Swarming20170523(fmt.Sprintf("skia-ct-vm-%03d", num), ipAddress))
	vm.DataDisk.SizeGb = 3000
	return vm
}

// Configs for Windows GCE instances.
func WinConfigs(vm *gce.Instance, ipAddress, pw, setupScriptPath, startupScriptPath, chromebotScript string) *gce.Instance {
...
func WinSwarmingBot(num int, ipAddress, pw, setupScriptPath, startupScriptPath, chromebotScript string) *gce.Instance {
	...
	return WinConfigs(vm, ipAddress, pw, setupScriptPath, startupScriptPath, chromebotScript)
}
