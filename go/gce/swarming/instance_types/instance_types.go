...
const (
	...
	WIN_SOURCE_IMAGE             = "projects/windows-cloud/global/images/windows-server-2016-dc-v20180710"
)

var (
	// "Constants"
	SETUP_SCRIPT_LINUX_PATH    = path.Join("go", "gce", "swarming", "setup-script-linux.sh")
	SETUP_SCRIPT_WIN_PATH      = path.Join("scripts", "win_setup.ps1")
	STARTUP_SCRIPT_WIN_PATH    = path.Join("scripts", "win_startup.ps1")
	CHROME_BOT_SCRIPT_WIN_PATH = path.Join("scripts", "chromebot-schtask.ps1")

	internalNameRegexp = regexp.MustCompile("^skia-")
)
...
func Internal(vm *gce.Instance) *gce.Instance {
	vm.Name = internalNameRegexp.ReplaceAllString(vm.Name, INTERNAL_NAME_PREFIX)
...
func GetLinuxScripts(ctx context.Context, checkoutRoot, workdir string) (string, error) {
	return path.Join(checkoutRoot, SETUP_SCRIPT_LINUX_PATH), nil
}
...
func GetWindowsScripts(ctx context.Context, checkoutRoot, workdir string) (string, string, string, error) {
	...
	setupBytes, err := ioutil.ReadFile(path.Join(checkoutRoot, SETUP_SCRIPT_WIN_PATH))
	if err != nil {
		return "", "", "", err
	}
	...
	startupBytes, err := ioutil.ReadFile(path.Join(checkoutRoot, STARTUP_SCRIPT_WIN_PATH))
	if err != nil {
		return "", "", "", err
	}
	...
	chromebotBytes, err := ioutil.ReadFile(path.Join(checkoutRoot, CHROME_BOT_SCRIPT_WIN_PATH))
	if err != nil {
		return "", "", "", err
	}
