...
sudo apt-get --assume-yes install build-essential mercurial libosmesa-dev libexpat1-dev clang llvm \
  ...
  libc++-dev libc++abi-dev gperf bison usbutils lsb-release
...
TOKEN="$(curl "${TOKEN_URL}" -H "Metadata-Flavor: Google" | \
...
HOSTNAME=`hostname`
curl ${SWARMING}/bot_code?bot_id=$HOSTNAME -H "Authorization":"Bearer $TOKEN" \
  -o /b/s/swarming_bot.zip
