...
func setAll(ci *ChangeInfo, labels map[string]interface{}) {
	for key, value := range labels {
		set(ci, key, value.(int))
	}
}
...
func unsetAll(ci *ChangeInfo, labels map[string]interface{}) {
	for key, value := range labels {
		unset(ci, key, value.(int))
	}
}
