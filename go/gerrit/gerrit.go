...
func (c *CodeReviewCache) Add(key string, value *ChangeInfo) {
	c.mutex.Lock()
...
func (c *CodeReviewCache) Get(key string) (*ChangeInfo, bool) {
	fmt.Printf("\nGetting: %s", key)
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *CodeReviewCache) poll() {
	...
	for _, issue := range issues {
		fmt.Printf("\nRemoving: %s", issue.ChangeId)
		c.cache.Remove(issue.ChangeId)
	}
