...
const (
	...
	CODEREVIEW_STATE_DISAPPROVE = -1
	CODEREVIEW_STATE_NONE       = 0
	CODEREVIEW_STATE_APPROVE    = 1

	COMMITQUEUE_STATE_NONE    = 0
	COMMITQUEUE_STATE_DRY_RUN = 1
	COMMITQUEUE_STATE_SUBMIT  = 2
)
...
func (g *Gerrit) SendToDryRun(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{COMMITQUEUE_LABEL: COMMITQUEUE_STATE_DRY_RUN})
}

func (g *Gerrit) SendToCQ(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{COMMITQUEUE_LABEL: COMMITQUEUE_STATE_SUBMIT})
}

func (g *Gerrit) RemoveFromCQ(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{COMMITQUEUE_LABEL: COMMITQUEUE_STATE_NONE})
}

// Utility methods for interacting with the CODEREVIEW_LABEL.

func (g *Gerrit) Approve(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{CODEREVIEW_LABEL: CODEREVIEW_STATE_APPROVE})
}

func (g *Gerrit) NoScore(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{CODEREVIEW_LABEL: CODEREVIEW_STATE_NONE})
}

func (g *Gerrit) DisApprove(issue *ChangeInfo, message string) error {
	return g.setReview(issue, message, map[string]interface{}{CODEREVIEW_LABEL: CODEREVIEW_STATE_DISAPPROVE})
}
