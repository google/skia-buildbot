...
func (g *Gerrit) GetIssueProperties(changeId string) (*ChangeInfo, error) {
	...
	// Make patchset objects with the created timestamps.
	patchsets := []PatchSet{}
	for _, r := range fullIssue.Revisions {
		...
		patchsets = append(patchsets, PatchSet{Created: parseTime(r.CreatedString)})
	}
