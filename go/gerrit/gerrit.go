...
//type Label struct {
//	LabelType    map[string]LabelValue
//	DefaultValue int
//}
...
// TODO(rmistry): Add boolean to approve as well!
...
// rmistry: do here!
// TODO(rmistry): Do not get full issue properties here. Only return what needs to be returned. Callers can then get more info if they need it.
...
func (g *Gerrit) Search(limit int, terms ...*SearchTerm) ([]*ChangeInfo, error) {
	...
	for {
		...
		for _, issue := range data {
			...
			fullIssue, err := g.GetIssueProperties(issue.Issue)
			if err != nil {
				return nil, err
			}
			...
			fmt.Println(fullIssue.Revisions)
			//fmt.Println("LABELS!!")
			//fmt.Println(fullIssue.Labels)
			//fmt.Println(fullIssue.Labels[COMMITQUEUE_LABEL]["all"].Value)
			//fmt.Println(fullIssue.Labels[CODEREVIEW_LABEL]["all"].Value)
