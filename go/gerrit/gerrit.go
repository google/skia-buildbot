...
func (g *Gerrit) GetIssueProperties(changeId string) (*ChangeInfo, error) {
	...
	for id, r := range fullIssue.Revisions {
		...
		//p.Created = parseTime(p.CreatedString)
		//fullIssue.Patchsets[k].Created = parseTime(fullIssue.Patchsets[k].CreatedString)
		//fmt.Println(p.CreatedString)
		//fmt.Println(p.Created)
		//p := fullIssue.Patchsets[k]
		//fmt.Println(fullIssue.Patchsets[k].Created)
		//fullIssue.Patchsets[k].Created = parseTime(fullIssue.Patchsets[k].CreatedString)
		//fullIssue.Patchsets[k].Created = parseTime(p.CreatedString)
		patchsets = append(patchsets, &PatchSet{RevisionId: id, Created: parseTime(r.CreatedString)})
	}
	sort.Sort(patchsets)
	fullIssue.Patchsets = patchsets
	for _, p := range fullIssue.Patchsets {
...
// TODO(rmistry): Need to sort patchsets, keep newest first??
...
// NEEDED add a comment with signed in user I guess.
// Required
...
// setReview the Set Review endpoint of the Gerrit API to add messages and/or set labels for a
// patchset. API documentation:
// https://gerrit-review.googlesource.com/Documentation/rest-api-changes.html#set-review
func (g *Gerrit) setReview(changeId, revisionId, message string, labels map[string]string) error {
	return nil
}

//HERE HERE
// Set revisionId to the PatchSet.
// AddComment adds a message for a patchset.
...
func (g *Gerrit) AddComment(issue *ChangeInfo, message string) error {
	...
	return g.post(fmt.Sprintf("/a/changes/%s/revisions/%s/review", issue.ChangeId, latestPatchset.RevisionId), url.Values{})
}
...
func (g *Gerrit) Abandon(issue *ChangeInfo) error {
	return g.post(fmt.Sprintf("/a/changes/%s/abandon", issue.ChangeId), url.Values{})
}
...
func (g *Gerrit) GetCredentials(gitCookiesPath string) (map[string]string, error) {
...
func (g *Gerrit) post(suburl string, data url.Values) error {
	...
	//req := g.client.Post(g.url+suburl, strings.NewReader("message=Testing"))
	// message, notify, labels, tag
	// https://gerrit-review.googlesource.com/Documentation/rest-api-changes.html#set-review
	// https://cs.chromium.org/chromium/infra/infra/libs/gerrit_api/gerrit_api.py
	req, err := http.NewRequest("POST", g.url+suburl, strings.NewReader("{\"message\": \"testing\"}"))
	//req, err := http.NewRequest("POST", g.url+suburl, strings.NewReader(data.Encode()))
	if err != nil {
		return err
	}
	//req.Header.Set("Content-Type", "application/")
	creds, err := g.GetCredentials("/usr/local/google/home/rmistry/.gitcookies")
	if err != nil {
		return err
	}
	crUrl := g.url
	...
	req.Header.Set("Authorization", "Basic "+base64.StdEncoding.EncodeToString([]byte(creds[".googlesource.com"]))) // TODO(rmistry): Need some sort of Cookie domain matching stuff...
	req.Header.Set("Accept", "application/json")
	req.Header.Set("Accept-encoding", "gzip")
	req.Header.Set("Content-Type", "application/json")
	fmt.Println("mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm")
	fmt.Println(g.url + suburl)
	fmt.Println(creds[".googlesource.com"])
	fmt.Println("Basic " + base64.StdEncoding.EncodeToString([]byte(creds[".googlesource.com"])))
	resp, err := g.client.Do(req)

	////data.Add("xsrf_token", r.xsrfToken)
	//resp, err := g.client.PostForm(g.url+suburl, data)
...
func (g *Gerrit) Search(limit int, terms ...*SearchTerm) ([]*ChangeInfo, error) {
	...
	//searchUrl := fmt.Sprintf("/changes/?q=%s&n=%d", queryString(terms), queryLimit)
	...
	for {
		...
		for _, issue := range data {
			...
			for _, p := range fullIssue.Patchsets {
				fmt.Println("ppppp")
				fmt.Println(p.Created)
				fmt.Println(p.RevisionId)
			}
			//fmt.Println(fullIssue.Issue)
			if err != nil {
				return nil, err
			} else {
...
// TODO(rmistry): Find out if this works!!!
// TODO(rmistry): Try to get the number for the issues. Will need this for the cache and for the trybot stuff and other things in general!
// buildset:patch/gerrit/skia-review.googlesource.com/2370/2
...
func (r *Rietveld) GetTrybotResults(issueID int64, patchsetID int64) ([]*buildbucket.Build, error) {
	return buildbucket.NewClient(r.client).GetTrybotsForCL(issueID, patchsetID, "rietveld", r.url)
}
...
func (c *CodeReviewCache) poll() {
	...
	issues, err := c.gerritAPI.Search(10000, c.gerritAPI.SearchModifiedAfter(time.Now().Add(-c.timeDelta)))
	if err != nil {
	...
	fmt.Println("Lenght of the cache is:")
	fmt.Println(c.cache.Len())
}
