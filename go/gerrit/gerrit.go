...
func (g *Gerrit) SearchModifiedAfter(after time.Time) *SearchTerm {
	fmt.Println("IN MODIFIED STUFF!!!!!!!!!!")
	fmt.Println(strings.Trim(strings.Split(after.UTC().String(), "+")[0], " "))
	return &SearchTerm{
		Key:   "after",
...
func (g *Gerrit) Search(limit int, terms ...*SearchTerm) ([]*ChangeInfo, error) {
	searchUrl := fmt.Sprintf("/changes/?q=%s", queryString(terms))
	...
	for _, issue := range data {
		...
		fmt.Println(time.Parse(TIME_FORMAT, issue.Created))
		fmt.Println(issue.Updated)
		fmt.Println(issue.Submitted)
		fmt.Println(issue.Project)
		fmt.Println(issue.Number)
		// TODO(rmistry): Do same as riteveld in string and non string stuff.
		// TODO(rmistry): Do issue properties same as rietveld to get the details of an issue!
	}
	//for _, issue := range data.Results {
	//	fullIssue, err := g.GetIssueProperties(issue.Issue, false)
	//	if err != nil {
	//		return nil, err
	//	} else {
