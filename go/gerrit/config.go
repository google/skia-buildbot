...
var (
	CONFIG_ANDROID = &Config{
		...
		CqSuccessLabels: map[string][]int{
			...
			PRESUBMIT_VERIFIED_LABEL: []int{PRESUBMIT_VERIFIED_LABEL_ACCEPTED},
		},
		CqFailureLabels: map[string][]int{
			AUTOSUBMIT_LABEL:         []int{AUTOSUBMIT_LABEL_SUBMIT},
			PRESUBMIT_VERIFIED_LABEL: []int{PRESUBMIT_VERIFIED_LABEL_REJECTED},
		},
...
// allWithMultipleValues returns true iff all of the given label keys and one of
// their acceptable values are set on the change. Returns true if the given map of
// labels is empty.
