...
func TestGerritSearch(t *testing.T) {
	...
	api := NewGerrit("https://skia-review.googlesource.com", nil)
	...
	keys, err := api.Search(5, SearchModifiedAfter(time.Now().Add(-time.Hour)))
	assert.NoError(t, err)
	assert.Equal(t, 5, len(keys))

	// TODO(rmistry): Will need to test the polling crap as well!
}
