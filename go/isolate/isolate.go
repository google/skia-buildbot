...
func WriteIsolatedGenJson(t *Task, genJsonFile, isolatedFile string) error {
	...
	if err != nil {
		return skerr.Fmt("Failed to create %s: %s", genJsonFile, err)
	}
	if err := json.NewEncoder(f).Encode(&gen); err != nil {
		if err2 := f.Close(); err != nil {
			return skerr.Fmt("Failed to write %s: %s\nAnd failed to close the file with: %s", genJsonFile, err, err2)
		}
		return skerr.Fmt("Failed to write %s: %s", genJsonFile, err)
	}
