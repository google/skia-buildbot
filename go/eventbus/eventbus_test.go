...
func TestSynStorageEvents(t *testing.T) {
	...
	for i := 0; i < nEvents; i++ {
		eventBus.PublishStorageEvent(NewStorageEvent(TEST_BUCKET,
			fmt.Sprintf("whatever/path/somefile-%d", i), now, ""))
		eventBus.PublishStorageEvent(NewStorageEvent(TEST_BUCKET,
			fmt.Sprintf(TEST_PREFIX+"/whatever/path/somefile-%d", i), now, ""))
		eventBus.PublishStorageEvent(NewStorageEvent(TEST_BUCKET,
			fmt.Sprintf(TEST_PREFIX+"/whatever/path/somefile-%d.json", i), now, ""))
	}
