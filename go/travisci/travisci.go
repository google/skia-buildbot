...
func (t *TravisCI) GetLatestBuildDetails(pullNumber int) (*Build, error) {
	params := url.Values{}
	params.Add("limit", "1")
	params.Add("sort_by", "id:desc")
	params.Add("event_type", "pull_request")
	//params.Add("created_by", createdBy)
	params.Add("pull_request_number", strconv.Itoa(pullNumber))
	...
	if len(builds.Builds) == 0 {
		// No builds were found. This is ok. Return an empty Build obj
		return &Build{}, nil
	} else if len(builds.Builds) != 1 {
		// We should never get more than 1 build.
