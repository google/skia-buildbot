...
type emulatorInfo struct {
	...
	bazelRbeCmd string // Command and arguments to start the emulator on RBE.
...
func computeCockroachDBCmd() string {
	...
	cockroachDbStoreDir := os.Getenv("COCKROACHDB_STORE_DIR")
...
// GetEmulatorHost returns the contents of the *_EMULATOR_HOST environment variable corresponding to
// the given emulator, or the empty string if the environment variable is unset.
func GetEmulatorHost(emulator Emulator) string {
...
// GetEmulatorHostEnvVar returns the name of the *_EMULATOR_HOST environment variable corresponding
...
// StartEmulator starts an emulator. It panics if the emulator is already running.
func StartEmulator(emulator Emulator) error {
	if IsRunning(emulator) {
		panic(fmt.Sprintf("The %s emulator is already running.", emulator))
	}

	emulatorInfo := getEmulatorInfo(emulator)

	cmdString := emulatorInfo.cmd
	if bazel.InRBE() && emulatorInfo.bazelRbeCmd != "" {
		cmdString = emulatorInfo.bazelRbeCmd
	}

	programAndArgs := strings.Split(fmt.Sprintf(cmdString, emulatorInfo.port), " ")
	...
	if bazel.InRBE() {
		cmd.SysProcAttr = &syscall.SysProcAttr{
			...
			// alongside the tests which controls the emulator lifecycle. Any of these approaches would
...
func StopAllEmulators() error {
	...
	// IsRunning(emulator) should return false for all known emulators after this function call.
	for _, emulator := range AllEmulators {
