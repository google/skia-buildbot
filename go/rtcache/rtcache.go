...
type ReadThroughCache interface {
	...
	// GetAll returns all item identified by 'id' or an error if any of them
	// cannot be retrieved. If any items are not in the cache, a worker function
	// is called per item to retrieve them.
	GetAll(ctx context.Context, id []string) ([]interface{}, error)
...
// cache. 'ctx' and 'id' are the same that were passed to GetAll(...).
