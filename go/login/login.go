...
func isAuthorized(email string) bool {
	sklog.Infof("is authorized %s", email)
	parts := strings.Split(email, "@")
	if len(parts) != 2 {
	...
	if viewAllow != nil {
		sklog.Errorf("checking viewAllow %#v", viewAllow)
...
func setActiveAllowLists(authAllowList string) {
	...
	sklog.Infof("Domain sllow lists %#v", activeUserDomainAllowList)
	sklog.Infof("user allow lists %#v", activeUserEmailAllowList)
