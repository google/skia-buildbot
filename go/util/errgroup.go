...
func (g *NamedErrGroup) Wait() error {
	...
	for name, err := range g.errs {
		e := fmt.Sprintf("\t%s: %s\n", name, err)
		wrote, err := msg.Write([]byte(e))
		if err != nil {
			// According to the docs, err is always nil.
			sklog.Fatal(err)
		}
		if wrote != len(e) {
			// According to the docs, this shouldn't happen either.
			sklog.Fatalf("Failed to merge errors; wrote %d bytes instead of %d", wrote, len(e))
		}
	}
