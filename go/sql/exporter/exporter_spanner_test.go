...
type testTableUuid struct {
	TableUuid []tableUuidRow
}

type tableUuidRow struct {
	ColumnOne uuid.UUID `sql:"column_one UUID PRIMARY KEY"`
}

func TestGenerateSQL_UuidTranslatedByDefault(t *testing.T) {
	gen := GenerateSQL(testTableUuid{}, "test_package_one", SchemaOnly, Spanner, nil)
	// We cannot have backticks in the multistring literal, so we substitute $$ for them.
	expectedOutput := strings.ReplaceAll(`package test_package_one
...
const Schema = $$CREATE TABLE IF NOT EXISTS TableUuid (
  column_one TEXT PRIMARY KEY,
  createdat TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
) TTL INTERVAL '1095 days' ON createdat;
$$
...
func TestGenerateSQL_UuidKeptWithFlag(t *testing.T) {
	conv := DefaultSpannerConverter()
	conv.PostgresUUID = true
	gen := GenerateSQL(testTableUuid{}, "test_package_one", SchemaOnly, Spanner, conv)
	expectedOutput := strings.ReplaceAll(`package test_package_one
