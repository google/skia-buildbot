...
// to generate the SQL schema. If a malformed type is passed in, this function
// will panic.
...
// order they appear in the struct. A package name is taken in to be included in
// the returned string. If a malformed type is passed in, this function will
// panic.
//
...
func columnNames(inputType interface{}) string {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		for j := 0; j < row.NumField(); j++ {
			...
			if strings.Contains(sqlText, "STORED") || strings.HasPrefix(sqlText, "INDEX") || strings.HasPrefix(sqlText, "PRIMARY") {
				continue
			}
			body.WriteString("\n")
