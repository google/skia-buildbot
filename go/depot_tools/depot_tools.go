...
func FindVersion(recipesCfgFile string) (string, error) {
	...
	if !ok {
		return "", skerr.Fmt("No dependency found for depot_tools.")
	}
...
func Sync(ctx context.Context, workdir, recipesCfgFile string) (string, error) {
	...
	sklog.Errorf("Want depot_tools at %s", version)
	...
	sklog.Errorf("Have depot_tools at %s", hash)
	if hash == version {
		return co.Dir(), nil
	}
	...
	if err := co.Fetch(ctx); err != nil {
		return "", skerr.Wrapf(err, "Failed to fetch repo in %s: %s", co.Dir())
	}
	if err := co.Cleanup(ctx); err != nil {
		return "", skerr.Wrapf(err, "Failed to cleanup repo in %s: %s", co.Dir())
	}
	if _, err := co.Git(ctx, "reset", "--hard", version); err != nil {
		return "", skerr.Wrapf(err, "Failed to reset repo in %s: %s", co.Dir())
	}
	hash, err = co.RevParse(ctx, "HEAD")
	if err != nil {
		return "", skerr.Wrap(err)
	...
	sklog.Errorf("Successfully synced depot_tools to %s", version)
	return co.Dir(), nil
}
...
func GetDepotTools(ctx context.Context, workdir, recipesCfgFile string) (string, error) {
	...
	sklog.Errorf("Finding depot_tools...")

	// Check the environment. Bots may not have a full Git checkout, so
	// just return the dir.
	depotTools := os.Getenv(DEPOT_TOOLS_TEST_ENV_VAR)
	if depotTools != "" {
		sklog.Errorf("Found %s environment variable.", DEPOT_TOOLS_TEST_ENV_VAR)
		if _, err := os.Stat(depotTools); err == nil {
			sklog.Errorf("Found depot_tools in dir specified in env.")
			return depotTools, nil
		}
		sklog.Errorf("depot_tools is not present in dir specified in env.")
