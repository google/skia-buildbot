...
func GetDepotTools(ctx context.Context, workdir, recipesCfgFile string) (string, error) {
	...
	if depotTools != "" {
		...
		return "", skerr.Wrapf(err, "%s=%s but dir does not exist!", DEPOT_TOOLS_TEST_ENV_VAR, depotTools)
	}
	...
	if err == nil && gclient != "" {
		...
		return skerr.Wrap(Sync(ctx, path.Dir(path.Dir(gclient)), recipesCfgFile))
	}

	// Sync to the given workdir.
	sklog.Infof("Syncing depot_tools.")
	return skerr.Wrap(Sync(ctx, workdir, recipesCfgFile))
}
