...
func TestParseDeps(t *testing.T) {
	...
	// Dict iteration is not consistent, but ParseDeps returns entries in
	// the order they appear.
	assertdeep.Equal(t, []*DepsEntry{
		{
			Id:      "simple-repo.git",
	...
	// We should get exactly the same results from ParseDeps, minus the
	// positions.
	deps2, err := ParseDeps(fakeDepsContent)
	require.NoError(t, err)
	assertdeep.Equal(t, deps, deps2)
...
func TestParseDepsRealWorld(t *testing.T) {
	...
	checkDeps := func(repo string, rev string, expect []*depsEntryPos) {
		...
		for idx, dep := range actual {
			actualMap[dep.Id] = &depsEntryPos{
				DepsEntry: dep,
				Pos:       poss[idx],
			}
		}
		for _, e := range expect {
			assertdeep.Equal(t, e, actualMap[e.Id])
		}
	}

	// Chromium DEPS. We expect this to be the most complex example of a
	// DEPS file. Check a few example dependencies.
	checkDeps("https://chromium.googlesource.com/chromium/src.git", "9f0b31d5560995206ce92535935c2989913bd5bd", []*depsEntryPos{
		{
