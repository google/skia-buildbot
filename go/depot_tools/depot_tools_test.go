...
func TestDEPSExtractor(t *testing.T) {
	...
	ext_1 := NewRegExDEPSExtractor(DefaultSkiaVarRegEx)
	assert.Equal(t, VAR_COMMIT, ext_1.ExtractCommit(TEST_VAR_DEPS, nil))

	ext_2 := NewRegExDEPSExtractor(DefaultSkiaURLRegEx)
	assert.Equal(t, URL_COMMIT, ext_2.ExtractCommit(TEST_URL_DEPS, nil))
}
