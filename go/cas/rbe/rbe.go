...
// SplitDigest splits the given digest string, which is in the form
// `<hash>/<sizeBytes>`, into a hash and size.
func SplitDigest(digest string) (string, int64, error) {
	split := strings.Split(digest, "/")
	if len(split) != 2 {
		return "", 0, skerr.Fmt("invalid digest format; expected \"<hash>/<size>\" but got %q", digest)
	}
	size, err := strconv.ParseInt(split[1], 10, 64)
	if err != nil {
		return "", 0, skerr.Wrapf(err, "invalid digest format; expected \"<hash>/<size>\" but got %q", digest)
	}
	return split[0], size, nil
}

// MakeDigest creates a digest string from the given hash and size.
func MakeDigest(hash string, size int64) string {
...
func (c *Client) Upload(ctx context.Context, root string, paths []string) (string, error) {
	...
	rootDigest, chunkers, _, err := tree.ComputeMerkleTree(root, &is, chunker.DefaultChunkSize, filemetadata.NewNoopCache())
	if err != nil {
...
func checkNodePropertiesIdentical(a, b *remoteexecution.NodeProperties) error {
	if (a != nil) != (b != nil) {
		return skerr.Fmt("One file has NodeProperties while the other does not")
	}
	if a != nil {
		...
		if (a.UnixMode != nil) != (b.UnixMode != nil) {
			return skerr.Fmt("One file has UnixMode while the other does not")
		}
		if a.UnixMode != nil {
