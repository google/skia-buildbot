...
func TestMigrateData(t *testing.T) {
	...
	defer cleanupSrc()
	srcCount, err := srcClient.Count(ctx, NewQuery(testKind))
	...
	destClient := DS
	defer func() {
		_, err := DeleteAll(destClient, testKind, true)
		require.NoError(t, err)
	}()
	destCountEmpty, err := destClient.Count(ctx, NewQuery(testKind))
	require.NoError(t, err)
	require.Equal(t, 0, destCountEmpty)

	// Migrate from source to destination reusing the old keys.
	require.NoError(t, MigrateData(ctx, srcClient, destClient, testKind, false /* createNewKey */))
	wait(t, destClient, testKind, nEntries)
	destCountPopulated, err := destClient.Count(ctx, NewQuery(testKind))
	require.NoError(t, err)
	...
	require.NoError(t, MigrateData(ctx, srcClient, destClient, testKind, true /* createNewKey */))
	wait(t, destClient, testKind, nEntries)
	destCountPopulated, err = destClient.Count(ctx, NewQuery(testKind))
	require.NoError(t, err)
	require.Equal(t, nEntries, destCountPopulated)
