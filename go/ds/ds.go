...
func DeleteAll(client *datastore.Client, kind Kind, wait bool) error {
	for {
		...
		for _, slice := range keySlices {
			if err = client.DeleteMulti(ctx, slice); err != nil {
				break
			}
		}
		count := 0
		if wait {
			if count, err = Count(client, kind); err != nil {
				return err
			}
		}
		if count == 0 {
...
func (k *keySliceIterator) next() ([]*datastore.Key, bool, error) {
	...
	fmt.Printf("next: 6  %s %d \n", newCursorStr, len(retKeys))
