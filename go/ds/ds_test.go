...
func TestMigrateData(t *testing.T) {
	...
	// Spot check to make sure source keys exist in destination.
	assert.NoError(t, destClient.Get(ctx, entitiesSrc[0].Key, &testEntity{}))
	assert.NoError(t, destClient.Get(ctx, entitiesSrc[10].Key, &testEntity{}))
	assert.NoError(t, destClient.Get(ctx, entitiesSrc[100].Key, &testEntity{}))
	assert.NoError(t, destClient.Get(ctx, entitiesSrc[nEntries-1].Key, &testEntity{}))
}
