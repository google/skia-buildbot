...
func NewCloudLogger(ctx context.Context, projectId, logId string, ts oauth2.TokenSource, labels map[string]string) (*cloudLogger, error) {
	...
	Infof("Connected Cloud Logging; logs can be found here: https://pantheon.corp.google.com/logs/viewer?project=%s&resource=project%%2Fproject_id%%2F%s&logName=projects%%2F%s%%2Flogs%%2F%s", projectId, projectId, projectId, logId)
	return &cloudLogger{
		logger: logger,
	}, nil
}
