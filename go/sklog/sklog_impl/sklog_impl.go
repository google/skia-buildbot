...
func (s Severity) StackdriverString() rune {
...
type Logger interface {
	...
	// This method handles both Print-like and Printf-like formatting; `fmt` will
	...
	// severity is Fatal.
	Log(depth int, severity Severity, fmt string, args ...interface{})

	// Log, Flush (if necessary), then end the program. This method should not
	// return.
	LogAndDie(depth int, severity Severity, fmt string, args ...interface{})
...
func Log(depth int, severity Severity, fmt string, args ...interface{}) {
	sawLogWithSeverity(severity)
	logger.Log(depth+1, severity, fmt, args...)
}

// Package-level LogAndDie function; for use by sklog package.
func LogAndDie(depth int, severity Severity, fmt string, args ...interface{}) {
	// In LogAndDie, there is no callback to sawLogWithSeverity, as the program will soon exit
	// and the counter will be reset to 0.
	logger.LogAndDie(depth+1, severity, fmt, args...)
...
// documented.
func LogMessageToString(fmt string, args ...interface{}) string {
	if len(fmt) == 0 {
