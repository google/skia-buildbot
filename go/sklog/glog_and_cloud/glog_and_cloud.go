...
// go.skia.org/infra/go/sklog/stackdriver_json if you want structured cloud logs.
func (skl sklogger) StructuredLog(obj interface{}) {
	b, err := json.Marshal(obj)
	if err != nil {
		skl.Log(0, sklog_impl.Error, "StructuredLog message failed to marshal; %s %v", err, obj)
	} else {
		skl.Log(0, sklog_impl.Info, "%s", b)
	}
}
