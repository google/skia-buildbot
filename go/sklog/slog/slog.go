...
// NewStdErr creates a new Adapter that either logs to stderr, or does no logging, depending upon the
// value of mode. It uses github.com/jcgregorio/logger to implement the _slog.Logger interface.
//
...
//
func NewStdErr(mode SLogLogMode) *Adapter {
	if mode == SLogStderr {
...
func (a *Adapter) StructuredLog(obj interface{}) {
	...
	if err != nil {
		a.Log(0, sklog_impl.Warning, "StructuredLog message failed to marshal; %s %v", err, obj)
	} else {
