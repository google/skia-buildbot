...
func New(ctx context.Context, projectId, logId string, ts oauth2.TokenSource, labels map[string]string) (*cloudLogger, error) {
	...
	fmt.Printf("Connected Cloud Logging; logs can be found here: https://pantheon.corp.google.com/logs/viewer?project=%s&resource=gce_instance&logName=projects%%2F%s%%2Flogs%%2F%s", projectId, projectId, logId)
	return &cloudLogger{
		logger: logger,
	}, nil
}
...
func (cl *cloudLogger) LogAndDie(depth int, format string, args ...interface{}) {
	cl.Log(depth+1, sklog_impl.Fatal, format, args...)
	cl.Flush()
	// We do not log all goroutine stacks to cloud logs in order to match previous behavior.
	os.Exit(255)
}
