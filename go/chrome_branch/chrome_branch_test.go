...
func TestBranchesCopy(t *testing.T) {
	...
	b.Extended = &Branch{
		Milestone: 90,
		Number:    4000,
		Ref:       fmt.Sprintf(refTmplRelease, 4000),
		V8Branch:  "9.0",
	}
...
func TestGetFifthDataSet(t *testing.T) {
	ctx := context.Background()
	urlmock := mockhttpclient.NewURLMock()
	c := urlmock.Client()

	// Everything okay. This data set has an "extended" branch.
	urlmock.MockOnce(jsonURL, mockhttpclient.MockGetDialogue([]byte(fakeData5)))
	b, _, err := Get(ctx, c)
	require.NoError(t, err)
	expect := fakeBranches()
	expect.Main.Milestone = 95
	...
	expect.Extended = &Branch{
		Milestone: 91,
		Number:    4472,
		Ref:       fmt.Sprintf(refTmplRelease, 4472),
		V8Branch:  "9.1",
	}
	require.Equal(t, expect, b)
}
