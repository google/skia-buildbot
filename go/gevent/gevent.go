...
// New returns an instance of eventbus.EventBus that is a node in a distributed eventbus.
// Each instance is a node in a distributed event bus that allows to send events on an arbitrary number
// of channels.
// - projectID is the id of the GCP project where the PubSub topic should live.
// - topicName is the topic to use. It is assume that all message on this topic are messages of the
//   event bus.
// - subscriberName is an id that uniquely identifies this node within the event bus network.
...
func (d *distEventBus) setupClientTopicSub(projectID, topicName, subscriberName string) error {
	...
	// Create a client.
	var err error
	d.client, err = pubsub.NewClient(ctx, projectID, option.WithCredentialsFile("./service-account.json"))
	if err != nil {
		return err
	}
