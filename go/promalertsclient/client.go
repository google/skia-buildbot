...
type Alert struct {
	model.Alert
	// TODO: remove this ugly hack after we update to 0.7 and we can poll
	// /api/v1/alerts instead of /api/v1/alerts/groups.  Right now /alerts
	// doesn't have a way to tell if an alert is silenced.
	Silenced bool
}
...
// API call.  See /prometheus/alertmanager/dispatch the (possibly changing)
// details
