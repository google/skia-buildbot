...
func TestSunnyDayMissing(t *testing.T) {
	...
	mc.On("Get", "https://myalerts.skia.org/api/v1/alerts").Return(&mockResponse, nil)

	alerts, err := client.GetAlerts("BotMissing")
	assert.NoError(t, err)
	assert.Len(t, alerts, 4, "There are exactly 4 missing bots")

	assert.Equal(t, "skia-rpi-061", string(alerts[0].Labels["bot"]))
	assert.Equal(t, "skia-e-win-032", string(alerts[1].Labels["bot"]))
	assert.Equal(t, "skia-e-linux-001", string(alerts[2].Labels["bot"]))
	assert.Equal(t, "skia-e-win-055", string(alerts[3].Labels["bot"]))
}

func TestSunnyDayQuarantined(t *testing.T) {
	testutils.SmallTest(t)
	mc := &mockhttpclient{}
	defer mc.AssertExpectations(t)
	client := New(mc, "myalerts.skia.org")
	...
	alerts, err := client.GetAlerts("BotQuarantined")
	assert.NoError(t, err)
	assert.Len(t, alerts, 3, "There are exactly 3 quarantined bots")

	assert.Equal(t, "skia-e-win-032", string(alerts[0].Labels["bot"]))
	assert.Equal(t, "skia-rpi-054", string(alerts[1].Labels["bot"]))
