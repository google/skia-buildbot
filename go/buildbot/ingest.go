...
func FindCommitsForBuild(db DB, b *Build, repos map[string]*repograph.Graph) ([]string, int, []string, error) {
...
func getBuildFromMaster(master, builder string, buildNumber int, repos map[string]*repograph.Graph) (*Build, error) {
	var build Build
	...
	if build.Repository == "" {
		...
		for repoUrl, repo := range repos {
			if repo.Get(build.GotRevision) != nil {
				r = repoUrl
				break
			}
		}
		if r == "" {
			glog.Warningf("Unable to find repo for commit %s", build.GotRevision)
		} else {
...
func retryGetBuildFromMaster(master, builder string, buildNumber int, repos map[string]*repograph.Graph) (*Build, error) {
	defer metrics2.FuncTimer().Stop()
	var b *Build
	var err error
...
func IngestBuild(db DB, b *Build, repos map[string]*repograph.Graph) error {
	defer metrics2.FuncTimer().Stop()
...
func ingestNewBuilds(db DB, m string, repos map[string]*repograph.Graph) error {
	defer metrics2.FuncTimer().Stop()
...
func IngestNewBuildsLoop(db DB, repos map[string]*repograph.Graph) error {
	local, ok := db.(*localDB)
	if !ok {
