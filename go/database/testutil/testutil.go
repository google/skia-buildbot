...
func MySQLVersioningTests(t testing.TestingT, dbName string, migrationSteps []database.MigrationStep) {
...
func GetMySQlLock(t testing.TestingT, conf *database.DatabaseConfig) *LockDB {
...
func (l *LockDB) Close(t testing.TestingT) {
	var result int
...
func ClearMySQLTables(t testing.TestingT, vdb *database.VersionedDB) {
	stmt := `SHOW TABLES`
...
// defer util.Close(db)
// ... Tests here ...
func SetupMySQLTestDatabase(t testing.TestingT, migrationSteps []database.MigrationStep) *MySQLTestDatabase {
...
func (d *MySQLTestDatabase) Close(t testing.TestingT) {
...
func testDBVersioning(t testing.TestingT, vdb *database.VersionedDB) {
	// get the DB version
...
func downgradeDB(t testing.TestingT, vdb *database.VersionedDB) {
	// downgrade to 0
	err := vdb.Migrate(0)
	assert.NoError(t, err)
	dbVersion, err := vdb.DBVersion()
