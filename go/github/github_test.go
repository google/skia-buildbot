...
func TestRerunLatestCheckSuite(t *testing.T) {
	...
	testutils.SmallTest(t)
	githubClient, err := NewGitHub(context.Background(), "flutter", "flutter", httputils.NewTimeoutClient())
	assert.NoError(t, err)
	err = githubClient.RerunLatestCheckSuite("dd67ef3239d9ef0dee086acfacd559f33d288f32")
	assert.NoError(t, err)
}
