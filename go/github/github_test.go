...
func TestListMatchingReferencesE2E(t *testing.T) {
	unittest.SmallTest(t)
	...
	githubClient, err := NewGitHub(context.Background(), "skia-flutter-autoroll", "engine", githubHttpClient)
	require.NoError(t, err)

	refs, err := githubClient.ListMatchingReferences("skia-flutter-autoroll", "engine", "refs/heads/skia-flutter-autoroll-")
	fmt.Println(refs)
	require.NoError(t, err)
...
func TestDeleteReference(t *testing.T) {
	...
	testRepoName := "gotham"
	r := mux.NewRouter()
	md := mockhttpclient.MockDeleteDialogueWithResponseCode("", nil, nil, http.StatusOK)
