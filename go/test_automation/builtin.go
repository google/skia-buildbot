...
func EnsureGitCheckout(s *Step, dest string, rs db.RepoState) (co *git.Checkout, err error) {
	...
	if _, err := os.Stat(dest); err == nil {
		...
		if _, err := os.Stat(path.Join(dest, ".git")); err == nil {
			...
			// one. Ensure that it has the correct remote.
			...
			if len(remotes) == 1 {
				for _, field := range strings.Fields(remotes[0]) {
					if field == rs.Repo {
						okay = true
						break
					}
			...
			// Nothing to do here, since okay is already false.
		} else {
			return nil, err
		}
		if !okay {
	...
	if rs.IsTryJob() {
		...
		if _, err := co.Git(s.Ctx(), "checkout", "FETCH_HEAD"); err != nil {
			return nil, err
		}
