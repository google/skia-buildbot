...
// command as a Step. It handles calls to Start(), Done(), and Fail(), as
// appropriate.
func (s *Step) RunCwd(cwd string, command ...string) (string, error) {
	var output string
	return output, s.Do(func(s *Step) error {
		out, err := exec.RunCwd(s.Ctx(), cwd, command...)
		output = out
		return err
	})
}
