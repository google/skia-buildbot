...
func TestSetIfUnsetCache(t *testing.T) {
	testutils.SmallTest(t)

	ctx := context.Background()

	// Basic tests for the cache.
	test := func(c *SetIfUnsetCache) {
		k := "key"
		got, err := c.Get(ctx, k)
		...
		val2Str := "world"
		val2 := &val2Str
		_, err = c.SetIfUnset(ctx, k, func(ctx context.Context) (Value, error) {
			return val2, nil
		})
		assert.NoError(t, err)
		got, err = c.Get(ctx, k)
		...
		_, err = c.SetIfUnset(ctx, "key2", func(ctx context.Context) (Value, error) {
			return val2, nil
		})
		assert.NoError(t, err)
...
func TestSetIfUnsetCacheLocking(t *testing.T) {
	testutils.SmallTest(t)
