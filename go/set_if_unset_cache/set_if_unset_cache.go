...
// backingCache to read and write through. The returned SetIfUnsetCache is
...
func (c *SetIfUnsetCache) Cleanup(ctx context.Context, shouldDelete func(context.Context, string, Value) bool) error {
	var err error
	c.ForEach(ctx, func(ctx context.Context, key string, val Value) {
		del := false
		if val == nil {
			...
			// Ask the caller if we should delete the entry.
			del = shouldDelete(ctx, key, val)
		}
		if del {
			err = c.deleteLocked(ctx, key)
			if err != nil {
				return
			}
		}
...
// not call any methods on the SetIfUnsetCache. Only iterates entries in the
...
// Len returns the number of entries in the cache. This only includes entries in
// the local cache; does not count entries from the backingCache.
