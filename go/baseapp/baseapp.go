...
func cspString(allowedHosts []string, options []Option) string {
	...
	//
	unsafeEval := ""
	if hasWASMOption(options) {
		unsafeEval = "'unsafe-eval'"
	}

	return fmt.Sprintf("base-uri 'none';  img-src 'self' ; object-src 'none' ; style-src 'self'  https://fonts.googleapis.com/ https://www.gstatic.com/ 'unsafe-inline' ; script-src 'strict-dynamic' $NONCE 'unsafe-inline' %s %s https: http: ; report-uri /cspreport ;", unsafeEval, addScriptSrc)
}

func securityMiddleware(allowedHosts []string, options []Option) mux.MiddlewareFunc {
	...
	secureMiddleware := secure.New(secure.Options{
		...
		ContentSecurityPolicy: cspString(allowedHosts, options),
		IsDevelopment:         *Local,
	})
...
func Serve(constructor Constructor, allowedHosts []string, options ...Option) {
	...
	middleware = append(middleware,
		...
		securityMiddleware(allowedHosts, options),
	)
	r.Use(middleware...)
