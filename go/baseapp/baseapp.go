...
type App interface {
	AddHandlers(*mux.Router)             // Content endpoints.
	AddXSSProtectedHandlers(*mux.Router) // JSON API endpoints.
}
...
func applySecurityMiddleware(h http.Handler, salt []byte, domain string) http.Handler {
	...
	return secureMiddleware.Handler(h)
}

// Add opts for security and callbacks.
func Serve(constructor Constructor, name string, domain string, middlewares ...mux.MiddlewareFunc) {
	// Do common init.
	common.InitWithMust(
		name,
	...
	// Setup the salt needed for XSS protection.
	...
	// Apply the XSS protection to a subrouter at /_.
	s := r.PathPrefix("/_").Subrouter()
	s.Use(csrf.Protect(salt, csrf.Secure(!*Local), csrf.Path("/")))
	app.AddXSSProtectedHandlers(s)

	// Layer on all the middleware.
	h := applySecurityMiddleware(r, salt, domain)
