...
func cspString(allowedHosts []string, local bool, options []Option) string {
	...
	//
	return fmt.Sprintf("base-uri 'none';  img-src 'self' ; object-src 'none' ; style-src 'self'  https://fonts.googleapis.com/ https://www.gstatic.com/ 'unsafe-inline' ; script-src %s 'strict-dynamic' $NONCE 'unsafe-inline' https: http: ; report-uri /cspreport ;", addScriptSrc)
}
