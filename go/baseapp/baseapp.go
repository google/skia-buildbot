...
func securityMiddleware(allowedHosts []string) mux.MiddlewareFunc {
	...
	cspString := fmt.Sprintf("base-uri 'none';  img-src 'self' ; object-src 'none' ; style-src 'self'  https://fonts.googleapis.com/ https://www.gstatic.com/ ; script-src $NONCE 'unsafe-inline' %s https: http: ; report-uri /cspreport ;", addScriptSrc)
