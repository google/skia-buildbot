...
func WithUpdateCQConfig(oldCfgBytes []byte, fn func(*Config) error) ([]byte, error) {
	...
	// Compare the old config bytes to the new. The files should be
	// identical up until the new ConfigGroup. This adds back any comments
	// that were lost when we parsed the file.
