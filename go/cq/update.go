...
func WithUpdateCQConfig(ctx context.Context, starlarkConfigFile, generatedDir string, fn func(*build.File) error) error {
	...
	// Update the config.
	newCfgBytes, err := WithUpdateCQConfigBytes(oldCfgBytes, fn)
	if err != nil {
...
func WithUpdateCQConfigBytes(oldCfgBytes []byte, fn func(*build.File) error) ([]byte, error) {
	// Parse the Config.
	f, err := build.ParseDefault(DefaultMainStarFile /* TODO */, oldCfgBytes)
	if err != nil {
