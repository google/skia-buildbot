...
func TestCloneBranch(t *testing.T) {
	...
	t.Run("clone all", func(t *testing.T) {
		actual, err := WithUpdateCQConfigBytes([]byte(fakeConfig), func(f *build.File) error {
...
func TestWithUpdateCQConfig(t *testing.T) {
	...
	env := depot_tools.Env(depotTools)
	for _, envVar := range env {
		split := strings.SplitN(envVar, "=", 2)
		if len(split) == 2 {
			require.NoError(t, os.Setenv(split[0], split[1]))
		}
	}

	mainStarFile := filepath.Join(tmp, "main.star")
