...
func (c *Client) ReportCQStats(cqTryBots []string, issue int64) error {
	...
	if len(trybots) == 0 {
		glog.Infof("No trybot results were found for %s/c/%d/%d", gerrit.GERRIT_SKIA_URL, issue, latestPatchsetId)
		return nil
	}
	...
	for _, t := range trybots {
		...
		fmt.Println(t.Parameters.BuilderName)
		fmt.Println(t.Bucket)
		fmt.Println(t.Id)
		fmt.Println(t.Url)
		createdTime := time.Time(t.Created).UTC()
		if startTimeOfCQBots.After(createdTime) {
			startTimeOfCQBots = createdTime
		}
		...
		duration := completedTime.Sub(createdTime).Seconds()
		metrics2.RawAddInt64PointAtTime("cq_watcher.after_commit.trybot_duration", durationTags, int64(duration), endTimeOfCQBots)
		// Need to upload metrics of all CQ builders for this CL. So that when you query a CL you can tell how long it spent on each builder in the CQ.
	}

	fmt.Println("END OF EVERYTHING")
	fmt.Println(startTimeOfCQBots)
	fmt.Println(endTimeOfCQBots)
	fmt.Println(endTimeOfCQBots.Sub(startTimeOfCQBots).Seconds())

	tryBotsDuration := int64(endTimeOfCQBots.Sub(startTimeOfCQBots).Seconds())
	metrics2.RawAddInt64PointAtTime("cq_watcher.after_commit.total_duration", map[string]string{"issue": fmt.Sprintf("%d", issue), "patchset": fmt.Sprintf("%d", latestPatchsetId)}, int64(tryBotsDuration), endTimeOfCQBots)
