...
const (
	...
	// Constants for ReportCQStatsForInFlightCL.
	THRESHOLD_FOR_CQ_TRYBOTS         = time.Minute // TODO(rmistry): Change to 45 mins??
	MAX_FACTOR_FOR_ACTIVE_CQ_TRYBOTS = 3
)
...
func (c *Client) ReportCQStatsForInFlightCL(issue, latestPatchsetId int64, builds []*buildbucket.Build) {
	...
	for _, b := range builds {
		...
		if duration >= int64(THRESHOLD_FOR_CQ_TRYBOTS.Seconds()) {
			// TODO(rmistry): Problem here is that you could keep reporting the same long running trybot.
			glog.Infof("\t%s was created at %s and still running after %d seconds. The threshole is %d seconds", b.Parameters.BuilderName, createdTime, duration, THRESHOLD_FOR_CQ_TRYBOTS.Seconds())
			durationTags := map[string]string{
				"issue":    fmt.Sprintf("%d", issue),
				"patchset": fmt.Sprintf("%d", latestPatchsetId),
				"trybot":   b.Parameters.BuilderName,
			}
			metrics2.RawAddInt64PointAtTime("cq_watcher.in_flight.trybot_duration_above_threshold", durationTags, duration, currentTime)
		}
	}
	if totalTriggeredCQBots >= MAX_FACTOR_FOR_ACTIVE_CQ_TRYBOTS*len(c.cqTryBots) {
		numTags := map[string]string{
			"issue":     fmt.Sprintf("%d", issue),
			"patchset":  fmt.Sprintf("%d", latestPatchsetId),
			"threshold": fmt.Sprintf("%d", MAX_FACTOR_FOR_ACTIVE_CQ_TRYBOTS*len(c.cqTryBots)),
		}
		// TODO(rmistry): Problem here is that you could keep reporting the same trybot_num_above_threshold.
		glog.Warningf("\t%d total CQ bots were triggered. That is more than the %dx%d threshold", totalTriggeredCQBots, MAX_FACTOR_FOR_ACTIVE_CQ_TRYBOTS, len(c.cqTryBots))
		metrics2.RawAddInt64PointAtTime("cq_watcher.in_flight.trybot_num_above_threshold", numTags, int64(totalTriggeredCQBots), currentTime)
	}
}
