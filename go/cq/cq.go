...
type GetCQTryBots func() (util.StringSet, error)
...
func getCQTryBots(repo string) (util.StringSet, error) {
	...
	return util.NewStringSet(tryJobs), nil
}
...
func (c *Client) RefreshCQTryBots() error {
	...
	deletedTryBots := c.cqTryBots.Complement(tryBots)
...
func (c *Client) isCQTryBot(builderName string) bool {
	cqTryBotsMutex.RLock()
	isCQTrybot := util.In(builderName, c.cqTryBots.Keys())
	cqTryBotsMutex.RUnlock()
	return isCQTrybot
}
