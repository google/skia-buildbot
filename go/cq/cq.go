...
func (c *Client) RefreshCQTryBots() error {
	...
	deletedTryBots := []string{}
	for _, bot := range c.cqTryBots {
		if !util.In(bot, tryBots) {
			deletedTryBots = append(deletedTryBots, bot)
		}
	}
...
func (c *Client) ReportCQStatsForLandedCL(cqBuilds []*buildbucket.Build, gerritURL string) {
	...
	for _, b := range cqBuilds {
		...
		landedTrybotDurationMetric := c.getInflightTrybotDurationMetric(b.Parameters.BuilderName)
...
func (c *Client) ReportCQStatsForInFlightCL(cqBuilds []*buildbucket.Build, gerritURL string) {
	...
	metricsToDelete := []metrics2.Int64Metric{}
	for _, b := range cqBuilds {
		...
		metricsToDelete = append(metricsToDelete, inflightTrybotDurationMetric)
	}

	// Delete all metrics now.
	sklog.Info("Deleting the metric now")
	for _, m := range metricsToDelete {
		// Delete the metric, we do not need trybot duration to be continuously reported.
		if err := m.Delete(); err != nil {
			sklog.Errorf("Could not delete inflight metric: %s", err)
		}
	}
...
func (c *Client) purgeMetrics(tryBots []string) error {
	for _, b := range tryBots {
