...
func (c *Client) ReportCQStatsForLandedCL(change, patchsetId int64, cqBuilds []*buildbucket.Build) {
	...
	for _, b := range cqBuilds {
		...
		durationTags := map[string]string{
			...
			"completed_time": completedTime.String(),
		}
		...
		landedTrybotDurationMetric := metrics2.GetInt64Metric(fmt.Sprintf("%s.%s.%s", c.metricName, LANDED_METRIC_NAME, LANDED_TRYBOT_DURATION), durationTags)
	...
	totalDurationTags := map[string]string{
		"issue":            fmt.Sprintf("%d", change),
		"patchset":         fmt.Sprintf("%d", patchsetId),
		"end_time_cq_bots": endTimeOfCQBots.String(),
	}
	landedTotalDurationMetric := metrics2.GetInt64Metric(fmt.Sprintf("%s.%s.%s", c.metricName, LANDED_METRIC_NAME, LANDED_TOTAL_DURATION), totalDurationTags)
...
func (c *Client) ReportCQStatsForInFlightCL(issue, patchsetId int64, cqBuilds []*buildbucket.Build) {
	...
	for _, b := range cqBuilds {
		...
		inflightTrybotDurationMetric := metrics2.GetInt64Metric(fmt.Sprintf("%s.%s.%s", c.metricName, INFLIGHT_METRIC_NAME, INFLIGHT_TRYBOT_DURATION), durationTags)
	...
	trybotNumDurationMetric := metrics2.GetInt64Metric(fmt.Sprintf("%s.%s.%s", c.metricName, INFLIGHT_METRIC_NAME, INFLIGHT_TRYBOT_NUM), numTags)
