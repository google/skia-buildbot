...
func (c *Client) ReportCQStatsForLandedCL(issue, latestPatchsetId int64, builds []*buildbucket.Build) {
	...
	metrics2.RawAddInt64PointAtTime(fmt.Sprintf("%s.%s.%s", c.metricName, LANDED_METRIC_NAME, LANDED_TOTAL_DURATION), map[string]string{"issue": fmt.Sprintf("%d", issue), "patchset": fmt.Sprintf("%d", latestPatchsetId)}, maximumTrybotDuration, endTimeOfCQBots)
}
