...
// State is the name of a single state in the state machine.
type State string
...
// NextFn returns the next State, and the TransitionFn from the current State
// to the next State, based on the current State. It is okay to return nil for
// a noop TransitionFn.
...
func (b *Builder) BuildPersistent(file string) (StateMachine, error) {
	...
	if err == nil {
		b.initialState = State(string(contents))
