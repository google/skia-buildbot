...
func (b *Builder) Build(ctx context.Context, gcsClient gcs.GCSClient, gsPath string) (*StateMachine, error) {
	...
	sm := &StateMachine{
		current:     cachedState,
		gcs:         gcsClient,
		gcsOpts:     gcs.FILE_WRITE_OPTS_TEXT,
		funcs:       b.funcs,
	...
	if err := sm.gcs.SetFileContents(ctx, sm.file, sm.gcsOpts, []byte(sm.Current())); err != nil {
		return nil, err
	}
	return sm, nil
}
...
func (sm *StateMachine) Transition(ctx context.Context, dest string) error {
	...
	if err := sm.gcs.SetFileContents(ctx, sm.busyFile, sm.gcsOpts, []byte(dest)); err != nil {
		return err
	}
	defer func() {
	...
	sm.current = dest
	return sm.gcs.SetFileContents(ctx, sm.file, sm.gcsOpts, []byte(sm.current))
}
