...
func (d *DownloadHelper) Download(name, hash string) (err error) {
	...
	if err != nil {
		return skerr.Fmt("Download helper can't get reader for %s: %s", name, err)
	}
	var f *os.File
	f, err = os.Create(filepath)
	if err != nil {
		return skerr.Fmt("Download helper cannot create filepath %s: %s", filepath, err)
	}
	defer func() {
		err2 := f.Close()
		if err == nil && err2 != nil {
			err = skerr.Fmt("Failed to close the file with: %s", err2)
		}
	}()
	_, err = io.Copy(f, resp)
	if err != nil {
		return skerr.Fmt("Download helper can't download %s: %s", name, err)
	}
...
func (d *DownloadHelper) MaybeDownload(name, hash string) (err error) {
	filepath := path.Join(d.workdir, name)
	var f *os.File
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return skerr.Fmt("Failed to open %s: %s", filepath, err)
		}
	}
	defer func() {
		err2 := f.Close()
	...
	info, err = f.Stat()
	if err != nil {
		return skerr.Fmt("Failed to stat %s: %s", filepath, err)
	}
	if info.Mode() != 0755 {
	...
	if err != nil {
		return skerr.Fmt("Failed to read %s: %s", filepath, err)
	}
