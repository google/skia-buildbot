...
func (g *gcsclient) FileReader(ctx context.Context, path string) (io.ReadCloser, error) {
	...
	// to get the compressed content (NewReader decompresses on the server side), and wrap the reader
	// in a gzip.Reader.
...
// WithGzipWriter writes to a GCS object using the given function, compressing the data with gzip
