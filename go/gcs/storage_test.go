...
// captureFileWriterGCSClient captures FileWriter args for TestWithWriter* and TestWithGzipWriter*.
...
func TestWithWriterSimple(t *testing.T) {
	...
	assert.NoError(t, WithWriter(c, ctx, path, opts, func(w io.Writer) error {
...
func TestWithWriterError(t *testing.T) {
	testutils.SmallTest(t)
	...
	assert.Equal(t, WithWriter(c, ctx, path, opts, func(w io.Writer) error {
		return err
	}), err)
	// The context should be canceled.
	assert.Equal(t, context.Canceled, c.fileWriterCtx.Err())
...
func TestWithGzipWriterSimple(t *testing.T) {
	testutils.SmallTest(t)
	...
	assert.NoError(t, WithGzipWriter(c, ctx, path, func(w io.Writer) error {
		_, err := w.Write([]byte(contents))
		return err
	}))
	// The context should be canceled.
