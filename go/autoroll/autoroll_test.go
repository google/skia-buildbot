...
func TestTryResultsFromGithubChecks(t *testing.T) {
	...
	context3 := "Check3"
	context4 := "Check4"
	checks := []github_api.RepoStatus{
		// Pending check.
		{ID: &id1, Context: &context1, State: &pendingState},
		// Failed check.
		{ID: &id2, Context: &context2, State: &failureState},
		// Error check.
		{ID: &id3, Context: &context3, State: &errorState},
		// Success check.
		{ID: &id4, Context: &context4, State: &successState},
	}
