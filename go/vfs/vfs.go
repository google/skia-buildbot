...
// CloseFS adds a Close method to FS, allowing implementations to clean up any
// associated resources when the caller is finished.
...
// ReadDirFile represents a virtual directory.
...
func ReadDir(ctx context.Context, fs FS, path string) (rv []os.FileInfo, rvErr error) {
	...
	dir, ok := f.(ReadDirFile)
	if !ok {
		return nil, skerr.Fmt("%s is not a directory", path)
	}
	return dir.ReadDir(ctx, -1)
}
