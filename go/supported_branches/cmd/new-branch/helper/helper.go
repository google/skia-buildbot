...
func AddSupportedBranch(repoUrl, branch, owner, deleteBranch string, excludeTrybots []string, submit bool) error {
	...
	mainStarFile := filepath.Join(tmp, cq.DefaultMainStarFile)
	if err := repo.DownloadFileAtRef(ctx, cq.DefaultMainStarFile, baseCommit, mainStarFile); err != nil {
		return skerr.Wrapf(err, "failed to download %q", cq.DefaultMainStarFile)
	}
