...
func TestEvalNoModifyTile(t *testing.T) {
	...
	if got, want := testRows[",config=8888,os=Ubuntu12,"][0], 1.234; got != want {
...
func TestNorm(t *testing.T) {
	...
	if got, want := rows[",name=t1,"][0], 1.0; !near(got, want) {
...
func TestRatio(t *testing.T) {
	...
	for i, want := range []float64{0.5, 0.5, 0.04, 0.1, 0, e} {
...
func TestFill(t *testing.T) {
	...
	formula := `fill(filter("config=8888"))`
	rows, err := ctx.Eval(formula)
	if err != nil {
...
func TestLog(t *testing.T) {
	ctx := newTestContext(Rows{
		",name=t1,": []float64{1, 10, 100, -1, 0, 1e100},
	})
	formula := `log(filter(""))`
	rows, err := ctx.Eval(formula)
	if err != nil {
