...
var (
	...
	testRows = Rows{
		",config=8888,os=Ubuntu12,": []float64{e, 1.234, e},
		",config=gpu,os=Ubuntu12,":  []float64{e, 1.236, e},
	}
)
...
func TestEvalNoModifyTile(t *testing.T) {
	...
	if got, want := testRows[",config=8888,os=Ubuntu12,"][0], MISSING_DATA_SENTINEL; got != want {
		t.Errorf("Tile incorrectly modified: Got %v Want %v", got, want)
	}
	if got, want := rows["fill(,config=8888,os=Ubuntu12,)"][0], 1.234; got != want {
...
func TestNorm(t *testing.T) {
	...
	if got, want := rows["norm(,name=t1,)"][0], 1.0; !near(got, want) {
...
func TestAve(t *testing.T) {
	ctx := newTestContext(Rows{
		",name=t1,": []float64{1.0, -1.0, e, e},
		",name=t2,": []float64{e, 2.0, -2.0, e},
	})
	formula := `ave(filter(""))`
	...
	for i, want := range []float64{1.0, 0.5, -2.0, e} {
...
func TestRatio(t *testing.T) {
	ctx := newTestContext(Rows{
		",name=t1,": []float64{10, 4, 100, 50, 9999, 0},
		",name=t2,": []float64{5, 2, 4, 5, 0, 1000},
	})
	...
	for i, want := range []float64{2.0, 2.0, 25, 10, e, 0} {
...
func TestSum(t *testing.T) {
	...
	for i, want := range []float64{1.0, 1.0, -2.0, e} {
		if got := rows[formula][i]; !near(got, want) {
			t.Errorf("Distance mismatch: Got %v Want %v", got, want)
		}
	}
...
func TestGeo(t *testing.T) {
	ctx := newTestContext(Rows{
		",name=t1,": []float64{1.0, -1.0, 2.0, e},
		",name=t2,": []float64{e, 2.0, 8.0, -2.0},
	})
	formula := `geo(filter(""))`
	rows, err := ctx.Eval(formula)
	if err != nil {
	...
	for i, want := range []float64{1.0, 2.0, 4.0, e} {
		if got := rows[formula][i]; !near(got, want) {
			t.Errorf("Distance mismatch: Got %v Want %v", got, want)
		}
	}
...
func TestLog(t *testing.T) {
	ctx := newTestContext(Rows{
		",name=t1,": []float64{1, 10, 100, -1, 0, e},
	})
	formula := `log(filter(""))`
	rows, err := ctx.Eval(formula)
	if err != nil {
