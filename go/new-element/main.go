...
`)
	...
	if _, err := os.Stat(targetDirectory); err != nil && os.IsNotExist(err) {
		log.Fatalf("The %q directory doesn't exist.", targetDirectory)
	}
	name := os.Args[1]

	// Create directory for the element.
	if err := os.Mkdir(filepath.Join(targetDirectory, name), 0755); err != nil {
	...
	files := map[string]string{
		"file-demo.html": filepath.Join(targetDirectory, name, name+"-demo.html"),
		"file-demo.js":   filepath.Join(targetDirectory, name, name+"-demo.js"),
		"file.js":        filepath.Join(targetDirectory, name, name+".js"),
		"file.scss":      filepath.Join(targetDirectory, name, name+".scss"),
		"index.js":       filepath.Join(targetDirectory, name, "index.js"),
	}

	// Confirm files don't already exist.
	for _, target := range files {
		if exists(target) {
			log.Fatalf("The file %q already exists, will not overwrite.", target)
		}
	}
