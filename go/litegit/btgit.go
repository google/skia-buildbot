...
var ColumnFamilies = []string{cfCommit}

type GitStore interface {
	Put(commits []*vcsinfo.LongCommit, commitIndices []int) error
	Get(hash string) (*vcsinfo.LongCommit, error)
	GetIndex(hash string) (int, error)
	RangeN(startIndex, endIndex int) ([]*vcsinfo.IndexCommit, error)
	IndexCommitRange(start, end time.Time) ([]*vcsinfo.IndexCommit, error)
	Clear() error
}
...
func NewBTGitStore(config *BTGitStoreConfig) (GitStore, error) {
	// Create the client.
	client, err := bigtable.NewClient(context.TODO(), config.ProjectID, config.InstanceID)
	if err != nil {
		return nil, skerr.Fmt("Error creating client: %s", err)
	}
...
func (b *btGitStore) IndexCommitRange(start, end time.Time) ([]*vcsinfo.IndexCommit, error) {
...
func (b *btGitStore) Clear() error {
	// Do a table scan to get all the row names

	// Purge everything
	return nil
}

func (b *btGitStore) GetIndex(hash string) (int, error) {
	return 0, nil
}
...
func (b *btGitStore) Get(hash string) (*vcsinfo.LongCommit, error) {
	return nil, nil
}
