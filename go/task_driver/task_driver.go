...
func Init(projectId, taskId, taskName, output *string, local *bool) (*Step, error) {
	...
	if *local {
		...
		if la == nil {
			sklog.Error("Local auth is nil!")
		} else {
			sklog.Infof("Got local auth: %s\n%v", la.DefaultAccountID, la.Accounts)
		}
		authenticator := luci_auth.NewAuthenticator(ctx, luci_auth.SilentLogin, luci_auth.Options{
