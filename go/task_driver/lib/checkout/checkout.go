...
func ValidateCheckout(s *task_driver.Step, dest string, rs db.RepoState) (bool, error) {
	if _, err := os_steps.Stat(s, filepath.Join(dest, ".git")); err == nil {
		gd := git.GitDir(dest)

		// Run "git status" and log the result, in case it's needed for
		// forensics.
		output, err := gd.Git(s.Ctx(), "status")
		if err != nil {
		...
		output, err = gd.Git(s.Ctx(), "remote", "-v")
		if err != nil {
			return false, err
		}
		...
		if len(remotes) == 0 {
			...
			if _, err := gd.Git(s.Ctx(), "rev-parse", "HEAD"); err != nil {
...
func EnsureGitCheckout(s *task_driver.Step, dest string, rs db.RepoState) (co *git.Checkout, err error) {
	...
	if _, err := os_steps.Stat(s, dest); err == nil {
		// If the dest dir is present but not in a reasonable state,
		// delete it.
		okay, err := ValidateCheckout(s, dest, rs)
		if err != nil {
			return nil, err
		}
		if !okay {
			sklog.Infof("Removing incompatible checkout in %s", dest)
			if err := os_steps.RemoveAll(s, dest); err != nil {
				return nil, err
			}
		}
	}
	...
	if _, err := os_steps.Stat(s, dest); err != nil {
		if os.IsNotExist(err) {
			...
			if err := os_steps.MkdirAll(s, filepath.Dir(dest)); err != nil {
				return nil, err
			}
			if _, err := exec.RunCwd(s.Ctx(), filepath.Dir(dest), "git", "clone", rs.Repo, dest); err != nil {
				return nil, err
			}
		} else {
			return nil, err
	...
	if rs.IsTryJob() {
		...
		if err := co.FetchRefFromRepo(s.Ctx(), rs.Repo, ref); err != nil {
			return nil, err
		}
...
func EnsureGitCheckoutWithDEPS(s *task_driver.Step, workdir string, rs db.RepoState) (co *git.Checkout, err error) {
