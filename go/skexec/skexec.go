...
func (e Exec) GetOutput(command *Command) (string, error) {
	...
	if err != nil {
		if command.Verbose != Silent {
			outputForLog := bytes.Buffer{}
			if result != "" && !(command.LogStdout && command.LogStderr) {
				outputForLog.WriteString("; ")
				if !resultHasStdout {
					outputForLog.WriteString("Stderr")
				} else if !resultHasStderr {
					outputForLog.WriteString("Stdout")
				} else {
					outputForLog.WriteString("Stdout+Stderr")
				}
				if overrun > 0 {
					outputForLog.WriteString(" (truncated)")
				}
				outputForLog.WriteString(":\n")
				outputForLog.WriteString(result)
			}
			sklog.Errorf("Command exited with %s: %s%s", err, DebugString(command), outputForLog.String())
		}
		return result, err
	}
	if overrun > 0 {
