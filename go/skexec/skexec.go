/*
	...
	It is recommended to create a package-global Exec variable to allow	injecting a mock Exec for
	testing without interfering with other packages.
	Code:
...
type Exec interface {
	...
	// occurred.
	GetOutput(command *Command) (string, error)
	...
	RunSimple(commandLine string) (string, error)
	...
	// other error.
	RunCwd(cwd string, args ...string) (string, error)
	...
	RunAsync(command *Command) <-chan error
}
...
// ExecImpl implements Exec using Run.
...
func (e ExecImpl) Run(command *Command) error {
...
// command.Stdout and/or command.Stderr are/is nil. Also returns a non-nil error if any failure
// occurred.
func (e ExecImpl) GetOutput(command *Command) (string, error) {
...
// an error if the command exited with a non-zero status or there is any other error. Note: don't
// expect this function to do anything smart with quotes or escaped spaces.
func (e ExecImpl) RunSimple(commandLine string) (string, error) {
...
// stderr. May also return an error if the command exited with a non-zero status or there is any
// other error.
func (e ExecImpl) RunCwd(cwd string, args ...string) (string, error) {
	command := &Command{
		Name: args[0],
		Args: args[1:],
		Dir:  cwd,
...
// RunAsync starts the command and then returns. Clients can listen for the command to end on the
// returned channel. Use command.Context with context.WithCancel to kill the process.
func (e ExecImpl) RunAsync(command *Command) <-chan error {
...
// Reset is a convenience for *exec = NewExec(), but can be used in a defer.
func Reset(exec *Exec) {
	*exec = NewExec()
}
