...
func Init(appName string, influxClient *influxdb.Client) error {
	...
	clientI, err := NewClient(influxClient, tags, DEFAULT_REPORT_FREQUENCY)
	if err != nil {
		return err
	}
	c := clientI.(*Client)
...
// data points.
func NewClient(influxClient *influxdb.Client, defaultTags map[string]string, reportFrequency time.Duration) (ClientI, error) {
