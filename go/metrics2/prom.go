...
var (
	...
	// env_tags are tags create by environment variables beginning with METRICS_ that will be added to all metrics.
	env_tags = map[string]string{}
)

// parseEnvTags parses our environment variables that are to be used
// in all metrics.
func parseEnvTags() map[string]string {
	...
	for _, keyValue := range os.Environ() {
		...
		if strings.HasPrefix(parts[0], PREFIX) {
			tagName := clean(strings.ToLower(parts[0][len(PREFIX):]))
			if tagName == "" {
				continue
			}
			ret[tagName] = value
