		...
		for period, metrics := range byPeriod {
			...
			for _, mx := range metrics {
				if err := m.updateMetric(ev, mx); err != nil {
					errs = append(errs, fmt.Errorf("Failed to update metric: %s", err))
					continue
				}
			}
		}
	...
	for stream, byPeriod := range m.dynamicMetrics {
		for period, metrics := range byPeriod {
			...
			for _, mx := range metrics {
				...
				if err != nil {
					errs = append(errs, fmt.Errorf("Failed to update dynamic metric: %s", err))
					continue
				}
				for k, v := range got {
	...
	for k, v := range m.currentDynamicMetrics {
		if _, ok := gotDynamicMetrics[k]; !ok {
			if err := v.Delete(); err != nil {
				errs = append(errs, fmt.Errorf("Failed to delete old metric: %s", err))
			}
		}
	}
