...
// InitWithMetrics2 runs normal Init functions, tracks runtime metrics and uses Cloud Logging
// if skipMetadataAndLogging is false.
func InitWithCloudLogging(appName, logName string, influxHost, influxUser, influxPassword, influxDatabase *string, skipMetadataAndLogging *bool) {
	Init()
	StartMetrics2(appName, influxHost, influxUser, influxPassword, influxDatabase, *skipMetadataAndLogging)
	// disable cloud logging when run locally.
	if !*skipMetadataAndLogging {
		StartCloudLogging(appName, logName)
	}
}
...
// host instance has the sklog.CLOUD_LOGGING_WRITE_SCOPE set. It exits fatally if anything
// goes wrong.
func StartCloudLogging(appName, logName string) {
	...
	if err = sklog.InitCloudLogging(c, hostname, logName, metricsCallback); err != nil {
		sklog.Fatal(err)
	}
}
