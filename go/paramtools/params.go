...
// that make up the ParamSet to a very small number of bytes, usually
// just two bytes per key/value pair in a Params.
//
...
// for that key (1).
//
// That sequence of offsets []int64{0, 1} can be easily compressed
// via binary.PutVarint into a slice of bytes just two bytes long:
//
//  []byte{0x00, 0x02}
//
...
// Check returns all the keys and their values that don't exist in the
// OrderedParamSet.
...
//
// The byte slice is a gzip compressed JSON serialization of the
// OrderedParamSet.
...
type paramsEncoder struct {
	// keys maps the key name to its offset in OrdereredParamSet.KeyOrder.
	keys map[string]int

	// values maps the key key offset to a map that maps the value to its offset.
	values []map[string]int
}

// encode takes a Params and finds the indexes of both the key and its value in
// the OrderedParamSet and then uses binary Varint encoding to turn that slice
// of key/value ints into a much smaller slice of bytes.
...
//
// The representation is integer comma separated pairs separated by :'s. For example:
//
//   :0,1:1,3:3,0:
func (p *paramsEncoder) encodeAsString(params Params) (string, error) {
	...
	for key, value := range params {
		...
		ret = append(ret, fmt.Sprintf("%d,%d:", keyIndex, valueIndex))
	}
	return strings.Join(ret, ""), nil
}

// EncodeParams encodes the Params as a slice of bytes.
...
// DecodeParams decodes the Params from a slice of bytes.
func (o *OrderedParamSet) DecodeParams(b []byte) (Params, error) {
	...
	if len(ints)%2 == 1 {
		return nil, fmt.Errorf("Wrong number of indexes in decoded slice.")
	}
	ret := Params{}
...
func (o *OrderedParamSet) DecodeParamsFromString(s string) (Params, error) {
	...
	for _, pair := range strings.Split(s, ":") {
		...
		keyIndex, err := strconv.ParseInt(parts[0], 10, 64)
		if err != nil {
			return nil, fmt.Errorf("Failed to parse key: %s", parts)
		}
		valueIndex, err := strconv.ParseInt(parts[1], 10, 64)
		if err != nil {
			return nil, fmt.Errorf("Failed to parse value: %s", parts)
		}
		if keyIndex > int64(len(o.KeyOrder)) || keyIndex < 0 {
			return nil, fmt.Errorf("Got invalid key index: %d", keyIndex)
		}
