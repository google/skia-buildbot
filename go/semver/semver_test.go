...
func TestParseVersion(t *testing.T) {
	test := func(t *testing.T, re *regexp.Regexp, version string, expect interface{}) {
		t.Run(version, func(t *testing.T) {
			ints, err := parseVersion(re, version)
	...
	t.Run("major only", func(t *testing.T) {
		...
		test(t, re, "1.0.0", []int{1})
		test(t, re, "1.2.0", ErrNoMatch)
		test(t, re, "1.2.3", ErrNoMatch)
	})
	t.Run("major and minor", func(t *testing.T) {
		re := regexp.MustCompile(`^(\d+)\.(\d+).0$`)
		test(t, re, "1.0.0", []int{1, 0})
		test(t, re, "1.2.0", []int{1, 2})
		test(t, re, "1.2.3", ErrNoMatch)
	})
	t.Run("all fields", func(t *testing.T) {
		re := regexp.MustCompile(`^(\d+)\.(\d+).(\d+)$`)
		test(t, re, "1.0.0", []int{1, 0, 0})
		test(t, re, "1.2.0", []int{1, 2, 0})
		test(t, re, "1.2.3", []int{1, 2, 3})
	})
