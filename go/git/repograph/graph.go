...
// updateFrom updates the Graph using the given Updater and returns the lists of
// new and deleted commits, or any error which occurred.
func (r *Graph) updateFrom(ctx context.Context, ud Updater) ([]*vcsinfo.LongCommit, []*vcsinfo.LongCommit, error) {
	// Retrieve the new commits.
	sklog.Info("Updating repograph.Graph...")
	if err := ud.Update(ctx); err != nil {
		return nil, nil, err
	}

	// Obtain the list of branches.
	sklog.Info("  Getting branches...")
	newBranchesList, err := ud.Branches(ctx)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, branch := range newBranchesList {
		...
		for len(toProcess) > 0 {
			...
			details, err := ud.Get(ctx, []string{c})
			if err != nil {
				return nil, nil, err
			}
	...
	r.branches = make([]*git.Branch, 0, len(newBranchesMap))
	...
	sort.Sort(git.BranchList(r.branches))
	return newCommits, removedCommits, nil
}
...
func (r *Graph) update(ctx context.Context, cb func(*Graph) error) ([]*vcsinfo.LongCommit, []*vcsinfo.LongCommit, error) {
	...
	added, removed, err := newGraph.updateFrom(ctx, r.updater)
	if err != nil {
		return nil, nil, err
	}
	if cb != nil {
...
// not committed.
func (m Map) update(ctx context.Context, cb func(*Graph) error) (map[string][]*vcsinfo.LongCommit, map[string][]*vcsinfo.LongCommit, error) {
	...
	for repoUrl, r := range m {
		...
		defer r.updateMtx.Unlock()
		newGraph := r.shallowCopy()
		a, r, err := newGraph.updateFrom(ctx, r.updater)
		if err != nil {
			return nil, nil, err
		}
		added[repoUrl] = a
...
// error for any Graph, no changes are committed.
func (m Map) UpdateWithCallback(ctx context.Context, cb func(*Graph) error) error {
