...
type Commit struct {
	*vcsinfo.LongCommit
	...
	// following each commit's first parent backward in history. Some
	// clients need this information to create a clearly-defined line of
	// commits while ignoring branches and merges. It is also used during
	// Recurse as an estimate of the number of commits which might be
	// visited, to prevent excessive resizing of the visited map.
	Index int
}
...
func (c *Commit) Recurse(f func(*Commit) error) error {
	return c.recurse(f, make(map[*Commit]bool, c.Index))
}
...
func (r *Graph) updateBranchInfo() {
	...
	membership := make(map[*Commit]map[string]bool, len(r.commits))
