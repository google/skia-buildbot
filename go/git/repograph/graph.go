...
func New(repo *git.Repo) (*Graph, error) {
	...
	var r gobGraph
	if err := util.ReadGobFile(cacheFile, &r); err != nil {
...
// UpdateAndReturnNewCommits syncs the local copy of hte repo and loads new
...
// cached data but does NOT update the Graph; the caller is responsible for
// doing so before using the Graph if up-to-date data is required.
