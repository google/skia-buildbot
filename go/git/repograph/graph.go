...
func (r *Graph) update(ctx context.Context, returnNewCommits bool) ([]*vcsinfo.LongCommit, error) {
	...
	commits, err := r.UpdateFromGitStore(ctx, r.gitstore, returnNewCommits)
	if err != nil {
		return nil, err
	}
	return commits, nil
...
// ShallowCopy() returns a shallow copy of the Graph; the Commit objects are not
// deep-copied.
func (r *Graph) ShallowCopy() *Graph {
	...
	branches := make([]*git.Branch, 0, len(r.branches))
