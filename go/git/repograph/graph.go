...
type Graph struct {
	...
	// repo is only set if NewLocalGraph() or New() is used.
	repo    *git.Repo
	repoMtx sync.Mutex

	// gitstore is only set if NewBigTableGraph is used.
...
// and workdir. Obtains cached data but does NOT update the Graph; the caller is
...
// NewBigTableGraph returns a Graph instance which is backed by BigTable.
func NewBigTableGraph(ctx context.Context, gs gitstore.GitStore) (*Graph, error) {
	rv := &Graph{
		commits:     map[string]int{},
		commitsData: []*Commit{},
		gitstore:    gs,
...
// Each string argument is a commit hash, or a commit hash preceded by '^'. Per
// "git rev-list" docs, the returned commit hashes consist of all commits
// reachable from the commits specified without '^' and not reachable from the
// commits specified with '^'.
...
func topologicalSortHelper(commits map[*Commit]bool) []*Commit {
	...
	for len(commits) > 0 {
		for commit, _ := range commits {
			c := commit
			for {
				// If a given commit has no children, then we can add it
				// to the return list.
				if len(children[c]) == 0 {
					rv = append(rv, c)
					delete(commits, c)
...
// Obtains cached data but does NOT update the Map; the caller is responsible
// for doing so before using the Map if up-to-date data is required.
...
// NewBigTableMap returns a Map instance with Graphs for the given GitStores.
func NewBigTableMap(ctx context.Context, repos map[string]gitstore.GitStore) (Map, error) {
	rv := make(map[string]*Graph, len(repos))
