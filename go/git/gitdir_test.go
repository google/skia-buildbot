...
func TestGitBranch(t *testing.T) {
	...
	_, err = g.Git(ctx, "fetch", git.DefaultRemote)
	require.NoError(t, err)
	...
	exec_testutils.Run(t, ctx, gb.Dir(), git, "push", git.DefaultRemote, "refs/heads/meta/config")
	exec_testutils.Run(t, ctx, gb.Dir(), git, "update-ref", "refs/tags/meta/config", commits[3])
	exec_testutils.Run(t, ctx, gb.Dir(), git, "push", git.DefaultRemote, "refs/tags/meta/config")
