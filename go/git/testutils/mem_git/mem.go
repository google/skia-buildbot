...
var (
	baseTime = time.Unix(1571926390, 0)
)

// MakeCommit creates a LongCommit.
func MakeCommit(t sktest.TestingT, msg, branch string, parents ...*vcsinfo.LongCommit) *vcsinfo.LongCommit {
	index := 0
	ts := baseTime
	...
	lc.Hash = fmt.Sprintf("%040x", md5.Sum([]byte(j)))
	return lc
}
...
func (g *MemGit) makeCommit(ctx context.Context, msg string, parentHashes []string) *vcsinfo.LongCommit {
	...
	lc := MakeCommit(g.t, msg, g.branch, parents...)
