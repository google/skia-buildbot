...
func TestCheckout(t *testing.T) {
	...
	// Log.
	out, err := c.Git("rev-list", "origin/master")
	assert.NoError(t, err)
	gotCommits := strings.Fields(out)
	...
	got, err := c.Git("rev-parse", "HEAD")
	assert.NoError(t, err)
	assert.Equal(t, commit, strings.TrimSpace(got))
	...
	clean := func() bool {
		...
		h1, err := c.Git("rev-parse", "master")
		assert.NoError(t, err)
		h2, err := c.Git("rev-parse", "origin/master")
		assert.NoError(t, err)
		h3, err := c.Git("rev-parse", "HEAD")
		if h1 != h2 {
			return false
		}
		if h2 != h3 {
