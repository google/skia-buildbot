...
func NewRepo(repoUrl, workdir string) (*Repo, error) {
	...
	if err == nil {
		...
		rv.commitsLen = r.CommitsLen
		for i := 0; i < rv.commitsLen; i++ {
			rv.commitsData[i].repo = rv
		}
...
func (r *Repo) addCommit(hash string) error {
	...
	if r.commitsLen == len(r.commitsData) {
		newCommits := make([]Commit, 2*len(r.commitsData))
		copy(newCommits, r.commitsData)
		r.commitsData = newCommits
	}
	r.commits[hash] = r.commitsLen
	r.commitsData[r.commitsLen].Hash = hash
	r.commitsData[r.commitsLen].Parents = parents
	r.commitsData[r.commitsLen].repo = r
	r.commitsLen++
	return nil
}
