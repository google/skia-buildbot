...
func TestLog(t *testing.T) {
	...
	*/
	...
	checkGitiles := func(fn func(context.Context, string, ...LogOption) ([]*vcsinfo.LongCommit, error), from, to string, expect []string) {
		...
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, hashes(log), expect)
	...
	checkGit := func(from, to string, expect []string, args ...string) {
		...
		deepequal_testutils.AssertDeepEqual(t, log, expect)
	}
...
func TestLogPagination(t *testing.T) {
	...
	check := func(from, to *vcsinfo.LongCommit, expectCommits []*vcsinfo.LongCommit) {
		...
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, expect, log)
...
040000 tree 81b1fde7557bd75ad0392143a9d79ed78d0ed4ab    testutils`))
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []string{"gitiles.go", "gitiles_test.go"}, files)
	deepequal_testutils.AssertDeepEqual(t, []string{"testutils"}, dirs)
...
`
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []string{"gitiles.go", "gitiles_test.go", "testutils/testutils.go"}, files)
}
