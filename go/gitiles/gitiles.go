...
// Reverse is a LogOption which indicates that the commits in the Log should be
// returned in reverse order from the typical "git log" ordering, ie. each
// commit's parents appear before the commit itself.
func Reverse() LogOption {
	return &logOption{v: "reverse=true"}
}

// BatchSize is a LogOption which indicates the number of commits which should
// be included in each batch of commits returned by Log.
func BatchSize(n int) LogOption {
	return &logOption{v: fmt.Sprintf("n=%d", n)}
}
...
// Log returns Gitiles' equivalent to "git log" for the given expression, which
// can be a branch name, commit hash, or expression like "a..b".
...
func (r *Repo) LogLinear(ctx context.Context, from, to string, opts ...LogOption) ([]*vcsinfo.LongCommit, error) {
	...
	commits, err := r.Log(ctx, fmt.Sprintf("%s..%s", from, to), opts...)
	if err != nil {
		return nil, err
	}
	if len(commits) == 0 {
