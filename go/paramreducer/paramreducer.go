...
/*
	...
	Always include the complete paramset for the empty query
	to cover edge cases.
...
func New(q url.Values, full paramtools.ParamSet) (*Reducer, error) {
	// So break apart each query on its keys, i.e. each sub-query with a single
	// key missing.  Match each one. If it matches then add all the value for
	// keys!='missing key' to the paramset.
