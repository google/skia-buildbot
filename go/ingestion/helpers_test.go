...
func TestCompareSources(t *testing.T) {
	...
	_, err = gsSource.Poll(START_TIME, END_TIME, gsCh)
	assert.NoError(t, err)
	gsResults := retFn()

	fsCh, retFn := getCollectingChan(t)
	_, err = fsSource.Poll(START_TIME, END_TIME, fsCh)
	assert.NoError(t, err)
	fsResults := retFn()
...
func testSource(t *testing.T, src Source) {
	...
	_, err := src.Poll(START_TIME, END_TIME, ch)
	assert.NoError(t, err)
	...
	ch, retFn = getCollectingChan(t)
	_, err = src.Poll(BEGINNING_OF_TIME, END_OF_TIME, ch)
	assert.NoError(t, err)
