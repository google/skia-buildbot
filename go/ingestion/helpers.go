...
func (g *gsResultFileLocation) Open() (io.ReadCloser, error) {
	...
	o := func() error {
		...
		if err != nil {
			return skerr.Wrapf(err, "accessing %s/%s failed: %s", g.bucket, g.name)
		}
		...
		if g.content, err = ioutil.ReadAll(reader); err != nil {
			g.content = nil
			return skerr.Wrapf(err, "error reading content of %s/%s: %s", g.bucket, g.name)
		}

		if oa, err := obj.Attrs(context.Background()); err != nil {
			g.content = nil
			return skerr.Wrapf(err, "error reading attributes of %s/%s: %s", g.bucket, g.name)
		} else {
	...
	if g.content == nil {
		if err := backoff.Retry(o, exp); err != nil {
			return nil, skerr.Wrapf(err, "could not read gcs://%s/%s with retries: %s", g.bucket, g.name)
		}
	}
