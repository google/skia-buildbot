...
func getSource(id string, dataSource *sharedconfig.DataSource, client *http.Client) (Source, error) {
	...
	if dataSource.Bucket != "" {
		return NewGoogleStorageSource(id, dataSource.Bucket, dataSource.Dir, client, nil)
	}
...
func (g *GoogleStorageSource) SetEventChannel(resultCh chan<- []ResultFileLocation) error {
	if g.eventBus != nil {
		objRegExp, err := regexp.Compile(`.*\.json`)
		if err != nil {
			return sklog.FmtErrorf("Unable to compile regular expression: %s", err)
		}
		eventType, err := g.eventBus.RegisterStorageEvents(g.bucket, g.rootDir, objRegExp, g.storageClient)
		if err != nil {
			sklog.FmtErrorf("Unable to register storage event: %s", err)
		}
