...
import { HintableObject, Hintable } from 'common-sk/modules/hintable';
...
export class JobSearchSk extends ElementSk {
          ...
          <td>
            ...
            >
              ...
              ${Object.entries(searchTerms)
                .filter(
                  (entry: [string, DisplaySearchTerm]) =>
                    !ele.searchTerms.get(entry[0])
                )
                .map(
                  (entry: [string, DisplaySearchTerm]) => html`
  ...
  private updateQuery: (() => void) | null = null;
  ...
  connectedCallback() {
    ...
    this.updateQuery = stateReflector(
      () => {
        const rv: { [key: string]: string } = {};
        this.searchTerms.forEach((term: searchTerm) => {
          rv[term.key] = term.value;
        });
        return rv;
      },
      (state: HintableObject) => {
        const newSearchTerms: Map<string, searchTerm> = new Map();
        Object.entries(state).forEach((entry: [string, Hintable]) => {
          newSearchTerms.set(entry[0], {
            key: entry[0],
            value: entry[1].toString(),
          });
        });
        // Perform the search unless we just got to this page with no terms in
        // the URL.
        if (newSearchTerms.size != 0 || !this.initialUpdate) {
          this.search();
        }
      }
    );
