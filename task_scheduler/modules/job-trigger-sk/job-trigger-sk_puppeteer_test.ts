...
describe('job-trigger-sk', () => {
  ...
  describe('screenshots', () => {
    ...
    it('deletes job from list', async () => {
      ...
      await takeScreenshot(testBed.page, 'task_scheduler', 'job-trigger-sk-deleted');
    });
    it('adds job to list', async () => {
      await testBed.page.click("add-icon-sk");
      await takeScreenshot(testBed.page, 'task_scheduler', 'job-trigger-sk-added');
    });
    it('triggers jobs', async () => {
      await testBed.page.focus(".job_specs_input");
      await testBed.page.keyboard.type("my-job");
      await testBed.page.focus(".commit_input");
      await testBed.page.keyboard.type("abc123");
      await takeScreenshot(testBed.page, 'task_scheduler', 'job-trigger-sk-pre-trigger');
      await testBed.page.click("send-icon-sk");
      await takeScreenshot(testBed.page, 'task_scheduler', 'job-trigger-sk-post-trigger');
    });
  });
});
