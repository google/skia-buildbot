...
export class JobTriggerSk extends ElementSk {
  ...
  <div class="container ${ele.triggered && ele.triggered.length > 0 ? "" : "hidden"}">
    <h2>Triggered Jobs</h2>
    ${ele.triggered.map((job: TriggeredJob) => html`
  ...
  private triggered: TriggeredJob[] = [];
  ...
  private triggerJobs() {
    ...
    this.rpc.triggerJobs(req).then((resp: TriggerJobsResponse) => {
      this.triggered = resp.jobIds!.map((id: string, index: number) => {
        return {
