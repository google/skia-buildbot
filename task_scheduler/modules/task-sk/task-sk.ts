...
export class TaskSk extends ElementSk {
  private static template = (ele: TaskSk) => html`
    <div class="container">
      ...
      <table>
        <tr><td>ID</td><td>${ele.task!.id}</td></tr>
        <tr><td>Name</td><td>${ele.task!.taskKey!.name}</td></tr>
        <tr>
          <td>Status</td>
        ...
        <tr>
          <td>Repo</td>
          <td>
            <a href="${ele.task!.taskKey!.repoState!.repo}" target="_blank">${ele.task!.taskKey!.repoState!.repo}</a>
          </td>
        </tr>
        <tr>
          <td>Revision</td>
          <td>
            <a href="${ele.revisionLink}" target="_blank">${ele.task!.taskKey!.repoState!.revision}</a>
          </td>
        </tr>
        <tr>
          <td>Swarming Task</td>
  ...
  private reload() {
    ...
    this.rpc!.getTask({
      ...
      const end = this.task.finishedAt ? new Date(this.task.finishedAt) : new Date();
      this.duration = diffDate(start.getTime(), end.getTime());
      if (this.task.taskKey?.repoState?.patch) {
        this.isTryJob = true;
        this.codereviewLink = this.task.taskKey!.repoState!.patch!.server
      }
      this.revisionLink = this.task.taskKey!.repoState!.repo + "/+show/" + this.task.taskKey!.repoState!.revision;
      const textColor = taskStatusToTextColor[this.task.status];
