...
import (
	...
	"go.skia.org/infra/go/vm_creation/vm_server"
)

func TaskSchedulerBase(name, ipAddress string) *vm_creation.Instance {
	vm := vm_server.Server20170518(name, ipAddress)
	vm.DataDisk.SizeGb = 1000
	vm.DataDisk.Type = vm_creation.DISK_TYPE_PERSISTENT_SSD
	...
	vm.Metadata["startup-script"] = "startup-script.sh"
...
func TaskSchedulerProd() *vm_creation.Instance {
	return TaskSchedulerBase("skia-task-scheduler", "104.154.112.128")
}

func TaskSchedulerInternal() *vm_creation.Instance {
	return TaskSchedulerBase("skia-task-scheduler-internal", "104.154.112.135")
}

func TaskSchedulerTest() *vm_creation.Instance {
...
func main() {
	vm_server.Main(map[string]*vm_creation.Instance{
