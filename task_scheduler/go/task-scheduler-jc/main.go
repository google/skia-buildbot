...
var (
	...
	buildbucketBucket        = flag.String("tryjob-bucket", tryjobs.BUCKET_PRIMARY, "Which Buildbucket bucket to use for try jobs.")
	buildbucketProject       = flag.String("buildbucket-project", "skia", "luci-config project in which the buildbucket bucket is defined.")
	buildbucketTarget        = flag.String("buildbucket-target", "", "Buildbucket backend target name used to address this scheduler.")
	buildbucketPubSubProject = flag.String("buildbucket-pubsub_project", "", "Pub/sub project used for sending messages to Buildbucket.")
	bypassGitWrapper         = flag.Bool("bypass-git-wrapper", false, "If set, bypass the git wrapper and call git directly.")
	host                     = flag.String("host", "localhost", "HTTP service host")
	port                     = flag.String("port", ":8000", "HTTP service port for the web server (e.g., ':8000')")
	disableTryjobs           = flag.Bool("disable-try-jobs", false, "If set, no try jobs will be picked up.")
	...
	numSyncWorkers           = flag.Int("sync-workers", syncer.DefaultNumWorkers, "Number of sync worker goroutines to use.")
	useGitCache              = flag.Bool("use-git-cache", true, "Whether or not to use a Git cache for syncing.")
)
