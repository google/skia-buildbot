...
func main() {
	...
	ts, err := scheduling.NewTaskScheduler(ctx, tsDb, bl, period, *commitWindow, serverURL, repos, isolateClient, swarm, httpClient, *scoreDecay24Hr, *swarmingPools, *pubsubTopicName, taskCfgCache, isolateCache, tokenSource, diagClient, diagInstance)
	if err != nil {
		sklog.Fatal(err)
	}
	cleanup.AtExit(func() {
	...
	jc, err := job_creation.NewJobCreator(ctx, tsDb, bl, period, *commitWindow, wdAbs, serverURL, repos, isolateClient, httpClient, tryjobs.API_URL_PROD, *tryJobBucket, common.PROJECT_REPO_MAPPING, depotTools, gerrit, taskCfgCache, isolateCache, tokenSource)
	if err != nil {
		sklog.Fatal(err)
	}
