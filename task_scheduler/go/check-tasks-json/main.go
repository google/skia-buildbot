...
var (
	...
	showAll        = flag.Bool("show-all", false, "If set, show all task+machine groups as opposed to only those with no matches.")
)
...
func main() {
	...
	report, err := checkTasksJSON(ctx, tasksCfg, swarm)
	if err != nil {
		sklog.Fatal(err)
	}
...
type Report struct {
	NoMatchingMachines []*Group
	NoMatchingTasks    []*Group
	Okay               []*Group
}
...
func checkTasksJSON(ctx context.Context, tasksCfg *specs.TasksCfg, swarm swarmingv2.SwarmingV2Client) (*Report, error) {
	...
	report := &Report{
		...
		Okay:               make([]*Group, 0, len(groups)),
	}
	for _, group := range groups {
		report.Okay = append(report.Okay, group)
	}
...
func findLastTask(ctx context.Context, swarm swarmingv2.SwarmingV2Client, taskNames []string) (*apipb.TaskResultResponse, error) {
	...
	for _, taskName := range taskNames {
		...
		for _, task := range resp.Items {
			if task.CreatedTs.AsTime().After(lastTask.CreatedTs.AsTime()) {
				lastTask = task
			}
		}
	}
