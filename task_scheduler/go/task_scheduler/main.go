...
func main() {
	...
	ts, err := auth.NewLegacyTokenSource(*local, oauthCacheFile, "", auth.SCOPE_READ_WRITE)
	if err != nil {
		sklog.Fatal(err)
	}
	...
	if *local {
		...
		clientConfig := httputils.DefaultClientConfig().WithTokenSource(ts).With2xxOnly()
		clientConfig.DialTimeout = 3 * time.Minute
		swarm, err = swarming.NewApiClient(clientConfig.Client(), *swarmingServer)
		if err != nil {
			sklog.Fatal(err)
		}
	}
