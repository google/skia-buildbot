...
func runServer(serverURL string, taskDb db.RemoteDB) {
	...
	r.HandleFunc("/json/blacklist", login.RestrictEditor(jsonBlacklistHandler)).Methods(http.MethodPost, http.MethodDelete)
	r.HandleFunc("/json/job/{id}", jsonJobHandler)
	r.HandleFunc("/json/job/{id}/cancel", login.RestrictEditor(jsonCancelJobHandler)).Methods(http.MethodPost)
	r.HandleFunc("/json/jobs/search", jsonJobSearchHandler)
	r.HandleFunc("/json/task", login.RestrictEditor(jsonTaskHandler)).Methods(http.MethodPost, http.MethodPut)
	...
	r.HandleFunc("/json/trigger", login.RestrictEditor(jsonTriggerHandler)).Methods(http.MethodPost, http.MethodOptions)
	...
	dbRouter.Use(login.RestrictMW(allowed.NewAllowedFromList(VALID_DB_EMAILS)))
