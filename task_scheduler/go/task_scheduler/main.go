...
func mainHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := mainTemplate.Execute(w, struct {
		...
		httputils.ReportError(w, err, "Failed to execute template.")
		return
	}
}
...
func jsonBlacklistHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodDelete {
		...
		if err := json.NewDecoder(r.Body).Decode(&msg); err != nil {
			httputils.ReportError(w, err, fmt.Sprintf("Failed to decode request body: %s", err))
			return
		}
		defer util.Close(r.Body)
		if err := ts.GetBlacklist().RemoveRule(msg.Id); err != nil {
			httputils.ReportError(w, err, fmt.Sprintf("Failed to delete blacklist rule: %s", err))
			return
		}
		...
		if len(rule.Commits) == 2 {
			...
			if err != nil {
				httputils.ReportError(w, err, fmt.Sprintf("Failed to create commit range rule: %s", err))
				return
			}
			rule = *rangeRule
		}
		if err := ts.GetBlacklist().AddRule(&rule, repos); err != nil {
			httputils.ReportError(w, err, fmt.Sprintf("Failed to add blacklist rule: %s", err))
			return
		}
	}
	resp := &struct {
...
func jsonTriggerHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, j := range msg {
		...
		if err != nil {
			httputils.ReportError(w, err, "Unable to find the given commit in any repo.")
			return
		}
		id, err := ts.TriggerJob(context.Background(), repoName, j.Commit, j.Name)
		if err != nil {
			httputils.ReportError(w, err, "Failed to trigger jobs.")
			return
		}
		ids = append(ids, id)
	}
	if err := json.NewEncoder(w).Encode(ids); err != nil {
		httputils.ReportError(w, err, "Failed to encode response.")
		return
	}
}
...
func jsonJobHandler(w http.ResponseWriter, r *http.Request) {
	...
	id, ok := mux.Vars(r)["id"]
	if !ok {
		httputils.ReportError(w, nil, "Job ID is required.")
		return
	}
	...
	if err != nil {
		...
		httputils.ReportError(w, err, "Error retrieving Job.")
		return
	}
...
func jsonCancelJobHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to cancel job.")
		return
	}
...
func jobTimelineHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to retrieve Job.")
		return
	}
	...
	for _, summaries := range job.Tasks {
		for _, t := range summaries {
			...
			if err != nil {
				httputils.ReportError(w, err, "Failed to retrieve Task.")
				return
			}
			swarmingTask, err := swarm.GetTask(task.SwarmingTaskId, true)
			if err != nil {
				httputils.ReportError(w, err, "Failed to retrieve Swarming task.")
				return
			}
...
func taskHandler(w http.ResponseWriter, r *http.Request) {
	...
	id, ok := mux.Vars(r)["id"]
	if !ok {
		httputils.ReportError(w, nil, "Task ID is required.")
		return
	}
...
func jsonJobSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := httputils.ParseFormValues(r, &params); err != nil {
		httputils.ReportError(w, err, "Failed to parse request parameters.")
		return
	}
	jobs, err := db.SearchJobs(tsDb, &params)
	if err != nil {
		httputils.ReportError(w, err, "Failed to search for jobs.")
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func jsonTaskSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to search for tasks.")
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func googleVerificationHandler(w http.ResponseWriter, r *http.Request) {
	if _, err := w.Write([]byte("google-site-verification: google2c59f97e1ced9fdc.html")); err != nil {
		httputils.ReportError(w, err, fmt.Sprintf("Failed to write response: %s", err))
		return
	}
}
