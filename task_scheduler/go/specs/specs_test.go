...
func TestEvalConditional(t *testing.T) {
	vars := map[string]string{
		VARIABLE_CODEREVIEW_SERVER: "my-server",
		VARIABLE_ISSUE:             "my-issue",
		VARIABLE_ISSUE_SHORT:       "my",
		VARIABLE_PATCH_STORAGE:     "my-patch-storage",
		VARIABLE_PATCHSET:          "my-patchset",
		VARIABLE_REPO:              "my-repo",
		VARIABLE_REVISION:          "my-revision",
		VARIABLE_TASK_NAME:         "",
	}
	tc := []struct {
		in  string
		...
		// Empty match.
		{
			in:  Conditional(VARIABLE_REPO, "", ""),
			out: "",
		},
		// Non-empty positive match.
		{
			in:  Conditional(VARIABLE_REPO, "we have repo!", "no repo"),
			out: "we have repo!",
		},
		// Non-empty negative match.
		{
			in:  Conditional(VARIABLE_TASK_NAME, "named task", "unnamed task"),
			out: "unnamed task",
		},
		// "Dangerous" characters.
		{
			in:  Conditional(VARIABLE_REPO, "abc?def", "no repo"),
			out: "abc?def",
		},
		{
			in:  Conditional(VARIABLE_REPO, "abc\"def", "no repo"),
		...
		// Conditional contains a variable expansion.
		{
			in:  Conditional(VARIABLE_REPO, PLACEHOLDER_REPO, "no repo"),
			out: PLACEHOLDER_REPO,
		},
	}
	for _, c := range tc {
		assert.Equal(t, c.out, EvalConditional(c.in, vars))
	}
}
