...
func TestAddedTaskSpecs(t *testing.T) {
	...
	// c3 adds Party and Cleanup
	gb.CreateBranchTrackBranch("branchy-mcbranch-face", "master")
	cfg, err := ReadTasksCfg(gb.Dir())
	assert.NoError(t, err)
	cfg.Jobs["Party"] = &JobSpec{TaskSpecs: []string{"Cleanup"}}
	cfg.Tasks["Party"] = &TaskSpec{
		Dependencies: []string{specs_testutils.BuildTask},
		Isolate:      "swarm_recipe.isolate",
	}
	cfg.Tasks["Cleanup"] = &TaskSpec{
		Dependencies: []string{"Party"},
		Isolate:      "swarm_recipe.isolate",
	}
	gb.Add("infra/bots/tasks.json", testutils.MarshalJSON(t, cfg))
	c3 := gb.Commit()
	...
	assert.True(t, addedTaskSpecs[rs3]["Party"])
	assert.True(t, addedTaskSpecs[rs3]["Cleanup"])
	assert.Equal(t, 0, len(addedTaskSpecs[rs4]))
	assert.Equal(t, 2, len(addedTaskSpecs[rs5]))
	assert.True(t, addedTaskSpecs[rs5]["Party"])
	assert.True(t, addedTaskSpecs[rs5]["Cleanup"])
	assert.Equal(t, 1, len(addedTaskSpecs[rs2]))
	assert.True(t, addedTaskSpecs[rs2][specs_testutils.PerfTask])
}
