...
func NewTaskCfgCache(workdir string, repos map[string]*gitrepo.Repo) *TaskCfgCache {
	return &TaskCfgCache{
		cache:           map[db.RepoState]*TasksCfg{},
		mtx:             sync.Mutex{},
		recentCommits:   map[string]time.Time{},
...
func (c *TaskCfgCache) readTasksCfg(rs db.RepoState) (*TasksCfg, error) {
	...
	rsCpy := rs.Copy()
	rsCpy.Repo = r.Repo().Dir()
	repo, err := TempGitRepo(r.Repo(), rsCpy)
	if err != nil {
		return nil, fmt.Errorf("Could not read task cfg; failed to check out RepoState %q: %s", rs, err)
	}
	defer func() {
		if err := repo.Delete(); err != nil {
			glog.Errorf("Failed to remove: %s", repo.Dir())
		}
	}()

	cfg, err := ReadTasksCfg(repo.Dir())
	if err != nil {
	...
	d := r.Get(rs.Revision) // TODO(borenet): Use a details cache.
	if d == nil {
...
func TempGitRepo(repo *git.Repo, rs db.RepoState) (*git.TempCheckout, error) {
