...
var (
	...
	CIPD_PKGS_GIT     = pkgs(cipd.PkgsGit...)
	CIPD_PKGS_GSUTIL  = pkgs(cipd.MustGetPackage("infra/gsutil"))
	CIPD_PKGS_ISOLATE = pkgs(
		cipd.MustGetPackage("infra/tools/luci/isolate/${platform}"),
		cipd.MustGetPackage("infra/tools/luci/isolated/${platform}"),
	)
	CIPD_PKGS_PYTHON  = pkgs(cipd.PkgsPython...)
	CIPD_PKGS_KITCHEN = append(pkgs(
...
// Convert cipd.Package instances to []*CipdPackage.
func pkgs(p ...*cipd.Package) []*CipdPackage {
	rv := make([]*CipdPackage, 0, len(p))
	for _, pkg := range p {
		rv = append(rv, (*CipdPackage)(pkg))
	}
	return rv
}
