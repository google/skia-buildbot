...
func (c *TaskCfgCache) readTasksCfg(rs db.RepoState) (*TasksCfg, error) {
	...
	// latency.
	rsCpy := rs.Copy()
	altUpstream := path.Join(c.repos.Workdir(), path.Base(rs.Repo))
	rsCpy.Repo = altUpstream
	repoDir, err := TempGitRepo(rsCpy)
	if err != nil {
...
func TempGitRepo(rs db.RepoState) (rv string, rvErr error) {
	// Create a temporary dir for the git checkout.
	tmp, err := ioutil.TempDir("", fmt.Sprintf("tmp_%s", path.Base(rs.Repo)))
	if err != nil {
		return "", err
	}
	defer func() {
