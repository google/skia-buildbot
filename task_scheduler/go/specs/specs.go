...
type TaskCfgCache struct {
	// protected by mtx
	byRepoState   map[db.RepoState]*cacheEntry
...
// gobCacheEntry contains fields that can be deduplicated across cacheEntry
// instances.
...
type gobTaskCfgCache struct {
	AddedTasksCache map[db.RepoState]util.StringSet
	Entries         []gobCacheEntry
	RecentCommits   map[string]time.Time
	RecentJobSpecs  map[string]time.Time
	RecentTaskSpecs map[string]time.Time
	// Value is an index into Entries.
...
func NewTaskCfgCache(ctx context.Context, repos repograph.Map, depotToolsDir, workdir string, numWorkers int) (*TaskCfgCache, error) {
	...
	if err == nil {
		...
		c.byRepoState = make(map[db.RepoState]*cacheEntry, len(gobCache.RepoStates))
		...
		for rs, idx := range gobCache.RepoStates {
			if idx < 0 || idx >= len(gobCache.Entries) {
				return nil, fmt.Errorf("Corrupt cache file %q: RepoStates index %d out of range.", file, idx)
			}
			gobCacheEntry := gobCache.Entries[idx]
			c.byRepoState[rs] = &cacheEntry{
				c:    c,
				cfg:  gobCacheEntry.Cfg,
				err:  gobCacheEntry.Err,
				hash: gobCacheEntry.Hash,
				rs:   rs,
			}
		}
		...
		c.byRepoState = map[db.RepoState]*cacheEntry{}
...
func (c *TaskCfgCache) write() error {
	...
	return util.WithWriteFile(c.file, func(w io.Writer) error {
		gobCache := gobTaskCfgCache{
			...
			Entries:         make([]gobCacheEntry, 0, len(c.byRepoState)),
			RecentCommits:   c.recentCommits,
			RecentJobSpecs:  c.recentJobSpecs,
			RecentTaskSpecs: c.recentTaskSpecs,
			RepoStates:      make(map[db.RepoState]int, len(c.byRepoState)),
		}
		// We need to key gobCacheEntry by both hash and err. In the case that a
		// patch doesn't apply, hash will be all-zero, but err could vary.
		type entryKey struct {
			hash TasksCfgFileHash
			err  string
		}
		entryIdxMap := make(map[entryKey]int, len(c.byRepoState))
		for _, e := range c.byRepoState {
			if e.cfg == nil && e.err == "" {
			...
			idx, ok := entryIdxMap[key]
			if !ok {
				idx = len(gobCache.Entries)
				entryIdxMap[key] = idx
				gobCache.Entries = append(gobCache.Entries, gobCacheEntry{
					Cfg:  e.cfg,
					Err:  e.err,
					Hash: e.hash,
				})
