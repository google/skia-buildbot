...
type CachedValue struct {
	...
	// Error stores a permenent error. Mutually-exclusive with Isolated.
	Error string
}
...
func (c *backingCache) Set(ctx context.Context, key string, value set_if_unset_cache.Value) error {
	var buf bytes.Buffer
	if err := gob.NewEncoder(&buf).Encode(value.(*CachedValue)); err != nil {
		return err
	}
...
func New(ctx context.Context, btProject, btInstance string, ts oauth2.TokenSource) (*Cache, error) {
	...
	return &Cache{
		cache: set_if_unset_cache.New(&backingCache{
			table: table,
		}),
	}, nil
}
...
func (c *Cache) Get(ctx context.Context, rs types.RepoState, isolateFile string) (*isolate.IsolatedFile, error) {
...
func (c *Cache) SetIfUnset(ctx context.Context, rs types.RepoState, fn func(context.Context) (*CachedValue, error)) error {
	_, err := c.cache.SetIfUnset(ctx, rs.RowKey(), func(ctx context.Context) (set_if_unset_cache.Value, error) {
		cv, err := fn(ctx)
		return cv, err
	})
	return err
