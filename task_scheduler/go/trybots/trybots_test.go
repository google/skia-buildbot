...
func TestHeartbeats(t *testing.T) {
	...
	// Test bogus return from Buildbucket.
	MockHeartbeats(t, mock, now, []*db.Job{j2}, map[string]*heartbeatResp{
		j2.Id: &heartbeatResp{
			Error: &errMsg{
				Message: "fail",
			},
	...
	assert.EqualError(t, trybots.sendHeartbeats(now), "Got errors sending heartbeats: [Error sending heartbeat for job; no valid ID returned from buildbucket so can't cancel the job.]")
	assert.True(t, mock.Empty())
}
...
func TestPoll(t *testing.T) {
	...
	// Multiple new builds, fail jobStarted, ensure that the failed job is
	// canceled and the others are properly added.
	builds = makeBuilds(5)
	failBuild = builds[failIdx]
	MockPeek(mock, builds, now, "", "", nil)
	builds = append(builds[:failIdx], builds[failIdx+1:]...)
