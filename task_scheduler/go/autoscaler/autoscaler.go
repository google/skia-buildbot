...
func (a *Autoscaler) autoscale(candidateDimensions [][]string, now time.Time) error {
	for idx, scaler := range a.scalers {
		...
		metrics2.GetInt64Metric("autoscaler_target", tags).Update(int64(target))
		metrics2.GetInt64Metric("autoscaler_need_bots", tags).Update(int64(needBots))
		metrics2.GetInt64Metric("autoscaler_delta", tags).Update(int64(delta))
