...
func TestAutoscaler(t *testing.T) {
	...
	as := New([]*autoscaler.Autoscaler{s1})
	now := time.Now()
	...
	// Hack the targetTimes to make things predictable.
	for _, scaler := range as.scalers {
		as.targetTime[scaler] = now
	}
	...
	for lastStopped > 0 {
		...
		stopping := instances[lastStopped-stoppingN : lastStopped]
