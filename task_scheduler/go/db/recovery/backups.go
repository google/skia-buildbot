...
const (
	...
	// DB_FILE_NAME_EXTENSION is added to the base filename, along with .gz.
...
func (b *gsDBBackup) backupDB(now time.Time, basename string) (err error) {
	...
	objectname := fmt.Sprintf("%s/%s/%s.%s.gz", DB_BACKUP_DIR, now.Format("2006/01/02"), basename, DB_FILE_NAME_EXTENSION)
...
// triggerDir and the number of times backupDB has failed for this trigger.
...
func (b *gsDBBackup) writeTriggerFile(basename string, retries int) {
	filename := path.Join(b.triggerDir, basename)
	content := []byte(strconv.Itoa(retries))
	if err := ioutil.WriteFile(filename, content, 0666); err != nil {
		glog.Errorf("Unable to write new retry count (%d) to trigger file %s: %s", retries, filename, err)
	}
}

// deleteTriggerFile removes the given trigger file indicating that the backup
// succeeded or retries are exhausted.
func (b *gsDBBackup) deleteTriggerFile(basename string) {
	filename := path.Join(b.triggerDir, basename)
	if err := os.Remove(filename); err != nil {
		glog.Errorf("Unable to remove trigger file %s: %s", filename, err)
	}
}
...
func (b *gsDBBackup) maybeBackupDB(now time.Time) {
	...
	if err := b.backupDB(now, basename); err != nil {
		...
		if attemptCount >= RETRY_COUNT {
			...
			b.deleteTriggerFile(basename)
		} else {
			b.writeTriggerFile(basename, attemptCount)
		}
	} else {
