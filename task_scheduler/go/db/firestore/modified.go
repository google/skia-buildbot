...
// firestore.WatchQuery with a query by DbModified time. Runs until the given
...
// ModifiedTasks passes slices of Tasks along the returned channel as they are
// modified in the DB.
func ModifiedTasks(ctx context.Context, d db.DB) <-chan []*types.Task {
...
// ModifiedJobs passes slices of Jobs along the returned channel as they are
// modified in the DB.
func ModifiedJobs(ctx context.Context, d db.DB) <-chan []*types.Job {
...
// ModifiedTaskComments passes slices of TaskComments along the returned channel
// as they are modified in the DB.
func ModifiedTaskComments(ctx context.Context, d db.DB) <-chan []*types.TaskComment {
	outCh := make(chan []*types.TaskComment)
	go func() {
		for snap := range modifiedCh(ctx, d.(*firestoreDB).taskComments(), KEY_DB_MODIFIED) {
...
// ModifiedTaskSpecComments passes slices of TaskSpecComments along the returned
// channel as they are modified in the DB.
func ModifiedTaskSpecComments(ctx context.Context, d db.DB) <-chan []*types.TaskSpecComment {
	outCh := make(chan []*types.TaskSpecComment)
	go func() {
		for snap := range modifiedCh(ctx, d.(*firestoreDB).taskSpecComments(), KEY_DB_MODIFIED) {
...
// ModifiedCommitComments passes slices of CommitComments along the returned
// channel as they are modified in the DB.
func ModifiedCommitComments(ctx context.Context, d db.DB) <-chan []*types.CommitComment {
	outCh := make(chan []*types.CommitComment)
	go func() {
		for snap := range modifiedCh(ctx, d.(*firestoreDB).commitComments(), KEY_DB_MODIFIED) {
