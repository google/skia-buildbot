...
// caller must provide four functions:
//
...
//    for a mutex.
// 3. A restart function which is only called in the case of an error which will
...
//    index.
func (d *firestoreDB) dateRangeHelper(coll *fs.CollectionRef, start, end time.Time, init func(int), elem func(int, *fs.DocumentSnapshot) error, restart func(int) error) error {
	...
	// Run the queries.
	return firestore.IterDocsInParallel(queries, DEFAULT_ATTEMPTS, GET_MULTI_TIMEOUT, elem, restart)
}
