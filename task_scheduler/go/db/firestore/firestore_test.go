...
func setup(t *testing.T) (db.DBCloser, func()) {
	...
	parentDoc := fmt.Sprintf("test/%s", uuid.New())
	d, err := NewDB(context.Background(), "skia-firestore", parentDoc, nil)
	assert.NoError(t, err)
	cleanup := func() {
		_, err := d.(*firestoreDB).client.Doc(parentDoc).Delete(context.Background())
		assert.NoError(t, err)
