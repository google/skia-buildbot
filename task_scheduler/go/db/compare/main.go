...
func TaskLess(a *types.Task, b *types.Task) bool {
...
type StrictSortTask []*types.Task

func (s StrictSortTask) Len() int           { return len(s) }
func (s StrictSortTask) Less(i, j int) bool { return TaskLess(s[i], s[j]) }
func (s StrictSortTask) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
...
func JobLess(a *types.Job, b *types.Job) bool {
	if a.Created.Equal(b.Created) {
		return a.Id < b.Id
	}
	return a.Created.Before(b.Created)
...
type StrictSortJob []*types.Job

func (s StrictSortJob) Len() int           { return len(s) }
func (s StrictSortJob) Less(i, j int) bool { return JobLess(s[i], s[j]) }
func (s StrictSortJob) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
...
func compareByChunk(oldDB, newDB db.DB, output func(interface{}) error) error {
	...
	if err := util.IterTimeChunks(BEGINNING_OF_TIME, NOW, TIME_CHUNK, func(start, end time.Time) error {
		chunks = append(chunks, &timeChunk{
			...
			end:   firestore.FixTimestamp(end),
		})
		return nil
	}); err != nil {
		return err
			...
			Outer1:
				for _, newComment := range newComments {
					for oldI, oldComment := range oldComments {
						oldComment.Timestamp = firestore.FixTimestamp(oldComment.Timestamp)
