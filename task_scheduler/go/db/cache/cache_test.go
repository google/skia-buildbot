...
func testGetTasksForCommits(t *testing.T, c TaskCache, b *types.Task) {
	for _, commit := range b.Commits {
		...
		deepequal_testutils.AssertDeepEqual(t, b, found)

		tasks, err := c.GetTasksForCommits(types.DEFAULT_TEST_REPO, []string{commit})
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, b, tasks[commit][b.Name])
	}
}
...
func TestTaskCache(t *testing.T) {
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, map[string]map[string]*types.Task{
		"b": {
			t1.Name: t1,
			t3.Name: t3,
		},
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, got, t1)
}
...
func TestTaskCacheGetTasksFromDateRange(t *testing.T) {
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{t1}, tasks)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{t1, t2}, tasks)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{t1, t2, t3}, tasks)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{t2, t3}, tasks)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{t3}, tasks)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*types.Task{}, tasks)
}
...
func assertTaskInSlice(t *testing.T, task *types.Task, slice []*types.Task) {
	for _, other := range slice {
		if task.Id == other.Id {
			deepequal_testutils.AssertDeepEqual(t, task, other)
			return
		}
	}
...
func TestTaskCacheExpiration(t *testing.T) {
...

	{
		...
		for _, task := range []*types.Task{tasks[0], tasks[1]} {
			...
			require.NoError(t, err)
			deepequal_testutils.AssertDeepEqual(t, task, cachedTask)
...

	{
		...
		deepequal_testutils.AssertDeepEqual(t, orderedTasks, allCachedTasks)
...

	{
		...
		allCachedTasks, err := c.GetTasksFromDateRange(timeStart, timeStart.Add(20*time.Minute))
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, newTasks, allCachedTasks)
		...
		unfinishedTasks, err := c.UnfinishedTasks()
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, newTasks, unfinishedTasks)
	}
}
...
func TestJobCache(t *testing.T) {
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, j1, test)
	jobs, err := c.GetJobsByRepoState(j1.Name, j1.RepoState)
	require.NoError(t, err)
	require.Equal(t, 1, len(jobs))
	deepequal_testutils.AssertDeepEqual(t, jobs[0], test)
	...
	test, err = c.GetJob(j2.Id)
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, j2, test)
	jobs, err = c.GetJobsByRepoState(j2.Name, j2.RepoState)
	require.NoError(t, err)
	require.Equal(t, 2, len(jobs))
	deepequal_testutils.AssertDeepEqual(t, jobs[1], test)
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, got, j1)
}
...
func testGetUnfinished(t *testing.T, expect []*types.Job, cache JobCache) {
	...
	deepequal_testutils.AssertDeepEqual(t, expect, jobs)
}
...
func assertJobInSlice(t *testing.T, job *types.Job, slice []*types.Job) {
	for _, other := range slice {
		if job.Id == other.Id {
			deepequal_testutils.AssertDeepEqual(t, job, other)
			return
		}
	}
...
func assertJobsCached(t *testing.T, c JobCache, jobs []*types.Job) {
	...
	for _, job := range jobs {
		...
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, job, cachedJob)
		...
		for _, otherJob := range cachedJobs {
			if job.Id == otherJob.Id {
				deepequal_testutils.AssertDeepEqual(t, job, otherJob)
				found = true
			}
		}
...
func TestJobCacheGetMatchingJobsFromDateRange(t *testing.T) {
	...
	test := func(names []string, start, end time.Time, expect ...*types.Job) {
		...
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, expectByName, jobs)
	}
