...
type TaskCache interface {
	...
	// OnModifiedTasks sets a callback function which is called whenever
	// modified tasks are received from the DB. Overwrites any previously-
	// set callback. Used for testing.
	OnModifiedTasks(func())
}
...
// See documentation for TaskCache interface.
func (c *taskCache) OnModifiedTasks(fn func()) {
	c.modMtx.Lock()
	defer c.modMtx.Unlock()
	c.onModFn = fn
}
...
type JobCache interface {
	...
	// OnModifiedJobs sets a callback function which is called whenever
	// modified jobs are received from the DB. Overwrites any previously-
	// set callback. Used for testing.
	OnModifiedJobs(func())
}
...
// See documentation for JobCache interface.
func (c *jobCache) OnModifiedJobs(fn func()) {
	c.modMtx.Lock()
	defer c.modMtx.Unlock()
	c.onModFn = fn
}
