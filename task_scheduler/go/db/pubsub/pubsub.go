...
const (
	...
	// Attribute sent with all pubsub messages; currently unused.
	ATTR_ID = "id"
)
...
// NewTaskPublisher creates a TaskPublisher instance.
...
// NewJobPublisher creates a JobPublisher instance.
...
// function for every pubsub message. If the callback returns an error, the
// message will be re-sent at a later time, so if the data cannot be processed,
// the callback should return no error to prevent the message from being
// re-sent.
...
// NewTaskSubscriber creates a subscriber instance which calls the given
// callback function for every pubsub message. If the callback returns an error,
// the message will be re-sent at a later time, so if the data cannot be
// processed, the callback should return no error to prevent the message from
// being re-sent.
...
// NewJobSubscriber creates a subscriber instance which calls the given
// callback function for every pubsub message. If the callback returns an error,
// the message will be re-sent at a later time, so if the data cannot be
// processed, the callback should return no error to prevent the message from
// being re-sent.
