package pubsub

import (
	"bytes"
	"context"
	"encoding/gob"
	"errors"
	"time"

	"cloud.google.com/go/pubsub"
	"go.skia.org/infra/go/sklog"
	"go.skia.org/infra/task_scheduler/go/db"
	"go.skia.org/infra/task_scheduler/go/db/local_db"
)

const (
	// Default project ID.
	PROJECT_ID = "skia-public"

	// Known topic names.
	TOPIC_TASKS          = "task-scheduler-modified-tasks"
	TOPIC_TASKS_INTERNAL = "task-scheduler-modified-tasks-internal"
	TOPIC_JOBS           = "task-scheduler-modified-jobs"
	TOPIC_JOBS_INTERNAL  = "task-scheduler-modified-jobs-internal"

	// Attribute sent with all pubsub messages; job or task ID.
	ATTR_ID = "id"
)

// publisher sends pubsub messages for modified tasks and jobs.
type publisher struct {
	topic *pubsub.Topic
}

// newPublisher is a helper function for NewTaskPublisher.
func newPublisher(c *pubsub.Client, topic string) (*publisher, error) {
	t := c.Topic(topic)
	exists, err := t.Exists(context.Background())
	if err != nil {
		return nil, err
	}
	if !exists {
		if _, err := c.CreateTopic(context.Background(), topic); err != nil {
			return nil, err
		}
	}
	return &publisher{
		topic: t,
	}, nil
}

// publish publishes a pubsub message for the given data.
func (p *publisher) publish(id string, data interface{}) error {
	buf := bytes.Buffer{}
	if err := gob.NewEncoder(&buf).Encode(data); err != nil {
		return err
	}
	return p.publishGOB(map[string][]byte{
		id: buf.Bytes(),
	})
}

// publishGOB publishes a pubsub message for the given gob-encoded data.
func (p *publisher) publishGOB(byId map[string][]byte) error {
	ctx := context.Background()
	res := make([]*pubsub.PublishResult, 0, len(byId))
	for id, data := range byId {
		res = append(res, p.topic.Publish(ctx, &pubsub.Message{
			Data: data,
			Attributes: map[string]string{
				ATTR_ID: id,
			},
		}))
	}
	for _, result := range res {
		_, err := result.Get(ctx)
		if err != nil {
			return err
		}
	}
	return nil
}

// TaskPublisher sends pubsub messages for modified tasks.
type TaskPublisher struct {
	*publisher
}

// NewTaskPublisher creates a TaskPublisher instance. It creates the given topic
// if it does not already exist.
func NewTaskPublisher(c *pubsub.Client, topic string) (*TaskPublisher, error) {
	pub, err := newPublisher(c, topic)
	if err != nil {
		return nil, err
	}
	return &TaskPublisher{pub}, nil
}

// Publish publishes a pubsub message for the given task.
func (p *TaskPublisher) Publish(t *db.Task) error {
	return p.publish(t.Id, t)
}

// JobPublisher sends pubsub messages for modified jobs.
type JobPublisher struct {
	*publisher
}

// NewJobPublisher creates a JobPublisher instance. It creates the given topic
// if it does not already exist.
func NewJobPublisher(c *pubsub.Client, topic string) (*JobPublisher, error) {
	pub, err := newPublisher(c, topic)
	if err != nil {
		return nil, err
	}
	return &JobPublisher{pub}, nil
}

// Publish publishes a pubsub message for the given job.
func (p *JobPublisher) Publish(j *db.Job) error {
	return p.publish(j.Id, j)
}

// subscriber uses pubsub to watch for modified data.
type subscriber struct {
	client   *pubsub.Client
	callback func(*pubsub.Message) error
	id       string
	topic    string

	cancel context.CancelFunc
	sub    *pubsub.Subscription
}

// newSubscriber creates a subscriber instance which calls the given callback
// function for every pubsub message. The callback function should not Ack/Nack
// the message; this is done automatically based on the return value of the
// callback: if the callback returns an error, the message is Nack'd and will be
// re-sent at a later time, otherwise the message is Ack'd and will not be
// re-sent. Therefore, if the message data is not valid or otherwise cannot ever
// be processed, the callback should return nil to prevent the message from
// being re-sent.
func newSubscriber(c *pubsub.Client, topic, label string, callback func(*pubsub.Message) error) (*subscriber, error) {
	// Create a pubsub subscription. This will return an error if we somehow
	// reused an ID.
	id := topic + "+" + label + "_" + time.Now().Format(local_db.TIMESTAMP_FORMAT)
	return &subscriber{
		client:   c,
		callback: callback,
		topic:    topic,
		id:       id,
		sub:      c.Subscription(id),
	}, nil
}

// SubscriberID returns the ID of the pubsub subscription.
func (s *subscriber) SubscriberID() string {
	return s.id
}

// start causes the subscriber to start watching for modified data.
func (s *subscriber) start() error {
	if s.cancel != nil {
		return errors.New("Subscriber is already running.")
	}
	ctx, cancel := context.WithCancel(context.Background())
	sub, err := s.client.CreateSubscription(ctx, s.id, pubsub.SubscriptionConfig{
		Topic: s.client.Topic(s.topic),
	})
	if err != nil {
		return err
	}

	// Start receiving pubsub messages for this ID.
	go func() {
		if err := sub.Receive(ctx, func(ctx context.Context, m *pubsub.Message) {
			select {
			case <-ctx.Done():
				sklog.Warning("Received pubsub message but the context has been canceled.")
				m.Nack()
			default:
				if err := s.callback(m); err != nil {
					sklog.Warningf("Callback failed for pubsub message: %s", err)
					m.Nack()
				} else {
					m.Ack()
				}
			}
		}); err != nil {
			sklog.Errorf("Pubsub subscription receive failed: %s", err)
		}
	}()
	s.cancel = cancel
	return nil
}

// Stop causes the subscriber to stop watching for tasks.
func (s *subscriber) Stop() error {
	if s.cancel == nil {
		return errors.New("Subscriber is not running.")
	}
	s.cancel()
	if err := s.client.Subscription(s.id).Delete(context.Background()); err != nil {
		return err
	}
	s.cancel = nil
	return nil
}

// TaskSubscriber uses pubsub to watch for modified tasks.
type TaskSubscriber struct {
	*subscriber
}

// NewTaskSubscriber creates a subscriber which calls the given callback
// function for every pubsub message. Acknowledgement of the message is done
// automatically based on the return value of the callback: if the callback
// returns an error, the message is Nack'd and will be re-sent at a later time,
// otherwise the message is Ack'd and will not be re-sent. Therefore, if the
// task is not valid or otherwise cannot ever be processed, the callback should
// return nil to prevent the message from being re-sent.
func NewTaskSubscriber(c *pubsub.Client, topic, label string, callback func(*db.Task) error) (*TaskSubscriber, error) {
	s, err := newSubscriber(c, topic, label, func(m *pubsub.Message) error {
		var t db.Task
		if err := gob.NewDecoder(bytes.NewReader(m.Data)).Decode(&t); err != nil {
			sklog.Errorf("Failed to decode task from pubsub message: %s", err)
			return nil // We will never be able to process this message.
		}
		return callback(&t)
	})
	if err != nil {
		return nil, err
	}
	if err := s.start(); err != nil {
		return nil, err
	}
	return &TaskSubscriber{s}, nil
}

// JobSubscriber uses pubsub to watch for modified jobs.
type JobSubscriber struct {
	*subscriber
}

// NewJobSubscriber creates a subscriber which calls the given callback
// function for every pubsub message. Acknowledgement of the message is done
// automatically based on the return value of the callback: if the callback
// returns an error, the message is Nack'd and will be re-sent at a later time,
// otherwise the message is Ack'd and will not be re-sent. Therefore, if the
// job is not valid or otherwise cannot ever be processed, the callback should
// return nil to prevent the message from being re-sent.
func NewJobSubscriber(c *pubsub.Client, topic, label string, callback func(*db.Job) error) (*JobSubscriber, error) {
	s, err := newSubscriber(c, topic, label, func(m *pubsub.Message) error {
		var j db.Job
		if err := gob.NewDecoder(bytes.NewReader(m.Data)).Decode(&j); err != nil {
			sklog.Errorf("Failed to decode job from pubsub message: %s", err)
			return nil // We will never be able to process this message.
		}
		return callback(&j)
	})
	if err != nil {
		return nil, err
	}
	if err := s.start(); err != nil {
		return nil, err
	}
	return &JobSubscriber{s}, nil
}
