...
var AssertDeepEqual func(t testing.TestingT, expected, actual interface{})

func findModifiedTasks(t testing.TestingT, m ModifiedTasksReader, id string, expect ...*types.Task) {
...
func findModifiedJobs(t testing.TestingT, m ModifiedJobsReader, id string, expect ...*types.Job) {
	// Note that the slice only works because we don't call
...
func findModifiedComments(t testing.TestingT, m ModifiedComments, id string, e1 []*types.TaskComment, e2 []*types.TaskSpecComment, e3 []*types.CommitComment) {
	// Note that the slice only works because we don't call
	// TrackModifiedJob more than once for any given job, otherwise
	// we'd have to use a map and compare DbModified.
...
func TestTaskDB(t testing.TestingT, db TaskDB) {
...
func TestTaskDBConcurrentUpdate(t testing.TestingT, db TaskDB) {
	// Insert a task.
...
func testUpdateTasksWithRetriesSimple(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTasksWithRetriesSuccess(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTasksWithRetriesErrorInFunc(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTasksWithRetriesErrorInPutTasks(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTasksWithRetriesExhausted(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTaskWithRetriesSimple(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTaskWithRetriesSuccess(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTaskWithRetriesErrorInFunc(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTaskWithRetriesExhausted(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func testUpdateTaskWithRetriesTaskNotFound(t testing.TestingT, db TaskDB) {
	begin := time.Now()
...
func TestUpdateTasksWithRetries(t testing.TestingT, db TaskDB) {
...
func TestJobDB(t testing.TestingT, db JobDB) {
...
func TestJobDBTooManyUsers(t testing.TestingT, db JobDB) {
...
// has been updated in the DB.
func TestJobDBConcurrentUpdate(t testing.TestingT, db JobDB) {
	// Insert a job.
...
func TestCommentDB(t testing.TestingT, db CommentDB) {
...
func TestTaskDBGetTasksFromDateRangeByRepo(t testing.TestingT, db TaskDB) {
	r1 := common.REPO_SKIA
...
func TestTaskDBGetTasksFromWindow(t testing.TestingT, db TaskDB) {
	now := time.Now()
...
func TestModifiedTasks(t testing.TestingT, m ModifiedTasks) {
...
func TestMultipleTaskModifications(t testing.TestingT, m ModifiedTasks) {
	id, err := m.StartTrackingModifiedTasks()
	assert.NoError(t, err)
...
func TestModifiedJobs(t testing.TestingT, m ModifiedJobs) {
...
func TestMultipleJobModifications(t testing.TestingT, m ModifiedJobs) {
	id, err := m.StartTrackingModifiedJobs()
	assert.NoError(t, err)
...
func TestModifiedComments(t testing.TestingT, m ModifiedComments) {
...
func TestMultipleCommentModifications(t testing.TestingT, m ModifiedComments) {
	id, err := m.StartTrackingModifiedComments()
	assert.NoError(t, err)
...
func TestUpdateDBFromSwarmingTask(t testing.TestingT, db TaskDB) {
...
func TestUpdateDBFromSwarmingTaskTryJob(t testing.TestingT, db TaskDB) {
	// Create task, initialize from swarming, and save.
	now := time.Now().UTC().Round(time.Microsecond)
	task := &types.Task{
		TaskKey: types.TaskKey{
