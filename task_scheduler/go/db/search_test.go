...
func TestJobSearch(t *testing.T) {
	...
	*isForce = j.IsForce
	...
	checkMatches := func(p *JobSearchParams) {
		...
		deepequal_testutils.AssertDeepEqual(t, j, jobs[0])
	}
...
func TestJobSearchParamsJson(t *testing.T) {
	...
	*testIsForce = true
	p := &JobSearchParams{}
	deepequal_testutils.AssertDeepEqual(t, p, decode(`{}`))

	p.IsForce = testIsForce
	deepequal_testutils.AssertDeepEqual(t, p, decode(`{"is_force": true}`))

	*p.IsForce = false
	deepequal_testutils.AssertDeepEqual(t, p, decode(`{"is_force": false}`))
}
