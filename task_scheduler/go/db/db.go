...
// JobDB is used by the job scheduler to store Jobs.
type JobDB interface {
	JobReader

	// PutJob inserts or updates the Job in the database. Job's Id field must
	// be empty if it is a new Job. PutJob will set Job.DbModified.
	PutJob(*Job) error

	// PutJobs inserts or updates the Jobs in the database. Each Jobs' Id field
	// must be empty. Each Jobs' DbModified field will be set.
	PutJobs([]*Job) error
}
