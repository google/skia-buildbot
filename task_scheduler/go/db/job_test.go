...
func TestJobDeriveStatus(t *testing.T) {
	...
	j1 := &Job{
		Dependencies: map[string][]string{"test": []string{"build"}},
		Name:         "j1",
		RepoState: RepoState{
			Repo:     "my-repo",
	...
	t1 := &MiniTask{Status: TASK_STATUS_RUNNING}
	j1.Tasks = map[string][]*MiniTask{"build": []*MiniTask{t1}}
	assert.Equal(t, j1.DeriveStatus(), JOB_STATUS_IN_PROGRESS)
	...
	t2 := &MiniTask{Status: TASK_STATUS_PENDING}
	...
	// Add the test task.
	t3 := &MiniTask{Status: TASK_STATUS_RUNNING}
	j1.Tasks["test"] = []*MiniTask{t3}
	assert.Equal(t, j1.DeriveStatus(), JOB_STATUS_IN_PROGRESS)
