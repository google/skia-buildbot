...
// for whose dependencies the function has already been called. The passed-in
// function should return two values, a boolean and an error. Returning true
// causes iteration over the dependency graph to stop. Returning an error does
// the same but causes TraverseDependencies to return the same error.
