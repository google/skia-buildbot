...
func (c *jobCache) expireAndUpdate(start time.Time, jobs []*Job) {
	...
	for _, job := range jobs {
		if c.getJobTimestamp(job).Before(start) {
			glog.Warningf("Updated job %s after expired. %#v", job.Id, job)
		} else {
...
// RepoMapGetRevisionTimestamp returns a GetRevisionTimestamp function that gets
// the revision timestamp from repos.
func RepoMapGetRevisionTimestamp(repos *gitinfo.RepoMap) GetRevisionTimestamp {
	return func(repo, revision string) (time.Time, error) {
		r, err := repos.Repo(repo)
		if err != nil {
			return time.Time{}, err
		}
		ts := r.Timestamp(revision)
