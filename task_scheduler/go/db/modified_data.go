...
func (m *modifiedData) GetModifiedEntries(id string) ([][]byte, error) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
	if _, ok := m.expiration[id]; !ok {
		return nil, ErrUnknownId
	}
	rv := make([][]byte, 0, len(m.data[id]))
	for _, d := range m.data[id] {
		rv = append(rv, d)
	}
...
func NewModifiedTasks() *ModifiedTasks {
...
func NewModifiedJobs() *ModifiedJobs {
	return &ModifiedJobs{
		m: &modifiedData{},
	}
}
