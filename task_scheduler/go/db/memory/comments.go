...
func NewCommentBox() *CommentBox {
	...
	go func() {
		for data := range modTC {
			...
			for ch := range rv.modTCClients {
				...
				// production anyway.
				go func(ch chan<- []*types.TaskComment, data []*types.TaskComment) {
	...
	go func() {
		for data := range modTSC {
			...
			for ch := range rv.modTSClients {
				// NOTE: This would cause a goroutine leak if a
				// client stopped reading from their channel. We
				// could add a timeout for the channel write,
				// but this isn't intended to be used in
				// production anyway.
				go func(ch chan<- []*types.TaskSpecComment, data []*types.TaskSpecComment) {
	...
	go func() {
		for data := range modCC {
			...
			for ch := range rv.modCCClients {
				// NOTE: This would cause a goroutine leak if a
				// client stopped reading from their channel. We
				// could add a timeout for the channel write,
				// but this isn't intended to be used in
				// production anyway.
				go func(ch chan<- []*types.CommitComment, data []*types.CommitComment) {
