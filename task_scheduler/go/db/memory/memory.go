...
func (d *InMemoryTaskDB) PutTasks(tasks []*types.Task) error {
	sklog.Errorf("PutTasks(%d)", len(tasks))
	defer sklog.Errorf("PutTasks(%d) done", len(tasks))
	...
	for _, task := range tasks {
		...
		d.tasks[task.Id] = cpy
		ids = append(ids, task.Id+": "+task.Name)
	}
	// Send the modified tasks to any listeners.
	sklog.Errorf("queuing %v", ids)
...
func (d *InMemoryTaskDB) ModifiedTasksCh(ctx context.Context) <-chan []*types.Task {
	...
	go func() {
		...
		for {
			select {
			case mod := <-localCh:
				ids := make([]string, 0, len(mod))
				for _, t := range mod {
					ids = append(ids, t.Id+": "+t.Name)
				}
				sklog.Errorf("Sending to client: %v", ids)
				rv <- mod
				// Corresponds to Add() in NewInMemoryTaskDB.
				d.modClientsWg.Done()
				sklog.Errorf("Done sending to client: %v", ids)
			case <-done:
				close(rv)
