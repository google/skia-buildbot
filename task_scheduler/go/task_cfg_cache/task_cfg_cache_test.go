...
func TestTaskSpecs(t *testing.T) {
	...
	assert.NoError(t, cache.Set(ctx, rs1, tu.BaseTasksCfg, nil))
	assert.NoError(t, cache.Set(ctx, rs2, tu.NextTasksCfg, nil))
...
func TestAddedTaskSpecs(t *testing.T) {
	...
	assert.True(t, addedTaskSpecs[rs1][tu.BuildTaskName])
	assert.True(t, addedTaskSpecs[rs1][tu.TestTaskName])
	assert.Equal(t, 1, len(addedTaskSpecs[rs2]))
	assert.True(t, addedTaskSpecs[rs2][tu.PerfTaskName])
...
func TestTaskCfgCacheStorage(t *testing.T) {
	...
	// One entry.
	assert.NoError(t, c.Set(ctx, rs1, tu.BaseTasksCfg, nil))
	check(rs1)
	...
	assert.NoError(t, repos.Update(ctx))
	assert.NoError(t, c.Set(ctx, rs3, tu.NextTasksCfg, nil))
	assert.NoError(t, c.Set(ctx, rs4, tu.NextTasksCfg, nil))
