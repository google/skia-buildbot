...
func TestTaskSpecs(t *testing.T) {
	...
	assert.NoError(t, cache.Set(ctx, rs1, tu.TasksCfg1, nil))
	assert.NoError(t, cache.Set(ctx, rs2, tu.TasksCfg2, nil))
...
func TestTaskCfgCacheStorage(t *testing.T) {
	...
	// One entry.
	assert.NoError(t, c.Set(ctx, rs1, tu.TasksCfg1, nil))
	check(rs1)
	...
	assert.NoError(t, repos.Update(ctx))
	assert.NoError(t, c.Set(ctx, rs3, tu.TasksCfg2, nil))
	assert.NoError(t, c.Set(ctx, rs4, tu.TasksCfg2, nil))
