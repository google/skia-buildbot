...
func (b *Blacklist) Update() error {
	...
	rules := map[string]*Rule{}
	q := b.coll.Where("Timestamp", ">=", time.Unix(0, 0))
...
func (b *Blacklist) AddRule(r *Rule, repos repograph.Map) error {
	...
	r.Timestamp = time.Now().UTC().Truncate(firestore.TS_RESOLUTION)
...
func ValidateRule(r *Rule, repos repograph.Map) error {
	...
	if util.TimeIsZero(r.Timestamp) {
		return errors.New("Rules must have a valid timestamp.")
	}
