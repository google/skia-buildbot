...
import (
	...
	"go.skia.org/infra/task_scheduler/go/utils"
...
type JobCreator struct {
	...
	// testWaitGroup keeps track of any goroutines the JobCreator methods
...
func NewJobCreator(ctx context.Context, d db.DB, bl *blacklist.Blacklist, period time.Duration, numCommits int, workdir, host string, repos repograph.Map, isolateClient *isolate.Client, c *http.Client, buildbucketApiUrl, trybotBucket string, projectRepoMapping map[string]string, depotTools string, gerrit gerrit.GerritInterface, taskCfgCache *task_cfg_cache.TaskCfgCache, isolateCache *isolate_cache.Cache, ts oauth2.TokenSource) (*JobCreator, error) {
...
func (s *JobCreator) gatherNewJobs(ctx context.Context, repoUrl string, repo *repograph.Graph) ([]*types.Job, error) {
	...
	newJobs := []*types.Job{}
	if err := utils.RecurseAllBranches(ctx, repoUrl, repo, s.window, func(repoUrl string, r *repograph.Graph, c *repograph.Commit) error {
...
func (s *JobCreator) initCaches(ctx context.Context) error {
	...
	for repoUrl, repo := range s.repos {
		if err := utils.RecurseAllBranches(ctx, repoUrl, repo, s.window, func(_ string, _ *repograph.Graph, c *repograph.Commit) error {
