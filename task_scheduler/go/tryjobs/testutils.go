...
func setup(t *testing.T) (string, *TryJobIntegrator, *mockhttpclient.URLMock) {
	...
	os.Mkdir(repoDir, os.ModePerm)
	testutils.Run(t, repoDir, "git", "init")
	testutils.Run(t, repoDir, "git", "remote", "add", "origin", ".")
	os.MkdirAll(path.Join(repoDir, "infra", "bots"), os.ModePerm)
...
func Params(t *testing.T, builder, project, revision, server, issue, patchset string) buildbucket.BuildBucketParameters {
	p := buildbucket.BuildBucketParameters{
		BuilderName: builder,
		Properties: buildbucket.BuildBucketProperties{
	...
	if server == rietveldUrl {
		...
		p.Properties.RietveldIssue = int64(issueInt)
		p.Properties.RietveldPatchset = int64(patchsetInt)
