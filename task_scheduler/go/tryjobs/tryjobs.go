...
// getRunningBuildIds obtains the list of Buildbucket build IDs for in-progresss
// try jobs.
func (t *TryJobIntegrator) getRunningBuildIds() ([]int64, error) {
	defer metrics2.FuncTimer().Stop()

	resp, err := t.bb.Search().Bucket(t.bucket).Status("STARTED").Do()
	if err != nil {
		return nil, err
	}
	if resp.Error != nil {
		return nil, fmt.Errorf("Search call returned error. %s: %s", resp.Error.Reason, resp.Error.Message)
	}
	buildIds := make([]int64, 0, len(resp.Builds))
