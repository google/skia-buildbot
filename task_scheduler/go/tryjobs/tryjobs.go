...
const (
	// API URLs
	API_URL_PROD    = "https://cr-buildbucket.appspot.com/_ah/api/buildbucket/v1/"
	API_URL_TESTING = "http://localhost:8008/_ah/api/buildbucket/v1/"
...
func (t *TryJobIntegrator) sendHeartbeats(now time.Time) error {
	...
	send := func(jobs []*db.Job) {
		...
		for _, result := range resp.Results {
			...
			if result.Error != nil {
				if j == nil {
					errs = append(errs, fmt.Errorf("Error sending heartbeat for job; no valid ID returned from buildbucket so can't cancel the job."))
				} else {
					// Cancel the job.
	...
	for i, j := 0, LEASE_BATCH_SIZE; i < len(jobs); i, j = j, j+LEASE_BATCH_SIZE {
		if j > len(jobs) {
			j = len(jobs)
		}
		if j > i {
