...
func (t *TryJobIntegrator) Start() {
	util.Repeat(HEARTBEAT_INTERVAL, t.stopHeartbeats, func() {
		if err := t.sendHeartbeats(time.Now()); err != nil {
			glog.Error(err)
		}
	})
	util.Repeat(POLL_INTERVAL, t.stopPolling, func() {
...
// Stop halts the TryJobIntegrator's heartbeat and polling loops.
func (t *TryJobIntegrator) Stop() {
	t.stopHeartbeats <- true
	t.stopPolling <- true
}
