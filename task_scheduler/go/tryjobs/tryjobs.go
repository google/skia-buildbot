...
func (t *TryJobIntegrator) insertNewJob(ctx context.Context, buildId int64) error {
	...
	if build.Input.GerritChanges == nil || len(build.Input.GerritChanges) != 1 {
		return t.remoteCancelBuild(buildId, fmt.Sprintf("Invalid Build %d: input is missing GerritChanges: %+v", buildId, build.Input))
	}
	...
	rs := types.RepoState{
		Patch: types.Patch{
			Server:    fmt.Sprintf("https://%s", gerritChange.Host),
