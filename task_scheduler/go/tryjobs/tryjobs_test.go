...
func TestGetRepo(t *testing.T) {
	...
	// Test basic.
	url, r, patchRepo, err := trybots.getRepo(patchProject)
	assert.NoError(t, err)
	...
	// Bogus repo.
	_, _, _, err = trybots.getRepo("bogus")
...
func TestGetRevision(t *testing.T) {
	...
	_, r, _, err := trybots.getRepo(patchProject)
	assert.NoError(t, err)
	c := r.Get("master").Hash
...
func TestInsertNewJob(t *testing.T) {
	...
	MockCancelBuild(mock, b2.Id, fmt.Sprintf("Invalid Build %d: input: ", b2.Id), nil)
