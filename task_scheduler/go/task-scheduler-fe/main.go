...
func jsonJobHandler(w http.ResponseWriter, r *http.Request) {
	...
	for taskName := range job.Dependencies {
		...
		if !ok {
			httputils.ReportError(w, fmt.Errorf("Job %s (%s) points to unknown task %q at repo state: %+v", job.Id, job.Name, taskName, job.RepoState), "Failed to retrieve Tasks cfg", http.StatusInternalServerError)
			return
		}
