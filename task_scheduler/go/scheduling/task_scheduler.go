...
const (
	...
	// SCHEDULING_LIMIT_PER_TASK_SPEC is the maximum number of tasks we'll
	// schedule in a given round, per TaskSpec. This is to avoid errors due
	// to DB transaction size limits.
...
func getCandidatesToSchedule(bots []*swarming_api.SwarmingRpcsBotInfo, tasks []*taskCandidate) []*taskCandidate {
	...
	for _, c := range tasks {
		// To avoid errors resulting from DB transaction size limits,
		// we restrict the number of tasks triggered per TaskSpec (we
		// insert tasks into the DB in chunks by TaskSpec) to half of
		// the DB transaction size limit (since we may need to update
		// an existing whose blamelist was split by the new task).
