...
func NewTaskScheduler(d db.DB, period time.Duration, numCommits int, workdir string, repos repograph.Map, isolateClient *isolate.Client, swarmingClient swarming.ApiClient, c *http.Client, timeDecayAmt24Hr float64, buildbucketApiUrl, trybotBucket string, projectRepoMapping map[string]string) (*TaskScheduler, error) {
	...
	tryjobs, err := tryjobs.NewTryJobIntegrator(buildbucketApiUrl, trybotBucket, c, d, w, projectRepoMapping, repos, taskCfgCache)
	if err != nil {
		return nil, err
	}
...
func (s *TaskScheduler) AddTasks(taskMap map[string]map[string][]*db.Task) error {
	...
	var lastConcurrentUpdateErr error
	...
	if len(queue) > 0 {
		return fmt.Errorf("AddTasks: %d consecutive ErrConcurrentUpdate; most recent: %s", db.NUM_RETRIES, lastConcurrentUpdateErr)
	}
	return nil
}
