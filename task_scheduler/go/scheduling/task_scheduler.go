...
func writeMainLoopDiagnosticsToGCS(ctx context.Context, start time.Time, end time.Time, gcsClient gcs.GCSClient, allCandidates map[types.TaskKey]*taskCandidate, freeBots []*swarming_api.SwarmingRpcsBotInfo, scheduleErr error) error {
	...
	defer cancel()
	return gcs.WithWriteFileGzip(gcsClient, ctx, path, func(w io.Writer) error {
...
func (s *TaskScheduler) MainLoop(ctx context.Context) error {
	...
	if s.gcsClient != nil {
		...
		go func() {
			...
			util.LogErr(writeMainLoopDiagnosticsToGCS(ctx, start, end, s.gcsClient, allCandidates, bots, err))
		}()
	}
