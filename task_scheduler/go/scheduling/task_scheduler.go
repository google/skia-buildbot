...
func (s *TaskScheduler) findTaskCandidates(commitsByRepo map[string][]string) (map[string][]*taskCandidate, error) {
	...
	for taskKey, spec := range specs {
		if rule := s.bl.MatchRule(taskKey.Name, taskKey.Revision); rule != "" {
			glog.Warningf("Skipping blacklisted task candidate: %s @ %s due to rule %q", taskKey.Name, taskKey.Revision, rule)
			continue
		}
		c := &taskCandidate{
			IsolatedHashes: nil,
			TaskKey:        taskKey.Copy(),
			Score:          0.0,
			TaskSpec:       spec,
		}
