...
func NewTaskScheduler(ctx context.Context, d db.DB, bl *blacklist.Blacklist, period time.Duration, numCommits int, workdir, host string, repos repograph.Map, isolateClient *isolate.Client, swarmingClient swarming.ApiClient, c *http.Client, timeDecayAmt24Hr float64, buildbucketApiUrl, trybotBucket string, projectRepoMapping map[string]string, pools []string, pubsubTopic, depotTools string, gerrit gerrit.GerritInterface, btProject, btInstance string, ts oauth2.TokenSource, diagClient gcs.GCSClient) (*TaskScheduler, error) {
...
func writeMainLoopDiagnosticsToGCS(ctx context.Context, start time.Time, end time.Time, diagClient gcs.GCSClient, allCandidates map[types.TaskKey]*taskCandidate, freeBots []*swarming_api.SwarmingRpcsBotInfo, scheduleErr error) error {
	defer metrics2.FuncTimer().Stop()
...
func (s *TaskScheduler) MainLoop(ctx context.Context) error {
	...
	if s.diagClient != nil {
		...
		go func() {
			...
			util.LogErr(writeMainLoopDiagnosticsToGCS(ctx, start, end, s.diagClient, allCandidates, bots, err))
		}()
	}
