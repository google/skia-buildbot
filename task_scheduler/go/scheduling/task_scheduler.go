...
func (s *TaskScheduler) TriggerPeriodicJobs(ctx context.Context, triggerName string) error {
	// TODO(borenet): The Kubernetes Cron jobs which trigger this function
	// need to be idempotent, as they may be triggered more than once. How
	// do we prevent double-triggering these jobs? On a related note, how do
	// we create metrics for job triggering? We'd need the metrics to
	// persist between server restarts (ie. longer than k8s pod lifespan).
	if !util.In(triggerName, specs.PERIODIC_TRIGGERS) {
		return fmt.Errorf("Invalid periodic trigger %q", triggerName)
	}
	jobs := []*types.Job{}
