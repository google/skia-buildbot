	...
	for repoUrl, r := range s.repos {
		if err := r.RecurseAllBranches(func(c *gitrepo.Commit) (bool, error) {
			...
			for name, jobSpec := range cfg.Jobs {
				deps, err := jobSpec.GetTaskSpecDAG(cfg)
				if err != nil {
					return false, err
				}
...
func (s *TaskScheduler) updateUnfinishedJobs() error {
	...
	for _, j := range jobs {
		...
		minis := make(map[string][]*db.MiniTask, len(tasks))
		for k, v := range tasks {
			cpy := make([]*db.MiniTask, 0, len(v))
			for _, t := range v {
				cpy = append(cpy, t.MakeMiniTask())
			}
			minis[k] = cpy
		}
		changed := !reflect.DeepEqual(minis, j.Tasks)
		j.Tasks = minis
		status := j.DeriveStatus()
		changed = changed || (status != j.Status)
		if changed {
			j.Status = status
			if j.Done() {
				j.Finished = time.Now()
