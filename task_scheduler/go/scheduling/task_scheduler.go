...
const (
	...
	GCS_MAIN_LOOP_DIAGNOSTICS_DIR = "diagnostics/MainLoop"
...
func writeMainLoopDiagnosticsToGCS(ctx context.Context, start time.Time, end time.Time, gcsClient gcs.GCSClient, allCandidates map[types.TaskKey]*taskCandidate, freeBots []*swarming_api.SwarmingRpcsBotInfo, scheduleErr error) error {
	defer metrics2.FuncTimer().Stop()
	...
	path := path.Join(GCS_MAIN_LOOP_DIAGNOSTICS_DIR, filenameBase+".json")
	ctx, cancel := context.WithTimeout(ctx, GCS_DIAGNOSTICS_WRITE_TIMEOUT)
	defer cancel()
	return gcs.WithGzipWriter(gcsClient, ctx, path, func(w io.Writer) error {
...
func (s *TaskScheduler) MainLoop(ctx context.Context) error {
	...
	scheduleErr := s.scheduleTasks(ctx, bots, queue)

	// scheduleErr can indicate a partial error; write diagnostics in either case.
	if s.gcsClient != nil {
		end := time.Now()
		s.testWaitGroup.Add(1)
		go func() {
			defer s.testWaitGroup.Done()
			util.LogErr(writeMainLoopDiagnosticsToGCS(ctx, start, end, s.gcsClient, allCandidates, bots, scheduleErr))
		}()
	}
