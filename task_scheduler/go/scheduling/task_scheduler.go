...
func (s *TaskScheduler) findTaskCandidatesForJobs(unfinishedJobs []*db.Job) (map[db.TaskKey]*taskCandidate, error) {
	...
	gatherDeps = func(j *db.Job, tsName string) error {
		...
		c := &taskCandidate{
			TaskKey:  j.MakeTaskKey(tsName),
			TaskSpec: spec,
		}
		candidates[c.TaskKey] = c
...
func (s *TaskScheduler) filterTaskCandidates(candidates map[db.TaskKey]*taskCandidate) (map[string]map[string][]*taskCandidate, error) {
	defer timer.New("TaskScheduler.filterTaskCandidates").Stop()

	bySpec := map[string]map[string][]*taskCandidate{}
	total := 0
	...
	for _, c := range candidates {
		...
		candidates, ok := bySpec[c.Repo]
		if !ok {
			candidates = map[string][]*taskCandidate{}
			bySpec[c.Repo] = candidates
		}
...
func (s *TaskScheduler) processTaskCandidate(c *taskCandidate, now time.Time, cache *cacheWrapper, commitsBuf []*gitrepo.Commit) error {
	if c.IsTryJob() {
		c.Score = CANDIDATE_SCORE_TRY_JOB
		return nil
	}
	...
	if c.IsForceRun() {
		c.Score = CANDIDATE_SCORE_FORCE_RUN
		return nil
	}
...
func (s *TaskScheduler) gatherNewJobs() error {
	...
	// TODO(borenet): Obtain new Jobs from BuildBucket for trybots.

	// TODO(borenet): Add new Jobs for manual triggers.
...
func (s *TaskScheduler) MainLoop() error {
	...
	// Update the caches again to include the newly-inserted tasks.
	if err := s.tCache.Update(); err != nil {
		return err
	}
	if err := s.jCache.Update(); err != nil {
