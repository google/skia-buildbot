...
func main() {
  ...
  'variables': {
	...
	s, err := scheduling.NewTaskScheduler(d, time.Duration(math.MaxInt64), workdir, []string{"skia.git"}, isolateClient, swarmingClient, http.DefaultClient, 0.9, trybots.BUCKET_TESTING)
	assertNoError(err)
