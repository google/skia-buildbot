...
func TestFindTaskCandidatesForJobs(t *testing.T) {
	...
	test := func(jobs []*types.Job, expect map[types.TaskKey]*taskCandidate) {
		...
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, actual, expect)
	}
...
func TestComputeBlamelist(t *testing.T) {
	...
	test := func(tc *testCase) {
		...
		deepequal_testutils.AssertDeepEqual(t, tc.Expected, commits)
...
func TestGetCandidatesToSchedule(t *testing.T) {
	...
	deepequal_testutils.AssertDeepEqual(t, []*taskCandidate{t1}, rv)
	...
	deepequal_testutils.AssertDeepEqual(t, []*taskCandidate{t2}, rv)
	checkDiags([]*swarming_api.SwarmingRpcsBotInfo{}, []*taskCandidate{t1})
	...
	deepequal_testutils.AssertDeepEqual(t, []*taskCandidate{t2, t1}, rv)
	require.Equal(t, []string{b1.BotId, b2.BotId}, t1.Diagnostics.Scheduling.MatchingBots)
	...
	deepequal_testutils.AssertDeepEqual(t, []*taskCandidate{t1, t2}, rv)
...
func TestSchedulingE2E(t *testing.T) {
	...
	deepequal_testutils.AssertDeepEqual(t, expect, tasks)
...
func TestSchedulerStealingFrom(t *testing.T) {
	...
	deepequal_testutils.AssertDeepEqual(t, expect, task.Commits)
	...
	for i := 0; i < 9; i++ {
		...
		deepequal_testutils.AssertDeepEqual(t, old, new.Union(updatedOld))
...
func TestMultipleCandidatesBackfillingEachOther(t *testing.T) {
	...
	deepequal_testutils.AssertDeepEqual(t, expect1, t1.Commits)
	deepequal_testutils.AssertDeepEqual(t, expect2, t2.Commits)
	deepequal_testutils.AssertDeepEqual(t, expect3, t3.Commits)
...
func TestGetTasksForJob(t *testing.T) {
	...
	for _, j := range jobs {
		tasksByName, err := s.getTasksForJob(j)
		require.NoError(t, err)
		deepequal_testutils.AssertDeepEqual(t, expect[j.Id], tasksByName)
	}
...
func TestUpdateUnfinishedTasks(t *testing.T) {
	...
	require.NoError(t, err)
	deepequal_testutils.AssertDeepEqual(t, []*swarming_api.SwarmingRpcsTaskRequestMetadata{m1, m2}, got)
	...
	for _, task := range tasks {
		...
		deepequal_testutils.AssertDeepEqual(t, task, got)
	}
}
...
func TestAddTasksFailure(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		for _, task := range modTasks {
			...
			deepequal_testutils.AssertDeepEqual(t, toil2, task)
