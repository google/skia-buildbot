...
func setup(t *testing.T) (context.Context, *git_testutils.GitBuilder, db.DB, *swarming_testutils.TestClient, *TaskScheduler, *mockhttpclient.URLMock, func()) {
	...
	assert.NoError(t, err)
	s, err := NewTaskScheduler(ctx, d, time.Duration(math.MaxInt64), 0, tmp, "fake.server", repos, isolateClient, swarmingClient, urlMock.Client(), 1.0, tryjobs.API_URL_TESTING, tryjobs.BUCKET_TESTING, projectRepoMapping, swarming.POOLS_PUBLIC, "", depotTools, g, "test-project", "test-instance", nil)
	assert.NoError(t, err)
...
func TestComputeBlamelist(t *testing.T) {
	...
	depotTools := depot_tools_testutils.GetDepotTools(t, ctx)
	tcc, err := specs.NewTaskCfgCache(ctx, repos, depotTools, tmp, 1, "test-project", "test-instance", nil)
	assert.NoError(t, err)
...
func testMultipleCandidatesBackfillingEachOtherSetup(t *testing.T) (context.Context, *git_testutils.GitBuilder, db.DB, *TaskScheduler, *swarming_testutils.TestClient, []string, func(*types.Task), func()) {
  ...
  'variables': {
	...
	s, err := NewTaskScheduler(ctx, d, time.Duration(math.MaxInt64), 0, workdir, "fake.server", repos, isolateClient, swarmingClient, mockhttpclient.NewURLMock().Client(), 1.0, tryjobs.API_URL_TESTING, tryjobs.BUCKET_TESTING, projectRepoMapping, swarming.POOLS_PUBLIC, "", depotTools, g, "test-project", "test-instance", nil)
	assert.NoError(t, err)
...
func TestGetTasksForJob(t *testing.T) {
	...
	assert.NoError(t, err)
	expect[j3.Id][specs_testutils.BuildTask] = []*types.Task{t1, t2}
	expect[j4.Id][specs_testutils.BuildTask] = []*types.Task{t1, t2}
	expect[j5.Id][specs_testutils.BuildTask] = []*types.Task{t1, t2}
	for _, j := range jobs {
		tasksByName, err := s.getTasksForJob(j)
		assert.NoError(t, err)
		deepequal.AssertDeepEqual(t, expect[j.Id], tasksByName)
	...
	expect[j4.Id][specs_testutils.TestTask] = []*types.Task{t4}
	expect[j5.Id][specs_testutils.PerfTask] = []*types.Task{t5}
	for _, j := range jobs {
		tasksByName, err := s.getTasksForJob(j)
		assert.NoError(t, err)
		deepequal.AssertDeepEqual(t, expect[j.Id], tasksByName)
