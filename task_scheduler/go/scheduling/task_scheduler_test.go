...
func TestProcessTaskCandidate(t *testing.T) {
	...
	assert.NoError(t, s.processTaskCandidate(c, now, cache, commitsBuf, newTasks))
	...
	var err error
	newTasks, err = s.taskCfgCache.GetAddedTaskSpecsForRepoStates([]db.RepoState{c.RepoState})
	assert.NoError(t, err)
	assert.NoError(t, s.processTaskCandidate(c, now, cache, commitsBuf, newTasks))
