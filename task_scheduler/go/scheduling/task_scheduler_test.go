...
func getC1(t *testing.T, gb *git_testutils.GitBuilder) string {
	return getCommit(t, gb, "HEAD^")
}

func getC2(t *testing.T, gb *git_testutils.GitBuilder) string {
	return getCommit(t, gb, "HEAD")
}
...
func TestGatherNewJobs(t *testing.T) {
	...
	makeDummyCommits(gb, 5, branchName)
	gb.CheckoutBranch("master")
	makeDummyCommits(gb, 5, "master")
	assert.NoError(t, s.updateRepos())
...
func makeDummyCommits(gb *git_testutils.GitBuilder, numCommits int, branch string) {
	gb.CheckoutBranch(branch)
...
func testMultipleCandidatesBackfillingEachOtherSetup(t *testing.T) (*git_testutils.GitBuilder, db.DB, *TaskScheduler, *swarming.TestClient, []string, func(*db.Task), func()) {
  ...
  'variables': {
	...
	assert.NoError(t, err)
	gb.Add(specs.TASKS_CFG_FILE, string(cfgBytes))
	gb.Commit()
	...
	assert.NoError(t, err)
	swarmingClient := swarming.NewTestClient()
	repo, err := repograph.NewGraph(gb.RepoUrl(), workdir)
	assert.NoError(t, err)
	repos := repograph.Map{
		gb.RepoUrl(): repo,
	}
...
func TestTrybots(t *testing.T) {
	...
	issue := "10001"
	patchset := "20002"
	gb.UploadFakeGerritCL(issue, patchset)
