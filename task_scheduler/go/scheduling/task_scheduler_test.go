...
func setup(t *testing.T) (context.Context, *git_testutils.GitBuilder, db.DB, *swarming_testutils.TestClient, *TaskScheduler, *mockhttpclient.URLMock, func()) {
	...
	s, err := NewTaskScheduler(ctx, d, nil, time.Duration(math.MaxInt64), 0, tmp, "fake.server", repos, isolateClient, swarmingClient, urlMock.Client(), 1.0, tryjobs.API_URL_TESTING, tryjobs.BUCKET_TESTING, projectRepoMapping, swarming.POOLS_PUBLIC, "", depotTools, g, btProject, btInstance, nil, gcs.NewMemoryGCSClient("diag_unit_tests"))
	assert.NoError(t, err)
...
func lastDiagnostics(t *testing.T, s *TaskScheduler) taskSchedulerMainLoopDiagnostics {
	...
	lastname := ""
	assert.NoError(t, s.diagClient.AllFilesInDirectory(ctx, GCS_MAIN_LOOP_DIAGNOSTICS_DIR, func(item *storage.ObjectAttrs) {
