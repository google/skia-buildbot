...
func setup(t *testing.T) (*util.TempRepo, db.DB, *gitinfo.RepoMap, *gitinfo.GitInfo, *swarming.TestClient, *TaskScheduler, *mockhttpclient.URLMock) {
	...
	s, err := NewTaskScheduler(d, time.Duration(math.MaxInt64), tr.Dir, []string{repoName}, isolateClient, swarmingClient, urlMock.Client(), 1.0, trybots.API_URL_TESTING, trybots.BUCKET_TESTING, projectRepoMapping)
	assert.NoError(t, err)
...
func TestMultipleCandidatesBackfillingEachOther(t *testing.T) {
  ...
  'variables': {
	...
	isolateClient.ServerUrl = isolate.FAKE_SERVER_URL
	swarmingClient := swarming.NewTestClient()
	s, err := NewTaskScheduler(d, time.Duration(math.MaxInt64), workdir, []string{repoName}, isolateClient, swarmingClient, mockhttpclient.NewURLMock().Client(), 1.0, trybots.API_URL_TESTING, trybots.BUCKET_TESTING, projectRepoMapping)
	assert.NoError(t, err)
...
func TestTrybots(t *testing.T) {
	...
	b.ParametersJson = testutils.MarshalJson(t, trybots.Params(t, testTask, "skia", rs.Revision, rs.Server, rs.Issue, rs.Patchset))
	trybots.MockPeek(mock, []*buildbucket_api.ApiBuildMessage{b}, now, "", "", nil)
	trybots.MockTryLeaseBuild(mock, b.Id, now, nil)
	trybots.MockJobStarted(mock, b.Id, now, nil)
	assert.NoError(t, s.trybots.Poll(now))
	...
	trybots.MockJobSuccess(mock, tryJob, now, nil)
