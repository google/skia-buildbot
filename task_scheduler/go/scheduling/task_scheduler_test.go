...
func setup(t *testing.T) (context.Context, *mem_git.MemGit, *memory.InMemoryDB, *swarming_testutils.TestClient, *TaskScheduler, *mockhttpclient.URLMock, func()) {
	...
	s, err := NewTaskScheduler(ctx, d, nil, time.Duration(math.MaxInt64), 0, repos, isolateClient, swarmingClient, urlMock.Client(), 1.0, swarming.POOLS_PUBLIC, "", taskCfgCache, isolateCache, nil, test_gcsclient.NewMemoryClient("diag_unit_tests"), btInstance)
	require.NoError(t, err)
...
func TestFindTaskCandidatesForJobs(t *testing.T) {
	...
	require.NoError(t, err)
	assertdeep.Equal(t, tcc_testutils.TasksCfg1, cfg1) // Sanity check.
	require.NoError(t, s.taskCfgCache.Set(ctx, rs2, tcc_testutils.TasksCfg2, nil))
	cfg2, err := s.taskCfgCache.Get(ctx, rs2)
	require.NoError(t, err)
	assertdeep.Equal(t, tcc_testutils.TasksCfg2, cfg2) // Sanity check.
...
func testMultipleCandidatesBackfillingEachOtherSetup(t *testing.T) (context.Context, gitstore.GitStore, db.DB, *TaskScheduler, *swarming_testutils.TestClient, []string, func(*types.Task), func()) {
	...
	s, err := NewTaskScheduler(ctx, d, nil, time.Duration(math.MaxInt64), 0, repos, isolateClient, swarmingClient, mockhttpclient.NewURLMock().Client(), 1.0, swarming.POOLS_PUBLIC, "", taskCfgCache, isolateCache, nil, test_gcsclient.NewMemoryClient("diag_unit_tests"), btInstance)
	require.NoError(t, err)
