...
// given set of RepoStates, keyed by TaskKey.
func (c *taskCfgCache) GetTaskSpecsForRepoStates(rs []db.RepoState) (map[db.TaskKey]*TaskSpec, error) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	rv := map[db.TaskKey]*TaskSpec{}
	for _, s := range rs {
		...
		if err != nil {
			...
			glog.Errorf("Failed to obtain tasks cfg at %s: %s", s.Revision, err)
			continue
		}
