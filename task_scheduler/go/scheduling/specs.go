...
// Lock locks the cache to be read/written by the current user.
func (c *taskCfgCache) Lock() {
	c.mtx.Lock()
}

// Unlock unlocks the cache.
func (c *taskCfgCache) Unlock() {
	c.mtx.Unlock()
}
...
func (c *taskCfgCache) GetTaskSpec_Locked(rs db.RepoState, name string) (*TaskSpec, error) {
