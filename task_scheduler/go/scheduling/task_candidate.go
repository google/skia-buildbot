...
func (c *taskCandidate) allDepsMet(cache cache.TaskCache) (bool, map[string]string, error) {
	...
	if len(missingDeps) > 0 {
		c.DiagnosticsOrDummy().Filtering = &taskCandidateFilteringDiagnostics{
...
func (s *taskCandidate) DiagnosticsOrDummy() *taskCandidateDiagnostics {
	if s.Diagnostics != nil {
		return s.Diagnostics
	}
	return &taskCandidateDiagnostics{}
}
...
type taskCandidateSchedulingDiagnostics struct {
	// True if SCHEDULING_LIMIT_PER_TASK_SPEC was reached for this task spec.
