...
func mainHandler(w http.ResponseWriter, r *http.Request) {
	...
	if cl != "" {
		...
		if err != nil {
			httputils.ReportError(w, fmt.Errorf("Not a valid integer id for an issue."), "The CL given is not valid.")
			return
		}
		d, err = docset.NewDocSetForIssue(context.Background(), *workDir, *docRepo, issue)
		if err == docset.IssueCommittedErr {
			httputils.ReportError(w, err, "Failed to load the given CL, that issue is closed.")
			return
		}
		if err != nil {
			httputils.ReportError(w, err, "Failed to load the given CL.")
			return
		}
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to load file")
		return
	}
	if raw {
