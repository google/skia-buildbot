...
import (
	...
	"go.skia.org/infra/coverage/go/ingester"
...
var (
	coverageIngester ingester.CoverageIngester = nil
...
func main() {
	...
	r.HandleFunc("/ingested", ingestedHander)
...
func ingestedHander(w http.ResponseWriter, r *http.Request) {
	if coverageIngester == nil {
		http.Error(w, "Server not ready yet", http.StatusServiceUnavailable)
		return
	}
	type list struct {
		List []ingester.IngestedResults `json:"list"`
	}
...
func setupFileIngestion() error {
	...
	coverageIngester = ingester.New(*extractDir, gcsClient)

	cycle := func(v vcsinfo.VCS, coverageIngester ingester.CoverageIngester) {
		sklog.Info("Begin coverage ingest cycle")
		if err := v.Update(false, false); err != nil {
	...
	go func(v vcsinfo.VCS, coverageIngester ingester.CoverageIngester) {
