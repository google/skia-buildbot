...
func mockProcessingSteps() *db.MockCoverageCache {
	...
	calculateCoverage = func(outputPath string, folders ...string) (common.CoverageSummary, error) {
...
func TestCallToCombine(t *testing.T) {
	...
	calculateCoverage = func(outputPath string, folders ...string) (common.CoverageSummary, error) {
		...
		if len(folders) == 1 && folders[0] == someFolder {
			called *= 3
			assert.Equal(t, "", outputPath, "Expecting outputPath for anything that's not the combined coverage")
			...
			called *= 7
			// TODO(kjlubick): Make this the correct path.
			assert.Equal(t, "", outputPath, "Change me")
		} else {
...
func TestCoverageDataParsingLLVM5(t *testing.T) {
	...
	actualHTML, err := parsed.ToHTMLPage(TemplateData{
		FileName: "test.cpp",
		Commit:   "adbde2143",
...
func TestCalculateTotalCoverageOutputHTML(t *testing.T) {
	testutils.MediumTest(t)
	tpath, _ := testutils.TempDir(t)
	//defer cleanup()

	calculateTotalCoverageSetup(t, tpath)

	combinedOut := path.Join(tpath, "Combined")
	_, err := defaultCalculateTotalCoverage(combinedOut, path.Join(tpath, "Some-Config", "coverage"), path.Join(tpath, "Other-Config", "coverage"))
	assert.NoError(t, err)

	assert.Equal(t, testutils.MustReadFile("combined.main.cpp.html"),
		read(t, path.Join(combinedOut, "coverage", "foo", "main.cpp.html")))
	assert.Equal(t, testutils.MustReadFile("combined.one-header.h.html"),
		read(t, path.Join(combinedOut, "coverage", "foo", "one-header.h.html")))
	assert.Equal(t, testutils.MustReadFile("combined.two-header.h.html"),
		read(t, path.Join(combinedOut, "coverage", "foo", "two-header.h.html")))
	assert.Equal(t, testutils.MustReadFile("combined.index.html"),
		read(t, path.Join(combinedOut, "index.html")))
