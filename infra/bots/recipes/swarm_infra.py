...
def RunSteps(api):
  ...
  with api.step.context({'cwd': infra_dir.join('res', 'js')}):
    api.step('chrome', ['google-chrome', '--user-data-dir=/tmp/karma-4633399', '--no-default-browser-check', '--no-first-run', '--disable-default-apps', '--disable-popup-blocking', '--disable-translate', 'http://localhost:9876/?id=4633399'], env=env)
