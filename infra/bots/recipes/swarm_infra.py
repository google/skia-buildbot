...
def RunSteps(api):
  ...
  gopath = api.path['start_dir'].join('gopath')
  ...
  with api.context(cwd=infra_dir, env=env):
    ...
    api.step('go install errcheck',
             cmd=['go', 'install', '-v', 'github.com/kisielk/errcheck'])
    api.step('go install goimports',
             cmd=['go', 'install', '-v', 'golang.org/x/tools/cmd/goimports'])
