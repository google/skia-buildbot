...
def RunSteps(api):
  ...
  api.step('echo is_trybot', cmd=['echo', str(is_trybot)])
  api.step('echo  issue', cmd=['echo', str(issue)])
  api.step('echo  patchset', cmd=['echo', str(patchset)])
...
def GenTests(api):
  ...
  yield (
      ...
      api.properties(patch_issue='123456', patch_set='3', patch_ref='a/b/c')
  )
