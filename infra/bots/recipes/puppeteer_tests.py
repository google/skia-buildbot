...
def RunSteps(api):
  ...
  if (api.properties.get('patch_issue', '') and
      ...
      api.properties.get('task_id', '') and
      api.properties['task_id'] != '0'):
    is_trybot = True
  ...
  api.step('echo task_id', cmd=['echo', 'task_id = ' + str(task_id)])
