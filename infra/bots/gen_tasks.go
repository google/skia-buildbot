...
var (
	...
	JOBS = []string{
		...
		"Infra-PerCommit-PushAppsFromSkiaWASMDockerImage",
...
func createPushAppsFromSkiaWASMDockerImage(b *specs.TasksCfgBuilder, name string) string {
	cipd := append([]*specs.CipdPackage{}, specs.CIPD_PKGS_GIT...)
	cipd = append(cipd, b.MustGetCipdPackageFromAsset("go"))
	cipd = append(cipd, b.MustGetCipdPackageFromAsset("protoc"))
	...
	t := &specs.TaskSpec{
		...
		CipdPackages: cipd,
		Command: []string{
			"./push_apps_from_skia_wasm_image",
			"--project_id", "skia-swarming-bots",
			"--task_id", specs.PLACEHOLDER_TASK_ID,
			"--task_name", name,
			"--workdir", ".",
...
func process(b *specs.TasksCfgBuilder, name string) {
	...
	} else if strings.Contains(name, "PushAppsFromSkiaWASMDockerImage") {
		deps = append(deps, createPushAppsFromSkiaWASMDockerImage(b, name))
