...
func goTests(b *specs.TasksCfgBuilder, name string) string {
	...
	env := map[string]string{
		...
		"PATH": "/src/bin:/usr/local/google-cloud-sdk/bin:/go/bin:/usr/local/go/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
	}
	image := filepath.Join("infra", "bots", "task_drivers", "go_tests", "image.sha256")
	task := dockerTask(b, name, env, ".", image, cmd)
