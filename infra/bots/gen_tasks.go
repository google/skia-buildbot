...
func bazelBuild(b *specs.TasksCfgBuilder, name string) string {
	...
	t := &specs.TaskSpec{
		...
		EnvPrefixes: map[string][]string{
			"PATH": {"cipd_bin_packages", "cipd_bin_packages/bin", "bazel", "bazel/bin", "bazel/bazel/bin"},
		},
		ServiceAccount: SERVICE_ACCOUNT_COMPILE,
	}
	b.MustAddTask(name, t)
...
func bazelBuild2(b *specs.TasksCfgBuilder, name string) string {
	cipd := append([]*specs.CipdPackage{}, specs.CIPD_PKGS_GIT_LINUX_AMD64...)
	cipd = append(cipd, b.MustGetCipdPackageFromAsset("bazel"))
	...
	t := &specs.TaskSpec{
		...
		Dimensions:   linuxGceDimensions(MACHINE_TYPE_MEDIUM),
		EnvPrefixes: map[string][]string{
			"PATH": {"cipd_bin_packages", "cipd_bin_packages/bin", "bazel", "bazel/bin"},
		},
		ServiceAccount: SERVICE_ACCOUNT_COMPILE,
	}
	b.MustAddTask(name, t)
...
func bazelBuild3(b *specs.TasksCfgBuilder, name string) string {
	cipd := append([]*specs.CipdPackage{}, specs.CIPD_PKGS_GIT_LINUX_AMD64...)
	cipd = append(cipd, b.MustGetCipdPackageFromAsset("bazel"))
	...
	t := &specs.TaskSpec{
		...
		Dimensions:   linuxGceDimensions(MACHINE_TYPE_MEDIUM),
		EnvPrefixes: map[string][]string{
			"PATH": {"cipd_bin_packages", "cipd_bin_packages/bin", "bazel"},
		},
		ServiceAccount: SERVICE_ACCOUNT_COMPILE,
	}
	b.MustAddTask(name, t)
...
func process(b *specs.TasksCfgBuilder, name string) {
	...
	if strings.Contains(name, "Experimental") {
		...
		deps = append(deps, bazelBuild2(b, name+"2"))
		deps = append(deps, bazelBuild3(b, name+"3"))
	} else {
		// Infra tests.
