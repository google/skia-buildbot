...
func dockerExperiment(b *specs.TasksCfgBuilder, name string) string {
	...
	t := &specs.TaskSpec{
		...
		Command: []string{
			"/bin/bash",
			"./infra/bots/task_drivers/docker-build-and-run/docker-build-and-run.sh",
			fmt.Sprintf("--project_id=%s", "skia-swarming-bots"),
			fmt.Sprintf("--task_id=%s", specs.PLACEHOLDER_TASK_ID),
			fmt.Sprintf("--task_name=%s", name),
			"--build-dir=/SRC/kube/git-checkout",
