...
// kitchenTask returns a specs.TaskSpec instance which uses Kitchen to run a
// recipe.
func helloWorldKitchenTask(name, recipe, isolate, serviceAccount string, dimensions []string, extraProps map[string]string, outputDir string) *specs.TaskSpec {
	cipd := append([]*specs.CipdPackage{}, specs.CIPD_PKGS_KITCHEN...)
	properties := map[string]string{
		"buildername":   name,
		"swarm_out_dir": specs.PLACEHOLDER_ISOLATED_OUTDIR,
...
func infra(b *specs.TasksCfgBuilder, name string) string {
	...
	if strings.Contains(name, "Puppeteer") {
		task = helloWorldKitchenTask(name, "puppeteer_tests", "whole_repo.isolate", SERVICE_ACCOUNT_COMPILE, linuxGceDimensions(machineType), nil, OUTPUT_NONE)
	} else {
