...
func main() {
	...
	*workdir, err = filepath.Abs(*workdir)
	...
	err = td.Do(ctx, td.Props("Set Go Environment").Env(env), func(ctx context.Context) (rvErr error) {
		// Print Go info.
		...
		/*if _, err := td.StartStep(ctx).Props("setup database").RunCwd(path.Join(infraDir, "go", "database"), "./setup_test_db"); err != nil {
			return err
		}*/
