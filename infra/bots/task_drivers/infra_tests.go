...
func main() {
	...
	*workdir, err = filepath.Abs(*workdir)
	...
	if !strings.Contains(*taskName, "Race") {
		...
		if _, err := s.Step().Name("install go deps").Env(env).RunCwd(infraDir, "./scripts/install_go_deps.sh"); err != nil {
			sklog.Fatal(err)
		}
	}
	...
	if _, err := s.Step().Name("Run Unit Tests").Env(env).RunCwd(infraDir, cmd...); err != nil {
		sklog.Fatal(err)
	}
}
