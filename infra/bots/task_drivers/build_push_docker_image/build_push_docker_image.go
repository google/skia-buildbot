...
func main() {
	...
	client, err := pubsub.NewClient(ctx, continuous_deploy.TOPIC_PROJECT_ID, option.WithTokenSource(ts))
	if err != nil {
		td.Fatal(ctx, err)
	}
	...
	// TEMPORARY
	if err := td.Do(ctx, td.Props(fmt.Sprintf("Publish pubsub msg to %s", continuous_deploy.TOPIC)).Infra(), func(ctx context.Context) error {
		// Publish to the pubsub topic.
		b, err := json.Marshal(&continuous_deploy.BuildInfo{
			ImageName: *imageName,
			Tag:       tag,
		})
		if err != nil {
