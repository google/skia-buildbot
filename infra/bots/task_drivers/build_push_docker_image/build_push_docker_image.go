...
const (
	// TODO(rmistry): Keep this somewhere else? in continuous builds v2.
	// TOPIC is the pubsub topic for docker image messages.
	TOPIC            = "skia-docker-builds"
	TOPIC_PROJECT_ID = "skia-public"
)

// TODO(rmistry): Also keep this somewhere else. In continuous builds v2.
...
func main() {
	...
	client, err := pubsub.NewClient(ctx, TOPIC_PROJECT_ID, option.WithTokenSource(ts))
	if err != nil {
		td.Fatal(ctx, err)
	}
	...
	// TEMPORARY
	if err := td.Do(ctx, td.Props(fmt.Sprintf("Publish pubsub msg to %s", TOPIC)).Infra(), func(ctx context.Context) error {
