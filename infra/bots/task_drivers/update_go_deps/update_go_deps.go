...
func main() {
	...
	// Perform steps to update the dependencies.
	{
		...
		// our code.
		out, err := golang.Go(ctx, co.Dir(), "list", "-m", "-f", "{{if not (or .Main .Indirect)}}{{.Path}}{{end}}", "all")
		if err != nil {
			td.Fatal(ctx, err)
		}
		deps := strings.Split(strings.TrimSpace(out), "\n")
		...
		if _, err := golang.Go(ctx, co.Dir(), cmd...); err != nil {
			td.Fatal(ctx, err)
		}
