...
func main() {
	...
	if len(modFiles) > 0 {
		...
		if *local {
			...
			gitcookiesPath = path.Join(wd, ".gitcookies")
			ts, err = auth.NewLUCIContextTokenSource(auth.SCOPE_GERRIT)
		}
		if err != nil {
			td.Fatal(ctx, err)
		}
		...
		ci, err := gerrit.CreateAndEditChange(g, *gerritProject, "master", "Update Go deps", *revision, func(g gerrit.GerritInterface, ci *gerrit.ChangeInfo) error {
			for _, f := range modFiles {
				contents, err := os_steps.ReadFile(ctx, path.Join(co.Dir(), f))
				if err != nil {
					return err
				}
				if err := g.EditFile(ci, f, string(contents)); err != nil {
					return err
				}
			}
			return nil
