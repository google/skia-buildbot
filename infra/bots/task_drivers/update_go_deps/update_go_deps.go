...
func main() {
	...
	if len(modFiles) > 0 {
		if _, err := co.Git(ctx, "config", "--local", "user.name", *serviceAccount); err != nil {
			td.Fatal(ctx, err)
		}
		if _, err := co.Git(ctx, "config", "--local", "user.email", *serviceAccount); err != nil {
			td.Fatal(ctx, err)
		}
		if _, err := co.Git(ctx, "config", "--list"); err != nil {
			td.Fatal(ctx, err)
		}
		...
		depotTools, err := checkout.EnsureGitCheckout(ctx, path.Join(wd, "depot_tools"), types.RepoState{
