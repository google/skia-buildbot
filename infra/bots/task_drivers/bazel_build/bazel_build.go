...
func main() {
	...
	if _, err := exec.RunCwd(ctx, wd, "pwd"); err != nil {
		// td.Fatal(ctx, err)
	}
	...
	if _, err := exec.RunCwd(ctx, wd, "ls", "-l", "bazel/bazel/bin/bazel"); err != nil {
		// td.Fatal(ctx, err)
	}
	...
	if _, err := exec.RunCwd(ctx, wd, "env"); err != nil {
		// td.Fatal(ctx, err)
	}

	if _, err := exec.RunCwd(ctx, repoDir, "which", "bazel"); err != nil {
		// 	td.Fatal(ctx, err)
	}

	if _, err := exec.RunCwd(ctx, repoDir, "whereis", "bazel"); err != nil {
		// 	td.Fatal(ctx, err)
	}
