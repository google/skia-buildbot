...
func main() {
	...
	// DO NOT SUBMIT
	if _, err := exec.RunCwd(ctx, wd, "du", "-h", "--max-depth=2"); err != nil {
		td.Fatal(ctx, err)
	}
	if _, err := exec.RunCwd(ctx, "/mnt/pd0", "du", "-h", "--max-depth=5"); err != nil {
		td.Fatal(ctx, err)
	}
	...
	bazel := func(args ...string) {
		// TODO(lovisolo): Add --config=remote once we figure out how to authenticate against RBE.
