		...
		platformErrGroup.Go(platform, func() error {
			return td.Do(ctx, td.Props(fmt.Sprintf("Build %s", platform)), func(ctx context.Context) error {
				...
				return d.RunInContainer(ctx, props, func(ctx context.Context, c *docker.Container) error {
					if _, err := exec.RunCwd(ctx, ".", "ls", "-alh", filepath.Join(repoAbs, "go_deps")); err != nil {
						return err
					}
					if _, err := c.Run(ctx, dockerRepoPath, env, "ls", "-alh", path.Join(dockerRepoPath, "go_deps")); err != nil {
						return err
					}
					// Build all non-test binaries for this platform.
					if _, err := c.Run(ctx, dockerRepoPath, env, "go", "build", "-v", "--trimpath", "-o", dockerBinTmpPath, "./..."); err != nil {
						return err
					}
