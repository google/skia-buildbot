...
func main() {
	...
	if _, err := exec.RunCwd(ctx, ".", "go", "build", "-o", binTmpPath, "./..."); err != nil {
		td.Fatal(ctx, err)
	}
	...
	copyTestPath := filepath.Join(".", "go_deps", "copy_test.sh")
	...
	output, err := exec.RunCwd(ctx, ".", "go", "test", "-vet=off", "-json", "-exec", execCmd, "./...")
	if err != nil {
		td.Fatal(ctx, err)
	}
