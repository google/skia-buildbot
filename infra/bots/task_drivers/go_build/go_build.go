...
func main() {
	...
	if err := d.RunInContainer(ctx, props, func(ctx context.Context, c *docker.Container) error {
		if _, err := c.Run(ctx, dockerRepoPath, nil, "pwd"); err != nil {
			return err
		}
		if _, err := c.Run(ctx, dockerRepoPath, nil, "ls", "-alh"); err != nil {
			return err
		}
		if _, err := c.Run(ctx, dockerRepoPath, nil, "env"); err != nil {
			return err
		}
		if _, err := c.Run(ctx, dockerRepoPath, nil, "go", "env"); err != nil {
			return err
		}
		...
		for _, platform := range *platforms {
			...
			platformErrGroup.Go(platform, func() error {
				return td.Do(ctx, td.Props(fmt.Sprintf("Build %s", platform)), func(ctx context.Context) error {
					...
					binOutPath := path.Join(platformOutPath, "bin")
					if _, err := c.Run(ctx, dockerRepoPath, env, "mkdir", "-p", binOutPath); err != nil {
						return err
					}
					if _, err := c.Run(ctx, dockerRepoPath, env, "go", "build", "-v", "--trimpath", "-o", binOutPath, "./..."); err != nil {
						return err
					}

					// Determine which tests to build for this platform.
					out, err := c.Run(ctx, dockerRepoPath, env, "go", "list", "-f", "{{if .TestGoFiles}}{{.ImportPath}}{{end}}", "./...")
					if err != nil {
						return err
					}
					...
					if _, err := c.Run(ctx, dockerRepoPath, env, "mkdir", "-p", testOutPath); err != nil {
						return err
					}
					testErrGroup := util.NewNamedErrGroup()
					for _, pkg := range testPackages {
						...
						testErrGroup.Go(pkg, func() error {
							// TODO(borenet): We do want to run "go vet" at some
							// point, but I don't think we want to block compilation
							// on it.
							_, err := c.Run(ctx, dockerRepoPath, env, "go", "test", "-v", "--trimpath", "-vet=off", "-c", "-o", dest, localPath)
							return err
						})
					}
					return testErrGroup.Wait()
				})
			})
		}
		return platformErrGroup.Wait()
	}); err != nil {
		td.Fatal(ctx, err)
	}
}
