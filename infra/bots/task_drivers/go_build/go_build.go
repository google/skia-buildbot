...
func main() {
	...
	if err := d.RunInContainer(ctx, props, func(ctx context.Context, c *docker.Container) error {
		if _, err := c.Run(dockerRepoPath, nil, "pwd"); err != nil {
			return err
		}
		if _, err := c.Run(dockerRepoPath, nil, "ls", "-alh"); err != nil {
			return err
		}
		if _, err := c.Run(dockerRepoPath, nil, "env"); err != nil {
			return err
		}
		if _, err := c.Run(dockerRepoPath, nil, "go", "env"); err != nil {
			return err
		}
		...
		for _, platform := range *platforms {
			...
			g.Go(platform, func() error {
				return td.Do(ctx, td.Props(fmt.Sprintf("Build %s", platform)), func(ctx context.Context) error {
					...
					if _, err := c.Run(dockerRepoPath, env, "mkdir", "-p", binOutPath); err != nil {
						return err
					}
					if _, err := c.Run(dockerRepoPath, env, "go", "build", "-v", "--trimpath", "-o", binOutPath, "./..."); err != nil {
						return err
					}

					// Determine which tests to build for this platform.
					out, err := c.Run(dockerRepoPath, env, "go", "list", "-f", "{{if .TestGoFiles}}{{.ImportPath}}{{end}}", "./...")
					if err != nil {
						return err
					}
					...
					if _, err := c.Run(dockerRepoPath, env, "mkdir", "-p", testOutPath); err != nil {
						return err
					}
					for _, pkg := range testPackages {
						...
						g.Go(pkg, func() error {
							...
							// on it.
							_, err := c.Run(dockerRepoPath, env, "go", "test", "-v", "--trimpath", "-vet=off", "-c", "-o", dest, localPath)
							return err
						})
					}
					return nil
