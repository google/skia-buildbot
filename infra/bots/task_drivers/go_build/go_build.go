		...
		platformErrGroup.Go(platform, func() error {
			return td.Do(ctx, td.Props(fmt.Sprintf("Build %s", platform)), func(ctx context.Context) error {
				return d.RunInContainer(ctx, props, func(ctx context.Context, c *docker.Container) error {
					...
					if _, err := c.Run(ctx, dockerRepoPath, env, "mkdir", "-p", dockerBinOutPath); err != nil {
						return err
					}
					...
					output, err := c.Run(ctx, dockerRepoPath, env, "go", "test", "-json", "--trimpath", "-exec", execCmd, "./...")
					if err != nil {
						return err
					}
					return td.Do(ctx, td.Props("Copy test binaries"), func(context.Context) error {
						...
						for _, line := range lines {
							...
							m := copyTestPathRegex.FindStringSubmatch(line)
							if len(m) == 2 {
						...
						return os_steps.RemoveAll(ctx, dockerTestTmpPath)
					})
				})
			})
		})
