...
func buildPushImage(ctx context.Context, appName, buildCmd, image, tag, repo, configDir, workDir string, topic *pubsub.Topic, additionalEnv map[string]string) error {
	err := td.Do(ctx, td.Props(fmt.Sprintf("Build & Push %s Image", appName)).Infra(), func(ctx context.Context) error {
		...
		if err := docker.Run(ctx, "gcr.io/skia-public/infra:prod", buildCmd, configDir, []string{fmt.Sprintf("%s:/OUT", workDir)}, env); err != nil {
			return err
		}
		...
		if err := docker.Build(ctx, workDir, imageWithTag, configDir, buildArgs); err != nil {
			return err
		}
...
func buildPushFiddlerImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
	// Create temporary dir to use as the workdir for the image.
	...
	return buildPushImage(ctx, "Fiddler", buildCmd, image, tag, repo, configDir, tempDir, topic, nil)
}
...
func buildPushDebuggerImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
	...
	return buildPushImage(ctx, "Debugger", buildCmd, image, tag, repo, configDir, tempDir, topic, nil)
}
...
func buildPushDebuggerAssetsImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
	...
	return buildPushImage(ctx, "Debugger-Assets", buildCmd, image, tag, repo, configDir, tempDir, topic, nil)
}
...
func buildPushApiImage(ctx context.Context, tag, repo, configDir, checkoutDir string, topic *pubsub.Topic) error {
	...
	//}
	//defer os_steps.RemoveAll(ctx, tempDir)
	tempDir := filepath.Join(os.TempDir(), "api")
	// if err := os.Mkdir(filepath.Join(os.TempDir(), "api"), os.FileMode(0511)); err != nil {
	if err := os.Mkdir(filepath.Join(os.TempDir(), "api"), 0777); err != nil {
		return err
	}
	...
	if err := docker.Run(ctx, "gcr.io/skia-public/doxygen:testing-slim", doxygenCmd, configDir, volumes, env); err != nil {
		fmt.Println("ERRRRRRRRROR")
		fmt.Println(err)
		fmt.Println(doxygenCmd)
		fmt.Println(checkoutDir)
		fmt.Println(tempDir)
	...
	return buildPushImage(ctx, "Api", buildCmd, image, tag, repo, configDir, tempDir, topic, additionalEnv)
}
