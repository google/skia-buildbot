...
func buildPushImage(ctx context.Context, appName, buildCmd, imageName, tag, repo, configDir string, topic *pubsub.Topic) error {
	err := td.Do(ctx, td.Props(fmt.Sprintf("Build & Push %s Image", appName)).Infra(), func(context.Context) error {
		...
		image := fmt.Sprintf("gcr.io/skia-public/%s", imageName)
		imageWithTag := fmt.Sprintf("%s:%s", image, tag)
		buildArgs := map[string]string{
			// TODO(rmistry): Change this to skia-release-v2.
			"IMAGE_NAME": "skia-release",
			"TAG":        tag,
		}
		if err := docker.Build(ctx, tempDir, imageWithTag, tag, buildArgs); err != nil {
			return err
		}
...
func buildPushApiImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
	imageName := fmt.Sprintf("gcr.io/skia-public/%s", API_IMAGE_NAME)
	buildCmd := "cd /home/skia/golib/src/go.skia.org/infra/api' && make release_ci"
	return buildPushImage(ctx, "Debugger-Assets", buildCmd, imageName, tag, repo, configDir, topic)
}

func publishToTopic(ctx context.Context, imageName, tag, repo string, topic *pubsub.Topic) error {
