...
func buildPushFiddlerImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
	err := td.Do(ctx, td.Props("Build & Push Fiddler Image").Infra(), func(context.Context) error {
		// Create temporary dir to use for fiddler image.
		fiddlerDir, err := os_steps.TempDir(ctx, "", "")
		if err != nil {
			return err
		}
		defer os_steps.RemoveAll(ctx, fiddlerDir)
		...
		if err := docker.Run(ctx, "gcr.io/skia-public/infra:prod", fmt.Sprintf("%s:/OUT", fiddlerDir), tag, buildCmd, configDir, env); err != nil {
			return err
		}

		// Build the image using docker.
		fiddlerImage := fmt.Sprintf("gcr.io/skia-public/%s", FIDDLER_IMAGE_NAME)
		//     args: ['build', '-t', 'gcr.io/$PROJECT_ID/debugger:$COMMIT_SHA', '/workspace/__debugger_staging']
		if err := docker.Build(ctx, fiddlerDir, fmt.Sprintf("gcr.io/skia-public/%s", FIDDLER_IMAGE_NAME), tag); err != nil {
			return err
		}

		// Push the docker image.
		fiddlerImageWithTag := fmt.Sprintf("%s:%s", fiddlerImage, tag)
		if err := docker.Push(ctx, fiddlerImageWithTag, configDir); err != nil {
			return err
		}

		// Send pubsub msg.
		return publishToTopic(ctx, fiddlerImage, tag, repo, topic)
	})
	return err
}
