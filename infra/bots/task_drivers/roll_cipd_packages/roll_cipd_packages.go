...
func main() {
	...
	c, _, err := auth_steps.InitHttpClient(ctx, *local, auth.ScopeUserinfoEmail, auth.ScopeGerrit)
	if err != nil {
		td.Fatal(ctx, err)
	}
	var cc *cipd.Client
	...
	const baseChangeID = ""
	if err := gerrit_steps.UploadCL(ctx, g, co, *gerritProject, git.MainBranch, rs.Revision, baseChangeID, "Update CIPD Packages", reviewers, isTryJob); err != nil {
		td.Fatal(ctx, err)
	}
}
