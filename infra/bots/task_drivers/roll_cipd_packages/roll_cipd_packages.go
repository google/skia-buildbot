		...
		for _, pkg := range pkgs {
			...
			if err := td.Do(ctx, td.Props(fmt.Sprintf("Find latest %s", name)).Infra(), func(ctx context.Context) error {
				...
				for _, tag := range desc.Tags {
					if err := td.Do(ctx, td.Props(fmt.Sprintf("Tag: %s", tag.Tag)), func(ctx context.Context) error {
						return nil
					}); err != nil {
						return err
					}
					...
					if strings.HasPrefix(tag.Tag, "version:") {
						newVersionTag = tag.Tag
						break
					}
					// Fall back to choosing the most recent
					// tagged commit based on repo+revision.
					if strings.HasPrefix(tag.Tag, "git_repository:") {
						repos = append(repos, strings.TrimPrefix(tag.Tag, "git_repository:"))
					}
					if strings.HasPrefix(tag.Tag, "git_revision:") {
						revs = append(revs, strings.TrimPrefix(tag.Tag, "git_revision:"))
					}
				}
				if newVersionTag == "" {
					...
					if len(commits) > 0 {
						...
						newVersionTag = "git_revision:" + commits[0].Hash
					}
				}
				if newVersionTag == "" {
