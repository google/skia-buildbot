...
func main() {
	...
	// Set up Bazel.
	bzl, err := bazel.New(ctx, gitDir.Dir(), *local, *rbe, *rbeKey)
	if err != nil {
		td.Fatal(ctx, err)
	}
	defer func() {
		if err := bzl.Cleanup(ctx); err != nil {
			td.Fatal(ctx, err)
		}
	}()

	// Print out the Bazel version for debugging purposes.
	if _, err := bzl.Bazel(ctx, "version"); err != nil {
		td.Fatal(ctx, err)
	}
	...
	if _, err := bzl.Bazel(ctx, "run", "//:buildifier"); err != nil {
		td.Fatal(ctx, err)
	}
	failIfNonEmptyGitDiff()

	// Regenerate //go_repositories.bzl from //go.mod with Gazelle, and fail if there are any diffs.
	if _, err := bzl.Bazel(ctx, "run", "//:gazelle", "--", "update-repos", "-from_file=go.mod", "-to_macro=go_repositories.bzl%go_repositories"); err != nil {
		td.Fatal(ctx, err)
	}
	failIfNonEmptyGitDiff()

	// Update all Go BUILD targets with Gazelle, and fail if there are any diffs.
	if _, err := bzl.Bazel(ctx, "run", "//:gazelle", "--", "update", "."); err != nil {
		td.Fatal(ctx, err)
	}
	failIfNonEmptyGitDiff()

	// Build all code in the repository. The tryjob will fail upon any build errors.
	if err := bzl.Build(ctx, "//..."); err != nil {
		td.Fatal(ctx, err)
	}
}
