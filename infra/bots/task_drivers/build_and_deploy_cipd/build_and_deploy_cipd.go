...
func main() {
	...
	// Perform the build(s).
	skiaInfraRbeKeyFile := filepath.Join(".", "skia_infra_rbe_key", "rbe-ci.json")
	if err := td.Do(ctx, td.Props("Build"), func(ctx context.Context) error {
		for _, pkg := range pkgs {
			...
			cmd := []string{
				"bazel", "build",
				fmt.Sprintf("--platforms=%s", pkg.bazelPlatform),
			}
			if *rbe {
				cmd = append(cmd, []string{
					"--config=remote",
					"--google_credentials=" + skiaInfraRbeKeyFile,
				}...)
			}
			...
			for _, path := range *includePaths {
				...
				for _, path := range paths {
					...
					if _, err := os_steps.Stat(ctx, path); err == nil {
						...
						if err := os_steps.Rename(ctx, path, dest); err != nil {
							return err
						}
					}
				}
