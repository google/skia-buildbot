...
func main() {
	...
	if err := td.Do(ctx, td.Props("Create output dirs").Infra(), func(ctx context.Context) error {
	...
	if err := td.Do(ctx, td.Props("Build"), func(ctx context.Context) error {
		for _, pkg := range pkgs {
			if err := td.Do(ctx, td.Props(pkg.cipdPlatform), func(ctx context.Context) error {
				// "bazel clean" first.
				if _, err := exec.RunCwd(ctx, *buildDir, "bazel", "clean"); err != nil {
					return err
				}
				...
				if *rbe {
					...
					if *rbeKey != "" {
						rbeKeyFile, err := filepath.Rel(*buildDir, *rbeKey)
						if err != nil {
							return err
						}
						cmd = append(cmd, "--google_credentials="+rbeKeyFile)
					}
				}
				cmd = append(cmd, fmt.Sprintf("--platforms=%s", pkg.bazelPlatform))
				cmd = append(cmd, *targets...)
				if _, err := exec.RunCwd(ctx, *buildDir, cmd...); err != nil {
					return err
				}
				...
				for _, path := range *includePaths {
					...
					for _, path := range paths {
						...
						if _, err := os_steps.Stat(ctx, path); err == nil {
							...
							if err := os_steps.Copy(ctx, path, dest); err != nil {
								return err
							}
							break
							found = true
