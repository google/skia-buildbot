...
func buildPushCTImage(ctx context.Context, tag, repo, configDir string, topic *pubsub.Topic) error {
...
func main() {
	...
	if err := buildPushCTImage(ctx, tag, rs.Repo, configDir, topic); err != nil {
		td.Fatal(ctx, err)
	}
}
