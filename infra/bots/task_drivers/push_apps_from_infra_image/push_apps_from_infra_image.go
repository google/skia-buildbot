...
const (
	...
	CT_IMAGE_NAME          = "ctfe"
	K8s_CHECKER_IMAGE_NAME = "k8s-checker"
)
...
func buildPushCTImage(ctx context.Context, tag, repo, configDir string, changedFiles []string, topic *pubsub.Topic) error {
	...
	for _, changedFile := range changedFiles {
		if strings.HasPrefix(changedFile, "ct/") {
			foundCTChange = true
			break
		}
	}
	...
	return docker.BuildPushImageFromInfraV2(ctx, "CT", buildCmd, image, tag, repo, configDir, tempDir, topic, volumes, infraCommonEnv, nil)
}
...
func main() {
	...
	// Figure out which tag to use for docker build and push and what the changed files are.
	tag := rs.Revision
	changedFiles := []string{}
	...
	// Build and push the below apps only if commit hashes are specified.
