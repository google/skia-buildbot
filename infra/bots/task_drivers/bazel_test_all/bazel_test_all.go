...
func main() {
	...
	// automatically.
	if *local {
		defer os_steps.RemoveAll(ctx, bazelCacheDir)
	}
...
func testLocally(ctx context.Context) {
	// Initialize a fake Git repository. Some tests require this. We receive the code via Isolate, but
	// it doesn't include the .git dir.
	//
	// If --local is passed, we assume that a developer is running the task driver locally from a Git
	// checkout.
	if !*local {
	...
	// Start the emulators.
	// if _, err := golang.Go(ctx, repoDir, "run", "./scripts/run_emulators", "start"); err != nil {
	...
	// })
	// defer func() {
	// 	if _, err := golang.Go(ctx, repoDir, "run", "./scripts/run_emulators", "stop"); err != nil {
	// 		td.Fatal(ctx, err)
	// 	}
	// }()
