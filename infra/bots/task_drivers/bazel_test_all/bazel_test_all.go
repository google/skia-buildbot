...
func testLocally(ctx context.Context) {
	...
	if !*local {
		...
		// Check out depot_tools and add it to $PATH.
		depotTools, err := git.NewCheckout(ctx, common.REPO_DEPOT_TOOLS, workDir)
		ctx = td.WithEnv(ctx, []string{"PATH=%(PATH)s:" + depotTools.Dir()})
	}
	...
	bazel(ctx, "test", "//autoroll/go/repo_manager:repo_manager_test", "--test_output=all", "--test_arg=-test.v", "--test_arg=-alsologtostderr", "--test_arg=-test.run=TestGithubCipdDEPSRepoManager")
}
