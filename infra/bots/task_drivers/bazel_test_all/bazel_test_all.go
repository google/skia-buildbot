...
func testLocally(ctx context.Context, bzl *bazel.Bazel) (rvErr error) {
	...
	if !*local {
		...
		//
		// One possible fix is to set VPYTHON_VIRTUALENV_ROOT to a location within the Swarming task's
		// directory which is independent of any Go tests, and which will be cleaned up by Swarming
		// after the task ends.
		//
