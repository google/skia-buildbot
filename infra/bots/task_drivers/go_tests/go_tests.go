...
func main() {
	...
	if err := td.Do(ctx, td.Props("Start emulators"), func(ctx context.Context) error {
		...
		log := func(msg string, args ...interface{}) {
			_, _ = cmd.Stdout.Write([]byte(fmt.Sprintf(msg, args...)))
		}
		...
		for {
			output, err := exec.CommandContext(ctx, "netstat", "tulp").CombinedOutput()
			if err != nil {
				return err
			}
			...
			for wf := range waitingFor {
				if strings.Contains(outputStr, wf) {
					log(wf + " is ready")
			...
			if len(waitingFor) == 0 {
				...
				return fmt.Errorf("Emulators failed to start within %s", timeout)
			} else {
