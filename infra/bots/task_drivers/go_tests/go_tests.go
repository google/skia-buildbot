...
func main() {
	...
	contents, err := os_steps.ReadFile(ctx, filepath.Join(testDataDirAbs, "infra", "bots", "task_drivers", "go_tests", ".gitconfig_fake"))
	if err != nil {
		td.Fatal(ctx, err)
	}
	gitconfig := filepath.Join(tmp, ".gitconfig")
	if err := os_steps.WriteFile(ctx, gitconfig, contents, os.ModePerm); err != nil {
		td.Fatal(ctx, err)
	}
	ctx = td.WithEnv(ctx, []string{fmt.Sprintf("HOME=%s", tmp)})
	...
	for i := 0; i < *workers; i++ {
		...
		go func() {
			...
			for t := range testCh {
				...
				// TODO(borenet): This was done to prevent multiple tests from
				// trying to sync depot tools into the same directory at once.
				// It fixes that problem, but having N checkouts of depot tools
				// is wasteful.  Consider performing one sync before starting
				// to run tests.
				testTmp, err := ioutil.TempDir(tmp, "")
				if err != nil {
					td.Fatal(ctx, err)
				}
				testEnv := td.WithEnv(ctx, []string{fmt.Sprintf("TMPDIR=%s", testTmp)})
				t.err = golang.TestExecutable(testEnv, testDataDirAbs, t.path, t.pkgName, testArgs...)
				resultCh <- t
			}
		}()
	}
