...
func main() {
	...
	for i := 0; i < *workers; i++ {
		...
		go func() {
			...
			for t := range testCh {
				...
				fmt.Println(fmt.Sprintf("test %s", t.pkgName))
	...
	go func() {
		...
		findTestsErr = td.Do(ctx, td.Props("Find Tests"), func(ctx context.Context) error {
			fmt.Println("finding tests")
			defer func() {
				close(testCh)
				fmt.Println("done finding tests")
			}()
	...
	go func() {
		...
		fmt.Println("done collecting test results")
		if findTestsErr != nil {
			errs = append(errs, findTestsErr)
		}
		fmt.Println("Emitting final result")
		if len(errs) > 0 {
			result <- errs[0]
		} else {
			result <- nil
