...
func main() {
	...
	runEmulatorsDir := filepath.Join(testDirAbs, "scripts", "run_emulators")
	emulatorVars, err := exec.RunCwd(ctx, ".", filepath.Join(runEmulatorsDir, "run_emulators.sh"))
	if err != nil {
		td.Fatal(ctx, err)
	}
	defer func() {
		if _, err := exec.RunCwd(ctx, ".", filepath.Join(runEmulatorsDir, "stop_emulators.sh")); err != nil {
			td.Fatal(ctx, err)
		}
	}()
