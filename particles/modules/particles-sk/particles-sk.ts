...
export class ParticlesSk extends ElementSk {
  ...
  private static displayDialog = (ele: ParticlesSk) => html`
  <particles-config-sk .state=${ele._state} .width=${ele._width} .height=${ele._height}></particles-config-sk>
  `;

  private static renderParticlesPlayer = (ele: ParticlesSk) => html`
  ...
  private static jsonEditor = (ele: ParticlesSk) => {
    if (!ele._showEditor) {
      return '';
    }
    return html`
  ...
  private static displayLoaded = (ele: ParticlesSk) => html`
  ${ParticlesSk.gallery(ele)}
  ...
  <div class=controls>
    ...
    <button ?hidden=${!ele._hasEdits} @click=${ele.applyEdits}>Apply Edits</button>
    <div class=download>
  ...
  <section class=figures>
    <figure>
      ${ParticlesSk.renderParticlesPlayer(ele)}
    </figure>
  </section>
  
  ${ParticlesSk.jsonEditor(ele)}
  `;

  private static displayLoading = (ele: ParticlesSk) => html`
  <div class=loading>
