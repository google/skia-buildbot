...
export class ParticlesPlayerSk extends ElementSk {
  ...
  private _initializeParticles(particlesJSON: any) {
    ...
    // TODO(jcgregorio) Note that we only support multiple rows and not multiple
    //  columns. Add support for multiple columns since uniforms can be 2x2,
    //  3x3, or 4x4 matrices. Remember that the matrices are column major.
    this.sliders = [];
    ...
    for (let i = 0; i < an.getEffectUniformCount(); i++) {
      ...
      if (name.startsWith('slider_')) {
        ...
        // TODO(jcgregorio) Also do columns, and group them together?
        for (let row = 0; row < uniform.rows; row++) {
          let id = `${name.substring('slider_'.length)}`;
          if (uniform.rows > 1) {
            id += `_${row}`;
