...
type TaskFinishedSlice []*buildbot.Build
...
func main() {
	...
	repos, err := repograph.NewMap([]string{common.REPO_SKIA} /*common.PUBLIC_REPOS*/, wd)
	if err != nil {
		sklog.Fatal(err)
	}

	timeWindow := 7 * 24 * time.Hour
	end := time.Date(2016, time.September, 8, 0, 0, 0, 0, time.UTC)
	start := end.Add(-timeWindow)

	tasks, err := db.GetBuildsFromDateRange(start, end)
	if err != nil {
		sklog.Fatal(err)
	}
	...
	byTaskSpec := make(map[string]map[string][]*buildbot.Build, len(repos))
	for _, t := range tasks {
		// Filter by repo, since the DB might contain tasks from repos
		// we don't care about.
		if _, ok := repos[t.Repository]; !ok {
			continue
		}
		...
		m, ok := byTaskSpec[t.Repository]
		if !ok {
			m = map[string][]*buildbot.Build{}
			byTaskSpec[t.Repository] = m
		}
		m[t.Builder] = append(m[t.Builder], t)
	}
	...
	// for each commit:
	commitStrs := make([]string, 0, len(commits))
	for _, repoCommits := range commits {
		for _, c := range repoCommits {
			commitStrs = append(commitStrs, c.Hash)
		}
	}
	buildsByCommit, err := db.GetBuildsForCommits(commitStrs, nil)
	if err != nil {
		sklog.Fatal(err)
	}
	for repoUrl, c := range commits {
		...
		for _, commit := range c {
			...
			// Find all builds for this commit.
			builds := buildsByCommit[commit.Hash]

			// for each task spec:
			sklog.Infof("  Finding task specs.")
			commitTasks := make([]*buildbot.Build, 0, len(buildsByCommit[commit.Hash]))
			...
			for _, b := range builds {
				...
				var first *buildbot.Build
				for _, task := range byTaskSpec[repoUrl][spec] {
					if task.GotRevision == "" {
						continue
					}
					c := repos[repoUrl].Get(task.GotRevision)
					ha, ok := hasAncestor[task.GotRevision]
					if !ok {
						ha = map[string]bool{}
						hasAncestor[task.GotRevision] = ha
					}
