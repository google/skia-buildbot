...
func TestAccum(t *testing.T) {
	...
	assert.Equal(t, int64(12), a.values["some.duration"]["bot foo task bar"].duration)
	assert.Equal(t, int64(1), a.values["some.duration"]["bot foo task bar"].numDuration)
	assert.Equal(t, "foo", a.values["some.duration"]["bot foo task bar"].tags["bot"])
	a.Add("some.duration", tags, 30)
	assert.Equal(t, int64(42), a.values["some.duration"]["bot foo task bar"].duration)
	assert.Equal(t, int64(2), a.values["some.duration"]["bot foo task bar"].numDuration)
	a.Report()
