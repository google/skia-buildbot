...
func getLastIngestionTs(em *events.EventMetrics, repos repograph.Map) (time.Time, error) {
	timeEnd := time.Now()
	window := time.Hour
	for {
		...
		for repo := range repos {
			s := em.GetEventStream(fmtStream(repo))
			ev, err := s.Range(timeStart, timeEnd)
			if err != nil {
...
func Start(ctx context.Context, taskDb db.TaskReader, repos repograph.Map, tcc *task_cfg_cache.TaskCfgCache, btProject, btInstance string, ts oauth2.TokenSource) error {
	...
	lastFinished, err := getLastIngestionTs(em, repos)
	if err != nil {
