...
func Start(ctx context.Context, taskDb db.TaskReader, workdir, recipesCfgFile, bigtableInstance string, ts oauth2.TokenSource) error {
	// Setup.
	...
	tcc, err := specs.NewTaskCfgCache(ctx, repos, depotTools, path.Join(workdir, "taskCfgCache"), 1, bigtableInstance, ts)
	if err != nil {
