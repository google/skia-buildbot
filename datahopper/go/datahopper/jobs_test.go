...
func assertEvent(t *testing.T, ev *events.Event, j *db.Job) {
	...
	var job db.Job
	assert.NoError(t, gob.NewDecoder(bytes.NewBuffer(ev.Data)).Decode(&job))
...
func TestUpdate(t *testing.T) {
	...
	jobs := []*db.Job{
		...
		makeJob(start.Add(3*time.Minute), "A", db.JOB_STATUS_IN_PROGRESS, NORMAL, time.Hour),
...
func TestRange(t *testing.T) {
	...
	jobs := []*db.Job{
		makeJob(start.Add(-time.Nanosecond), "A", db.JOB_STATUS_SUCCESS, NORMAL, time.Minute),
		makeJob(start, "A", db.JOB_STATUS_SUCCESS, NORMAL, time.Minute),
		makeJob(start.Add(time.Nanosecond), "A", db.JOB_STATUS_SUCCESS, NORMAL, time.Minute),
		makeJob(start.Add(time.Minute), "A", db.JOB_STATUS_SUCCESS, NORMAL, time.Minute),
	}
	assert.NoError(t, jdb.PutJobs(jobs))
	...
	test(before, before, 0, 0)
	test(before, jobs[0].Created, 0, 0)
