...
func makeJob(created time.Time, name string, status types.JobStatus, jobType jobTypeString, duration time.Duration) *types.Job {
	...
	if jobType == jobType_Forced {
		j.IsForce = true
	} else if jobType == jobType_TryJob {
		j.Issue = "1234"
		j.PatchRepo = "nou.git"
		j.Patchset = "1"
		j.Server = "Jeeves"
...
func assertJobEvent(t *testing.T, ev *events.Event, j *types.Job) {
	require.Equal(t, jobStream, ev.Stream)
	var job types.Job
...
func TestJobUpdate(t *testing.T) {
	...
	jobs := []*types.Job{
		...
		makeJob(start.Add(-time.Minute), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
		makeJob(start.Add(time.Minute), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
		makeJob(start.Add(2*time.Minute), "A", types.JOB_STATUS_FAILURE, jobType_Normal, time.Minute),
		// 3: Filtered out -- not Done.
		makeJob(start.Add(3*time.Minute), "A", types.JOB_STATUS_IN_PROGRESS, jobType_Normal, time.Minute),
		makeJob(start.Add(4*time.Minute), "A", types.JOB_STATUS_MISHAP, jobType_Normal, time.Hour),
		makeJob(start.Add(5*time.Minute), "A", types.JOB_STATUS_CANCELED, jobType_Normal, time.Hour),
		makeJob(start.Add(6*time.Minute), "B", types.JOB_STATUS_SUCCESS, jobType_TryJob, time.Minute),
		makeJob(start.Add(7*time.Minute), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Hour),
	}
	require.NoError(t, jdb.PutJobs(context.Background(), jobs))
	<-wait
	require.NoError(t, edb.update())
	evs, err := edb.Range(jobStream, start.Add(-time.Hour), start.Add(time.Hour))
...
func TestJobRange(t *testing.T) {
	...
	jobs := []*types.Job{
		makeJob(base.Add(-time.Nanosecond), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
		makeJob(base, "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
		makeJob(base.Add(time.Nanosecond), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
		makeJob(base.Add(time.Minute), "A", types.JOB_STATUS_SUCCESS, jobType_Normal, time.Minute),
	}
	require.NoError(t, jdb.PutJobs(context.Background(), jobs))
	<-wait
	require.NoError(t, edb.update())
	...
	test := func(start, end time.Time, startIdx, count int) {
		evs, err := edb.Range(jobStream, start, end)
		require.NoError(t, err)
...
func TestComputeAvgJobDuration(t *testing.T) {
	...
	jobsStatus := []*types.Job{
		makeJob(created, "AllStatus", types.JOB_STATUS_SUCCESS, jobType_Normal, 10*time.Minute),
		makeJob(created, "AllStatus", types.JOB_STATUS_SUCCESS, jobType_Normal, 10*time.Minute),
		makeJob(created, "AllStatus", types.JOB_STATUS_FAILURE, jobType_Normal, 13*time.Minute),
		makeJob(created, "AllStatus", types.JOB_STATUS_CANCELED, jobType_Normal, time.Minute),
		makeJob(created, "AllStatus", types.JOB_STATUS_MISHAP, jobType_Normal, time.Minute),
		makeJob(created, "IgnoredStatus", types.JOB_STATUS_CANCELED, jobType_Normal, time.Minute),
		makeJob(created, "IgnoredStatus", types.JOB_STATUS_MISHAP, jobType_Normal, time.Minute),
	}
	require.NoError(t, jdb.PutJobs(context.Background(), jobsStatus))
	<-wait

	expect("AllStatus", jobType_Normal, jobsStatus[0:3])

	jobsType := []*types.Job{
		makeJob(created, "OnlyForced", types.JOB_STATUS_SUCCESS, jobType_Forced, 10*time.Minute),
		makeJob(created, "OnlyForced", types.JOB_STATUS_FAILURE, jobType_Forced, 11*time.Minute),
		makeJob(created, "NormalAndTryjob", types.JOB_STATUS_SUCCESS, jobType_Normal, 10*time.Minute),
		makeJob(created, "NormalAndTryjob", types.JOB_STATUS_SUCCESS, jobType_TryJob, 11*time.Minute),
		makeJob(created, "NormalAndTryjob", types.JOB_STATUS_FAILURE, jobType_TryJob, 12*time.Minute),
		makeJob(created, "NormalAndTryjob", types.JOB_STATUS_FAILURE, jobType_Normal, 9*time.Minute),
		makeJob(created, "AllTypes", types.JOB_STATUS_SUCCESS, jobType_Normal, 10*time.Minute),
		makeJob(created, "AllTypes", types.JOB_STATUS_SUCCESS, jobType_TryJob, 11*time.Minute),
		makeJob(created, "AllTypes", types.JOB_STATUS_SUCCESS, jobType_Forced, 12*time.Minute),
	}
	require.NoError(t, jdb.PutJobs(context.Background(), jobsType))
	<-wait

	expect("OnlyForced", jobType_Forced, jobsType[0:2])
	expect("NormalAndTryjob", jobType_Normal, []*types.Job{jobsType[2], jobsType[5]})
	expect("NormalAndTryjob", jobType_TryJob, jobsType[3:5])
	expect("AllTypes", jobType_Normal, jobsType[6:7])
	expect("AllTypes", jobType_TryJob, jobsType[7:8])
	expect("AllTypes", jobType_Forced, jobsType[8:9])

	require.NoError(t, edb.update())
	evs, err := edb.Range(jobStream, created.Add(-time.Hour), created.Add(time.Hour))
	require.NoError(t, err)
...
func TestComputeJobFailureMishapRate(t *testing.T) {
	...
	{
		name := "AllStatus"
		addJob(name, types.JOB_STATUS_SUCCESS, jobType_Normal)
		addJob(name, types.JOB_STATUS_SUCCESS, jobType_Normal)
		addJob(name, types.JOB_STATUS_FAILURE, jobType_Normal)
		addJob(name, types.JOB_STATUS_CANCELED, jobType_Normal)
		addJob(name, types.JOB_STATUS_MISHAP, jobType_Normal)
	...
	}
	{
		...
		name := "OnlyTryjobs"
		addJob(name, types.JOB_STATUS_SUCCESS, jobType_TryJob)
		addJob(name, types.JOB_STATUS_FAILURE, jobType_TryJob)
		addJob(name, types.JOB_STATUS_MISHAP, jobType_TryJob)
...
func TestOverdueJobSpecMetrics(t *testing.T) {
	...
	check := func(buildAge, testAge, perfAge string) {
		...
		require.Equal(t, buildAge, metrics2_testutils.GetRecordedMetric(t, measurementOverdueJobSpecs, tags))

		tags["job_name"] = tcc_testutils.TestTaskName
		require.Equal(t, testAge, metrics2_testutils.GetRecordedMetric(t, measurementOverdueJobSpecs, tags))

		tags["job_name"] = tcc_testutils.PerfTaskName
		require.Equal(t, perfAge, metrics2_testutils.GetRecordedMetric(t, measurementOverdueJobSpecs, tags))
	}
