...
type jobEventDB struct {
	...
	// Do not lock mtx when calling methods on em. em calls methods of jobEventDB
	// with em.mtx locked, in particular Range, which locks mtx, creating a
	// deadlock if we have locked mtx and called a method on em that locks em.mtx.
	mtx sync.Mutex
}
