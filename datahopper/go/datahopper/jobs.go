...
const (
	...
	// MAX_JOB_DURATION is the assumed maximum time a job might take to
	// complete.
	MAX_JOB_DURATION = 7 * 24 * time.Hour
)
...
func (m *jobMetrics) reset() error {
	...
	now := time.Now()
	jobs, err := m.db.GetJobsFromDateRange(m.lastRan.Add(-MAX_JOB_DURATION), now)
	if err != nil {
...
func StartJobMetrics(workdir, taskSchedulerDbUrl string, ctx context.Context) (*jobMetrics, error) {
	...
	m := &jobMetrics{
		...
		em:         em,
		jobStreams: map[string]*events.EventStream{},
		lastRan:    time.Now().Add(-MAX_JOB_DURATION),
		queryId:    "",
	}
	if err := m.reset(); err != nil {
		return nil, err
...
// Close performs cleanup for the job metrics.
func (m *jobMetrics) Close() error {
	return m.edb.Close()
}
