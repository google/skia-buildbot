...
// for Jobs with status SUCCESS or FAILURE, given a slice of Events created by jobEventDB.update. The first return value
// will contain the tags "job_name" (db.Job.Name) and "job_type" (one of "normal", "tryjob",
// "forced"), and the second return value will be the corresponding average Job duration. Returns an
// error if Event.Data can't be GOB-decoded as a db.Job.
