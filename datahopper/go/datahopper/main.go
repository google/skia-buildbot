...
func main() {
	...
	skiaRepo, err := repograph.NewGraph(common.REPO_SKIA, reposDir)
	if err != nil {
		glog.Fatal(err)
	}
	infraRepo, err := repograph.NewGraph(common.REPO_SKIA_INFRA, reposDir)
	if err != nil {
		glog.Fatal(err)
	}
	repos := map[string]*repograph.Graph{
		common.REPO_SKIA:       skiaRepo,
		common.REPO_SKIA_INFRA: infraRepo,
	}
