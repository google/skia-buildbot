...
var (
	...
	perfPrefix = flag.String("perf_duration_prefix", "task_duration", "The folder name in the bucket that task duration metric shoudl be written.")
)
...
func main() {
	...
	bnp, err := buildbot.DefaultBuilderNameParser()
	if err != nil {
		sklog.Fatal(err)
	}
	...
	// Swarming tasks.
	if err := swarming_metrics.StartSwarmingTaskMetrics(w, swarm, context.Background(), pc, bnp); err != nil {
		sklog.Fatal(err)
	}
