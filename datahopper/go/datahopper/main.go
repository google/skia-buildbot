...
func main() {
	...
	// Shared repo objects.
	skiaRepo, err := gitinfo.CloneOrUpdate(common.REPO_SKIA, path.Join(*workdir, path.Base(common.REPO_SKIA)), true)
	if err != nil {
		glog.Fatal(err)
	}
	infraRepo, err := gitinfo.CloneOrUpdate(common.REPO_SKIA_INFRA, path.Join(*workdir, path.Base(common.REPO_SKIA_INFRA)), true)
	if err != nil {
		glog.Fatal(err)
	}

	// Set up buildbot DB.
	db, err := buildbot.NewLocalDB(path.Join(*workdir, "buildbot.db"), gitinfo.NewRepoMap(*workdir))
	if err != nil {
		glog.Fatal(err)
	}
	...
	// Data ingestion goroutines.
	go updateReposLoop(skiaRepo, infraRepo)
	go trackIngestionProgress(db)
	go ingestBuildbotStats(db)
	go trackOfflineBuildslaves()
	go trackRepoCommits(skiaRepo, infraRepo)
	go trackLastSuccessfulDBBackup()

	// Run a backup server.
	glog.Fatal(buildbot.RunBackupServer(db, *httpPort))
}

// Periodically sync the Git repos.
func updateReposLoop(skiaRepo, infraRepo *gitinfo.GitInfo) {
...
// Measure buildbot ingestion process.
func trackIngestionProgress(db buildbot.DB) {
...
func ingestBuildbotStats(db buildbot.DB) {
...
// Number of commits in each repo.
func trackRepoCommits(skiaRepo, infraRepo *gitinfo.GitInfo) {
...
// Helper function used by trackLastSuccessfulDBBackup.
func setLastBackupTime(gsClient *storage.Client, lv *metrics2.Liveness) error {
	last := time.Time{}
...
func trackLastSuccessfulDBBackup() {
	...
	if err := setLastBackupTime(gsClient, lv); err != nil {
		glog.Fatal(err)
	}
	for _ = range time.Tick(10 * time.Minute) {
		if err := setLastBackupTime(gsClient, lv); err != nil {
