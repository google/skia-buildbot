...
func TestLoadSwarmingTasks(t *testing.T) {
	...
	mp := buildbot.NewMockBuilderNameParser()
	now := time.Now()
	...
	mp.On("ParseBuilderName", mock.AnythingOfType("string")).Return(map[string]string{}, nil)
...
func TestPerfUpload(t *testing.T) {
	...
	mp.On("ParseBuilderName", "Test-MyOS").Return(map[string]string{
		"os":   "MyOS",
		"role": "Test",
	}, nil)
	...
	mp.On("ParseBuilderName", "Perf-MyOS").Return(map[string]string{
		"os":   "MyOS",
		"role": "Perf",
	}, nil)
