...
func loadSwarmingTasks(s swarming.ApiClient, edb events.EventDB, perfClient perfclient.PerfClient, bnp buildbot.BuilderNameParser, lastLoad, now time.Time, revisit []string) ([]string, error) {
...
func reportDurationToPerf(t *swarming_api.SwarmingRpcsTaskRequestMetadata, perfClient perfclient.PerfClient, now time.Time, bnp buildbot.BuilderNameParser) {

	// Pull taskName from tags, because the task name could be changed (e.g. retries)
	// and that would make ParseBuilderName not happy.
	taskName := ""
	taskRevision := ""
	...
	parsed, err := bnp.ParseBuilderName(taskName)
	if err != nil {
		sklog.Warningf("Could not parse builder name of taskName: %s", err)
		return
	}
...
func startLoadingTasks(swarm swarming.ApiClient, ctx context.Context, edb events.EventDB, perfClient perfclient.PerfClient, bnp buildbot.BuilderNameParser) {
...
func StartSwarmingTaskMetrics(workdir string, swarm swarming.ApiClient, ctx context.Context, perfClient perfclient.PerfClient, bnp buildbot.BuilderNameParser) error {
