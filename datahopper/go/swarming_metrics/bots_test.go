...
func TestDivergentWindowsSkoloOSVersionMatch(t *testing.T) {
	testutils.SmallTest(t)
	...
	genBot := func(id string) *swarming_api.SwarmingRpcsBotInfo {
		return &swarming_api.SwarmingRpcsBotInfo{
			BotId:      id,
			LastSeenTs: now.Add(-time.Minute).Format("2006-01-02T15:04:05"),
			Dimensions: swarming.StringMapToBotDimensions(map[string][]string{
				"os":   []string{"Windows", "Windows-10", "Windows-10-17134.345"},
	...
	actual := metrics_util.GetRecordedMetric(t, MEASUREMENT_DIVERGENT_WINDOWS_SKOLO_OS_VERSION, tags)
	assert.Equal(t, "0", actual)
}

func TestDivergentWindowsSkoloOSVersionMismatch(t *testing.T) {
	testutils.SmallTest(t)
