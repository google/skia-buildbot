...
const (
	...
	// commit queue config; its value is 0 or 1.
	...
	// a given CQ tryjob for a given branch.
	METRIC_BOT_EXISTS = "cq_cfg_bot_exists_for_tryjob"
)

// repoMetrics is a struct used for collecting all supported branch metrics for
// a given repo.
type repoMetrics struct {
	branchExists map[metrics2.Int64Metric]struct{}
	tryjobExists map[metrics2.Int64Metric]struct{}
	botExists    map[metrics2.Int64Metric]struct{}
}
...
func metricsForRepo(repo *gitiles.Repo, oldMetrics *repoMetrics, botDimsList []map[string][]string) (*repoMetrics, error) {
	newMetrics := &repoMetrics{
		branchExists: map[metrics2.Int64Metric]struct{}{},
		tryjobExists: map[metrics2.Int64Metric]struct{}{},
		botExists:    map[metrics2.Int64Metric]struct{}{},
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("Failed to get supported branches for %s: %s", repo.URL, err)
	}
	cqCfg, err := cq.GetCQConfig(repo)
	if err != nil {
		return nil, fmt.Errorf("Failed to get CQ config for %s: %s", repo.URL, err)
	}
	for _, branch := range sbc.Branches {
		// Find the CQ trybots for this branch.
		cqTrybots, err := cq.GetCQTryBots(cqCfg, branch.Ref)
		if err != nil {
			return nil, fmt.Errorf("Failed to get CQ trybots for %s from CQ config: %s\nConfig: %+v", branch.Ref, err, cqCfg)
		}
		...
		newMetrics.branchExists[branchExistsMetric] = struct{}{}

		// Obtain the tasks cfg for this branch.
		var buf bytes.Buffer
		if err := repo.ReadFileAtRef(specs.TASKS_CFG_FILE, branch.Ref, &buf); err != nil {
			return nil, fmt.Errorf("Failed to read %s on %s of %s: %s", specs.TASKS_CFG_FILE, branch.Ref, repo.URL, err)
		}
		tasksCfg, err := specs.ParseTasksCfg(buf.String())
		if err != nil {
			return nil, fmt.Errorf("Failed to parse %s on %s of %s: %s", specs.TASKS_CFG_FILE, branch.Ref, repo.URL, err)
		}
		...
		for _, job := range cqTrybots {
			...
			newMetrics.tryjobExists[jobExistsMetric] = struct{}{}
			...
			if ok {
				...
				for taskName, taskSpec := range tasks {
					...
					if err != nil {
						return nil, fmt.Errorf("Failed to parse dimensions for %s on %s; %s\ndims: %+v", taskName, branch.Ref, err, taskSpec.Dimensions)
					}
					canRunTask := false
				...
				newMetrics.botExists[botExistsMetric] = struct{}{}
			}
		}
	}

	// Delete unused old metrics.
	for m, _ := range oldMetrics.branchExists {
		if _, ok := newMetrics.branchExists[m]; !ok {
			if err := m.Delete(); err != nil {
				...
				// next cycle.
				newMetrics.branchExists[m] = struct{}{}
			}
		}
	}
	for m, _ := range oldMetrics.tryjobExists {
		if _, ok := newMetrics.tryjobExists[m]; !ok {
			if err := m.Delete(); err != nil {
				sklog.Errorf("Failed to delete metric: %s", err)
				// Add the metric to newMetrics so that we'll
				// have the chance to delete it again on the
				// next cycle.
				newMetrics.tryjobExists[m] = struct{}{}
			}
		}
	}
	for m, _ := range oldMetrics.botExists {
		if _, ok := newMetrics.botExists[m]; !ok {
			if err := m.Delete(); err != nil {
				sklog.Errorf("Failed to delete metric: %s", err)
				// Add the metric to newMetrics so that we'll
				// have the chance to delete it again on the
				// next cycle.
				newMetrics.botExists[m] = struct{}{}
			}
		}
	}
	return newMetrics, nil
...
func cycle(repos []*gitiles.Repo, oldMetrics map[string]*repoMetrics, swarm swarming.ApiClient, pools []string) (map[string]*repoMetrics, error) {
	...
	newMetrics := make(map[string]*repoMetrics, len(repos))
	for _, repo := range repos {
		nm, err := metricsForRepo(repo, oldMetrics[repo.URL], botDimsList)
		if err != nil {
			return nil, err
		}
		newMetrics[repo.URL] = nm
	}
	return newMetrics, nil
}
