...
The reference documentation for our Bazel build can be found at the following Golinks:
 ...
 - [go/skia-infra-bazel-backend](http://go/skia-infra-bazel-bazel)
...
Tip: make a Bash alias with `alias bazel="bazelisk"` and add it to your `~/.bash_aliases` file.

### Get your RBE credentials

This step is necessary for the `--config=remote` flag to work.
...
```
$ gcloud iam service-accounts keys create path/to/rbe-developer.json \
      --project skia-infra \
...
Our Bazel build is tested on RBE via the following tasks:
 ...
 - Infra-PerCommit-Build-Bazel-RBE (equivalent to `bazel build //... --config=remote`)
 - Infra-PerCommit-Test-Bazel-RBE (equivalent to `bazel test //... --config=remote`)
...
As an insurance policy against RBE outages, we also have the following tasks:
 ...
 - Infra-PerCommit-Build-Bazel-Local (equivalent to `bazel build //...`)
 - Infra-PerCommit-Test-Bazel-Local (equivalent to `bazel test //...`)

These tasks tend to be a bit more brittle than their RBE counterparts, so they are excluded from
the CQ.
...
By default, Bazel will build and test targets on the host system. To build on RBE, add flag
`--config=remote`, e.g.:
...
write access to the RBE cache. This is in line with the
[RBE best practices](https://cloud.google.com/remote-build-execution/docs/remote-cache-overview),
which recommend only granting write access to the CI environment. This is because the CI
