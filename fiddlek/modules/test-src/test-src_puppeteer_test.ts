...
describe('test-src', () => {
  ...
  beforeEach(async () => {
    await testBed.page.goto(`${testBed.baseUrl}/dist/test-src.html`);
  });

  it('should render the demo page', async () => {
    // Smoke test.
    expect(await testBed.page.$$('test-src')).to.have.length(1);
  });

  it('should take a screenshot', async () => {
    await testBed.page.setViewport({ width: 1200, height: 600 });
    await takeScreenshot(testBed.page, 'change-me-to-the-app-name', 'test-src');
  });
});
