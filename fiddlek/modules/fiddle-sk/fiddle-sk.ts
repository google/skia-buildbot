...
${err.text}</pre
  ...
  private async run() {
    try {
      this._runResults.fiddleHash = '';
      this.querySelectorAll<SpinnerSk>('spinner-sk').forEach((sp) => {
        sp.active = true;
      });
      var body: FiddleContext = {
        version: '',
        sources: '',
        fiddlehash: '',
        name: '',
      ...
      this._runResults = (await jsonOrThrow(request)) as RunResults;
      this._render();
      this.dispatchEvent(
