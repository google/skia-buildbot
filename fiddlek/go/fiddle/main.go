...
func scrapHandler(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := httputils.DefaultClientConfig().With2xxOnly().Client().Get(fmt.Sprintf("http://scrapexchange:9000/_/tmpl/%s/%s/%s", typ, hashOrName, scrap.CPP))
	if err != nil {
	...
	runResp, err, msg := runImpl(r.Context(), req)
	if err != nil {
		httputils.ReportError(w, err, msg, http.StatusInternalServerError)
		return
	}

	http.Redirect(w, r, "/c/"+runResp.FiddleHash, http.StatusTemporaryRedirect)
}
