...
var (
	...
	chatWebHooksFile      = flag.String("chat_webhooks_file", "", "OAuth token cache file for sending email.")
...
func main() {
	...
	if err != nil {
		sklog.Fatalf("Failed to read client secrets from %q: %s", *emailTokenCacheFile, err)
	}
	emailAuth, err = email.NewGMail(cfg.ClientID, cfg.ClientSecret, *emailTokenCacheFile)
	if err != nil {
