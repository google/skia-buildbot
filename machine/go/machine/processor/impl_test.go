...
Current cooling devices from HAL:
	...
	p := newProcessorForTest(t)
	next := p.Process(ctx, previous, event)
	assert.Equal(t, "Temperature is too hot: 44.1 > 40 (C)", next.Dimensions[machine.DimQuarantined][0])
	//assert.Equal(t, 44.1, next.Battery)
	assert.Equal(t, float64(44.1), metrics2.GetFloat64Metric("machine_processor_device_temperature_c", map[string]string{"machine": "skia-rpi2-0001"}).Get())
