...
var (
	...
	thermalServiceTemperature = regexp.MustCompile(`\tTemperature{mValue=([\d\.]+),\s+mType=\d+,\s+mName=(\S+)\smStatus=(\d)`)
)
...
func (p *ProcessorImpl) Process(ctx context.Context, previous machine.Description, event machine.Event) machine.Description {
	...
	temperature, ok := temperatureFromAndroid(event.Android)
	if ok {
		...
		metrics2.GetFloat64Metric("machine_processor_device_temperature_c", map[string]string{"machine": machineID}).Update(temperature)
	}
...
//
// The output from the thermalservice looks like:
//
...
func temperatureFromAndroid(android machine.Android) (float64, bool) {
	temps := []float64{}
	...
	if len(temps) == 0 {
		...
		if temperatureMatch != nil {
			...
			if err == nil {
				temps = append(temps, float64(temp10C)/10)
			}
		}
	}
	if len(temps) == 0 {
		return badTemperature, false
	}
	ret := badTemperature
