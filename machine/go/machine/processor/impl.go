...
func (p *processor) Process(ctx context.Context, current machine.Description, event machine.Event) machine.Description {
	...
	properties := parseAndroidProperties(event.Android.GetProp)
	dimensions := dimensionsFromAndroidProperties(properties)

	// The device_type dimension is reported for both Android and iOS devices.
	if len(current.Dimensions["device_type"]) > 0 && len(dimensions["device_type"]) == 0 {
		dimensions["quarantined"] = []string{fmt.Sprintf("Device %q has gone missing", current.Dimensions["device_type"])}
	}
	ret := machine.DupDescription(current)
