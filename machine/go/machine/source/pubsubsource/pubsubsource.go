...
const machineEventChannelSize = 100
...
func New(ctx context.Context, instanceConfig config.InstanceConfig) (*Source, error) {
	ts, err := auth.NewDefaultTokenSource(*baseapp.Local, pubsub.ScopePubSub)
	if err != nil {
	...
	subName := fmt.Sprintf("%s-%s", instanceConfig.Source.Topic, "prod")
	if *baseapp.Local {
