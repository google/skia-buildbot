...
export class PlaySk extends ElementSk {
  ...
  /**
   * @prop {string} visual - Specifies the visual style of the playback element.
   * Must be set. Possible values include:
   *  'full': shows all five buttons and a textbox for controlling delay.
   *  'simple': shows only a play button, using a distinct icon.
   */
  get visual(): string {
    return this.getAttribute('visual') || 'full';
  }

  set visual(val: string) {
    this.setAttribute('visual', val);
  }
