...
export class ResourcesSk extends ElementSk {
  private static template = (ele: ResourcesSk) =>
    html`
<p>${ele._list.length} images were stored in this file. Note that image indices here are file\
 indices, which corresponded 1:1 to the gen ids that the images had during recording. If an image\
 appears more than once, that indicates there were multiple copies of it in memory at record time.\
 These indices appear in the 'imageIndex' field of commands using them. This metadata is only\
 recorded for multi frame (mskp) files from android, so nothing is shown here for other skps
</p>
