...
Command types can also be filted by clicking on their names in the histogram"
  ...
  private _layerInfo: LayerInfo = {
    layerUse: new Map<number, number[]>(),
    layerNames: new Map<number, string>(),
  };
  ...
  processCommands(cmd: SkpJsonCommandList) {
    ...
    this._layerInfo.layerUse = new Map<number, number[]>();
    this._layerInfo.layerNames = new Map<number, string>();
    ...
    cmd.commands.forEach((com, i) => {
      ...
      if (name in INDENTERS) {
        ...
        if (!this._layerInfo.layerUse.has(node)) {
          this._layerInfo.layerUse.set(node, []);
        }
        this._layerInfo.layerUse.get(node)!.push(i);
