...
describe('commands-sk', () => {
  ...
  describe('element behavior', () => {
    ...
    it('can process a second command list after loading a first', () => {
      ...
      expect((document.getElementById('rangelo') as HTMLInputElement).value)
        .to.equal('0');
      expect((document.getElementById('rangehi') as HTMLInputElement).value)
        .to.equal('2'); // the highest index
      expect((document.getElementById('text-filter') as HTMLInputElement).value)
  ...
  it('Can apply a range filter while a negative text filter is applied', () => {
    ...
    // only one op, the save at position 4, satisfies both filters
    expect(element.countFiltered).to.equal(7);
