...
<dl>
  ...
  <dt><b>Color</b></dt>
  <dd><div class=color-preview id=prevColor style="background-color: ${ele._rgb}"></div>${ele._rgb}</dd>
  <dd>${ele._hex}</dd>
</dl>
...
<details>
  ...
  connectedCallback() {
    ...
    document.addEventListener('move-zoom-cursor', (e) => {
      ...
      this.update();
      this._render();
      // TODO(nifong): there seems to be a bug with updating.
      // despite this event being recieved and processed every mouse move event, the
      // zoom canvas doesn't appear redrawn unless the mouse is almost stopped.
    });
