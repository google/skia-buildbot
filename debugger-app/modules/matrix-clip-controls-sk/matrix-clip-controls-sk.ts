...
// this is all put into one object because they change in unison when moving to a new command or frame
// and the debugger wasm hands it to us this way
export interface MatrixClipInfo {
  clip: number[]; // array of left top right bottom
  matrix: number[][]; // 3x3 or 4x4
}
...
export class MatrixClipControlsSk extends ElementSk {
  private static template = (ele: MatrixClipControlsSk) =>
    html`<div>
      ...
      <h3>Matrix</h3>
      <table>
        ${ ele._info.matrix.map((row: number[]) => html`<tr>${ row.map((i: number) => html`<td>${i}</td>`) }</tr>`) }
      </table>
    </div>`;
  ...
  private _showClip: boolean = false;
  private _showAndroidClip: boolean = false;
  private _showOrigin: boolean = false;
  ...
  private _clipHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
    this._showClip = element.checked;
    console.log('this._showClip = '+this._showClip);
  }

  private _androidClipHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
    this._showAndroidClip = element.checked;
    console.log('this._showAndroidClip = '+this._showAndroidClip);
  }

  private _originHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
    this._showOrigin = element.checked;
    console.log('this._showOrigin = '+this._showOrigin);
  }
};
