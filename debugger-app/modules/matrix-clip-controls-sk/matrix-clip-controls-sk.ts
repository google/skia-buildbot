...
// this is all put into one object because they change in unison when moving to a new command or
// frame and the debugger wasm hands it to us this way
...
export class MatrixClipControlsSk extends ElementSk {
  ...
  _matrixTable(m: number[][]) {
    return m.map((row: number[]) => html`<tr>${ row.map((i: number) => html`<td>${i}</td>`) }</tr>`);
  }

  // controls change handlers
  _clipHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
    this._showClip = element.checked;
  }

  _androidClipHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
    this._showAndroidClip = element.checked;
  }

  _originHandler(e: Event) {
    var element = <CheckOrRadio> e.target;
