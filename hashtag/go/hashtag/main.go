...
func (srv *server) indexHandler(w http.ResponseWriter, r *http.Request) {
	...
	if hashtag != "" {
		...
		markdownArtifacts := []source.Artifact{}
		wg.Add(1)
		go func() {
			defer wg.Done()
			for artifact := range srv.markdownSource.ByHashtag(hashtag) {
		...
		templateContext.MarkdownList = markdownArtifacts
