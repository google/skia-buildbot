...
func (srv *server) searchByHashtag(w http.ResponseWriter, r *http.Request) {
	...
	list, err := srv.filesService.List().Corpora("drive").IncludeTeamDriveItems(true).Q(fmt.Sprintf("fullText contains '#%s'", hashtag)).Do()
	if err != nil {
	...
	if err := srv.templates.ExecuteTemplate(w, "searchByHashtag.html", searchContext{
		// Look in webpack.config.js for where the nonce templates are injected.
		Nonce: secure.CSPNonce(r.Context()),
		List:  list,
	}); err != nil {
