...
func (d *driveSource) toString(q source.Query) string {
	...
	ret = append(ret, fmt.Sprintf("fullText contains %q", q.Value))

	if !q.Begin.IsZero() {
		ret = append(ret, fmt.Sprintf("modifiedTime > %q", q.Begin.Format("2006-01-02")))
	}
	if !q.End.IsZero() {
		ret = append(ret, fmt.Sprintf("modifiedTime < %q", q.End.Format("2006-01-02")))
	}
