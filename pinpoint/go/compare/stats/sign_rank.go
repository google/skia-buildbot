...
func newWilcoxonDistribution(n int) (*dist, error) {
	if n <= 0 {
		return nil, skerr.Fmt("input n (%v) should be a non zero integer", n)
	}
...
func (d dist) pSignRank(x float64, lowerTail bool) float64 {
	x = math.Round(x + 1e-7)
	if x < 0 {
		if lowerTail {
			return 0
...
func (d dist) qSignRank(x float64, lowerTail bool) (float64, error) {
	if x < 0 || x > 1 {
		return 0, skerr.Fmt("input probability x (%v) should between 0 and 1", x)
	}
	if lowerTail {
		if x == 0 {
			return 0, nil
		...
		if x == 1 {
			return float64(len(d.pmf) - 1), nil
		}
	} else {
		if x == 1 {
			return 0, nil
	...
	if x <= 0.5 {
		for {
			if q >= len(d.pmf) {
				return 0, skerr.Fmt("q (%v) should be less than length of pmf (%v)", q, len(d.pmf))
			}
			p += d.pmf[q]
			if p >= x {
				break
