...
func polylintTest(cwd, fileName string) *test {
	...
	return &test{
		...
		run: func() (string, error) {
			...
			if err != nil {
				if _, err2 := exec.LookPath(cmd[0]); err2 != nil {
					return string(outputBytes), fmt.Errorf(needInstallErr, cmd[0], err)
				}
...
//polylintTests creates a list of *test from all directories in polymerPaths
func polylintTests() []*test {
	tests := make([]*test, 0)
	for _, p := range polymerPaths {
...
func checkMockeryIsUpToDate() error {
	...
	for {
		...
		// If we cannot find it, we return an error. This is to work around the fact that we
		// changed mockery to be something installed via go get to something brought in via CIPD.
		// The gocache held onto the out of date version and we want to clean it up with this
		// approach.
		...
		o, err = exec.Command(mExe, "--version").CombinedOutput()
		if err != nil {
