...
def sk_element(name, srcs, sass_srcs, deps = [], sass_deps = [], sk_element_deps = []):
    ...
    # Fold the sk_element_deps into deps and sass_deps.
    for sk_element_dep in sk_element_deps:
        deps.append(sk_element_dep)
        sass_deps.append(sk_element_dep + "_styles")

    ts_library(
        name = name,
        srcs = srcs,
        deps = deps + [
            # Add common dependencies for convenience. Almost all sk_elements need this.
...
def sk_page(name, deps = [], sass_deps = [], sk_element_deps = [], assets_serving_path = "/", nonce = None):
    ...
    # Fold the sk_element_deps into deps and sass_deps.
    all_deps = [dep for dep in deps]
    all_sass_deps = [dep for dep in sass_deps]
    for sk_element_dep in sk_element_deps:
        all_deps.append(sk_element_dep)
