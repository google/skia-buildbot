...
# Re-export all front-end rules in order to make this .bzl file the only import necessary from our
# BUILD.bazel files.
...
def sk_element(
    ...
    extra_ts_deps = [
        "//infra-sk/modules/ElementSk:elementsk",
    ...
    ts_library(
        name = name,
        srcs = ts_srcs,
        deps = all_ts_deps + [dep for dep in extra_ts_deps if dep not in all_ts_deps],
        visibility = visibility,
    )
...
def nodejs_test(
    ...
    _nodejs_test(
        ...
        data = ["//:tsconfig.json", src] + deps + [dep for dep in extra_deps if dep not in deps],
        templated_args = [
