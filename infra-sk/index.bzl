...
def sk_element_puppeteer_test(name, srcs, demo_page_server, deps = []):
    """Defines a Puppeteer test for the demo page served by given demo page server.
    ...
    bundled in a single .zip file per test target. For example, if we run a Puppeteer test with e.g.
    ...
    Args:
      ...
      demo_page_server: Label for the demo_page_server target.
