...
export class QuerySk extends ElementSk {
  ...
  private _valuesChanged(
    ...
    if (this._fast!.value.trim() !== '') {
      ...
      if (!e.detail.regex && this._query[key] && this._query[key][0] === '~') {
        this._query[key] = [];
      }
      ...
      this.applyInvert(key, e.detail.invert);
  ...
  private applyInvert(key: string, invert: boolean) {
    const values = this._query[key];
    if (!values || !values.length) {
      return;
    }
    if (invert && values[0][0] === '!') {
      return;
    } else if (!invert && values[0][0] !== '!') {
      return;
    } else if (invert && values[0][0] !== '!') {
