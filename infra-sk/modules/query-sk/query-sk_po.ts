...
export class QuerySkPO extends PageObject {
  getFilter() {
    return this.$$apply('#fast', (filter) => filter.value);
  }

  async setFilter(value: string) {
    await this.$$apply('#fast', (filter) => filter.setValue(value));
  }

  async clickClearFilter() {
    return this.$$apply('button.clear_filters', (btn) => btn.click());
  }

  async clickClearSelections() {
    return this.$$apply('button.clear_selections', (btn) => btn.click());
  }

  getKeys() {
    return this.$map('select-sk div', (div) => div.innerText);
  }

  async getSelectedKey(key: string) {
    return this.$$apply('select-sk div[selected]', (div) => div.innerText);
  }

  async clickKey(key: string) {
    const keyDiv = await this.$find('select-sk div', async (div) => (await div.innerText) === key);
    await keyDiv!.click();
  }

  getQueryValuesSkPO() {
    return this.$$apply('query-values-sk', async (el) => new QueryValuesSkPO(el));
  }
