...
describe('query-sk', () => {
  ...
  it('inverts query values on invert even if they are hidden by a filter.', async () => {
    // Make a selection.
    querySk.current_query = 'arch=x86';
  ...
  it('removes the invert from query values on when leaving invert even if they are hidden by a filter.', async () => {
    // Make a selection.
    querySk.current_query = 'arch=!x86';
