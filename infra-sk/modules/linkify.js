/** @module infra-sk/modules/linkify */

/**
 * Returns a DOM Element that has the following changes:
 *   * URLs are wrapped in anchor tags
 *   * newlines are expanded as <br/>
 *   * bug shorthand (e.g. skia:1234) is wrapped in anchor tags.
 *
 * This DOM Element can be used in a lit-html template.
 * https://lit-html.polymer-project.org/guide/template-reference#node
 */
export function formatAndLinkify(s) {
  if (!s) {
    return '';
  }
  const div = document.createElement('div');
  // sanitize the incoming string, so we aren't vulnerable to XSS. This has the effect of
  // replacing '<' and '>' with '&lt;' and '&rt;', which mitigates the threat.
  div.innerText = s;
  s = div.innerHTML;

  // Replace http://... with actual links
  const sub = '<a href="$&" target=_blank rel=noopener>$&</a>';
  s = s.replace(/https?:\/\/\S+/g, sub);
  // Replace newlines with <br/>
  s = s.replace(/(?:\r\n|\n|\r)/g, '<br/>');

  // Replace things like skia:1234 with actual links
  for (const project in PROJECTS_TO_ISSUETRACKERS) {
    const re = new RegExp(project + ':[0-9]+', 'g');
    const foundBugs = s.match(re);
    if (!foundBugs) {
      continue;
    }
    const issueTrackerPrefix = PROJECTS_TO_ISSUETRACKERS[project];
    for (const foundBug of foundBugs) {
      const bugNumber = foundBug.split(':')[1];
      s = s.replace(foundBug, `<a href="${issueTrackerPrefix + bugNumber}"
                                  target=_blank rel=noopener>${foundBug}</a>`);
    }
  }
  div.innerHTML = s;
  return div;
}

const PROJECTS_TO_ISSUETRACKERS = {
  'chromium': 'http://crbug.com/',
  'skia': 'http://skbug.com/',
};
