...
export class ParamSetSkPO extends PageObject {
  async getTitles() {
    ...
    return this.$map('tr:nth-child(1) th:not(:nth-child(1))', (th) => th.innerText);
  }
  ...
  private async _forEachValue(
    ...
    await this.$each('tr:not(:nth-child(1))', async (tr) => {
      const key = await tr.$$apply('th', (th) => th.innerText); // One key per row.

      // Iterate over all cells. Each cells corresponds to one ParamSet.
      await tr.$each('td', async (td, paramSetIndex) => {

        // Visit each value for the current key and ParamSet.
        await td.$each('div', (div) => fn(paramSetIndex, key!, div));
      });
    });
  }
};
