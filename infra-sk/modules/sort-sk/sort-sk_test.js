...
describe('sort-sk', function() {
  ...
  <div id=stuffToBeSorted>
    ...
    <pre data-clustersize=100 data-stepsize=0.6>Size=100  Step=0.6</pre>
      ...
      clusterButton.click();
      assert.deepEqual([100, 50, 10], getValues('clustersize'));
      clusterButton.click();
      assert.deepEqual([10, 50, 100], getValues('clustersize'));

      stepButton.click();
      assert.deepEqual([1.2, 0.6, 0.5], getValues('stepsize'));
      stepButton.click();
      assert.deepEqual([0.5, 0.6, 1.2], getValues('stepsize'));
    })
  });
  ...
  it('sorts alphabetically with alpha attribute', function() {
    return window.customElements.whenDefined('sort-sk').then(() => {
      container.innerHTML = `
  <sort-sk alpha target=stuffToBeSorted2>
    <button id=name data-key=name data-default=down>Name</button>
    <button id=level data-key=level>Level</button>
  </sort-sk>
  ...
  `;
      ...
      nameButton.click();
      assert.deepEqual(['bar', 'baz', 'foo'], getValues('name'));
      nameButton.click();
      assert.deepEqual(['foo', 'baz', 'bar'], getValues('name'));

      levelButton.click();
      assert.deepEqual(['gamma', 'beta', 'alpha'], getValues('level'));
      levelButton.click();
      assert.deepEqual(['alpha', 'beta', 'gamma'], getValues('level'));
    })
  });
});
