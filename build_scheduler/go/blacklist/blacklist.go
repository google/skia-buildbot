...
func validateCommit(c string, repos map[string]*repograph.Graph) error {
	commit, _, _, err := repograph.FindCommitInGraphs(c, repos)
	if err != nil {
		return err
	}
	if commit.Hash != c {
...
func NewCommitRangeRule(name, user, description string, builderPatterns []string, startCommit, endCommit string, repos map[string]*repograph.Graph) (*Rule, error) {
	_, repoName, repo, err := repograph.FindCommitInGraphs(startCommit, repos)
	if err != nil {
		return nil, err
	}
	_, repoName2, _, err := repograph.FindCommitInGraphs(endCommit, repos)
	if err != nil {
		return nil, err
	}
	if repoName != repoName2 {
		return nil, fmt.Errorf("Commits %s and %s are from different repos (%s vs %s)", startCommit, endCommit, repoName, repoName2)
	}
