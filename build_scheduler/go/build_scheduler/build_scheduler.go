...
func (bs *BuildScheduler) Trigger(builderNames []string, commit string) ([]*buildbucket.Build, error) {
	...
	author := ""
	details, repoName, _, err := repograph.FindCommitInGraphs(commit, bs.repos)
	if err != nil {
		return nil, err
	}
...
func StartNewBuildScheduler(period time.Duration, scoreThreshold, scoreDecay24Hr float64, db buildbot.DB, bb *buildbucket.Client, repos map[string]*repograph.Graph, workdir string, local bool) *BuildScheduler {
	// Build the queue.
