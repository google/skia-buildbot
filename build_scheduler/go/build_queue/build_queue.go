...
func NewBuildQueue(period time.Duration, repos map[string]*repograph.Graph, scoreThreshold, timeDecay24Hr float64, bl *blacklist.Blacklist, db buildbot.DB) (*BuildQueue, error) {
...
func (q *BuildQueue) updateRepo(repo *repograph.Graph, now time.Time) (map[string][]*BuildCandidate, error) {
	...
	// Create slices of hashes.
	recentCommitsList := make([]string, 0, len(recentCommits))
	for _, c := range recentCommits {
		recentCommitsList = append(recentCommitsList, c.Hash)
	}
	q.setRecentCommits(recentCommitsList)

	recentCommitsPreloadList := make([]string, 0, len(recentCommits))
	for _, c := range recentCommits {
		recentCommitsPreloadList = append(recentCommitsPreloadList, c.Hash)
	}

	// Get all builds associated with the recent commits in the preload.
	buildsByCommit, err := q.db.GetBuildsForCommits(recentCommitsPreloadList, nil)
	if err != nil {
		return nil, fmt.Errorf(errMsg, err)
	}
