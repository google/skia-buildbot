...
func TestGitCAS(t *testing.T) {
	...
	// producing an error.
	{
		...
		dst, cleanupDst := testutils.TempDir(t)
		defer cleanupDst()
		require.NoError(t, Download(ctx, local, dst, hash1))
		require.NoError(t, Download(ctx, local, dst, hash2))
		actual := getTree(ctx, t, dst)
		assertdeep.Equal(t, map[string]fileInfo{
			filepath.Join(".", "myfile"): fileInfo{
				Name: "myfile",
				Mode: 0750, // TODO(borenet): Why doesn't this match os.ModePerm?
				Hash: f1Hash,
			},
			filepath.Join(".", "mydir", "other"): fileInfo{
				Name: "other",
				Mode: 0750,
				Hash: f2Hash,
			},
			filepath.Join(".", "mydir", "other2"): fileInfo{
				Name: "other2",
				Mode: 0750,
				Hash: f3Hash,
			},
	...
	// UploadItems.
	{
		...
		hash, err := UploadItems(ctx, local, src, items)
		require.NoError(t, err)
		dst, cleanupDst := testutils.TempDir(t)
		defer cleanupDst()
		require.NoError(t, Download(ctx, local, dst, hash))
