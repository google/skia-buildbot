...
type workPublisher interface {
	PublishWork(ctx context.Context, grouping paramtools.Params, left []types.Digest) error
}
...
func (p *pubsubDiffPublisher) PublishWork(ctx context.Context, grouping paramtools.Params, left []types.Digest) error {
	body, err := json.Marshal(diff.WorkerMessage{
		Version:         diff.WorkerMessageVersion,
		Grouping:        grouping,
		AdditionalLeft:  left,
		AdditionalRight: nil, // We don't need to specify anything additional on the right
	})
	if err != nil {
