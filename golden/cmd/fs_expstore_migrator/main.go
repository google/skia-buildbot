...
func (s *sqlDBV1) storeTriageRecords(ctx context.Context, records map[string][]v3TriageRecord) error {
	...
	return util.ChunkIter(len(mRecords), chunkSize, func(startIdx int, endIdx int) error {
		...
		statement := "INSERT INTO ExpectationsRecords (user_name, time, num_changes) VALUES "
		...
		statement += " RETURNING expectations_record_id"
...
func (s *sqlDBV1) storeExpectations(ctx context.Context, entriesByPartition map[string][]v3ExpectationEntry) error {
