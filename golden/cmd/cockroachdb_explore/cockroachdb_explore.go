...
func writeDiffMetrics(ctx context.Context, db *pgx.Conn) error {
	...
	for _, dbd := range data_kitchen_sink.MakePixelDiffsForCorpusNameGrouping() {
		leftDigestBytes := sqltest.MustDigestToBytes(dbd.LeftDigest)
		rightDigestBytes := sqltest.MustDigestToBytes(dbd.RightDigest)
	...
	_, err := db.Exec(ctx, statements.CreateDiffMetricsClosestView())
	if err != nil {
		return skerr.Wrapf(err, "creating DiffMetricsClosestView")
	}
...
func writePrimaryBranchExpectations(ctx context.Context, db *pgx.Conn) error {
	for _, tle := range data_kitchen_sink.MakePrimaryBranchTriageHistory() {
		...
		for _, delta := range tle.Details {
			...
			digestBytes := sqltest.MustDigestToBytes(delta.Digest)
			labelAfterInt := 0
...
INSERT INTO Traces (trace_id, grouping_id, keys) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
		...
		headBytes := sqltest.MustDigestToBytes(trace.AtHead())
...
(trace_id, most_recent_commit_id, grouping_id, keys, expectation_label, digest, options_id) VALUES
($1, 0, $2, $3, $4, $5, $6)
ON CONFLICT DO NOTHING`,
			traceID, groupingID, keysJSON, sql.LabelUntriaged, headBytes, optionsID)
		if err != nil {
			return skerr.Wrapf(err, "inserting grouping for trace %s", tp.ID)
		}
...
UPSERT INTO SourceFiles (source_file_id, source_file, last_ingested) VALUES ($1, $2, $3)`,
			sourceFileID, fakeFile, time.Now())
		if err != nil {
			return skerr.Wrapf(err, "inserting source file %s - %s", sourceFileID, fakeFile)
		}
		...
		for i := 0; i < len(trace.Digests); i++ {
			...
			digestBytes := sqltest.MustDigestToBytes(trace.Digests[i])
			commitID := i + 1
...
VALUES ($1, $2, $3, $4, $5, $6, $7)`,
				traceID, shard, commitID, groupingID, digestBytes, optionsID, sourceFileID)
			if err != nil {
...
ON CONFLICT DO NOTHING`,
				groupingID, digestBytes, sql.LabelUntriaged,
			)
			if err != nil {
...
func writeChangelistData(ctx context.Context, db *pgx.Conn) error {
	...
	for _, dataPoint := range data_kitchen_sink.MakeDataFromTryJobs() {
		...
		groupingJSON, groupingID := sql.SerializeMap(grouping)
		digestBytes := sqltest.MustDigestToBytes(dataPoint.Digest)
