...
func writeChangelistExpectations(ctx context.Context, db *pgx.Conn) error {
	for cID, entries := range data_kitchen_sink.MakeChangelistsTriageHistory() {
		...
		for _, tle := range entries {
			row := db.QueryRow(ctx, `
INSERT INTO ExpectationRecords (changelist_id, user_name, time, num_changes)
...
VALUES ($1, $2, $3, $4, $5)`,
				...
				_, err = db.Exec(ctx, `
UPSERT INTO ChangelistExpectations (changelist_id, grouping_id, digest, label, expectation_record_id)
VALUES ($1, $2, $3, $4, $5)`,
...
func writeChangelistParams(ctx context.Context, db *pgx.Conn, keys, options map[string]string, clID, psID string) error {
	insert := "INSERT INTO ChangelistParams (key, value, changelist_id, patchset_id) VALUES "
...
func writeChangelistData(ctx context.Context, db *pgx.Conn) error {
	...
	for _, dataPoint := range data_kitchen_sink.MakeDataFromTryJobs() {
		keysJSON, traceHash := sql.SerializeMap(dataPoint.Keys)
		optsJSON, optsHash := sql.SerializeMap(dataPoint.Options)
		...
		_, err = db.Exec(ctx,
			`INSERT INTO ChangelistTraces (changelist_trace_id, keys) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
			traceHash, keysJSON)
		if err != nil {
			return skerr.Wrapf(err, "inserting keys %s - %s", traceHash, dataPoint.Keys)
		}
		...
		_, err = db.Exec(ctx,
			...
			optsHash, optsJSON)
		if err != nil {
			return skerr.Wrapf(err, "inserting options %s - %s", optsHash, dataPoint.Options)
		}
		...
		_, err = db.Exec(ctx,
			`UPSERT INTO ChangelistValues (changelist_trace_id, tryjob_id, digest, grouping_id, options_id,
         changelist_id, patchset_id, source_file_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
			traceHash, dataPoint.TryJobID, digestBytes, groupingHash, optsHash,
		...
		err = writeToChangelistExpectationsUnlessInExpectations(ctx, db, crsCLID, groupingHash, digestBytes)
		if err != nil {
...
func updateIgnoredTraces(ctx context.Context, db *pgx.Conn) error {
	...
	clStatement := `UPDATE ChangelistTraces SET matches_any_ignore_rule = `
