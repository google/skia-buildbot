...
func main() {
	...
	must(writeChangelistData(ctx, db))
...
func writeDiffMetrics(ctx context.Context, db *pgx.Conn) error {
	...
	for _, dbd := range data_kitchen_sink.MakePixelDiffsForCorpusNameGrouping() {
		...
		_, err = db.Exec(ctx,
         ...
         max_channel_diff, max_rgba_diff, dimensions_differ, combined_metric, updated_ts)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9), ($2, $1, $3, $4, $5, $6, $7, $8, $9)`,
			leftDigestBytes, rightDigestBytes, m.NumDiffPixels, m.PixelDiffPercent,
			util.MaxInt(m.MaxRGBADiffs[:]...), m.MaxRGBADiffs[:], m.DimDiffer, m.CombinedMetric,
			now,
		)
		if err != nil {
...
func writePrimaryBranchExpectations(ctx context.Context, db *pgx.Conn) error {
	for _, tle := range data_kitchen_sink.MakePrimaryBranchTriageHistory() {
		...
		for _, delta := range tle.Details {
			groupingJSON, groupingHash := sql.SerializeMap(delta.Grouping)
...
VALUES ($1, $2, $3, $4)`,
				groupingHash, digestBytes, labelAfterInt, recordUUID,
			)
			if err != nil {
			...
			_, err = db.Exec(ctx, `
UPDATE ValuesAtHead
SET expectation_label = $3, expectation_record_id = $4
WHERE grouping_id = $1 and digest = $2
RETURNING NOTHING`,
				groupingHash, digestBytes, labelAfterInt, recordUUID,
			)
			if err != nil {
				return skerr.Wrapf(err, "writing expectations to head table %s", delta)
			}
		}
	}
	return nil
...
func writeChangelistExpectations(ctx context.Context, db *pgx.Conn) error {
	for cID, entries := range data_kitchen_sink.MakeChangelistsTriageHistory() {
		...
		for _, tle := range entries {
			row := db.QueryRow(ctx, `
INSERT INTO ExpectationRecords (branch_name, user_name, time, num_changes)
  VALUES ($1, $2, $3, $4) RETURNING expectation_record_id`,
...
VALUES ($1, $2, $3, $4, $5)`,
					crsCLID, groupingHash, digestBytes, labelAfterInt, recordUUID,
				)
				if err != nil {
					return skerr.Wrapf(err, "writing expectations %s", delta)
				}
...
func writePrimaryBranchTraceData(ctx context.Context, db *pgx.Conn) error {
	...
	for _, tp := range data_kitchen_sink.MakeTraces() {
		...
		optsJSON, optsHash := sql.SerializeMap(trace.Options())
		grouping := groupingFor(trace.Keys())
		groupingJSON, groupingHash := sql.SerializeMap(grouping)

		_, err := db.Exec(ctx, `
INSERT INTO Traces (trace_id, grouping_id, keys) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
			traceHash, groupingHash, keysJSON)
		if err != nil {
		...
		_, err = db.Exec(ctx, `INSERT INTO ValuesAtHead
(trace_id, most_recent_commit_id, grouping_id, keys, expectation_label) VALUES
($1, 0, $2, $3, $4)
ON CONFLICT DO NOTHING`,
			traceHash, groupingHash, keysJSON, sql.LabelUntriaged)
		if err != nil {
			return skerr.Wrapf(err, "inserting grouping for trace %s", tp.ID)
		}
...
ON CONFLICT DO NOTHING`,
			...
			_, err = db.Exec(ctx, `UPDATE Commits SET has_data = true WHERE commit_id = $1 RETURNING NOTHING`,
				commitID,
			)
			if err != nil {
...
WHERE trace_id = $1
RETURNING NOTHING`,
				traceHash, commitID, digestBytes,
			)
			if err != nil {
				return skerr.Wrapf(err, "writing most recent commit_id %d (digest %x) to trace %s", commitID, digestBytes, keysJSON)
			}
		}
...
func writePrimaryParams(ctx context.Context, db *pgx.Conn, keys, options map[string]string, commitID int) error {
	...
	insert += sql.ValuesPlaceholders(valuesPerRow, numRows)
...
func writeChangelistParams(ctx context.Context, db *pgx.Conn, keys, options map[string]string, clID, psID string) error {
	insert := "INSERT INTO SecondaryBranchParams (key, value, branch_name, version_name) VALUES "
...
func writeChangelistData(ctx context.Context, db *pgx.Conn) error {
	...
	for _, dataPoint := range data_kitchen_sink.MakeDataFromTryJobs() {
		keysJSON, traceID := sql.SerializeMap(dataPoint.Keys)
		optsJSON, optionsID := sql.SerializeMap(dataPoint.Options)
		grouping := groupingFor(dataPoint.Keys)
		groupingJSON, groupingID := sql.SerializeMap(grouping)
		digestBytes, err := sql.DigestToBytes(dataPoint.Digest)
		if err != nil {
			return skerr.Wrap(err)
		}

		_, err = db.Exec(ctx, `
INSERT INTO SecondaryBranchTraces (secondary_branch_trace_id, grouping_id, keys)
VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
			traceID, groupingID, keysJSON)
		if err != nil {
			return skerr.Wrapf(err, "inserting keys %s - %s", traceID, dataPoint.Keys)
		}
		...
		_, err = db.Exec(ctx,
			...
			optionsID, optsJSON)
		if err != nil {
			return skerr.Wrapf(err, "inserting options %s - %s", optionsID, dataPoint.Options)
		}

		_, err = db.Exec(ctx,
			`INSERT INTO Groupings (grouping_id, keys) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
			groupingID, groupingJSON)
		if err != nil {
			return skerr.Wrapf(err, "inserting grouping %s - %s", groupingID, grouping)
		}
		...
		crsCLID := qualifyIDWithSystem(dataPoint.PatchSet.CRS, dataPoint.PatchSet.CL)
		crsPSID := qualifyIDWithSystem(dataPoint.PatchSet.CRS, dataPoint.PatchSet.PS)
		_, err = db.Exec(ctx, `
UPSERT INTO SecondaryBranchValues (secondary_branch_trace_id, tryjob_id, digest, grouping_id, options_id,
         branch_name, version_name, source_file_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
			traceID, dataPoint.TryJobID, digestBytes, groupingID, optionsID,
			crsCLID, crsPSID, sourceFileHash[:])
		if err != nil {
			return skerr.Wrapf(err, "inserting data for CL/PS %#v", dataPoint.PatchSet)
		}

		err = writeChangelistParams(ctx, db, dataPoint.Keys, dataPoint.Options, crsCLID, crsPSID)
		if err != nil {
			return skerr.Wrapf(err, "inserting params for CL/PS %#v", dataPoint.PatchSet)
		}

		//err = writeToChangelistExpectationsUnlessInExpectations(ctx, db, crsCLID, groupingID, digestBytes)
		//if err != nil {
...
// writeToChangelistExpectationsUnlessInExpectations writes a sentinel value to
// ChangelistExpectations iff there is not already an entry in the primary branch's Expectations.
// The reason for this conditional write is to reduce the size of the ChangelistExpectations
// Table. An ingester should probably have a local cache of grouping+digests to speed this check up.
func writeToChangelistExpectationsUnlessInExpectations(ctx context.Context, db *pgx.Conn, crsCLID string, groupingHash []byte, digestBytes []byte) error {
	// This sentinel value defaults to the primary branch's label (if any) otherwise, it is
	// considered untriaged. We don't need to do this in a transaction because there are other ways
	// for Expectations and ChangelistExpectations to have entries for the same digest/grouping (e.g.
	// a digest shows up first in a CL and then on the primary branch before the CL lands). The
	// check is just to remove *most* of the redundancy.

	row := db.QueryRow(ctx, `SELECT COUNT(*) FROM Expectations WHERE grouping_id = $1 and digest = $2;`,
		groupingHash, digestBytes,
	)
	c := 0
	err := row.Scan(&c)
	if c != 0 {
		return nil // no need to write the sentinel value
	}

	_, err = db.Exec(ctx, `
INSERT INTO SecondaryBranchExpectations (branch_name, grouping_id, digest, label)
VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`,
		crsCLID, groupingHash, digestBytes, sql.FallbackToPrimaryBranch,
	)
	if err != nil {
		return skerr.Wrapf(err, "writing sentinel value expectations (if needed) for %s-%x-%x", crsCLID, groupingHash, digestBytes)
	}
	return nil
}
...
func writeIgnoreRules(ctx context.Context, db *pgx.Conn) error {
	...
	insert += sql.ValuesPlaceholders(valuesPerRow, len(rules))
...
func updateIgnoredTraces(ctx context.Context, db *pgx.Conn) error {
	...
	statement += condition
	statement += `WHERE matches_any_ignore_rule IS NULL RETURNING NOTHING;`
	_, err = tx.Exec(ctx, statement, arguments...)
	if err != nil {
		return skerr.Wrapf(err, "Updating primary traces with statement: %s", statement)
	}

	clStatement := `UPDATE SecondaryBranchTraces SET matches_any_ignore_rule = `
	clStatement += condition
	clStatement += `WHERE matches_any_ignore_rule IS NULL RETURNING NOTHING;`
	_, err = tx.Exec(ctx, clStatement, arguments...)
	if err != nil {
		return skerr.Wrapf(err, "Updating changelist traces with statement: %s", clStatement)
	}

	headStatement := `UPDATE ValuesAtHead SET matches_any_ignore_rule = `
	headStatement += condition
	headStatement += `WHERE matches_any_ignore_rule IS NULL RETURNING NOTHING;`
