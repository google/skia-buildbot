...
RankedDiffs AS (
	...
	from DiffMetrics
	where left_digest > x'00' and left_digest < x'0001'
),
TopTenDiffs AS (
	SELECT * FROM RankedDiffs where diffRank <= 100
)
...
WITH
DiffsWithLabels AS (
    SELECT DISTINCT DiffMetrics.*, max(label) OVER(PARTITION BY right_digest) AS max_label
    FROM DiffMetrics
    INNER LOOKUP JOIN Expectations ON Expectations.digest = DiffMetrics.right_digest
    WHERE left_digest > x'00' and left_digest < x'0001'
),
RankedDiffs AS (
