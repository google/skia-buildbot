...
> SELECT encode(keys_hash, 'hex'), jsonb_pretty(keys) FROM Traces where keys @> '{"color mode": "GREY", "name": "triangle"}';
> SELECT keys FROM Traces WHERE keys_hash = x'47109b059f45e4f9d5ab61dd0199e2c9';
> SELECT commit_number, encode(digest, 'hex') FROM TraceValues WHERE trace_id = x'47109b059f45e4f9d5ab61dd0199e2c9';

# Get trace data for grey triangle traces
> SELECT encode(TraceValues.trace_id, 'hex') AS trace, TraceValues.commit_number, 
  encode(TraceValues.digest, 'hex') AS digest
FROM
  TraceValues
JOIN
  (SELECT trace_id FROM Traces 
   WHERE Traces.keys @> '{"color mode": "GREY", "name": "triangle"}') AS Traces
