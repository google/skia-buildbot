...
INNER LOOKUP JOIN
  (SELECT trace_id, digest, grouping_id FROM TraceValues@grouping_digest_commit_idx 
   WHERE commit_id > 0) AS TraceValues -- This range is just to show it possible
ON TraceValues.grouping_id = Expectations.grouping_id
  AND TraceValues.digest = Expectations.digest
INNER LOOKUP JOIN 
...
SELECT encode(TraceValues.digest, 'hex'), Traces.keys FROM
  (SELECT grouping_id, digest FROM Expectations@label_idx
   WHERE label = 0) AS Expectations
JOIN
  (SELECT digest, grouping_id, trace_id, commit_id FROM TraceValues@grouping_digest_commit_idx
...
FROM
  (SELECT digest FROM TraceValues@grouping_commit_digest_idx 
...
SELECT DISTINCT ON (left_digest, label)
  ...
  num_diff_pixels, max_rgba_diff, dimensions_differ, ExpectationRecords.user_name
FROM
  (SELECT digest, trace_id FROM TraceValues@grouping_commit_digest_idx 
   WHERE TraceValues.commit_id > 0 
     AND TraceValues.grouping_id = x'aa8d3c14238a4f717b9a99f7fe3735a7') AS TraceValues
JOIN
...
SELECT corpus, name, label, count(label) FROM (
  ...
  FROM
   Expectations
  INNER LOOKUP JOIN
    (SELECT trace_id, grouping_id, digest FROM TraceValues@grouping_digest_commit_idx
...
ProbableMatchesFromCL AS (
  ...
  END AS label
   FROM
   (SELECT grouping_id, digest, label FROM ChangelistExpectations@changelist_label_idx
...
ExpectationsFromPrimaryBranch AS (
  ...
  LEFT JOIN
    (SELECT grouping_id, digest FROM ChangelistExpectations@changelist_label_idx
...
JOIN
  (SELECT keys, changelist_trace_id FROM ChangelistTraces@ignored_idx
