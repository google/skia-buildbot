...
// instance/service pair that will be deployed.
func (g *Goldpushk) regenerateConfigFiles(ctx context.Context) error {
	...
	return g.forAllDeployableUnits(func(unit DeployableUnit) error {
		...
		if unit.configMapTemplate != "" {
			if err := g.runKubeConfGen(ctx, unit.Instance, unit.getConfigMapFileTemplatePath(g.skiaInfraRootPath), unit.getConfigMapFilePath(g.skiaInfraRootPath)); err != nil {
