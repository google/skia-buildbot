...
func (g *Goldpushk) getUptimesSingleCluster(ctx context.Context, units []DeployableUnit, now time.Time) (map[DeployableUnitID]time.Duration, error) {
	...
	jsonPathExpr += "{'app:'}"
	jsonPathExpr += "{.metadata.labels.app}"
	jsonPathExpr += "{'  podName:'}"
	jsonPathExpr += "{.metadata.name}"
	jsonPathExpr += "{'  ready:'}"
	jsonPathExpr += "{.status.conditions[?(@.type == 'Ready')].status}"
	jsonPathExpr += "{'  readyLastTransitionTime:'}"
	jsonPathExpr += "{.status.conditions[?(@.type == 'Ready')].lastTransitionTime}"
	jsonPathExpr += "{'\\n'}"
	jsonPathExpr += "{end}"
