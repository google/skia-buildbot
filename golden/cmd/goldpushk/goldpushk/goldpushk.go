...
func (g *Goldpushk) monitor(ctx context.Context, units []DeployableUnit, getUptimes uptimesFn, sleep sleepFn) error {
	...
	// consists of three columns: UPTIME, READY and NAME. The first two are 10 characters wide each.
	...
	// no monitor, e.g. gold-skia-diffserver.
	longestName := 0
	for _, unit := range units {
	...
	for {
		...
		for _, unit := range units {
			...
			// We now check if it does have an uptime, and update the variables above accordingly.
