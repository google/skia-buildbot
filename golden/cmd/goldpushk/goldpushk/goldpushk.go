...
func (g *Goldpushk) Run(ctx context.Context) error {
	...
	// Deploy canaries.
	if err := g.deployCanaries(ctx); err != nil {
		return err
	}
	...
	if err := g.deployServices(ctx); err != nil {
		return err
	}
...
// deployCanaries deploys the canaried deployable units.
func (g *Goldpushk) deployCanaries(ctx context.Context) error {
	if len(g.canariedDeployableUnits) == 0 {
		return nil
	}
...
// deployCanaries deploys the non-canaried deployable units.
func (g *Goldpushk) deployServices(ctx context.Context) error {
	if len(g.canariedDeployableUnits) == 0 {
