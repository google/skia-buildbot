...
type Goldpushk struct {
	...
	// A stub-able function that copies the instance configurations to the checkout directory.
	copyConfigsToCheckout func(instanceConfigDir string, gitCheckoutDir string) error
}
...
func New(deployableUnits []DeployableUnit, canariedDeployableUnits []DeployableUnit, skiaInfraRootPath string, dryRun, noCommit bool, minUptimeSeconds, uptimePollFrequencySeconds int, k8sConfigRepoUrl string) *Goldpushk {
	return &Goldpushk{
		...
		copyConfigsToCheckout: defaultCopyConfigsToCheckout,
	}
}
...
// defaultCopyConfigsToCheckout copies all json5 configurations from the provided directory
// into the given checkout directory. Upon copying, the files will have the prefix "gold-".
func defaultCopyConfigsToCheckout(configDir, checkoutDir string) error {
