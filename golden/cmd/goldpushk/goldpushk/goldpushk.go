...
//
// If the DeployableUnit has a ConfigMap file (e.g.
// gold-skiapublic-skiacorrectness) this will be a path inside $SKIA_INFRA_ROOT.
//
...
func (g *Goldpushk) getConfigMapFilePath(unit DeployableUnit) (string, bool) {
	switch {
	case unit.configMapFile != "":
		return filepath.Join(g.rootPath, unit.configMapFile), true
	case unit.configMapName != "" && unit.configMapTemplate != "":
