...
type DeploymentOptions struct {
	...
	// If configMapName is empty, then configMapName and configMapTemplate should
	// be empty as well. Otherwise only one of them should be set. Both fields are
...
func (u *DeployableUnit) getDeploymentFileTemplatePath(goldenRootPath string) string {
	return filepath.Join(goldenRootPath, k8sConfigTemplatesDir, fmt.Sprintf("gold-%s-template.yaml", u.Service))
}
