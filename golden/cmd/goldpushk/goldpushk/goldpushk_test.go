...
func TestGoldpushkCheckOutGitRepositories(t *testing.T) {
	...
	// Create two fake "skia-public-config" and "skia-corp-config" Git repos
	...
	// Create goldpushk instance under test. We pass it the file://... URLs to the
	// two Git repositories created above.
	g := New([]DeployableUnit{}, []DeployableUnit{}, "", false, fakeSkiaPublicConfig.RepoUrl(), fakeSkiaCorpConfig.RepoUrl())

	// Check out repositories. This will clone the two above repositories with
	// "git clone file://...".
	...
	// earlier. This just a basic sanity check.
	...
	// Assert that the contents of file "which-repo.txt" on each clone matches the
	// contents of the same file on the corresponding origin repository.
