...
func TestNew(t *testing.T) {
	...
	expected := &Goldpushk{
		...
		rootPath:                   "path/to/buildbot/golden",
		dryRun:                     true,
		noCommit:                   true,
		minUptimeSeconds:           30,
...
func TestGoldpushkGetConfigMapFilePath(t *testing.T) {
	...
	// Create the goldpushk instance under test.
	const goldenRoot = "/path/to/buildbot/golden"
	g := Goldpushk{
		rootPath: goldenRoot,
	}
	addFakeK8sConfigRepoCheckout(&g)
	...
	assertConfigMapFileEquals(publicUnitWithConfigMapFile, goldenRoot, "k8s-instances/skia-public/authorized-params.json5")
