...
func TestGoldpushkGetDeploymentFilePath(t *testing.T) {
	...
	// Make goldpushk clone the repositories above. This needs to happen before
	// we call Goldpushk.getDeploymentFilePath(), as it will return a path into
	// the corresponding cloned repository.
	err := g.checkOutGitRepositories(ctx)
	assert.NoError(t, err)

	// Gather the DeployableUnits we will call Goldpushk.getDeploymentFilePath()
	// with.
...
func TestGoldpushkGetConfigMapFilePath(t *testing.T) {
	...
	g := New([]DeployableUnit{}, []DeployableUnit{}, skiaInfraRoot, false, fakeSkiaPublicConfig.RepoUrl(), fakeSkiaCorpConfig.RepoUrl())

	// Make goldpushk clone the repositories above. This needs to happen before
	// we call Goldpushk.getConfigMapFilePath(), as it will return a path into the
	// corresponding cloned repository.
	err := g.checkOutGitRepositories(ctx)
	assert.NoError(t, err)

	// Gather the DeployableUnits we will call Goldpushk.getConfigMapFilePath()
	// with.
	s := BuildDeployableUnitSet()
...
func TestRegenerateConfigFiles(t *testing.T) {
	...
	// Create two fake "skia-public-config" and "skia-corp-config" Git repos on
	// the local file system (i.e. "git init" two temporary directories).
	fakeSkiaPublicConfig := testutils.GitInit(t, context.Background())
	fakeSkiaCorpConfig := testutils.GitInit(t, context.Background())
	defer fakeSkiaPublicConfig.Cleanup()
	defer fakeSkiaCorpConfig.Cleanup()
	...
	// Make goldpushk clone the repositories above. This needs to happen before
	// we call Goldpushk.regenerateConfigFiles(), as it will write out the
	// regenerated configuration files into said repositories.
	err := g.checkOutGitRepositories(context.Background())
	assert.NoError(t, err)
