...
func TestGetContainersUptimeSeconds(t *testing.T) {
	unittest.SmallTest(t)
...
`
	...
	// Call code under test.
	uptime, err := g.getContainersUptimeSeconds(commandCollectorCtx, units, now)
	assert.NoError(t, err)

	// Assert that we get the expected uptimes.
	assert.Len(t, uptime, 2)
	assert.Equal(t, uptime[makeID(Chrome, BaselineServer)], int64(29))     // 17:58:02 - 17:57:33
	assert.Equal(t, uptime[makeID(ChromeGPU, BaselineServer)], int64(159)) // 17:58:02 - 17:55:23
}
