...
func main() {
	...
	delay := int(*flagMinDelay) + rand.Intn(int(*flagMaxDelay)-int(*flagMinDelay)+1)
	sklog.Infof("Sleeping for %d seconds before starting healthy_server.", time.Duration(delay)/time.Second)
