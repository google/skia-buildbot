...
func main() {
	...
	usageActualDelayStr := "Actual delay will be a random number of seconds between --min_delay and --max_delay."
	flagMinDelay := flag.Int("min_delay", 0, "Minimum delay in seconds before launching the health check server (default: 0). " + usageActualDelayStr)
	flagMaxDelay := flag.Int("max_delay", 0, "Minimum delay in seconds before launching the health check server (default: 0). " + usageActualDelayStr)
	flag.Parse()
	if *flagInstance == "" {
	...
	if *flagMinDelay > *flagMaxDelay {
		sklog.Fatalf("Flag --min_delay should be less than --max_delay (given values are %d and %d, respectively).", *flagMinDelay, *flagMaxDelay)
	}
	...
	delay := *flagMinDelay + r.Intn(*flagMaxDelay - *flagMinDelay + 1)
	sklog.Infof("Sleeping for %d seconds before starting healthy_server.", delay)
