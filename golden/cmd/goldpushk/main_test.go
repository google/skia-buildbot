...
func TestParseAndValidateFlagsErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		_, _, err := parseAndValidateFlags(goldpushk.BuildProductionDeployableUnitSet(), tc.flagInstances, tc.flagServices, tc.flagCanaries)
...
func TestParseAndValidateFlagsSuccess(t *testing.T) {
	...
	for _, tc := range testCases {
		deployableUnits, canariedDeployableUnits, err := parseAndValidateFlags(goldpushk.BuildProductionDeployableUnitSet(), tc.flagInstances, tc.flagServices, tc.flagCanaries)
...
func TestParseAndValidateFlagsTestingSuccess(t *testing.T) {
	...
	for _, tc := range testCases {
		deployableUnits, canariedDeployableUnits, err := parseAndValidateFlags(goldpushk.BuildTestingDeployableUnitSet(), tc.flagInstances, tc.flagServices, tc.flagCanaries)
		deployableUnitIDs := mapUnitsToIDs(deployableUnits)
		canariedDeployableUnitIDs := mapUnitsToIDs(canariedDeployableUnits)
