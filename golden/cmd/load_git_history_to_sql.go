...
const createCommitTableSQL = `
CREATE TABLE IF NOT EXISTS Commits (
  -- The commit_number; a monotonically increasing number as we follow the primary branch through time.
...
func main() {
	...
	dbCmd := exec.Command("cockroach", "sql", "--insecure", "--host=localhost:26257",
		"--database="+*dbName,
		"--execute="+createCommitTableSQL)
	out, err := dbCmd.CombinedOutput()
	if err != nil {
		sklog.Fatalf("Could not create table: %s %s", err, out)
	}
	...
	conf, err := pgx.ParseConfig("postgresql://root@localhost:26257/" + *dbName + "?sslmode=disable")
	if err != nil {
	...
	err = util.ChunkIter(len(commitsToStore), chunkSize, func(startIdx int, endIdx int) error {
		...
		upsertStatement := "UPSERT INTO Commits (commit_number, git_hash, commit_time, author, subject) VALUES "
		...
		for i, c := range batch {
			...
			arguments = append(arguments, c.CommitNumber)
