...
type frontendServerConfig struct {
	...
	// Should Gold track ChangeLists looking for ChangeListExpectations.
	CLTracking bool `json:"cl_tracking"`
	...
	// Interval at which the indexer calculates the search index.
	IndexInterval config.Duration `json:"index_interval"`
	...
	// Auth2 redirect url. Only used when in production
	...
	// How often to re-fetch the tile. TODO(kjlubick) explain how this differs from IndexInterval.
...
func main() {
	...
	// Set up login
	useRedirectURL := fsc.RedirectURL
	if fsc.Local {
		useRedirectURL = fmt.Sprintf("http://localhost%s/oauth2callback/", fsc.Port)
	}
	sklog.Infof("The allowed list of users is: %q", fsc.AuthorizedUsers)
	if err := login.Init(useRedirectURL, strings.Join(fsc.AuthorizedUsers, " "), fsc.ClientSecretFile); err != nil {
	...
	useRepoURL := fsc.GitRepoURL
	if foundRepoURL, ok := bt_gitstore.RepoURLFromID(ctx, btConf, fsc.GitRepoURL); ok {
	...
	if !fsc.ForceLogin && publiclyViewableParams == nil {
		sklog.Fatalf(" A non-empty map of publiclyViewableParams must be provided if force_login=false.")
	}

	// openSite indicates whether this can expose all end-points. The user still has to be
	// authenticated.
	openSite := (publiclyViewableParams == nil) || fsc.ForceLogin
...
type frontendConfig struct {
	...
	IsPublic           bool   `json:"isPublic" optional:"true"` // If true this is not open but restrictions apply.
}
