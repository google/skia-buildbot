...
func main() {
	...
	// Set up a subrouter for the '/json' routes. This makes routing faster,
	// but also returns a failure when an /json route is requested that doesn't exit.
	jsonRouter := router.PathPrefix("/json").Subrouter()
	trim := func(r string) string { return strings.TrimPrefix(r, "/json") }
	// trim := func(r string) string { return r }
	...
	jsonRouter.HandleFunc(trim(web.EXPECATIONS_ROUTE), handlers.JsonBaselineHandler).Methods("GET")
	jsonRouter.HandleFunc(trim(web.EXPECATIONS_ISSUE_ROUTE), handlers.JsonBaselineHandler).Methods("GET")
	...
	jsonRouter.PathPrefix("/json").HandlerFunc(http.NotFound)
