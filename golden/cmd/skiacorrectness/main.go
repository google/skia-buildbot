...
func main() {
	...
	go func() {
		...
		if *eventTopic != "" {
			...
			evt, err = gevent.New(*projectID, *eventTopic, nodeName, evtBusTriggerCh, option.WithTokenSource(tokenSource))
			if err != nil {
		...
		// Start receiving events if we have a distributed event bus.
