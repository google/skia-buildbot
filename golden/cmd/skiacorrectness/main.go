...
func main() {
	...
	// // FIXME(kjlubick): delete all this code before landing
	sklog.Infof("hello")
	// t := timer.New("get_tile")
	// tile, _, err := traceStore.GetTile(context.Background(), *nCommits)
	// sklog.Errorf("err: %v", err)
	// sklog.Infof("num traces: %d", len(tile.Traces))
	// t.Stop()
	//select {}
	for {
		t := timer.New("get_dense_tile")
		tile, _, err := traceStore.GetDenseTile(context.Background(), *nCommits)
		if err != nil {
			sklog.Errorf("err: %s", err)
		} else {
			sklog.Infof("num traces: %d", len(tile.Traces), err)
		}
		t.Stop()
		//select {}
		//break
