...
func main() {
	...
	v1JSON := func(rpcRoute string, handlerFunc http.HandlerFunc) *mux.Route {
		return versionedRPC(rpcRoute, "v1", handlerFunc, jsonRouter, false)
	}
	...
	v1JSON(trim(shared.KnownHashesRoute), handlers.TextKnownHashesProxy).Methods("GET")
	...
	v1JSON(trim(shared.ExpectationsRoute), handlers.BaselineHandler).Methods("GET")
	...
	v1Root := func(rpcRoute string, handlerFunc http.HandlerFunc) *mux.Route {
		return versionedRPC(rpcRoute, "v1", handlerFunc, rootRouter, true)
	}
...
// versionedRPC sets up a route on the given router with a wrapper to count the number of calls to the given
// versioned endpoint. A versioned endpoint looks like /json/v1/what/ever.
