...
import (
	...
	"go.skia.org/infra/golden/go/clstore/duelclstore"
...
func mustInitializeReviewSystems(fsc *frontendServerConfig, fc *firestore.Client, hc *http.Client, db *pgxpool.Pool) []clstore.ReviewSystem {
	...
	for _, cfg := range fsc.CodeReviewSystems {
		...
		sqlCS := sqlclstore.New(db, cfg.ID)
		rs = append(rs, clstore.ReviewSystem{
			ID:          cfg.ID,
			Client:      crs,
			Store:       duelclstore.New(fireCS, sqlCS),
