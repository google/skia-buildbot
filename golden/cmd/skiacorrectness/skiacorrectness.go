...
func main() {
	...
	maybeMustStartCommenters(ctx, fsc, reviewSystems, searchAPI)

	statusWatcher := mustMakeStatusWatcher(ctx, vcs, expStore, expChangeHandler, tileSource)

	maybeMustStartExpectationsCleanupProcess(ctx, fsc, expStore, ixr)
...
// maybeMustStartDebugServer starts an internal HTTP server for debugging purposes if requested.
func maybeMustStartDebugServer(fsc *frontendServerConfig) {
...
// maybeMustStartCommenters starts a background process that comments on CLs for each of the review
...
func maybeMustStartCommenters(ctx context.Context, fsc *frontendServerConfig, reviewSystems []clstore.ReviewSystem, searchAPI search.SearchAPI) {
	if fsc.IsAuthoritative() && !fsc.DisableCLTracking {
...
// maybeMustStartExpectationsCleanupProcess starts a process that will garbage-collect any stale
// expectations, unless the Gold instance is not authoritative (e.g. when running locally).
func maybeMustStartExpectationsCleanupProcess(ctx context.Context, fsc *frontendServerConfig, expStore *fs_expectationstore.Store, ixr *indexer.Indexer) {
