...
func mustMakeRootRouter(fsc *frontendServerConfig, handlers *web.Handlers, diffStore diff.DiffStore) *mux.Router {
	...
	//
	// TODO(lovisolo): It would be nice to eventually fold this and the /dist resource handler into
	//                 one, in a manner that's independent from Webpack and that plays nicely with the
	//                 Bazel build.
