...
func main() {
	...
	for _, crs := range crsNames {
		...
		for {
			...
			if err := storePSsToSQL(ctx, db, crs, xcl, patchsetsPerCL); err != nil {
...
func storePSsToSQL(ctx context.Context, db *pgxpool.Pool, crs string, xcl []code_review.Changelist, patchsets [][]code_review.Patchset) error {
	const statement = `
  ...
  commented_on_cl, last_checked_if_comment_necessary) VALUES `
	...
	for i, cl := range xcl {
		for _, ps := range patchsets[i] {
			totalPS++
			arguments = append(arguments, qualify(crs, ps.SystemID), crs, qualify(crs, cl.SystemID),
