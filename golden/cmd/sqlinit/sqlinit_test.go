...
func TestGetSchedules_AllCadencesSet_Success(t *testing.T) {
	...
	schedules := getSchedules(testTable{}, "testdbname")
...
INTO 'gs://skia-gold-sql-backups/testdbname/daily'
    RECURRING '@daily'
    FULL BACKUP ALWAYS;
CREATE SCHEDULE testdbname_weekly
FOR BACKUP TABLE testdbname.TableTwo
INTO 'gs://skia-gold-sql-backups/testdbname/weekly'
    RECURRING '@weekly'
    FULL BACKUP ALWAYS;
CREATE SCHEDULE testdbname_monthly
FOR BACKUP TABLE testdbname.TableFour
INTO 'gs://skia-gold-sql-backups/testdbname/monthly'
    RECURRING '@monthly'
    FULL BACKUP ALWAYS;
`)
}
...
func TestGetSchedules_MissingCadence_Panics(t *testing.T) {
	...
	assert.Panics(t, func() {
		getSchedules(tableMissingFrequency{}, "testdbname")
	})
}
