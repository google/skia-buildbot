...
func main() {
	...
	var sourcesToScan []ingestion.GCSSource
	...
	sourcesToScan = append(sourcesToScan, ingestion.GCSSource{
		Client: gcsClient,
		Bucket: primaryConf.Sources[0].Bucket,
		Prefix: primaryConf.Sources[0].Prefix,
	})
	primaryBranchProcessor, err := ingestion_processors.PrimaryBranchBigTable(ctx, vcs, primaryConf, &sourcesToScan[0])
	if err != nil {
...
func startBackupPolling(ctx context.Context, isc ingestionServerConfig, sourcesToScan []ingestion.GCSSource, pss *pubSubSource) {
...
func getTimesToPoll(_ context.Context, duration time.Duration) (time.Time, time.Time) {
	// TODO(kjlubick) get time from context for tests
	now := time.Now()
