...
func main() {
	...
	logOpts := []common.Opt{
		common.PrometheusOpt(&dsc.PromPort),
	}

	common.InitWithMust("diffWorker", logOpts...)
	ctx := context.Background()

	db := mustInitSQLDatabase(ctx, dsc)
	gis := mustMakeGCSImageSource(ctx, dsc)
...
func mustInitSQLDatabase(ctx context.Context, dsc diffCalculatorConfig) *pgxpool.Pool {
	if dsc.SQLDatabaseName == "" {
		sklog.Fatalf("Must have SQL Database Information")
	}
	url := sql.GetConnectionURL(dsc.SQLConnection, dsc.SQLDatabaseName)
	...
	sklog.Infof("Connected to SQL database %s", dsc.SQLDatabaseName)
	return db
}

func mustMakeGCSImageSource(ctx context.Context, dsc diffCalculatorConfig) worker.ImageSource {
...
func listen(ctx context.Context, dsc diffCalculatorConfig, subscriberID string, p processor) error {
	...
	t := psc.Topic(dsc.DiffWorkTopic)
	if exists, err := t.Exists(ctx); err != nil {
		return skerr.Wrapf(err, "checking for existing topic %s", dsc.DiffWorkTopic)
	} else if !exists {
