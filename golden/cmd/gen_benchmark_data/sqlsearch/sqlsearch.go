...
func main() {
	...
	err = loadCaches(ctx, db)
	if err != nil {
		sklog.Fatalf("Loading caches %s", err)
	}
...
var groupingCache = map[sql.MD5Hash]string{}
var optionsCache = map[sql.MD5Hash]string{}

func loadCaches(ctx context.Context, db *pgxpool.Pool) error {
	defer timer.New("load caches").Stop()
	const groupingStatement = `SELECT grouping_id, keys FROM Groupings;`
	rows, err := db.Query(ctx, groupingStatement)
	if err != nil {
		sklog.Fatalf("Querying grouping %s", err)
	}
	defer rows.Close()

	var id sql.GroupingID
	var hash sql.MD5Hash
	for rows.Next() {
		var keys string
		err := rows.Scan(&id, &keys)
		if err != nil {
			return skerr.Wrapf(err, "Scanning grouping")
		}
		copy(hash[:], id)
		groupingCache[hash] = keys
	}
	rows.Close()

	const optionsStatement = `SELECT options_id, keys FROM Options;`
	rows, err = db.Query(ctx, optionsStatement)
	if err != nil {
		sklog.Fatalf("Querying options %s", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &keys)
		if err != nil {
			return skerr.Wrapf(err, "Scanning option")
		}
		copy(hash[:], id)
		optionsCache[hash] = keys
	}
	rows.Close()
	return nil
}
...
func doUntriagedSearchAtHead(ctx context.Context, db *pgxpool.Pool, corpus string, tileStarts, denseTileWidth int) ([]DigestResult, error) {
	...
	eg.Go(func() error {
		...
		// Limit to the first 50 to emulate loading just the trace history for the first page
		// of results.
		return fillInHistory(ctx, db, preliminaryResults[:50], tileStarts)
	})
	eg.Go(func() error {
...
func findUntriagedUnignored(ctx context.Context, db *pgxpool.Pool, corpus string, tileStarts, denseTileWidth int) ([]DigestResult, error) {
	...
	const statement = `
SELECT trace_id, most_recent_commit_id, digest, keys, 
  expectation_record_id, options_id, grouping_id
FROM ValuesAtHead
...
`
	...
	for _, row := range results {
		...
		newTrace.History[row.commitIdx-tileStarts] = DataPoint{
			Digest:  digest,
			Options: optionsCache[sql.AsMD5Hash(row.optionsID)],
		}
		grouping := groupingCache[sql.AsMD5Hash(row.groupingID)]

		key := digestAndGrouping(digest) + digestAndGrouping(grouping)
