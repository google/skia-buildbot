...
func main() {
	...
	results := doUntriagedSearchAtHead(ctx, db, maxCorpus, tileStarts, denseTileWidth)
...
func findBreakdownByCorpus(ctx context.Context, db *pgxpool.Pool) string {
  ...
  SELECT ValuesAtHead.keys ->> 'source_type' AS corpus FROM ValuesAtHead
...
func doUntriagedSearchAtHead(ctx context.Context, db *pgxpool.Pool, corpus string, tileStarts, denseTileWidth int) []DigestResult {
	// find trace_id, digest, grouping, keys, options_id of untriaged, unignored traces matching the corpus
	preliminaryResults := findUntriagedUnignored(ctx, db, corpus, tileStarts, denseTileWidth)
	// - Get history for trace_ids
	fillInHistory(ctx, db, preliminaryResults, tileStarts)
	// - Get closest diffs for digests based on groupings
	findComparisonDigests(ctx, db, preliminaryResults, tileStarts)

	return preliminaryResults
}

func findUntriagedUnignored(ctx context.Context, db *pgxpool.Pool, corpus string, tileStarts, denseTileWidth int) []DigestResult {
	...
	const statement = `
SELECT trace_id, most_recent_commit_id, digest, ValuesAtHead.keys, expectation_record_id,
  Options.keys, Groupings.grouping_id, Groupings.keys
FROM ValuesAtHead
INNER JOIN Groupings on ValuesAtHead.grouping_id = Groupings.grouping_id
INNER JOIN Options on ValuesAtHead.options_id = Options.options_id
WHERE most_recent_commit_id >= $1 AND expectation_label = 0 AND matches_any_ignore_rule = FALSE AND
ValuesAtHead.keys ->> 'source_type' = $2;
`
	rows, err := db.Query(ctx, statement, tileStarts, corpus)
	if err != nil {
		sklog.Fatalf("fetching from ValuesAtHead %s", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			sklog.Fatalf("reading values at head row %s", err)
		}
		results = append(results, r)
	}
	rows.Close()
...
func fillInHistory(ctx context.Context, db *pgxpool.Pool, results []DigestResult, tileStarts int) {
...
WHERE trace_id IN `
	...
	if err != nil {
		sklog.Fatalf("fetching from TraceValues %s", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			sklog.Fatalf("reading row %s", err)
		}
		copy(lookup[:], rowID)
...
func findComparisonDigests(ctx context.Context, db *pgxpool.Pool, results []DigestResult, starts int) {
...
ComparisonBetweenUntriagedAndObserved AS (
    ...
    WHERE DiffMetricsClosestView.left_digest IN %s
)
...
ORDER BY left_digest, label, combined_metric ASC, num_diff_pixels ASC, right_digest ASC;`
	...
	if err := eg.Wait(); err != nil {
		sklog.Fatalf("Fetching metrics %s", err)
	}
}
