...
// files
func combineAndCompress(prefix string) error {
	// 20 is 4 times the number of nodes we have in an effort to balance the import
	// work across the nodes
	const finalNumberOfZippedFiles = 20
	...
	// Combine files, sort the lines (which sorts them by primary key, speeding importing), and
	// then remove duplicates (also speeding up importing).
	err = util.ChunkIterParallel(context.Background(), len(tsvFiles), len(tsvFiles)/finalNumberOfZippedFiles, func(_ context.Context, startIdx int, endIdx int) error {
		files := tsvFiles[startIdx:endIdx]
		joined := strings.Join(files, " ")
		bashCmd := fmt.Sprintf("cat %s | sort | uniq > out_tsv/combined_%s_%d.tsv", joined, prefix, startIdx)
