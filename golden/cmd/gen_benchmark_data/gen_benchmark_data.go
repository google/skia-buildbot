...
func fillData(ctx context.Context, commitOffset int) error {
	...
	for i := 0; i < numTests; i++ {
		...
		ignoredTraces, err := storePrimaryBranchTraceData(ctx, tracePairs, fakeFile, commitOffset)
		if err != nil {
		...
		for j := 0; j < numCLs; j++ {
			...
			if err := storeChangeListTraceData(ctx, clID, psID, fakeFile, tracePairs, numUntriagedFromPrimary, ignoredTraces); err != nil {
				return skerr.Wrap(err)
			}
		}
	}
...
func storePrimaryBranchTraceData(ctx context.Context, pairs []tiling.TracePair, sourceFile string, commitOffset int) (map[sql.MD5Hash]bool, error) {
...
func storeChangeListTraceData(ctx context.Context, clID, psID, sourceFile string, pairs []tiling.TracePair, numUntriaged int, ignoredTraces map[sql.MD5Hash]bool) error {
	sfh := md5.Sum([]byte(sourceFile))
	sourceFileHash := sfh[:] // convert array to slice
	...
	err = writeSecondaryTracesToTSV(ctx, data, ignoredTraces)
	if err != nil {
		return skerr.Wrap(err)
	}
...
func writeTracesToTSV(_ context.Context, pairs []tiling.TracePair, exp expectationResult) (map[sql.MD5Hash]bool, error) {
	ignoredTraces := map[sql.MD5Hash]bool{}
	...
	for _, tp := range pairs {
		...
		traceHash := sql.AsMD5Hash(traceID)
		ignoredStatus := false
...
func writeValuesAtHeadToTSV(_ context.Context, pairs []tiling.TracePair, exp expectationResult, ignoredTraces map[sql.MD5Hash]bool, offset int) error {
	grouping := groupingFor(pairs[0].Trace.Keys())
	// we know all the traces have the same grouping, since they were created from the same test.
	_, groupingBytes := sql.SerializeMap(grouping)
	groupingID := hex.EncodeToString(groupingBytes)
	...
	writeValueAtHead := func(traceID sql.TraceID, mostRecentCommit int, digest types.Digest, options sql.OptionsID,
		keys, corpus string, label sql.ExpectationsLabel, recordID string, ignored bool) {
		buf.WriteString(`\x`)
		buf.WriteString(hex.EncodeToString(traceID))
		buf.WriteRune('\t')
	...
	for _, tp := range pairs {
		...
		traceHash := sql.AsMD5Hash(traceID)
		isIgnored := ignoredTraces[traceHash]
		writeValueAtHead(traceID, offset+mostRecentCommit, digest, optionsID, keysJSON, trace.Corpus(), label, recordID, isIgnored)
	}
	sort.Strings(lines)
...
func writeSecondaryTracesToTSV(ctx context.Context, data []clDataPoint, ignoredTraces map[sql.MD5Hash]bool) error {
	path := filepath.Join("out_tsv", "secondarytraces_"+strconv.Itoa(rand.Int())+".tsv")
	f, err := os.Create(path)
	if err != nil {
		return skerr.Wrap(err)
	}
	...
	grouping := groupingFor(data[0].Keys)
	// we know all the traces have the same grouping, since they were created from the same test.
	_, groupingBytes := sql.SerializeMap(grouping)
	groupingID := hex.EncodeToString(groupingBytes)
	...
	for _, dp := range data {
		traceKeys, traceID := sql.SerializeMap(dp.Keys)
		traceHash := sql.AsMD5Hash(traceID)
		writeValue(traceID, traceKeys, ignoredTraces[traceHash])
	}
	sort.Strings(lines)
	if _, err := f.WriteString(strings.Join(lines, "")); err != nil {
		return skerr.Wrap(err)
...
IMPORT TABLE SecondaryBranchParams (
  ...
  PRIMARY KEY (branch_name, version_name, key, value)
	...
	"secondarytraces": `DROP TABLE SecondaryBranchTraces;
IMPORT TABLE SecondaryBranchTraces (
  secondary_branch_trace_id BYTES PRIMARY KEY,
  corpus STRING NOT NULL,
  grouping_id BYTES NOT NULL,
  keys JSONB NOT NULL,
  matches_any_ignore_rule BOOL,
...
IMPORT TABLE TraceValues (
  ...
  options_id BYTES NOT NULL,
  source_file_id BYTES NOT NULL,
  INDEX trace_commit_digest_idx (trace_id, commit_id, digest, options_id),
...
IMPORT TABLE ValuesAtHead (
  ...
  INVERTED INDEX keys_idx (keys),
  INDEX corpus_ignored_label_commit_idx (corpus, matches_any_ignore_rule, expectation_label, most_recent_commit_id),
