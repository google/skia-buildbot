...
On the top level the following field are required:

* *gitHash*
* *key*
* *issue* (only for tryjobs)
* *patchset* (only for tryjobs)
* *buildbucket_build_id* (only for tryjobs)

Within each entry in 'results' the following fields are required:

* *key*
* *md5*

The union of the 'key' field of each result and top level 'key' have to
contain 'name' and 'source_type'.
