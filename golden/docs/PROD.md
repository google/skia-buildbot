...
GoldTooManyCLs
--------------
There are many open CLs that have recently seen data from Gold. This may indicate a higher load
on CodeReviewSystems (e.g. Gerrit, GitHub) than usual, as we scan over all of these to see if
they are still open. This may indicate issues with marking CLs as closed, as well.
...
to comment on them or not in a while. This might mean we are seeing errors when talking to
Firestore or to the Code Review System (CRS). Check the logs on that pod's frontend server
(skiacorrectness) to see what's up.

This might mean we are doing too much and running out of quota to talk to the CRS.
