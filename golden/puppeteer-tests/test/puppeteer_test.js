...
describe('puppeteer', function() {
  ...
  it('takes screenshots', async () => {
    await page.goto(`http://localhost:${server.address().port}`);
    await page.screenshot({path: path.join(outputDir(), 'Test-Puppeteer-Hello-World.png')});
  });
});
...
const startTestServer = () => {
  const app = express();
  app.get('/', (_, res) => {
    res.send(`<html><body><h1>hello</h1><p>world</p><p>Random: ${Math.random()}</p></body></html>`);
  });
