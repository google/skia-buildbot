...
exports.setUpPuppeteerAndDemoPageServer = () => {
  ...
  before(async () => {
    ...
    rv.baseUrl = baseUrl; // Make baseUrl available to tests.
  ...
  beforeEach(async () => {
    rv.page = await browser.newPage(); // Make page available to tests.
    ...
    // Gold.
    await rv.page.setCookie(
      { url: rv.baseUrl, name: 'puppeteer', value: 'true' },
    );
  });
