...
func TestAdd(t *testing.T) {
	...
	entries := []*types.ParsedIngestionEntry{
		&types.ParsedIngestionEntry{
			...
			Params:  paramtools.Params{"name": "shadermaskfilter_image"},
			Digest:  "123",
		},
		&types.ParsedIngestionEntry{
			TraceID: "x86:GCC:8888:shadermaskfilter_image",
			Params:  paramtools.Params{"name": "shadermaskfilter_image", "gamma_correct": "no"},
			Digest:  "456",
		},
		&types.ParsedIngestionEntry{
			TraceID: "x86:GCC:8888:oval",
			Params:  paramtools.Params{"name": "oval"},
			Digest:  "789",
		},
	}
	...
	key, _, _ = traceKeys("something non-existant", 102)
	err = ds.DS.Get(ctx, key, &trace)
	assert.Error(t, err)

	// Then load the oval.
	key, _, _ = traceKeys("x86:GCC:8888:oval", 102)
	err = ds.DS.Get(ctx, key, &trace)
	assert.NoError(t, err)
	assert.True(t, strings.HasSuffix(trace.TileShard, "-2"))
	o := d.orderedParamSetCache[3]
	p, err := o.DecodeParams(trace.Params)
	assert.NoError(t, err)
	assert.Equal(t, "oval", p["name"])
	//	trace.Values
