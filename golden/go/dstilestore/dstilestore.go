...
func (d *DSTileStore) addInTx(job *Job) error {
	...
	for _, e := range job.Entries {
		...
		tvk := traceValueKey(commitIndexToTileOffset(job.CIndex), traceKey)
		traceValue := TraceValue{
			Offset: digestOffsets[e.Digest],
		}

		// Write traceValue back to datastore.
		if _, err := d.client.Put(d.ctx, tvk, &traceValue); err != nil {
			sklog.Errorf("Error writing TraceValue: %s", err)
			continue
		}
	}
