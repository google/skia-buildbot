...
type DSTileStore struct {
	...
	jobs   chan map[string]*types.ParsedIngestionEntry
...
func (d *DSTileStore) addInTx(job *Job) error {
	...
	keys := make([]*datastore.Key, 0, len(entries))
	for key, entry := range job.Entries {
	...
	if _, err := tx.PutMulti(keys, traces); err != nil {
		if err := tx.Rollback(); err != nil {
...
func (d *DSTileStore) Add(cindex int, entries map[string]*types.ParsedIngestionEntry) error {
	...
	if err := d.addTestNamesAsNeeded(names); err != nil {
