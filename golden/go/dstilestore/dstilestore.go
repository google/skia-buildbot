...
func (d *DSTileStore) addInTx(job *Job) error {
	...
	for len(job.Entries) > 0 {
		...
		d.client.RunInTransaction(d.ctx, func(tx *datastore.Transaction) error {
			...
			if err := tx.GetMulti(allDsKeys, &allTraces); err != nil {
				...
				for i, err := range multiError {
					if err == datastore.ErrNoSuchEntity {
						allTraces[i] = d.newTrace(job.CIndex, e, allShards[i])
					} else if err == nil {
						// No error.
					} else {
					...
					allTraces[i].Values = append(allTraces[i].Values, intsVarEncoded(commitIndexToTileOffset(cindex), digestOffsets[job.Entries[i].Digest])...)
				}
			}
			if err := tx.PutMulti(allDsKeys, allTraces); err != nil {
