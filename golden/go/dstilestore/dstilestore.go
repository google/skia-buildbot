...
type TraceValue struct {
	Offset int
	// TODO(jcgregorio) Also store shard number?
}
...
// Returns a new Trace.
func (d *DSTileStore) newTrace(cindex int, keys, options paramtools.Params, traceID string) (*Trace, error) {
	...
	for _, e := range job.Entries {
		...
		if !ok {
			...
			if err := d.client.Get(d.ctx, traceKey, &trace); err != nil {
				...
				if err == datastore.ErrNoSuchEntity {
					var tr *Trace
					tr, err = d.newTrace(job.CIndex, e.Keys, e.Options, e.TraceID)
					if err != nil {
					...
					if _, err := d.client.Put(d.ctx, traceKey, &trace); err != nil {
...
func (d *DSTileStore) Add(cindex int, entries []*types.ParsedIngestionEntry) error {
	// First combine all the keys and options into a paramset
	// Then make add any missing keys or values to our paramset map.

	paramset := paramtools.ParamSet{}
	for _, e := range entries {
		paramset.AddParams(e.Keys)
		paramset.AddParams(e.Options)
	}

	_, err := d.updateTileParamSet(cindex, paramset)
	if err != nil {
...
func (o *OrderedParamSet) Check(p paramtools.ParamSet) paramtools.ParamSet {
...
// Does this have a mutex, or do we hold that outside?
func (o *OrderedParamSet) Update(p paramtools.ParamSet) {
...
type DSParamSet struct {
	Encoded []byte // JSON serialized and gzip's ParamSet
}

func NewDSParamSet(p *OrderedParamSet) (*DSParamSet, error) {
	var b bytes.Buffer
	gz := gzip.NewWriter(&b)
...
func (dsp *DSParamSet) toParamSet() (*OrderedParamSet, error) {
	b := bytes.NewBuffer(dsp.Encoded)
...
func (c *paramConverter) convert(p paramtools.Params) ([]int32, error) {
	return nil, nil
}

func (d *DSTileStore) updateTileParamSet(cindex int, paramset paramtools.ParamSet) (*paramConverter, error) {
	return nil, nil
	// Any new keys or value from the most recent paramset?
	/*
		key := paramsetDSKey(cindex)
		var p DSParamSet
		d.client.Get(d.ctx, key, &p)
	*/
}
