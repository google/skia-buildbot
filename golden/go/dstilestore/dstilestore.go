...
type DSTileStore struct {
	...
	mutex      sync.Mutex // Protects access to traceCache.
...
func (d *DSTileStore) digestIndex(cindex int, testName, digest string) (int, error) {
	var offset int = -1

	// If local lookup failed, look in datastore.
...
func (d *DSTileStore) addInTx(job *Job) error {
	...
	digestIndex, err := d.digestIndex(job.CIndex, testName, job.Entry.Digest)
	if err != nil {
...
func (d *DSTileStore) traceToCache(key string, tce *TraceCacheEntry) {
	//	sklog.Warningln("traceToCache attempting to lock")
	d.mutex.Lock()
	//	sklog.Warningln("traceToCache acquires lock")
...
func (d *DSTileStore) traceFromCache(key string) (*TraceCacheEntry, bool) {
	...
	//	sklog.Warningln("traceFromCache attempting to lock")
	d.mutex.Lock()
	//	sklog.Warningln("traceFromCache acquires lock")
	defer d.mutex.Unlock()
	var ok bool
