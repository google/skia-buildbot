...
func (t *Trace) Add(tileOffset, digestIndex int) {
	t.Trace[tileOffset] = digestIndex
}
...
// dsKey returns the datastore key for a Trace and Params.
func dsKey(traceID string, cindex int) string {
	h := md5.New()
...
func (d *DSTileStore) tileShard(cindex) string {
	shard := d.rng.Int63n(NUM_SHARDS)
	fmt.Sprintf("%d-%d", shard, commitIndexToTileNumber(cindex))
}

// Returns a new Trace and Params along with the key they should be stored at.
...
func digestKey(tileNumber int, testName string) string {
	return fmt.Sprintf("%s-%d", testName, tileNumber)
}

func digestDSKey(tileNumber int, testName string) *datastore.Key {
	key := ds.NewKey(ds.DIGESTS_GOLD)
	key.Name = digestKey(tileNumber, testName)
	return key
}

func (d *DSTileStore) digestIndex(tileNumber int, testName, digest string) int {
	var offset int = -1
	d.mutex.Lock()
	defer d.mutex.Unlock()
	cached := d.digestsCache[digestKey(tileNumber, testName)]
	if digest != nil {
...
func (d *DSTileStore) addDigest(cindex int, digest, testName string) (index, error) {
	...
	_, err := d.client.RunInTransaction(d.ctx, func(tx *datastore.Transaction) error {
		...
		if err := tx.Put(key, &dig); err == nil {
			d.mutex.Lock()
			defer d.mutex.Unlock()
			d.cached[key.Name] = dig
		}
...
func (d *DSTileStore) addInTx(job *Job) error {
	...
	if digestIndex == -1 {
		var err error
		if digestIndex, err = d.addDigest(job.Entry.Digest, testName); err != nil {
			return fmt.Errorf("Failed to write new digest for test %s %s: %s", job.Entry.Digest, testName, err)
		}
	}

	// In a TX:
	// Does the trace exits?
	// If not then add the new Trace and Params.
	// Else Modify/Write the Trace.
	_, err := d.client.RunInTransaction(d.ctx, func(tx *datastore.Transaction) error {
		// Load the Trace
		var trace Trace
		key := ds.NewKey(ds.TRACE_GOLD)
		key.Name = dsKey(job.Entry.TraceID, job.CIndex)
		if err := tx.Get(key, &trace); err != nil {
			// If it doesn't exist, them populate it.
			if err == datastore.ErrNoSuchEntity {
				tr, pm, err := newTrace(job.CIndex, job.Entry.Keys, job.Entry.Options, job.Entry.TraceID)
				if err != nil {
