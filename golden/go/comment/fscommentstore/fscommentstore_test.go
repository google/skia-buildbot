...
// TestCreateAndUpdateComment_UpdateAllUpdatableFields_Success makes sure that when we update a
// comment, we only update the fields that are mutable. That is, the CreatedBy and CreatedTS fields
// should stay the same, and everything else can take on the new values.
func TestCreateAndUpdateComment_UpdateAllUpdatableFields_Success(t *testing.T) {
	unittest.LargeTest(t)
	c, cleanup := firestore.NewClientForTesting(t)
	defer cleanup()
	...
	// Edit all fields of the comment, including those that are should not be modified
	// by the store (CreatedBy and CreatedTS).
	actualComments, err := f.ListComments(ctx)
	require.NoError(t, err)
