...
// synchronize a local traceCache with the data in Firestore, thus it does not need to poll. We
// don't do any searches on our data, so keeping all the ignore rules in RAM (backed by Firestore)
// is a fine solution for the sake of performance (and avoiding unnecessary Firestore traffic).
type StoreImpl struct {
	client *ifirestore.Client

	traceMutex sync.RWMutex
	traceCache map[trace.ID]trace.Comment
}
...
// toTraceComment converts a Firestore traceEntry into a trace.Comment. The QueryToMatch field
// will be a normalized copy of the one from the passed in entry (to avoid a race condition between
// a potentially cached traceEntry and the returned value).
func toTraceComment(id trace.ID, r traceEntry) trace.Comment {
...
// toEntry converts a trace.Comment into a Firestore traceEntry. The QueryToMatch field will be
// a normalized copy of the one from the passed in comment (to avoid a race condition between
// a potentially cached traceEntry and the passed in value).
...
// ListComments implements the comment.Store interface. It returns the local traceCache of trace
// comments, never re-fetching from Firestore because the query snapshots should keep the local
// traceCache up to date, give or take a few seconds.
func (s *StoreImpl) ListComments(_ context.Context) ([]trace.Comment, error) {
	s.traceMutex.RLock()
	defer s.traceMutex.RUnlock()
	rv := make([]trace.Comment, 0, len(s.traceCache))
	for _, c := range s.traceCache {
		rv = append(rv, c)
	}
...
func (s *StoreImpl) UpdateComment(ctx context.Context, comment trace.Comment) error {
	...
	s.traceMutex.Lock()
	defer s.traceMutex.Unlock()
	s.traceCache[comment.ID] = toTraceComment(comment.ID, updatedComment)
	return nil
}
...
func (s *StoreImpl) DeleteComment(ctx context.Context, id trace.ID) error {
	...
	if _, err := s.client.Collection(traceCommentCollection).Doc(string(id)).Delete(ctx); err != nil {
