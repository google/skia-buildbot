...
func (a *analyzerStore) AddTile(tile *tiling.Tile) error {
	tracesByTest := map[string][]*types.GoldenTrace{}

	// Iterate over tile and add the tests to their respective tiles.
	for _, trace := range tile.Traces {
		// Group the tests by test Name
		gTrace := trace.(*types.GoldenTrace)
		testName := gTrace.Params_[types.PRIMARY_KEY_FIELD]
		tracesByTest[testName] = append(tracesByTest[testName], gTrace)
	}

	for testName, traces := range tracesByTest {
		testStore, err := a.getTestStore(testName)
		if err != nil {
			return err
		}

		if err = testStore.addTraces(traces); err != nil {
			return err
		}
	}
...
func (a *analyzerStore) getTestStore(testName string) (*oneTestStore, error) {
	return nil, nil
}
...
// oneTestStore
type oneTestStore struct {
}

func (o *oneTestStore) addTraces(traces []*types.GoldenTrace) error {
	return nil
}
