...
func (c *DSIgnoreStore) Delete(ctx context.Context, idStr string) error {
	id, err := strconv.ParseInt(idStr, 10, 64)
	if err != nil {
		return skerr.Wrapf(err, "id must be int64: %q", idStr)
	}
	if id <= 0 {
		return skerr.Fmt("Given id does not exist: %d", id)
	}
