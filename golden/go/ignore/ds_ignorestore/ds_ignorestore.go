...
func (c *DSIgnoreStore) Delete(ctx context.Context, idStr string) (int, error) {
	...
	if err != nil {
		return ignore.NotDeleted, skerr.Wrapf(err, "id must be int64: %q", idStr)
	}
	if id <= 0 {
		return ignore.NotDeleted, skerr.Fmt("Given id does not exist: %d", id)
	}
	...
	if err != nil {
		...
		return ignore.NotDeleted, skerr.Wrap(err)
	}
