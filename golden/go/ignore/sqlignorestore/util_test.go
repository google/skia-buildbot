...
func TestConvertIgnoreRules_Success_cdb(t *testing.T) {

	condition, args := ConvertIgnoreRules(nil, config.CockroachDB)
...
func TestConvertIgnoreRules_Success(t *testing.T) {
	...
	condition, args := ConvertIgnoreRules(nil, config.Spanner)
	assert.Equal(t, "false", condition)
	assert.Empty(t, args)
	...
	assert.Equal(t, `((COALESCE(keys ->> $1::TEXT IN ($2), FALSE)))`, condition)
	assert.Equal(t, []interface{}{"key1", "alpha"}, args)
	...
	const expectedCondition = `((COALESCE(keys ->> $1::TEXT IN ($2, $3), FALSE) AND COALESCE(keys ->> $4::TEXT IN ($5), FALSE))
OR (COALESCE(keys ->> $6::TEXT IN ($7, $8, $9), FALSE)))`
	assert.Equal(t, expectedCondition, condition)
	assert.Equal(t, []interface{}{"key1", "alpha", "beta", "key2", "gamma", "key3", "delta", "epsilon", "zeta"}, args)
}

func TestUpdateIgnoredTraces_StartsNull_SetToCorrectValue_cdb(t *testing.T) {
...
func TestUpdateIgnoredTraces_StartsNotNull_UpdatedToCorrectValues_cdb(t *testing.T) {
	existingData := dks.Build()
...
func TestUpdateIgnoredTraces_PartiallySet_UpdatedToCorrectValues_cdb(t *testing.T) {
	existingData := dks.Build()
...
/*
// TODO(pasthana): This test passes by itself, but fails when run alongside
// other tests. It is likely that it is happening because a large number of
// rows are being inserted into the database.
func TestUpdateIgnoredTraces_MultipleBatches_Success_cdb(t *testing.T) {
...
/*
// TODO(pasthana): This test is disabled because it is resulting in
// DEADLINE_EXCEEDED error. It is likely that it is happening because a large
// number of rows are being inserted into the database. Debug it later.
...
func TestUpdateIgnoredTraces_NullableRules_SetToCorrectValue_cdb(t *testing.T) {
