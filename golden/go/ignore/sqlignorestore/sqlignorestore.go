...
VALUES ($1, $2, $3, $4, $5)`, rule.CreatedBy, rule.CreatedBy, rule.Expires, rule.Note, v)
	...
	return skerr.Wrap(updateTraces(ctx, tx))
}
...
// updateTraces gets all current ignore rules and apply them to all Traces and ValuesAtHead.
// This is done in the passed in transaction, so if it fails, we can rollback.
func updateTraces(ctx context.Context, tx pgx.Tx) error {
