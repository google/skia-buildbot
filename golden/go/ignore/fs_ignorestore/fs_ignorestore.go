...
// synchronize a local cache with the data in Firestore, thus it does not need to poll.
...
func (s *StoreImpl) Delete(ctx context.Context, id string) (int, error) {
	if id == "" {
		return ignore.NotDeleted, skerr.Fmt("ID for ignore rule cannot be empty")
	}
	s.client.Collection(rulesCollection).Doc(id)
	if wr, err := s.client.Collection(rulesCollection).Doc(id).Delete(ctx); err != nil {
		return ignore.NotDeleted, skerr.Wrapf(err, "deleting ignore rule with id %s", id)
		...
		return ignore.NotDeleted, nil
	}
	return ignore.Deleted, nil
}
