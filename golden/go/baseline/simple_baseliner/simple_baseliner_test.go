...
func TestFetchBaselineSunnyDay(t *testing.T) {
	...
	baseliner := New(mes, "github")

	b, err := baseliner.FetchBaseline(masterBranch, false)
	...
	assert.Equal(t, masterBranch, b.CodeReviewSystem)
...
func TestFetchBaselineChangeListSunnyDay(t *testing.T) {
	...
	baseliner := New(mes, crs)

	b, err := baseliner.FetchBaseline(clID, false)
	assert.NoError(t, err)
