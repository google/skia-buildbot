...
// for a given tryjob.
func TestFetchBaselineIssuePatchsetSunnyDay(t *testing.T) {
	testutils.SmallTest(t)
	...
	IotaNewDigest := "1115fba4ce5b4cb9ffd595beb63e7389"
	KappaNewDigest := "222d894f5b680a9f7bd74c8004b7d88d"
	LambdaNewDigest := "3333fe3127b984e4ff39f4885ddb0d98"

	additionalTriages := &baseline.CommitableBaseline{
		Baseline: types.TestExp{
			"brand-new-test": map[string]types.Label{
				IotaNewDigest:  types.POSITIVE,
				KappaNewDigest: types.NEGATIVE,
			},
			three_devices.BetaTest: map[string]types.Label{
	...
	// mock the extra expectations that the baseliner appends
	...
	deepequal.AssertDeepEqual(t, types.TestExp{
		...
		three_devices.AlphaTest: map[string]types.Label{
