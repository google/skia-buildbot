...
type BaselinerImpl struct {
	...
	// currCommitSource is the latest commit source we have.
	currCommitSource baseline.CommitSource
...
func (b *BaselinerImpl) PushMasterBaselines(commitSource baseline.CommitSource, targetHash string) (*baseline.CommitableBaseLine, error) {
	defer timer.New("PushmasterBaselines").Stop()
	b.mutex.Lock()
	defer b.mutex.Unlock()
	if commitSource == nil {
		commitSource = b.currCommitSource
	}
	if commitSource == nil {
	...
	tileCommits := commitSource.AllCommits()
	extraCommits, err := b.getCommitsSince(tileCommits[len(tileCommits)-1])
	if err != nil {
		return nil, err
	}

	perCommitBaselines, err := baseline.GetBaselinesPerCommit(exps, commitSource, extraCommits)
	if err != nil {
	...
	b.currCommitSource = commitSource
...
func (b *BaselinerImpl) PushIssueBaseline(issueID int64, commitSource baseline.CommitSource, tallies *tally.Tallies) error {
	...
	baseLine, err := baseline.GetBaselineForIssue(issueID, tryjobs, tryjobResults, exp, commitSource.AllCommits())
	if err != nil {
...
func (b *BaselinerImpl) getMasterExpectations(commitHash string) (*baseline.CommitableBaseLine, error) {
	...
	if bLine, ok := b.baselineCache[commitHash]; ok {
