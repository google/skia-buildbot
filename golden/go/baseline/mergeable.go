...
// This implements a serialization format for expectations/baselines that can be merged automatically
// with Git with a low probability of merge conflicts.
// Expectations are stored in a text file following this structure:
// 	- Each line contains the expectations for one test.
// 	- The tokens of each line are separated by an arbitrary number of spaces.
...
var (
	...
	// validDigestLabel is used to verify that the digest pair follows the format described above.
	...
	for testName, digests := range baseLine {
		...
		for d, label := range digests {
			if !isMD5.MatchString(d) {
				return sklog.FmtErrorf("Expected hex-encoded MD5 hash. Got: '%s'", d)
			}
...
// filterEmpty returns a copy of the input array without all strings that are empty or only contain
...
func parseLine(line string) (string, map[string]types.Label, error) {
	parts := filterEmpty(strings.Split(line, " "))

	// We need to have at least one digest
	if len(parts) < 2 {
		return "", nil, sklog.FmtErrorf("Expectations need to contain at least one image digest. Got: '%s'", line)
	}
	...
	for _, digestLabel := range parts[1:] {
		...
		// Check if they are strictly monotonically increasing. This also covers the case of duplicate
		// digests.
		if digest <= prev {
			return "", nil, sklog.FmtErrorf("Digests for test '%s' are not sorted or there are duplicates. Got sequence: '%s %s'", testName, prev, digest)
		}
...
func splitDigestLabel(digestLabel string) (string, types.Label, error) {
	...
	if len(matchedGroups) != 3 {
		return "", types.UNTRIAGED, sklog.FmtErrorf("Invalid digest/label entry: '%s'", digestLabel)
	}
