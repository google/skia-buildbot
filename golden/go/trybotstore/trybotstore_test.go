...
func TestCloudTrybotStore(t *testing.T) {
	...
	store, err := NewCloudTrybotStore(common.PROJECT_ID, "gold-localhost-stephana", serviceAccountFile)
	assert.NoError(t, err)
...
func testTrybotStore(t *testing.T, store TrybotStore) {
	...
	// Delete all the trybots after we are done.
	// defer func() {
	// 	assert.NoError(t, store.DeleteIssue(issueID))
	// }()

	expChangeKeys, err := store.(*cloudTrybotStore).getExpChangesForIssue(issueID)
	assert.NoError(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 5; j++ {
			...
			for k := 0; k < 5; k++ {
				...
				label := (i + j + k) % 3
				changes.Add(testName, digest, types.Label(label))
			}
		}
