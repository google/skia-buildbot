...
func (c *cloudTrybotStore) AddChange(issueID int64, changes map[string]types.TestClassification, userId string) error {
	...
	ctx := context.Background()
	// start a transaction. We only want to add changes if all of them can be added.
	addFn := func(tx *datastore.Transaction) error {
		expChange := &ExpChange {

		}
		changeKey, err := tx.Put(ctx, datastore.IncompleteKey(kind_ExpChange, nil), &expChange)
		if err != nil {
			return err
		}
		...
		_, err := tx.PutMulti(tdeKeys, testChanges)
		return err
	}

	// insert the Change action and get the key

	// inerst all the test changes in batches.

	_, err := c.client.RunInTransaction(ctx, addFn)
	return err
}
	...
	for idx, patchsetID := range patchsetIDs {
		func(idx int, patchsetID int64) {
			egroup.Go(func() error {
				...
				sklog.Infof("QU: %s", spew.Sdump(query))
