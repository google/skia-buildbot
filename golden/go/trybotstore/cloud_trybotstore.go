...
func (c *cloudTrybotStore) GetTryjob(issueID, buildNumber int64) (*Tryjob, error) {
	ret := &Tryjob{}
	if err := c.client.Get(context.Background(), c.getTryjobKey(buildNumber), ret); err != nil {
...
func (c *cloudTrybotStore) GetTryjobResults(issueID, patchsetID int64) ([]*Tryjob, []*TryjobResult, error) {
	return nil, nil, nil
}

func (c *cloudTrybotStore) UpdateTryjobResult(tryjob *Tryjob, results []*TryjobResult) error {
	tryjobKey := c.getTryjobKey(tryjob.Buildnumber)
...
func (c *cloudTrybotStore) getTryjobResultKey(tryjobKey *datastore.Key, keyStr string) *datastore.Key {
	return datastore.NameKey(kind_TryjobResult, keyStr, tryjobKey)
}
