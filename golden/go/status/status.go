...
func New(storages *storage.Storage) (*StatusWatcher, error) {
	metricClient := metrics2.GetDefaultClient()

	ret := &StatusWatcher{
		storages:          storages,
		allUntriagedGauge: metricClient.GetInt64Metric(METRIC_ALL, map[string]string{"type": types.UNTRIAGED.String()}),
		allPositiveGauge:  metricClient.GetInt64Metric(METRIC_ALL, map[string]string{"type": types.POSITIVE.String()}),
		allNegativeGauge:  metricClient.GetInt64Metric(METRIC_ALL, map[string]string{"type": types.NEGATIVE.String()}),
		totalGauge:        metricClient.GetInt64Metric(METRIC_TOTAL, nil),
		corpusGauges:      map[string]map[types.Label]metrics2.Int64Metric{},
	}
