...
func New(ctx context.Context, swc StatusWatcherConfig) (*StatusWatcher, error) {
	ret := &StatusWatcher{
		...
		allUntriagedGauge:   metrics2.GetInt64Metric(allMetric, map[string]string{"type": string(expectations.Untriaged.String())}),
		allPositiveGauge:    metrics2.GetInt64Metric(allMetric, map[string]string{"type": string(expectations.Positive.String())}),
		allNegativeGauge:    metrics2.GetInt64Metric(allMetric, map[string]string{"type": string(expectations.Negative.String())}),
		...
		if _, ok := byCorpus[corpus]; !ok {
			...
			if _, ok := s.corpusGauges[corpus]; !ok {
				s.corpusGauges[corpus] = map[expectations.Label]metrics2.Int64Metric{
					expectations.Untriaged: metrics2.GetInt64Metric(corpusMetric, map[string]string{"type": string(expectations.Untriaged.String()), "corpus": corpus}),
					expectations.Positive:  metrics2.GetInt64Metric(corpusMetric, map[string]string{"type": string(expectations.Positive.String()), "corpus": corpus}),
					expectations.Negative:  metrics2.GetInt64Metric(corpusMetric, map[string]string{"type": string(expectations.Negative.String()), "corpus": corpus}),
				}
			}
		}
