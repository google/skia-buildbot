...
// This contains the routines necessary to consume and emit JSON to be processed
...
func ParseGoldResults(r io.Reader) (*GoldResults, []string, error) {
	...
	// Note: validationErrs implements the error interface.
	if errMessages != nil {
		// recreate err
		return nil, errMessages, nil
	}
	return &ret, nil, nil
}
...
func (r *rawGoldResults) parseValidate() []string {
	...
	var ret []string
	issueInconsistent := r.Issue == "" && (r.BuildBucketID != "" || r.Patchset != "")
	addErrMessage(&ret, issueInconsistent, "", "fields %s, %s, %s must all be empty or all be filled.", jn["Issue"], jn["Patchset"], jn["BuildBucketID"])
...
func (g *GoldResults) Validate() ([]string, error) {
	...
	// Validate the fields
	addErrMessage(&errMsg, govalidator.IsHexadecimal(g.GitHash), "field '%s' must be hexadecimal. Received '%s'", jn["GitHash"])
