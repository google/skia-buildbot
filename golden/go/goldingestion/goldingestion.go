...
func (g *goldProcessor) getCanonicalCommitHash(targetHash string) (string, error) {
	if !isCommit(g.vcs, targetHash) {
		...
		foundCommit := g.secondaryDepsExtractor.ExtractCommit(g.secondaryVCS.GetFile("DEPS", targetHash))
		if foundCommit == "" {
			return "", fmt.Errorf("Unable to extract DEPS file for commit %s in secondary repo.", targetHash)
		}
