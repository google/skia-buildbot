...
func newDSGoldProcessor(vcs vcsinfo.VCS, config *sharedconfig.IngesterConfig, client *http.Client) (ingestion.Processor, error) {
	...
	lastCommits := vcs.LastNIndex(1)
	cindex, err := vcs.IndexOf(ret.ctx, lastCommits[0].Hash)
	if err != nil {
		return nil, err
	}
	ret.client.PreFillCache(cindex)
	ret.client.PreFillCache(cindex - 50) // TODO(jcgregorio) 50 should be TILE_SIZE.
	return ret, nil
}
