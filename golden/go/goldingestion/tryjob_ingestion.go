...
func newGoldTryjobProcessor(vcs vcsinfo.VCS, config *sharedconfig.IngesterConfig, clientx *http.Client) (ingestion.Processor, error) {
	...
	// Currently only applies to the Skia repo.
...
func (g *goldTryjobProcessor) Process(ctx context.Context, resultsFile ingestion.ResultFileLocation) error {
	...
	return g.setTryjobToState(issueID, dmResults.BuildBucketID, tryjob, tryjobstore.TRYJOB_INGESTED)
}

func (g *goldTryjobProcessor) setTryjobToState(issueID, tryjobID int64, tryjob *tryjobstore.Tryjob, newStatus tryjobstore.TryjobStatus) error {
	if tryjob == nil {
		// load it
...
func (g *goldTryjobProcessor) tryjobUpdatedHandler(evData interface{}) {
	...
	if g.noUpload(tryjob.Builder, tryjob.MasterCommit, g.cfgFile) {
		// Mark as ingested.
		g.setTryjobToState(tryjob.IssueID, tryjob.BuildBucketID, nil, tryjobstore.TRYJOB_INGESTED)
	}
}
