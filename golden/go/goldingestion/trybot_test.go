...
func TestTrybotGoldProcessor(t *testing.T) {
	server, serverAddress, processor := RunGoldTrybotProcessor(t, TRYBOT_TRACE_DB_FILE, TRYBOT_SHAREDB_DIR, TRYBOT_INGESTION_FILE, TEST_DATA_DIR, TEST_CODE_RIETVELDREVIEW_URL, TEST_CODE_GERRITREVIEW_URL)
	...
	issueID, patchsetID := ExtractIssueInfo(commitIDs[0], processor.rietveldReview, processor.gerritReview)
	assert.Equal(t, "1", patchsetID)
	assert.Equal(t, "1381483003", issueID)
	issueID, patchsetID = ExtractIssueInfo(&tracedb.CommitID{}, processor.rietveldReview, processor.gerritReview)
