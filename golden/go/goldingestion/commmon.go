...
const (
	// VCS_SRC indicates that a commit came from a VCS, i.e. git.
	VCS_SRC SourceType = "g:"

	// TRYBOT_SRC indidicates that a commit came from a trybot.
	TRYBOT_SRC SourceType = "t:"
)
...
// FilterCommitIDs returns all entries in commitIDs that were generated by the
// supplied source type.
func FilterCommitIDs(commitIDs []*tracedb.CommitID, srcType SourceType) []*tracedb.CommitID {
	ret := make([]*tracedb.CommitID, 0, len(commitIDs))
	for _, cid := range commitIDs {
		if strings.HasPrefix(cid.Source, string(srcType)) {
			ret = append(ret, cid)
		}
	}
	return ret
...
// StripSourcePrefix removes the source prefix from the provided commitIDs.
func StripSourcePrefix(commitIDs []*tracedb.CommitID) {
	for _, cid := range commitIDs {
		for _, prefix := range allPrefixes {
			if strings.HasPrefix(cid.Source, prefix) {
				cid.Source = cid.Source[len(prefix):]
				break
			}
		}
	}
