...
const (
	...
	// up on a tile. If a trace exists on shard N in tile A, it will be on shard N for all tiles.
...
func (d *digestID) UnmarshalBinary(data []byte) error {
	...
	if n <= 0 {
		return skerr.Fmt("Error decoding digestID from %x; Uvarint returned %d", data, n)
	}
	*d = digestID(val)
	return nil
}
...
// CommitsWithData returns the indexes of the commits with at least one non-missing
// digest in at least one trace.
func (t traceMap) CommitsWithData() []int {
	if len(t) == 0 {
		return nil
	}
	numCommits := 0
...
func (t traceMap) PrependTraces(other traceMap) {
	...
	for id, trace := range other {
		gt := trace.(*types.GoldenTrace)
		numOtherCommits = len(gt.Digests)

		original, ok := t[id].(*types.GoldenTrace)
		if ok {
			original.Digests = append(gt.Digests, original.Digests...)
			...
			digests := gt.Digests
			for i := 0; i < numCommits; i++ {
				digests = append(digests, types.MISSING_DIGEST)
			}
			t[id] = &types.GoldenTrace{
				Keys:    gt.Keys,
				Digests: digests,
			}
		}
	...
	for id, trace := range t {
		if _, ok := other[id]; !ok {
			gt := trace.(*types.GoldenTrace)
			digests := make([]types.Digest, 0, numOtherCommits+numCommits)
			for i := 0; i < numOtherCommits; i++ {
				digests = append(digests, types.MISSING_DIGEST)
			}
			gt.Digests = append(digests, gt.Digests...)
		}
	}
}
