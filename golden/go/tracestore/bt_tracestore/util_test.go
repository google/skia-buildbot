...
func TestShardedRowName(t *testing.T) {
	...
	shard := int32(3) // arbitrary
	mostRecentTileKey := TileKey(math.MaxInt32 - 1)
	veryOldTileKey := TileKey(57)

	// Example RowName for a digest
	// digests are stored in a row based on the first two characters and a
	// column with the remaining characters.
	digestPrefix := string(AlphaDigest[:2])
	assert.Equal(t, "03:ts:d:2147483646:aa", shardedRowName(shard, typDigestMap, mostRecentTileKey, digestPrefix))
	assert.Equal(t, "03:ts:d:0000000057:aa", shardedRowName(shard, typDigestMap, veryOldTileKey, digestPrefix))

	// Example RowName for a trace
	encodedTrace := ",0=1,1=3,3=0,"
	assert.Equal(t, "03:ts:d:2147483646:,0=1,1=3,3=0,", shardedRowName(shard, typDigestMap, mostRecentTileKey, encodedTrace))
}

func TestExtractKeyFromRowName(t *testing.T) {
	unittest.SmallTest(t)

	assert.Equal(t, "ad", extractKey("03:ts:d:2147483646:ad"))
	assert.Equal(t, "", extractKey("03:ts:d:2147483646:"))
	assert.Equal(t, ",0=1,1=3,3=0,", extractKey("03:ts:d:2147483646:,0=1,1=3,3=0,"))
}
