...
digests in the cells, but this isn't ideal for these reasons:
  ...
  1. traceIDs get really long with lots of params - BT has a limit of 4kb per row name (aka a row key).
...
Gold (and perf) shards its data based on the subkey (essentially subkey % 32)
...
tablets for better performance (rows on BT are stored sorted by the entire row name).
...
(32 by default). The value for "subkey" is the encoded ParamSet (and from this subkey a shard is derived).
