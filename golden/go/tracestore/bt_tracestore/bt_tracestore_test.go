...
func TestBTTraceStoreGetDenseTile(t *testing.T) {
	...
	// With no data, we should get an empty tile
	actualTile, _, err := traceStore.GetDenseTile(ctx, len(commits))
	assert.NoError(t, err)
	assert.NotNil(t, actualTile)
	assert.Empty(t, actualTile.Traces)
	...
	assert.NoError(t, err)
	assert.Len(t, allCommits, sparseCommits-realCommitIndices[0])
	...
	actualTile, allCommits, err = traceStore.GetDenseTile(ctx, 2)
	assert.NoError(t, err)
	assert.Len(t, allCommits, sparseCommits-realCommitIndices[1])
...
func MockSparseVCSWithCommits(commits []*tiling.Commit) (*mock_vcs.VCS, []*vcsinfo.LongCommit) {
	...
	longCommits := make([]*vcsinfo.LongCommit, sparseCommits)
	hashes := []string{}
	...
	for i, c := range commits {
		...
		hashes[index] = c.Hash
		longCommits[realCommitIndices[i]] = &vcsinfo.LongCommit{
			ShortCommit: &vcsinfo.ShortCommit{
				Hash:    c.Hash,
				Author:  c.Author,
	...
	mvcs.On("ByIndex", ctx, realCommitIndices[0]).Return(longCommits[firstRealCommitIdx], nil)
	mvcs.On("From", longCommits[firstRealCommitIdx].Timestamp.Add(-1*time.Millisecond)).Return(hashes[firstRealCommitIdx:], nil)
	mvcs.On("LastNIndex", 1).Return(indexCommits[sparseCommits-1:])
	mvcs.On("DetailsMulti", ctx, hashes[firstRealCommitIdx:], false).Return(longCommits[firstRealCommitIdx:], nil)
	// For the partial tile, i.e. only the last 2 commits
	secondRealCommitIdx := realCommitIndices[1]
	mvcs.On("ByIndex", ctx, secondRealCommitIdx).Return(longCommits[secondRealCommitIdx], nil)
	mvcs.On("From", longCommits[secondRealCommitIdx].Timestamp.Add(-1*time.Millisecond)).Return(hashes[secondRealCommitIdx:], nil)
	mvcs.On("DetailsMulti", ctx, hashes[secondRealCommitIdx:], false).Return(longCommits[secondRealCommitIdx:], nil)
