...
// TODO(kjlubick): add test for adding multiple entries for a commit (like Skia does)
// TODO(kjlubick): add test for getTileKey, calcShardedRowName, rowAndColNameFromDigest
...
func TestBTTraceStorePutGet(t *testing.T) {
	...
	actualTile, _, err := traceStore.GetTile(ctx, len(commits), false)
	assert.NoError(t, err)
...
func TestBTTraceStorePutGetThreaded(t *testing.T) {
	...
	readTile := func() {
		defer wg.Done()
		_, _, err := traceStore.GetTile(ctx, len(commits), true)
		assert.NoError(t, err)
	}
	go readTile()
