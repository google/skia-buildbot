...
func TestBTTraceStorePutGetThreaded(t *testing.T) {
	...
	readTile := func() {
		defer wg.Done()
		_, _, err := traceStore.GetSparseTile(ctx, len(commits))
		assert.NoError(t, err)
	}
	go readTile()
...
func TestGetTileKey(t *testing.T) {
	...
	for _, test := range tests {
		mvcs.On("IndexOf", ctx, commit).Return(test.InputIndex, nil).Once()
		key, index, err := traceStore.getTileKey(ctx, commit)
		assert.NoError(t, err)
	...
	mvcs.On("IndexOf", ctx, commit).Return(-1, errors.New("whoops")).Once()
	_, _, err = traceStore.getTileKey(ctx, commit)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "whoops")
	assert.Contains(t, err.Error(), "could not look up")
}
...
func TestBTTraceStoreLargeTile(t *testing.T) {
	...
	_, _, err = traceStore.getTileKey(ctx, tile.Commits[maxIndex].Hash)
	assert.NoError(t, err)
