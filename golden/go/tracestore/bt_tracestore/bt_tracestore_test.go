...
// TestBTTraceStoreGetSparseTile puts in a few data points sparsely spaced throughout
// time and makes sure we can call GetSparseTile to get them condensed together
// (i.e. with all the empty commits tossed out).
func TestBTTraceStoreGetSparseTile(t *testing.T) {
	unittest.LargeTest(t)
	unittest.RequiresBigTableEmulator(t)

	commits := data.MakeTestCommits()
	mvcs, lCommits := MockSparseVCSWithCommits(commits)
	defer mvcs.AssertExpectations(t)

	btConf := BTConfig{
		ProjectID:  "should-use-the-emulator",
		InstanceID: "testinstance",
		TableID:    "three_devices_test_sparse",
		VCS:        mvcs,
	}
	...
	// With no data, we should get an empty tile
	actualTile, _, err := traceStore.GetSparseTile(ctx, len(commits))
	assert.NoError(t, err)
	assert.NotNil(t, actualTile)
	assert.Empty(t, actualTile.Traces)
	...
	// make sure we only get the length we ask for
	actualTile, actualCommits, err = traceStore.GetSparseTile(ctx, 2)
	assert.NoError(t, err)

	assert.NotEqual(t, tile, actualTile)
	assert.Len(t, actualTile.Commits, 2)
	assert.Equal(t, commits[1:], actualCommits)

	// spot check a trace:
	assert.Equal(t, &types.GoldenTrace{
		Digests: types.DigestSlice{data.AlphaBad1Digest, data.AlphaGood1Digest},
		Keys: map[string]string{
			"device":                data.AnglerDevice,
			types.PRIMARY_KEY_FIELD: string(data.AlphaTest),
			types.CORPUS_FIELD:      "gm",
		},
	}, actualTile.Traces[",device=angler,name=test_alpha,source_type=gm,"])
}
...
func TestGetTileKey(t *testing.T) {
	...
	for _, test := range tests {
		key, index, err := traceStore.getTileKey(ctx, test.InputRepoIndex)
		assert.NoError(t, err)
...
const sparseCommits = 1100

var realCommitIndices = []int{300, 501, 557}

func MockSparseVCSWithCommits(commits []*tiling.Commit) (*mock_vcs.VCS, []*vcsinfo.LongCommit) {
	mvcs := &mock_vcs.VCS{}
	if len(commits) != len(realCommitIndices) {
		panic("commits should be same length as realCommitIndices")
	}

	// Create many synthetic commits. We only need to make these
	// synthetic commits as IndexCommit because when the sparse
	// tile is created, these synthetic commits will be thrown out
	// before the full details are gotten.
	indexCommits := make([]*vcsinfo.IndexCommit, sparseCommits)
	hashes := []string{}
	for i := 0; i < sparseCommits; i++ {
	...
	longCommits := make([]*vcsinfo.LongCommit, len(commits))
	realHashes := []string{}
	for i, c := range commits {
		...
		realHashes = append(realHashes, c.Hash)
		hashes[index] = c.Hash
	...
	// For the full tile, i.e all 3 commits
	mvcs.On("ByIndex", ctx, realCommitIndices[0]).Return(longCommits[0], nil)
	mvcs.On("From", longCommits[0].Timestamp.Add(-1*time.Millisecond)).Return(hashes[realCommitIndices[0]:], nil)
	mvcs.On("LastNIndex", 1).Return(indexCommits[sparseCommits-1:])
	mvcs.On("DetailsMulti", ctx, realHashes, false).Return(longCommits, nil)
	// For the partial tile, i.e. only the last 2 commits
	mvcs.On("ByIndex", ctx, realCommitIndices[1]).Return(longCommits[1], nil)
	mvcs.On("From", longCommits[1].Timestamp.Add(-1*time.Millisecond)).Return(hashes[realCommitIndices[1]:], nil)
	mvcs.On("DetailsMulti", ctx, realHashes[1:], false).Return(longCommits[1:], nil)
