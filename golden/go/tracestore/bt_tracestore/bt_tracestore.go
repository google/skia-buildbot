...
func (b *BTTraceStore) createPutMutations(entries []*tracestore.Entry, ts time.Time, tk TileKey, commitOffset int, ops *paramtools.OrderedParamSet, dm *DigestMap) ([]string, []*bigtable.Mutation, error) {
	...
	for _, entry := range entries {
		...
		rowName := b.calcShardedRowName(tk, typTrace, string(traceKey))
...
func (b *BTTraceStore) loadTile(ctx context.Context, tileKey TileKey) (*EncTile, error) {
	...
	// Load the DigestMap and the OrderedParamSet so the caller can
	// decode the data from the tile.
...
func (b *BTTraceStore) rowAndColNameFromDigest(tileKey TileKey, digest types.Digest) (string, string) {
	key := string(digest[:3])
	colName := string(digest[3:])
	return b.calcShardedRowName(tileKey, typDigestMap, key), colName
}
...
func (b *BTTraceStore) getDigestMap(ctx context.Context, tileKey TileKey) (*DigestMap, error) {
	...
	for shard := int32(0); shard < b.shards; shard++ {
		func(shard int32) {
			egroup.Go(func() error {
				prefRange := bigtable.PrefixRange(shardedRowName(shard, typDigestMap, tileKey, ""))
				var idx int64
...
func (b *BTTraceStore) getOrAddDigests(ctx context.Context, tileKey TileKey, digests []types.Digest, digestMap *DigestMap) (*DigestMap, error) {
	...
	for idx, digest := range digests {
		...
		rowName, colName := b.rowAndColNameFromDigest(tileKey, digest)
	...
	// Return the unused IDs for laster use.
	b.returnIDs(unusedIDs)
	return b.getDigestMap(ctx, tileKey)
}

// updateDigestMap returns the current DigestMap for a given tileKey.
