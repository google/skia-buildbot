...
func (b *BTTraceStore) Put(ctx context.Context, commitHash string, entries []*tracestore.Entry, ts time.Time) error {
	...
	tileKey, commitIndex, err := b.getTileKey(ctx, commitHash)
	if err != nil {
...
func (b *BTTraceStore) GetTile(ctx context.Context, nCommits int) (*tiling.Tile, []*tiling.Commit, error) {
	...
	for idx, encTile := range encTiles {
		...
		if idx == (len(encTiles) - 1) {
			...
			// endCommitOffset)
			endOffset = endCommitOffset
		}
		segLen := endOffset - startCommitOffset + 1
		...
		for _, ic := range idxCommits[commitIDX : commitIDX+segLen] {
...
func (b *BTTraceStore) getTileKey(ctx context.Context, commitHash string) (tileKey, int, error) {
	...
	tileIndex := int32(commitIndex) / b.tileSize
	commitIndex = commitIndex % int(b.tileSize)
