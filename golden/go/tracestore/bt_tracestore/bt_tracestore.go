...
func (b *BTTraceStore) getTracesInRange(ctx context.Context, startTileKey, endTileKey tileKey, startCommitIndex, endCommitIndex int) (traceMap, paramtools.ParamSet, error) {
	...
	for idx := nTiles - 1; idx >= 0; idx-- {
		...
		for encodedKey, encValues := range encTile.traces {
			// at this point, encodedKey looks like ,0=1,1=3,3=0,
			// See params.paramsEncoder
			...
			if _, ok := tileTraces[traceKey]; !ok {
				...
				if opts, ok := options[idx][encodedKey]; ok {
					params.Add(opts)
				}
			...
			digests := encValues[startOffset : startOffset+segLen]
...
func (b *BTTraceStore) GetDenseTile(ctx context.Context, nCommits int) (*tiling.Tile, []*tiling.Commit, error) {
	...
	for i := 0; i < maxTilesForDenseTile; i++ {
		...
		if err != nil {
			return nil, nil, skerr.Wrapf(err, "could not load commits from tile %d", tKey)
		}
		...
		filledCommits := traces.CommitIndicesWithData(endIdx)
...
func (b *BTTraceStore) loadTile(ctx context.Context, tileKey tileKey) (*encTile, error) {
	...
	var traces map[encodedTraceID][DefaultTileSize]types.Digest
	egroup.Go(func() error {
		var err error
...
func (b *BTTraceStore) loadOptions(ctx context.Context, tileKey tileKey) (map[encodedTraceID]paramtools.Params, error) {
	// FIXME
	return map[encodedTraceID]paramtools.Params{}, nil
				...
				err := b.table.ReadRows(ctx, prefixRange, func(row bigtable.Row) bool {
					...
					for i := len(cols) - 1; i >= 0; i-- {
						...
						if len(cols[i].Value) > 1 {
							p = decodeParamsCached(cols[i].Value)
							break
						}
					}
					if p == nil {
...
func (b *BTTraceStore) loadEncodedTraces(ctx context.Context, tileKey tileKey) (map[encodedTraceID][DefaultTileSize]types.Digest, error) {
	defer metrics2.FuncTimer().Stop()
	var egroup errgroup.Group
	shardResults := [DefaultShards]map[encodedTraceID][DefaultTileSize]types.Digest{}
	traceCount := int64(0)
	...
	for shard := int32(0); shard < DefaultShards; shard++ {
		func(shard int32) {
			...
			egroup.Go(func() error {
				...
				target := map[encodedTraceID][DefaultTileSize]types.Digest{}
				shardResults[shard] = target
				var parseErr error
				err := b.table.ReadRows(ctx, prefixRange, func(row bigtable.Row) bool {
					atomic.AddInt64(&traceCount, 1)
	...
	// Merge all the results together
	ret := make(map[encodedTraceID][DefaultTileSize]types.Digest, traceCount)
	for _, r := range shardResults {
...
func (b *BTTraceStore) getOPS(ctx context.Context, tk tileKey) (*opsCacheEntry, bool, error) {
	...
	defer cancel()
	row, err := b.table.ReadRow(tctx, tk.OpsRowName(), bigtable.RowFilter(bigtable.LatestNFilter(1)))
	if err != nil {
