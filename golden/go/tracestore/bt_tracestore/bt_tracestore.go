...
func (b *BTTraceStore) createPutMutations(entries []*tracestore.Entry, ts time.Time, tk tileKey, commitIndex int, ops *paramtools.OrderedParamSet) ([]string, []*bigtable.Mutation, error) {
	...
	for _, entry := range entries {
		...
		pBytes, err := encodeParams(entry.Options)
		if err != nil {
			return nil, nil, skerr.Wrapf(err, "could not encode options map for trace %s (%v)", sTrace, entry.Options)
		}
	...
	for idx, encTile := range encTiles {
		...
		for encodedKey, encValues := range encTile.traces {
			...
			if _, ok := tileTraces[traceKey]; !ok {
				...
				// We only want the latest options, so start looking at
				// the latest tile for options and work our way back.
				if idx == nTiles-1 {
					// We can avoid an extra encode if we are already on
					...
					for i := nTiles - 1; i >= 0; i-- {
						et := encTiles[i]
						str, err := et.ops.EncodeParamsAsString(params)
						if err != nil {
							return nil, nil, skerr.Wrapf(err, "could not re-encode params: %#v", params)
						}
						encTrace := encodedTraceID(str)
						if _, ok := et.traces[encTrace]; ok {
							if opts, ok := options[i][encTrace]; ok {
								params.Add(opts)
							}
							// if a tile has a trace, but no options, we
							// can stop looking because we know the most
							// recent trace has none.
							break
						}
					}
				}
