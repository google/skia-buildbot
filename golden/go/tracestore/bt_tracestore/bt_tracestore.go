...
func (b *BTTraceStore) Put(ctx context.Context, commitHash string, entries []*tracestore.Entry, ts time.Time) error {
	...
	FIXME(kjlubick): I think the traceid needs to be in the column
	and the commit needs to be on the row, not the way it is now.
...
func (b *BTTraceStore) loadTraces(ctx context.Context, tileKey TileKey) (map[EncodedTraceId][]DigestID, error) {
	...
	for shard := int32(0); shard < b.shards; shard++ {
		func(shard int32) {
			egroup.Go(func() error {
				prefixRange := bigtable.PrefixRange(shardedRowName(shard, typTrace, tileKey, ""))
...
func (b *BTTraceStore) getDigestMap(ctx context.Context, tileKey TileKey) (*DigestMap, error) {
	// Query all digest prefixes in parallel.
	var egroup errgroup.Group
	resultCh := make(chan map[types.Digest]DigestID, len(digestPrefixes))
	total := int64(0)
	for _, digestPrefix := range digestPrefixes {
		func(digestPrefix string) {
			egroup.Go(func() error {
				prefRange := bigtable.PrefixRange(rowName(typDigestMap, tileKey, digestPrefix))
				var idx int64
				...
				if err != nil {
					return skerr.Fmt("problem fetching digestMap with prefix %s: %s", digestPrefix, err)
				}
				if parseErr != nil {
					return parseErr
				}
...
func (b *BTTraceStore) getIDs(ctx context.Context, n int) ([]DigestID, error) {
	...
	maxID := int64(binary.BigEndian.Uint64(encMaxID))

	lastID := maxID - toRequest
	...
	for i := lastID; i < maxID; i++ {
		...
		if missing > 0 {
			ids = append(ids, DigestID(i))
		} else {
			// ... and put the remainder in the store for later.
			b.availIDs = append(b.availIDs, DigestID(i))
		}
		missing--
	}
...
func (b *BTTraceStore) getOrAddDigests(ctx context.Context, tileKey TileKey, digests []types.Digest, digestMap *DigestMap) (*DigestMap, error) {
	...
	for idx, digest := range digests {
		rowName, colName := rowAndColNameFromDigest(tileKey, digest)
