...
func (b *BTTraceStore) getIDs(ctx context.Context, tileKey TileKey, n int) ([]DigestID, error) {
	...
	rowName := rowName(typIDCounter, tileKey, "")
	rmw := bigtable.NewReadModifyWrite()
	rmw.Increment(idCounterFamily, idCounterColumn, missing)
	row, err := b.table.ApplyReadModifyWrite(ctx, rowName, rmw)
	if err != nil {
		return nil, err
	}

	encMaxID := []byte(rowMap(row).GetStr(idCounterFamily, idCounterColumn))
...
func (b *BTTraceStore) getOrAddDigests(ctx context.Context, tileKey TileKey, digests []types.Digest, digestMap *DigestMap) (*DigestMap, error) {
	...
	for idx, digest := range digests {
		...
		idVal := availIDs[idx]
		addMut.Set(digestMapFamily, colName, bigtable.ServerTime, []byte(strconv.Itoa(int(idVal))))
