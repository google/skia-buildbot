...
type BTTraceStore struct {
	...
	opsCache sync.Map
	// Prevents race conditions when updating the OrderedParamSet (OPS)
	// from multiple processes. Without this, two processes could try to
	// add a param to the OPS and only one would succeed (the other would
	// lose the race).
	opsMutex sync.Mutex
...
func (b *BTTraceStore) Put(ctx context.Context, commitHash string, entries []*tracestore.Entry, ts time.Time) error {
	...
	tileKey, commitIndex, err := b.getTileKey(ctx, repoIndex)
	if err != nil {
		return skerr.Fmt("cannot derive tilekey/index: %s", err)
	}
...
func (b *BTTraceStore) GetTile(ctx context.Context, nCommits int) (*tiling.Tile, []*tiling.Commit, error) {
	...
	c := idxCommits[0]
	startTileKey, startCommitIndex, err := b.getTileKey(ctx, c.Index)
	if err != nil {
		return nil, nil, skerr.Fmt("could not derive tile key for start commit %s (repoIndex %d): %s", c.Hash, c.Index, err)
	}

	c = idxCommits[len(idxCommits)-1]
	endTileKey, endCommitIndex, err := b.getTileKey(ctx, c.Index)
	if err != nil {
		return nil, nil, skerr.Fmt("could not derive tile key for end commit %s (repoIndex %d): %s", c.Hash, c.Index, err)
	}
...
// GetSparseTile implements the TraceStore interface. It fetches the most recent tile and sees if there
// is enough non-empty data, then queries the next oldest tile until it has nCommits non-empty commits.
// Of note, due to this tile by tile procedure, the ParamsSet may have a set of params that does not
// actually correspond to a trace (this shouldn't be a problem, but is worth calling out).
func (b *BTTraceStore) GetSparseTile(ctx context.Context, nCommits int) (*tiling.Tile, []*tiling.Commit, error) {
	defer metrics2.FuncTimer().Stop()
	...
	c := idxCommits[0]
	tKey, endIdx, err := b.getTileKey(ctx, c.Index)
	if err != nil {
		return nil, nil, skerr.Fmt("could not derive tile key for end commit %s (repoIndex %d): %s", c.Hash, c.Index, err)
	}
	tileStartIdx := c.Index - endIdx
	...
	for {
		...
		filledCommits := traces.CommitsWithData()
		...
		for _, tileIdx := range filledCommits {
			commitsWithData = append(commitsWithData, tileStartIdx+tileIdx)
		}
	...
	hashesWithData := make([]string, len(commitsWithData))
	for i, idx := range commitsWithData {
		hashesWithData[i] = hashes[idx-oldestIdx]
	}

	commits, err := b.makeTileCommits(ctx, hashesWithData)
	if err != nil {
...
func (b *BTTraceStore) getTileKey(ctx context.Context, repoIndex int) (tileKey, int, error) {
	tileIndex := int32(repoIndex) / b.tileSize
	commitIndex := repoIndex % int(b.tileSize)
	return tileKeyFromIndex(tileIndex), commitIndex, nil
}
...
// calcShardedRowName deterministically assigns a given subkey (e.g. traceID)
...
// Specificially, we need to look up the details to get the author information.
