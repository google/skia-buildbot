...
type BTTraceStore struct {
	...
	opsCache sync.Map
	// Prevents race conditions when updating the OrdredParamSet from multiple processes.
	opsMutex sync.Mutex
			...
			egroup.Go(func() error {
				...
				err := b.table.ReadRows(ctx, prefixRange, func(row bigtable.Row) bool {
					...
					for _, col := range row[traceFamily] {
						...
						dID, err := strconv.ParseInt(string(col.Value), 10, 64)
						if err != nil {
						...
						target[traceKey][idx] = digestID(dID)
					}
					return true
				})
				if err != nil {
