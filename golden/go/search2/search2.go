...
func (s *Impl) GetBlamesForUntriagedDigests(ctx context.Context, corpus string) (BlameSummaryV1, error) {
	...
	// Return the trace histories for those traces (maybe also materialized view)
	historiesByDigest, err := s.getHistoriesForTraces(ctx, tracesByDigest)
	if err != nil {
		return BlameSummaryV1{}, skerr.Wrap(err)
	}
	...
	// Go back through the trace histories and stop when you get to the thing that is not at head
	// Do something with previous-"good" and newly-untriaged and digest (blame.go for inspiration)
	ranges := combineIntoRanges(ctx, historiesByDigest, groupings, commits)
	// Sort so those ranges with more untriaged digests come first.
	sort.Slice(ranges, func(i, j int) bool {
		if ranges[i].TotalUntriagedDigests == ranges[j].TotalUntriagedDigests {
			return ranges[i].CommitRange < ranges[j].CommitRange
		}
		return ranges[i].TotalUntriagedDigests > ranges[j].TotalUntriagedDigests
	})
...
func (s *Impl) getHistoriesForTraces(ctx context.Context, traces map[groupingDigestKey][]schema.TraceID) (map[groupingDigestKey][]traceData, error) {
	...
	const statement = `SELECT trace_id, commit_id, encode(digest, 'hex') FROM TraceValues
WHERE commit_id >= $1 and trace_id = ANY($2)`
	...
	rv := make(map[groupingDigestKey][]traceData, len(traces))
	traceDataByTraceID := make(map[schema.MD5Hash]traceData, len(tracesToLookup))
	var key schema.MD5Hash
	traceKey := key[:]
	for rows.Next() {
		var traceID schema.TraceID
		...
		copy(traceKey[:], traceID)
		existingData, ok := traceDataByTraceID[key]
		if !ok {
			// Make a new slice of digests (traceData) and associated it with the correct
			// grouping+digest
			t := make(traceData, traceLength)
			traceDataByTraceID[key] = t
			gdk := tracesToDigest[key]
			rv[gdk] = append(rv[gdk], t)
			existingData = t
		}
		...
		// Finally, insert the digest into the appropriate spot in history
		existingData[idx] = digest
	}
	return rv, nil
}
...
func combineIntoRanges(ctx context.Context, dataByDigest map[groupingDigestKey][]traceData, groupings map[schema.MD5Hash]paramtools.Params, commits []web_frontend.Commit) []BlameEntry {
	spew.Dump(dataByDigest)
	panic("not impl")
}
