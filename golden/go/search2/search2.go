...
WHERE commit_id >= $1 and trace_id = ANY($2)`
	...
	for rows.Next() {
		...
		if !ok {
			...
			tracesByDigest[gdk] = append(tracesByDigest[gdk], t)
			existingData = t
		}
		...
		existingData[idx] = types.Digest(hex.EncodeToString(digest))
	...
	// Put into a sorted slice for determinism
...
func combineIntoRanges(ctx context.Context, digests []untriagedDigestAtHead, groupings map[schema.MD5Hash]paramtools.Params, commits []web_frontend.Commit, exp map[expectationKey]expectations.Label) []BlameEntry {
	...
	for _, data := range digests {
		...
		for _, tr := range data.traces {
			...
			// idx is now either -1 (for beginning of tile) or the index of the last
			// known
	...
	// Sort data so the "biggest changes" come first (and perform other clenaups)
