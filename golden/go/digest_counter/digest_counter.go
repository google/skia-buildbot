...
type DigestCounter interface {
	...
	MaxDigestsByTest() map[types.TestName]util.StringSet
...
type Counter struct {
	...
	maxCountsByTest  map[types.TestName]util.StringSet
}
...
func (t *Counter) MaxDigestsByTest() map[types.TestName]util.StringSet {
...
func calculate(tile *tiling.Tile) (map[tiling.TraceId]DigestCount, map[types.TestName]DigestCount, map[types.TestName]util.StringSet) {
	...
	for k, tr := range tile.Traces {
		...
		testName := types.TestName(tr.Params()[types.PRIMARY_KEY_FIELD])
	...
	maxCountsByTest := make(map[types.TestName]util.StringSet, len(testDigestCount))
	for testName, dCount := range testDigestCount {
		...
		for digest, count := range dCount {
			if count == maxCount {
				maxCountsByTest[testName][string(digest)] = true
			}
		}
	}
