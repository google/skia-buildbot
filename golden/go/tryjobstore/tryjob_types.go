...
func (t *Tryjob) MarshalJSON() ([]byte, error) {
	// Create a wrapping struct around the Tryjob that produces the correct output.
	temp := struct {
		*wrapTryjob
		Updated TimeJsonMs `json:"updated"` // Override the Updated field to produce a timestamp in MS.
	}{
		wrapTryjob: (*wrapTryjob)(t),
		Updated:    TimeJsonMs(t.Updated),
	}
	return json.Marshal(&temp)
}

// wrapTryjob is a dummy type to avoid recursive call to Tryjob.MarshallJSON
type wrapTryjob Tryjob
