...
func (c *DSTryjobStore) UpdateIssue(details *tryjobstore.Issue, updateFn tryjobstore.NewValueFn) error {
	_, err := c.updateEntity(c.getIssueKey(details.ID), (*newerIssue)(details), nil, false, updateFn)
	return err
}
...
func (c *DSTryjobStore) CommitIssueExp(issueID int64, commitFn func() error) error {
	...
	setCommittedFn := func(tx *datastore.Transaction) error {
		...
		_, err = c.updateEntity(key, (*newerIssue)(issue), tx, true, nil)
		return err
	}
...
func (c *DSTryjobStore) UpdateTryjob(buildBucketID int64, tryjob *tryjobstore.Tryjob, newValFn tryjobstore.NewValueFn) error {
	...
	newTryjob, err := c.updateEntity(c.getTryjobKey(buildBucketID), (*newerTryjob)(tryjob), nil, false, newValFn)
	if err != nil {
		return err
	}
	nt := newTryjob.(*newerTryjob)
	c.eventBus.Publish(tryjobstore.EV_TRYJOB_UPDATED, copyTryjob((*tryjobstore.Tryjob)(nt)), true)
	return nil
}
