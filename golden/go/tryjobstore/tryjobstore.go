...
func (c *cloudTryjobStore) GetExpectations(issueID int64) (exp *expstorage.Expectations, err error) {
	...
	expChangeKeys, expChanges, err := c.getExpChangesForIssue(issueID, -1, -1, false)
	if err != nil {
		return nil, err
	}
	...
	for _, exp := range expChanges {
		sklog.Infof("Time: %v ", time.Unix(exp.TimeStamp/1000, (exp.TimeStamp%1000)*int64(time.Millisecond)))
