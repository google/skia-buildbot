...
func (c *cloudTryjobStore) GetIssue(issueID int64, loadTryjobs bool, targetPatchsets []int64) (*IssueDetails, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Error (%s) in getEntity for %s: %s", sklog.LineRef(), key, err)
	}
	...
	if loadTryjobs {
		...
		if err != nil {
			return nil, fmt.Errorf("Error (%s) getting tryjobs for issue: %s", sklog.LineRef(), err)
		}

		sklog.Infof("ISSUE: %s", spew.Sdump(target))
		sklog.Infof("TJOBS: %s", spew.Sdump(tryjobs))

		for _, tj := range tryjobs {
			ps := target.findPatchsetx(tj.PatchsetID)
			if ps == nil {
				return nil, fmt.Errorf("Error (%s). Unable to find patchset %d in issue %d:", sklog.LineRef(), tj.PatchsetID, target.ID)
			}
