...
type TryjobStore interface {
	...
	// return will contain the results with indices matching the Tryjobs in the
	// first return value.
...
// issue and list of patchsets. If keysOnly is try only the keys of the tryjobs will
// be returned. If filterDup is true duplicate tryjobs will be filtered out for
// reach patchset, only keeping the newest.
...
// Each Tryjob instance that was loaded contains a 'Key' field, so the key can
// easily be retrieved from there.
