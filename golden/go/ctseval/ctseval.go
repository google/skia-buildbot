...
func NewCTSEvalator(gsBucket, gsPath, baseDir string, httpClient *http.Client, ingestFreq time.Duration, timeWindowDays int, ixr *indexer.Indexer, storages *storage.Storage) (*CTSEvaluator, error) {
	...
	if ingestFreq != 0 {
		...
		ret.ingester.Start(context.Background())
	}
	return ret, nil
}
...
func (c *CTSEvaluator) List(offset, size int) ([]*CTSRun, error) {
	...
	for _, oneFile := range files {
		if strings.HasPrefix(oneFile.Name(), TestRunIDPrefix) && !oneFile.IsDir() {
...
func (c *CTSEvaluator) Details(runID string) (*CTSRunDetails, error) {
	fullPath := filepath.Join(c.dataDir, runID+".json")
...
func (c *CTSEvaluator) DeviceResults(runID, deviceID string) ([]*CTSTestResult, error) {
	fullPath := filepath.Join(c.dataDir, runID, deviceID)
	if !fileutil.FileExists(fullPath) {
		return nil, fmt.Errorf("Unkown run id %s and device id %s", runID, deviceID)
	}
...
func (c *CTSEvaluator) Process(ctx context.Context, resultsFile ingestion.ResultFileLocation) error {
...
func (c *CTSEvaluator) addResult(runDetails *CTSRunDetails, idx *indexer.SearchIndex, exps *expstorage.Expectations) error {
	...
	// Write the
	fName := filepath.Join(c.dataDir, runDetails.CTSRun.ID+".json")
	sklog.Infof("OUTPUT FILENAME: %s", fName)
	err := util.WithWriteFile(fName, func(w io.Writer) error {
	...
	// Result directory
	resultDir := filepath.Join(c.dataDir, runDetails.CTSRun.ID)
	if resultDir, err = fileutil.EnsureDirExists(resultDir); err != nil {
		return err
	}

	for _, dev := range runDetails.Devices {
		resultFileName := filepath.Join(resultDir, dev.ID)
		err := util.WithWriteFile(resultFileName, func(w io.Writer) error {
			return json.NewEncoder(w).Encode(dev.Results)
		})
		if err != nil {
			return err
		}
