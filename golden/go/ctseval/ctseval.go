...
func (c *CTSEvaluator) Process(resultsFile ingestion.ResultFileLocation) error {
	...
	for _, dev := range metaData.Devices {
		for _, versionID := range dev.VersionIDs {
			// Derive the result directory for the device and version and process it.
			path := fmt.Sprintf(devPathTmpl, gsDir, dev.ID, versionID)
			resultDev := &Device{FirebaseDevice: dev, VersionID: versionID}
			if err := c.processOneDevice(path, resultDev); err != nil {
				resultDev.ProcessingErr = err.Error()
				sklog.Errorf("Error processing device %s (version %s): %s", dev.Name, versionID, err)
			}
	...
	if err := c.addResult(idx, exps, result); err != nil {
		return err
	}
...
func (c *CTSEvaluator) processOneDevice(dirPath string, dev *Device) error {
	...
	sklog.Infof("Result path: %s", resultPath)
...
func (c *CTSEvaluator) addResult(idx *indexer.SearchIndex, exps *expstorage.Expectations, runDetails *CTSRunDetails) error {
	// Get the tallies to extract digests by test name.
	tallies := idx.TalliesByTest(false)
	for _, dev := range runDetails.Devices {
		if dev == nil {
			sklog.Infof("RESULT is nil WHYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY !")
		}
		for _, runResult := range dev.Results {
			...
			if _, ok := tallies[testName]; !ok {
				runResult.ErrorMsg = fmt.Sprintf("Processing error: Unknown test: %s", testName)
				continue
			}
			...
			if err != nil {
				runResult.ErrorMsg = fmt.Sprintf("Unable to diff %s (%s) against known digests.", runResult.ImgPath, runResult.ImgID)
				continue
			}

			// // Add the diffs to the run result.
			// runResult.ClosestMetric, runResult.Diff = getSmallestDiff(diffs, runResult.Name, diff.METRIC_COMBINED, idx)
	...
	// Write the entire test run to  disk.
...
func getSmallestDiff(diffMap map[string]interface{}, testName, metric string, idx *indexer.SearchIndex) (string, *search.SRDiffDigest) {
	if len(diffMap) == 0 {
		return "", nil
	}
	...
	for digest, metricRec := range diffMap {
		diffMetrics := metricRec.(*diff.DiffMetrics)
		if diffMetrics.Diffs[metric] < minVal {
			minVal = diffMetrics.Diffs[metric]
			minDigest = digest
		}
	}

	return metric, &search.SRDiffDigest{
		Test:        testName,
		Digest:      minDigest,
		DiffMetrics: diffMap[minDigest].(*diff.DiffMetrics),
		ParamSet:    idx.GetParamsetSummary(testName, minDigest, false),
	}
}
...
func (c *CTSEvaluator) getRunDetails(path string) (*CTSRunDetails, error) {
	...
	runResult := &CTSRunDetails{}
	if err := json.NewDecoder(file).Decode(runResult); err != nil {
		return nil, err
	}
