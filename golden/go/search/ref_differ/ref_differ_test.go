...
func TestGetRefDiffsSunnyDay(t *testing.T) {
	...
	ref, diffs := rd.GetRefDiffs(metric, matches, TestName, untriagedDigest, params, rhsQuery, is)
	assert.Equal(t, ref, common.PositiveRef)
...
// TestGetRefDiffsNoMatches tests the case when the first digest for a test
// is uploaded an there are no positive nor negative matches seen previously.
func TestGetRefDiffsNoMatches(t *testing.T) {
	unittest.SmallTest(t)
	...
	is := types.ExcludeIgnoredTraces
	ref, diffs := rd.GetRefDiffs(metric, matches, TestName, untriagedDigest, params, rhsQuery, is)
	assert.Equal(t, ref, common.NoRef)
	assert.NotNil(t, diffs)
