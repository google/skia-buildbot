...
func iterTile(query *Query, addFn AddFn, acceptFn AcceptFn, exp *expstorage.Expectations, idx *indexer.SearchIndex) error {
	tile := idx.GetTile(query.IncludeIgnores)
	exp, err := storages.ExpectationsStore.Get().Get()
	if err != nil {
		return err
	}
