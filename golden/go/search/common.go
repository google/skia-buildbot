...
func iterTile(query *Query, addFn AddFn, acceptFn AcceptFn, exp ExpSlice, idx *indexer.SearchIndex) error {
	...
	lastTraceIdx, traceView, err := getSubtraceFn(tile, query.FCommitBegin, query.FCommitEnd)
	if err != nil {
		return err
	}
...
func getSubtraceFn(tile *tiling.Tile, startHash, endHash string) (int, traceViewFn, error) {
