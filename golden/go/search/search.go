...
func CompareTest(ctQuery *CTQuery, storages *storage.Storage, idx *indexer.SearchIndex) (*CTResponse, error) {
	...
	rowLimit := ctQuery.RowQuery.Limit
	columnLimit := ctQuery.ColumnQuery.Limit

	// Build the rows, sort them and trim them.
	rows := getSortedRows(rowDigests, ctQuery.SortRows, ctQuery.RowsDir, rowLimit, idx)
	...
	for idx, rowElement := range rows {
		go func(idx int, digest string, paramSet paramtools.ParamSet) {
			defer wg.Done()
			cells[idx], err = getDiffRow(storages.DiffStore, digest, columnDigests[idx], ctQuery.SortColumns, ctQuery.ColumnsDir)
			if err != nil {
...
func filterTile(query *Query, testName string, storages *storage.Storage, idx *indexer.SearchIndex) (map[string]paramtools.ParamSet, error) {
	...
	addFn := func(digest string, trace tiling.Trace) {
...
func filterTileWithMatch(query *Query, testName string, matchFields []string, condParams map[string]paramtools.ParamSet, storages *storage.Storage, idx *indexer.SearchIndex) (map[string][]string, error) {
	ret := make(util.StringSet, len(idx.TalliesByTest()[testName]))

	var acceptFn AcceptFn
	if len(matchFields) == 0 {
		acceptFn = func(trace tiling.Trace) bool { return true }
	} else {
		acceptFn = func(trace tiling.Trace) bool {
			params := trace.Params()
			for _, field := range matchFields {
				for _, paramSet := range condParams {
...
// Iter tile
type AcceptFn func(trace tiling.Trace) bool
type AddFn func(digest string, trace tiling.Trace)
...
func iterTile(query *Query, addFn AddFn, acceptFn AcceptFn, storages *storage.Storage, idx *indexer.SearchIndex) error {
	...
	if acceptFn == nil {
		acceptFn = func(tr tiling.Trace) bool { return true }
	}
...
func getSortedRows(entries map[string]paramtools.ParamSet, sortField, sortDir string, limit int, idx *indexer.SearchIndex) []*CTRow {
	tallies := idx.TalliesByTest()["something"]
