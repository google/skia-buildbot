...
// the provided instance of CTQuey is consistent in that the row query and
...
func CompareTest(ctq *CTQuery, storages *storage.Storage, idx *indexer.SearchIndex) (*CTResponse, error) {
	...
	rows := getSortedRows(rowDigests, ctq.SortRows, ctq.RowsDir, ctq.RowQuery.Limit, idx)
...
// filterTile iterates over the tile and finds digests match the given query.
// It returns the digests and
...
// tested against the query) and calls addFn to add a digests and it's trace.
