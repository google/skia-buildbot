...
func (s *SearchImpl) Search(ctx context.Context, q *query.Search) (*frontend.SearchResponse, error) {
	...
	exp, err := s.getExpectationsForCL(q.ChangeListID, crs)
	if err != nil {
...
func (s *SearchImpl) GetDigestDetails(ctx context.Context, test types.TestName, digest types.Digest) (*frontend.DigestDetails, error) {
	...
	exp, err := s.getExpectationsForCL("", "")
	if err != nil {
		return nil, skerr.Wrap(err)
	}
...
// getExpectationsForCL returns a slice of expectations that should be
...
// tile are returned.
func (s *SearchImpl) getExpectationsForCL(clID, crs string) (common.ExpSlice, error) {
