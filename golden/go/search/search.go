// Package search package contains the core functionality for searching for digests across a
// sliding window of the last N commits. N is set per instance, but is typically between
// 100 and 500 commits.
package search
...
func (s *SearchImpl) getDrawableTraces(test types.TestName, digest types.Digest, last int, exp expectations.Classifier, traces map[tiling.TraceID]*types.GoldenTrace, comments []frontend.TraceComment) *frontend.TraceGroup {
	...
	for i, traceID := range traceIDs {
		...
		// digest of focus will be digestStatus[1] and so on, up until we hit maxDistinctDigestsToPresent.
		for j := last; j >= 0; j-- {
			...
			if d != digest {
				if index := digestIndex(d, digestStatuses); index != -1 {
					...
					if len(digestStatuses) < maxDistinctDigestsToPresent {
						...
						refDigestStatus = maxDistinctDigestsToPresent - 1
					}
				}
			}
