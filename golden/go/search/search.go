...
func (s *SearchImpl) GetDigestDetails(ctx context.Context, test types.TestName, digest types.Digest, clID, crs string) (*frontend.DigestDetails, error) {
	...
	// We put the data in an SRDigest to re-use the search functions
	...
	// Again, we wrap the result in a slice so we can re-use the search functions.
...
func (s *SearchImpl) queryChangeList(ctx context.Context, q *query.Search, idx indexer.IndexSearcher, exp expectations.Classifier) ([]*frontend.SRDigest, error) {
	...
	byGroupingAndDigest := map[groupAndTest]*frontend.SRDigest{}
	addByGroupAndDigest := func(test types.TestName, digest types.Digest, params paramtools.Params) {
		...
		key := groupAndTest{grouping: test, digest: digest}
	...
	ret := make([]*frontend.SRDigest, 0, len(byGroupingAndDigest))
