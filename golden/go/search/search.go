...
func (s *SearchImpl) Search(ctx context.Context, q *query.Search) (*frontend.SearchResponse, error) {
	...
	s.prepareTraceGroups(displayRet, exp, traceComments)
...
func (s *SearchImpl) GetDigestDetails(ctx context.Context, test types.TestName, digest types.Digest, clID, crs string) (*frontend.DigestDetails, error) {
	...
	if len(result.TraceGroup.Traces) > 0 {
		...
		traceComments = s.getTraceComments(ctx)
		s.prepareTraceGroups(results, exp, traceComments)
	}
...
// on the frontend.
func (s *SearchImpl) prepareTraceGroups(searchResults []*frontend.SearchResult, exp expectations.Classifier, comments []frontend.TraceComment) {
	for _, di := range searchResults {
		// Add the drawable traces to the result.
		s.fillInFrontEndTraceData(di.Test, di.Digest, exp, &di.TraceGroup, comments)
	}
}
...
func (s *SearchImpl) fillInFrontEndTraceData(test types.TestName, digest types.Digest, exp expectations.Classifier, traceGroup *frontend.TraceGroup, comments []frontend.TraceComment) {
