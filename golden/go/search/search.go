...
func CompareDigests(test types.TestName, left, right types.Digest, storages *storage.Storage, idx *indexer.SearchIndex) (*SRDigestDiff, error) {
	// Get the diff between the two digests
	diffResult, err := storages.DiffStore.Get(diff.PRIORITY_NOW, left, []types.Digest{right})
	if err != nil {
		return nil, err
	}
...
func filterTile(query *Query, storages *storage.Storage, idx *indexer.SearchIndex) (map[types.Digest]paramtools.ParamSet, error) {
...
func getFilterByTileFunctions(matchFields []string, condDigests map[types.Digest]paramtools.ParamSet, target *map[types.Digest]types.DigestSet) (AcceptFn, AddFn) {
	*target = make(map[types.Digest]types.DigestSet, len(condDigests))
	...
	if len(matchFields) >= 0 {
		matching := make([]types.Digest, 0, len(condDigests))
		acceptFn = func(params paramtools.Params, digests []types.Digest) (bool, interface{}) {
			matching = matching[:0]
		...
		addFn = func(test types.TestName, digest types.Digest, traceID tiling.TraceId, trace *types.GoldenTrace, acceptRet interface{}) {
			for _, d := range acceptRet.([]types.Digest) {
...
func filterTileWithMatch(query *Query, matchFields []string, condDigests map[types.Digest]paramtools.ParamSet, storages *storage.Storage, idx *indexer.SearchIndex) (map[types.Digest]types.DigestSet, error) {
...
func getDiffs(diffStore diff.DiffStore, digest types.Digest, colDigests []types.Digest, sortDir, diffMetric string, limit int32) ([]*CTDiffMetrics, int, error) {
