...
func prepareTraceGroups(searchResults []*frontend.SearchResult, exp expectations.Classifier, comments []frontend.TraceComment, appendPrimaryDigest bool) {
	for _, di := range searchResults {
		...
		fillInFrontEndTraceData(di.Test, di.Digest, exp, &di.TraceGroup, comments, appendPrimaryDigest)
	}
}
...
func fillInFrontEndTraceData(test types.TestName, digest types.Digest, exp expectations.Classifier, traceGroup *frontend.TraceGroup, comments []frontend.TraceComment, appendPrimaryDigest bool) {
