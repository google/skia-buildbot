...
func (q *Query) IgnoreState() types.IgnoreState {
	is := types.ExcludeIgnoredDigests
	if q.IncludeIgnores {
		is = types.IncludeIgnoredDigests
	}
	return is
}
...
func CompareDigests(test types.TestName, left, right types.Digest, storages *storage.Storage, idx *indexer.SearchIndex) (*SRDigestDiff, error) {
	...
	diffResult, err := storages.DiffStore.Get(diff.PRIORITY_NOW, left, types.DigestSlice{right})
	if err != nil {
		return nil, err
	}
	...
	return &SRDigestDiff{
		Left: &SRDigest{
			...
			ParamSet: idx.GetParamsetSummary(test, left, types.IncludeIgnoredDigests),
		},
		Right: &SRDiffDigest{
			Test:        test,
			Digest:      right,
			Status:      exp.Classification(test, right).String(),
			ParamSet:    idx.GetParamsetSummary(test, right, types.IncludeIgnoredDigests),
...
func CompareTest(ctq *CTQuery, storages *storage.Storage, idx *indexer.SearchIndex) (*CTResponse, error) {
	...
	testSummaries := idx.GetSummaries(types.ExcludeIgnoredDigests)
