...
func (qf queryFuture) getResult() (*gtile.SubSearchable, error) {
	ret := <-qf
	successRet, ok := ret.(*gtile.SubSearchable)
	if ok {
...
func (s *SearchAPI) Search2(ctx context.Context, q *Query) (*NewSearchResponse, error) {
	...
	if getRefDiffs {
		...
		ret, err = s.getDiffs(ctx, inter, rhsDigestsFt, q.Metric, q.Match, q.IncludeIgnores, exp, idx)
		if err != nil {
			return nil, err
		}
...
func (s *SearchAPI) getDiffs(ctx context.Context, inter srInterMap, rhsDigestsFt queryFuture, metric string, match []string, includeIgnores bool, exp ExpSlice, idx *indexer.SearchIndex) ([]*SRDigest, error) {
	...
	retDigests := make([]*SRDigest, 0, inter.numDigests())
	var wg sync.WaitGroup
	for testName, digests := range inter {
		wg.Add(1)
		go func(testName string, digests map[string]*srIntermediate) {
			searchable := rhsSearch.SearchPrimary(testName)
			matchVals := make([][]string, len(match))
			for digest, inter := range digests {
				for idx, key := range match {
					matchVals[idx] = inter.params[key]
				}

				strDigests, _ := searchable.MatchVals(matchVals).Digests()
				diffsMap, err := s.storages.DiffStore.Get(diff.PRIORITY_NOW, digest, strDigests)
				if err != nil {
					sklog.Errorf("Error calculating diffs: %s", err)
				} else {
					retDigests = append(retDigests, s.getDigestRefDiffs(digest, diffsMap, metric))
				}
				searchable.Reset()
			}
			wg.Done()
...
func (s *SearchAPI) getDigestRefDiffs(digest string, diffsMap map[string]interface{}, metric string) *SRDigest {
	return nil
}
