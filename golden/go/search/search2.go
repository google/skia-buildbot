...
func (s *SearchAPI) getDiffs(ctx context.Context, inter srInterMap, rhsDigestsFt queryFuture, metric string, match []string, includeIgnores bool, exp ExpSlice, idx *indexer.SearchIndex) ([]*SRDigest, error) {
	...
	retDigests := make([]*SRDigest, inter.numDigests())
	retIdx := 0
	var wg sync.WaitGroup
	for testName, digests := range inter {
		wg.Add(1)
		go func(testName string, digests map[string]*srIntermediate, startIdx int) {
			...
			for digest, inter := range digests {
				...
				strDigests, intDigests := searchable.MatchVals(matchVals).Digests()
				ret, err := s.storages.DiffStore.Get(diff.PRIORITY_NOW, digest, strDigests)
				if err != nil {
					sklog.Errorf("Error calculating diffs: %s", err)
				} else {
					retDigests[startIdx] = getDigestWithRefs(digest, ret, metric)
				}
				startIdx++
				searchable.Reset()
			}
			wg.Done()
		}(testName, digests, retIdx)
		retIdx += len(digests)
	}
	wg.Wait()
