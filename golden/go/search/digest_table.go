...
func (s *SearchImpl) GetDigestTable(q *query.DigestTable) (*frontend.DigestTable, error) {
	...
	rowDigests, err := s.filterTileCompare(q.RowQuery)
	if err != nil {
		return nil, err
	}
	...
	rows := getDTRows(rowDigests, q.SortRows, q.RowsDir, q.RowQuery.Limit, q.RowQuery.IgnoreState(), s.indexSource.GetIndex())
	...
	columnDigests, err := s.filterTileWithMatch(q.ColumnQuery, q.Match, rowDigests)
	if err != nil {
		return nil, err
	}
	...
	testSummaries := s.indexSource.GetIndex().GetSummaries(types.ExcludeIgnoredTraces)
...
func (s *SearchImpl) filterTileCompare(q *query.Search) (map[types.Digest]paramtools.ParamSet, error) {
	...
	if err := iterTile(q, addFn, nil, common.ExpSlice{exp}, s.indexSource.GetIndex()); err != nil {
		return nil, err
	}
	return ret, nil
}
...
func (s *SearchImpl) filterTileWithMatch(q *query.Search, matchFields []string, condDigests map[types.Digest]paramtools.ParamSet) (map[types.Digest]types.DigestSet, error) {
	...
	if err := iterTile(q, addFn, acceptFn, common.ExpSlice{exp}, s.indexSource.GetIndex()); err != nil {
		return nil, err
	}
	return ret, nil
}
