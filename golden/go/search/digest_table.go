...
func (c *SearchImpl) GetDigestTable(q *query.DigestTable) (*frontend.DigestTable, error) {
	// Retrieve the row digests.
	rowDigests, err := c.filterTileCompare(q.RowQuery)
	if err != nil {
		return nil, err
	}
	...
	rows := getDTRows(rowDigests, q.SortRows, q.RowsDir, q.RowQuery.Limit, q.RowQuery.IgnoreState(), c.indexSource.GetIndex())
	...
	columnDigests, err := c.filterTileWithMatch(q.ColumnQuery, q.Match, rowDigests)
	if err != nil {
		return nil, err
	}
	...
	for idx, rowElement := range rows {
		go func(idx int, digest types.Digest) {
			...
			var total int
			var err error
			rows[idx].Values, total, err = getDiffs(c.diffStore, digest, columnDigests[digest].Keys(), q.ColumnsDir, q.Metric, q.ColumnQuery.Limit)
			if err != nil {
	...
	testSummaries := c.indexSource.GetIndex().GetSummaries(types.ExcludeIgnoredTraces)
	...
	ret := &frontend.DigestTable{
		...
		Corpus:    q.RowQuery.Values.Get(types.CORPUS_FIELD),
...
func (c *SearchImpl) filterTileCompare(q *query.Search) (map[types.Digest]paramtools.ParamSet, error) {
	...
	if err := iterTile(q, addFn, nil, common.ExpSlice{exp}, c.indexSource.GetIndex()); err != nil {
		return nil, err
	}
	return ret, nil
}
...
func (c *SearchImpl) filterTileWithMatch(q *query.Search, matchFields []string, condDigests map[types.Digest]paramtools.ParamSet) (map[types.Digest]types.DigestSet, error) {
	...
	if err := iterTile(q, addFn, acceptFn, common.ExpSlice{exp}, c.indexSource.GetIndex()); err != nil {
		return nil, err
	}
	return ret, nil
}
