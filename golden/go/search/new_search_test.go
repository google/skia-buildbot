...
func TestNewSearchAPI(t *testing.T) {
	...
	testQuery := "fdiffmax=-1&fref=false&frgbamax=-1&head=true&include=false&limit=50&match=gamma_correct&match=name&metric=combined&neg=false&pos=false&query=source_type%3Dgm&sort=desc&unt=true"
	checkQuery(t, api, idx, testQuery)
}
...
func checkQuery(t *testing.T, api *SearchAPI, idx *indexer.SearchIndex, qStr string) {
	q := &Query{}
