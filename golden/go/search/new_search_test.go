...
// but BetaBrandNewDigest remains negative.
...
func TestSearchThreeDevicesChangeListSunnyDay(t *testing.T) {
	...
	mcls.On("GetChangeList", anyctx, clID).Return(code_review.ChangeList{
		SystemID: clID,
	...
	assert.NoError(t, err)
	assert.NotNil(t, resp)
	assert.Len(t, anglerGroup, 1) // make sure the group maps were not mutated.
