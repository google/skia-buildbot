...
func ParseCTQuery(r io.ReadCloser, limitDefault int32, q *CompareTests) error {
	...
	if q.RowQuery.Values, err = url.ParseQuery(q.RowQuery.QueryStr); err != nil {
		return skerr.Wrapf(err, "parsing RowQuery %s", q.RowQuery.QueryStr)
	}
	if q.ColumnQuery.Values, err = url.ParseQuery(q.ColumnQuery.QueryStr); err != nil {
		return skerr.Wrapf(err, "parsing ColumnQuery %s", q.ColumnQuery.QueryStr)
	}

	rowCorpus := q.RowQuery.Values.Get(types.CORPUS_FIELD)
	colCorpus := q.ColumnQuery.Values.Get(types.CORPUS_FIELD)
...
func ParseSearch(r *http.Request, q *Search) error {
	...
	q.Values = validate.QueryFormValue(r, "query")
	q.RQuery = validate.QueryFormValue(r, "rquery")
