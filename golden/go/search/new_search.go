...
const (
	// MAX_REF_DIGESTS is the maxium number of digests we want to show
	// in a dotted line of traces. We assume that more than digests yield no additional
	// information, because the trace is likely to be flaky.
	MAX_REF_DIGESTS = 8
)

// TODO (stephana): Remove the Search(...) function in
// search.go once the search function implemented here
// is feature complete. This requires renaming some of
// the types that currently have a "SR" prefix.
// This includes these types:
//     SRDigest
//     SRDiffDigest
//     NewSearchResponse
...
// by the Search function of SearchAPI type.
...
// a primary digest and the digest given here. What the
// the primary digest is follows from the context.
...
// SearchAPI is that type that exposes a query API
type SearchAPI struct {
	storages *storage.Storage
	ixr      *indexer.Indexer
}

// Create a new instance of the SearchAPI.
...
// srIntermediate is the intermediate represenation of a single digest
...
// getReferenceDiffs compares all digests collected in 'inter' and compares them
// to the other known results for the test at hand.
func (s *SearchAPI) getReferenceDiffs(q *Query, inter map[string]map[string]*srIntermediate, exp *expstorage.Expectations, idx *indexer.SearchIndex) []*SRDigest {
	...
	// This allows to allocate the result below without and avoid using
...
// the slice that should be shown on the page with the offset in the entire
// result set.
...
// getDrawableTraces returns an instance of Traces which allows to draw the traces for the given test/digest.
