...
// getReferenceDiffs compares all digests collected in the intermediate representation
// and compares them to the other known results for the test at hand.
func (s *SearchAPI) getRefDiffs(ctx context.Context, inter srInterMap, metric string, match []string, includeIgnores bool, exp ExpSlice, idx *indexer.SearchIndex) {
	ctx, span := trace.StartSpan(ctx, "search/getRefDiffs")
	defer span.End()
