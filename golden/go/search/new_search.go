...
// getReferenceDiffs compares all digests collected in the intermediate representation
// and compares them to the other known results for the test at hand.
func (s *SearchAPI) getReferenceDiffsFromInter(ctx context.Context, inter srInterMap, metric string, match []string, includeIgnores bool, exp ExpSlice, idx *indexer.SearchIndex) {
	// Query the tile if there is a right hand side query.
	gTile := idx.GTile(includeIgnores)

	rhsQuery := url.Values{}

	intRHS := gTile.IntParamSet(paramtools.ParamSet(rhsQuery))
	rhsInter := gTile.Search(intRHS)

	ctx, span := trace.StartSpan(ctx, "search/getReferenceDiffsFromInter")
	defer span.End()

	// refDiffer := NewRefDiffer(exp, s.storages.DiffStore, idx)
	var wg sync.WaitGroup
	for _, digestsByTest := range inter {
		wg.Add(1)
		go func(testName string, digestsByTest map[string]*srIntermediate) {
			defer wg.Done()
			for digest, inter := range digestsByTest {
				  diffss.getRefDiffFast(rhsQuery, digest, inter)

			subQuery := map[string][]string{
				types.PRIMARY_KEY_FIELD: {"whatever"},
			}

			// This gets everything on the right hand side.
			newSet := rhsInter.Search(gTile.IntParamSet(subQuery))
			digests := newSet.Digests()
			s.getRefDiffs(getCachedDiffs(digests, digestsByTest))
