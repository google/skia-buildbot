...
func (s *SearchAPI) Search(ctx context.Context, q *Query) (*NewSearchResponse, error) {
	...
	if getRefDiffs {
		...
		s.getReferenceDiffs(ctx, ret, q.Metric, q.Match, paramtools.ParamSet(q.rhsQuery.Query), q.IncludeIgnores, exp, idx)
		if err != nil {
			return nil, err
		}
...
func (s *SearchAPI) getReferenceDiffs(ctx context.Context, resultDigests []*SRDigest, metric string, match []string, rhsQuery paramtools.ParamSet, includeIgnores bool, exp ExpSlice, idx *indexer.SearchIndex) {
	...
	for _, retDigest := range resultDigests {
		go func(retDigest *SRDigest) {
			closestRef, refDiffs := refDiffer.GetRefDiffs(ctx, metric, match, retDigest.Test, retDigest.Digest, retDigest.ParamSet, rhsQuery, includeIgnores)
