...
func (s *SearchAPI) CreateBaseline(ctx context.Context, issueID, patchsetID int64) error {
	...
	issue, err := s.extractIssueDigests(ctx, q, idx, issueExp, issueInter.add)
	if err != nil {
	...
	baseLine := s.getBaseline(masterInter, issueInter)
...
func (s *SearchAPI) getMasterDigests(issue *tryjobstore.Issue, patchsetID int64) (srInterMap, error) {
	...
	baseExp, err := e.storages.GetExpectationsForCommit(patchset.ParentCommit)
	if err != nil {
