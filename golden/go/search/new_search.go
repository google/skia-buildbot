...
func (s *SearchAPI) Search(q *Query) (*NewSearchResponse, error) {
	...
	// Get the index.
	idx := s.ixr.GetIndex()
	// if err := ValidateQuery(q, idx); err != nil {
	// 	return nil, err
	// }

	// Iterate through the tile and get everything matching the query.
	...
	total := len(ret)
	ret = s.sortAndLimitDigests(q, ret, q.Offset, q.Limit)
...
func (s *SearchAPI) afterDiffResultFilter(digestInfo []*SRDigest, q *Query) []*SRDigest {
	// Only do anything if all filters of interest were set.
	if (q.FRGBAMax <= 0) && (q.FDiffMax <= 0) {
		return digestInfo
	}
	...
	for _, digest := range digestInfo {
		...
		if ok && digest.ClosestRef == "" {
			glog.Infof("Got ok with an emtpy closestRef")
		}

		// Filter all digests where MaxRGBA is above a certain threshold.
		if (q.FRGBAMax > 0) && (!ok || (util.MaxInt(ref.MaxRGBADiffs...) > q.FRGBAMax)) {
			glog.Infof("Ignoring rgbamax: %s", digest.Digest)
			continue
		}

		// Filter all digests where the diff is below the given threshold.
		if (q.FDiffMax > 0) && (!ok || (ref.Diffs[q.Metric] > q.FDiffMax)) {
			glog.Infof("Ignoring diffmax: %s", digest.Digest)
			continue
		}

		// If selected only consider digests that have a reference to compare to.
		if q.FRef && !ok {
			glog.Infof("Ignoring fref: %s", digest.Digest)
			continue
		}
...
// TODO(stephana)
func (s *SearchAPI) sortAndLimitDigests(q *Query, digestInfo []*SRDigest, offset, limit int) []*SRDigest {
