...
func (s *SearchAPI) Search(q *Query) (*NewSearchResponse, error) {
	...
	ret := s.getReferenceDiffs(q, inter, idx)
	if err != nil {
		return nil, err
	}
...
// TODO(stephana)
func (s *SearchAPI) getReferenceDiffs(q *Query, inter map[string]map[string]*srIntermediate, idx *indexer.SearchIndex) []*SRDigest {
	...
	for testName, testDigests := range inter {
		for digest, interValue := range testDigests {
			go func(interValue *srIntermediate, index int) {
				closestRef, refDiffs := referenceDiffs(s.storages)
				digestInfo := &SRDigest{
					Test:       test,
					Digest:     digest,
					Status:     "FILLME",
