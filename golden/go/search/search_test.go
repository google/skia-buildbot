...
// TestNewlySeenUntriagedUnignoredDigestsSunnyDay models the case where a set of TryJobs has
...
// for this CL (tjUntriagedGamma), and one trace matched an ignore rule (tjUntriagedDelta). Thus,
// We only expect tjUntriagedAlpha and tjUntriagedBeta to be reported to us.
func TestNewlySeenUntriagedUnignoredDigestsSunnyDay(t *testing.T) {
	unittest.SmallTest(t)
	...
	const tjUntriagedAlpha = types.Digest("aaaa65e567de97c8a62918401731c7ec")
	const tjUntriagedBeta = types.Digest("bbbb34f7c915a1ac3a5ba524c741946c")
	const tjUntriagedGamma = types.Digest("cccc41bf4584e51be99e423707157277")
	const tjUntriagedDelta = types.Digest("dddd84e51be99e42370715727765e563")
	...
	mes.On("ForChangeList", clID, crs).Return(issueStore, nil)
	var ie expectations.Expectations
	ie.Set(data.AlphaTest, tjUntriagedGamma, expectations.Negative)
	issueStore.On("Get", testutils.AnyContext).Return(&ie, nil)
	mes.On("Get", testutils.AnyContext).Return(data.MakeTestExpectations(), nil)
	...
	dl, err := s.NewlySeenUntriagedUnignoredDigests(context.Background(), expectedID)
	require.NoError(t, err)
	assert.Equal(t, []types.Digest{tjUntriagedAlpha, tjUntriagedBeta}, dl.Digests)
}
