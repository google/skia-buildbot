...
func TestSearch_ThreeDevicesCorpusWithComments_CommentsInResults(t *testing.T) {
	...
	betaBullheadComment := trace.Comment{
		ID:        "4",
	...
	mcs.On("ListComments", testutils.AnyContext).Return([]trace.Comment{commentAppliesToNothing, alphaTestComment, betaBullheadComment, bullheadComment}, nil)
	...
	assert.Equal(t, []frontend.TraceComment{
		f(bullheadComment), f(alphaTestComment), f(betaBullheadComment), f(commentAppliesToNothing),
	}, resp.TraceComments)

	// These numbers are indices into the resp.TraceComments
