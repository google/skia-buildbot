...
func TestGetDrawableTraces_DigestIndicesAreCorrect(t *testing.T) {
	...
	test := func(desc string, inputDigests []types.Digest, expectedData []int) {
		t.Run(desc, func(t *testing.T) {
			...
			rv := s.getDrawableTraces("whatever", d0, len(inputDigests)-1, makeStubClassifier(), traces, nil)
	...
	test("missing digests",
		...
		[]int{missingDigestIndex, 1, missingDigestIndex, 0, missingDigestIndex})
	test("multiple missing digest in a row",
		[]types.Digest{mm, mm, mm, d1, d1, mm, mm, mm, d0, mm, mm},
		[]int{missingDigestIndex, missingDigestIndex, missingDigestIndex, 1, 1, missingDigestIndex, missingDigestIndex, missingDigestIndex, 0, missingDigestIndex, missingDigestIndex})
	test("all the same",
...
// makeStubClassifier returns an expectations.Classifier that returns Positive for every call.
func makeStubClassifier() *mock_expectations.Classifier {
	mes := &mock_expectations.Classifier{}
	mes.On("Classification", mock.Anything, mock.Anything).Return(expectations.Positive)
	return mes
}
