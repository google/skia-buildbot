...
func (r *RefDiffer) GetRefDiffs(metric string, match []string, test, digest string, params paramtools.ParamSet, traces []tiling.Trace) (string, map[string]*SRDiffDigest) {
...
func (r *RefDiffer) getDigestsWithLabel(test string, match []string, params paramtools.ParamSet, paramsByDigest map[string]paramtools.ParamSet, unavailable map[string]*diff.DigestFailure, targetLabel types.Label) []string {
	...
	for d, digestParams := range paramsByDigest {
		// class := r.exp.Classification(test, d)
		// paramsMatch := paramSetsMatch(match, params, digestParams)
		// fmt.Printf("CLASS: %s\n", class)
		// fmt.Printf("PARAM: %v     %s    --    %s\n", paramsMatch, spew.Sprint(params), spew.Sprint(digestParams))
