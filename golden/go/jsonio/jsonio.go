...
type rawGoldResults struct {
	...
	// (once we get passed then, TryJob results that old probably won't matter)
...
func (r *rawGoldResults) parse() (*GoldResults, error) {
	...
	if r.ChangeListID == "" && r.GerritChangeListID == "" {
		...
		if r.ChangeListID != "" {
			...
			// Set GerritChangeListID to 0 or -1 to indicate "master branch"
			if n, err := strconv.ParseInt(r.GerritChangeListID, 10, 64); err != nil {
				return nil, skerr.Wrapf(err, "invalid value for issue: %q", r.GerritChangeListID)
			} else {
				ret.GerritChangeListID = n
...
func (g *GoldResults) Validate(ignoreResults bool) error {
	...
	// Validate the fields
	if !regExHexadecimal.MatchString(g.GitHash) {
		return skerr.Fmt("field %q must be hexadecimal. Received '%s'", jn["GitHash"], g.GitHash)
	}
	if len(g.Key) == 0 || !hasNonEmptyKV(g.Key) {
		return skerr.Fmt("field %q must not be empty and must not have empty keys or values", jn["Key"])
	}
	...
	if !ignoreResults {
		...
		for _, r := range g.Results {
			if err := r.validate(); err != nil {
				return skerr.Wrapf(err, "validating field %q", jn["Results"])
			}
		}
	}
...
func (r *Result) validate() error {
	...
	jn := resultJsonMap
	if len(r.Key) == 0 || !hasNonEmptyKV(r.Key) {
		return skerr.Fmt("field %q must be non-empty and must not have empty keys or values", jn["Key"])
	}
	...
	if !regExHexadecimal.MatchString(string(r.Digest)) {
		return skerr.Fmt("field %q must be hexadecimal", jn["Digest"])
	}
	return nil
}
