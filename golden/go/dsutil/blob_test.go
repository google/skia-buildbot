...
func TestBlobStore(t *testing.T) {
	...
	// for any type that can be searialized to JSON.
	...
	assert.NoError(t, err)
	changeFrags := len(fragments)
	assert.True(t, changeFrags > 1)

	// Make sure we have the right number of fragments in the store.
	nFragments, err := client.Count(ctx, ds.NewQuery(blobFragKind))
	assert.NoError(t, err)
	assert.Equal(t, changeFrags, nFragments)
