...
// It will add the key to the set of recently added keys and removes any keysUpdate
...
// Recently is a container for recent changes that is used to store the keys in
// the database and to manipulate them once they are loaded in the database.
// It's members should never be accessed directly. Instead the Add and
...
type Recently struct {
	...
	// Added contains the sorted keys that were deleted within the consistency time window
	Deleted []*datastore.Key
}

// Combine assumes that all keys are time based (created via TimeSortableKey) and
// therefore sortable. It merges the keys of the two slices,
...
// sortableIDMask is a bit mask witht the lowest 63 bits set to 1. It is used to
...
// 	bumped.
// The key id is this value XORed with sortableIDMask (lowest 63 bit set to 1).
