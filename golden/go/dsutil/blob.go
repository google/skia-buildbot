...
func (b *BlobStore) Save(data interface{}) (key *datastore.Key, err error) {
	defer timer.New("BlobStore.Save").Stop()
	ctx := context.TODO()
	blob := &blobParent{}
	...
	purgeKeys := blob.Children
	defer func() {
		if err != nil {
			util.LogErr(b.client.DeleteMulti(ctx, purgeKeys))
		}
	}()
...
// sequence of entities. It returns the keys of the newsly created entities that
// contain the blob.
