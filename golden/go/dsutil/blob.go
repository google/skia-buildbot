...
// maxBlobFragSize is the size of a blob fragment and slightly smaller than a
// the maximum size of an entity which is 1MiB - 4
const maxBlobFragSize = 1000 * 1000
...
func jsonEncodeBlobParts(src interface{}) ([]*blobFrag, error) {
	...
	nPages := int(math.Ceil(float64(len(body)) / float64(maxBlobFragSize)))

	ret := make([]*blobFrag, nPages)
	for p := 0; p < nPages; p++ {
		start := p * maxBlobFragSize
		ret[p] = &blobFrag{
			Body: body[start:util.MinInt(start+maxBlobFragSize, len(body))],
		}
	}
	return ret, nil
}
...
// keysFromISlice converts a slice of []interface{} to []*datastore.Key
...
// iSliceFromKeys converts a slice of []*datastore.Key to []interface{}
