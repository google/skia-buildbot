...
func TestRecently(t *testing.T) {
	...
	deepequal_testutils.AssertDeepEqual(t, expContainer, container)
	...
	deepequal_testutils.AssertDeepEqual(t, expCombined, someRecentKeys.Combine(someQueryResult))

	// Make sure we get the recent key changes if the query result is empty
	expCombined = []*datastore.Key{k4, k3, k1}
	deepequal_testutils.AssertDeepEqual(t, expCombined, someRecentKeys.Combine(nil))

	// Make sure we get the query result when the recent keys are empty
	expCombined = []*datastore.Key{k5, k4, k3, k2}
	deepequal_testutils.AssertDeepEqual(t, expCombined, (&Recently{}).Combine(someQueryResult))
