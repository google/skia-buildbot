...
func TestListHelper(t *testing.T) {
	...
	containerKey := ds.NewKey(ds.RECENT_KEYS)
	containerKey.Name = "test-list-helper"
	listHelper := NewListHelper(client, containerKey, DefaultConsistencyDelta)

	// Make sure we get an empty slice if we never call the helper.
	foundKeySlice, err := listHelper.GetRecent()
	assert.NoError(t, err)
	assert.NotNil(t, foundKeySlice)
	assert.Equal(t, 0, len(foundKeySlice))
