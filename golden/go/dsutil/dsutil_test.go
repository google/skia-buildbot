...
func TestKeyContainer(t *testing.T) {
	...
	testutils.AssertDeepEqual(t, expContainer, container)
	...
	testutils.AssertDeepEqual(t, expCombined, someRecentKeys.Combine(someQueryResult))

	// Make sure we get the recent key changes if the query result is empty
	expCombined = []*datastore.Key{k4, k3, k1}
	testutils.AssertDeepEqual(t, expCombined, someRecentKeys.Combine(nil))

	// Make sure we get the query result when the recent keys are empty
	expCombined = []*datastore.Key{k5, k4, k3, k2}
	testutils.AssertDeepEqual(t, expCombined, (&Recently{}).Combine(someQueryResult))
...
func TestTimeBasedKeyID(t *testing.T) {
	...
	ts2 := util.TimeStamp(time.Millisecond)
	keyID2 := getSortableTimeID(ts2)
	assert.NotEqual(t, GetTimeFromID(keyID), keyID2)
	epoch := time.Time()
	assert.Equal(t, 0)
}
