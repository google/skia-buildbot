...
// GetChangeLists provides a mock function with given fields: ctx, startIdx, limit, opts
func (_m *Store) GetChangeLists(ctx context.Context, startIdx int, limit int, opts *clstore.SearchOptions) ([]code_review.ChangeList, int, error) {
	ret := _m.Called(ctx, startIdx, limit, opts)

	var r0 []code_review.ChangeList
	if rf, ok := ret.Get(0).(func(context.Context, int, int, *clstore.SearchOptions) []code_review.ChangeList); ok {
		r0 = rf(ctx, startIdx, limit, opts)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]code_review.ChangeList)
		}
	...
	var r1 int
	if rf, ok := ret.Get(1).(func(context.Context, int, int, *clstore.SearchOptions) int); ok {
		r1 = rf(ctx, startIdx, limit, opts)
	} else {
		r1 = ret.Get(1).(int)
	}

	var r2 error
	if rf, ok := ret.Get(2).(func(context.Context, int, int, *clstore.SearchOptions) error); ok {
		r2 = rf(ctx, startIdx, limit, opts)
	} else {
		r2 = ret.Error(2)
	}
