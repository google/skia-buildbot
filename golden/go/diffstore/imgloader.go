...
func (il *ImageLoader) downloadImg(gsPath string) ([]byte, error) {
	...
	if err != nil {
		return nil, skerr.Fmt("Unable to retrieve attributes for %s/%s: %.80s", il.gsBucketClient.Bucket(), objLocation, err)
	}
	...
	for i := 0; i < MAX_URI_GET_TRIES; i++ {
		...
		err = func() error {
			...
			if err != nil {
				return fmt.Errorf("New reader failed for %s/%s: %s", il.gsBucketClient.Bucket(), objLocation, err)
			}
