...
type ImageLoader struct {
	...
	// gsImageRunDir is the GCS directory that specifies the run, usually in userid-timestamp format.
	gsImageRunDir string
...
func newImgLoader(client *http.Client, baseDir, imgDir string, gsBucketNames []string, gsImageBaseDirs string, gsImageRunDir string, usesDigests bool, maxCacheSize int) (*ImageLoader, error) {
...
func (il *ImageLoader) downloadImgFromBucket(digest, bucketName string) ([]byte, error) {
	objLocation := filepath.Join(il.gsImageBaseDirs, il.gsImageRunDir, getDigestImageFileName(digest))
	...
	if !il.usesDigests {
		...
		objLocation = filepath.Join(il.gsImageBaseDirs, il.gsImageRunDir, getDigestImageFileName(gsPath))
	}
