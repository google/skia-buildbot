...
func TestNetDiffStoreGetSunnyDay(t *testing.T) {
	...
	nds, err := diffstore.NewForTesting(msc, mockAddress)
...
// TestNetDiffStoreGetCache tests to make sure we cache the results locally for individual results
// to save on network requests.
func TestNetDiffStoreGetCache(t *testing.T) {
	unittest.SmallTest(t)

	msc := &mocks.DiffServiceClient{}
	defer msc.AssertExpectations(t)

	firstRequest := &diffstore.GetDiffsRequest{
		MainDigest:   string(digest1),
		RightDigests: []string{string(digest2)},
	}
	msc.On("GetDiffs", testutils.AnyContext, firstRequest).Return(&diffstore.GetDiffsResponse{
		Diffs: encodeToBytes(map[types.Digest]*diff.DiffMetrics{
			digest2: {
				MaxRGBADiffs: [4]int{1, 2, 3, 4},
			},
		}),
	}, nil).Once()
	secondRequest := &diffstore.GetDiffsRequest{
		MainDigest:   string(digest1),
		RightDigests: []string{string(digest3)},
	}
	msc.On("GetDiffs", testutils.AnyContext, secondRequest).Return(&diffstore.GetDiffsResponse{
		Diffs: encodeToBytes(map[types.Digest]*diff.DiffMetrics{
			digest3: {
				MaxRGBADiffs: [4]int{5, 6, 7, 8},
			},
	...
	metrics, err := nds.Get(context.Background(), digest1, types.DigestSlice{digest2})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Len(t, metrics, 2)
	assert.Equal(t, map[types.Digest]*diff.DiffMetrics{
		digest2: {
			MaxRGBADiffs: [4]int{1, 2, 3, 4},
		},
		digest3: {
...
// version of the failures map and the
...
func TestNetDiffStoreUnavailableSunnyDayDebounce(t *testing.T) {
	unittest.SmallTest(t)
