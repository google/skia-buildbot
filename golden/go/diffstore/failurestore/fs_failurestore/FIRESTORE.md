...
fields below.
	  ...
	  Digest string  # The digest that failed.
	  Reason string  # One of "corrupted", "http_error" or "other". See type diff.DiffErr.
	  TS     int     # Timestamp in milliseconds since the epoch.

Documents in this collection keyed by digest ID and timestamp, e.g. "<digest>-<ts>".
...
Usage
-----
Digest failures are queried all at once; when failures are read the entire collection is retrieved.
