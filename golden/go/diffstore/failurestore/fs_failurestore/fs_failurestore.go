...
func toStoreEntry(failure *diff.DigestFailure) *storeEntry {
	// Convert failure.TS into a time.Time instance.
	durationSinceEpoch := time.Duration(failure.TS) * time.Millisecond // failure.TS is expressed in milliseconds since epoch.
	secondsSinceEpoch := int64(durationSinceEpoch.Seconds())
	nanoseconds := int64(durationSinceEpoch - time.Duration(secondsSinceEpoch)*time.Second)
	timestamp := time.Unix(secondsSinceEpoch, nanoseconds)
...
func (s *FSImpl) UnavailableDigests(ctx context.Context) (map[types.Digest]*diff.DigestFailure, error) {
	...
	// Populate results.
	results := make(map[types.Digest]*diff.DigestFailure)
...
func (s *FSImpl) PurgeDigestFailures(ctx context.Context, digests types.DigestSlice) error {
	...
	// Retrieve all unavailable digests.
