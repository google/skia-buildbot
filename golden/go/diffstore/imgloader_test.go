...
func TestImageLoaderGetSingleDigestNotFound(t *testing.T) {
	...
	// Failure is stored.
	df1 := mock.MatchedBy(diffFailureMatcher(digest1, "http_error"))
	mockFailureStore.On("AddDigestFailure", df1).Return(nil)
	df2 := mock.MatchedBy(diffFailureMatcher(digest1, "other"))
	mockFailureStore.On("AddDigestFailureIfNew", df2).Return(nil)
...
func TestImageLoaderGetMultipleDigestsDigest1FoundInBucketDigest2NotFound(t *testing.T) {
	...
	// Failure is stored.
	df1 := mock.MatchedBy(diffFailureMatcher(digest2, "http_error"))
	mockFailureStore.On("AddDigestFailure", df1).Return(nil)
	df2 := mock.MatchedBy(diffFailureMatcher(digest2, "other"))
	mockFailureStore.On("AddDigestFailureIfNew", df2).Return(nil)
...
func diffFailureMatcher(digest types.Digest, reason diff.DiffErr) func(failure *diff.DigestFailure) bool {
	return func(failure *diff.DigestFailure) bool {
