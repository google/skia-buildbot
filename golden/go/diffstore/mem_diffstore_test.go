...
func TestMemDiffStoreGetSunnyDay(t *testing.T) {
	...
	expectImageWillBeRead(mgc, image1GsPath, image1Md5Hash, image1)
	expectImageWillBeRead(mgc, image2GsPath, image2Md5Hash, image2)
	expectImageWillBeRead(mgc, image3GsPath, image3Md5Hash, image3)
	...
	diffStore, err := NewMemDiffStore(mgc, gsImageBaseDir, 1, mms, mfs)
	require.NoError(t, err)
...
// TestMemDiffStoreGetGCSIntegration performs the Get operation backed by real GCS.
func TestMemDiffStoreGetGCSIntegration(t *testing.T) {
	...
	// Assume everything is a cache miss
	mms.On("LoadDiffMetrics", mock.Anything).Return(nil, nil)
	mms.On("SaveDiffMetrics", common.DiffID(original, cross), dm).Return(nil)

	diffStore, err := NewMemDiffStore(gcsClient, gsImageBaseDir, 1, mms, mfs)
	require.NoError(t, err)
...
func TestFailureHandlingGet(t *testing.T) {
	...
	img := fmt.Sprintf("%s/%s.png", gsImageBaseDir, invalidDigest1)
	mgc.On("GetFileObjectAttrs", testutils.AnyContext, img).Return(nil, errors.New("not found"))
	img = fmt.Sprintf("%s/%s.png", gsImageBaseDir, invalidDigest2)
	mgc.On("GetFileObjectAttrs", testutils.AnyContext, img).Return(nil, errors.New("not found"))
...
func TestGetUnavailable(t *testing.T) {
	...
	diffStore, err := NewMemDiffStore(nil, gsImageBaseDir, 1, nil, mfs)
	require.NoError(t, err)
