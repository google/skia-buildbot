...
func TestGoldDiffStoreMapper(t *testing.T) {
	...
	// Test for GCS paths.
	gcsImgID1 := GCSPathToImageID(TEST_BUCKET, TEST_PATH_IMG_1)
	...
	exp = GS_PREFIX + "/" + TEST_BUCKET + "/" + TEST_PATH_IMG_1
	assert.Equal(t, exp, localPath)
	assert.Equal(t, gsBucket, TEST_BUCKET)
...
func TestBase62Encoding(t *testing.T) {
	someStr := "sadkl;j;sadkfjsadklfjasdlk+z/=)fjasdl;kjasdl;fasdl;fasdl;fasdlfkjasdlfj/asdfj"
	for n := 0; n < 1000; n++ {
		start := rand.Int() % (len(someStr) + 1)
		rStr := someStr[start:]
...
func TestGCSImageIDs(t *testing.T) {
	imgID1 := GCSPathToImageID(TEST_BUCKET, TEST_PATH_IMG_1)
	bucket, path := ImageIDToGCSPath(imgID1)
	assert.Equal(t, TEST_BUCKET, bucket)
	...
	assert.Equal(t, TEST_BUCKET+"/"+TEST_PATH_IMG_1, decImgID1+"."+IMG_EXTENSION)
}
