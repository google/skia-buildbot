...
func NewMemDiffStore(client *http.Client, baseDir string, gsBucketNames []string, gsImageBaseDir string, gigs int, diffFn diff.DiffFn, codec util.LRUCodec, mapper IDPathMapper) (diff.DiffStore, error) {
	...
	mStore, err := newMetricStore(baseDir, mapper.SplitDiffID, codec)
	if err != nil {
		return nil, err
	}
...
func (d *MemDiffStore) saveDiffInfoAsync(diffID, leftDigest, rightDigest string, dm interface{}, imgBytes []byte) {
	d.wg.Add(2)
	go func() {
		defer d.wg.Done()
		if err := d.metricsStore.saveDiffMetric(diffID, dm); err != nil {
