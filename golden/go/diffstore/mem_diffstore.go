...
func (d *MemDiffStore) WarmDigests(priority int64, digests []types.Digest, sync bool) {
	missingDigests := make([]types.Digest, 0, len(digests))
...
func (d *MemDiffStore) WarmDiffs(priority int64, leftDigests []types.Digest, rightDigests []types.Digest) {
...
func (d *MemDiffStore) Get(priority int64, mainDigest types.Digest, rightDigests []types.Digest) (map[types.Digest]interface{}, error) {
	if mainDigest == "" {
...
func (m *MemDiffStore) PurgeDigests(digests []types.Digest, purgeGCS bool) error {
	...
	digestSet := make(map[types.Digest]bool, len(digests))
	...
	handlerFunc := func(w http.ResponseWriter, r *http.Request) {
		...
		if dir == DEFAULT_IMG_DIR_NAME {
			...
			if !m.imgLoader.IsOnDisk(imgDigest) {
				_, pendingWrites, err := m.imgLoader.Get(diff.PRIORITY_NOW, []types.Digest{imgDigest})
				if err != nil {
...
func (d *MemDiffStore) diffMetricsWorker(priority int64, id string) (interface{}, error) {
	...
	imgs, _, err := d.imgLoader.Get(priority, []types.Digest{leftDigest, rightDigest})
	if err != nil {
		return nil, err
	}
...
func (d *MemDiffStore) getDiffIds(leftDigests, rightDigests []types.Digest) []string {
