...
func (m *MemDiffStore) ImageHandler(urlPrefix string) (http.Handler, error) {
	...
	handlerFunc := func(w http.ResponseWriter, r *http.Request) {
		...
		if dir == DEFAULT_IMG_DIR_NAME {
			...
			localRelPath, _ = m.imgLoader.ImagePaths(imgDigest)
			...
			leftImgLocalRelPath, _ := m.imgLoader.ImagePaths(leftImgDigest)
			rightImgLocalRelPath, _ := m.imgLoader.ImagePaths(rightImgDigest)
