...
func NewMemDiffStore(client *http.Client, baseDir string, gsBucketNames []string, gsImageBaseDirs string, gsImageRunDir string, usesDigests bool, gigs int) (diff.DiffStore, error) {
	...
	imgLoader, err := newImgLoader(client, baseDir, imgDir, gsBucketNames, gsImageBaseDirs, gsImageRunDir, usesDigests, imageCacheCount)
	if err != err {
		return nil, err
	}
