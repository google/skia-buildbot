...
func (m *MemDiffStore) ImageHandler(urlPrefix string) (http.Handler, error) {
	handlerFunc := func(w http.ResponseWriter, r *http.Request) {
		...
		if dir == DEFAULT_IMG_DIR_NAME {
			...
			// Write output image to the http.ResponseWriter. Content-Type is set automatically based on the
			// first 512 bytes of written data. See docs for ResponseWriter.Write() for details.
