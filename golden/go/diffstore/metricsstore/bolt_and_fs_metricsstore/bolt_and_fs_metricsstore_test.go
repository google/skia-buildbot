...
func TestSave(t *testing.T) {
	...
	actualDiffMetrics, err := s.(*storeImpl).boltStore.LoadDiffMetrics(id)
	assert.NoError(t, err)
	assert.Equal(t, expectedDiffMetrics, actualDiffMetrics)

	// Assert that the Firestore-backed store contains it.
	actualDiffMetrics, err = s.(*storeImpl).fsStore.LoadDiffMetrics(id)
	assert.NoError(t, err)
	assert.Equal(t, expectedDiffMetrics, actualDiffMetrics)
}
...
func TestLoad(t *testing.T) {
	...
	err = s.(*storeImpl).boltStore.SaveDiffMetrics(id, boltDiffMetrics)
	assert.NoError(t, err)
	err = s.(*storeImpl).fsStore.SaveDiffMetrics(id, firestoreDiffMetrics)
	assert.NoError(t, err)
...
func TestPurge(t *testing.T) {
	...
	err = s.(*storeImpl).boltStore.SaveDiffMetrics(id, diffMetrics)
	assert.NoError(t, err)
	err = s.(*storeImpl).fsStore.SaveDiffMetrics(id, diffMetrics)
	assert.NoError(t, err)
	...
	dm, err := s.(*storeImpl).boltStore.LoadDiffMetrics(id)
	assert.NoError(t, err)
	assert.Nil(t, dm)
	dm, err = s.(*storeImpl).fsStore.LoadDiffMetrics(id)
	assert.NoError(t, err)
	assert.Nil(t, dm)
}
