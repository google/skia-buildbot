...
func (t *TrybotState) updateTryjobState(issueID int64, params *trybotstore.Parameters, patchsetID int64, tryjob *trybotstore.Tryjob) error {
	...
	// Add the Tryjob.
	if err := t.trybotStore.AddTryjob(issueID, patchsetID, tryjob); err != nil {
		return err
	}
	return nil
}
...
func extractPatchsetDetails(changeInfo *gerrit.ChangeInfo) map[int64]*trybotstore.PatchsetDetail {
	ret := make(map[int64]*trybotstore.PatchsetDetail, len(changeInfo.Patchsets))
	for _, revision := range changeInfo.Patchsets {
		ret[revision.Number] = &trybotstore.PatchsetDetail{
			ID: revision.Number,
		}
	}
	return ret
