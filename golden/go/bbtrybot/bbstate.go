		...
		for build := range buildsCh {
			...
			go func(build *bb_api.ApiCommonBuildMessage) {
				...
				if err := t.updateTryjobState(params, tryjob); err != nil {
					...
					sklog.Infof("Updated issue and tryjob (%d, %d)", tryjob.IssueID, tryjob.Buildnumber)
...
func (t *TrybotState) updateTryjobState(params *trybotstore.Parameters, tryjob *trybotstore.Tryjob) error {
	// Find the existing issue in the trybot store.
	issue, err := t.trybotStore.GetIssue(tryjob.IssueID, false, nil)
	if err != nil {
		return err
	}
	...
	// Add the Tryjob.
	if err := t.trybotStore.UpdateTryjob(tryjob.IssueID, tryjob); err != nil {
		return fmt.Errorf("Error updating issue and tryjob (%d, %d). Got error: %s", tryjob.IssueID, tryjob.Buildnumber, err)
	}
	return nil
}
...
func (t *TrybotState) setIssueDetails(issueID int64, changeInfo *gerrit.ChangeInfo, issue *trybotstore.IssueDetails) {
...
func getTryJobInfo(build *bb_api.ApiCommonBuildMessage, params *trybotstore.Parameters) (*trybotstore.Tryjob, error) {
	...
	if status == trybotstore.TRYJOB_UNKNOWN {
		return nil, fmt.Errorf("Unknown trybot state. Got (status, result): (%s, %s)", build.Status, build.Result)
	}
