...
func (t *TrybotState) start() {
	...
	go func() {
		for build := range buildsCh {
			// Parse the parameters extracted from the
			params := &Parameters{}
			...
			patchsetID, tryjob, err := getTryJobInfo(build, params)
			if err != nil {
				sklog.Errorf("Error extracting tryjob info: %s", err)
				continue
			}

			if err := t.updateTryjobState(build, params, patchsetID, tryjob); err != nil {
...
func (t *TrybotState) updateTryjobState(build *bb_api.ApiCommonBuildMessage, params *Parameters, patchsetID int64, tryjob *Tryjob) error {
	...
	changed, err := issue.addBuild(patchsetID, tryjob)
	if err != nil {
		return err
	}

	if changed {
		if err := t.trybotStore.Add(issue); err != nil {
			sklog.Errorf("Unable to save change issue information: %s", err)
		}
	}
	return nil
}
...
func setIssueDetails(issueID int64, changeInfo *gerrit.ChangeInfo, issue *IssueDetails) {
	...
	issue.PatchsetDetails = extractPatchsetDetails(changeInfo)
...
func getTryJobInfo(build *bb_api.ApiCommonBuildMessage, params *Parameters) (int64, *Tryjob, error) {
	patchsetID, err := strconv.ParseInt(params.Properties.GerritPatchset, 10, 64)
	if err != nil {
		return 0, nil, err
	}

	// Translate the two result fields into one for tryjobs.
	var status TryjobStatus
	switch build.Status {
