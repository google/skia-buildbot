...
// AllLabel is a list of all possible Label values. The index of each element in this list must
// match its corresponding fs_expectationstore.fsExpectationsLabel value (Untriaged = 0, etc.).
...
func ValidLabel(label Label) bool {
	for _, validLabel := range AllLabel {
		if label == validLabel {
			return true
		}
	}
