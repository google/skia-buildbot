...
const (
	...
	// These paramaters were chosen somewhat arbitrarily to make sure that if Firestore takes a while,
...
// set of expectation entries into the local RAM cache. It will generally be the case that the
// primary expectation store (i.e. the one for the master partition) will have Initialize called,
// because it will persist for a while and be passed around to many places. Instances of Store
// returned by ForChangeList are, in generaly, not going to have this called since they will not
// persist very long.
...
func (s *Store2) ForChangeList(id, crs string) expectations.Store {
	if id == "" || crs == "" {
...
func (s *Store2) makeEntriesAndChanges(ctx context.Context, now time.Time, delta []expectations.Delta, firstIdx, lastIdx int) ([]expectationEntry2, []expectationChange, error) {
...
func (s *Store2) updateEntryCacheIfNeeded(ctx context.Context) error {
...
func (s *Store2) GetCopy(ctx context.Context) (*expectations.Expectations, error) {
	if s.hasSnapshotsRunning {
		// If the snapshots are running, we first check to see if we have a fresh Expectations.
		s.returnCacheMutex.Lock()
		defer s.returnCacheMutex.Unlock()
...
// to expedite the process. The fetched expectationEntry2 will be cached in the s.entryCache, under
...
func (s *Store2) loadExpectations(ctx context.Context) (*expectations.Expectations, error) {
	...
	toCache := map[expectations.ID]expectationEntry2{}
	for _, entries := range es {
...
func (s *Store2) QueryLog(ctx context.Context, offset, size int, details bool) ([]expectations.TriageLogEntry, int, error) {
	if offset < 0 || size <= 0 {
...
func (s *Store2) UndoChange(ctx context.Context, changeID, userID string) error {
	if s.mode == ReadOnly {
		return ReadOnlyErr
	}
	defer metrics2.FuncTimer().Stop()
...
func (s *Store2) UpdateLastUsed(ctx context.Context, ids []expectations.ID, now time.Time) error {
	if s.partition != masterPartition {
		return skerr.Fmt("Cannot call Update Last Used except on the master partition")
	}
	if len(ids) == 0 {
		return nil
	}
...
func (s *Store2) MarkUnusedEntriesForGC(context.Context, expectations.Label, time.Time) (int, error) {
	panic("implement me")
	// TODO(kjlubick) setting a field on expectationEntry2 is probably easier than doing stuff with
	//   Untriaged, especially now that we have ranges.
	//   First query on time, and then filter the positive/negative stuff after, since we might have
	//   to do range calculation.
}

func (s *Store2) GarbageCollect(context.Context) (int, error) {
	panic("implement me")
}
...
var _ expectations.Store = (*Store2)(nil)

// Make sure Store2 fulfills the expectations.GarbageCollector interface
var _ expectations.GarbageCollector = (*Store2)(nil)
