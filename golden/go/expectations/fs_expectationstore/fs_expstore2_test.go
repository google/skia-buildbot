...
func TestMarkUnusedEntriesForGC_CLEntriesNotAffected_Success(t *testing.T) {
	...
	clExp := exp.ForChangeList("cl1234", "crs")
...
func getRawCLEntry(ctx context.Context, t *testing.T, c *ifirestore.Client, name types.TestName, digest types.Digest, crsCLID string) *expectationEntry2 {
	entry := expectationEntry2{Grouping: name, Digest: digest}
