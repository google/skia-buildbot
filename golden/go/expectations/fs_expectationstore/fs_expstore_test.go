...
// TestUpdateLastUsed_NoEntriesToUpdate_NothingChanges
...
// TestUpdateLastUsed_OneEntry_Success calls UpdateLastUsed with one entry and verifies that only
// the last_used field is modified and only for the specified entry.
func TestUpdateLastUsed_OneEntry_Success(t *testing.T) {
	unittest.LargeTest(t)
	c, cleanup := firestore.NewClientForTesting(t)
	defer cleanup()
	ctx, cancel := context.WithCancel(context.Background())
...
func TestMarkUnusedEntriesForGC_EntriesRecentlyUsed_NoEntriesMarked_Success(t *testing.T) {
	...
	test := func(name string, label expectations.Label, ts time.Time) {
		n, err := exp.MarkUnusedEntriesForGC(ctx, label, ts)
		require.NoError(t, err)
		assert.Equal(t, 0, n)
...
// TestMarkUnusedEntriesForGC_FirstEntryAffected tests where a single entry (the first) is marked
// as untriaged.
func TestMarkUnusedEntriesForGC_FirstEntryAffected(t *testing.T) {
	unittest.LargeTest(t)
...
// TestMarkUnusedEntriesForGC_MiddleEntryAffected tests where the middle entry (the only negative)
// entry is marked as untriaged.
func TestMarkUnusedEntriesForGC_MiddleEntryAffected(t *testing.T) {
	unittest.LargeTest(t)
	...
	exp, err := New(ctx, c, nil, ReadWrite)
	require.NoError(t, err)
	entryOne, _, entryThree := populateFirestore(ctx, t, c, updatedLongAgo)

	n, err := exp.MarkUnusedEntriesForGC(ctx, expectations.Negative, entryThree.LastUsed.Add(time.Minute))
	require.NoError(t, err)
	assert.Equal(t, 1, n)
...
func TestGarbageCollect_MultipleEntriesDeleted(t *testing.T) {
	...
	actualEntryOne := getRawEntry(ctx, t, c, entryOne.Grouping, entryOne.Digest)
	require.Nil(t, actualEntryOne)
	actualEntryTwo := getRawEntry(ctx, t, c, entryTwo.Grouping, entryTwo.Digest)
	require.NotNil(t, actualEntryTwo)
	assert.Equal(t, expectations.Negative, actualEntryTwo.Label)
	actualEntryThree := getRawEntry(ctx, t, c, entryThree.Grouping, entryThree.Digest)
