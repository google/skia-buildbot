...
type DiffStore interface {
	...
	// specified in dRest.
	Get(priority int64, mainDigest types.Digest, rightDigests []types.Digest) (map[types.Digest]interface{}, error)
	...
	WarmDigests(priority int64, digests []types.Digest, sync bool)

	// WarmDiffs will calculate the difference between every digests in
	// leftDigests and every in digests in rightDigests.
	WarmDiffs(priority int64, leftDigests []types.Digest, rightDigests []types.Digest)
	...
	PurgeDigests(digests []types.Digest, purgeGCS bool) error
}
