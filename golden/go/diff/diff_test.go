...
func assertDiffs(t *testing.T, d1, d2 string, expectedDiffMetrics *DiffMetrics) {
	img1, err := OpenNRGBA(filepath.Join(TESTDATA_DIR, d1+".png"))
	if err != nil {
		t.Fatal("Failed to open test file: ", err)
	}
	img2, err := OpenNRGBA(filepath.Join(TESTDATA_DIR, d2+".png"))
	if err != nil {
		t.Fatal("Failed to open test file: ", err)
	}
...
func loadBenchmarkImages() {
	var err error
	img1, err = OpenNRGBA(filepath.Join(TESTDATA_DIR, "4029959456464745507.png"))
	if err != nil {
		sklog.Fatal("Failed to open test file: ", err)
	}
	img2, err = OpenNRGBA(filepath.Join(TESTDATA_DIR, "16465366847175223174.png"))
	if err != nil {
		sklog.Fatal("Failed to open test file: ", err)
	}
}
