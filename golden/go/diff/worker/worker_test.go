...
func TestCalculateDiffs_CorruptedImage_PartialData(t *testing.T) {
	...
	_, err := db.Exec(ctx, `INSERT INTO ProblemImages (digest, num_errors, latest_error, error_ts)
VALUES ($1, $2, $3, $4)`, dks.DigestA04Unt, 100, "not found", time.Date(2020, time.January, 1, 1, 1, 1, 0, time.UTC))
	require.NoError(t, err)
	waitForSystemTime()
	...
	problem := actualProblemImageRows[0]
	assert.Equal(t, string(dks.DigestA04Unt), problem.Digest)
	// The sentinel value at 100. This should be greater than that because of the new error.
