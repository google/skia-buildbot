...
func main() {
	...
	serviceName, err := getServiceName(*local)
	if err != nil {
		sklog.Fatalf("Error getting unique service name: %s", err)
	}

	// Set up tracing via the sktrace.
	traceClient, err := sktrace.NewTraceClient(common.PROJECT_ID, serviceName, tokenSource)
	if err != nil {
	...
	if *eventTopic != "" {
		evt, err = gevent.New(common.PROJECT_ID, *eventTopic, serviceName, option.WithTokenSource(tokenSource))
		if err != nil {
			sklog.Fatalf("Unable to create global event client. Got error: %s", err)
		}
		sklog.Infof("Global eventbus for topic '%s' and subscriber '%s' created.", *eventTopic, serviceName)
	} else {
		evt = eventbus.New()
	}
...
// getServiceName generates a subscriber name based on the hostname and
// whether we are running locally or in the cloud. This is enough to distinguish
// between hosts.
func getServiceName(local bool) (string, error) {
