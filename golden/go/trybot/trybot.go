...
type TrybotResults struct {
	...
	gerritPatchsetCache   *cache.Cache
}
...
func NewTrybotResults(tileBuilder tracedb.BranchTileBuilder, rietveldAPI *rietveld.Rietveld, gerritAPI *gerrit.Gerrit, ingestionStore *goldingestion.IngestionStore) *TrybotResults {
	ret := &TrybotResults{
		...
		gerritPatchsetCache:   cache.New(PATCHSET_CACHE_EXPIRATION, PATCHSET_CACHE_CLEANUP_INTERVAL),
	}
	return ret
}
...
func (t *TrybotResults) extractGerritPatchsetDetails(issueID, patchsetID int64) (*PatchsetDetail, error) {
	...
	for _, build := range builds {
		...
		// TODO(stephana): Set the whether the tryjob has been ingested once the
		// the build scheduler/buildbucket has an API to retrieve the status of a job.
		// This will require a different call to the ingestion store but we will have
		// to keep track in the above code whether a job needs to be checked for
		// ingestion status.  i.e.:
		// if checkIngested && t.ingestionStore.IsIngested(config.CONSTRUCTOR_GOLD, tj.Master, tj.Builder, tj.BuildNumber) {
		// 	status = TRYJOB_INGESTED
		// 	tjIngested++
		// }
...
func getIssuePatchSetsAndModified(cid *tracedb.CommitIDLong, isGerrit bool) ([]int64, int64) {
	if isGerrit {
		...
		return changeInfo.GetPachsetIDs(), changeInfo.Updated.Unix()
	}
