...
// non-deterministic, for example, when using an errgroup. It checks that the message matches
// a context being canceled or contains the given strings.
func assertErrorWasCanceledOrContains(t *testing.T, err error, messages ...string) {
	require.Error(t, err)
	e := err.Error()
