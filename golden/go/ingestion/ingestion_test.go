...
func TestStart_ProcessesDataFromSources_Success(t *testing.T) {
	...
	sourceOne := newFakeSource()
	sourceTwo := newFakeSource()
	sources := []Source{sourceOne, sourceTwo}

	ingester, err := newIngester("foo", noPollingConfig(), nil, sources, mp, mis, meb)
	require.NoError(t, err)
...
func TestStart_PollsDataFromSources_ResultsAlreadyProcessed_Success(t *testing.T) {
	...
	ingester, err := newIngester("foo", lastHourPollingConfig(), nil, sources, mp, mis, meb)
	require.NoError(t, err)
	require.NotNil(t, ingester)
...
// The following was generated by mockery, but copied here to avoid a dependency cycle.
