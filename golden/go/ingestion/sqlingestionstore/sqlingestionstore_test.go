...
func TestSetIngested_WritesToDeprecatedIngestedFiles_cdb(t *testing.T) {
	...
	assert.ElementsMatch(t, []schema.DeprecatedIngestedFileRow{{
...
func TestWasIngested_AlreadyStored_Success_cdb(t *testing.T) {
	ctx := context.Background()
	db := sqltest.NewCockroachDBForTestsWithProductionSchema(ctx, t)
	store := New(db)
...
func TestWasIngested_StoredLater_Success_cdb(t *testing.T) {
	ctx := context.Background()
	db := sqltest.NewCockroachDBForTestsWithProductionSchema(ctx, t)
	store := New(db)
