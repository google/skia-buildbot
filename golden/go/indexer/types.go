...
func (c *ChangeListIndex) Copy() *ChangeListIndex {
	rv := &ChangeListIndex{
		...
		UntriagedResults: make(map[tjstore.CombinedPSID][]tjstore.TryJobResult, len(c.UntriagedResults)),
	}
	// Copy the map values
	for key, xtr := range c.UntriagedResults {
		newTr := make([]tjstore.TryJobResult, len(xtr))
		copy(newTr, xtr)
		rv.UntriagedResults[key] = newTr
	}
	return rv
}
