...
func (ix *Indexer) sendCLWorkToDiffCalculators(ctx context.Context, xtjr []tjstore.TryJobResult, clID string) error {
	...
	left, right, err := addDataFromPrimaryBranch(ctx, idx, digestsPerGrouping, exp)
	if err != nil {
		return skerr.Wrap(err)
	}
...
func addDataFromPrimaryBranch(ctx context.Context, idx *SearchIndex, leftDigests map[hashableGrouping]types.DigestSet, exp expectations.ReadOnly) (map[hashableGrouping]types.DigestSet, map[hashableGrouping]types.DigestSet, error) {
