...
func (ix *Indexer) calcChangeListIndices(ctx context.Context, state interface{}) error {
	...
	// If needed, we can probably do this in 4 parallel chunks.
	for _, cl := range xcl {
		if err := ctx.Err(); err != nil {
		...
		clExp, err := issueExpStore.Get(ctx)
		if err != nil {
			return skerr.Wrapf(err, "loading expectations for cl %s (%s)", cl.SystemID, crs)
		}
		exp := expectations.Join(clExp, masterExp)

		key := fmt.Sprintf("%s_%s", crs, cl.SystemID)
		...
		if !ok || clIdx.ComputedTS.Before(cl.Updated) {
			...
			filtered := filterUntriagedResults(xtjr, exp)
			if !ok {
		...
		// Re-apply ignore rules, expectations on existing TryJob results. Then, update the timestamp
		// and repush it (unless cl.Updated is too old?)
		for psID, xtjr := range clIdx.UntriagedResultsProduced {
			// One of these entries might be newly created (and therefore already
			if psID.Equal(alreadyFilteredPS) {
				continue
			}
			clIdx.UntriagedResultsProduced[psID] = filterUntriagedResults(xtjr, exp)
		}
		clIdx.ComputedTS = now
		ix.changeListIndices.Set(key, clIdx, ttlcache.DefaultExpiration)
	}
	return nil
}

func filterUntriagedResults(xtjr []tjstore.TryJobResult, exp expectations.Classifier) []tjstore.TryJobResult {
	var rv []tjstore.TryJobResult
	for _, tjr := range xtjr {
		tn := types.TestName(tjr.ResultParams[types.PrimaryKeyField])
		if exp.Classification(tn, tjr.Digest) == expectations.Untriaged {
			rv = append(rv, tjr)
		}
	}
	return rv
