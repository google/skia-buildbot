...
type countsAndBlames []*summary.DigestsForTest
...
func (idx *SearchIndex) GetSummaries(is types.IgnoreState) []*summary.DigestsForTest {
	return idx.summaries[is]
}

// CalcSummaries implements the IndexSearcher interface.
func (idx *SearchIndex) CalcSummaries(query url.Values, is types.IgnoreState, head bool) ([]*summary.DigestsForTest, error) {
...
func calcSummaries(state interface{}) error {
	...
	for _, is := range types.IgnoreStates {
		...
		if len(idx.testNames) > 0 && len(idx.summaries[is]) > 0 {
			idx.summaries[is] = summary.OverwriteWithNew(idx.summaries[is], sum)
		} else {
