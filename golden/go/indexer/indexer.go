...
func preSliceData(state interface{}) error {
	...
	for _, is := range types.IgnoreStates {
		...
		for id, tr := range t.Traces {
			...
			idx.preSliced[ignoreOnly] = append(idx.preSliced[ignoreOnly], types.TracePair{
				ID:    id,
				Trace: gt,
			})
			...
			idx.preSliced[ignoreAndCorpus] = append(idx.preSliced[ignoreAndCorpus], types.TracePair{
				ID:    id,
				Trace: gt,
			})
			...
			idx.preSliced[ignoreCorpusTest] = append(idx.preSliced[ignoreCorpusTest], types.TracePair{
				ID:    id,
				Trace: gt,
			})
		}
