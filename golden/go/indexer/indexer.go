...
func New(storages *storage.Storage, interval time.Duration) (*Indexer, error) {
	...
	countsNode := root.Child(calcDigestCounts)
	...
	paramsNode := pdag.NewNodeWithParents(calcParamsets, countsNode)

	// Write known hashes after ignores are computed
	writeHashes := countsNode.Child(writeKnownHashesList)

	// Summaries depend on DigestCounter and Blamer.
	summariesNode := pdag.NewNodeWithParents(calcSummaries, countsNode, blamerNode)
	...
	pdag.NewNodeWithParents(ret.setIndex, summariesNode, paramsNode, writeHashes)
...
func (ixr *Indexer) writeIssueBaseline(evData interface{}) {
	...
	idx := ixr.GetIndex()
	if err := ixr.storages.Baseliner.PushIssueBaseline(issueID, idx.cpxTile, idx.dCounters[types.ExcludeIgnoredDigests]); err != nil {
...
func calcBlame(state interface{}) error {
	...
	b, err := blame.New(idx.cpxTile.GetTile(types.ExcludeIgnoredDigests), exp)
	if err != nil {
		idx.blamer = nil
...
func writeKnownHashesList(state interface{}) error {
	...
	go func() {
		byTest := idx.DigestCountsByTest(types.IncludeIgnoredDigests)
