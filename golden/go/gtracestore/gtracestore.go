...
type TraceStore interface {
	Put(ctx context.Context, commit *vcsinfo.LongCommit, entries []*Entry, ts time.Time) error
...
func (b *btTraceStore) Put(ctx context.Context, commit *vcsinfo.LongCommit, entries []*Entry, ts time.Time) error {
	...
	commitOffset := strconv.Itoa(commitIndex % int(b.tileSize))
	...
	for _, entry := range entries {
		...
		mut.Set(cfTraceVals, commitOffset, bigtable.Time(ts), []byte(strconv.Itoa(int(digestID))))
...
func (b *btTraceStore) updateDigestMap(tileKey TileKey, digests util.StringSet) (*DigestMap, error) {
	if b.cacheOps { /}
	b.dcMutex.RLock()
	digestMap, ok := b.digestCache[tileKey]
	b.dcMutex.RUnlock()
	...
	idsMap, err := b.getDigestIDs(digests)
	if err != nil {
		return nil, err
	}
	digestMap.Update(idsMap)

	b.dcMutex.Lock()
	b.digestCache[tileKey] = digestMap
	b.dcMutex.Unlock()
