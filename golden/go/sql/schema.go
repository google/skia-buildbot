...
--  INDEX commit_grouping_digest_idx (commit_id DESC, digest),
-- This index is for finding the Paramset for a digest
   INDEX grouping_digest_commit_idx (grouping_id, digest, commit_id),
-- This index is used to fetch trace history easily for select traces.
...
CREATE TABLE IF NOT EXISTS TiledTraceDigests (
  trace_id BYTES,
  start_commit_id INT4, -- Contains the digests seen on this trace in 
                        -- the range [start_commit_id, start_commit_id+width)
  digest BYTES,
  PRIMARY KEY (trace_id, start_commit_id, digest)
);

CREATE TABLE IF NOT EXISTS SecondaryBranchValues (
  secondary_branch_trace_id BYTES NOT NULL, -- MD5 hash of the key/values
  branch_name STRING NOT NULL, -- e.g. "gerrit_12345" or "chrome_m86"
  version_name STRING NOT NULL, -- e.g. the patchset or a branch commit
  digest BYTES NOT NULL, -- MD5 hash of the pixel data
  grouping_id BYTES NOT NULL,
  options_id BYTES NOT NULL, -- MD5 hash of the options string
  source_file_id BYTES NOT NULL, -- MD5 hash of the source file name
  tryjob_id STRING, -- e.g. "buildbucket_12345".
...
CREATE TABLE IF NOT EXISTS SecondaryBranchTraces (
  secondary_branch_trace_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON of the keys
  corpus STRING NOT NULL,
  grouping_id BYTES NOT NULL,
  keys JSONB NOT NULL, -- The trace's keys, e.g. {"color mode":"RGB", "device":"walleye", etc}
  matches_any_ignore_rule BOOL,
...
CREATE TABLE IF NOT EXISTS Options ( -- contains option keys (for all branches)
  options_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON.
  keys JSONB NOT NULL, -- keys, e.g. {"ext": "png"}
  INVERTED INDEX keys_idx (keys)
);

CREATE TABLE IF NOT EXISTS Groupings ( -- contains groupings (for all branches)
...
--  INDEX label_idx (label),
  ...
  PRIMARY KEY (grouping_id, digest) -- start_index should be on primary key too eventually.
  -- This could be adapted to deal with ranges of expectation using a start_index and an end_index.
);
...
CREATE TABLE IF NOT EXISTS SecondaryBranchExpectations (
  ...
  label SMALLINT NOT NULL, -- 0 for untriaged, 1 for positive, 2 for negative
  expectation_record_id UUID, -- If not null, the record that set this value
--  INDEX branch_label_idx (branch_name, label),
...
-- This could be adapted to deal with ranges of expectation using a start_index, end_index_before
-- and end_index_after.
);

CREATE TABLE IF NOT EXISTS ExpectationRecords (
  expectation_record_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  branch_name STRING, -- e.g. CodeReviewSystem and CL ID "gerrit_12345". Can be NULL for primary branch.
  user_name STRING,
...
-- do this via SQL statements (in a clean way).
  ...
  max_rgba_diff INT2[] NOT NULL, -- max delta in the red, green, blue, alpha channels.
  combined_metric FLOAT4 NOT NULL,
  dimensions_differ BOOL NOT NULL,
  updated_ts TIMESTAMP WITH TIME ZONE NOT NULL, -- when this metric was computed or updated. Allows for cleanup.
