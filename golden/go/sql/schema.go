...
CREATE TABLE IF NOT EXISTS TraceValues (
  ...
  INDEX expectations_idx (grouping_id, digest, commit_id DESC), -- Allows for easier joins with Expectations
-- This index allows us to look up data just by trace and commit, it is particularly handy for
-- getting data at HEAD.
  INDEX trace_and_commit_idx (trace_id, commit_id) STORING (grouping_id, digest),
...
CREATE TABLE IF NOT EXISTS TryJobValues (
  trace_id BYTES NOT NULL, -- MD5 hash of the trace string
  tryjob_id STRING NOT NULL, -- e.g. "buildbucket_12345"
  digest BYTES NOT NULL, -- MD5 hash of the pixel data
  grouping_id BYTES NOT NULL,
  options_id BYTES NOT NULL, -- MD5 hash of the options string
  changelist_id STRING NOT NULL, -- e.g. "gerrit_12345" (denormalized from tryjob_id for easy joins)
  patchset_id STRING NOT NULL,
  source_file_id BYTES NOT NULL, -- MD5 hash of the source file name
  PRIMARY KEY (changelist_id, patchset_id, trace_id)
);
...
CREATE TABLE IF NOT EXISTS TryjobTraces (
  tryjob_trace_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON of the keys
  keys JSONB NOT NULL, -- The trace's keys, e.g. {"color mode":"RGB", "device":"walleye", etc}
  matches_any_ignore_rule BOOL,
  INVERTED INDEX keys_idx (keys),
  INDEX ignored_idx (matches_any_ignore_rule)
...
CREATE TABLE IF NOT EXISTS PrimaryBranchParams (
  ...
  commit_id INT2 NOT NULL,
  PRIMARY KEY (commit_id, key, value)
);

CREATE TABLE IF NOT EXISTS TryjobParams (
  key STRING NOT NULL,
  value STRING NOT NULL,
...
CREATE TABLE IF NOT EXISTS Options ( -- contains option keys (for either Tryjobs or primary branch)
  options_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON.
  keys JSONB NOT NULL, -- keys, e.g. {"ext": "png"}
  INVERTED INDEX keys_idx (keys)
);

CREATE TABLE IF NOT EXISTS Groupings ( -- contains groupings (for either Tryjobs or primary branch)
...
CREATE TABLE IF NOT EXISTS ChangelistExpectations (
  ...
  start_index INT4, -- Reserved for future use with expectation ranges
  end_index INT4, -- Reserved for future use with expectation ranges
  INDEX changelist_idx (changelist_id, label),
