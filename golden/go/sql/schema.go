...
CREATE TABLE IF NOT EXISTS TraceValues (
  ...
  INDEX (commit_id, grouping_id, digest), -- Allows for easier joins with Expectations
  INDEX (trace_id, commit_id),
...
CREATE TABLE IF NOT EXISTS TryJobValues (
  ...
  crs_cl_id STRING NOT NULL, -- CodeReviewSystem and CL ID e.g. "gerrit_12345"
...
CREATE TABLE IF NOT EXISTS Traces (
  ...
  INVERTED INDEX (keys),
  INDEX (matches_any_ignore_rule)
);

CREATE TABLE IF NOT EXISTS CLTraces (
  cl_trace_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON of the keys
  keys JSONB NOT NULL, -- The trace's keys, e.g. {"color mode":"RGB", "device":"walleye", etc}
  matches_any_ignore_rule BOOL,
  INVERTED INDEX (keys),
  INDEX (matches_any_ignore_rule)
...
CREATE TABLE IF NOT EXISTS Options ( -- contains option keys (for either CL or primary branch)
  options_id BYTES PRIMARY KEY, -- MD5 hash of the stringified JSON.
  keys JSONB NOT NULL, -- keys, e.g. {"ext": "png"}
  INVERTED INDEX (keys)
);

CREATE TABLE IF NOT EXISTS Groupings ( -- contains groupings (for either CL or primary branch)
...
CREATE TABLE IF NOT EXISTS CLExpectations (
  ...
  start_index INT4, -- Reserved for future use with expectation ranges
  end_index INT4, -- Reserved for future use with expectation ranges
  PRIMARY KEY (digest, crs_cl_id, grouping_id) -- start_index should be on primary key too eventually.
);
