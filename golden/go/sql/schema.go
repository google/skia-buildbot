...
-- selectively turn those on or off?
  INVERTED INDEX keys_idx (keys),
  INDEX ignored_idx (matches_any_ignore_rule)
-- Probably want an index on grouping_id and maybe matches_any_ignore_rule
);
...
CREATE TABLE IF NOT EXISTS SecondaryBranchExpectations (
  ...
  PRIMARY KEY (branch_name, digest, grouping_id)
);
