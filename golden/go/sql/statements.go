...
// ComputeTraceValueShard computes a shard in the range [0,numShards) based off the given trace
// hash bytes. This shard range was chosen to be a small number (relative to the thousands of ranges
// that the TraceValues database occupies) that is larger than the number of nodes in the
...
func ComputeTraceValueShard(traceHash []byte) []byte {
	return []byte{traceHash[0] % numShards}
}
