...
func SerializeMap(m map[string]string) (string, []byte) {
	...
	if len(m) == 0 {
		...
		var err error
		// TODO(kjlubick) use a dedicated implementation of map[string]string -> string to improve
		// performance and reduce need for panic/err. easyjson will probably work.
		str, err = json.Marshal(m)
		if err != nil {
			// Since we are marshaling a map of strings, this should never error in practice.
			panic(err)
		}
	}
