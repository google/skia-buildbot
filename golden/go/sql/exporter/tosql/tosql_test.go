...
type tableTwoRow struct {
	...
	indexForSearch struct{} `sql:"   INDEX comp_two_idx (comp_two DESC)   "`
	// We have this index for even better reasons.
	otherIndex struct{} `sql:"INDEX comp_two_idx (comp_two ASC)"`
}
...
CREATE TABLE IF NOT EXISTS TableTwo (
  ...
  INDEX comp_two_idx (comp_two DESC),
  INDEX comp_two_idx (comp_two ASC)
);
$$
`, "$$", "`")
