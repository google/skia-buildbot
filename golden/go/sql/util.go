...
func DeserializeMap(s string) (map[string]string, error) {
	m := map[string]string{}
...
func DigestToBytes(d types.Digest) (Digest, error) {
	if len(d) != 32 {
		return Digest{}, skerr.Fmt("Empty/missing/invalid digest passed in %q", d)
	}
	out := make(Digest, md5.Size)
...
func ConvertLabelToString(e ExpectationsLabel) expectations.Label {
	switch e {
	case LabelUntriaged:
	...
	sklog.Warningf("Unknown label: %d", e)
	return expectations.Untriaged
}
