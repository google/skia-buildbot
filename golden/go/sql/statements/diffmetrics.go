...
const diffMetricsClosestViewStatement = `
WITH
InitialRankings AS (
...
func CreateDiffMetricsClosestViewShard(shard byte) string {
	if shard == 255 {
		return fmt.Sprintf(diffMetricsClosestViewStatement, `WHERE left_digest > x'ff'`)
	}
	r := fmt.Sprintf(`WHERE left_digest > x'%02x' and left_digest < x'%02x'`, shard, shard+1)
	return fmt.Sprintf(diffMetricsClosestViewStatement, r)
}
...
func CreateDiffMetricsClosestView() string {
	return fmt.Sprintf(diffMetricsClosestViewStatement, "")
}
