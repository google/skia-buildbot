...
// MakeLocalCockroachDBWithSchema starts up an instance of cockroachdb and fills it out with the
// database schema. It returns the postgresql connection url and a function to run to delete all
// data from the db and stop the cockroach instance.
func MakeLocalCockroachDBWithSchema(t *testing.T) (string, func()) {
	...
	return "postgresql://root@localhost:26257/db_for_tests?sslmode=disable", func() {
		out, err := exec.Command("cockroach", "sql", "--insecure", "--host=localhost:26257",
			`--execute=DROP DATABASE db_for_tests CASCADE;`).CombinedOutput()
		assert.NoError(t, err, "dropping test database: %s", out)
