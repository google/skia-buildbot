...
DELETE FROM ChangelistValues RETURNING NOTHING;
DELETE FROM Commits RETURNING NOTHING;
DELETE FROM Traces RETURNING NOTHING;
DELETE FROM ChangelistTraces RETURNING NOTHING;
DELETE FROM PrimaryBranchParams RETURNING NOTHING;
DELETE FROM ChangelistParams RETURNING NOTHING;
DELETE FROM Options RETURNING NOTHING;
DELETE FROM Groupings RETURNING NOTHING;
DELETE FROM SourceFiles RETURNING NOTHING;
DELETE FROM Expectations RETURNING NOTHING;
DELETE FROM ChangelistExpectations RETURNING NOTHING;
DELETE FROM ExpectationDeltas RETURNING NOTHING;
DELETE FROM ExpectationRecords RETURNING NOTHING;
DELETE FROM DiffMetrics RETURNING NOTHING;
DELETE FROM IgnoreRules RETURNING NOTHING;
DELETE FROM Changelists RETURNING NOTHING;
DELETE FROM Patchsets RETURNING NOTHING;
DELETE FROM Tryjobs RETURNING NOTHING;
DELETE FROM ValuesAtHead RETURNING NOTHING;
`).CombinedOutput()
	require.NoError(t, err, "creating test database: %s", out)
}

// MustDigestToBytes returns the byte representation of a given digest or panics if it is invalid.
func MustDigestToBytes(d types.Digest) sql.Digest {
	b, err := sql.DigestToBytes(d)
	if err != nil {
		panic(skerr.Wrapf(err, "invalid digest %", d))
	}
	return b
}
