...
func RemoveOldDataAndResetSchema(t *testing.T) {
	// FIXME Dropping database each time doesn't work, as sometimes something locks up.
	//   If deleting all the rows from the table continues to work, delete the leftover.
	//fmt.Println("alpha")
	//out, err := exec.Command("cockroach", "sql", "--insecure", "--host=localhost:26257",
	//	`--execute=DROP DATABASE IF EXISTS db_for_tests CASCADE;`).CombinedOutput()
	//assert.NoError(t, err, "dropping test database: %s", out)
	//
	//fmt.Println("beta")
	//out, err = exec.Command("cockroach", "sql", "--insecure", "--host=localhost:26257",
	//	`--execute=CREATE DATABASE db_for_tests;`).CombinedOutput()
	//require.NoError(t, err, "creating test database: %s", out)
