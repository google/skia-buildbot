...
type CachedTileSourceConfig struct {
	...
	PubliclyViewableParamFile string
...
type CachedTileImpl struct {
	CachedTileSourceConfig

	optInParams paramtools.ParamSet
...
func New(c CachedTileSourceConfig) (*CachedTileImpl, error) {
	cti := &CachedTileImpl{
		CachedTileSourceConfig: c,
	}
	return cti, cti.loadParamFile(c.PubliclyViewableParamFile)
}
...
func (s *CachedTileImpl) loadParamFile(fName string) error {
	if fName == "" {
		sklog.Infof("No restrictions placed on params.")
		return nil
	}
	...
	if err := json5.NewDecoder(f).Decode(&s.optInParams); err != nil {
		return err
	}

	// Make sure the whitelist is not empty.
	empty := true
	for _, values := range s.optInParams {
...
// do not change.
func (s *CachedTileImpl) GetTile() (types.ComplexTile, error) {
	s.mutex.Lock()
	...
	retIgnoredTile, ignoreRules, err := ignore.FilterIgnored(rawTile, s.IgnoreStore)
	if err != nil {
		return nil, err
	}
...
func (s *CachedTileImpl) getCondensedTile(ctx context.Context, lastCpxTile types.ComplexTile) (*tiling.Tile, []*tiling.Commit, []int, error) {
	if s.NCommits <= 0 {
		ret := tiling.NewTile()
...
func (s *CachedTileImpl) filterTile(tile *tiling.Tile) *tiling.Tile {
	if s.optInParams == nil {
		return tile
	}
	...
	for traceID, trace := range tile.Traces {
		if tiling.Matches(trace, url.Values(s.optInParams)) {
...
func (s *CachedTileImpl) checkCommitableIssues(cpxTile types.ComplexTile) {
	go func() {
