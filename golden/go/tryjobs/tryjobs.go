...
type TryjobMonitor struct {
	...
	writeGerritMonitor *CondInt64Monitor
}
...
func NewTryjobMonitor(tryjobStore tryjobstore.TryjobStore, gerritAPI gerrit.GerritInterface, siteURL string, eventBus eventbus.EventBus) *TryjobMonitor {
	ret := &TryjobMonitor{
		...
		eventBus:           eventBus,
		writeGerritMonitor: NewCondInt64Monitor(1),
	}
...
type CondInt64Monitor struct {
	cond        sync.Cond
	countMap    map[int64]int
	nConcurrent int
}

func NewCondInt64Monitor(nConcurrent int) *CondInt64Monitor {
	return &CondInt64Monitor{
		countMap:    map[int64]int{},
...
type MonitorLock func()

func (m MonitorLock) Release() { m() }

func (m *CondInt64Monitor) Enter(id int64) MonitorLock {
