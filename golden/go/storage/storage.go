...
func (s *Storage) GetLastTileTrimmed() (*types.ComplexTile, error) {
	...
	if s.IsSparseTile {
		rawTile, commitsSum, err = s.getNewCondensedTile(s.lastCpxTile.TileWithIgnores)
		if err != nil {
...
func (s *Storage) getNewCondensedTile(lastTile *tiling.Tile) (*tiling.Tile, *types.CommitsSummary, error) {
	ctx := context.TODO()
	var err error
	...
	for idx := 0; idx < commitsLen; idx++ {
		for _, trace := range sparseTile.Traces {
			...
			if gTrace.Values[idx] != types.MISSING_DIGEST {
				targetCommits = append(targetCommits, allCommits[idx].Hash)
				cardinalities[idx]++
				break
			}
		}
