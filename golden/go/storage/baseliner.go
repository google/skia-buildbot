...
func NewBaseliner(gStorageClient *GStorageClient, expectationsStore expstorage.ExpectationsStore, issueExpStoreFactory expstorage.IssueExpStoreFactory, tryjobStore tryjobstore.TryjobStore) (*Baseliner, error) {
...
func (b *Baseliner) PushIssueBaseline(issueID int64, tile *tiling.Tile, tallies *tally.Tallies) error {
	...
	if err != nil {
		return skerr.Fmt("Error error calculating issue baseline: %s", err)
	}
...
func (b *Baseliner) getMasterExpectations(commitHash string) (*baseline.CommitableBaseLine, error) {
	...
	return b.gStorageClient.ReadBaseline(commitHash, 0)
}
