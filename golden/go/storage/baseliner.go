...
func (b *Baseliner) getMasterExpectations(commitHash string) (*baseline.CommitableBaseLine, error) {
	...
	sklog.Infof("Read expecations for %s from storage", commitHash)
	ret, err := b.gStorageClient.ReadBaseline(commitHash, 0)
	if err != nil {
		return nil, err
	}
	sklog.Infof("Done reading expecations for %s from storage", commitHash)

	// Look up the commit to see if it's valid.
	if ret == nil {
		sklog.Infof("\n\nCommit %s not found in GCS\n\n", commitHash)
