...
func newModularTryjobProcessor(ctx context.Context, _ vcsinfo.VCS, config ingestion.Config, client *http.Client) (ingestion.Processor, error) {
	...
	var reviewSystems []clstore.CRSPackage
	for _, crsName := range crsNames {
		...
		reviewSystems = append(reviewSystems, clstore.CRSPackage{
			ID:     crsName,
			Client: crsClient,
...
func (g *goldTryjobProcessor) getPatchSet(ctx context.Context, system clstore.CRSPackage, psOrder int, psID, clID string) (code_review.PatchSet, error) {
...
// getCodeReviewSystem returns the CRSPackage associated with the crs, or false if there was no
// match.
func (g *goldTryjobProcessor) getCodeReviewSystem(crs string) (clstore.CRSPackage, bool) {
