...
func (g *goldTryjobProcessor) Process(ctx context.Context, rf ingestion.ResultFileLocation) error {
	...
	ps, err := g.getPatchSet(ctx, psOrder, psID, clID, psID)
	if err != nil {
...
func (g *goldTryjobProcessor) getPatchSet(ctx context.Context, psOrder int, psID, clID, crs string) (code_review.PatchSet, error) {
	...
	if psID != "" {
		...
		if err == clstore.ErrNotFound {
			...
			if err != nil {
				return ps, skerr.Wrapf(err, "could not get patchsets for %s cl %s", crs, clID)
			}
			...
			for _, p := range xps {
				if p.SystemID == psID {
					if err := g.changeListStore.PutPatchSet(ctx, p); err != nil {
						return ps, skerr.Wrapf(err, "could not store PS %s of CL %q to clstore", psID, clID)
					}
					return p, nil
				}
			}
			...
			return ps, ingestion.IgnoreResultsFileErr

		} else if err != nil {
			return ps, skerr.Wrapf(err, "fetching PS from clstore with id %s for CL %q", psID, clID)
		}
	...
	if err == clstore.ErrNotFound {
		...
		for _, p := range xps {
			if p.Order == psOrder {
				if err := g.changeListStore.PutPatchSet(ctx, p); err != nil {
					return ps, skerr.Wrapf(err, "could not store PS %d of CL %q to clstore", psOrder, clID)
				}
				return p, nil
			}
		}
		...
		return ps, skerr.Wrapf(err, "fetching PS from clstore with order %d for CL %q", psOrder, clID)
	}
	// already found the PS in the store
	return ps, nil
}
