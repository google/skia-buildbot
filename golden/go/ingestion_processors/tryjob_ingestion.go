...
func newModularTryjobProcessor(ctx context.Context, _ vcsinfo.VCS, config *sharedconfig.IngesterConfig, client *http.Client) (ingestion.Processor, error) {
	...
	if strings.TrimSpace(hashesPath) == "" {
		return nil, skerr.Fmt("missing gcs known hashes path (e.g. 'flutter/hashes_files/gold-flutter-hashes.txt')")
	}
	gsClientOpt := storage.GCSClientOptions{
		HashesGSPath: hashesPath,
		Dryrun:       true,
	}
...
// false otherwise.
func (g *goldTryjobProcessor) hasUntriagedDigests(results []tjstore.TryJobResult, exp expectations.Classifier, rules paramtools.ParamMatcher, knownDigests types.DigestSet) bool {
