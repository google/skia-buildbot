...
func TryjobSQL(ctx context.Context, config ingestion.Config, client *http.Client, db *pgxpool.Pool, src ingestion.GCSSource) (ingestion.Processor, error) {
...
func (g *goldTryjobProcessor) HandlesFile(name string) bool {
	return strings.HasPrefix(name, g.source.Prefix)
}
...
func (g *goldTryjobProcessor) Process(ctx context.Context, fileName string) error {
	...
	if err != nil {
		return skerr.Wrapf(err, "could not process file %s/%s", g.source.Bucket, fileName)
	}
