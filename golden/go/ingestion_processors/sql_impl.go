...
func (s *sqlProcessor) storeKeysOptionsAndFlatten(ctx context.Context, gr *jsonio.GoldResults) ([]sqlTraceValue, error) {
	...
	if err := s.batchStore(ctx, insertKeys, keysToStore); err != nil {
		// There was an error, clear so we have to store everything again, just to be sure.
		s.keysOptionsCache.Purge()
		return nil, skerr.Wrap(err)
	}
	if err := s.batchStore(ctx, insertOptions, optionsToStore); err != nil {
		// There was an error, clear so we have to store everything again, just to be sure.
		s.keysOptionsCache.Purge()
		return nil, skerr.Wrap(err)
	}
...
const insertKeys = `INSERT INTO TraceIDs (trace_hash, keys) VALUES `
const insertOptions = `INSERT INTO OptionIDs (options_hash, options) VALUES `

const doNothingOnConflict = `ON CONFLICT DO NOTHING`

func (s *sqlProcessor) batchStore(ctx context.Context, query string, toStore []jsonAndHash) error {
	if len(toStore) == 0 {
		return nil
	}
	...
	if err != nil {
		return skerr.Wrapf(err, "storing %d JSON entries with query header: %s", len(toStore), query)
	}
	return nil
}
...
func (s *sqlProcessor) storeValues(ctx context.Context, values []sqlTraceValue, commitNum int, sourceFileHash []byte) error {
	...
	// When this was 10, the insert took ~33ms. At 100, it took ~100ms, indicating that the throughput
	// maximum is leveling off. 1000 was also tried, but this might have caused dramatically slower
	// ingestion times (possibly exacerbated by the fact there are many go routines per ingester).
	const chunkSize = 100
	err := util.ChunkIter(len(values), chunkSize, func(startIdx int, endIdx int) error {
		batch := values[startIdx:endIdx]

		upsertStatement := `UPSERT INTO TraceValues (trace_hash, commit_number, digest, options_hash, source_file_hash) VALUES`
		...
		for i, value := range batch {
			...
			arguments = append(arguments, value.digest)
			arguments = append(arguments, value.optionsHash)
			arguments = append(arguments, sourceFileHash)
		}
		_, err := s.db.ExecContext(ctx, upsertStatement, arguments...)
		return err
	})
	if err != nil {
