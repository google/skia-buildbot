...
func TestSQLProcess(t *testing.T) {
	...
	t.Run("DataFromFutureCommitExists_Success", func(t *testing.T) {
...
func subTest_AllNewData_Success(t *testing.T, p ingestion.Processor) {
	...
	assertTraceHasMostRecentCommitAndDigest(t, db, alphaTracePK, commitFKOfThisFile, three_devices.AlphaNegativeDigest)
	assertTraceHasMostRecentCommitAndDigest(t, db, betaTracePK, commitFKOfThisFile, three_devices.BetaPositiveDigest)
...
func subTest_DataFromPreviousCommitExists_Success(t *testing.T, p ingestion.Processor) {
	...
	alphaTracePK := storeTrace(t, db, alphaKeys)
	...
	initValueAtHead(t, db, alphaKeys, alphaGroupingPK, three_devices.AlphaNegativeDigest, previousCommitFK)
	...
	assertTraceHasMostRecentCommitAndDigest(t, db, alphaTracePK, commitFK, three_devices.AlphaNegativeDigest)
	assertTraceHasMostRecentCommitAndDigest(t, db, betaTracePK, commitFK, three_devices.BetaPositiveDigest)
...
func subTest_DataFromFutureCommitExists_Success(t *testing.T, p ingestion.Processor) {
	...
	initValueAtHead(t, db, alphaKeys, alphaGroupingPK, three_devices.AlphaNegativeDigest, futureCommitFK)
	storeExpectation(t, db, alphaGroupingPK, three_devices.AlphaNegativeDigest, expectations.Negative)
	...
	assertTraceHasMostRecentCommitAndDigest(t, db, alphaTracePK, futureCommitFK, three_devices.AlphaNegativeDigest)
	assertTraceHasMostRecentCommitAndDigest(t, db, betaTracePK, commitFK, three_devices.BetaPositiveDigest)
...
func subTest_OverwriteTraceValues_Success(t *testing.T, p ingestion.Processor) {
	...
	initValueAtHead(t, db, alphaKeys, alphaGroupingPK, three_devices.AlphaNegativeDigest, previousFileCommitFK)
...
func storeCommitData(t *testing.T, db *pgxpool.Pool, commits []tiling.Commit) {
	...
	statement += sql.ValuesPlaceholders(valuesPerRow, len(commits))
...
INSERT INTO Traces (trace_id, keys) VALUES ($1, $2)`, m)
}
...
func storeKeyValueMap(t *testing.T, db *pgxpool.Pool, statement string, m map[string]string, extra ...interface{}) []byte {
	mJSON, keysHashPK := sql.SerializeMap(m)
...
func initValueAtHead(t *testing.T, db *pgxpool.Pool, trace map[string]string, groupingFK []byte, digest types.Digest, commitFK int) {
	traceKeyJSON, traceID := sql.SerializeMap(trace)

	dBytes, err := sql.DigestToBytes(digest)
	require.NoError(t, err)

	_, err = db.Exec(context.Background(), `INSERT INTO ValuesAtHead
(trace_id, grouping_id, keys, digest, expectation_label, most_recent_commit_id) VALUES
($1, $2, $3, $4, $5, $6)
`, traceID, groupingFK, traceKeyJSON, dBytes, sql.LabelUntriaged, commitFK)
	require.NoError(t, err)
}
...
func assertKeyAndHashAreStored(t *testing.T, db *pgxpool.Pool, statement string, m map[string]string) []byte {
	mJSON, expectedHash := sql.SerializeMap(m)
...
func assertTraceHasMostRecentCommitAndDigest(t *testing.T, db *pgxpool.Pool, tracePK []byte, expectedCommitFK int, expectedDigest types.Digest) {
	row := db.QueryRow(context.Background(), `
SELECT most_recent_commit_id, encode(digest, 'hex') FROM ValuesAtHead where trace_id = $1`, tracePK)

	var actualCommitFK int
	var actualDigest types.Digest
	require.NoError(t, row.Scan(&actualCommitFK, &actualDigest), "Expected one row where trace_id = %v", tracePK)
	assert.Equal(t, expectedCommitFK, actualCommitFK, "incorrect commit id for trace %x", tracePK)
	assert.Equal(t, expectedDigest, actualDigest, "incorrect digest for trace %x", tracePK)
}
...
func storePrimaryBranchParams(t *testing.T, db *pgxpool.Pool, commitFK int, toStore []keyValue) {
	...
	statement += sql.ValuesPlaceholders(valuesPerRow, len(toStore))
