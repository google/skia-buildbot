...
func storeKeyValueMap(t *testing.T, db *pgxpool.Pool, m map[string]string) []byte {
	require.Fail(t, "Need to update for schema")
...
func storeTraceValue(t *testing.T, db *pgxpool.Pool, digest types.Digest, traceHashFK []byte, commitNumber int, optionsFK, groupingFK, sourceFileFK []byte) {
	...
	_, err = db.Exec(context.Background(), `
INSERT INTO TraceValues (trace_id, shard, commit_id, grouping_hash, digest, 
...
func assertDigestOnTraceAtCommit(t *testing.T, db *pgxpool.Pool, digest types.Digest, traceHashFK []byte, commitNumber int) {
	...
	require.NoError(t, err, "expected one row where trace_id = %v and commit_id = %d", traceHashFK, commitNumber)

	assert.Equal(t, digest, storedDigest, "Incorrect digest where trace_id = %v and commit_id = %d", traceHashFK, commitNumber)
}
...
func assertOptionsGroupingSourceFileOnTraceAtCommit(t *testing.T, db *pgxpool.Pool, optionsFK, groupingFK, sourceFileFK, traceFK []byte, commitNumber int) {
	row := db.QueryRow(context.Background(),
		`SELECT options_id, grouping_hash, source_file_id FROM TraceValues WHERE trace_id = $1 AND commit_id = $2;`,
	...
	require.NoError(t, err, "expected one row where trace_id = %v and commit_id = %d", traceFK, commitNumber)
