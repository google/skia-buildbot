...
func TestSQLProcess(t *testing.T) {
	...
	t.Run("SQLErrorResultsInRollback_Error", func(t *testing.T) {
		subTest_SQLErrorResultsInRollback_Error(t, ingester)
	})
}
...
func subTest_SQLErrorResultsInRollback_Error(t *testing.T, p ingestion.Processor) {
	fCtx := context.WithValue(context.Background(), intentionallyReturnError, intentionallyReturnError)
	...
	require.Error(t, p.Process(fCtx, jsonResult))
	assertTraceCountMetricIs(t, p, 0)
	assertKeyValueCacheIsEmpty(t, p)

	assertTableIsEmpty(t, db, "Expectations")
	assertTableIsEmpty(t, db, "Groupings")
	assertTableIsEmpty(t, db, "Options")
	assertTableIsEmpty(t, db, "PrimaryBranchParams")
	assertTableIsEmpty(t, db, "SourceFiles")
	assertTableIsEmpty(t, db, "Traces")
	assertTableIsEmpty(t, db, "TraceValues")
}
...
func assertKeyValueCacheIsEmpty(t *testing.T, p ingestion.Processor) {
	s := p.(*sqlProcessor)

	assert.Equal(t, 0, s.keyValueCache.Len())
}
...
func assertTableIsEmpty(t *testing.T, db *pgxpool.Pool, tableName string) {
	row := db.QueryRow(context.Background(), `SELECT COUNT(*) FROM `+tableName)
	count := -1
	require.NoError(t, row.Scan(&count), "counting rows in %s", tableName)
	assert.Equal(t, 0, count, "expected exactly zero rows in", tableName)
}
