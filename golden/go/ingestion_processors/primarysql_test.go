...
func TestPrimarySQL_Process_AllNewData_Success(t *testing.T) {
	...
	const squareTraceKeys = `todo square`
	const triangleTraceKeys = `todo triangle`
	const circleTraceKeys = `todo circle`
	const expectedCommitID = "0000000100"
	src := fakeGCSSourceFromFile(t, "primary1.json")
	s, err := PrimaryBranchSQL(ctx, src, map[string]string{sqlTileWidthConfig: "5"}, db)
...
func getAllTraces(ctx context.Context, t *testing.T, db *pgxpool.Pool) []schema.TraceRow {
	rows, err := db.Query(ctx, `SELECT * FROM Traces`)
	require.NoError(t, err)
	defer rows.Close()
