...
func (s *sqlProcessor) storeTraceMetadata(ctx context.Context, gr *jsonio.GoldResults, commitID int) ([]sqlTraceValue, map[string]bool, error) {
	...
	if err := batchCreateTraces(ctx, s.db, tracesToCreate, commitID); err != nil {
		return nil, nil, skerr.Wrap(err)
	}

	if err := batchUpdateTraces(ctx, s.db, tracesToUpdate, commitID); err != nil {
		return nil, nil, skerr.Wrap(err)
	}
...
func batchUpdateTraces(ctx context.Context, db sqlExecutor, toUpdate []tracePK, commitFK int) error {
	...
	// This value seems to be a good balance of throughput
	const chunkSize = 100
