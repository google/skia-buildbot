...
func (s *sqlProcessor) storeValues(ctx context.Context, values []sqlTraceValue, commitNum int, sourceFileHash []byte) error {
	...
	// This sorting will hopefully make it so that we are are focusing on a few shards at a time
	// as we send them to the SQL database.
	sort.Slice(values, func(i, j int) bool {
		// Sorting on the first byte approximates sorting them on the shard value.
