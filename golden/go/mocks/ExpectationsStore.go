...
func (_m *ExpectationsStore) Get() (types.ExpectationsHandler, error) {
	ret := _m.Called()

	var r0 types.ExpectationsHandler
	if rf, ok := ret.Get(0).(func() types.ExpectationsHandler); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(types.ExpectationsHandler)
		}
	}
...
// removeChange provides a mock function with given fields: changes
func (_m *ExpectationsStore) removeChange(changes types.TestExp) error {
	ret := _m.Called(changes)

	var r0 error
	if rf, ok := ret.Get(0).(func(types.TestExp) error); ok {
		r0 = rf(changes)
	} else {
		r0 = ret.Error(0)
	}
