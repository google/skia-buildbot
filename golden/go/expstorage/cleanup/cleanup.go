...
func cleanup(ctx context.Context, cleaner expstorage.Cleaner, posMax time.Duration, negMax time.Duration, now time.Time) error {
	if posMax > 0 {
		if n, err := cleaner.DeleteEntriesUnusedAndModifiedBefore(ctx, expectations.Positive, now.Add(-posMax)); err != nil {
			return skerr.Wrapf(err, "deleting positive expectation entries before %s", now.Add(-posMax))
		} else {
	...
	if negMax > 0 {
		if n, err := cleaner.DeleteEntriesUnusedAndModifiedBefore(ctx, expectations.Negative, now.Add(-negMax)); err != nil {
			return skerr.Wrapf(err, "deleting negative expectation entries before %s", now.Add(-negMax))
		} else {
			sklog.Infof("%d negative expectations have aged out", n)
		}
	}
	// A time very far into the future.
	cleanAll := time.Date(3000, time.January, 0, 0, 0, 0, 0, time.UTC)
	// Clean out all untriaged expectations - they don't really need to be in the DB.
	if n, err := cleaner.DeleteEntriesUnusedAndModifiedBefore(ctx, expectations.Untriaged, cleanAll); err != nil {
		return skerr.Wrapf(err, "deleting untriaged expectation entries before %s", now)
	} else {
