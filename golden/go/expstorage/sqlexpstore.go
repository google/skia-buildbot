...
func (c *CachingExpectationStore) AddChange(changedTests map[string]types.TestClassification, userId string) error {
	...
	c.eventBus.Publish(EV_EXPSTORAGE_CHANGED, changedTests)
	return nil
}
...
func (c *CachingExpectationStore) addChangeToCache(evtChangedTests interface{}) {
	...
	for test, digests := range changedTests {
		...
		if len(changedTests[test]) == 0 {
			delete(changedTests, test)
		}
	}
...
func (c *CachingExpectationStore) removeChange(changedDigests map[string]types.TestClassification) error {
	...
	// Fire an event that will trigger the addition to the cache.
	c.eventBus.Publish(EV_EXPSTORAGE_CHANGED, changedDigests)
