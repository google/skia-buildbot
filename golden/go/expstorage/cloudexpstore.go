...
func (c *cloudExpStore) UndoChange(changeID int64, userID string) (map[string]types.TestClassification, error) {
	...
	egroup.Go(func() error {
		...
		if _, expSlice, err = c.getTestDigestExps(nil, expChangeKey); err != nil {
	...
	details := make([]TDESlice, len(allKeys))
...
func (c *cloudExpStore) makeChange(changes map[string]types.TestClassification, userId string, undoChangeID int64) (err error) {
	...
	tcKeys, testChanges := buildTDESlice(changes, c.testDigestExpEntity, changeKey)
...
func (c *cloudExpStore) getExpChangeKeys(atBeforeID int64) ([]*datastore.Key, error) {
	...
	var recentKeys []*datastore.Key
	egroup.Go(func() error {
		var err error
		recentKeys, err = c.recentKeysList.GetRecent()
		return err
	})
	...
	ret := dsutil.Combine(recentKeys, queryKeys)
...
func (c *cloudExpStore) getTestDigestExps(tx *datastore.Transaction, parentKey *datastore.Key) ([]*datastore.Key, TDESlice, error) {
