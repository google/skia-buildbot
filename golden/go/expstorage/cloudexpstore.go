...
func (c *CloudExpStore) makeChange(changes types.TestExp, userId string, timeStampMs int64, undoChangeID int64, transactional bool) (changeKey *datastore.Key, err error) {
	...
	blobKeyx, err := c.blobStore.Save(changes)
	if err != nil {
		return nil, sklog.FmtErrorf("Saving changes to blob failed: %s", err)
	}

	checkCopy := types.TestExp{}
	if err := c.blobStore.Load(blobKeyx, &checkCopy); err != nil {
		return nil, err
	}
	fmt.Printf("Found change (%s): %s\n", blobKeyx, spew.Sdump(checkCopy))
	...
	actions := dsutil.TxActions{}
	actions.AddRollbackFn(func() error { return c.blobStore.Delete(blobKeyx) })
	...
	foundExp, _, err := c.loadCurrentExpectations(nil)
	if err != nil {
		return nil, err
	}
