...
func NewCloudExpectationsStore(client *datastore.Client, eventBus eventbus.EventBus) (ExpectationsStore, IssueExpStoreFactory, error) {
	...
	store := &cloudExpStore{
		...
		eventExpStorageChanged: EV_EXPSTORAGE_CHANGED,
		client:                 client,
		eventBus:               eventBus,
	}
	...
	factory := func(issueID int64) ExpectationsStore {
		return &cloudExpStore{
			...
			eventExpStorageChanged: EV_TRYJOB_EXP_CHANGED,
			client:                 client,
			eventBus:               eventBus,
		}
	}
...
func (c *cloudExpStore) makeChange(changes map[string]types.TestClassification, userId string, undoChangeID int64) (err error) {
	...
	if !testChanges.empty() || !expsFound.empty() {
	...
	if c.eventBus != nil {
		c.eventBus.Publish(c.eventExpStorageChanged, changes, true)
	}
	return nil
}
...
func (c *cloudExpStore) UndoChange(changeID int64, userID string) (map[string]types.TestClassification, error) {
	...
	// Get the expectations change keys before the current one.

	// Load all expectations change for each

	// Update the keys are are interested in
	fmt.Printf("CHANGE ID: %d\n%s\n", changeID, spew.Sdump(expChange))
...
func (c *cloudExpStore) removeChange(changes map[string]types.TestClassification) error {
	...
	if c.eventBus != nil {
		c.eventBus.Publish(c.eventExpStorageChanged, changes, false)
	}
	return nil
}
