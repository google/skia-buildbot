...
func (c *CloudExpStore) Get() (*Expectations, error) {
	...
	_, err := c.loadJsonBlob(nil, c.expectationsKey, &ret)
	return &Expectations{Tests: ret}, err
}
...
func (c *CloudExpStore) PutExpectations(exps map[string]types.TestClassification) error {
	return c.updateCurrentExpectations(nil, exps, true)
}
...
func (c *CloudExpStore) removeChange(changes map[string]types.TestClassification) error {
	...
	updateFn := func(tx *datastore.Transaction) error {
		return c.updateCurrentExpectations(tx, changes, false)
	}
...
func (c *CloudExpStore) makeChange(changes map[string]types.TestClassification, userId string, timeStampMs int64, undoChangeID int64, transactional bool) (err error) {
	...
	blobKeys, err := c.writeJsonBlobParts(c.testDigestExpKind, changes)
	if err != nil {
		return err
	}
	purgeKeys := append([]*datastore.Key(nil), blobKeys...)
	...
	defer func() {
		if err != nil {
			go func() {
				util.LogErr(c.client.DeleteMulti(context.TODO(), purgeKeys))
			}()
		}
	}()
	...
	updateFn := func(tx *datastore.Transaction) error {
		...
		// // Update the overall expectations unless it is disabled.
		if err := c.updateCurrentExpectations(tx, changes, false); err != nil {
			return err
		}
	...
	// 	// // Update the overall expectations unless it is disabled.
	// 	if err := c.updateCurrentExpectations(tx, changes); err != nil {
	// 		return err
	// 	}
...
func (c *CloudExpStore) updateCurrentExpectations(tx *datastore.Transaction, changes map[string]types.TestClassification, overwrite bool) error {
	currentExp := map[string]types.TestClassification{}
	parent, err := c.loadJsonBlob(tx, c.expectationsKey, currentExp)
	if err != nil {
		return err
	}
	...
	return c.updateJsonBlob(tx, c.expectationsKey, parent, c.testDigestExpKind, currentExp)
}
...
func (c *CloudExpStore) loadJsonBlob(tx *datastore.Transaction, parentKey *datastore.Key, dst interface{}) (*BlobParent, error) {
	parent := &BlobParent{}
	if err := c.getTx(tx, parentKey, parent); err != nil {
		return nil, err
	}
	...
	for idx, key := range parent.Children {
		...
		if err := c.getTx(tx, key, blobParts); err != nil {
			return nil, err
		}
	}
...
func (c *CloudExpStore) updateJsonBlob(tx *datastore.Transaction, parentKey *datastore.Key, parent *BlobParent, kind ds.Kind, value interface{}) error {
	if parent != nil {
		if err := c.deleteBlob(tx, parentKey, parent); err != nil {
			return err
		}
	}

	var err error
	parent.Children, err = c.writeJsonBlobParts(kind, value)
	if err != nil {
		return err
	}
...
func (c *CloudExpStore) writeJsonBlobParts(kind ds.Kind, value interface{}) ([]*datastore.Key, error) {
	...
	for idx, part := range blobParts {
		func(idx int, part *BlobPart) {
			egroup.Go(func() error {
				var err error
				keys[idx], err = c.client.Put(ctx, ds.NewKey(kind), part)
				return err
			})
		}(idx, part)
	}
...
func (c *CloudExpStore) deleteBlob(tx *datastore.Transaction, parentKey *datastore.Key, parent *BlobParent) error {
	if len(parent.Children) == 0 {
		return nil
	}

	if err := c.deleteMultiTx(tx, parent.Children); err != nil {
		return err
	}
...
// func (c *CloudExpStore) loadExpectations(tx *datastore.Transaction, parentKey *datastore.Key) (*BlobParent, map[string]types.TestClassification, error) {

// 	var err error
// 	if tx != nil {
// 		err = tx.Get(parentKey, parent)
// 	} else {
// 		err = c.client.Get(context.TODO(), parentKey, parent)
// 	}
...
func (c *CloudExpStore) getTx(tx *datastore.Transaction, key *datastore.Key, dst interface{}) error {
	if tx != nil {
		return tx.Get(key, dst)
	}
	return c.client.Get(context.TODO(), key, dst)
}

func (c *CloudExpStore) deleteMultiTx(tx *datastore.Transaction, keys []*datastore.Key) error {
	if tx != nil {
		return tx.DeleteMulti(keys)
	}
	return c.client.DeleteMulti(context.TODO(), keys)
}

func (c *CloudExpStore) getPutFn(tx *datastore.Transaction) func(*datastore.Key, interface{}) error {
	if tx != nil {
	...
	return func(k *datastore.Key, val interface{}) error {
		_, err := c.client.Put(context.TODO(), k, val)
		return err
	}
}

// func (c *CloudExpStore) getMultiTx(tx *datastore.Transaction, keys []*datastore.Key, dst interface{}) error {
// 	if tx != nil {
// 		return tx.GetMulti(keys, dst)
// 	}
// 	return c.client.GetMulti(context.TODO(), keys, dst)
// }

// func (b BlobParts) getKeys(kind ds.Kind, parentKey *datastore.Key) []*datastore.Key {
// 	ret := make([]*datastore.Key, len(b))
// 	for idx := range b {
// 		ret[idx] = ds.NewKey(kind)
// 	}
// 	return ret
// }
