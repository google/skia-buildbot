...
func (c *CloudExpStore) AddChange(changes map[string]types.TestClassification, userID string) error {
	return c.makeChange(changes, userID, 0, 0)
}
...
func (c *CloudExpStore) UndoChange(changeID int64, userID string) (map[string]types.TestClassification, error) {
	...
	return changes, c.makeChange(changes, userID, 0, changeID)
}
...
func (c *CloudExpStore) CalcExpectations() error {
	// Get the keys of all expectation changes.
	keys, err := c.getExpChangeKeys(0)
	if err != nil {
		return err
	}
	// sklog.Infof("CalcExp: keys loaded")
	// q := ds.NewQuery(c.changeEntity).Filter("OK =", true).Order("TimeStamp").KeysOnly()
	// iterCh, err := ds.IterKeys(c.client, q)
	...
	for p := nPages - 1; p >= 0; p-- {
		start := p * pageSize
		end := util.MinInt(start+pageSize, len(keys))
		pageKeys := keys[start:end]
		tdeBuf := make([]TDESlice, len(pageKeys))
		...
		for idx, key := range pageKeys {
			func(idx int, key *datastore.Key) {
				egroup.Go(func() error {
					var err error
					_, tdeBuf[idx], err = c.getTestDigestExps(nil, key, false)
					return err
				})
			}(idx, key)
		}
		if err := egroup.Wait(); err != nil {
			return sklog.FmtErrorf("Error retrieving change expectations: %s", err)
		}

		for i := len(keys) - 1; i >= 0; i-- {
			exp.AddDigests(tdeBuf[i].toExpectations(false).Tests)
		}
		sklog.Infof("CalcExp: %d exps added", len(keys))
	}
	sklog.Infof("All expectations changes processed.")

	// for i := len(keys) - 1; i >= 0; i-- {
	// 	key := keys[i]
	// 	_, changeSlice, err := c.getTestDigestExps(nil, key, false)
	// 	if err != nil {
	// 		return sklog.FmtErrorf("Error retrieving change expectations: %s", err)
	// 	}
	// 	exp.AddDigests(changeSlice.toExpectations(false).Tests)
	// 	sklog.Infof("CalcExp: exps added")
	// }
	return c.putCurrentExpectations(nil, buildTDESlice(exp.Tests))
}
...
func (c *CloudExpStore) makeChange(changes map[string]types.TestClassification, userId string, timeStampMs int64, undoChangeID int64) (err error) {
...
func (c *CloudExpStore) putCurrentExpectations(tx *datastore.Transaction, currentExp TDESlice) error {
