...
func (c *CloudExpStore) Get() (types.ExpectationsHandler, error) {
...
func (c *CloudExpStore) UndoChange(changeID int64, userID string) (types.TestExp, error) {
	...
	exps, err := c.CalcExpectations(prevChangeKeys)
	if err != nil {
...
func (c *CloudExpStore) CalcExpectations(keys []*datastore.Key) (types.ExpectationsHandler, error) {
	...
	ret := types.NewExpectationsHandler(nil)
...
func (c *CloudExpStore) updateCurrentExpectations(tx *datastore.Transaction, changes types.TestExp, overwrite bool, actions *dsutil.TxActions) (err error) {
	...
	if overwrite || (currentExp == nil) {
		currentExp = types.NewExpectationsHandler(changes.DeepCopy())
	} else {
...
func (c *CloudExpStore) loadCurrentExpectations(tx *datastore.Transaction) (types.ExpectationsHandler, *expectationsState, error) {
	...
	return types.NewExpectationsHandler(testExp), expState, err
}
