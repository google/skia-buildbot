...
func (c *CloudExpStore) CalcExpectations() (*Expectations, error) {
	t := timer.New("Loading exp keys")
	sklog.Infof("Starting to load keys")
	...
	for idx, key := range keys {
		...
		func(idx int, key *datastore.Key) {
			egroup.Go(func() error {
				...
				exps, err := c.getExpectations(key)
				changes[idx] = exps
				curr := atomic.AddInt32(&total, 1)
				if curr%1000 == 0 {
					sklog.Infof("%d / %d loaded", curr, len(keys))
				}
				return err
			})
		}(idx, key)
...
func (c *CloudExpStore) getExpChangeKeys(beforeID int64) ([]*datastore.Key, error) {
	...
	ctx := context.TODO()
	egroup.Go(func() error {
		defer timer.New("Loading keys timer xxx").Stop()
...
func (c *CloudExpStore) getExpectations(expChangeKey *datastore.Key) (map[string]types.TestClassification, error) {
	ctx := context.TODO()
	expChange := &ExpChange{}
