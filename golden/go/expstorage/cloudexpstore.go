...
//
// We use instances of TDESlice to record expectations and expectation changes.
...
type cloudExpStore struct {
	...
	// for the master of a Gerrit issue
...
// an ID that created via TimeSortableKey and we only want to retrieve keys that are
...
// getTestDigestExpectations gets all expectations for the given change. If
...
func (c *cloudExpStore) deleteExpChange(key *datastore.Key) {
	...
	children, _, err := c.getTestDigestExps(nil, key, true)
	if err != nil {
		sklog.Errorf("Error deleting expectation change %d: %s", key.ID, err)
	} else if len(children) > 0 {
		egroup.Go(func() error { return c.client.DeleteMulti(ctx, children) })
	}

	// Delete the change itself.
	egroup.Go(func() error { return c.client.Delete(ctx, key) })
