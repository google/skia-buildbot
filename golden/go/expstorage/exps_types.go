...
// buildTDESlice converts the given expectation(change)s to a TDESlice instance
// for storage in the cloud datastore.
func buildTDEObjs(expChange map[string]types.TestClassification) (TDEObjs, error) {
	var buf bytes.Buffer
	if err := json.NewEncoder(&buf).Encode(expChange); err != nil {
		return nil, err
	}
	body := buf.Bytes()
...
func (t TDEObjs) getKeys(kind ds.Kind, parentKey *datastore.Key) []*datastore.Key {
	ret := make([]*datastore.Key, len(t))
	for idx := range t {
