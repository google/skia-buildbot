...
func (f *Store) DeleteEntriesUnusedAndModifiedBefore(ctx context.Context, label expectations.Label, ts time.Time) (int, error) {
	...
	err := f.client.IterDocs(ctx, "delete_unused_expectations", label.String(), q, 3, 10*time.Minute, func(doc *firestore.DocumentSnapshot) error {
		...
		// We can't have multiple inequality filters for multiple properties on a single query.
		...
		toDelete = append(toDelete, expstorage.Delta{
			...
			Digest:   er.Digest,
			// Setting things to Untriaged will cause the entries to be deleted.
	...
	if label == expectations.Untriaged {
		...
		if err != nil {
			return 0, skerr.Wrapf(err, "removing untriaged expectations from firestore")
		}
	} else {
		// This has the nice benefit of including entries in the triage log, so they could be undone
		// if needed.
