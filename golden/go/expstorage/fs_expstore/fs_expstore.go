...
type Store struct {
	...
	mode   AccessMode
	clID   string // Gerrit or GitHub ID. Empty string means master branch.
...
type expectationEntry struct {
	...
	ChangeListID string         `firestore:"cl_id"`
}
...
func (f *Store) initQuerySnapshot(ctx context.Context) error {
	q := f.client.Collection(expectationsCollection).Where(changeListIDField, "==", masterBranch)
...
func (f *Store) getExpectationsForCL() (types.Expectations, error) {
	expDelta, err := f.loadExpectationsSharded(f.clID, clShards)
	if err != nil {
		return nil, skerr.Wrapf(err, "loading expectations delta for CL %s from firestore", f.clID)
	}
	return expDelta, nil
}

// loadExpectationsSharded returns an Expectations object from the expectationsCollection,
// with all Expectations belonging to the passed in clID (can be masterBranch).
func (f *Store) loadExpectationsSharded(clID string, shards int) (types.Expectations, error) {
	defer metrics2.FuncTimer().Stop()
	q := f.client.Collection(expectationsCollection).Where(changeListIDField, "==", clID)
...
func (f *Store) QueryLog(ctx context.Context, offset, size int, details bool) ([]expstorage.TriageLogEntry, int, error) {
	...
	q = q.Where(changeListIDField, "==", f.clID).Where(committedField, "==", true)
