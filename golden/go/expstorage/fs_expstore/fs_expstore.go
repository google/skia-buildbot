...
// that is any issue-specific Expectations applied to the master Expectations.
...
func (f *Store) getIssueExpectations() (types.Expectations, error) {
	...
	if err != nil {
		return nil, skerr.Fmt("could not load expectations for issue %d from firestore: %s", f.issue, err)
	}
