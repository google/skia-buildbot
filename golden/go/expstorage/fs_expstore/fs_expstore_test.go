...
// TestEventBusAddMaster makes sure proper event bus signals are sent
...
// TestEventBusAddIssue makes sure proper event bus signals are sent
...
// TestEventBusUndo tests that event bus signals are properly sent during Undo.
...
func TestIssueExpectationsQueryLog(t *testing.T) {
	...
	// request up to 10 to make sure we would get the master
