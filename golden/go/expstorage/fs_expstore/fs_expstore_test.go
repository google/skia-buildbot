...
func TestSetUsed_OneEntry_Success(t *testing.T) {
	...
	err = mb.SetUsed(ctx, []expstorage.Delta{
		{
			Grouping: data.AlphaTest,
			Digest:   data.AlphaGood1Digest,
		},
	...
	assert.True(t, entryTwo.LastUsed.Equal(neg.LastUsed))
}
...
// TestUntriageOldEntries_EntriesNotDeleted checks that we don't untriage entries that are not old
// enough and that don't match our query.
func TestUntriageOldEntries_EntriesNotDeleted(t *testing.T) {
	unittest.LargeTest(t)
	...
	test := func(name string, label expectations.Label, ts time.Time) {
		...
		n, err := mb.UntriageOldEntries(ctx, label, ts)
		require.NoError(t, err)
		assert.Equal(t, 0, n)

		// Make sure both are actually there
...
// TestUntriageOldEntries_SingleEntryAffected tests where a single entry (the oldest) is marked
// as untriaged.
func TestUntriageOldEntries_SingleEntryAffected(t *testing.T) {
	unittest.LargeTest(t)
	...
	n, err := mb.UntriageOldEntries(ctx, expectations.Positive, entryThree.LastUsed.Add(-time.Minute))
	require.NoError(t, err)
	assert.Equal(t, 1, n)
...
// TestUntriageOldEntries_MiddleEntryAffected tests where the middle entry (the only negative)
// entry is marked as untriaged.
func TestUntriageOldEntries_MiddleEntryAffected(t *testing.T) {
	unittest.LargeTest(t)
	...
	n, err := mb.UntriageOldEntries(ctx, expectations.Negative, entryThree.LastUsed.Add(time.Minute))
	require.NoError(t, err)
	assert.Equal(t, 1, n)
...
// TestUntriageOldEntries_MultipleEntriesAffected tests where we mark both positive entries as
// untriaged (not matching the negative one in the middle).
func TestUntriageOldEntries_MultipleEntriesAffected(t *testing.T) {
	unittest.LargeTest(t)
...
// TestUntriageOldEntries_ModifiedRecently tests where we don't untriage digests that have
// not been seen in a while, but were modified recently.
func TestUntriageOldEntries_ModifiedRecently(t *testing.T) {
	unittest.LargeTest(t)
...
// TestUntriageOldEntries_MultipleEntriesDeleted tests case where we untriage two entries and then
// prune those untriaged entries so they are not in Firestore anymore.
func TestPruneUntriagedEntries_MultipleEntriesPruned(t *testing.T) {
	unittest.LargeTest(t)
	...
	require.NoError(t, err)
	assert.Equal(t, 2, n)
	n, err = mb.PruneUntriagedEntries(ctx)
	require.NoError(t, err)
	assert.Equal(t, 2, n)
