...
func TestAddChange_MasterBranch_NotifierEventsCorrect(t *testing.T) {
	...
	notifier := expstorage.NewEventHandler(true)
	...
	assert.Eventually(t, func() bool {
		...
		return calledMatchesInAnyOrder(expected, calledWith)
	}, 5*time.Second, 100*time.Millisecond)
}

// Due to there being multiple shards that are listening for Firestore changes, we can't guarantee
// the order of the deltas in events, so we sort them and compare them to make sure our tests are
// not flaky.
func calledMatchesInAnyOrder(expected, actual []expstorage.Delta) bool {
	sortChanges := func(a []expstorage.Delta) {
		sort.Slice(a, func(i, j int) bool {
			leftDelta, rightDelta := a[i], a[j]
			return leftDelta.Grouping < rightDelta.Grouping || (leftDelta.Grouping == rightDelta.Grouping &&
				leftDelta.Digest < rightDelta.Digest)
		})
	}
	sortChanges(expected)
	sortChanges(actual)
	return deepequal.DeepEqual(expected, actual)
}
