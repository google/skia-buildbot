...
expstore_expectations_v2   | label: ASC updated: ASC  # this will go away, see UntriageOldEntries
...
That same process then calls UntriageOldEntries, which sets the `label` to untriaged for all
...
default to Untriaged. As such, PruneUntriagedEntries deletes all `expectationEntry` documents
