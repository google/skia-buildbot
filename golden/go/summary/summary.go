...
type Summary struct {
	...
	UntHashes []types.Digest         `json:"untHashes"`
	Num       int                    `json:"num"`
...
func (s *Summary) clone() *Summary {
	ret := &Summary{}
	*ret = *s
	ret.UntHashes = append([]types.Digest(nil), s.UntHashes...)
...
func contains(tests []types.TestName, t types.TestName) bool {
	for _, x := range tests {
		if x == t {
			return true
		}
...
func (s *Summaries) CalcSummaries(tile *tiling.Tile, testNames []types.TestName, query url.Values, head bool) (map[types.TestName]*Summary, error) {
	...
	for id, tr := range tile.Traces {
		...
		if len(testNames) > 0 && contains(testNames, name) {
			continue
		}
	...
	for name, traces := range filtered {
		digestMap := map[types.Digest]bool{}
		corpus := ""
		...
		digests := make([]types.Digest, 0, len(digestMap))
		for d := range digestMap {
			digests = append(digests, d)
		}
		ret[name] = s.makeSummary(name, e, s.storages.DiffStore, corpus, digests)
	}
	t.Stop()
...
func (s *Summaries) makeSummary(name types.TestName, e types.TestExpBuilder, diffStore diff.DiffStore, corpus string, digests []types.Digest) *Summary {
	pos := 0
	neg := 0
	unt := 0
...
func diameter(digests []types.Digest, diffStore diff.DiffStore) int {
	// TODO Parallelize.
	lock := sync.Mutex{}
	max := 0
	wg := sync.WaitGroup{}
	for {
		...
		wg.Add(1)
		go func(d1 types.Digest, d2 []types.Digest) {
			defer wg.Done()
