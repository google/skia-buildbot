...
// Calculate calculates a slice of DigestsForTest for the given data. It will have its entries
// sorted by TestName first, then sorted by Corpus in the event of a tie.
func Calculate(smc Utils, tile *tiling.Tile, testNames types.TestNameSet, query url.Values, head bool) ([]*DigestsForTest, error) {
	return smc.calcSummaries(tile, testNames, query, head)
}

// OverwriteWithNew creates a new []*DigestsForTest from this and the passed
...
func (s *Utils) calcSummaries(tile *tiling.Tile, testNames types.TestNameSet, query url.Values, head bool) ([]*DigestsForTest, error) {
...
func (s *Utils) makeSummary(name types.TestName, exp expectations.ReadOnly, corpus string, digests types.DigestSlice) *DigestsForTest {
	pos := 0
	neg := 0
	unt := 0
