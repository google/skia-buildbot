...
// StartCacheWarming starts warming the caches for data we want to serve quickly.
...
// startCLCacheProcess starts a go routine to warm the CL Summary cache. This way, most summaries are
...
func (wh *Handlers) startStatusCacheProcess(ctx context.Context, windowSize int) {
	go util.RepeatCtx(ctx, time.Minute, func(ctx context.Context) {
		...
		row := wh.DB.QueryRow(ctx, `SELECT git_hash, commit_id, commit_time, author_email, subject
...
ORDER BY CommitsWithData.commit_id DESC LIMIT 1`)
		...
		if err := row.Scan(&gs.LastCommit.Hash, &gs.LastCommit.ID, &ts,
			&gs.LastCommit.CommitTime, &gs.LastCommit.Author, &gs.LastCommit.Subject); err != nil {
...
SELECT corpus, COUNT(digest) FROM DistinctNotIgnoredDigests
