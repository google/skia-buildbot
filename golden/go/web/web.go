...
func (wh *Handlers) ByBlameHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := wh.limitForAnonUsers(r); err != nil {
		httputils.ReportError(w, err, "Try again later")
		return
	}
	...
	if v := r.FormValue("query"); v != "" {
		...
		if qp, err = url.ParseQuery(v); err != nil {
			httputils.ReportError(w, err, "invalid input")
			return
		} else if qp.Get(types.CORPUS_FIELD) == "" {
			// If no corpus specified report an error.
			httputils.ReportError(w, skerr.Fmt("did not receive value for corpus"), "invalid input")
			return
		}
	}
	...
	if err != nil {
		httputils.ReportError(w, skerr.Wrapf(err, "computing blame %v", qp), "")
		return
	}
...
func (wh *Handlers) ChangeListsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Invalid pagination params.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Retrieving changelists results failed.")
		return
	}
...
func (wh *Handlers) ChangeListSummaryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		httputils.ReportError(w, nil, "Must specify 'id' of ChangeList.")
		return
	}

	rv, err := wh.getCLSummary(r.Context(), clID)
	if err != nil {
		httputils.ReportError(w, err, "could not retrieve data for the specified CL.")
		return
	}
...
func (wh *Handlers) SearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Search for digests failed.")
		return
	}
...
func (wh *Handlers) ExportHandler(w http.ResponseWriter, r *http.Request) {
	...
	if query.ChangeListID != "" || query.BlameGroupID != "" {
		...
		httputils.ReportError(w, errors.New(msg), msg)
		return
	}
	...
	if err := export.WriteTestRecords(ret, w); err != nil {
		httputils.ReportError(w, err, "Unable to serialized knowledge base.")
	}
}
...
func (wh *Handlers) DetailsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseForm(); err != nil {
		httputils.ReportError(w, err, "Failed to parse form values")
		return
	}
	test := r.Form.Get("test")
	digest := r.Form.Get("digest")
	if test == "" || !validation.IsValidDigest(digest) {
		httputils.ReportError(w, fmt.Errorf("Some query parameters are wrong or missing: %q %q", test, digest), "Missing query parameters.")
		return
	}

	ret, err := wh.SearchAPI.GetDigestDetails(types.TestName(test), types.Digest(digest))
	if err != nil {
		httputils.ReportError(w, err, "Unable to get digest details.")
		return
	}
...
func (wh *Handlers) DiffHandler(w http.ResponseWriter, r *http.Request) {
	...
	if test == "" || !validation.IsValidDigest(left) || !validation.IsValidDigest(right) {
		httputils.ReportError(w, fmt.Errorf("Some query parameters are missing or wrong: %q %q %q", test, left, right), "Missing query parameters.")
		return
	}

	ret, err := wh.SearchAPI.DiffDigests(types.TestName(test), types.Digest(left), types.Digest(right))
	if err != nil {
		httputils.ReportError(w, err, "Unable to compare digests")
		return
	}
...
func (wh *Handlers) IgnoresHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to retrieve ignore rules, there may be none.")
		return
	}
...
func (wh *Handlers) IgnoresUpdateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if user == "" {
		httputils.ReportError(w, fmt.Errorf("Not logged in."), "You must be logged in to update an ignore rule.")
		return
	}
	id, err := strconv.ParseInt(mux.Vars(r)["id"], 10, 0)
	if err != nil {
		httputils.ReportError(w, err, "ID must be valid integer.")
		return
	}
	req := &IgnoresRequest{}
	if err := parseJSON(r, req); err != nil {
		httputils.ReportError(w, err, "Failed to parse submitted data.")
		return
	}
	if req.Filter == "" {
		httputils.ReportError(w, fmt.Errorf("Invalid Filter: %q", req.Filter), "Filters can't be empty.")
		return
	}
	d, err := human.ParseDuration(req.Duration)
	if err != nil {
		httputils.ReportError(w, err, "Failed to parse duration")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Unable to update ignore rule.")
		return
	}
...
func (wh *Handlers) IgnoresDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	user := login.LoggedInAs(r)
	if user == "" {
		httputils.ReportError(w, fmt.Errorf("Not logged in."), "You must be logged in to add an ignore rule.")
		return
	}
	id, err := strconv.ParseInt(mux.Vars(r)["id"], 10, 0)
	if err != nil {
	...
	if numDeleted, err := wh.IgnoreStore.Delete(id); err != nil {
		httputils.ReportError(w, err, "Unable to delete ignore rule.")
...
func (wh *Handlers) IgnoresAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = wh.IgnoreStore.Create(ignoreRule); err != nil {
		httputils.ReportError(w, err, "Failed to create ignore rule.")
		return
	}
...
func (wh *Handlers) TriageHandler(w http.ResponseWriter, r *http.Request) {
	...
	user := login.LoggedInAs(r)
	if user == "" {
		httputils.ReportError(w, fmt.Errorf("Not logged in."), "You must be logged in to triage.")
		return
	}

	req := frontend.TriageRequest{}
	if err := parseJSON(r, &req); err != nil {
		httputils.ReportError(w, err, "Failed to parse JSON request.")
		return
	}
	sklog.Infof("Triage request: %#v", req)

	if err := wh.triage(r.Context(), user, req); err != nil {
		httputils.ReportError(w, err, "Could not triage")
		return
	}
...
func (wh *Handlers) ClusterDiffHandler(w http.ResponseWriter, r *http.Request) {
	...
	q := query.Search{Limit: 50}
	if err := query.ParseSearch(r, &q); err != nil {
		httputils.ReportError(w, err, "Unable to parse query parameter.")
		return
	}
	testName := q.TraceValues.Get(types.PRIMARY_KEY_FIELD)
	if testName == "" {
		httputils.ReportError(w, fmt.Errorf("test name parameter missing"), "No test name provided.")
		return
	}
...
func (wh *Handlers) ListTestsHandler(w http.ResponseWriter, r *http.Request) {
	...
	q := query.Search{}
	if err := query.ParseSearch(r, &q); err != nil {
		httputils.ReportError(w, err, "Failed to parse form data.")
		return
	}
	...
	if err := r.ParseForm(); err != nil {
		httputils.ReportError(w, err, "Invalid request.")
		return
	}
	...
	if !q.IncludeIgnores && q.Head && len(q.TraceValues) == 1 && hasSourceType {
		...
		if err != nil {
			httputils.ReportError(w, err, "Failed to calculate summaries.")
			return
		}
		for _, sum := range sumMap {
...
func (wh *Handlers) purgeDigests(w http.ResponseWriter, r *http.Request) bool {
	user := login.LoggedInAs(r)
	if user == "" {
		httputils.ReportError(w, fmt.Errorf("Not logged in."), "You must be logged in to clear digests.")
		return false
	}
	...
	if err := dec.Decode(&digests); err != nil {
		httputils.ReportError(w, err, "Unable to decode digest list.")
		return false
	}
	purgeGCS := r.URL.Query().Get("purge") == "true"

	if err := wh.DiffStore.PurgeDigests(digests, purgeGCS); err != nil {
		httputils.ReportError(w, err, "Unable to clear digests.")
		return false
	}
	return true
}
...
func (wh *Handlers) TriageLogHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Unable to retrieve triage log.")
		return
	}
...
func (wh *Handlers) TriageUndoHandler(w http.ResponseWriter, r *http.Request) {
	...
	user := login.LoggedInAs(r)
	if user == "" {
		httputils.ReportError(w, fmt.Errorf("Not logged in."), "You must be logged in to change expectations.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Unable to undo.")
		return
	}
...
func (wh *Handlers) CommitsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if cpxTile == nil {
		httputils.ReportError(w, nil, "Not loaded yet - try back later")
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
func (wh *Handlers) GitLogHandler(w http.ResponseWriter, r *http.Request) {
	...
	if start == end {
		...
		if err != nil {
			httputils.ReportError(w, err, "Failed to look up commit data.")
			return
		}
...
func (wh *Handlers) DigestTableHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := query.ParseDTQuery(r.Body, 5, &q); err != nil {
		httputils.ReportError(w, err, err.Error())
		return
	}
...
func (wh *Handlers) BaselineHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Fetching baselines failed.")
		return
	}
