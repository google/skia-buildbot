...
func (wh *WebHandlers) JsonCreateBaselineHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	baselineID, err := wh.SearchAPI.CreateBaseline(ctx, issueID, patchsetID)
	if err != nil {
		httputils.ReportError(w, r, err, "Generating a new baseline failed.")
		return
	}
	sendResponse(w, baselineID, http.StatusOK, nil)
}
...
func (wh *WebHandlers) JsonRefreshIssue(w http.ResponseWriter, r *http.Request) {
	...
	if err := wh.SearchAPI.ForceRefreshIssue(issueID); err != nil {
