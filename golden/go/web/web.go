...
func allUntriagedSummaries(idx *indexer.SearchIndex, query url.Values) (*tiling.Tile, map[types.TestName]*summary.Summary, error) {
	tile := idx.CpxTile().GetTile(types.IncludeIgnoredDigests)

	// Get a list of all untriaged images by test.
	sum, err := idx.CalcSummaries([]types.TestName{}, query, types.ExcludeIgnoredDigests, true)
	if err != nil {
...
func (wh *WebHandlers) JsonClusterDiffHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i, d := range searchResponse.Digests {
		...
		d3.ParamsetByDigest[d.Digest] = idx.GetParamsetSummary(d.Test, d.Digest, types.ExcludeIgnoredDigests)
...
func (wh *WebHandlers) JsonListTestsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !query.IncludeIgnores && query.Head && len(query.Query) == 1 && hasSourceType {
		sumMap := idx.GetSummaries(types.ExcludeIgnoredDigests)
...
func (wh *WebHandlers) JsonParamsHandler(w http.ResponseWriter, r *http.Request) {
	tile := wh.Indexer.GetIndex().CpxTile().GetTile(types.IncludeIgnoredDigests)
	w.Header().Set("Content-Type", "application/json")
