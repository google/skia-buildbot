...
// the tjstore.
func (wh *Handlers) getCLSummary(ctx context.Context, system clstore.CRSPackage, clID string) (frontend.ChangeListSummary, error) {
...
func (wh *Handlers) DetailsHandler(w http.ResponseWriter, r *http.Request) {
	...
	crs := r.Form.Get("crs")
	if clID != "" && crs == "" {
		http.Error(w, "must include issue and crs", http.StatusBadRequest)
		return
	}
...
func (wh *Handlers) triage(ctx context.Context, user string, req frontend.TriageRequest) error {
	if req.ChangeListID != "" && req.CodeReviewSystem == "" {
		return skerr.Fmt("Must include cl id and crs")
	}
	if _, ok := wh.getCodeReviewSystem(req.CodeReviewSystem); !ok {
		return skerr.Fmt("Unknown Code Review System")
	}
...
func (wh *Handlers) TriageLogHandler(w http.ResponseWriter, r *http.Request) {
	...
	logEntries, total, err := wh.getTriageLog(r.Context(), crs, changeList, offset, size, details)
...
func (wh *Handlers) ParamsHandler(w http.ResponseWriter, r *http.Request) {
	...
	crs := r.Form.Get("crs")
	if clID != "" && crs == "" {
		http.Error(w, "must include changelist_id and crs", http.StatusBadRequest)
		return
	}
	if _, ok := wh.getCodeReviewSystem(crs); !ok {
		http.Error(w, "Invalid Code Review System", http.StatusBadRequest)
...
func (wh *Handlers) BaselineHandler(w http.ResponseWriter, r *http.Request) {
	...
	// TODO(kjlubicK) make goldctl supply CRS
	bl, err := wh.Baseliner.FetchBaseline(r.Context(), clID, wh.ReviewSystems[0].ID, issueOnly)
	if err != nil {
...
func (wh *Handlers) getCodeReviewSystem(crs string) (clstore.CRSPackage, bool) {
	var system clstore.CRSPackage
	found := false
