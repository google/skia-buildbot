...
func (wh *Handlers) getIgnores(ctx context.Context, withCounts bool) ([]*frontend.IgnoreRule, error) {
	xr, err := wh.IgnoreStore.List(ctx)
	if err != nil {
		return nil, skerr.Wrapf(err, "fetching ignores from store")
	}

	xir := make([]*frontend.IgnoreRule, 0, len(xr))
	for _, r := range xr {
		ir, err := frontend.ConvertIgnoreRule(r)
		if err != nil {
	...
	if withCounts {
		if err := wh.addIgnoreCounts(ctx, xir); err != nil {
			return nil, skerr.Wrapf(err, "adding ignore counts to %d rules", len(xir))
		}
	}
...
func (wh *Handlers) addIgnoreCounts(ctx context.Context, rules []*frontend.IgnoreRule) error {
	defer metrics2.FuncTimer().Stop()
	sklog.Debug("adding counts to %d rules", len(rules))
