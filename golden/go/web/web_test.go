...
func TestGetClSummary_SunnyDay_Success(t *testing.T) {
	...
	gerritSystem := clstore.CRSPackage{
		ID:          gerritCRS,
		Store:       mcls,
		URLTemplate: "example.com/cl/%s#templates",
	}
	...
	wh := Handlers{
		HandlersConfig: HandlersConfig{
			ReviewSystems: []clstore.CRSPackage{gerritSystem},
			TryJobStore:   mtjs,
		},
	}
...
func TestBaselineHandler_Success(t *testing.T) {
	...
	mbf.On("FetchBaseline", testutils.AnyContext, "" /* =clID */, gerritCRS, false /* =issueOnly */).Return(bl, nil)
