...
func TestGetActiveChangeListsSunnyDay(t *testing.T) {
	...
	activeSearchOptions := mock.MatchedBy(func(so clstore.SearchOptions) bool {
		assert.Equal(t, 20, so.StartIdx)
		assert.Equal(t, 30, so.Limit)
		assert.True(t, so.OpenCLsOnly)
		assert.NotZero(t, so.After)
		return true
	})

	mcls.On("GetChangeLists", testutils.AnyContext, activeSearchOptions).Return(makeCodeReviewCLs(), 3, nil)
	mcls.On("System").Return("gerrit")
