...
func TestLatestPositiveDigest_Success(t *testing.T) {
	...
	digest := types.Digest("11111111111111111111111111111111")
	...
	assertJSONResponseWas(t, http.StatusOK, fmt.Sprintf(`{"digest":%q}`, digest), w)
}
...
func TestLatestPositiveDigest_SearchIndexerFailure_InternalServerError(t *testing.T) {
	...
	assert.Contains(t, w.Body.String(), fmt.Sprintf("Could not retrieve most recent positive digest for trace %q", traceId))
}
