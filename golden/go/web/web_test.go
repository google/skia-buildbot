...
func TestUpdateIgnoreRuleInvalidBody(t *testing.T) {
	unittest.SmallTest(t)
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/json/ignores/save/12345", strings.NewReader("invalid JSON"))
	r = setID(r, id)
	wh.IgnoresUpdateHandler(w, r)
...
func TestDeleteIgnoreRuleSunnyDay(t *testing.T) {
	...
	mis.On("Delete", testutils.AnyContext, id).Return(true, nil)
...
func TestDeleteIgnoreRuleNotFound(t *testing.T) {
	unittest.SmallTest(t)
	...
	mis.On("Delete", testutils.AnyContext, id).Return(false, nil)
...
func TestDeleteIgnoreRuleStoreError(t *testing.T) {
	unittest.SmallTest(t)
	...
	mis.On("Delete", testutils.AnyContext, id).Return(false, errors.New("firestore broke"))
