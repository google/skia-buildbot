...
func TestTriageMaster(t *testing.T) {
	...
	tr := frontend.TriageRequest{
		...
		TestDigestStatus: map[types.TestName]map[types.Digest]string{
			bug_revert.TestOne: {
				bug_revert.UntriagedDigestBravo: "negative",
			},
		},
	}
...
// TestTriageMaster tests a common case of a developer triaging a single test on a ChangeList
func TestTriageChangeList(t *testing.T) {
	unittest.SmallTest(t)

	mes := &mocks.ExpectationsStore{}
	clStore := &mocks.ExpectationsStore{}
	defer mes.AssertExpectations(t)
	defer clStore.AssertExpectations(t)

	clID := "12345"
	crs := "github"
	user := "user@example.com"

	mes.On("ForChangeList", clID, crs).Return(clStore)

	clStore.On("AddChange", testutils.AnyContext, types.Expectations{
		bug_revert.TestOne: {
			bug_revert.UntriagedDigestBravo: types.NEGATIVE,
		},
	}, user).Return(nil)
...
func TestBulkTriageMaster(t *testing.T) {
	...
	tr := frontend.TriageRequest{
		...
		TestDigestStatus: map[types.TestName]map[types.Digest]string{
			bug_revert.TestOne: {
				bug_revert.GoodDigestAlfa:       "untriaged",
				bug_revert.UntriagedDigestBravo: "negative",
			},
			bug_revert.TestTwo: {
				bug_revert.GoodDigestCharlie:    "positive",
				bug_revert.UntriagedDigestDelta: "negative",
			},
		},
	}
...
// TestTriageMaster tests a common case of a developer triaging a single test using the legacy
// code (which has "0" as key issue instead of empty string.
