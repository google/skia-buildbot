...
func TestBaselineHandler_Success(t *testing.T) {
	...
	// Prepare a dummy response from the BaselineFetcher and the corresponding JSON expected from the handler.
	bl := &baseline.Baseline{ChangeListID: "", CodeReviewSystem: "gerrit"}
	expectedJSONResponse := `{"md5":"","master":null,"crs":"gerrit"}`

	mbf.On("FetchBaseline", testutils.AnyContext, /* clID= */ "", "gerrit", /* issueOnly= */ false).Return(bl, nil)
...
func TestBaselineHandler_IssueSet_Success(t *testing.T) {
	...
	mbf.On("FetchBaseline", testutils.AnyContext, /* clID= */ "123456", "gerrit", /* issueOnly= */ false).Return(bl, nil)
	defer mbf.AssertExpectations(t)  // Assert that the method above was called exactly as expected.
...
func TestBaselineHandler_IssueSet_IssueOnly_Success(t *testing.T) {
	...
	mbf.On("FetchBaseline", testutils.AnyContext, /* clID= */ "123456", "gerrit", /* issueOnly= */ true).Return(bl, nil)
	defer mbf.AssertExpectations(t)  // Assert that the method above was called exactly as expected.
