...
// It uses some example data in which
func TestByQuerySunnyDay(t *testing.T) {
	...
	mis.On("GetBlame", bug_revert.TestOne, bug_revert.UntriagedDigestBravo, bug_revert.MakeTestCommits()).
		Return(makeBugRevertDeltaBlame(), nil)
	mis.On("GetBlame", bug_revert.TestTwo, bug_revert.UntriagedDigestDelta, bug_revert.MakeTestCommits()).
		Return(makeBugRevertDeltaBlame(), nil)
