...
// ExpectationsHandler is an interface to interact with expectations. It is mostly a wrapper around
// a TestExp instance, but does some intermediate processing if necessary and might filter results.
type ExpectationsHandler interface {
	// TestExp returns the underlying expectations. Generally this should not be altered since any
	// alteration would also change the ExpectationsHandler instance. If alterations are necessary
...
// NewExpectationsHandler returns an ExpectationsHandler instance that wraps around an instance
// of TestExp. If 'testExp' is not nil, then NewExpectationsHandler will take ownership of it and
// wrap around it.
func NewExpectationsHandler(testExp TestExp) *HandlerImpl {
	if testExp == nil {
		testExp = TestExp{}
	}
...
// TestExp implements the ExpectationsHandler interface.
func (b *HandlerImpl) TestExp() TestExp {
	return b.testExp
}

// Classification implements the ExpectationsHandler interface.
...
// AddTestExp implements the ExpectationsHandler interface.
...
// SetExpectation implements the ExpectationsHandler interface.
...
// Ensure HandlerImpl fulfills the ExpectationsHandler interface
var _ ExpectationsHandler = (*HandlerImpl)(nil)
