...
type ComplexTileImpl struct {
	// tileExcludeIgnoredDigests is the current tile without ignored traces.
	tileExcludeIgnoredDigests *tiling.Tile

	// tileIncludeIgnoredDigests is the current tile containing all available data.
	tileIncludeIgnoredDigests *tiling.Tile
...
func NewComplexTile(completeTile *tiling.Tile) *ComplexTileImpl {
	return &ComplexTileImpl{
		tileExcludeIgnoredDigests: completeTile,
		tileIncludeIgnoredDigests: completeTile,
	}
}
...
func (c *ComplexTileImpl) SetIgnoreRules(reducedTile *tiling.Tile, ignoreRules paramtools.ParamMatcher, irRev int64) {
	c.tileExcludeIgnoredDigests = reducedTile
	c.irRevision = irRev
...
func (c *ComplexTileImpl) SetSparse(sparseCommits []*tiling.Commit, cardinalities []int) {
	...
	if len(sparseCommits) == 0 {
		sparseCommits = c.tileIncludeIgnoredDigests.Commits
	}

	filled := len(c.tileIncludeIgnoredDigests.Commits)
	if len(cardinalities) == 0 {
		cardinalities = make([]int, len(sparseCommits))
		for idx := range cardinalities {
			cardinalities[idx] = len(c.tileIncludeIgnoredDigests.Traces)
		}
	} else {
...
func (c *ComplexTileImpl) FromSame(completeTile *tiling.Tile, ignoreRev int64) bool {
	return c != nil &&
		c.tileIncludeIgnoredDigests != nil &&
		c.tileIncludeIgnoredDigests == completeTile &&
		c.tileExcludeIgnoredDigests != nil &&
		c.irRevision == ignoreRev
}

// DataCommits fulfills the ComplexTile interface.
func (c *ComplexTileImpl) DataCommits() []*tiling.Commit {
	return c.tileIncludeIgnoredDigests.Commits
}
...
func (c *ComplexTileImpl) GetTile(is IgnoreState) *tiling.Tile {
	if is == IncludeIgnoredDigests {
		return c.tileIncludeIgnoredDigests
	}
	return c.tileExcludeIgnoredDigests
}
