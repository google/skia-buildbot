...
type iBuildBucketSvc interface {
...
func NewBuildBucketState(config *Config) (BuildIssueSync, error) {
	...
	var service iBuildBucketSvc
	if config.BuildBucketVersion == 1 {
		service, err = newBuildBucketV1(config.Client, config.BuildBucketURL, config.BuildBucketName, builderRegExp)
	} else if config.BuildBucketVersion == 2 {
		service, err = newBuildBucketV2(config.Client, config.BuildBucketURL, config.BuildBucketName, builderRegExp)
	} else {
		return nil, skerr.Fmt("Unknown BuildBucket version %d", config.BuildBucketVersion)
	}
	if err != nil {
		return nil, err
	}
