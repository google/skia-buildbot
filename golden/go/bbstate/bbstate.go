...
type BuildIssueSync interface {
	...
	// Gerrit and the Tryjob information from the build system. If the returned error is nil (and the
	// returned error is also nil), then we got a 404 when requesting the Issue.
...
func (b *BuildBucketState) SyncIssueTryjob(issueID, buildBucketID int64) (*tryjobstore.Issue, *tryjobstore.Tryjob, error) {
	...
	if err := b.updateTryjobState(tryjob); err != nil {
		// If we got a 404 from Gerrit we signal this back to caller with a nill result.
