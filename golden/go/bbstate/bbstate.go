...
const (
	...
	// DefaultTestBuilderRegex is the regexp that identifies tryjobs that run tests/produce images.
...
func NewBuildBucketState(config *Config) (IssueBuildFetcher, error) {
	...
	// compile the regular expression to identify builders we are i
...
func (b *BuildBucketState) ignoreBuild(build *bb_api.ApiCommonBuildMessage, params *tryjobstore.Parameters) bool {
	ret := !b.builderRegExp.Match([]byte(params.BuilderName))
	if !ret {
		sklog.Infof("Accepting: %s", params.BuilderName)
	}
	return ret
}
