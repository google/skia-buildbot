...
func (a *Annotation) IndexValues(indices []string) map[string][]string {
...
func (b *boltIssueStore) ByDigest(digest string) ([]string, error) {
	ret, err := b.store.ReadIndex(DIGEST_INDEX, []string{digest})
	if err != nil {
		return nil, err
	}
	return ret[digest], nil
}

// ByIgnore, see IgnoreStore interface.
func (b *boltIssueStore) ByIgnore(ignoreID string) ([]string, error) {
	ret, err := b.store.ReadIndex(IGNORE_INDEX, []string{ignoreID})
	if err != nil {
		return nil, err
	}
	return ret[ignoreID], nil
}

// ByTrace, see IgnoreStore interface.
func (b *boltIssueStore) ByTrace(traceID string) ([]string, error) {
	ret, err := b.store.ReadIndex(TRACE_INDEX, []string{traceID})
	if err != nil {
		return nil, err
	}
...
func (b *boltIssueStore) ByTest(testName string) ([]string, error) {
	ret, err := b.store.ReadIndex(TEST_INDEX, []string{testName})
	if err != nil {
		return nil, err
	}
	return ret[testName], nil
}
...
func (b *boltIssueStore) Add(delta *Annotation) error {
	...
	writeFn := func(tx *bolt.Tx, result []boltutil.Interface) ([]boltutil.Interface, error) {
		if result[0] != nil {
	...
	return b.store.Write([]boltutil.Interface{delta}, writeFn)
}
	...
	writeFn := func(tx *bolt.Tx, result []boltutil.Interface) ([]boltutil.Interface, error) {
		...
		if found != nil {
			...
			if rec.Subtract(delta) {
				...
				if err := b.store.Delete([]string{rec.IssueID}, tx); err != nil {
					return nil, err
				}
			}
		}
