...
func (b *boltIssueStore) index(tx *bolt.Tx, rec *Annotation, subtract bool) error {
	...
	if err := b.updateIndex(tx, TEST_INDEX, rec.TestNames, rec.IssueID, subtract); err != nil {
		return err
	}
	return nil
}
...
// mergeStrings merges the string arrays and removes duplicates.
...
// removeStrings removes all strings from tgt that appear in src.
