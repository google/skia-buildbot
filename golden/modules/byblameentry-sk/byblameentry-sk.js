...
<div class="blame">
  <p>
    <a href=${el._blameHref()} class=triage target="_blank">
      ${el.byBlameEntry.nDigests === 1
          ? '1 digest needs triaging'
          : `${el.byBlameEntry.nDigests} digests need triaging`}
    </a>
  </p>
  ...
  ${!el.byBlameEntry.affectedTests || el.byBlameEntry.affectedTests.length === 0
      ? ''
      : html`
          <table class=affected-tests>
            ...
            <tbody>
              ${el.byBlameEntry.affectedTests.map(
                  (test) => html`
                      <tr>
                        <td class=test>${test.test}</td>
                        <td class=num-digests>${test.num}</td>
                        <td>
                          <a href=${el._detailHref(test)} class=example-link>
                            ${test.sample_digest}
                          </a>
                        </td>
                      </tr>`)}
