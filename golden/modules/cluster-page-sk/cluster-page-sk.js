...
<div>
  <search-controls-sk .corpora=${ele._corpora}
      ...
      .searchCriteria="${ele._searchParams}"></search-controls-sk>
...
define('cluster-page-sk', class extends ElementSk {
  constructor() {
    ...
    this._currentCorpus = '';
    this._superImportantState = '';

    this._stateChanged = stateReflector(
      /* getState */() => {
        const hintObj = searchControls.ToHintableObject(
          this._searchCriteria);
        hintObj.super_important_state = this._superImportantState;
        return hintObj;
      }, /* setState */(newState) => {
        if (!this._connected) {
          return;
        }
        this._searchCriteria = searchControls.FromUrl(newState);
