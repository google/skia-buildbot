...
//   - A non-hightlighted dot is represented with [a-g], where 'a' represents
//     the most recent commit.
//   - A hightlighted dot is represented with [A-G].
//   - A blank position is represented with ' '.
const dotToAscii = (dotsSk, x,  y) => {
  const canvasX = dotToCanvasX(x);
  const canvasY = dotToCanvasY(y);

  // Sample a few pixels (north, east, south, west, center).
...
// Returns the #RRGGBB color for the pixel at (x, y) in the canvas.
...
// Finds the color in the haystack with the minimum Euclidean distance to
// the given color. This is necessary for pixels in the circumference of a dot
// due to canvas anti-aliasing.
const closestColor = (needle, haystack) => {
  const rgb = hexToRgb(needle);
  let closestColor = '', closestDistance = -1;
  haystack.forEach((color) => {
    const rgb2 = hexToRgb(color);
    const distance =  // Euclidean distance, squared.
        Math.pow(rgb[0] - rgb2[0], 2) +
        Math.pow(rgb[1] - rgb2[1], 2) +
        Math.pow(rgb[2] - rgb2[2], 2);
