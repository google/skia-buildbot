...
const closestColor = (needle, haystack) => {
  return haystack
    .map((color) => { return { color: color, dist: euclideanDistanceSq(needle, color) }; })
    .reduce((acc, cur) => { return ((acc.dist < cur.dist) ? acc : cur); })
    .color;
};
...
const dotsSkEventPromise = (dotsSk, event) => {
  let resolve;
  const promise = new Promise((_resolve) => { return resolve = _resolve; });
  const handler = (e) => {
