 ...
 *   a slice of commits that could have made up that dot.
 *
...
// Maps each dots-sk _id to the element itself. Members are only present if they
// have a pending mousemove update.
let pendingMouseMoves = {};

// Periodically process all pending mousemoves. We do not want to do any work
// on a mouse move event as that can very easily degrade browser performance,
// e.g. as the user drags the mouse pointer over the element. Processing mouse
// events in batches remedies this.
window.setInterval(() => {
...
define('dots-sk', class extends ElementSk {
  ...
  _draw() {
    let w = (this._value.tileSize - 1) * DOT_SCALE_X + 2 * DOT_OFFSET_X;
    let h = (this._value.traces.length - 1) * DOT_SCALE_Y + 2 * DOT_OFFSET_Y;
  ...
  /**
   * @prop commits {Object} An aarray of commits, such as:
   *
   *   [
   *     {
  ...
  _onMouseLeave() {
    let oldHoverIndex = this._hoverIndex;
    this._hoverIndex =  -1;
    this._overDrawTraceDots(oldHoverIndex);
    this._lastMouseMove = null;
  }
  ...
  _updatePendingMouseMove() {
    ...
    if (this._hoverIndex !== dotCoords.y) {
      ...
      this._overDrawTraceDots(oldIndex);
      this._overDrawTraceDots(this._hoverIndex);
    }

    // Set the cursor to a pointer if you are hovering over a dot.
    let found = false;
    let trace = this._value.traces[dotCoords.y];
    if (trace) {
  ...
  // When a dot is clicked on, produce the show-commits event with all the
  // commits that are included upto and including that dot.
  _onClick(e) {
    ...
    for (let i = trace.data.length - 1; i >= 0; i--) {
      if (trace.data[i].x === dotCoords.x) {
        found = true;
        if (i > 0) {
          // Look backwards in the trace for the previous commit with data. This
          // will
    ...
    let commitInfo = this._commits.slice(firstCommit, dotCoords.x + 1);
    commitInfo.reverse();
