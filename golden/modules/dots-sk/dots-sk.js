...
define('dots-sk', class extends ElementSk {
  ...
  _draw() {
    ...
    this._value.traces.forEach((trace, traceIndex) => {
      ...
      for (let i = trace.data.length; i >= 0; i--) {
        // -1 means missing
        if (trace.data[i] >= 0) {
  ...
  _computeBlamelist(trace, x) {
    ...
    let previousNonMissingPoint = 0;
    for (let i = x - 1; i >= 0; i--) {
      // -1 means missing
      if (trace.data[i] >= 0) {
        previousNonMissingPoint = i + 1;
        break;
      }
    }
    const blamelist = this._commits.slice(previousNonMissingPoint, x + 1);
    blamelist.reverse();
    // TODO(kjlubick) if previousNonMissingPoint is 0, that could mean this trace just
