...
export class SearchControlsSkPO extends PageObject {
  getCorpusSelectorPO() {
    return this.$$apply('corpus-selector-sk', async (el) => new CorpusSelectorSkPO(el));
  }

  getTraceFilterSkPO() {
    return this.$$apply('.traces > trace-filter-sk', async (el) => new TraceFilterSkPO(el));
  }

  getFilterDialogSkPO() {
    return this.$$apply('filter-dialog-sk', async (el) => new FilterDialogSkPO(el));
  }

  isIncludePositiveDigestsCheckboxChecked() {
    return this.$$evalDom('.include-positive-digests', (c) => (c as CheckOrRadio).checked);
  }

  async clickIncludePositiveDigestsCheckbox() {
    await this.$$apply('.include-positive-digests', (el) => el.click());
  }

  isIncludeNegativeDigestsCheckboxChecked() {
    return this.$$evalDom('.include-negative-digests', (c) => (c as CheckOrRadio).checked);
  }

  async clickIncludeNegativeDigestsCheckbox() {
    await this.$$apply('.include-negative-digests', (el) => el.click());
  }

  isIncludeUntriagedDigestsCheckboxChecked() {
    return this.$$evalDom('.include-untriaged-digests', (c) => (c as CheckOrRadio).checked);
  }

  async clickIncludeUntriagedDigestsCheckbox() {
    await this.$$apply('.include-untriaged-digests', (el) => el.click());
  }

  isIncludeDigestsNotAtHeadCheckboxChecked() {
    return this.$$evalDom('.include-digests-not-at-head', (c) => (c as CheckOrRadio).checked);
  }

  async clickIncludeDigestsNotAtHeadCheckbox() {
    await this.$$apply('.include-digests-not-at-head', (el) => el.click());
  }

  isIncludeIgnoredDigestsCheckboxChecked() {
    return this.$$evalDom('.include-ignored-digests', (c) => (c as CheckOrRadio).checked);
  }

  async clickIncludeIgnoredDigestsCheckbox() {
    await this.$$apply('.include-ignored-digests', (el) => el.click());
  }

  async clickMoreFiltersBtn() {
    return this.$$apply('.more-filters', (btn) => btn.click());
  }
