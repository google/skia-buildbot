...
export class SearchPageSk extends ElementSk {
  ...
  private _onKeyDown(event: KeyboardEvent) {
    ...
    switch(event.key) {
      ...
      // Zoom in.
      case 'w':
        this._openZoomDialogForSelectedResult();
        break;

      // Mark as positive.
      case 'a':
        this._triageSelectedResult('positive');
        break;

      // Mark as negative.
      case 's':
        this._triageSelectedResult('negative');
        break;

      // Mark as untriaged.
      case 'd':
        this._triageSelectedResult('untriaged');
        break;
  ...
  private _selectSearchResult(index: number) {
    if (index < 0 || index > (this._searchResponse?.digests || []).length - 1) return;
  ...
  private _triageSelectedResult(label: Label) {
  ...
  private _openZoomDialogForSelectedResult() {
    // TODO(lovisolo): Clean this up once DigestDetailsSk is ported to TypeScript.
