...
export class SearchPageSk extends ElementSk {
  ...
  private async _fetchCorporaOnce() {
    ...
    try {
      sendBeginTask(this);
      const statusResponse: StatusResponse = await fetch('/json/v1/trstatus', { method: 'GET' }).then(jsonOrThrow);
  ...
  private async _maybeFetchChangelistSummaryOnce() {
    ...
    try {
      sendBeginTask(this);
      this._changeListSummaryResponse = await fetch(`/json/v1/changelist/${this._crs}/${this._changelistId}`, { method: 'GET' })
        .then(jsonOrThrow);
      this._render();
      sendEndTask(this);
    } catch (e) {
