...
export class SearchPageSk extends ElementSk {
  ...
  private _master: boolean | null = null;
  private _patchsets: number | null = null;
  ...
  constructor() {
    ...
    this._stateChanged = stateReflector(
      /* getState */ () => {
        ...
        state.master = this._master || '';
        state.patchsets = this._patchsets || '';
        return state;
      },
      /* setState */ (newState) => {
        if (!this._connected) {
        ...
        this._master = (newState.master as boolean) || null;
        this._patchsets = (newState.patchsets as number) || null;
        this._render();
        this._fetchChangeListSummary(); // Called here because it needs the CRS and CL number.
  ...
  connectedCallback() {
    ...
    // We fetch the corpora and ParamSet just once, on page load.
    this._fetchCorpora();
    this._fetchParamSet();
  }
  ...
  private async _fetchChangeListSummary() {
    ...
    if (!this._crs || !this._issue) return;
    ...
    try {
      ...
      this._changeListSummaryResponse =
        await fetch(`/json/v1/changelist/${this._crs}/${this._issue}`, {method: 'GET'})
          .then(jsonOrThrow);
      this._render();
      sendEndTask(this);
    } catch(e) {
  ...
  private async _fetchSearchResults() {
    ...
    if (this._master) searchRequest.master = this._master;
    if (this._patchsets) searchRequest.patchsets = this._patchsets;
