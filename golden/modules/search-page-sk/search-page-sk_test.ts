  ...
  describe('keyboard shortcuts', () => {
    ...
    describe('navigation', () => {
      it('initially has an empty selection', async () => {
        await instantiate();
        expect(await searchPageSkPO.getSelectedDigest()).to.be.empty;
      });
      ...
      it('can navigate between digests with keys "J" and "K"', async () => {
        ...
        expect(await searchPageSkPO.getSelectedDigest()).to.deep.equal([firstDigest]);

        // Forward.
        await searchPageSkPO.typeKey('j');
        expect(await searchPageSkPO.getSelectedDigest()).to.deep.equal([secondDigest]);

        // Forward.
        await searchPageSkPO.typeKey('j');
        expect(await searchPageSkPO.getSelectedDigest()).to.deep.equal([thirdDigest]);
    ...
    describe('triaging', () => {
      it('cannot triage with "A", "S" and "D" keys when the selection is empty', async () => {
        ...
        expect(await searchPageSkPO.getDigestLabels()).to.deep.equal(
    ...
    describe('zoom', () => {
      it('cannot zoom with the "W" key when the selection is empty', async () => {
        ...
        expect(await searchPageSkPO.getDigestWithOpenZoomDialog()).to.be.empty;
      ...
      it('can zoom into the selected digest with the "W" key', async () => {
        ...
        await searchPageSkPO.typeKey('w');
        expect(await searchPageSkPO.getDigestWithOpenZoomDialog()).to.deep.equal([secondDigest]);
      });
