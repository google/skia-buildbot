...
export class LastCommitSk extends ElementSk {
  private static template = (ele: LastCommitSk) => html`
    <div class=last_commit>
      <a href="${LastCommitSk.commitURL(ele._status)}">
      Last Commit: ${LastCommitSk.lastCommitText(ele._status)}
      </a>
    <div>
  `;
  ...
  private _status: StatusResponse | null = null;
  ...
  connectedCallback() {
    ...
    const reload = () => {
      ...
      fetch('/json/v1/trstatus')
        .then(jsonOrThrow)
        .then((json) => {
          this._status = json as StatusResponse;
          this._render();
