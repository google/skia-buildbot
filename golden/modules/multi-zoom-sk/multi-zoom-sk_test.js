...
describe('multi-zoom-sk', () => {
  ...
  it('uses m to manually go through the images selected by the checkboxes', () => {
    // default values
    expect(multiZoomSk._rotatingView).to.equal(true);
    expect(multiZoomSk._rotateThrough).to.deep.equal([true, false, true]);

    // by default, only rotates through the left and right images (index 0 and 2).
    multiZoomSk._zoomedIndex = 0;
    sendKeyboardEvent('m');
    expect(multiZoomSk._rotatingView).to.equal(false);
    ...
    // id_1 is the diff check box
    $$('checkbox-sk.id_1 input', multiZoomSk).click();
    expect(multiZoomSk._rotateThrough).to.deep.equal([true, true, true]);
    ...
    $$('checkbox-sk.id_0 input', multiZoomSk).click();
    $$('checkbox-sk.id_1 input', multiZoomSk).click();
    expect(multiZoomSk._rotateThrough).to.deep.equal([false, false, true]);
