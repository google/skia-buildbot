...
<div class=previews_and_toggles>
  ${thumbnailAndToggle(ele._leftImageSrc, ele._leftLabel, leftImageIdx, ele)}
  ${thumbnailAndToggle(ele._diffImageSrc, 'Diff', diffImageIdx, ele)}
  ${thumbnailAndToggle(ele._rightImageSrc, ele._rightLabel, rightImageIdx, ele)}
</div>
...
// thumbnailAndToggle dynamically creates an img, a canvas and a checkbox-sk with classes "id_N"
...
<figure class=preview>
  <img class="thumbnail id_${idx}" src=${src} alt=${label} @load=${() => ele._loadedImage(idx)}>
  <canvas class="crosshair id_${idx}" width=${previewCanvasSize} height=${previewCanvasSize}
      @click=${(e) => ele._snapToClick(e, idx)}></canvas>
  <figcaption>
    <checkbox-sk label=${label} class="displayed for_spacing"></checkbox-sk>
    <checkbox-sk label=${label} class="id_${idx} ${idx === ele._zoomedIndex ? 'displayed' : ''}"
...
define('multi-zoom-sk', class extends ElementSk {
  ...
  _getCrosshairCanvas(imgIdx) {
    return $$(`canvas.id_${imgIdx}`, this);
  }

  _getImage(imgIdx) {
    return $$(`img.id_${imgIdx}`, this);
  }
