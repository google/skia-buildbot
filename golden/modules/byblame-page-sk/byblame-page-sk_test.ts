...
import { canvaskit, gm, svg, trstatus } from './demo_data';
import {
...
describe('byblame-page-sk', () => {
  ...
  const loadedByblamePageSk = (
    opts: { defaultCorpus?: string; baseRepoUrl?: string } = {}
  ): Promise<ByBlamePageSk> => {
    testOnlySetSettings({
  ...
  it('shows loading indicator', async () => {
    ...
    expect($$<HTMLElement>('.entries', byblamePageSk)!.innerText).to.equal(
      'Loading untriaged digests...'
    );
  ...
  it('correctly renders a page with empty results', async () => {
    ...
    expect($$<HTMLElement>('.entries', byblamePageSk)!.innerText).to.equal(
      'No untriaged digests for corpus canvaskit.'
    );
    expectHasEmptyBlames(byblamePageSk);
  });

  it('renders blames for default corpus if URL does not include a corpus', async () => {
    fetchMock.get('/json/v2/trstatus', trstatus);
  ...
  describe('Base repository URL', () => {
    ...
    it('renders commit links correctly with repo hosted on googlesource', async () => {
      const byblamePageSk = await loadedByblamePageSk({
        defaultCorpus: 'gm',
    ...
    it('renders commit links correctly with repo hosted on GitHub', async () => {
      const byblamePageSk = await loadedByblamePageSk({
        defaultCorpus: 'gm',
