...
export class NumericParamPO extends PageObject {
  ...
  async setValue(value: number) { await (await this.rangeInput).enterValue(value.toString()); }
}
...
export class FilterDialogSkPO extends PageObject {
  ...
  async setMinRGBADelta(value: number) { return (await this.minRGBADeltaPO).setValue(value); }

  async getMaxRGBADelta() { return (await this.maxRGBADeltaPO).getRangeInputValue(); }

  async setMaxRGBADelta(value: number) { return (await this.maxRGBADeltaPO).setValue(value); }
