...
export class FilterDialogSk extends ElementSk {
  ...
  private static _template = (el: FilterDialogSk) => html`
    ...
    <dialog class=filter-dialog>
      <div class=content>
        ...
        <div class=right-hand-query>
          <div class=query>
            ${!el._value || Object.keys(el._value!.diffConfig).length === 0
              ...
              : html`<paramset-sk .paramsets=${live([el._value?.diffConfig])}></paramset-sk>`}
          </div>
        ...
        <select id=sort-by
                .value=${live(el._value?.sortOrder)}
  ...
  open(paramSet: ParamSet, value: FilterDialogSkValue) {
    ...
    // We make a copy of the caller's value object because the value object gets updated on user
    // input, and if the user clicks the "Cancel" button after making some changes, we want the
    // caller's value object to remain intact.
    this._value = deepCopy(value);
