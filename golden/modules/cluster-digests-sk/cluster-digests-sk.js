...
<svg class=cluster width=${ele._width} height=${ele._height}></svg>
`;
...
define('cluster-digests-sk', class extends ElementSk {
  constructor() {
    ...
    // TODO(kjlubick) make this either setable by clients or dynamically respond to the size it
   ...
   */
  _layout() {
    const clusterSk = $$('svg.cluster', this);
    ...
    const chargeForce = d3Force.forceManyBody()
      ...
      .distanceMax(48); // Given our nodes have a radius of 12, if two nodes are 48 pixels apart,
                        // they are definately not overlapping, so we can stop counting their
                        // "charge". This should help performance by reducing computation needs.
