...
describe('triagelog-page-sk', () => {
  ...
  describe('URL parameters', () => {
    it('initializes paging based on the URL parameters', async () => {
      ...
      let queryString = '?details=true&offset=3&page_size=3';
      history.pushState(
          null,
          '',
  ...
  describe('RPC error', () => {
    it('should emit event "fetch-error" on RPC failure', async () => {
      ...
      expect($$('tbody').childElementCount).to.equal(0);
    });
  });
  ...
  function endTaskEvent(fn) {
    let promise = newEventPromise('end-task');
    fn();
    return promise;
  }
  ...
  function fetchErrorEvent(fn) {
    let promise = newEventPromise('fetch-error');
    fn();
    return promise;
  }
