...
const template = (el) => {
  ...
  const detailsTemplate = (entry) => !entry.details ? html`` : html`
    <tr class="details">
      <td></td>
  ...
  return html`
    ...
    <table>
      ...
      <tbody>
        ${el._entries.map(logEntryTemplate)}
      </tbody>
    </table>
  `;
};
...
define('triagelog-page-sk', class extends ElementSk {
  constructor() {
    ...
    this._stateChanged = stateReflector(
        /*getState*/() => {
          ...
          this._pageSize = newState.size || 20;
  ...
  _undoHandler(e) {
    ...
    // The undo handler returns the first results page without details, so we
    // need to uncheck the "Show details" checkbox.
    this._details = false;
    this._render();
  ...
  _fetchEntries() {
    this._fetch(`/json/triagelog?details=${this._details}&offset=${this._pageOffset}&size=${this._pageSize}`);
  }
  ...
  _handleServerResponse(promise) {
    promise
        ...
        .catch((e) => {
          ...
          this._sendFetchError(e, 'triagelog')
        });
  }
