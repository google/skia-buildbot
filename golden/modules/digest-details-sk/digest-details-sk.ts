...
export class DigestDetailsSk extends ElementSk {
    ...
    <div class=container>
      ...
      <div class=comparison>
        <div class=digest_labels>
          <span class="digest_label left">Left: ${ele._digest}</span>
          <span class=expand></span>
        ...
        <div class=comparison_data>
          ...
          <div>
            <button
                @click=${ele._toggleRightRef}
                ?disabled=${!ele._canToggle()}
                class=toggle_ref
                ?hidden=${ele._overrideRight || !ele.right}
                title=${toggleButtonMouseover(ele._canToggle())}>
              Toggle Reference
            </button>
    ...
    if (!ele.right) {
      return html`
        <div class=metrics_and_triage>
          ...
          ${DigestDetailsSk.triageHistory(ele)}
      </div>
      `;
    }
  ...
  private static triageHistory = (ele: DigestDetailsSk) => {
    if (ele._triageHistory.length === 0) return '';

    const mostRecent = ele._triageHistory[0];
    return html`
    ...
    return html`
      <div class=trace_info>
        <dots-sk
            ...
            @hover=${ele._hoverOverTrace}
            @mouseleave=${ele._clearTraceHighlights}
            @showblamelist=${ele._showBlamelist}>
        </dots-sk>
        <dots-legend-sk
            .digests=${ele._traces.digests}
            .changeListID=${ele.changeListID}
            .crs=${ele.crs}
            .test=${ele._grouping}
            .totalDigests=${ele._traces.total_digests || 0}>
        </dots-legend-sk>
      </div>
    `;
  };
