...
<div class=container>
  ...
  <div class=comparison>
    <div class=digest_labels>
      ...
      <span class=expand></span>
      <span class=test_name ?hidden=${!ele.right}>Right: ${ele.right && ele.right.digest}</span>
    </div>
    <div class=comparison_data>
      ...
      <div>
        ...
        <div ?hidden=${!ele.right || ele.right.status !== 'negative'} class=warning>
...
<div class=metrics_and_triage>
  <div>
    <a href=${diffPageHref(ele._grouping, ele._digest, ele.right.digest, ele._issue)}>
      Diff Details
    </a>
  </div>
  <div class=warning ?hidden=${!ele.right.dimDiffer}>Images differ in size!</div>
  <div>
...
const imageComparison = (ele) => {
  ...
  const diff = {
    src: diffImgSrc(ele._digest, ele.right.digest),
  };
  const right = {
    src: imgSrc(ele.right.digest),
    title: ele.right.status === 'positive' ? 'Closest Positive' : 'Closest Negative',
    detail: detailHref(ele._grouping, ele.right.digest, ele._issue),
  };
...
const paramset = (ele) => {
  ...
  if (ele.right) {
    input.titles.push(shorten(ele.right.digest));
