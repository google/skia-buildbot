...
import {
  shorten, imgSrc, diffImgSrc, detailHref, diffPageHref,
} from '../common';
...
<div class=container>
  <div class=top_bar>
    ...
    <a href=${ele._clusterHref()} target=_blank rel=noopener>
  ...
  <div class=comparison>
    <div class=digest_labels>
      ...
      <span class=expand></span>
      <span class=test_name ?hidden=${!ele._right}>Right: ${ele._right && ele._right.digest}</span>
    </div>
    <div class=comparison_data>
      <div>${detailsAndTriage(ele)}</div>
      <div>${imageComparison(ele)}</div>
      <div>
        <button @click=${ele._toggleRightRef} ?disabled=${!ele._canToggle()}
           title=${toggleButtonMouseover(ele._canToggle())}>Toggle Reference</button>
...
<div class=metrics_and_triage>
  <div>
    <a href=${diffPageHref(ele._grouping, ele._digest, ele._right.digest, ele._issue)}>
      Diff Details
    </a>
  </div>
  <div class=warning ?hidden=${!ele._right.dimDiffer}>Images differ in size!</div>
  <div>
    <span class=metric>Diff metric:</span>
    <span>${ele._right.diffs.combined.toFixed(3)}</span>
  </div>
  <div>
    <span class=metric>Diff %:</span>
    <span>${ele._right.pixelDiffPercent.toFixed(2)}</span>
  </div>
  <div>
    <span class=metric>Pixels:</span>
    <span>${ele._right.numDiffPixels}</span>
  </div>
  <div>
    <span class=metric>Max RGBA:</span>
    <span>[${ele._right.maxRGBADiffs.join(',')}]</span>
  </div>
  <triage-sk @change=${ele._triageChangeHandler} .value=${ele._status}></triage-sk>
  <triage-history-sk .history=${ele._triageHistory}></triage-history-sk>
</div>`;
...
const imageComparison = (ele) => {
  const left = {
    src: imgSrc(ele._digest),
    title: shorten(ele._digest),
    detail: detailHref(ele._grouping, ele._digest, ele._issue),
  };
  ...
  const diff = {
    src: diffImgSrc(ele._digest, ele._right.digest),
  };
  const right = {
    src: imgSrc(ele._right.digest),
    title: ele._rightRef === 'pos' ? 'Closest Positive' : 'Closest Negative',
    detail: detailHref(ele._grouping, ele._right.digest, ele._issue),
  };
  return html`<image-compare-sk .left=${left} .diff=${diff} .right=${right}></image-compare-sk>`;
};
