...
<div class=flex_vertical>
  <div class=flex_horizontal>
    ...
    <span class=expand></span>
    <span class=test_name>Right: ${ele._rightRef && ele._refDiffs[ele._rightRef].digest}</span>
  </div>
  <div class=flex_horizontal>
    <div>${detailsAndTriage(ele)}</div>
    <div>
      <image-compare-sk .left=${ele._leftImageHref()} .diff=${ele._diffImageHref()} 
            .right=${ele._rightImageHref()}>
      </image-compare-sk></div>
...
<div class="flex_vertical metrics">
  <div>Diff metric: ${right.diffs.combined.toFixed(3)}</div>
  <div>Diff %: ${(right.pixelDiffPercent * 100).toFixed(2)}</div>
  <div>Pixels: ${right.numDiffPixels}</div>
  <div>Max RGBA: ${right.maxRGBADiffs.join(',')}</div>
