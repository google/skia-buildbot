...
export class ChangelistControlsSkPO extends PageObject {
  ...
  isIncludeResultsFromPrimaryRadioChecked() {
    return this.selectOneDOMNodeThenApplyFn(
      '.inputs radio-sk:nth-child(1)', (el) => (el as CheckOrRadio).checked);
  }

  async clickIncludeResultsFromPrimaryRadio() {
    await this.selectOnePOEThenApplyFn('.inputs radio-sk:nth-child(1)', (el) => el.click());
  }

  isShowAllResultsRadioChecked() {
    return this.selectOneDOMNodeThenApplyFn(
      '.inputs radio-sk:nth-child(2)', (el) => (el as CheckOrRadio).checked);
  }

  async clickShowAllResultsRadio() {
    await this.selectOnePOEThenApplyFn('.inputs radio-sk:nth-child(2)', (el) => el.click());
  }
