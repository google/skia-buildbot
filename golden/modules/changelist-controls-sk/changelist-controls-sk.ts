...
export class ChangelistControlsSk extends ElementSk {
      ...
      <div class=inputs>
        ...
        <div class=radiogroup>
          <radio-sk label="exclude results from master"
                    ...
                    @change=${() => ele._onIncludeMasterChange(false)}>
          </radio-sk>
          <radio-sk label="show all results"
                    name=include_master
                    ?checked=${ele.include_master}
                    @change=${() => ele._onIncludeMasterChange(true)}>
          </radio-sk>
        </div>
      </div>
  ...
  /** The order of the patchset currently being shown. */
  ...
  /** Whether to show results that are also on master, as opposed to those that are exclusive. */
  get include_master() { return this._includeMaster; }

  set include_master(val) {
    this._includeMaster = (val as unknown as string) !== 'false' && !!val;
    this._render();
  }

  _onIncludeMasterChange(newVal: boolean) {
