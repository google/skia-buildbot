...
describe('list-page-sk', () => {
  ...
  describe('html layout', () => {
    ...
    it('should have links for searching and the cluster view', () => {
      ...
      expect(links[0].href).to.contain(
        '/search?unt=true&neg=true&pos=true&source_type=gm&query=name%3Dthis_is_another_test&head=true&include=false',
      );
      // Second link should be to cluster view (with a very similar href)
      expect(links[1].href).to.contain(
        '/cluster?unt=true&neg=true&pos=true&source_type=gm&query=name%3Dthis_is_another_test&head=true&include=false',
      );
    });
    ...
    it('updates the links based on toggle positions', () => {
      ...
      // First link should be to the search results
      expect(links[0].href).to.contain(
        '/search?unt=true&neg=true&pos=true&source_type=gm&query=name%3Dthis_is_another_test&head=false&include=true',
      );
      // Second link should be to cluster view (with a very similar href)
      expect(links[1].href).to.contain(
        '/cluster?unt=true&neg=true&pos=true&source_type=gm&query=name%3Dthis_is_another_test&head=false&include=true',
      );
    });
