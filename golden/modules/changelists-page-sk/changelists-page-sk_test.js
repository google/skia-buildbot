...
describe('changelists-page-sk', () => {
  ...
  function createElement(test) {
    const c = window.customElements.whenDefined('changelists-page-sk');
    const g =  window.customElements.whenDefined('gold-scaffold-sk');
    return Promise.all([c, g]).then(() => {
      container.innerHTML = `
      <gold-scaffold-sk>
  ...
  function whenPageLoads(test) {
    ...
    const fn = (e) => {
      // reset for next time
      container.removeEventListener('busy-end', fn);
      if (!ran) {
    ...
    container.addEventListener('busy-end', fn);
    createElement((e) => {
      ele = e;
    });
  }
