...
export class TraceFilterSkPO extends PageObject {
  getParamSetSkPO() {
    return this.$$apply('.selection paramset-sk', async (el) => new ParamSetSkPO(el));
  }

  getQueryDialogSkPO() {
    return this.$$apply('query-dialog-sk', async (el) => new QueryDialogSkPO(el));
  }
  ...
  async isEmptyFilterMessageVisible() {
    return (await this.$$('.selection .empty-placeholder')) !== null;
  }

  async isParamSetSkVisible() {
    return (await this.$$('.selection paramset-sk')) !== null;
  }

  async clickEditBtn() {
    return this.$$apply('.edit-query', (btn) => btn.click());
  }
