...
<dialog id=mutate>
  ...
  <button id=ok @click=${ele._commitMutation}>${ele._mutateID ? 'Update': 'Create'}</button>
</dialog>
...
define('ignores-page-sk', class extends ElementSk {
  ...
  _commitMutation() {
    ...
    if (editor.verifyFields()) {
      ...
      let did = 'created';
      if (this._mutateID) {
        url = `/json/ignores/save/${this._mutateID}`;
        did = 'updated';
      }
      ...
      fetch(url, {
        ...
        errorMessage(`Success! Ignore rule ${did}.`);
        this._fetch();
        this._sendDone();
  ...
  _delete(rule) {
    ...
    dialog.open('Are you sure you want to delete '+
      ...
      fetch('/json/ignores/del/'+rule.id, {
        ...
        errorMessage('Success! Ignore rule deleted.');
        this._fetch();
        this._sendDone();
