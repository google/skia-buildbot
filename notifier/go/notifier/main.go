...
func emailHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(to) == 0 {
		httputils.ReportError(w, fmt.Errorf("Missing email addresses in URL: %q", r.RequestURI), "Email addresses missing.")
		return
	}
	body, subject, err := alertmanager.Email(r.Body)
	if err != nil {
		httputils.ReportError(w, err, "Failed to generate an outgoing email.")
		return
	}

	if err := emailAuth.Send(FROM_ADDRESS, to, subject, body); err != nil {
		httputils.ReportError(w, err, "Failed to send outgoing email.")
		return
	}
}
...
func chatHandler(w http.ResponseWriter, r *http.Request) {
	...
	if to == "" {
		httputils.ReportError(w, fmt.Errorf("Missing room in URL: %q", r.RequestURI), "Chat room name missing.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to generate an outgoing chat.")
		return
	}
	...
	if err := chatbot.SendUsingConfig(body, to, threadId, chatBotConfigReader); err != nil {
		httputils.ReportError(w, err, "Failed to send outgoing chat.")
		return
	}
}
