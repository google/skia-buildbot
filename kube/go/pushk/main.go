...
func switchTo(ctx context.Context, clusterName string) error {
	...
	if config.GetString(fmt.Sprintf("clusters.%s.type", clusterName)) != "gke" {
		// TODO(jcgregorio) - Add support for k3s clusters.
		return fmt.Errorf("Unknown type of cluster.")
	}
	zone := config.GetString(fmt.Sprintf("clusters.%s.zone", clusterName))
	project := config.GetString(fmt.Sprintf("clusters.%s.project", clusterName))
