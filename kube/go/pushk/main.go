...
This will set the %s_CONFIG environment variable to point
to the checkout.
...
`, *project, *project, *project, upper(*project))
		flag.PrintDefaults()
	}
}
...
func main() {
	common.Init()
	repoDirEnv := fmt.Sprintf(REPO_DIR_ENV, upper(*project))
	...
	if len(changed) != 0 {
		...
		if err != nil {
			sklog.Fatalf("Failed to stage changed to the config repo: %s: %q", err, msg)
		}
