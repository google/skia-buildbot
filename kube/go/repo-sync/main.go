...
var (
	...
	source      = flag.String("source", "https://skia.googlesource.com/skia", "URL of the source repo.")
...
func main() {
	...
	if _, err := os.Stat(repoDir); os.IsNotExist(err) {
		...
		err := exec.Run(ctx, &exec.Command{
			Name:           "gcloud",
			Args:           []string{"source", "repos", "clone", *destination},
			Dir:            *workDir,
	...
	for _ = range time.Tick(*period) {
		...
		if err != nil {
			...
			sklog.Errorf("Failed to pull: %s", err)
			continue
		}
		...
		if err != nil {
			sklog.Errorf("git output: %q", out)
			sklog.Errorf("Failed to push: %s", err)
		}
