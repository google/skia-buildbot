...
func main() {
	...
	if !*local {
		...
		out, err := exec.RunSimple(ctx, "git config --global user.email \"skia-continuous-deploy@skia-public.iam.gserviceaccount.com\"")
		if err != nil {
			sklog.Fatal(err, out)
		}
		out, err = exec.RunSimple(ctx, "git config --global user.name \"skia-continuous-deploy\"")
		if err != nil {
			sklog.Fatal(err, out)
		}
