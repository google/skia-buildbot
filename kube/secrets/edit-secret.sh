...
FILES=$(${REL}/get-secret.sh ${CLUSTER} ${SECRET_NAME} | kubectl apply -f - --dry-run -o json | jq -j '.data | keys | .[] | . + " "')
echo ${FILES}
for FILE in ${FILES}; do
  ${REL}/get-secret.sh ${CLUSTER} ${SECRET_NAME} | kubectl apply -f - --dry-run -o json | jq -r ".data[\"${FILE}\"]" | base64 -d > /tmp/ramdisk/${FILE}
done
