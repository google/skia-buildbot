...
func runUntilExit(t *testing.T, f func()) {
	assert.Panics(t, f, "panicing signifies an exit")
}

type exitCodeCapturer struct {
	code      int
	wasCalled bool
}

func (e *exitCodeCapturer) exitWithCode(code int) {
	e.code = code
	e.wasCalled = true
	panic("exited")
}

func (e *exitCodeCapturer) AssertWasCalledWithCode(t *testing.T, code int) {
