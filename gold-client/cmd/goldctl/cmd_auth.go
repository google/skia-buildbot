...
func getAuthCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Authenticate against GCP and Gold instance",
		Long: `
...
Currently only service accounts are supported. `,
	...
	cmd.Flags().StringVarP(&env.flagWorkDir, "work-dir", "", "", "Temporary work directory")
...
func (a *authEnv) runAuthCmd(cmd *cobra.Command, args []string) {
	...
	goldClient, err := goldclient.NewCloudClient(a.flagWorkDir, "", false, nil)
	ifErrLogExit(cmd, err)

	err = goldClient.Auth(a.flagServiceAccount)
	ifErrLogExit(cmd, err)
