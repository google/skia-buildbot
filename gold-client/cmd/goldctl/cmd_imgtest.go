...
type imgTest struct {
	...
	testKeysFile    string   // A file with test-specific keys encoded as a JSON dictionary.
	testKeysStrings []string // Test-specific keys represented as key:value pairs.

	testOptionalKeysFile    string   // A file with test-specific optional keys encoded as a JSON dictionary.
...
`,
	...
	imgTestAddCmd.Flags().StringVar(&env.testKeysFile, "add-test-key-file", "", "A file with test-specific keys encoded as a JSON dictionary.")
	imgTestAddCmd.Flags().StringSliceVar(&env.testKeysStrings, "add-test-key", []string{}, "Any number of test-specific keys represented as key:value pairs.")
	imgTestAddCmd.Flags().StringVar(&env.testKeysFile, "add-test-optional-key-file", "", "A file with test-specific optional keys encoded as a JSON dictionary.")
	imgTestAddCmd.Flags().StringSliceVar(&env.testKeysStrings, "add-test-optional-key", []string{}, "Any number of test-specific optional keys represented as key:value pairs.")
...
func (i *imgTest) runImgTestAddCmd(cmd *cobra.Command, args []string) {
	...
	extraKeys := readKeyValuePairsFromFileOrKeyValueStrings(cmd, i.testKeysFile, i.testKeysStrings)
	if i.corpus != "" {
	...
	pass, err := goldClient.Test(types.TestName(i.testName), i.pngFile, extraKeys, extraOptions)
	ifErrLogExit(cmd, err)
...
// readKeyValuePairsFromFileOrKeyValueStrings reads key/value pairs encoded as a JSON literal from
// the given filename, or from the given slice of key:value strings if the filename is empty.
func readKeyValuePairsFromFileOrKeyValueStrings(cmd *cobra.Command, filename string, keyValueStrings []string) map[string]string {
	...
	if filename != "" {
		...
		if err = json.Unmarshal(j, &retval); err != nil {
