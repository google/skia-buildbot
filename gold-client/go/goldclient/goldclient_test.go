...
func TestCloudClient_MatchImageAgainstBaseline_FuzzyMatching_NoRecentPositiveDigests_Success(t *testing.T) {
	unittest.MediumTest(t) // This test reads/writes a small amount of data from/to disk.
	...
	got, algorithmName, err := goldClient.matchImageAgainstBaseline(testName, traceId, imageBytes, digest, optionalKeys)
	assert.NoError(t, err)
	assert.False(t, got)
	assert.Equal(t, imgmatching.FuzzyMatching, algorithmName)
}
