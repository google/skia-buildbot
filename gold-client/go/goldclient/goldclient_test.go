...
func TestLoadKnownHashes(t *testing.T) {
	...
	err = goldClient.SetSharedConfig(defaultSharedConfig)
	assert.NoError(t, err)
...
func TestInit(t *testing.T) {
	...
	assert.Equal(t, defaultSharedConfig, *state.SharedConfig)
}
...
func TestFinalizeNormal(t *testing.T) {
	...
	// Se don't need to call SetSharedConfig because the state should be
	// loaded from disk
...
var defaultSharedConfig = jsonio.GoldResults{
	GitHash: "abcd1234",
	Key: map[string]string{
		"os":  "WinTest",
		"gpu": "GPUTest",
