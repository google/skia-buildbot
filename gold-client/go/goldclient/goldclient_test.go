...
func TestDiff(t *testing.T) {
	...
	testName := types.TestName("ThisIsTheOnlyTest")
	const leftHash = "aaa6b72edc996515916348e8f4dd406d"
	const rightHash = "bbb"
	...
	err = goldClient.Diff(testName, testImgPath, outDir)
	require.NoError(t, err)

	leftImg, err := openNRGBAFromFile(filepath.Join(wd, "out", leftHash+".png"))
	require.NoError(t, err)
	assert.Equal(t, leftImg, image1)

	rightImg, err := openNRGBAFromFile(filepath.Join(wd, "out", "closest-"+rightHash+".png"))
	require.NoError(t, err)
	assert.Equal(t, rightImg, image2)

	diffImg, err := openNRGBAFromFile(filepath.Join(wd, "out", "diff.png"))
	require.NoError(t, err)
