...
func TestLoadKnownHashes(t *testing.T) {
	...
	hashesResp := httpResponse([]byte(HASHES_MOCK_TXT), "200 OK", http.StatusOK)
...
func TestLoadBaseline(t *testing.T) {
	...
	expectations := httpResponse([]byte(BASELINE_MOCK_JSON), "200 OK", http.StatusOK)
	httpClient.On("Get", "https://testing-gold.skia.org/json/expectations/commit/abcd1234?issue=867").Return(expectations, nil)
...
func TestInitGSUtil(t *testing.T) {
	...
	wd, cleanup := testutils.TempDir(t)
	defer cleanup()
	auth, err := InitGSUtil(wd)
	assert.NoError(t, err)
	assert.True(t, auth.GSUtil)
	assert.NoError(t, auth.Validate())
	// ensure the file exists
	assert.True(t, fileutil.FileExists(filepath.Join(wd, AUTH_FILE)))
...
func TestInitServiceAccountAuth(t *testing.T) {
	...
	auth, err := InitServiceAccountAuth(serviceLoc, wd)
	assert.NoError(t, err)
...
const HASHES_MOCK_TXT = `a9e1481ebc45c1c4f6720d1119644c20
