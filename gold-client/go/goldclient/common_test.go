...
func TestPostWithRetries_OneAttempt_Success(t *testing.T) {
	unittest.SmallTest(t)
	...
	mh.On("Post", url, contentType, body).Return(httpResponse([]byte("Hello, world!"), "200 OK", http.StatusOK), nil).Once()

	b, err := postWithRetries(mh, url, contentType, body)
	assert.NoError(t, err)
	assert.Equal(t, []byte("Hello, world!"), b)
}

func TestPostWithRetries_MultipleAttempts_Success(t *testing.T) {
	unittest.LargeTest(t) // Function under test sleeps for several milliseconds before retries.
	...
	mh.On("Post", url, contentType, body).Return(nil, errors.New("http.Client error")).Once()
	mh.On("Post", url, contentType, body).Return(httpResponse([]byte("Should be ignored."), "500 Internal Server Error", http.StatusInternalServerError), nil).Once()
	mh.On("Post", url, contentType, body).Return(httpResponse([]byte("Hello, world!"), "200 OK", http.StatusOK), nil).Once()
...
func TestPostWithRetries_MultipleAttempts_Error(t *testing.T) {
	unittest.LargeTest(t) // Function under test sleeps for several milliseconds before retries.
	...
	mh.On("Post", url, contentType, body).Return(httpResponse([]byte("Should be ignored."), "404 Not found", http.StatusNotFound), nil).Times(5)

	_, err := postWithRetries(mh, url, contentType, body)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "404")
}
