...
func (g gsutilUploader) uploadJson(data interface{}, tempFileName, gcsObjPath string) error {
	...
	return g.copy(tempFileName, prefixGCS(gcsObjPath))
}
...
func (g gsutilUploader) copy(src, dst string) error {
...
func newHttpUploader(httpClient *http.Client) (cloudUploader, error) {
	ret := &httpUploader{}
	var err error
	ret.client, err = gstorage.NewClient(context.TODO(), option.WithHTTPClient(httpClient))
	if err != nil {
...
func (h *httpUploader) copy(src, dst string) error {
	if strings.HasPrefix(src, gsPrefix) {
		return skerr.Fmt("Copying to a local file is not supported")
	}

	content, err := ioutil.ReadFile(src)
	if err != nil {
		return skerr.Fmt("Error reading file %s: %s", src, err)
	}

	return h.copyBytes(content, dst)
}
...
func (h *httpUploader) copyBytes(data []byte, dst string) error {
	...
	return gcs.WriteObj(handle, data)
}
