...
func (c *CloudClient) Diff(name types.TestName, imgFileName, outDir string) error {
	...
	u := fmt.Sprintf("/json/digests?test=%s", name)
	jb, err := getWithRetries(c.httpClient, u)
	if err != nil {
		return skerr.Wrapf(err, "reading images for test %s from gold (url: %s)", name, u)
	}
	...
	if len(dlr.Digests) == 0 {
		sklog.Warningf("Gold doesn't know of any digests that match %s", name)
		return nil
	}
	...
	for _, d := range dlr.Digests {
		db, err := c.downloadDigestToDir(d, digestsPath)
		if err != nil {
			return skerr.Wrap(err)
		}
		dr := bytes.NewBuffer(db)
		rightImg, err := png.Decode(dr)
		if err != nil {
			return skerr.Wrapf(err, "Invalid PNG stored in digest %s", d)
		}

		dm, diffImg := diff.PixelDiff(leftImg, rightImg)
		cdm := diff.CombinedDiffMetric(dm, nil, nil)
		sklog.Infof("combined diff of %s: %#v", d, cdm)
	...
	if err := ioutil.WriteFile(o, closestRightImg, 0644); err != nil {
		return skerr.Wrapf(err, "writing to %s", o)
	}
	...
	return diffFile.Close()
}

func (c *CloudClient) downloadDigestToDir(d types.Digest, path string) ([]byte, error) {
	p := filepath.Join(path, string(d)+".png")
	...
	dst, err := os.Create(p)
	if err != nil {
		return nil, skerr.Wrapf(err, "creating %s for writing", p)
	}
	// Don't defer a util.Close(dst) because if we have trouble writing to the file, we sometimes
	// see those problems when the Close() triggers, and don't want to mask that error.
	...
	rc, err := dl.Download(context.TODO(), img)
	if err != nil {
		return nil, skerr.Wrapf(err, "downloading %s", img)
	}
	defer util.Close(rc)
	var buf bytes.Buffer
	w := io.MultiWriter(dst, &buf)

	if _, err := io.Copy(w, rc); err != nil {
		err2 := dst.Close()
		return nil, skerr.Wrapf(err, "copying from GCS (dst Close err: %v)", err2)
	}
	return buf.Bytes(), dst.Close()
}
