...
func (c *CloudClient) MostRecentPositiveDigest(traceId tiling.TraceID) (types.Digest, error) {
	jsonBytes, err := getWithRetries(c.httpClient, c.resultState.GoldURL+"/json/latestpositivedigest/"+string(traceId))
	if err != nil {
		return "", skerr.Wrapf(err, "making request to %s/json/latestpositivedigest/%s", c.resultState.GoldURL, traceId)
	}

	mostRecentPositiveDigest := map[string]string{}
	if err := json.Unmarshal(jsonBytes, &mostRecentPositiveDigest); err != nil {
		return "", skerr.Wrapf(err, "parsing JSON response from %s/json/latestpositivedigest/%s", c.resultState.GoldURL, traceId)
	}

	digest, ok := mostRecentPositiveDigest["digest"]
	if !ok {
		return "", skerr.Wrapf(err, `JSON response from %s/json/latestpositivedigest/%s does not contain key "digest"`, c.resultState.GoldURL, traceId)
	}
