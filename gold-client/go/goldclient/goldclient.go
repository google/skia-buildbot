...
const (
	// JSON_PREFIX is the path prefix in the GCS bucket that holds JSON result files
	JSON_PREFIX = "dm-json-v1"

	// IMAGE_PREFIX is the path prefix in the GCS bucket that holds images.
	IMAGE_PREFIX = "dm-images-v1"

	// KNOWN_HASHES_PATH is path on the Gold instance to retrieve the known image hashes that do
	// not need to be uploaded anymore.
	KNOWN_HASHES_PATH = "json/hashes"

	// STATE_FILE is the name of the file that holds the state in the work directory
	// between calls
	STATE_FILE = "result-state.json"

	// JSON_TEMP_FILEName is the temporary file that is created to upload results via gsutil.
	JSON_TEMP_FILE = "gsutil_dm.json"

	// GOLD_HOST_TEMPLATE constructs the URL of the Gold instance from the instance id
	GOLD_HOST_TEMPLATE = "https://%s-gold.skia.org"

	// BUCKET_TEMPLATE constructs the name of the ingestion bucket from the instance id
	BUCKET_TEMPLATE = "skia-gold-%s"
)
...
// TESTING_overrideLoadAndHashImage allows overwriting the "LoadAndHashImage"
// function.  This should only be used in unit tests when we don't actually want
// to read/write from/to disk.
func (c *cloudClient) TESTING_overrideLoadAndHashImage(fn func(path string) ([]byte, string, error)) {
	c.loadAndHashImage = fn
}
...
func (c *cloudClient) addTest(name string, imgFileName string) (bool, error) {
	...
	if c.resultState.PerTestPassFail {
		egroup.Go(func() error {
			localFileName := filepath.Join(c.workDir, JSON_TEMP_FILE)
...
func (c *cloudClient) saveAuthOpt() error {
	outFile := filepath.Join(c.workDir, AUTH_FILE)
...
func (c *cloudClient) getResultStateFile() string {
	return filepath.Join(c.workDir, STATE_FILE)
}
...
func (r *resultState) loadKnownHashes() error {
	...
	hashesURL := fmt.Sprintf("%s/%s", r.GoldURL, KNOWN_HASHES_PATH)
...
func (r *resultState) getGCSImagePath(imgHash string) string {
	return fmt.Sprintf("%s/%s/%s.png", r.Bucket, IMAGE_PREFIX, imgHash)
}
...
const (
	...
	// to match the rest.
	BUCKET_SKIA_LEGACY      = "skia-infra-gm"
	HOST_SKIA_LEGACY        = "https://gold.skia.org"
	INSTANCE_ID_SKIA_LEGACY = "skia-legacy"
)

func getBucket(instanceID string) string {
	if instanceID == INSTANCE_ID_SKIA_LEGACY {
		return BUCKET_SKIA_LEGACY
	}
	return fmt.Sprintf(BUCKET_TEMPLATE, instanceID)
}

func getHostURL(instanceID string) string {
	if instanceID == INSTANCE_ID_SKIA_LEGACY {
		return HOST_SKIA_LEGACY
	}
	return fmt.Sprintf(GOLD_HOST_TEMPLATE, instanceID)
}
