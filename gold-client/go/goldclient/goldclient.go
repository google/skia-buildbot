...
func (c *CloudClient) Diff(ctx context.Context, name types.TestName, corpus, imgFileName, outDir string) error {
	...
	for _, d := range dlr.Digests {
		db, err := c.downloadDigestToDir(ctx, d, digestsPath)
		if err != nil {
...
func (c *CloudClient) downloadDigestToDir(ctx context.Context, d types.Digest, path string) ([]byte, error) {
