...
func loadAndHashImage(fileName string) ([]byte, types.Digest, error) {
	...
	var imgBytes []byte
	var img image.Image
	if err := util.WithReadFile(fileName, func(r io.Reader) error {
		var err error
		imgBytes, err = ioutil.ReadAll(r)
		if err != nil {
			return skerr.Wrapf(err, "loading file %s", fileName)
		}

		img, err = png.Decode(bytes.NewBuffer(imgBytes))
		if err != nil {
			return skerr.Wrapf(err, "decoding PNG in file %s", fileName)
		}
		return nil
	}); err != nil {
