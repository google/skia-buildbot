...
type GoldClient interface {
	...
	// TriageAsPositive triages the given digest for the given test and optional changelist ID as positive
	// by making a request to Gold's /json/triage endpoint.
...
func (c *CloudClient) TriageAsPositive(testName types.TestName, digest types.Digest, changeListId string) error {
	...
	triageRequest := &frontend.TriageRequest{
		TestDigestStatus: map[types.TestName]map[types.Digest]string{testName: {digest: "positive"}},
		ChangeListID:     changeListId,
	}
	jsonTriageRequest, err := json.Marshal(triageRequest)
	if err != nil {
		return skerr.Wrapf(err, `encoding frontend.TriageRequest into JSON for test "%s", digest "%s" and issue "%s"`, testName, digest, changeListId)
	}

	// Make /json/triage request. Response is always empty.
	_, err = postWithRetries(c.httpClient, c.resultState.GoldURL+"/json/triage", "application/json", bytes.NewReader(jsonTriageRequest))
	if err != nil {
		return skerr.Wrapf(err, `making POST request to %s/json/triage for test "%s", digest "%s" and issue "%s"`, c.resultState.GoldURL, testName, digest, changeListId)
	}
