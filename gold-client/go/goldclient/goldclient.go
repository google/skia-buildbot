...
// cloudUploader implementations provide funtions to upload to GCS.
type cloudUploader interface {
	...
	// An implemenation can use tempFileName for temporary storage of JSON data.
...
func (c *cloudClient) getUploader() (cloudUploader, error) {
	if c.oauthToken != nil {
		return newHttpUploader(c.httpClient)
	}
...
func (c *cloudClient) addTest(name string, imgFileName string) (bool, error) {
	...
	if err != nil {
		skerr.Fmt("Error retrieving uploader: %s", err)
	}
	...
	if !c.resultState.KnownHashes[imgHash] {
		...
		if err := uploader.copy(imgFileName, prefixGCS(gcsImagePath)); err != nil {
...
// SetServiceAccount loads a service account from the given file and uses it for requests to
