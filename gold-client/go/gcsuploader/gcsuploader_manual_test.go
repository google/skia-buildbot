...
func TestClientImpl(t *testing.T) {
	...
	t.Run("UploadToGCS_PermanentError_RetriesAndThenErrors", testUploadToGCS_PermanentError_RetriesAndThenErrors(c, randomSubfolder))
}

func setup(ctx context.Context, t *testing.T, c *gstorage.Client) string {
...
func testUploadToGCS_PermanentError_RetriesAndThenErrors(c *clientImpl, subfolder string) func(t *testing.T) {
	return func(t *testing.T) {
		require.Error(t, c.uploadToGCS(context.Background(), []byte("overwrite the data!"), "skia-this-will-permanently-fail/file.json"))
	}
}

func readGCSFile(ctx context.Context, t *testing.T, c *gstorage.Client, file string) []byte {
