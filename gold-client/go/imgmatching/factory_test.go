...
// commonFuzzyMatchingTestCases returns test cases shared between
// TestMatcherFactoryImpl_Make_FuzzyMatching and TestMatcherFactoryImpl_Make_SobelFuzzyMatching.
//
// The reason is that sobel.SobelFuzzyMatcher embeds fuzzy.FuzzyMatcher, thus we must test that the
// fuzzy.FuzzyMatcher parameters are correctly validated when either algorithm is specified.
func commonFuzzyMatchingTestCases() []fuzzyMatchingTestCase {
...
func TestMatcherFactoryImpl_Make_FuzzyMatching(t *testing.T) {
	...
	tests = append(tests, commonFuzzyMatchingTestCases()...)
...
func TestMatcherFactoryImpl_Make_SobelFuzzyMatching(t *testing.T) {
	...
	for _, tc := range commonFuzzyMatchingTestCases() {
