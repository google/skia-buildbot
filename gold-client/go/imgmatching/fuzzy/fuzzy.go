...
const (
	// MaxDifferentPixels is the optional key used to specify the maximum allowed number of different
	// pixels.
	MaxDifferentPixels = imgmatching.AlgorithmParamOptKey("fuzzy_max_different_pixels")

	// PixelDeltaThreshold is the optional key used to specify the maximum allowed per-pixel
	// dR + dG + dB + dA sum, where d{R,G,B,A} are the per-channel deltas.
	PixelDeltaThreshold = imgmatching.AlgorithmParamOptKey("fuzzy_pixel_delta_threshold")
)
