...
func TestMatcher_Match_Success(t *testing.T) {
	...
	for _, tc := range makeMatcherTestCases() {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertImagesEqualWithMessage(t, tc.expectedImage1WithEdgesRemoved, matcher.Image1WithEdgesRemoved(), "image1 with edges removed")
			assertImagesEqualWithMessage(t, tc.expectedImage2WithEdgesRemoved, matcher.Image2WithEdgesRemoved(), "image2 with edges removed")
...
// assertImagesEqual calls assertImagesEqualWithMessage with an empty message.
...
// expected and actual images encoded as SKTEXT if the assertion is false.
