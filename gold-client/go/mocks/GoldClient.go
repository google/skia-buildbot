...
func (_m *GoldClient) TriageAsPositive(testName types.TestName, digest types.Digest, algorithmName string, changeListId string) error {
	ret := _m.Called(testName, digest, algorithmName, changeListId)

	var r0 error
	if rf, ok := ret.Get(0).(func(types.TestName, types.Digest, string, string) error); ok {
