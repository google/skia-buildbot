...
for page_set in /b/storage/page_sets/$PAGESETS_TYPE/*.py; do
  if [[ -f $page_set ]]; then
    ...
    if [ "$PAGESETS_TYPE" == "Filtered" ]; then
      ...
      check_and_run_xvfb
      sudo DISPLAY=:0 src/tools/perf/record_wpr --extra-browser-args=--disable-setuid-sandbox --browser-executable=/b/storage/chromium-builds/${CHROMIUM_BUILD_DIR}/chrome --browser=exact $page_set
      if [ $? -eq 124 ]; then
