...
export function stateReflector(stateHolder, name, cb) {
  ...
  // The last state of the stateHolder. Used to determine if the stateHolder[name] has changed recently.
  let lastState = object.deepCopy(stateHolder[name]);
  ...
  const stateFromURL = () => {
    ...
    lastState = object.deepCopy(stateHolder[name]);
    cb();
  }
  ...
  return () => {
    if (Object.keys(object.getDelta(lastState, stateHolder[name])).length > 0) {
      lastState = object.deepCopy(stateHolder[name]);
