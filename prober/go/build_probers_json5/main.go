...
func main() {
	...
	// Read each, adding elements to a global map.
	// Serialize to *dest.
	files := []string{}
	filepath.Walk(*srcdir, func(path string, info os.FileInfo, err error) error {
		...
		sklog.Infoln(path, info.IsDir(), len(strings.Split(path, string(filepath.Separator))), *depth)
	...
	if err := json.NewEncoder(file).Encode(allProbes); err != nil {
