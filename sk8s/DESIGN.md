...
## Launch plans
  * Land new version of metadata that runs under kubernetes.
  ...
  * Migrate rest of legacy apps to kubernetes
    ...
    * power-cycle-daemon
    * NFS server? (or alternatively migrate all RPis to the kubernetes managed
      racks and stop using NFS).
...
safely make each rack a single kubernetes cluster.
