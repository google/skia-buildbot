...
the machine any time it wants. Since we are running in a container this is a
useless feature and we take that away swarming by adding a `sudo` script on the
PATH, which is just a simple bash script that `kill -9`s any process that runs
the script. Since the image runs as root a user that `kubectl exec`s into a running
image doesn't need to use sudo.

**qemu-arm-static**

This file allows the image to be run on x86 machine, which will then emulate the
arm7 processor, allowing the build process to be run on an x86 desktop and the
resulting image to also be run on an x86 desktop as well as an arm7 device. Note
that anything to do with running adb will probably not work on an x86 machine.
