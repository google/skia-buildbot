...
// SetIsRunningTask records if a swarming task is being run.
func (m *Machine) SetIsRunningTask(isRunning bool) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	m.runningTask = isRunning
}

// IsRunningTask returns true is a swarming task is currently running.
func (m *Machine) IsRunningTask() bool {
	m.mutex.Lock()
	defer m.mutex.Unlock()
