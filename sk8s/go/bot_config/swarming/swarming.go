...
func New(pythonExe, swarmingBotZip string) *Bot {
	...
	if strings.HasPrefix(host, "skia-i-") {
		swarmingURL = "https://chrome-swarming.appspot.com"
	...
	return &Bot{
		...
		metadataURL:    "http://metadata:8000/computeMetadata/v1/instance/service-accounts/default/token",
	}
}
...
func (b *Bot) bootstrap(ctx context.Context) error {
	...
	if _, err := os.Stat(downloadDir); os.IsNotExist(err) {
		if err := os.MkdirAll(downloadDir, 0777); err != nil {
			return skerr.Wrapf(err, "Failed to create download directory %q: %s", downloadDir)
		}
	}
...
func (b *Bot) Launch(ctx context.Context) error {
	...
	for {
		cmd := exec.CommandContext(ctx, b.pythonExe, b.swarmingBotZip, "start_bot")
