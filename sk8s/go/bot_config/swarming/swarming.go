...
type Bot struct {
	swarmingBotZip string
	pythonExe      string
}

// pythonExe and swarmingBotZip must be absolute file locations.
func New(pythonExe, swarmingBotZip string) *Bot {
	return &Bot{
		swarmingBotZip: swarmingBotZip,
		pythonExe:      pythonExe,
	}
}
...
func (b *Bot) Bootstrap(ctx context.Context) error {
	...
	} else if strings.HasPrefix("skia-d-") {
	...
	downloadDir := filepath.Dir(b.swarmingBotZip)
	if err != nil {
		return skerr.Wrapf(err, "Failed to create location to download bootstrap code.")
	}
	if _, err := os.Stat(downloadDir); os.IsNotExist(err) {
		if err := os.MkdirAll(downloadDir, 0777); err != nil {
			return skerr.Wrapf("Failed to create download directory %q: %s", downloadDir, err)
		}
	}

	// Open the file for writing.
	f, err := os.OpenFile(b.swarmingBotZip, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	if err != nil {
		return skerr.Wrapf(err, "Failed to open swarming bot code file %q", b.swarmingBotZip)
	}
	defer util.Close(f)


	// Request the service account token from the metadata server.
	req, err := http.NewRequest("GET", "http://metadata:8000/computeMetadata/v1/instance/service-accounts/default/token", nil)
	if err != nil {
	...
	req, err = http.NewRequest("GET", swarming, nil)
	if err != nil {
	...
	resp, err = client.Do(req)
	if err != nil {
		return skerr.Wrapf("Failed to make request for swarming bot code to %q", swarming)
	}
	defer util.Close(resp.Body)
...
func (b *Bot) Launch(ctx context.Context) error {
	if _, err := os.Stat(b.swarmingBotZip); os.IsNotExist(err) {
		if err := Bootstrap(ctx); err != nil {
			return err
		}
	}
	for {
		cmd := exec.CommandContext(ctx, pythonExe, swarmingBotZip, "start_bot")
