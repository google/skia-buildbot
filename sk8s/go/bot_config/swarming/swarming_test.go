...
func TestRunSwarmingCommand_ReturnsNilOnExitCodeZero(t *testing.T) {
	...
	// this programs os.Stderr output.
	...
	require.NoError(t, err)
	assert.Contains(t, string(b), `This is a line of logging output.`) // See TestFakeSwarmingExecutable_ExitCodeZero
}
...
func TestFakeSwarmingExecutable_ExitCodeZero(t *testing.T) {
	...
	fmt.Fprintf(os.Stderr, "This is a line of logging output.\n")
	os.Exit(0)
}
...
func TestRunSwarmingCommand_ReturnsErrorOnNonZeroExitCode(t *testing.T) {
	...
	err = bot.runSwarmingCommand(ctx)
	assert.Contains(t, err.Error(), "exit status 17") // See the exit code in TestFakeSwarmingExecutable_ExitCodeNonZero
}
...
func TestLaunch_IsCancellable(t *testing.T) {
	...
	// Call bootstrap, so when we do call Launch() bootstrap won't need to be
	// called and we'll fall directly into the for {} loop.
	err := bot.bootstrap(context.Background())
	require.NoError(t, err)
