...
var (
	...
	proplines = regexp.MustCompile(`(?m)^\[(.+)\]:\s*\[(.*)\].*$`)
)
...
func Properties(ctx context.Context) (map[string]string, error) {
	...
	cmd := exec.Command{
		Name: "adb",
		// We specify -H so if the adb server isn't running this call to adb won't
		// try to start a local server.
		Args:    []string{"-H", "127.0.0.1", "shell", "getprop"},
		Stdout:  stdout,
		Stderr:  stderr,
...
var (
	...
	packageVersionRe = regexp.MustCompile(`(?m)^\s*versionName=([^\s]+)`)
)
...
func packageVersion(ctx context.Context, errout io.Writer, pkg string) []string {
	...
	cmd := exec.Command{
		...
		// We specify -H so if the adb server isn't running this call to adb won't
		// try to start a local server.
		Args:    []string{"-H", "127.0.0.1", "shell", "dumpsys", "package", pkg},
		Stdout:  stdout,
		Stderr:  stderr,
		Timeout: 5 * time.Second,
	}
	if err := exec.Run(ctx, &cmd); err != nil {
		fmt.Fprintf(errout, "Error: Failed to run adb dumpsys package %q: %s", stderr.String(), err)
	}
...
func DimensionsFromProperties(ctx context.Context, errout io.Writer, dim map[string][]string) map[string][]string {
	fmt.Fprintf(errout, "Running DimensionsFromProperties")
	prop, err := Properties(ctx)
	if err != nil {
		fmt.Fprintf(errout, "Error: Failed to get properties: %s\n", err)
		return dim
	}
	...
	dim["device_gms_core_version"] = packageVersion(ctx, errout, "com.google.android.gms")
	dim["device_playstore_version"] = packageVersion(ctx, errout, "com.android.vending")
