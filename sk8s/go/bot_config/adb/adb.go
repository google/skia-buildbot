...
var (
	...
	proplines = regexp.MustCompile(`^\[(?P<key>.+)\]:\s*\[(?P<value>.*)\].*$`)
...
func Properties(ctx context.Context) (map[string]string, error) {
	...
	if err != nil {
		return nil, skerr.Wrapf(err, "Failed to get cmd stdout.")
	}
	defer util.Close(stdout)

	if err := cmd.Start(); err != nil {
		return nil, skerr.Wrapf(err, "Failed to start cmd.")
	}

	scanner := bufio.NewScanner(stdout)
	for scanner.Scan() {
		match := proplines.FindStringSubmatch(scanner.Text())
		if match != nil {
			ret[match[1]] = match[2]
		}
	}
