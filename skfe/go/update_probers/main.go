...
func main() {
	...
	probers.Delete(proberRedirectURLsPath...)
	probers.ArrayOfSize(0, proberRedirectURLsPath...)
	...
	domains := []string{}
	for _, cluster := range redirects.S(hostsPath...).Children() {
		domain := cluster.S("domains").Data().(string)
		// Is there a routes redirect?
		redirect := false
		for _, route := range cluster.S("routes").Children() {
	...
	for _, domain := range domains {
		probers.ArrayAppend(domain, proberRedirectURLsPath...)
	}
