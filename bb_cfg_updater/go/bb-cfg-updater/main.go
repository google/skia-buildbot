...
// abandonGerritChange abandons the specified change. Errors are logged.
func abandonGerritChange(ctx context.Context, g *gerrit.Gerrit, issue *gerrit.ChangeInfo) {
	if err := g.Abandon(ctx, issue, ""); err != nil {
		sklog.Error(err)
	}
}
	...
	go util.RepeatCtx(ctx, *pollingPeriod, func(ctx context.Context) {
		...
		if newCfg != existingCfg {
			...
			if err := updateBuildbucketCfg(ctx, g, repo, newCfg); err != nil {
				...
				sklog.Info("Sleep for 10 mins since there was a error with Gerrit to give things time to recover.")
