...
func (c *instance) Start(uuid string) (<-chan error, error) {
	runCmd := &exec.Command{
		Name:      "xvfb-run",
		Args:      []string{"--server-args", "-screen 0 1280x1024x24", "--server-num", fmt.Sprintf("%d", c.port), SKIASERVE, "--port", fmt.Sprintf("%d", c.port), "--source", "https://debugger.skia.org", "--hosted"},
...
func (s *Instances) getContainerFromPool(uuid string) *instance {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	...
	// TODO(jcgregorio) Kill old instances to open up space?
	if co != nil {
		s.instances[uuid] = co
	}
	return co
...
func (s *Instances) startContainer(uuid string) error {
	co := s.getContainerFromPool(uuid)
	if co == nil {
	...
	if err != nil {
		return fmt.Errorf("Failed to start container at port %d: %s", co.port, err)
	}
...
// one for that uuid.
func (s *Instances) getContainer(uuid string) *instance {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Instances) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/instanceStatus" {
		if r.Method == "GET" {
			...
			// TODO(jcgregorio) Confirm that co.uuid has been reset.
			s.mutex.Lock()
			defer s.mutex.Unlock()
			// Remove the entry for this container now that it has exited.
		...
		return
	} else if r.URL.Path == "/instanceNew" && r.Method == "GET" {
