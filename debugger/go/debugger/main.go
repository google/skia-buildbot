...
func loadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to retrieve the SKP.")
		return
	}
	if resp.StatusCode != 200 {
		httputils.ReportError(w, err, "Failed to retrieve the SKP, bad status code.")
		return
	}
	defer util.Close(r.Body)
	b, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		httputils.ReportError(w, err, "Failed to read body.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to create new multipart/form-file object to pass to skiaserve.")
		return
	}
	if _, err := formFile.Write(b); err != nil {
		httputils.ReportError(w, err, "Failed to copy SKP into multipart/form-file object to pass to skiaserve.")
		return
	}
	if err := multipartWriter.Close(); err != nil {
		httputils.ReportError(w, err, "Failed to close new multipart/form-file object to pass to skiaserve.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to create new request object to pass to skiaserve.")
		return
	}
	...
	if rec.Code >= 400 {
		httputils.ReportError(w, fmt.Errorf("Bad status from SKP upload: Status %d Body %q", rec.Code, rec.Body.String()), "Failed to upload SKP.")
	} else {
