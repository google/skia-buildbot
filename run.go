...
func main() {
	...
	if err := t.Step().Name("re-print pwd").Do(func(s *test_automation.Step) error {
		output, err := exec.RunCwd(s.Ctx(), ".", "pwd")
		if err != nil {
			return err
		}
		_, err = exec.RunCwd(s.Ctx(), ".", "echo", output)
		if err != nil {
			return err
		}
		return nil
	...
	if err := t.Step().Do(func(s *test_automation.Step) error {
		panic("halp")
		return nil
	}); err != nil {
		sklog.Fatal(err)
