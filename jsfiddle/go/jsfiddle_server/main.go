...
func codeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if hash == "" {
		sklog.Warningf("Blank hash requested %s", qp["hash"])
		http.Error(w, "Hash is required, cannot be blank.", http.StatusBadRequest)
		return
	}

	if "demo" == hash {
		cr := fiddleContext{}
		if "pathkit" == fiddleType {
			cr.Code = pathkitDemoCode
		}
