 ...
 * from the code. */
export const pickNames = (r: RegExp, s: string): string[] => {
  const ret: string[] = [];
  let match: string[] = [];
...
export class WasmFiddle extends HTMLElement {
  ...
  template: (ele: WasmFiddle)=> TemplateResult;
  ...
  /**
    ...
    */
  constructor(wasmPromise: Promise<CanvasKit | PathKit>, template: (ele: WasmFiddle)=> TemplateResult, libraryName: LibraryName, fiddleType: FiddleType) {
    super();
  ...
  // TODO(jcgregorio) Use infra-sk/modules/fps.
  //
  ...
  _benchmarkFPSInstance(currentRunID: number): ()=> void {
    let lastTime = 0;
    let frameIdx = 0;
  ...
  _enumerateWidgets(): void {
    this.sliders = pickNames(/#slider(\d):(\S+)/g, this.content);
    this.colorpickers = pickNames(/#color(\d):(\S+)/g, this.content);
  ...
  _resetCanvas(): HTMLCanvasElement {
