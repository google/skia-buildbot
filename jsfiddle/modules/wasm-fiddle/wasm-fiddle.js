...
function benchmarkFPSInstance(thisRunID) {
  let lastTime = null;
  let frameIdx = 0;
  ...
  return () => {
    ...
    if (frameIdx === 0) {
      ...
      fpsEle.innerText = `${(10000 / sum).toFixed(1)} FPS`;
      lastTime = performance.now();
    }
  };
}

function activeRunInstance(thisRunID) {
  ...
  run() {
    ...
    try {
      ...
      const f = new Function(
        ...
        'isRunning',
        this.content,
      ); // user provided code, as a string, which will be interpreted and executed.
      f(this.Wasm, canvas, consoleInterceptor, benchmarkFPSInstance(runID), activeRunInstance(runID));
    } catch (e) {
      errorMessage(e);
    }
  }
