    ...
    while(1) {
        ...
        if (status>>8 == (SIGTRAP | (PTRACE_EVENT_SECCOMP<<8))) {
            ...
            if (syscall == SYS_execve) {
                ...
                char *name = read_string( child, regs.rdi );
                if (NULL != strstr(name, "..")) {
                    CHILD_FAIL( "mkdir: No relative paths..." );
                }
                const char **prefixes = mkdir_allowed_prefixes;
                bool okay = false;
                for (; *prefixes != NULL; prefixes++) {
                    if (!strncmp(*prefixes, name, strlen(*prefixes))) {
                        okay = true;
                ...
                if (!okay) {
                    perror( name );
                    CHILD_FAIL( "mkdir: Invalid filename." );
                }
                free(name);
            } else if (syscall == SYS_unlink) {
                char *name = read_string( child, regs.rdi );
                if (NULL != strstr(name, "..")) {
                    CHILD_FAIL( "unlink: No relative paths..." );
                }
                const char **prefixes = unlink_allowed_prefixes;
                bool okay = false;
                for (; *prefixes != NULL; prefixes++) {
                    if (!strncmp(*prefixes, name, strlen(*prefixes))) {
                        okay = true;
                ...
                if (!okay) {
                    perror( name );
                    CHILD_FAIL( "unlink: Invalid filename." );
                }
                free(name);
            } else {
