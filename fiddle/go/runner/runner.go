...
func GitHashTimeStamp(fiddleRoot, gitHash string) (time.Time, error) {
	g, err := gitinfo.NewGitInfo(context.Background(), filepath.Join(fiddleRoot, "versions", gitHash), false, false)
	if err != nil {
...
func Run(ctx context.Context, checkout, fiddleRoot, depotTools, gitHash string, local bool, tmpDir string, opts *types.Options) (*types.Result, error) {
	...
	*/
	...
	exec_ctx := exec.Ctx(ctx)
	if err := exec_ctx.Run(mountCmd); err != nil {
	...
	defer func() {
		...
		if err := exec_ctx.Run(umountCmd); err != nil {
	...
	if err := exec_ctx.Run(runCmd); err != nil {
		runFailures.Inc(1)
