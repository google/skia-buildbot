...
func TestRun(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotNil(t, res)
	assert.Equal(t, "sudo mount -t overlayfs -o lowerdir=/tmp/fiddleroot/versions/abcdef,upperdir=upper,workdir=work none overlay", execStrings[0])
	...
	assert.NoError(t, err)
	assert.NotNil(t, res)
	assert.Equal(t, "sudo mount -t overlayfs -o lowerdir=/tmp/fiddleroot/versions/abcdef,upperdir=/tmp/draw0123/upper,workdir=/tmp/draw0123/work none /tmp/draw0123/overlay", execStrings[0])
