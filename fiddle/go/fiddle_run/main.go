...
func main() {
	...
	if *duration == 0 {
		...
		if err != nil {
			res.Execute.Errors = fmt.Errorf("Failed to create tmp dir for storing animation PNGs: %s", err)
			serializeOutput(res)
			return
		}
		for i := 0; i < FRAMES; i++ {
		...
		if err := createWebm("CPU", tmpDir); err != nil {
			res.Execute.Errors = fmt.Errorf("Failed to encode video: %s", err)
			serializeOutput(res)
			return
		}
		...
		// Encode webm's into base64 in res.
		b, err := ioutil.ReadFile(path.Join(tmpDir, fmt.Sprintf("%s.webm", "CPU")))
		if err != nil {
			res.Execute.Errors = fmt.Errorf("Failed to read resulting video: %s", err)
			serializeOutput(res)
			return
		}
		res.Execute.Output.AnimatedRaster = base64.StdEncoding.EncodeToString(b)

		b, err = ioutil.ReadFile(path.Join(tmpDir, fmt.Sprintf("%s.webm", "GPU")))
		if err != nil {
			res.Execute.Errors = fmt.Errorf("Failed to read resulting video: %s", err)
			serializeOutput(res)
			return
		}
		res.Execute.Output.AnimatedGpu = base64.StdEncoding.EncodeToString(b)

		serializeOutput(res)
	}
}

//  writes tmpDir/prefix.webm.
func createWebm(prefix, tmpDir string) error {
	...
	if err := exec.Run(runCmd); err != nil {
		return nil, fmt.Errorf("ffmpeg failed %#v: %s", *runCmd, err)
	}
...
func extractPNG(b64 string, res *types.Result, i int, prefix string, tmpDir string) error {
	...
	if err != nil {
		res.Execute.Errors = fmt.Errorf("Failed to decode frame %d of %s: %s", i, prefix, err)
		return err
	}
	if err := ioutil.WriteFile(path.Join(tmpDir, fmt.Sprintf("%s_%05d.png", prefix, i)), body, 0600); err != nil {
		res.Execute.Errors = fmt.Errorf("Failed to write frame %d of %s as a PNG: %s", i, prefix, err)
		return err
	}
	return nil
}
