...
// updateGerritIssue loads details about the issue from the Gerrit API and
// updates the AutoRollIssue accordingly.
func updateGerritIssue(ctx context.Context, cfg *GerritConfig, a *autoroll.AutoRollIssue, g gerrit.GerritInterface) (*gerrit.ChangeInfo, error) {
...
// the given gerrit.ChangeInfo. If an error is returned, the AutoRollIssue is
// not changed.
...
func newGerritRoll(ctx context.Context, cfg *GerritConfig, issue *autoroll.AutoRollIssue, g gerrit.GerritInterface, recent *recent_rolls.RecentRolls, issueUrlBase string, rollingTo *revision.Revision, cb func(context.Context, RollImpl) error) (RollImpl, error) {
	ci, err := updateGerritIssue(ctx, cfg, issue, g)
	if err != nil {
		return nil, err
	}
	return &gerritRoll{
		...
		retrieveRoll: func(ctx context.Context) (*gerrit.ChangeInfo, error) {
			return updateGerritIssue(ctx, cfg, issue, g)
		},
		rollingTo: rollingTo,
	}, nil
}
...
// updateGithubPullRequest loads details about the pull request from the GitHub
