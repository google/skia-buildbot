...
func updateIssueFromGitHub(ctx context.Context, a *autoroll.AutoRollIssue, g *github.GitHub, checksNumFunc func() (int, error), checksWaitFor []string, mergeMethodURL string) (*github_api.PullRequest, error) {
...
func getChecksNumFunc(ctx context.Context, config *GithubConfig, rollingTo string) func() (int, error) {
	if config.ChecksNumPyScriptURL == "" {
		...
		return func() (int, error) {
			...
			if err := util.WithWriteFile(f.Name(), func(w io.Writer) error {
				resp, err := client.Get(config.ChecksNumPyScriptURL)
				if err != nil {
					return fmt.Errorf("Could not GET from %s: %s", config.ChecksNumPyScriptURL, err)
				}
				defer util.Close(resp.Body)
				if _, err := io.Copy(w, resp.Body); err != nil {
					return fmt.Errorf("Error writing to %s: %s", f.Name(), err)
				}
				return nil
			}); err != nil {
			...
			cmd := fmt.Sprintf("python %s %s", f.Name(), rollingTo)
			cmdStdout, err := exec.RunSimple(ctx, cmd)
			if err != nil {
				return -1, fmt.Errorf("Error running %s: %s", config.ChecksNumPyScriptURL, err)
			}
			checksNumStr := strings.TrimRight(cmdStdout, "\n")
