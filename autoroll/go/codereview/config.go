...
func (c *GerritConfig) GetLabels(dryRun bool) map[string]int {
	cfg := GERRIT_CONFIGS[c.Config] // Assume we've already been validated.
	cqLabels := cfg.SetCqLabels
	if dryRun {
		cqLabels = cfg.SetDryRunLabels
	}
	return gerrit.MergeLabels(cqLabels, cfg.SelfApproveLabels)
}
