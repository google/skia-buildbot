...
func cfg(android bool) *GerritConfig {
...
func TestGerritRoll(t *testing.T) {
	...
	gr, err := NewGerritRoll(ctx, g.Gerrit, fullHash, recent, roll.Issue, c, nil)
	assert.NoError(t, err)
...
func TestGerritAndroidRoll(t *testing.T) {
	...
	g.MockGetIssueProperties(roll)
	g.MockGetTrybotResults(roll, nil)
	gr, err := NewGerritAndroidRoll(ctx, g.Gerrit, fullHash, recent, roll.Issue, c, nil)
	assert.NoError(t, err)
	assert.False(t, gr.IsFinished())
	assert.False(t, gr.IsSuccess())
	g.AssertEmpty()
