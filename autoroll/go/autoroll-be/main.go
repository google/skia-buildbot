...
func main() {
	...
	if g != nil {
		...
		if forkRepoURL != "" {
			...
			go func() {
				for range time.Tick(60 * time.Minute) {
					// Find all fork branches that start with the roller's name.
					...
					// branches older than a week and delete them.
					for _, r := range refs {
						forkBranchNameMatches := parent.REForkBranchWithTimestamp.FindStringSubmatch(*r.Ref)
						if len(forkBranchNameMatches) != 2 {
							sklog.Infof("Fork branch %s is not in expected format %s. Skipping it.", *r.Ref, parent.REForkBranchWithTimestamp)
							continue
						}
