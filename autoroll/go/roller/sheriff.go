...
func getSheriffHelper(sheriffUrl string) ([]string, error) {
	// If the passed-in sheriffUrl doesn't look like a URL, it's probably an
	// email address. Use it directly.
	if _, err := url.ParseRequestURI(sheriffUrl); err != nil {
		if strings.Count(sheriffUrl, "@") == 1 {
			return []string{sheriffUrl}, nil
		} else {
			return nil, fmt.Errorf("Sheriff must be an email address or a valid URL; %q doesn't look like either.", sheriffUrl)
		}
	}
	...
	resp, err := client.Get(sheriffUrl)
	if err != nil {
		return nil, err
	}
	defer util.Close(resp.Body)
	if strings.HasSuffix(sheriffUrl, ".js") {
		...
		return nil, fmt.Errorf("Unable to parse sheriff email(s) from %q. JSON: %q", sheriffUrl, body)
	}
}
