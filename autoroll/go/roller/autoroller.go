...
func NewAutoRoller(ctx context.Context, c AutoRollerConfig, emailer *email.GMail, g *gerrit.Gerrit, githubClient *github.GitHub, workdir, recipesCfgFile, serverURL, gitcookiesPath string, gcsClient gcs.GCSClient, client *http.Client, rollerName string, local bool) (*AutoRoller, error) {
	...
	if c.AFDORepoManager != nil {
		rm, err = repo_manager.NewAFDORepoManager(ctx, c.AFDORepoManager, workdir, g, serverURL, gitcookiesPath, nil, c.Gerrit, local)
	} else if c.AndroidRepoManager != nil {
		rm, err = repo_manager.NewAndroidRepoManager(ctx, c.AndroidRepoManager, workdir, g, serverURL, c.ServiceAccount, client, c.Gerrit, local)
		...
		rm, err = repo_manager.NewFuchsiaSDKRepoManager(ctx, c.FuchsiaSDKRepoManager, workdir, g, serverURL, gitcookiesPath, nil, c.Gerrit, local)
	} else if c.GithubRepoManager != nil {
		rm, err = repo_manager.NewGithubRepoManager(ctx, c.GithubRepoManager, workdir, githubClient, recipesCfgFile, serverURL, client, c.Github, local)
	} else if c.GithubDEPSRepoManager != nil {
		rm, err = repo_manager.NewGithubDEPSRepoManager(ctx, c.GithubDEPSRepoManager, workdir, githubClient, recipesCfgFile, serverURL, client, c.Github, local)
		...
		rm, err = repo_manager.NewNoCheckoutDEPSRepoManager(ctx, c.NoCheckoutDEPSRepoManager, workdir, g, recipesCfgFile, serverURL, gitcookiesPath, client, c.Gerrit, local)
	} else {
...
func (r *AutoRoller) retrieveRoll(ctx context.Context, issue int64) (roll.RollImpl, error) {
	return r.rm.RetrieveRoll(ctx, r.recent, issue, r.rollFinished)
}
...
func (r *AutoRoller) updateStatus(ctx context.Context, replaceLastError bool, lastError string) error {
	...
	if err := status.Set(ctx, r.roller, &status.AutoRollStatus{
		...
		IssueUrlBase:    r.cfg.CodeReview().GetIssueUrlBase(),
...
func (r *AutoRoller) rollFinished(ctx context.Context, justFinished roll.RollImpl) error {
	recent := r.recent.GetRecentRolls()
	...
	issueURL := fmt.Sprintf("%s%d", r.cfg.CodeReview().GetIssueUrlBase(), currentRoll.Issue)
