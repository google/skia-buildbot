...
const (
	// TODO(borenet): This is needed because the auth token is tied to this
	// address, but we no longer have an alert server.
	EMAIL_FROM_ADDRESS = "alertserver@skia.org"
)
...
func (r *AutoRoller) sendEmail(msg *email.Message) error {
	if r.emailer == nil {
		return nil
	}
	return r.emailer.SendMessage(msg)
}
...
func (r *AutoRoller) SetMode(ctx context.Context, mode, user, message string) error {
	...
	return r.sendEmail(&email.Message{
		SenderDisplayName: EMAIL_FROM_ADDRESS,
		To:                r.emails,
		Subject:           fmt.Sprintf("%s changed the %s into %s AutoRoller mode to %s", r.childName, r.parentName, user, mode),
