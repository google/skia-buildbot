...
func newAutoRoller(ctx context.Context, retrieveRoll func(context.Context, *AutoRoller, int64) (RollImpl, error), rm repo_manager.RepoManager, emailer *email.GMail, g *gerrit.Gerrit, workdir string, c AutoRollerConfig) (*AutoRoller, error) {
	...
	emails, err := getSheriff(c.ParentRepo, c.ChildName, c.Sheriff)
	if err != nil {
		return nil, err
	}
...
// NewAutoRoller returns an AutoRoller instance.
func NewAutoRoller(ctx context.Context, c AutoRollerConfig, depotTools string, emailer *email.GMail, g *gerrit.Gerrit, workdir string) (*AutoRoller, error) {
	// Validation and setup.
	if c.Strategy == "" {
		return nil, fmt.Errorf("Strategy is required.")
	}
	strat, err := repo_manager.GetNextRollStrategy(c.Strategy, c.ChildBranch, c.LKGR)
	if err != nil {
		return nil, err
	}
	...
	case "android":
		...
		rm, err = repo_manager.NewAndroidRepoManager(ctx, workdir, c.ParentBranch, c.ChildPath, c.ChildBranch, g, strat, c.PreUploadSteps, c.ServerURL)
	case "deps":
		rm, err = repo_manager.NewDEPSRepoManager(ctx, workdir, c.ParentRepo, c.ParentBranch, c.ChildPath, c.ChildBranch, depotTools, g, strat, c.PreUploadSteps, c.IncludeLog, c.GClientSpec, c.ServerURL)
	case "manifest":
		rm, err = repo_manager.NewManifestRepoManager(ctx, workdir, c.ParentRepo, c.ParentBranch, c.ChildPath, c.ChildBranch, depotTools, g, strat, c.PreUploadSteps, c.ServerURL)
	case "afdo":
		rm, err = repo_manager.NewAFDORepoManager(ctx, workdir, c.ParentRepo, c.ParentBranch, depotTools, g, c.ServerURL, nil)
	case "fuchsiaSDK":
		rm, err = repo_manager.NewFuchsiaSDKRepoManager(ctx, workdir, c.ParentRepo, c.ParentBranch, depotTools, g, c.ServerURL, nil)
	default:
	...
	return newAutoRoller(ctx, retrieveRoll, rm, emailer, g, workdir, c)
}
...
func (r *AutoRoller) Start(ctx context.Context, tickFrequency, repoFrequency time.Duration) {
	...
	cleanup.Repeat(30*time.Minute, func() {
		emails, err := getSheriff(r.parentRepo, r.childName, r.sheriff)
		if err != nil {
