...
func NewAutoRoller(ctx context.Context, c AutoRollerConfig, emailer *email.GMail, chatBotConfigReader chatbot.ConfigReader, g *gerrit.Gerrit, githubClient *github.GitHub, workdir, recipesCfgFile, serverURL, gitcookiesPath string, gcsClient gcs.GCSClient, client *http.Client, rollerName string, local bool, manualRolls manual.DB) (*AutoRoller, error) {
...
func (r *AutoRoller) handleManualRolls(ctx context.Context) error {
	reqs, err := r.manualRolls.GetIncomplete(r.cfg.RollerName)
	if err != nil {
	...
	for _, req := range reqs {
		...
		if req.Status == manual.STATUS_PENDING {
			...
			if err != nil {
				return fmt.Errorf("Failed to create manual roll: %s", err)
			}
		} else if req.Status == manual.STATUS_STARTED {
			split := strings.Split(req.Url, "/")
			i, err := strconv.Atoi(split[len(split)-1])
			if err != nil {
				return fmt.Errorf("Failed to parse issue number from URL: %s", err)
			}
			issueNum = int64(i)
		} else {
			sklog.Errorf("Found manual roll request in unknown status %q", req.Status)
			continue
		}
		roll, err := r.retrieveRoll(ctx, issueNum)
		if err != nil {
			return fmt.Errorf("Failed to retrieve manual roll: %s", err)
		}
		...
		if err := r.manualRolls.Insert(req); err != nil {
