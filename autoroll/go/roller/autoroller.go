...
func (r *AutoRoller) SetMode(ctx context.Context, mode, user, message string) error {
	...
	if err := r.notifier.Send(&notifier.Message{
		Subject:  fmt.Sprintf("The %s into %s AutoRoll mode was changed", r.childName, r.parentName),
		Body:     fmt.Sprintf("%s changed the mode to %q with message: %s", user, mode, message),
		Severity: notifier.SEVERITY_INFO,
	}); err != nil {
