...
func retrieveGithubPullRequest(ctx context.Context, g *github.GitHub, t *travisci.TravisCI, rm repo_manager.RepoManager, issueNum int64) (*github_api.PullRequest, *autoroll.AutoRollIssue, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Could not retrieve build details: %s", err)
	}
	...
	if len(travisBuilds) > 0 {
		...
		fmt.Println(travisBuilds[0].StartedAt)
	...
	for _, travisBuild := range travisBuilds {
		if travisBuild.Id != 0 {
			...
			case travisci.BUILD_STATE_CREATED:
				// Build is not completely ready yet.
				continue
...
func (r *githubRoll) IsFinished() bool {
	...
	if trybotsRanAndSucceeded && r.pullRequest.GetState() != github.CLOSED_STATE && !r.notifiedReviewers {
		...
		//if err := r.AddComment("@brianosman @Hixie This roll was created using the framework in skbug.com/7730 and should be ready to merge. Let @rmistry know if there are any problems."); err != nil {
