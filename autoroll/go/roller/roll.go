...
func retrieveGithubPullRequest(ctx context.Context, g *github.GitHub, t *travisci.TravisCI, rm repo_manager.RepoManager, issueNum int64) (*github_api.PullRequest, *autoroll.AutoRollIssue, error) {
	...
	travisBuild, err := t.GetLatestBuildDetails(int(issueNum))
	if err != nil {
		return nil, nil, fmt.Errorf("Could not retrieve build details")
	}
	// 2. If travis build is success then go ahead and hackily merge!
	if travisBuild.State == travisci.BUILD_STATE_PASSED {
	...
	buildStartedAt, err := time.Parse("2018-03-27T17:40:06Z", travisBuild.StartedAt)
	if err != nil {
...
func (r *githubRoll) IsFinished() bool {
	...
	return r.issue.AllTrybotsFinished() || r.pullRequest.GetState() == github.CLOSED_STATE || !r.issue.CommitQueue
}
