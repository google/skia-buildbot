...
func retrieveGithubPullRequest(ctx context.Context, g *github.GitHub, t *travisci.TravisCI, rm repo_manager.RepoManager, issueNum int64) (*github_api.PullRequest, *autoroll.AutoRollIssue, error) {
	...
	for _, travisBuild := range travisBuilds {
		if travisBuild.Id != 0 {
			...
			fmt.Println("I AM CALLING AL TRYBOTS SUCCEDED NO BODY ELSE!")
			if a.AllTrybotsSucceeded() {
				...
				// TODO(rmistry): Uncomment this when you can actually merge it.
				...
				// should be merged.
				if err := g.AddComment(int(issueNum), "@brianosman @Hixie This roll was created using the framework in skbug.com/7730 and should be ready to merge. Let @rmistry know if there are any problems."); err != nil {
					return nil, nil, fmt.Errorf("Error when adding the comment: %s", err)
				}
			} else {
...
func (r *githubRoll) IsFinished() bool {
	...
	fmt.Println(trybotsRanAndFinished)
