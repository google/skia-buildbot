...
func ProtoToConfig(cfg *config.Config) *AutoRollerConfig {
	...
	if rm, ok := cfg.RepoManager.(*config.Config_AndroidRepoManager); ok {
		...
		if p, ok := parent.(*config.ParentChildRepoManagerConfig_CopyParent); ok {
			...
			rv.CopyRepoManager = repo_manager.ProtoToCopyRepoManagerConfig(p.CopyParent, c)
			...
			if c, ok := child.(*config.ParentChildRepoManagerConfig_GitCheckoutChild); ok {
				rv.GithubDEPSRepoManager = repo_manager.ProtoToGithubDEPSRepoManagerConfig(p.DepsLocalGithubParent, c.GitCheckoutChild)
			} else if c, ok := child.(*config.ParentChildRepoManagerConfig_CipdChild); ok {
				rv.GithubCipdDEPSRepoManager = repo_manager.ProtoToGithubCipdDEPSRepoManagerConfig(p.DepsLocalGithubParent, c.CipdChild)
			}
			...
			if c, ok := child.(*config.ParentChildRepoManagerConfig_GitilesChild); ok {
				rv.DEPSGitilesRepoManager = repo_manager.ProtoToDEPSGitilesRepoManagerConfig(p.DepsLocalGerritParent, c.GitilesChild)
			} else if c, ok := child.(*config.ParentChildRepoManagerConfig_GitCheckoutChild); ok {
				rv.DEPSRepoManager = repo_manager.ProtoToDEPSRepoManagerConfig(p.DepsLocalGerritParent, c.GitCheckoutChild)
			}
		} else if p, ok := parent.(*config.ParentChildRepoManagerConfig_GitCheckoutGithubFileParent); ok {
			// There's currently only one supported type of child for this
			// parent.
			c := child.(*config.ParentChildRepoManagerConfig_GitCheckoutGithubChild).GitCheckoutGithubChild
			rv.GithubRepoManager = repo_manager.ProtoToGithubRepoManagerConfig(p.GitCheckoutGithubFileParent, c)
			...
			if c, ok := child.(*config.ParentChildRepoManagerConfig_CipdChild); ok {
				rv.GitilesCIPDDEPSRepoManager = repo_manager.ProtoToGitilesCIPDDEPSRepoManagerConfig(p.GitilesParent, c.CipdChild)
			} else if c, ok := child.(*config.ParentChildRepoManagerConfig_FuchsiaSdkChild); ok {
				rv.FuchsiaSDKRepoManager = repo_manager.ProtoToFuchsiaSDKRepoManagerConfig(p.GitilesParent, c.FuchsiaSdkChild)
			} else if c, ok := child.(*config.ParentChildRepoManagerConfig_GitilesChild); ok {
				rv.NoCheckoutDEPSRepoManager = repo_manager.ProtoToNoCheckoutDEPSRepoManagerConfig(p.GitilesParent, c.GitilesChild)
			} else if c, ok := child.(*config.ParentChildRepoManagerConfig_SemverGcsChild); ok {
				rv.SemVerGCSRepoManager = repo_manager.ProtoToSemVerGCSRepoManagerConfig(p.GitilesParent, c.SemverGcsChild)
			}
		}
	}
