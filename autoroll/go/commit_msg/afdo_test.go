...
func TestNamedTemplateAfdo_AllFeatures(t *testing.T) {
	...
	b := fakeBuilder(t)
	b.cfg.ChildName = "AFDO"
	b.cfg.Template = tmplNameAfdo
	result, err := b.Build(FakeCommitMsgInputs())
	require.NoError(t, err)
	require.Equal(t, `Roll AFDO from aaaaaaaaaaaa to cccccccccccc (2 revisions)
...
Also rolling transitive DEPS:
  parent/dep from dddddddddddd to eeeeeeeeeeee
