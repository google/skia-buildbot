...
var (
	...
	diff       = flag.Bool("diff", false, "Diff the generated commit message against the most recent actual commit message.")
...
func main() {
	...
	if err == nil && len(st.Recent) > 0 {
		...
		if cfg.Gerrit != nil {
			...
			ci, err := g.GetIssueProperties(ctx, lastRoll.Issue)
			if err != nil {
				log.Fatalf("Failed to get change: %s", err)
			}
			commit, err := g.GetCommit(ctx, ci.Issue, ci.Patchsets[len(ci.Patchsets)-1].ID)
			if err != nil {
			...
			// TODO
			log.Fatal("No Github support yet")
		} else {
			log.Fatal("Either Gerrit or Github is required.")
		}
	} else {
		log.Println("Could not find real roll; using fake inputs.")
	...
	if realCommitMsg != "" {
		...
		if diff != "" {
			...
			fmt.Println("Generated commit message is identical to existing:")
			fmt.Println(genCommitMsg)
		}
	} else {
