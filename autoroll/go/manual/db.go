...
// See documentation for DB interface.
func (d *firestoreDB) Insert(req *ManualRollRequest) (rvErr error) {
	...
	return d.client.RunTransaction("Insert", req.Id, DEFAULT_ATTEMPTS, INSERT_TIMEOUT, func(ctx context.Context, tx *fs.Transaction) error {
		...
		if !isNew {
			...
			if old.DbModified != oldDbModified {
				return ErrConcurrentUpdate
			}
		}
...
// See documentation for DB interface.
func (d *memoryDB) Insert(req *ManualRollRequest) (rvErr error) {
	if err := req.Validate(); err != nil {
		return err
	}
	isNew := req.Id == ""
