...
func testDB(t *testing.T, db DB) {
	...
	r.Id = ""
	r.RollerName = ""
	assert.EqualError(t, db.Insert(r), "RollerName is required.")

	// Verify that we can't update a request which doesn't already exist.
	r.RollerName = rollerName
	r.Id = "bogus"
	r.DbModified = time.Now()
	assert.EqualError(t, db.Insert(r), ErrNotFound.Error())

	// Verify that we can't insert a new request which has a non-zero
	// DbModified timestamp.
	r.Id = ""
	r.DbModified = time.Now()
	assert.EqualError(t, db.Insert(r), "Request has no ID but has non-zero DbModified timestamp.")

	// Insert and retrieve several requests.
	now := time.Now()
	...
	assert.EqualError(t, db.Insert(reqs[0]), "Request has an ID but has a zero DbModified timestamp.")
	...
	id := reqs[3].Id
	assert.NoError(t, db.Insert(reqs[3]))
	...
	oldDbModified = reqs[0].DbModified
	assert.EqualError(t, db.Insert(reqs[0]), ErrConcurrentUpdate.Error())
