...
func TestStatus(t *testing.T) {
	...
	// We should return nil until there's actually some data.
	assert.Equal(t, (*AutoRollStatus)(nil), c.Get())
	assert.Equal(t, (*AutoRollMiniStatus)(nil), c.GetMini())
	...
	// Cache should return nil until we Update(), at which point we should
	...
	assert.NoError(t, err)
	assert.Equal(t, (*AutoRollStatus)(nil), c2.Get())
	assert.Equal(t, (*AutoRollMiniStatus)(nil), c2.GetMini())
}
