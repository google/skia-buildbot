...
func (c *Checkout) GetRevision(ctx context.Context, id string) (*revision.Revision, error) {
	...
	if len(c.Dependencies) > 0 {
		rev.Dependencies = make(map[string]string, len(c.Dependencies))
		for _, dep := range c.Dependencies {
			contents, err := c.GetFile(ctx, dep.Path, id)
			if err != nil {
				return nil, skerr.Wrap(err)
			}
			version, err := version_file_common.GetPinnedRev(*dep, string(contents))
			if err != nil {
				return nil, skerr.Wrap(err)
			}
