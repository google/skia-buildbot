...
type GithubCipdDEPSRepoManagerConfig struct {
	...
	CipdAssetPath string `json:"cipdAssetPath"`
}
...
func (rm *githubCipdDEPSRepoManager) Update(ctx context.Context) error {
	...
	lastRollRev, err := rm.getLastRollRev(ctx, rm.cipdAssetPath, rm.cipdAssetName)
	if err != nil {
		return err
	}

	fmt.Printf("\n\nGOT %s for %s and %s\n\n", lastRollRev, rm.cipdAssetPath, rm.cipdAssetName)
	...
	// Hack for now. EXPERIMENTING DOWN
		...
		for _, instance := range instances {
			...
			if foundHead {
				notRolledRevs = append(notRolledRevs, &revision.Revision{
					...
					URL:         fmt.Sprintf(cipdPackageUrlTmpl, cipd.SERVICE_URL, rm.cipdAssetName, id),
				})
			}
		}
	}
