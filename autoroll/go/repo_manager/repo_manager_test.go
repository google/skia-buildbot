...
func setupAndroid(t *testing.T) (string, *git_testutils.GitBuilder, []string, *git_testutils.GitBuilder, func()) {
	wd, err := ioutil.TempDir("", "")
	assert.NoError(t, err)
...
func TestAndroidRepoManager(t *testing.T) {
	...
	wd, _, _, _, _ := setupAndroid(t)
	// wd, child, childCommits, parent, cleanup := setupAndroid(t)
	//defer cleanup() // UNDO THIS!
	// Set wd so that it does everything in same place.
	wd = "/repos/android_roller_test/"
	// Figure out what child is and then point it to skia!
	// childPath = "external/skia"
	rm, err := NewAndroidRepoManager(wd, "external/skia", 24*time.Hour, "git-cookie-authdaemon")
	assert.NoError(t, err)

	// TOOD(rmistry): Below needs to be reenabled when testing!
	...
	rp, err := rm.RolledPast("b66fa526b882f2472d731b42cba65fe05cea4268")
	assert.NoError(t, err)
	assert.True(t, rp)
	...
	// User, name only.
	assert.Equal(t, "31977622648@project.gserviceaccount.com", rm.User())
...
func testCreateNewAndroidRoll(t *testing.T, strategy string, expectIdx int) {
	...
	rm, err := NewAndroidRepoManager(wd, "external/skia", 24*time.Hour, "/usr/bin/repo", "git-cookie-authdaemon")
	assert.NoError(t, err)

	// Create a roll, assert that it's at tip of tree.
	issue, err := rm.CreateNewRoll(strategy, []string{"rmistry@google.com"}, "", false /* dry run */, true)
	fmt.Println(issue)
	assert.NoError(t, err)
...
func LATER_TestAndroidRepoManagerBatch(t *testing.T) {
