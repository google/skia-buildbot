...
func setupNoCheckout(t *testing.T, cfg *NoCheckoutRepoManagerConfig) (context.Context, string, RepoManager, *git_testutils.GitBuilder, *git_testutils.GitBuilder, *gitiles_testutils.MockRepo, *gitiles_testutils.MockRepo, []string, *mockhttpclient.URLMock, func()) {
  ...
  "%s": "%s@%s",
	...
	rm, err := NewNoCheckoutRepoManager(ctx, cfg, wd, g, recipesCfg, "fake.server.com", urlmock.Client())
	assert.NoError(t, err)
...
func noCheckoutCfg() *NoCheckoutRepoManagerConfig {
	return &NoCheckoutRepoManagerConfig{
		ChildBranch:   "master",
		ChildPath:     childPath,
...
func TestNoCheckoutRepoManagerSingle(t *testing.T) {
	cfg := noCheckoutCfg()
