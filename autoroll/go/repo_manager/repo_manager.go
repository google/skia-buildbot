...
// TODO(rmistry): Need to finish this!!!!!!!!!!!!!!!!!!!!
// func (r *androidRepoManager) GetChangeNumForHash(hash string) (int, error) {
func (r *androidRepoManager) GetChangeNumForHash(hash string) (int, error) {
...
func (r *androidRepoManager) CreateNewRoll(strategy string, emails []string, cqExtraTrybots string, dryRun, gerrit bool) (int64, error) {
	...
	issues, err := r.GetChangeNumForHash("37fc45faf90b70a453c8cdaef4a5f3168ce3be4e")
	if err != nil {
		return 0, err
	}
	fmt.Println(issues)
	...
	//	return 0, err
	//}
	//if _, err := exec.RunCwd(r.workdir, "git", "checkout", "-b", DEPS_ROLL_BRANCH, "-t", "origin/master", "-f"); err != nil {
	//	return 0, err
	//}
	...
	// rmistry: LEFT IT HERE!
	// Need to now do the merge.
	// Then the GN steps.
	// Then create commit msg.
	...
	// This section is tricky. Will have to be tested well.
...
`, len(commits), commitRange)
	...
	if _, commitErr := exec.RunCwd(r.childDir, "git", "commit", "-m", commitMsg); commitErr != nil {
		...
		return 0, fmt.Errorf("Nothing to merge; did somone already merge %s?: %s", commitRange, commitErr)
	}
	...
	// rmistry: HERE HERE HERE HERE HERE

	fmt.Println("Done with the merge. Now what????")
	return 0, nil
	...
	//commitMsg, err := exec.RunCwd(r.workdir, "git", "log", "-n1", "--format=%B", "HEAD")
	//if err != nil {
	//	return 0, err
	//}
	commitMsg += `
