...
func (r *androidRepoManager) cleanParent() error {
	if _, err := exec.RunCwd(r.workdir, "git", "clean", "-d", "-f", "-f"); err != nil {
		return err
	}
	_, _ = exec.RunCwd(r.workdir, "git", "rebase", "--abort")
	if _, err := exec.RunCwd(r.workdir, "git", "checkout", "origin/master", "-f"); err != nil {
		return err
	}
	_, _ = exec.RunCwd(r.workdir, "git", "branch", "-D", DEPS_ROLL_BRANCH)
...
func (r *androidRepoManager) CreateNewRoll(strategy string, emails []string, cqExtraTrybots string, dryRun, gerrit bool) (int64, error) {
	...
	if _, err := exec.RunCwd(r.workdir, "git", "config", "user.name", autoroll.ROLL_AUTHOR); err != nil {
		return 0, err
	}
	if _, err := exec.RunCwd(r.workdir, "git", "config", "user.email", autoroll.ROLL_AUTHOR); err != nil {
		return 0, err
	}
...
// TODO(rmistry): New Android Repo Manager?
