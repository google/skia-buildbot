...
type CommonRepoManagerConfig struct {
	...
	// Strategy for determining which commit(s) to roll. See repo_manager
	// package for more information.
	Strategy string `json:"strategy"`

	// Optional fields.

	// Named steps to run before uploading roll CLs. See repo_manager
	// package for more information.
...
func newDepotToolsRepoManager(ctx context.Context, c DepotToolsRepoManagerConfig, workdir, serverURL string, g *gerrit.Gerrit) (*depotToolsRepoManager, error) {
	if err := c.Validate(); err != nil {
		return nil, err
	}
