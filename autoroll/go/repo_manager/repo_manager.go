...
var (
	...
	//NewAndroidRepoManager func(string, string, time.Duration, string, string, *gerrit.Gerrit) (RepoManager, error) = NewAndroidRepoManager
...
func NewAndroidRepoManager(workdir, childPath string, frequency time.Duration, g *gerrit.Gerrit) (RepoManager, error) {
...
func (r *androidRepoManager) update() error {
	fmt.Println("UPDATING NEW ANDROID ROLL!!!!!!!!!!!!")
	...
	//if _, err := exec.RunCwd(r.workdir, "repo", "init", "-u", "https://googleplex-android.googlesource.com/a/platform/manifest", "-g", "all,-notdefault,-darwin", "-b", "master"); err != nil {
	//	return err
	//}
...
func (r *androidRepoManager) CreateNewRoll(strategy string, emails []string, cqExtraTrybots string, dryRun, gerrit bool) (int64, error) {
	fmt.Println("CREATING NEW ANDROID ROLL!!!!!!!!!!!!")
	...
	// TODO(rmistry): Use this when you test against closer to head. Is this working???
...
`, r.childPath, commitRange, len(commits), commitRange)
...
func (r *androidRepoManager) SendToGerritCQ(change *gerrit.ChangeInfo, comment string) error {
	// rmistry: Goal is to try to get this executed.
	fmt.Println("In Android's SendToGerritCQ")
	fmt.Println("In Android's SendToGerritCQ")
	fmt.Println("In Android's SendToGerritCQ")
	return nil
}
...
func (r *androidRepoManager) SendToGerritDryRun(change *gerrit.ChangeInfo, comment string) error {
	// rmistry: Goal is to try to get this executed.
	fmt.Println("In Android's SendToGerritDryRun")
	fmt.Println("In Android's SendToGerritDryRun")
	fmt.Println("In Android's SendToGerritDryRun")
	return nil
}
