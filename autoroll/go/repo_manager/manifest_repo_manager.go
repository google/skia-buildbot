...
func (mr *manifestRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	if !mr.local {
		if _, err := exec.RunCwd(ctx, mr.parentDir, "git", "config", "user.name", getLocalPartOfEmailAddress(mr.codereview.User())); err != nil {
			return 0, err
		}
		if _, err := exec.RunCwd(ctx, mr.parentDir, "git", "config", "user.email", mr.codereview.User()); err != nil {
			return 0, err
		}
	}
