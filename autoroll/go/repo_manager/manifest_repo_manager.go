...
func (mr *manifestRepoManager) CreateNewRoll(from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	if _, err := mr.exec.RunCwd(mr.parentDir, "git", "checkout", "-b", ROLL_BRANCH, "-t", fmt.Sprintf("origin/%s", mr.parentBranch), "-f"); err != nil {
		return 0, err
	}
	...
	if _, err := mr.exec.RunCwd(mr.parentDir, "git", "config", "user.name", mr.user); err != nil {
		return 0, err
	}
	if _, err := mr.exec.RunCwd(mr.parentDir, "git", "config", "user.email", mr.user); err != nil {
		return 0, err
	}
...
`, mr.childPath, commitRange, len(commits), childRepoName, childRepoName, commitRange, strings.Join(changeSummaries, "\n"), fmt.Sprintf(COMMIT_MSG_FOOTER_TMPL, mr.serverURL))
	...
	if _, addErr := mr.exec.RunCwd(mr.parentDir, "git", "add", manifestFileName); addErr != nil {
		return 0, fmt.Errorf("Failed to git add: %s", addErr)
	}
	if _, commitErr := mr.exec.RunCwd(mr.parentDir, "git", "commit", "-m", commitMsg); commitErr != nil {
	...
	if _, err := mr.exec.RunCommand(uploadCmd); err != nil {
		return 0, err
	}
	...
	if _, err := mr.exec.RunCommand(&exec.Command{
		Dir:  mr.parentDir,
		Env:  mr.GetEnvForDepotTools(),
		Name: "git",
