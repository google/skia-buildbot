...
func (rm *gcsRepoManager) updateHelper(ctx context.Context, strat strategy.NextRollStrategy, parentRepo *gitiles.Repo, baseCommit string) (*revision.Revision, *revision.Revision, []*revision.Revision, error) {
	...
	if lastIdx == -1 {
		sklog.Errorf("Last roll rev %q not found in available versions. This is acceptable for some rollers which allow outside versions to be rolled manually. A human should verify that this is the case. Using the single most recent available version for the not-yet-rolled revisions list.", lastRollRevId)
		lastIdx = 1
