...
func (rm *gcsRepoManager) updateHelper(ctx context.Context, parentRepo *gitiles.Repo, baseCommit string) (*revision.Revision, *revision.Revision, []*revision.Revision, error) {
	...
	if lastIdx == -1 {
		...
		if err != nil {
			sklog.Errorf("Failed to retrieve last roll rev at %s; arbitrarily using the second-most-recently uploaded revision as the last roll rev: %s", lastRevPath, err)
