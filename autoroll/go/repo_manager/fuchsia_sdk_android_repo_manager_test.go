...
func setupFuchsiaSDKAndroid(t *testing.T) (context.Context, string, RepoManager, *mockhttpclient.URLMock, *gitiles_testutils.MockRepo, *git_testutils.GitBuilder, func()) {
	...
	mockRun.SetDelegateRun(func(cmd *exec.Command) error {
		...
		if cmd.Name == "git" && strings.Contains(cmd.Dir, cfg.ChildPath) {
			var output string
			if cmd.Args[0] == "log" {
	...
	genSdkBpRepo.Add(ctx, path.Join(GEN_SDK_BP_DIR, GEN_SDK_BP), fmt.Sprintf(`
import os
path = os.environ['ANDROID_BUILD_TOP'] + '/%s'
...
`, cfg.ChildPath))
	...
	assert.NoError(t, os.Mkdir(sdkDir, os.ModePerm))
	testutils.WriteFile(t, path.Join(sdkDir, "file1"), "contents")
...
func mockDownloadSDK(t *testing.T, urlmock *mockhttpclient.URLMock, rev, wd string) {
	...
	sdkDir := path.Join(wd, "sdk")
	_, err := exec.RunCwd(context.Background(), sdkDir, "tar", "-czf", archive, ".")
	assert.NoError(t, err)
