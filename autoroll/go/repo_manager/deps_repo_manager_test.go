...
func setupDEPSRepoManager(t *testing.T) (context.Context, *config_vars.Registry, string, *git_testutils.GitBuilder, []string, *git_testutils.GitBuilder, *exec.CommandCollector, *vcsinfo.LongCommit, *gitiles_testutils.MockRepo, *mockhttpclient.URLMock, func()) {
...
`, childPath))
	...
	return ctx, setupRegistry(t), wd, child, childCommits, parent, mockRun, lastUpload, mockChild, urlmock, cleanup
}

func setupFakeGerrit(t *testing.T, wd string) *mockhttpclient.URLMock {
	urlMock := mockhttpclient.NewURLMock()

	// TODO(borenet): Is this needed?
	/*
...
func TestDEPSRepoManager(t *testing.T) {
	...
	ctx, reg, wd, childRepo, childCommits, parentRepo, _, _, _, urlmock, cleanup := setupDEPSRepoManager(t)
	defer cleanup()
...
func TestCreateNewDEPSRoll(t *testing.T) {
	...
	ctx, reg, wd, childRepo, _, parentRepo, _, _, _, urlmock, cleanup := setupDEPSRepoManager(t)
	defer cleanup()
	recipesCfg := filepath.Join(testutils.GetRepoRoot(t), recipe_cfg.RECIPE_CFG_PATH)
...
func TestDEPSRepoManagerIncludeLog(t *testing.T) {
	...
	test := func(includeLog bool) {
		ctx, reg, wd, childRepo, _, parentRepo, _, lastUpload, _, urlmock, cleanup := setupDEPSRepoManager(t)
		defer cleanup()
		recipesCfg := filepath.Join(testutils.GetRepoRoot(t), recipe_cfg.RECIPE_CFG_PATH)
...
func TestDEPSRepoManagerGclientSpec(t *testing.T) {
	...
	ctx, reg, wd, childRepo, _, parentRepo, mockRun, _, _, urlmock, cleanup := setupDEPSRepoManager(t)
	defer cleanup()
	recipesCfg := filepath.Join(testutils.GetRepoRoot(t), recipe_cfg.RECIPE_CFG_PATH)
...
func TestDEPSRepoManagerBugs(t *testing.T) {
	...
	test := func(bugLine, expect string) {
		// Setup.
		ctx, reg, wd, childRepo, childCommits, parentRepo, _, lastUpload, mockChild, urlmock, cleanup := setupDEPSRepoManager(t)
		defer cleanup()
		recipesCfg := filepath.Join(testutils.GetRepoRoot(t), recipe_cfg.RECIPE_CFG_PATH)
...
`, bugLine))
		...
		// Mock requests for Update.
		mockChild.MockGetCommit(ctx, "master")
		mockChild.MockLog(ctx, git.LogFromTo(childCommits[0], hash))
		mockChild.MockGetCommit(ctx, hash)
