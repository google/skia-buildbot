...
func (rm *assetRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	// Create the roll CL.
	if !rm.local {
		if _, err := exec.RunCwd(ctx, rm.parentDir, "git", "config", "user.name", getLocalPartOfEmailAddress(rm.codereview.User())); err != nil {
			return 0, err
		}
		if _, err := exec.RunCwd(ctx, rm.parentDir, "git", "config", "user.email", rm.codereview.User()); err != nil {
			return 0, err
		}
	}
