...
func (rm *githubDEPSRepoManager) CreateNewRoll(ctx context.Context, from, to *revision.Revision, rolling []*revision.Revision, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	if len(rm.transitiveDeps) > 0 {
		for childPath, parentPath := range rm.transitiveDeps {
			newRev, ok := from.Dependencies[childPath]
			if !ok {
				return 0, skerr.Fmt("From-revision %s is missing dependency entry for %s", from.Id, childPath)
			}
			oldRev, ok := to.Dependencies[childPath]
			if !ok {
				return 0, skerr.Fmt("To-revision %s is missing dependency entry for %s", from.Id, childPath)
			}
			if oldRev != newRev {
