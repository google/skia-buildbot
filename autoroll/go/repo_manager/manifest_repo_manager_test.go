...
func TestManifestRepoManager(t *testing.T) {
	...
	rm, err := NewManifestRepoManager(wd, parent.RepoUrl(), "master", childPath, "master", 24*time.Hour, depotTools, g)
	assert.NoError(t, err)
...
// TestCreateNewManifestRoll tests CreateNewRoll.
func TestCreateNewManifestRoll(t *testing.T) {
	...
	issue, err := rm.CreateNewRoll(ROLL_STRATEGY_BATCH, manifestEmails, "", false)
	assert.NoError(t, err)
