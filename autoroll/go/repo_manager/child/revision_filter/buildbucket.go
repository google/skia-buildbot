...
func (f BuildbucketRevisionFilter) Skip(ctx context.Context, r *revision.Revision) (string, error) {
	pred := &buildbucketpb.BuildPredicate{
		...
		Tags: []*buildbucketpb.StringPair{
			{Key: "buildset", Value: fmt.Sprintf(f.buildsetCommitTmpl, r.Id)},
		},
	}
...
// Buildbucket to filter revisions.
func NewBuildbucketRevisionFilter(client *http.Client, project, bucket, buildsetCommitTmpl string) (*BuildbucketRevisionFilter, error) {
	if project == "" || bucket == "" {
	...
	return &BuildbucketRevisionFilter{
		...
		project:            project,
		bucket:             bucket,
		buildsetCommitTmpl: buildsetCommitTmpl,
	}, nil
}
