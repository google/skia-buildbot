...
func NewGitCheckoutGithub(ctx context.Context, c *config.GitCheckoutGitHubChildConfig, workdir string, cr codereview.CodeReview, co git.Checkout) (*GitCheckoutGithubChild, error) {
	if err := c.Validate(); err != nil {
		return nil, skerr.Wrap(err)
	}
	child, err := NewGitCheckout(ctx, c.GitCheckout, workdir, cr, co)
	if err != nil {
		return nil, skerr.Wrap(err)
	}
