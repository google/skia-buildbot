...
func NewCIPD(ctx context.Context, c *config.CIPDChildConfig, client *http.Client, workdir string) (*CIPDChild, error) {
	...
	if c.GitilesRepo != nil {
		gitRepo, err = gitiles_common.NewGitilesRepo(ctx, c.GitilesRepo, nil, client)
		if err != nil {
			return nil, skerr.Wrap(err)
		}
	}
...
func (c *CIPDChild) GetRevision(ctx context.Context, id string) (*revision.Revision, error) {
	...
	if c.gitRepo != nil {
		...
		if gitRevision == "" {
			...
			sklog.Errorf("git_revision is: %s", gitRevision)
...
func (c *CIPDChild) Update(ctx context.Context, lastRollRev *revision.Revision) (*revision.Revision, []*revision.Revision, error) {
	...
	if c.gitRepo != nil {
		...
		notRolledCommits, err := c.gitRepo.LogFirstParent(ctx, lastRollRev.Id, tipRev.Id)
		if err != nil {
			return nil, nil, skerr.Wrap(err)
		}
...
func getGitRevisionFromCIPDInstance(instance *cipd_api.InstanceDescription) string {
	for _, tag := range instance.Tags {
		sklog.Errorf("Tag: %s", tag.Tag)
		split := strings.SplitN(tag.Tag, ":", 2)
		if len(split) != 2 {
			sklog.Errorf("Invalid CIPD tag %q; expected <key>:<value>", tag.Tag)
			continue
		}
		if gitRevisionTag == split[0] {
			return split[1]
		}
	}
	return ""
