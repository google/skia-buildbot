...
func newGithubRepoManager(ctx context.Context, c *GithubRepoManagerConfig, workdir string, githubClient *github.GitHub, recipeCfgFile, serverURL string) (RepoManager, error) {
	...
	parentDir := path.Join(wd, strings.TrimSuffix(path.Base(c.ParentRepoURL), ".git"))
	...
	childDir := path.Join(wd, strings.TrimSuffix(path.Base(c.ChildRepoURL), ".git"))
	if _, err := os.Stat(childDir); err != nil {
		if _, err := git.GitDir(wd).Git(ctx, "clone", c.ChildRepoURL); err != nil {
			return nil, err
		}
	}
...
func (rm *githubRepoManager) Update(ctx context.Context) error {
	...
	if !remoteFound {
		if _, err := git.GitDir(rm.parentDir).Git(ctx, "remote", "add", GITHUB_UPSTREAM_REMOTE_NAME, rm.parentRepoURL); err != nil {
			return err
		}
	}
	// Fetch upstream.
