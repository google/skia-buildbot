...
type GithubRepoManagerConfig struct {
	...
	// Buildbucket project to query for luci builds.
	BuildbucketProject string `json:"buildbucketProject"`
	// Buildbucket bucket to query for luci builds.
	BuildbucketBucket string `json:"buildbucketBucket"`
}
			...
			for _, notRolledRev := range notRolledRevs {
				pred := &buildbucketpb.BuildPredicate{
					...
					Tags: []*buildbucketpb.StringPair{
						{Key: "buildset", Value: fmt.Sprintf("commit/git/%s", notRolledRev.Id)},
					},
				}
				...
				for _, build := range builds {
					if build.Status == buildbucketpb.Status_SUCCESS {
						...
						sklog.Infof("[bbFilter] Build status of \"%s\" for %s was %s", build.Builder.Builder, notRolledRev.Id, build.Status)
						notRolledRev.InvalidReason = fmt.Sprintf("Luci build of \"%s\" for %s was %s", build.Builder.Builder, notRolledRev.Id, build.Status)
						break
					}
				}
