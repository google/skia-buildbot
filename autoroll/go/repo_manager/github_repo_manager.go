...
type GithubRepoManagerConfig struct {
	...
	//TravisCiToken   string `json:"travisci_token"`
...
func (c *GithubRepoManagerConfig) Validate() error {
	//if c.CommonRepoManagerConfig. == "" {
	//	return errors.New("GithubToken is required.")
	//}
	//if c.TravisCiToken == "" {
	//	return errors.New("TravisCiToken is required.")
	//}
	//if c.GithubRepoOwner == "" {
	//	return errors.New("GithubRepoOwner is required.")
	//}
	//if c.GithubRepoName == "" {
	//	return errors.New("GithubRepoName is required.")
	//}
...
func newGithubRepoManager(ctx context.Context, c *GithubRepoManagerConfig, workdir string, githubClient *github.GitHub, recipeCfgFile, serverURL string) (RepoManager, error) {
	...
	//githubClient, err := github.NewGitHub(ctx, c.GithubRepoOwner, c.GithubRepoName, githubHttpClient)
	//if err != nil {
	//	sklog.Fatalf("Could not create github client: %s", err)
	//}
	//travisCiClient, err := travisci.NewTravisCI(ctx, c.GithubRepoOwner, c.GithubRepoName, tToken)
	//if err != nil {
	//	sklog.Fatalf("Could not create github client: %s", err)
	//}
	...
	//gclient := path.Join(c.drm.depotTools, GCLIENT)
	...
	// TODO(rmistry): Is gclient and rolldep ok?
	// About about the below?
	// wd := path.Join(workdir, strings.TrimSuffix(path.Base(parentRepo), ".git"))
	// parentDir := path.Join(wd, "src/flutter")
	// childDir := path.Join(wd, childPath)
	// childRepo := &git.Checkout{GitDir: git.GitDir(childDir)}
	...
	//
	//c.DepotToolsRepoManagerConfig.parentDir
	//c.DepotToolsRepoManagerConfig.CommonRepoManagerConfig.parentDir
	// TODO(rmistry): This should be depot_tools manager.

	fmt.Println("BLAH BLAH BLAH")
	fmt.Println(c.DepotToolsRepoManagerConfig)
	...
	// TODO(rmistry): Make some sort of flag??
	//drm.parentDir = path.Join(wd, "src/flutter")
	dr.parentDir = path.Join(wd, "src/flutter")

	//depsConfig := &DEPSRepoManagerConfig{
	//	DepotToolsRepoManagerConfig: c.DepotToolsRepoManagerConfig,
	//	IncludeLog:                  true,
	//}
	//rm, err := newDEPSRepoManager(ctx, depsConfig, wd, nil, recipeCfgFile, serverURL)
	//if err != nil {
	//	return nil, err
	//}
	//drm, ok := rm.(*depsRepoManager)
	//if !ok {
	//	return nil, errors.New("Got unexpected type")
	//}
	...
	// childDir := path.Join(wd, childPath)
	// childRepo := &git.Checkout{GitDir: git.GitDir(childDir)}
	//gr.parentDir = path.Join(wd, "src/flutter")
	//fmt.Println("HERE HERE HER HERE")
	//fmt.Println(gr.workdir)
	//fmt.Println(gr.parentDir)
	//fmt.Println(gr.childDir)
	//fmt.Println(gr.childRepo.Dir())

	//sklog.Fatal("Make sure everything looks ok above!")
...
func (gr *githubRepoManager) Update(ctx context.Context) error {
	...
	// TODO(rmistry): why do you need this? does roll-deps do the right thing anyway? or do you need to do this in update?
...
func (gr *githubRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	// Collect all errors from pre-upload steps.
	preUploadErrors := []error{}
	for _, s := range gr.PreUploadSteps() {
		if err := s(ctx, gr.parentDir); err != nil {
			//sklog.Fatal("did't work")
			// TODO(rmistry): Make sure the error contains the stdout and collect it and add a comment about needing manual intervention.
	...
	fmt.Println("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
	fmt.Println("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
	fmt.Println(preUploadErrors)
	fmt.Println(len(preUploadErrors))
	...
	fmt.Println("Going through the pre upload errors now!!!!!!!!!")
	for _, preUploadErr := range preUploadErrors {
		fmt.Printf("Adding pre-upload error comment: %s", preUploadErr.Error())
