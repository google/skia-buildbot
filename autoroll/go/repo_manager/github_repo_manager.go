			...
			for _, notRolledRev := range notRolledRevs {
				...
				for _, build := range builds {
					if build.Status == buildbucketpb.Status_SUCCESS {
						// FIXME FIXME FIXME ("GOOD")
						sklog.Infof("[bbFilter] Build status of \"%s\" for %s was %s GOOD", build.Builder.Builder, notRolledRev.Id, build.Status)
						continue
					} else {
						// FIXME FIXME FIXME ("BAD")
						sklog.Infof("[bbFilter] Build status of \"%s\" for %s was %s BAD", build.Builder.Builder, notRolledRev.Id, build.Status)
						// FIXME FIXME FIXME
						//notRolledRev.InvalidReason = fmt.Sprintf("Missing required GCS file %q", gsPath)
						break
					}
