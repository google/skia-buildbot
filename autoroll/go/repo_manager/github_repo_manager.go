...
func (gr *githubRepoManager) Update(ctx context.Context) error {
	...
	// "git pull remote master" to get latest DEPS from the target repository.
...
func (gr *githubRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	// Make sure the forked repo is checked out at the same hash as the target
	// repo before creating the pull request.
	if _, err := exec.RunCwd(ctx, gr.parentDir, "git", "fetch", GITHUB_UPSTREAM_REMOTE_NAME, gr.parentBranch); err != nil {
		return 0, err
	}
	if _, err := exec.RunCwd(ctx, gr.parentDir, "git", "reset", "--hard", fmt.Sprintf("%s/%s", GITHUB_UPSTREAM_REMOTE_NAME, gr.parentBranch)); err != nil {
		return 0, err
	}
