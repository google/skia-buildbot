...
func (rm *githubRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	cmd := []string{"git", "log", fmt.Sprintf("%s..%s", from, to), "--date=short", "--no-merges", "--format='%ad %ae %s'"}
	logStr, err := exec.RunCwd(ctx, rm.childDir, cmd...)
	if err != nil {
		return 0, err
	}
	...
	log, err := git.GitDir(rm.parentDir).Git(ctx, "log", "-n1")
	if err != nil {
		return 0, err
	}
	sklog.Infof("Log: %s", log)
