...
// TODO(rmistry): Footer is duplicated???
const (
...
func (rm *githubRepoManager) Update(ctx context.Context) error {
	...
	sklog.Fatal("This is the problem")
	return nil
}
...
// TODO(rmistry): Remove all error logging that you added.
// See documentation for RepoManager interface.
func (rm *githubRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	user, repo := GetUserAndRepo(strings.TrimRight(remoteUrl, "\n"))
	...
	if err != nil {
		...
		fmt.Println("Error when commiting")
		fmt.Println(err)
		return 0, err
	}

	fmt.Println("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
	fmt.Println("ABOUT TO ADD LABLES STUFF")
	...
	if err := rm.githubClient.AddLabel(pr.GetNumber(), label); err != nil {
		fmt.Println("XXXXXXXXXXXXXXXXXX")
		fmt.Println("Error when adding labels")
		fmt.Println(err)
		return 0, err
	}
	fmt.Println("AFTER LABELS!!!!!!!!!!!")
	...
	if err := rm.githubClient.AddComment(pr.GetNumber(), fmt.Sprintf("%s : New roll has been created by %s", strings.Join(mentions, " "), rm.serverURL)); err != nil {
		fmt.Println("XXXXXXXXXXXXXXXXXX")
		fmt.Println("Error when adding comment")
		fmt.Println(err)
		return 0, err
	}
