...
func newGithubRepoManager(ctx context.Context, workdir, parentRepo, parentBranch, childPath, childBranch, depot_tools string, g *gerrit.Gerrit, strategy NextRollStrategy, preUploadStepNames []string, includeLog bool, gclientSpec, serverURL string) (RepoManager, error) {
	...
	// TODO(rmistry): Somehow get this out of r.gclientSpec or have a new flag?
...
func (gr *githubRepoManager) Update(ctx context.Context) error {
	...
	if _, err := exec.RunCwd(ctx, gr.parentDir, "git", "checkout", fmt.Sprintf("origin/%s", gr.parentBranch), "-b", ROLL_BRANCH); err != nil {
...
func (gr *githubRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	//// Clean to see what happens!
	//fmt.Println("SLEEPING!!!!!!!!")
	//time.Sleep(30 * time.Second)
	//util.LogErr(gr.cleanParent(ctx))
	//fmt.Println("SLEEPING!!!!!!!!")
	//time.Sleep(30 * time.Second)
	//sklog.Fatal("REACHED THE ENDPOINT")
	return -1, nil
