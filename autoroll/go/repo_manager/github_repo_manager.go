...
git log {{.RollingFrom}}..{{.RollingTo}} --no-merges --oneline

{{if .IncludeLog}}
{{range .Revisions}}
{{.String}} {{.Author}} {{.Timestamp}} {{.Description}}
{{end}}
{{end}}{{if len .TransitiveDeps}}
...
func (rm *githubRepoManager) CreateNewRoll(ctx context.Context, from, to *revision.Revision, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	commits, err := rm.childRepo.RevList(ctx, "--no-merges", fmt.Sprintf("%s..%s", from, to))
	if err != nil {
	...
	for i := len(details) - 1; i >= 0; i-- {
		...
		// Commit.
		if _, err := rm.parentRepo.Git(ctx, "commit", "-a", "-m", details[i].Body); err != nil {
			return 0, err
		}
	}
