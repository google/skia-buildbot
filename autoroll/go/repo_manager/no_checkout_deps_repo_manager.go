...
func (rm *noCheckoutDEPSRepoManager) createRoll(ctx context.Context, from, to, serverURL, cqExtraTrybots string, emails []string) (string, map[string]string, error) {
	...
	if len(rm.transitiveDeps) > 0 {
		...
		if len(updated) > 0 {
			extraStr = fmt.Sprintf("Also rolling transitive DEPS:\n%s", strings.Join(updated, "\n"))
		}
	}
	...
	commitMsg, err := buildCommitMsg(from, to, rm.childPath, cqExtraTrybots, rm.childRepoUrl, rm.serverURL, logStr, extraStr, bugs, len(nextRollCommits), rm.includeLog)
	if err != nil {
