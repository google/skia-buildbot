...
var (
	...
	NewDEPSRepoManager func(string, string, string, time.Duration, string, *gerrit.Gerrit) (*depsRepoManager, error) = newDEPSRepoManager
...
func newDEPSRepoManager(workdir, parentRepo, childPath string, frequency time.Duration, depot_tools string, g *gerrit.Gerrit) (*depsRepoManager, error) {
	gclient := GCLIENT
	rollDep := ROLL_DEP
	if depot_tools != "" {
