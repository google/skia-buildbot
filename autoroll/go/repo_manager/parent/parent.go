...
// ParentConfig provides configuration for a Parent.
type ParentConfig struct {
	// Exactly one of these should be defined.
	GitilesDEPS *GitilesDEPSConfig `json:"gitilesDeps"`
}

func (c ParentConfig) Validate() error {
	count := 0
	if c.GitilesDEPS != nil {
		count++
		if err := c.GitilesDEPS.Validate(); err != nil {
