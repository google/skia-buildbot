...
import (
	...
	"go.skia.org/infra/autoroll/go/repo_manager/common"
...
func NewFreeTypeParent(ctx context.Context, c GitilesDEPSConfig, reg *config_vars.Registry, workdir string, client *http.Client, serverURL string) (*gitilesParent, error) {
	...
	getChangesHelper := gitilesDEPSGetChangesForRollFunc(c.Dep, c.TransitiveDeps)
	getChangesForRoll := func(ctx context.Context, parentRepo *common.GitilesRepo, baseCommit string, from, to *revision.Revision, rolling []*revision.Revision) (map[string]string, []*TransitiveDep, error) {
...
func mergeInclude(ctx context.Context, include, from, to, baseCommit string, changes map[string]string, parentRepo *common.GitilesRepo, localChildRepo *git.Repo) error {
