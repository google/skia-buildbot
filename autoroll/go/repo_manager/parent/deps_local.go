...
func NewDEPSLocal(ctx context.Context, c DEPSLocalConfig, reg *config_vars.Registry, client *http.Client, serverURL, workdir, recipeCfgFile string) (*GitCheckoutParent, error) {
	...
	// See documentation for GitCheckoutGetLastRollRevFunc.
	getLastRollRev := VersionFileGetLastRollRevFunc(deps_parser.DepsFileName, c.Dep)
	...
	co := &git.Checkout{GitDir: git.GitDir(filepath.Join(workdir, repoRelPath))}
	dumpHooks(co)
	return NewGitCheckoutGerrit(ctx, c.GitCheckoutGerritConfig, reg, client, serverURL, workdir, co, getLastRollRev, createRoll)
}

func dumpHooks(co *git.Checkout) {
	sklog.Errorf("Hooks:")
	contents, err := ioutil.ReadDir(filepath.Join(co.Dir(), ".git", "hooks"))
	if err != nil {
		panic(err)
	}
