...
func NewCopy(ctx context.Context, cfg CopyConfig, reg *config_vars.Registry, client *http.Client, serverURL, workdir, userName, userEmail string, dep child.Child, uploadRoll git_common.UploadRollFunc) (*gitilesParent, error) {
	...
	getContentsAtRev := func(ctx context.Context, c child.Child, rev *revision.Revision) (map[string]string, error) {
		rv := map[string]string{}
		for _, cp := range cfg.Copies {
			if err := child.Walk(ctx, c, rev, cp.SrcRelPath, func(path string, info os.FileInfo, err error) error {
				if err != nil {
					return skerr.Wrap(err)
				}
				if info.IsDir() {
				...
				contents, err := c.ReadFile(ctx, rev, path)
				if err != nil {
					return skerr.Wrap(err)
				}
				rv[path] = contents
