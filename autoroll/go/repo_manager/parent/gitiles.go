...
type gitilesGetChangesForRollFunc func(context.Context, *common.GitilesRepo, string, *revision.Revision, *revision.Revision, []*revision.Revision) (map[string]string, []*TransitiveDep, error)

// gitilesGetLastRollRevFunc finds the last-rolled child revision ID from the
// repo at the given base commit.
type gitilesGetLastRollRevFunc func(context.Context, *common.GitilesRepo, string) (string, error)
...
func newGitiles(ctx context.Context, c GitilesConfig, reg *config_vars.Registry, client *http.Client, serverURL string, update gitilesGetLastRollRevFunc, getChangesForRoll gitilesGetChangesForRollFunc) (*gitilesParent, error) {
	...
	gr, err := common.NewGitilesRepo(ctx, c.GitilesConfig, reg, client)
	if err != nil {
		return nil, skerr.Wrap(err)
	}
