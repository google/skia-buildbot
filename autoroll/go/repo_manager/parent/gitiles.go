...
type gitilesGetChangesForRollFunc func(context.Context, *gitiles.Repo, string, *revision.Revision, *revision.Revision, []*revision.Revision) (map[string]string, error)

type gitilesUpdateFunc func(context.Context, *gitiles.Repo, string) (string, error)

// newGitiles returns a base for implementations of Parent which use Gitiles.
func newGitiles(ctx context.Context, c GitilesConfig, client *http.Client, serverURL string, update gitilesUpdateFunc, getChangesForRoll gitilesGetChangesForRollFunc) (Parent, error) {
...
func (p *gitilesParent) Update(ctx context.Context) (string, error) {
	...
	baseCommit, err := p.repo.Details(ctx, p.branch)
	if err != nil {
		return "", err
	}

	// Call the provided gitilesUpdateFunc.
...
func (p *gitilesParent) CreateNewRoll(ctx context.Context, from, to *revision.Revision, rolling []*revision.Revision, emails []string, cqExtraTrybots string, dryRun bool, commitMsg string) (int64, error) {
	...
	nextRollChanges, err := p.getChangesForRoll(ctx, p.repo, p.baseCommit, from, to, rolling)
	if err != nil {
