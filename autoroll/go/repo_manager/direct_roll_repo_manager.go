...
const (
	DIRECT_COMMIT_MSG = `Roll %s %s (%d commits)
...
%s
`
	DIRECT_VERSION_HASH_FILE = "dep.hash"
)

var (
	// Use this function to instantiate a RepoManager. This is able to be
	// overridden for testing.
	NewDirectRepoManager func(context.Context, *DirectRepoManagerConfig, string, *gerrit.Gerrit, string, string) (RepoManager, error) = newDirectRepoManager
)

// DirectRepoManagerConfig provides configuration for the direct
// RepoManager.
type DirectRepoManagerConfig struct {
...
// Validate the config.
func (c *DirectRepoManagerConfig) Validate() error {
	if c.ChildRepo == "" {
...
type directRepoManager struct {
	*depotToolsRepoManager
	childRepoUrl string
	versionFile  string
}

// newDirectRepoManager returns a RepoManager instance which rolls a dependency
// which is checked directly into a subdir of the parent repo.
func newDirectRepoManager(ctx context.Context, c *DirectRepoManagerConfig, workdir string, g *gerrit.Gerrit, recipeCfgFile, serverURL string) (RepoManager, error) {
	...
	rm := &directRepoManager{
		depotToolsRepoManager: drm,
		childRepoUrl:          c.ChildRepo,
		versionFile:           path.Join(drm.childDir, DIRECT_VERSION_HASH_FILE),
	}
...
func (rm *directRepoManager) Update(ctx context.Context) error {
	// Sync the projects.
	rm.repoMtx.Lock()
...
func (rm *directRepoManager) CreateNewRoll(ctx context.Context, from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	rm.repoMtx.Lock()
	defer rm.repoMtx.Unlock()
	...
	if _, err := parentRepo.Git(ctx, "rm", "-r", childFullPath); err != nil {
		return 0, err
	}
	...
	// Get list of changes.
	changeSummaryBlob := ""
	if true { // TODO(borenet): Someday we might want to add IncludeLog to the config.
	...
	commitMsg := fmt.Sprintf(DIRECT_COMMIT_MSG, rm.childPath, commitRange, len(commits), rm.childRepoUrl, commitRange, changeSummaryBlob, fmt.Sprintf(COMMIT_MSG_FOOTER_TMPL, rm.serverURL))
	if cqExtraTrybots != "" {
