...
func newAndroidRepoManager(ctx context.Context, workdir, parentBranch, childPath, childBranch string, g gerrit.GerritInterface, strategy NextRollStrategy, preUploadStepNames []string, serverURL string) (RepoManager, error) {
	...
	childRepo := git.NewCheckoutNoSync(ctx, childDir)
...
func (r *androidRepoManager) CreateNewRoll(from, to string, emails []string, cqExtraTrybots string, dryRun bool) (int64, error) {
	...
	if _, err := r.exec.RunCwd(r.childDir, "git", "merge", to, "--no-commit"); err != nil {
	...
	_, gnToBpErr := r.exec.RunCommand(&exec.Command{
		Env:  gnEnv,
		Dir:  r.childDir,
		Name: "python",
...
`, r.childPath, commitRange, len(commits), childRepoName, childRepoName, commitRange, strings.Join(changeSummaries, "\n"), fmt.Sprintf(COMMIT_MSG_FOOTER_TMPL, r.serverURL))
	...
	if _, uploadErr := r.exec.RunCommand(uploadCommand); uploadErr != nil {
		util.LogErr(r.abandonRepoBranch())
