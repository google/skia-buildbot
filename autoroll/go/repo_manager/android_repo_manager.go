...
var (
	...
	FILES_GENERATED_BY_GN_TO_GP = []string{android_skia_checkout.SkUserConfigAndroidRelPath, android_skia_checkout.SkUserConfigLinuxRelPath, android_skia_checkout.SkUserConfigMacRelPath, android_skia_checkout.AndroidBpRelPath}
...
func newAndroidRepoManager(ctx context.Context, c *AndroidRepoManagerConfig, workdir string, g gerrit.GerritInterface, serverURL, serviceAccount string, client *http.Client, gerritConfig *codereview.GerritConfig, local bool) (RepoManager, error) {
	...
	crm, err := newCommonRepoManager(c.CommonRepoManagerConfig, wd, serverURL, g, client, local)
	if err != nil {
		return nil, err
	}
...
// See documentation for RepoManager interface.
func (r *androidRepoManager) RetrieveRoll(ctx context.Context, recent *recent_rolls.RecentRolls, issue int64, cb func(context.Context, roll.RollImpl) error) (roll.RollImpl, error) {
	return roll.NewGerritAndroidRoll(ctx, r.g, r.FullChildHash, recent, issue, cb)
}
