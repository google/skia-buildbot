...
func (r *androidRepoManager) getCommitsNotRolled(ctx context.Context, lastRollRev string) ([]*revision.Revision, error) {
	...
	childRepoUrl, err := r.childRepo.Git(ctx, "remote", "get-url", UPSTREAM_REMOTE_NAME)
	if err != nil {
		return nil, err
	}
	return revision.FromLongCommits(fmt.Sprintf(r.revTmpl, strings.TrimSpace(childRepoUrl), "%s"), notRolled), nil
}
