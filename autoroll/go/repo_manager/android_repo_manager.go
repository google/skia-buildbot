...
var (
	...
	NewAndroidRepoManager func(string, string, time.Duration, *gerrit.Gerrit) (RepoManager, error) = newAndroidRepoManager
...
func newAndroidRepoManager(workdir, childPath string, frequency time.Duration, g *gerrit.Gerrit) (RepoManager, error) {
	...
	repoUrl := strings.Replace(g.Url(0), "-review", "", 1)
...
func (r *androidRepoManager) update() error {
	...
	if _, err := exec.RunCwd(r.childRepo.Dir(), "git", "config", "remote.goog.review", r.repoUrl); err != nil {
		return err
	}
...
func (r *androidRepoManager) getLastRollRev() (string, error) {
	...
	fmt.Println(commitLines[0])
	fmt.Println(commitLines[1])
	fmt.Println(len(commitLines))
	fmt.Println(indexWithMergeCommit)
