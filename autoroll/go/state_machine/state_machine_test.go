...
// A completely mocked implementation of AutoRollImpl.
type TestAutoRollImpl struct {
	t            *testing.T
	closedStatus string
	closedMsg    string
	isDryRun     bool
...
// Return a TestAutoRollImpl instance.
func NewTestAutoRollImpl(t *testing.T, rollingTo string, isDryRun bool) *TestAutoRollImpl {
	return &TestAutoRollImpl{
		t:         t,
		isDryRun:  isDryRun,
		rollingTo: rollingTo,
	}
...
// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) AddComment(string) error {
	return nil
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) Close(status, msg string) error {
...
func (r *TestAutoRollImpl) AssertClosed(status string) {
	assert.Equal(r.t, status, r.closedStatus)
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) IsFinished() bool {
	return r.normalResult != ""
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) IsSuccess() bool {
	return r.normalResult == "SUCCESS"
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) IsDryRunFinished() bool {
	return r.dryRunResult != ""
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) IsDryRunSuccess() bool {
	return r.dryRunResult == "SUCCESS"
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) RollingTo() string {
	return r.rollingTo
}

// Mark the roll as a success.
func (r *TestAutoRollImpl) SetSucceeded() {
	r.normalResult = "SUCCESS"
}

// Mark the roll as a failure.
func (r *TestAutoRollImpl) SetFailed() {
	r.normalResult = "FAILED"
}

// Mark the roll as a success.
func (r *TestAutoRollImpl) SetDryRunSucceeded() {
	r.dryRunResult = "SUCCESS"
}

// Mark the roll as a failure.
func (r *TestAutoRollImpl) SetDryRunFailed() {
	r.dryRunResult = "FAILED"
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) SwitchToDryRun() error {
	r.isDryRun = true
	return nil
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) SwitchToNormal() error {
	r.isDryRun = false
	return nil
}

// Assert that the roll is in dry run mode.
func (r *TestAutoRollImpl) AssertDryRun() {
	assert.True(r.t, r.isDryRun)
}

// Assert that the roll is not in dry run mode.
func (r *TestAutoRollImpl) AssertNotDryRun() {
	assert.False(r.t, r.isDryRun)
}

// See documentation for AutoRollImpl.
func (r *TestAutoRollImpl) Update() error {
	return nil
}
...
type TestAutoRollerImpl struct {
	...
	createNewRollResult *TestAutoRollImpl
	createNewRollError  error

	getActiveRollResult *TestAutoRollImpl
...
func (r *TestAutoRollerImpl) UploadNewRoll(from, to string, dryRun bool) error {
	...
	if r.getActiveRollResult == nil {
		r.getActiveRollResult = NewTestAutoRollImpl(r.t, to, dryRun)
	}
...
// an empty TestAutoRollImpl will be used.
func (r *TestAutoRollerImpl) SetUploadNewRollResult(roll *TestAutoRollImpl, err error) {
	r.createNewRollResult = roll
	r.createNewRollError = err
}

// See documentation for AutoRollerImpl.
func (r *TestAutoRollerImpl) GetActiveRoll() AutoRollImpl {
...
func (r *TestAutoRollerImpl) SetActiveRoll(roll *TestAutoRollImpl) {
...
func checkState(t *testing.T, sm state_machine.StateMachine, wanted string) {
	assert.Equal(t, state_machine.State(wanted), sm.Current())
}

// Perform a state transition and assert that we ended up in the given state.
func checkNextState(t *testing.T, sm state_machine.StateMachine, wanted string) {
	assert.NoError(t, sm.NextTransition())
	assert.Equal(t, state_machine.State(wanted), sm.Current())
}

// Shared setup.
func setup(t *testing.T) (state_machine.StateMachine, *TestAutoRollerImpl, func()) {
...
func TestNormal(t *testing.T) {
	...
	roll := r.GetActiveRoll().(*TestAutoRollImpl)
...
func testThrottle(t *testing.T, mode string) {
	...
	// Rather than waiting for the time window to pass, recreate everything
	// to obtain a new AutoDecrementCounter to fake it, assuming that the
	// counter works as it should.
	assert.NoError(t, ioutil.WriteFile(path.Join(workdir, "attempt_counter"), []byte("0"), os.ModePerm))
	sm2, err := New(r, workdir)
	assert.Equal(t, state_machine.State(throttled), sm2.Current())
	idle := S_NORMAL_IDLE
	if mode == autoroll_modes.MODE_DRY_RUN {
		idle = S_DRY_RUN_IDLE
	}
