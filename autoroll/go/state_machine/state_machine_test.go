...
func checkState(t *testing.T, sm *state_machine.PersistentStateMachine, wanted string) {
...
func checkNextState(t *testing.T, sm *state_machine.PersistentStateMachine, wanted string) {
	assert.NoError(t, sm.NextTransition())
	assert.Equal(t, state_machine.State(wanted), sm.Current())
}

// Shared setup.
func setup(t *testing.T) (*state_machine.PersistentStateMachine, *TestAutoRollerImpl, func()) {
...
func testThrottle(t *testing.T, mode string) {
	...
	if mode == autoroll_modes.MODE_DRY_RUN {
		checkNextState(t, sm, S_DRY_RUN_THROTTLED)
	} else {
		checkNextState(t, sm, S_NORMAL_THROTTLED)
	}
}
