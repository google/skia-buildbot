...
type RollCLImpl interface {
	...
	// RemoveFromCQ removes commit-queue labels set by the roller.
	RemoveFromCQ(context.Context) error
...
func New(ctx context.Context, impl AutoRollerImpl, n *notifier.AutoRollNotifier, gcsClient gcs.GCSClient, gcsPrefix string) (*AutoRollStateMachine, error) {
	...
	f(F_HUMAN_INTERVENED, func(ctx context.Context, roll RollCLImpl) error {
		if err := roll.RemoveFromCQ(ctx); err != nil {
	...
	b.T(S_DRY_RUN_SUCCESS_LEAVING_OPEN, S_DRY_RUN_SUCCESS_LEAVING_OPEN, F_UPDATE_ROLL)
