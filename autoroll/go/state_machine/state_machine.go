	...
	case S_DRY_RUN_ACTIVE:
		...
		if currentRoll.IsClosed() {
			if currentRoll.IsSuccess() {
				...
				// Someone closed the roll but the dry run was successful so
				// return dry run success.
				return S_DRY_RUN_SUCCESS, nil
			} else {
				// Someone closed the roll and the dry run had failed or had not
				// completed yet so return dry run failure.
