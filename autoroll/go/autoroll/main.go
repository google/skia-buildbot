...
import (
	...
	"go.skia.org/infra/go/quitquitquit"
...
var (
	...
	quitPort        = flag.String("quit_port", ":8001", "Port for /quitquitquit endpoint.")
...
func main() {
	...
	// Start the roller.
	quitquitquit.Init(*quitPort, context.Background())
	defer quitquitquit.Cancel()
	arb.Start(time.Minute /* tickFrequency */, 15*time.Minute /* repoFrequency */)

	// Feed AutoRoll stats into metrics.
	quitquitquit.Repeat(time.Minute, func() {
	...
	quitquitquit.Repeat(30*time.Minute, func() {
		emails, err := getSheriff()
		if err != nil {
