...
func main() {
	...
	if cfg.RollerType != "android" && cfg.RollerType != "google3" {
	...
	if cfg.RollerType == "google3" {
		arb, err = google3.NewAutoRoller(ctx, *workdir, common.REPO_SKIA, cfg.ChildBranch)
	} else {
		arb, err = roller.NewAutoRoller(ctx, cfg, depotTools, emailer, g, *workdir)
	}
	if err != nil {
		sklog.Fatal(err)
	}
	...
	cleanup.Repeat(time.Minute, func() {
		...
		metrics2.GetInt64Metric("autoroll_last_roll_result", map[string]string{"child_path": cfg.ChildPath}).Update(v)
	}, nil)
