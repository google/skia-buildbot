...
import (
	...
	"go.skia.org/infra/go/repeat_cleanup"
...
func main() {
	...
	// Start the roller.
	repeat_cleanup.Init(context.Background())
	defer repeat_cleanup.Cancel()
	arb.Start(time.Minute /* tickFrequency */, 15*time.Minute /* repoFrequency */)

	// Feed AutoRoll stats into metrics.
	repeat_cleanup.Repeat(time.Minute, func() {
	...
	repeat_cleanup.Repeat(30*time.Minute, func() {
		emails, err := getSheriff()
		if err != nil {
