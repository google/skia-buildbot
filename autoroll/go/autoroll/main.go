...
func main() {
	...
	if cfg.RollerType() != roller.ROLLER_TYPE_ANDROID && cfg.RollerType() != roller.ROLLER_TYPE_GOOGLE3 {
	...
	if cfg.RollerType() == roller.ROLLER_TYPE_GOOGLE3 {
		...
		arb, err = roller.NewAutoRoller(ctx, cfg, depotTools, emailer, g, *workdir, serverURL)
	}
	if err != nil {
		sklog.Fatal(err)
	}
