...
func TestNotifier(t *testing.T) {
	...
	assert.Equal(t, 1, len(t2.msgs))
	assert.Equal(t, "The childRepo into parentRepo AutoRoller mode was changed", t2.msgs[0].subject)
	assert.Equal(t, "test@skia.org changed the mode to \"STOPPED\" with message: &lt;b&gt;Staaahhp!&lt;/b&gt;"+footer, t2.msgs[0].m.Body)
	assert.Equal(t, notifier.SEVERITY_WARNING, t2.msgs[0].m.Severity)
