...
func TestNotifier(t *testing.T) {
	...
	assert.NoError(t, n.SendThrottled())
	assert.Equal(t, 3, len(t1.msgs))
	assert.Equal(t, "The childRepo into parentRepo AutoRoller is throttled!", t1.msgs[2].subject)
	assert.Equal(t, "The roller is throttled because it attempted to upload too many CLs in too short a time.", t1.msgs[2].m.Body)
