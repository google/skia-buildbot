...
func (s *autoRollServerImpl) GetStatus(ctx context.Context, req *GetStatusRequest) (*AutoRollStatus, error) {
	...
	return convertStatus(st, roller.Cfg, roller.Mode.CurrentMode(), roller.Strategy.CurrentStrategy(), manualReqs), nil
}
...
func convertStatus(st *status.AutoRollStatus, cfg *roller.AutoRollerConfig, mode *modes.ModeChange, strat *strategy.StrategyChange, manualReqs []*manual.ManualRollRequest) *AutoRollStatus {
	rv := &AutoRollStatus{
