...
// NewAutoRollServer creates and returns a GRPC server for the given autorollers.
func NewAutoRollServer(ctx context.Context, cfgs []*roller.AutoRollerConfig, manualRollDB manual.DB, viewers, editors, admins allowed.Allow) (http.Handler, error) {
	...
	srv := NewAutoRollRPCsServer(impl, &twirp.ServerHooks{
		RequestRouted: func(ctx context.Context) (context.Context, error) {
			...
			sklog.Errorf("Restrict: %s %#v", restrict, group)
...
// withLogin is middleware which attaches login info to the request context.
func withLogin(sub http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		user := login.LoggedInAs(r)
		ctx := context.WithValue(r.Context(), loginCtxKey, &user)
		sub.ServeHTTP(w, r.WithContext(ctx))
	})
}

// getLogin returns the email address of the logged-in user.
func getLogin(ctx context.Context) string {
	user := ctx.Value(loginCtxKey)
	if user != nil {
		return *user.(*string)
	}
	return ""
...
func (s *autoRollServerImpl) getRoller(roller string) (*autoroller, error) {
	...
	if !ok {
		return nil, fmt.Errorf("Unknown roller")
	}
	return rv, nil
}
...
func (s *autoRollServerImpl) View_GetRollers(ctx context.Context, req *GetRollersRequest) (*AutoRollMiniStatuses, error) {
	statuses := make([]*AutoRollMiniStatus, len(s.rollers))
	for name, roller := range s.rollers {
		statuses = append(statuses, convertMiniStatus(roller.status.GetMini(), name))
	}
...
func (s *autoRollServerImpl) View_GetMiniStatus(ctx context.Context, req *GetMiniStatusRequest) (*AutoRollMiniStatus, error) {
	...
	return convertMiniStatus(roller.status.GetMini(), req.Roller), nil
}
...
func (s *autoRollServerImpl) View_GetStatus(ctx context.Context, req *GetStatusRequest) (*AutoRollStatus, error) {
	...
	rv := &AutoRollStatus{
		MiniStatus:         convertMiniStatus(&st.AutoRollMiniStatus, req.Roller),
		Status:             st.Status,
...
func (s *autoRollServerImpl) Edit_SetMode(ctx context.Context, req *SetModeRequest) (*AutoRollStatus, error) {
	...
	if err := roller.mode.Add(ctx, req.Mode, req.User, req.Message); err != nil {
		// TODO(borenet): Is it safe to return the error here?
		return nil, err
	}
...
func (s *autoRollServerImpl) Edit_SetStrategy(ctx context.Context, req *SetStrategyRequest) (*AutoRollStatus, error) {
	...
	if err := roller.strategy.Add(ctx, req.Strategy, req.User, req.Message); err != nil {
		// TODO(borenet): Is it safe to return the error here?
		return nil, err
	}
	return s.View_GetStatus(ctx, &GetStatusRequest{Roller: req.Roller})
...
func convertMiniStatus(inp *status.AutoRollMiniStatus, roller string) *AutoRollMiniStatus {
