...
func TestStatus(t *testing.T) {
	...
	mockChild.MockLog(ctx, fmt.Sprintf("%s..%s", commits[0], commits[1]))
	...
	mockChild.MockGetCommit(ctx, "master")
	mockChild.MockLog(ctx, fmt.Sprintf("%s..%s", commits[0], commits[2]))
...
func TestAddOrUpdateIssue(t *testing.T) {
	...
	assert.NoError(t, a.AddOrUpdateIssue(ctx, issue2, http.MethodPut))
	mockChild.MockGetCommit(ctx, "master")
	mockChild.MockLog(ctx, fmt.Sprintf("%s..%s", commits[1], commits[2]))
	assert.NoError(t, a.UpdateStatus(ctx, "", true))
	...
	mockChild.MockGetCommit(ctx, "master")
	mockChild.MockLog(ctx, fmt.Sprintf("%s..%s", commits[2], commits[2]))
	assert.NoError(t, a.UpdateStatus(ctx, "", true))
	issue4.Closed = true
