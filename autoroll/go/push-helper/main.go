...
// variables.
func kubeConfGen(ctx context.Context, tmpl, srcConfig, dstConfig string, extraVars map[string]string) error {
	cmd := []string{
...
// instance.
func kubeConfGenBe(ctx context.Context, tmpl, srcConfig, dstConfig, configFileHash, configMapName, image string) error {
...
// frontend instance.
func kubeConfGenFe(ctx context.Context, tmpl, srcConfig, dstConfig, configDirHash, configMapName, image string) error {
	return kubeConfGen(ctx, tmpl, srcConfig, dstConfig, map[string]string{
...
func updateConfigs(ctx context.Context, project, configDir, k8sConfigDir, configMapName, configFileFe, latestImageFe, latestImageBe string, updateWhitelist map[string]bool) (rvErr error) {
	...
	if err := kubeConfGenFe(ctx, tmplFe, configFileFe, dstFe, dirSum, configMapName, imageFe); err != nil {
		return err
	}
	...
	for cfg, sum := range cfgSums {
		...
		if err := kubeConfGenBe(ctx, tmplBe, cfg, dst, sum, configMapName, image); err != nil {
			return err
		}
