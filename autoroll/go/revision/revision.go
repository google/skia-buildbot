...
func FromLongCommit(repoUrl string, c *vcsinfo.LongCommit) *Revision {
	return &Revision{
		Id:          c.Hash,
		Display:     c.Hash[:7],
		Description: c.Subject,
		Timestamp:   c.Timestamp,
		URL:         fmt.Sprintf("%s/+/%s", repoUrl, c.Hash),
	}
}

// FromLongCommits converts a slice of vcsinfo.LongCommits to a slice of
// Revisions.
func FromLongCommits(repoUrl string, commits []*vcsinfo.LongCommit) []*Revision {
	rv := make([]*Revision, 0, len(commits))
	for _, c := range commits {
		rv = append(rv, FromLongCommit(repoUrl, c))
	}
	return rv
}
