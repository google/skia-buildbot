...
func statusJsonHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !login.IsGoogler(r) {
		status.Error = ""
		if roller.Cfg.ParentIsInternalOnly {
...
func miniStatusJsonHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Overwrite the mode in the status object in case it has been updated
	// on the front end but the roller has not cycled to reflect the change
...
func runServer(ctx context.Context, serverURL string) {
	...
	if !*local {
		for _, roller := range rollers {
			if strings.Contains(roller.Cfg.RollerName, "google3") {
				arb, err := google3.NewAutoRoller(ctx, *workdir, roller.Cfg)
				if err != nil {
					sklog.Fatal(err)
				}
				arb.AddHandlers(r.PathPrefix("/google3").Subrouter())
