...
func runServer(ctx context.Context, serverURL string, cfgs []*roller.AutoRollerConfig, manualRollDB manual.DB) {
	...
	r.PathPrefix(rpc.AutoRollRPCsPathPrefix).Handler(rpcs)
...
func main() {
	...
	// Create the server.
	runServer(ctx, serverURL, cfgs, manualRollDB)
}
