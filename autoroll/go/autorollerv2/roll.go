...
func (r *gerritRoll) Close(result, msg string) error {
	...
	if err := r.g.Abandon(r.ci, msg); err != nil {
		...
		if err2 := r.Update(); err2 != nil {
			return fmt.Errorf("Failed to close the CL with error:\n%s\nAnd failed to update it with error:\n%s", err, err2)
		}
		if r.ci.IsClosed() {
			sklog.Errorf("Attempted to close the CL but it is already closed! Error: %s", err)
			return nil
		}
		return err
	}
...
func (r *gerritRoll) SwitchToDryRun() error {
	if err := r.g.SendToDryRun(r.ci, "Mode was changed to dry run"); err != nil {
		// It's possible that somebody abandoned the CL (or the CL
		// landed) while we were working. If that's the case, log an
		// error and move on.
		if err2 := r.Update(); err2 != nil {
			return fmt.Errorf("Failed to switch the CL to dry run with error:\n%s\nAnd failed to update it with error:\n%s", err, err2)
		}
		if r.ci.IsClosed() {
			sklog.Errorf("Attempted to switch the CL to dry run but it is already closed! Error: %s", err)
			return nil
		}
		return err
	}
...
func (r *gerritRoll) SwitchToNormal() error {
	if err := r.g.SendToCQ(r.ci, "Mode was changed to normal"); err != nil {
		// It's possible that somebody abandoned the CL (or the CL
		// landed) while we were working. If that's the case, log an
		// error and move on.
		if err2 := r.Update(); err2 != nil {
			return fmt.Errorf("Failed to switch the CL out of dry run with error:\n%s\nAnd failed to update it with error:\n%s", err, err2)
		}
		if r.ci.IsClosed() {
			sklog.Errorf("Attempted to switch the CL out of dry run but it is already closed! Error: %s", err)
			return nil
		}
		return err
	}
