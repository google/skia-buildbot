...
func NewModeHistory(ctx context.Context, roller, dbFile string) (*ModeHistory, error) {
	...
	if history, err := mh.getHistory(ctx); err != nil {
		...
		sklog.Warningf("Ingesting all mode change history into new DB from %s", dbFile)
		d, err := openDB(dbFile)
		if err != nil {
			return nil, err
		}
		...
		for _, mc := range allEntries {
			...
			if err := mh.put(ctx, mc); err != nil {
				return nil, fmt.Errorf("Failed to ingest old mode change history into new DB: %s", err)
			}
		}
	}
...
// put inserts the ModeChange into the database.
...
// getHistory retrieves recent mode changes from the DB.
...
func (mh *ModeHistory) refreshHistory(ctx context.Context) error {
	...
	if len(history) == 0 {
		sklog.Infof("Setting initial mode.")
