    ...
    <div class="status">
      <div id="loadstatus">
        ...
        <input
            ...
            ></input>
        Last loaded at <span>${ele.lastLoaded}</span>
      </div>
      <table>
            ...
            <table>
              ...
              ${status.recent.map((roll: Roll) => html`
                <tr>
                  ...
                  <td><human-date-sk date="${roll.modified}" diff></human-date-sk> ago</td>
        ...
        ${status.config.supportsManualRolls ? html`
          ...
          ${ele.rollCandidates.map((rollCandidate) => html`
            <tr class="rollCandidate">
              ...
              <td><human-date-sk date="${rollCandidate.revision.timestamp}"></human-date-sk></td>
              <td>${rollCandidate.roll ? rollCandidate.roll.requester : html``}</td>
              <td>${rollCandidate.roll ? html`<human-date-sk date="${rollCandidate.roll.timestamp}"></human-date-sk>` : html``}</td>
              <td>
...
class ARBStatusSk extends HTMLElement {
  ...
  _changeMode(submit: boolean) {
    ...
    fetch(window.location.pathname + "/json/mode", {
      ...
      this._modeChangePending = false;
      errorMessage("Failed to change the mode: " + err);
      this._render();
    });
  }
  ...
  _changeStrategy(submit: boolean) {
    ...
    fetch(window.location.pathname + "/json/strategy", {
      ...
      this._strategyChangePending = false;
      strategySelect.value = this._status.strategy.strategy;
      errorMessage("Failed to change the strategy: " + err);
      this._render();
    });
  }
