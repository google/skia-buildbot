...
var fakeStatus: Status = JSON.parse(String.raw`{
    "childHead": "", 
    "childName": "Skia", 
    "config": {
...
var params = toParamSet(window.location.search.substring(1));
...
fetchMock.post("/dist/arb-status-sk.html/json/mode", function(url: string, opts: fetchMock.MockOptions) {
  var res = <Mode>JSON.parse(<string><unknown>opts.body);
  ...
  if (validMode >= 0) {
    ...
    return new Response("Invalid mode: " + res.mode + "; valid modes: " + fakeStatus["validModes"], {status: 400});
  }
});
fetchMock.post("/dist/arb-status-sk.html/json/strategy", function(url: string, opts: fetchMock.MockOptions) {
  var res = <Strategy>JSON.parse(<string><unknown>opts.body);
  res["user"] = "you@google.com";
  ...
  if (validStrategy >= 0) {
    ...
    return new Response("Invalid strategy: " + res.strategy + "; valid strategies: " + fakeStatus["validStrategies"], {status: 400});
  }
});
fetchMock.post("/dist/arb-status-sk.html/json/unthrottle", {});
fetchMock.post("/dist/arb-status-sk.html/json/manual", function(url: string, opts: fetchMock.MockOptions) {
  var req = <ManualRollRequest>JSON.parse(<string><unknown>opts.body);
