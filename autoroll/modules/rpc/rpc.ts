...
const JSONToTryJob = (m: TryJobJSON): TryJob => {
  return {
    name: m.name || "",
    status: m.status as TryJob_Status,
    result: m.result as TryJob_Result,
    url: m.url || "",
    category: m.category || "",
  };
};
...
const JSONToAutoRollCL = (m: AutoRollCLJSON): AutoRollCL => {
  return {
    id: m.id || "",
    result: m.result as AutoRollCL_Result,
    subject: m.subject || "",
...
const JSONToManualRoll = (m: ManualRollJSON): ManualRoll => {
  return {
    ...
    result: m.result as ManualRoll_Result,
    status: m.status as ManualRoll_Status,
