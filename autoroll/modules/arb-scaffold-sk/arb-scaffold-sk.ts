...
class ARBScaffoldSk extends ElementSk {
  private _main: HTMLElement | null;
  private _busyTaskCount: number;
  private _taskQueueLength: number;
  private _spinner: SpinnerSk | null;
  ...
  /** @prop loginHost {string} Host name used for login. */
  get loginHost() { return window.location.host; }

  /** @prop taskQueueLength {number} Indicates the number of tasks in the queue. */
  get taskQueueLength() { return this._taskQueueLength; }

  /** @prop title {string} Reflects the app_title attribute for ease of use. */
