...
class FakeAutoRollService implements AutoRollService {
  ...
  setMode(req: SetModeRequest): Promise<SetModeResponse> {
    return new Promise((resolve, reject) => {
      ...
      const mc: ModeChange = {
        ...
        mode: req.mode,
        user: "you@google.com",
        time: new Date().toUTCString(), // TODO(borenet): Is this the right format?
        message: req.message,
      };
      this.status.mode = mc;
      resolve({
  ...
  createManualRoll(req: CreateManualRollRequest): Promise<CreateManualRollResponse> {
    ...
    const rv: ManualRoll = {
      ...
      timestamp: new Date("2017-08-28T03:51:10Z").toUTCString(), // TODO(borenet): Is this the right format?
