...
class SlideData {
  ...
  private onChange: (data: any | null) => void;

  constructor(label: string, helpString: string, required: boolean, onChange: (data: any | null) => void, tmpl: (_: SlideData) => TemplateResult) {
    this._label = label;
...
class StringSlideData extends SlideData {
  constructor(label: string, helpString: string, required: boolean, onChange: (data: any | null) => void) {
    super(label, helpString, required, onChange, (slideData: SlideData) => html`
      <input
        type="text"
...
class EnumSlideData extends SlideData {
  private static template = (slideData: SlideData) => html`
  <select @change=${(ev: InputEvent) => {
      const target = <HTMLSelectElement>ev.target;
      (<EnumSlideData>slideData).data = (<EnumSlideData>slideData).options[target.selectedIndex - 1];
    }}>
    <option hidden ?selected=${slideData.wrappedData == null}>
      Choose an option...
    </option>
    ${repeat((<EnumSlideData>slideData).options,
      (option: string, index: number) => option + "_" + index,
      (option: string, index: number) => html`
        <option ?selected=${slideData.wrappedData == option}>
          ${option}
        </option>
      `)
    }
...
`
  ...
  constructor(label: string, helpString: string, required: boolean, options: string[], onChange: (data: any | null) => void) {
    super(label, helpString, required, onChange, EnumSlideData.template);
...
class Slide {
  ...
  clear() {
    this._data.forEach((data: SlideData) => {
...
class ARBSlidesSk extends ElementSk {
  private static template = (slideSet: ARBSlidesSk) => html`
    <div>
      <button
    ...
    <div>
      ${repeat(slideSet.slides,
      (_: Slide, index: number) => "slide_" + index,
      (_: Slide, index: number) => html`
        <div class="${index == slideSet.currentIndex ? "" : "nodisplay"}">
          ${slideSet.slides[index].render()}
        </div>
      `)}
    </div>
  `;
  ...
  constructor() {
    super(ARBSlidesSk.template);
    const rollMethodSlideData = new EnumSlideData(
      "Roll Method",
      "Describes how the child project is rolled into the parent.",
      true,
      Object.values(rollMethod),
      () => {
        this._render();
      },
    );
    const childProjectSlideData = new EnumSlideData(
      "Child Project Type",
      "Type of project being rolled.",
      true,
    ...
    this.slides = [
      new Slide("Parent Project", [
        new EnumSlideData(
          "Parent Project Type",
          "Type of project which the roller will roll into.",
          true,
          Object.values(parentType),
          (data: any | null) => {
            rollMethodSlideData.options = Object.values(rollMethod).filter((value: rollMethod) => {
              if (!data) {
                return true;
              }
              return rollMethodSupported(<parentType>data, value);
            });
            childProjectSlideData.options = Object.values(childType).filter((value: childType) => {
              if (!data) {
                return true;
              }
              return childTypeSupported(<parentType>data, value);
            })
            this._render();
          },
        ),
      ...
      new Slide("Child Project", [childProjectSlideData]),
    ];
  }
  ...
  set currentIndex(index: number) {
    ...
    console.log("Navigating to slide " + this._currentIndex);

    // Clear all subsequent slides, in case we've moved backward.
    this.slides.forEach((slide: Slide, index: number) => {
  ...
  canMoveForward(): boolean {
    const rv = (this.currentSlide ? this.currentSlide.complete : false) && this.currentIndex < this.slides.length - 1;
