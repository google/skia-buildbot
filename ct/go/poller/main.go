...
func pollAndExecOnce(ctx context.Context, getPatchFunc GetPatchFunc) *sync.WaitGroup {
	fmt.Printf("XXXXXXXX before get oldest pending task %d", runtime.NumGoroutine())
	pending, err := frontend.GetOldestPendingTaskV2()
	fmt.Printf("XXXXXXXX after get oldest pending task %d", runtime.NumGoroutine())
...
func main() {
	...
	fmt.Printf("XXXXXXXX in poller %d", runtime.NumGoroutine())
	for range time.Tick(*pollInterval) {
		fmt.Printf("XXXXXXXX entering poller %d", runtime.NumGoroutine())
		healthyGauge.Update(1)
		fmt.Printf("XXXXXXXX after update in poller %d", runtime.NumGoroutine())
		pollAndExecOnce(ctx, ctutil.GetPatchFromStorage)
		fmt.Printf("XXXXXXXX in poller after poll and exec once %d", runtime.NumGoroutine())
