...
func getRunId(t *testing.T, cmd *skexec.Command) string {
...
func TestChromiumPerfExecute(t *testing.T) {
	...
	mockExec := skexec_testutils.Mock{}
	exec.SetRun(mockExec.Run)
	defer exec.Reset()
	mockExec.SetDefaultRun(func(cmd *skexec.Command) error {
	...
	err := task.Execute()
	assert.NoError(t, err)
	assert.Len(t, mockExec.Commands(), 1)
...
func TestPollAndExecOnce(t *testing.T) {
	...
	expect.Equal(t, "git pull", skexec.DebugString(commands[0]))
	expect.Equal(t, "make all", skexec.DebugString(commands[1]))
...
func TestPollAndExecOnceMultipleTasks(t *testing.T) {
	...
	expect.Equal(t, "make all", skexec.DebugString(commands[1]))
	expect.Equal(t, "capture_archives_on_workers", commands[2].Name)
	expect.Equal(t, "git pull", skexec.DebugString(commands[3]))
	expect.Equal(t, "make all", skexec.DebugString(commands[4]))
...
func TestPollAndExecOnceError(t *testing.T) {
	...
	exec.SetRun(mockExec.Run)
	defer exec.Reset()
	mockExec.AddRule("capture_archives_on_workers", "", fmt.Errorf("workers too lazy"))
	wg := pollAndExecOnce()
	wg.Wait()
	// Expect only one poll.
	expect.Equal(t, 1, mockServer.OldestPendingTaskReqCount())
