...
func getUsagePerf(ctx context.Context) {
	...
	query := ds.NewQuery(kind).Filter("TsAdded >", AFTER_Q3)
...
func getUsageCorrectness(ctx context.Context) {
	kind := ds.PIXEL_DIFF_TASKS

	query := ds.NewQuery(kind).Filter("TsAdded >", AFTER_Q3)
	var results []*pixel_diff.DatastoreTask
	_, err := ds.DS.GetAll(ctx, query, &results)
	if err != nil {
		sklog.Fatal(err)
	}
