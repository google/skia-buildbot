...
func getUsagePerf(ctx context.Context) {
	...
	var results []*chromium_perf.ChromiumPerfDatastoreTask
...
func getUsageAnalysis(ctx context.Context) {
	...
	query := ds.NewQuery(kind).Filter("TsAdded >", AFTER_Q1)
	var results []*chromium_analysis.ChromiumAnalysisDatastoreTask
	_, err := ds.DS.GetAll(ctx, query, &results)
	if err != nil {
		sklog.Fatal(err)
	}
...
func getUsageMetricsAnalysis(ctx context.Context) {
	...
	query := ds.NewQuery(kind).Filter("TsAdded >", AFTER_Q1)
	var results []*metrics_analysis.MetricsAnalysisDatastoreTask
	_, err := ds.DS.GetAll(ctx, query, &results)
	if err != nil {
		sklog.Fatal(err)
	}
