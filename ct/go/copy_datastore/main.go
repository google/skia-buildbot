...
func main() {
	...
	// Also migrate the ClusterTelemetryIDs KIND.
	ctIDsTask := &task_common.ClusterTelemetryIDs{}
	// Delete everything from the datastore first.
	if _, err := DeleteKind(ctx, dstClient, ds.CLUSTER_TELEMETRY_IDS); err != nil {
		sklog.Fatal(err)
	}
	// Migrate data.
	q := ds.NewQuery(ds.CLUSTER_TELEMETRY_IDS)
	for t := srcClient.Run(ctx, q); ; {
		key, err := t.Next(ctIDsTask)
		if err == iterator.Done {
			break
		}
		if err != nil {
		...
		_, err = dstClient.Put(ctx, key, ctIDsTask)
		sklog.Infof("%s: %s", ds.CLUSTER_TELEMETRY_IDS, key)
		if err != nil {
			sklog.Fatal(err)
		}
	}
