...
func updateTaskInDatastore(ctx context.Context, htmlOutputLink, noPatchOutputLink, withPatchOutputLink string) error {
	...
	return task_common.FindAndUpdateTask(ctx, &vars)
}
...
func runChromiumPerfOnWorkers() error {
	...
	if err := updateTaskInDatastore(ctx, htmlOutputLink, noPatchOutputLink, withPatchOutputLink); err != nil {
		return fmt.Errorf("Could not update task in datastore: %s", err)
	}
	return nil
}
