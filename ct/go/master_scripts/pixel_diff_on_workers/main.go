// pixel_diffs_on_workers is an application that captures screenshots of the
...
const (
	MAX_PAGES_PER_SWARMING_BOT_PIXEL_DIFF = 100
)
...
func sendEmail(recipients []string) {
	...
	emailSubject := fmt.Sprintf("Cluster telemetry pixel diff task has completed (%s)", *runID)
	failureHtml := ""
	viewActionMarkup := ""
	var err error
	...
	%s
	The screenshots output is <a href='%s'>here</a>.<br/>
	...
	Thanks!
	`
	emailBody := fmt.Sprintf(bodyTemplate, *pagesetType, util.GetSwarmingLogsLink(*runID), *description, failureHtml, htmlOutputLink, chromiumPatchLink, skiaPatchLink, customWebpagesLink, frontend.PixelDiffTasksWebapp)
...
func main() {
	...
	maxPages := MAX_PAGES_PER_SWARMING_BOT_PIXEL_DIFF
	...
	if _, err := util.TriggerSwarmingTask(*pagesetType, "pixel_diff", util.PIXEL_DIFF_ISOLATE, *runID, hardTimeout, ioTimeout, util.ADMIN_TASKS_PRIORITY, maxPages, numPages, isolateExtraArgs, workerDimensions, 1); err != nil {
		util.TriggerSwarmingTask(*pagesetType, "chromium_perf", util.CHROMIUM_PERF_ISOLATE, *runID, hardTimeout, 1*time.Hour, util.USER_TASKS_PRIORITY, MAX_PAGES_PER_SWARMING_BOT, numPages, isolateExtraArgs, util.GOLO_WORKER_DIMENSIONS, util.GetRepeatValue(*benchmarkExtraArgs, *repeatBenchmark))
