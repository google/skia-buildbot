...
func sendEmail(recipients []string) {
	...
	emailSubject := fmt.Sprintf("Run layout tests cluster telemetry task has completed (#%d)", *taskID)
	failureHtml := ""
	viewActionMarkup := ""
	var err error

	if taskCompletedSuccessfully {
		if viewActionMarkup, err = email.GetViewActionMarkup(pixelDiffResultsLink, "View Results", "Direct link to the results"); err != nil {
