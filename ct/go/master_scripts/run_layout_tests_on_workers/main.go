...
func main() {
	...
	if _, err := util.TriggerSwarmingTask(ctx, "" /* pagesetType */, "layout_test", util.LAYOUT_TESTS_ISOLATE, *runID, 24*time.Hour, 1*time.Hour, util.ADMIN_TASKS_PRIORITY, 1, NUM_SWARMING_BOTS_TO_RUN_ON, isolateExtraArgs, true /* runOnGCE */, 1, isolateDeps); err != nil {
	...
	for i := 1; i <= NUM_SWARMING_BOTS_TO_RUN_ON; i++ {
		...
		if err != nil {
			sklog.Errorf("Could not fetch %s: %s", gsOutputFilePath, err)
			continue
		}
		foundOneFile = true
	...
	fmt.Println("The consolidated output is available in %s", consolidatedOutputFile)
