...
var (
	...
	countStdoutText    = flag.String("count_stdout_txt", "", "Looks for the specified string in the stdout of web page runs. Every occurence of the text is counted and added to the CSV of the web page with the CT_stdout_count field")
...
func main() {
	...
	numSlaves, err := util.TriggerSwarmingTask(ctx, *pagesetType, "chromium_analysis", util.CHROMIUM_ANALYSIS_ISOLATE, *runID, 12*time.Hour, 1*time.Hour, priority, maxPagesPerBot, numPages, isolateExtraArgs, *runOnGCE, util.GetRepeatValue(*benchmarkExtraArgs, 1))
	if err != nil {
