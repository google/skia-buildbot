...
func main() {
	...
	//	return
	//}
	telemetryHash := "8b85b94e38ba021d70d0417582880904d415f941"
	fmt.Println(remotePatches)
	fmt.Println("This is telemetry hash!!!!!")
	fmt.Println(telemetryHash)
	deps := []string{telemetryHash}
	//sklog.Fatal("Do the stuff here!")
	...
	numSlaves, err := util.TriggerSwarmingTask(ctx, "" /* pagesetType */, "metrics_analysis", util.METRICS_ANALYSIS_ISOLATE, *runID, 12*time.Hour, 1*time.Hour, util.USER_TASKS_PRIORITY, MAX_PAGES_PER_SWARMING_BOT, numTraces, isolateExtraArgs, true /* runOnGCE */, util.GetRepeatValue(*benchmarkExtraArgs, 1), deps)
	if err != nil {
	...
	if strings.Contains(*benchmarkExtraArgs, "--output-format=csv") {
		if noOutputSlaves, err = util.MergeUploadCSVFiles(ctx, *runID, pathToPyFiles, gs, numTraces, MAX_PAGES_PER_SWARMING_BOT, true /* handleStrings */, util.GetRepeatValue(*benchmarkExtraArgs, 1)); err != nil {
