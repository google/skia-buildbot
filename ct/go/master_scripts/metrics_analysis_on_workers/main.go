...
func main() {
	...
	if strings.Contains(*benchmarkExtraArgs, "--output-format=csv") {
		if noOutputSlaves, err = util.MergeUploadCSVFiles(ctx, *runID, pathToPyFiles, gs, numTraces, MAX_PAGES_PER_SWARMING_BOT, true /* handleStrings */, 1 /* repeatValue */); err != nil {
...
func extractTracesFromAnalysisRun(outputPath string, gs *util.GcsUtil) error {
	...
	if err != nil {
		return fmt.Errorf("Could not read %s: %s", localCsvPath, err)
	}
	// Gather trace URLs from the CSV.
	traceURLs := []string{}
	for i := range headers {
		for j := range values {
			if headers[i] == "traceUrls" && values[j][i] != "" {
				for _, v := range strings.Split(values[j][i], ",") {
					traceURLs = append(traceURLs, v)
				}
			}
		}
	}
	if len(traceURLs) == 0 {
		return fmt.Errorf("There were no traceURLs found in %s", remoteCsvPath)
	}
