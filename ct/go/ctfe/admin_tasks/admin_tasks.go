...
func (task *AddRecreatePageSetsTaskVars) GetPopulatedDatastoreTask(ctx context.Context) (task_common.Task, error) {
	...
	id, err := task_common.GetNextId(ctx, ds.RECREATE_PAGESETS_TASKS, &RecreatePageSetsDatastoreTask{})
	if err != nil {
...
func (task *AddRecreateWebpageArchivesTaskVars) GetPopulatedDatastoreTask(ctx context.Context) (task_common.Task, error) {
	...
	id, err := task_common.GetNextId(ctx, ds.RECREATE_WEBPAGE_ARCHIVES_TASKS, &RecreateWebpageArchivesDatastoreTask{})
	if err != nil {
		return nil, fmt.Errorf("Could not get highest id: %s", err)
	}
