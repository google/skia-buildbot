...
func (task DatastoreTask) SendCompletionEmail(ctx context.Context, completedSuccessfully bool) error {
	...
	emailBody := fmt.Sprintf(bodyTemplate, ctutil.GetSwarmingLogsLink(runID), task.Description, failureHtml, task.RawOutput, chromiumPatchLink, catapultPatchLink, tracesLink, task_common.MetricsAnalysisTasksWebapp)
