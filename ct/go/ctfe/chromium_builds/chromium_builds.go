...
func getRevDataHandler(getLkgr func() (string, error), gitRepoUrl string, w http.ResponseWriter, r *http.Request) {
	...
	if revString == "" {
		httputils.ReportError(w, nil, "No revision specified")
		return
	}
	...
	if strings.EqualFold(revString, "LKGR") {
		lkgr, err := getLkgr()
		if err != nil {
			httputils.ReportError(w, nil, "Unable to retrieve LKGR revision")
		}
	...
	if err != nil {
		httputils.ReportError(w, err, "Unable to retrieve revision detail")
		return
	}
	...
	if resp.StatusCode != 200 {
		httputils.ReportError(w, nil, "Unable to retrieve revision detail")
		return
	}
