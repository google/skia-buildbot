...
func (task *AddTaskVars) GetPopulatedDatastoreTask() (task_common.Task, error) {
	...
	id, err := task_common.GetNextId(ds.CHROMIUM_BUILD_TASKS, &DatastoreTask{})
	if err != nil {
...
func Validate(chromiumBuild DatastoreTask) error {
	q := ds.NewQuery(chromiumBuild.GetDatastoreKind()).EventualConsistency()
	...
	count, err := ds.DS.Count(context.Background(), q)
	if err != nil {
		sklog.Info(err)
