...
func completedTasksHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, fmt.Sprintf("Could not atoi %s", r.FormValue("completed_after")), http.StatusInternalServerError)
		return
	}
	...
	for _, prototype := range task_types.Prototypes() {
		...
		for _, t := range tasks {
			if task_common.ParseBoolFormValue(r.FormValue("exclude_ctadmin_tasks")) {
			...
			usersMap[t.GetCommonCols().Username] = 1
		}
	}
