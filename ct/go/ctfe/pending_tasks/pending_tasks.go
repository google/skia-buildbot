...
// GetGCEPendingTasks returns all running tasks from all task types.
func GetGCEPendingTasks(ctx context.Context) (int, error) {
	runningGCETasksCount := 0
	for _, task := range task_types.Prototypes() {
		q := ds.NewQuery(task.GetDatastoreKind())
		...
		for _, t := range tasks {
			if t.RunsOnGCEWorkers() {
				runningGCETasksCount++
			}
		}
	}
	return runningGCETasksCount, nil
}
