...
func gatherCLData(detail clDetail, patch string) (map[string]string, error) {
	...
	clData["url"] = detail.CodereviewURL // TODO(rmistry): Maybe just create it from scratch?
...
func getCLHandler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(clURLString, RIETVELD_URL) {
		detail, err := getCLDetail(clURLString)
		if err != nil {
		...
		detail = clDetail{
			...
			Patchsets:     []int{len(change.Patchsets)},
			CodereviewURL: crURL,
		}
		patch, err = g.GetPatch(cl, "current")
		if err != nil {
