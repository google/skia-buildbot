...
// Setters for common cols.
func (dbrow *CommonCols) SetTsStarted(tsStarted int64) {
	dbrow.TsStarted = tsStarted
}
...
type AddTaskVars interface {
	...
	IsAdminTask() bool
	GetInsertQueryAndBinds() (string, []interface{}, error) // Remove
...
func updateDBTask(vars UpdateTaskVars, task Task) error {
	...
	if common.TsStarted != "" {
		fmt.Println("IN TsSTARTED STUFF HERE!!!!!!!!!!!!!!!!!!!!")
		tsStarted, err := strconv.ParseInt(common.TsStarted, 10, 64)
		if err != nil {
			return fmt.Errorf("Invalid tsStarted %s: %s", common.TsStarted, err)
		}
		fmt.Println(tsStarted)
		//task.GetCommonCols().SetTsStarted(tsStarted)
		fmt.Println(task.GetCommonCols().TsStarted)
		task.GetCommonCols().TsStarted = tsStarted
		fmt.Println(task.GetCommonCols().TsStarted)
		fmt.Println(task.GetCommonCols())
		fmt.Println(task)
