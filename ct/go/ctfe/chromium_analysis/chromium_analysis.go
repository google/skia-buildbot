...
func (task DatastoreTask) SendCompletionEmail(ctx context.Context, completedSuccessfully bool) error {
	...
	Thanks!
	`
	emailBody := fmt.Sprintf(bodyTemplate, task.Benchmark, task.PageSets, ctutil.GetSwarmingLogsLink(runID), task.Description, failureHtml, ctPerfHtml, task.RawOutput, archivedWebpagesText, path.Join(ctutil.GCSBucketName, task.ChromiumPatchGSPath), path.Join(ctutil.GCSBucketName, task.SkiaPatchGSPath), path.Join(ctutil.GCSBucketName, task.V8PatchGSPath), path.Join(ctutil.GCSBucketName, task.CatapultPatchGSPath), path.Join(ctutil.GCSBucketName, task.CustomWebpagesGSPath), task_common.ChromiumAnalysisTasksWebapp)
