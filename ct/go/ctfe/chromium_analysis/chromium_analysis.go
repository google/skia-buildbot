...
func (task DatastoreTask) SendCompletionEmail(ctx context.Context, completedSuccessfully bool) error {
	...
	emailBody := fmt.Sprintf(bodyTemplate, task.Benchmark, task.PageSets, ctutil.GetSwarmingLogsLink(runID), task.Description, failureHtml, ctPerfHtml, task.RawOutput, archivedWebpagesText, chromiumPatchLink, skiaPatchLink, v8PatchLink, catapultPatchLink, customWebpagesLink, task_common.ChromiumAnalysisTasksWebapp)
