...
func (task DatastoreTask) TriggerSwarmingTaskAndMail(ctx context.Context) error {
	...
	taskID, err := ctutil.TriggerMasterScriptSwarmingTask(ctx, runID, "run_chromium_perf_on_workers", ctutil.CHROMIUM_PERF_MASTER_ISOLATE, task_common.ServiceAccountFile, task.Platform, false, isolateArgs)
	if err != nil {
	...
	// Send start email.
	skutil.LogErr(ctutil.SendTaskStartEmail(task.DatastoreKey.ID, emails, "Chromium perf", runID, task.Description, fmt.Sprintf("Triggered %s benchmark on %s %s pageset.", task.Benchmark, task.Platform, task.PageSets)))
...
func (task DatastoreTask) SendCompletionEmail(ctx context.Context, completedSuccessfully bool) error {
	...
	Thanks!
	`
	chromiumPatchLink := ctutil.GCS_HTTP_LINK + filepath.Join(ctutil.GCSBucketName, task.ChromiumPatchGSPath)
	...
	emailBody := fmt.Sprintf(bodyTemplate, task.Benchmark, task.PageSets, ctutil.GetSwarmingLogsLink(runID), task.Description, failureHtml, ctPerfHtml, task.Results, chromiumPatchLink, skiaPatchLink, v8PatchLink, catapultPatchLink, chromiumPatchBaseBuildLink, customWebpagesLink, task_common.ChromiumPerfTasksWebapp)
