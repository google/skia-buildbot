...
func pollMasterScriptTasks(ctx context.Context) {
		...
		for _, prototype := range task_types.Prototypes() {
			...
			for _, task := range tasks {
				...
				if taskCompleted {
					...
					// Email using task.SendCompletionEmail..... (after adding this everywhere!!
				}
			}
		}
	}
...
func main() {
	...
	// Start poller for keeping an eye on master script swarming tasks.
	go pollMasterScriptTasks(ctx)
