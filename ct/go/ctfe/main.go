...
func resultsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, r, err, fmt.Sprintf("Unable to get data from %s", storageURL))
		return
	}
	defer skutil.Close(resp.Body)
	if resp.StatusCode != 200 {
		httputils.ReportError(w, r, nil, fmt.Sprintf("%s returned %d", storageURL, resp.StatusCode))
		return
	}
	w.Header().Set("Content-Type", resp.Header.Get("Content-Type"))
	if _, err := io.Copy(w, resp.Body); err != nil {
		httputils.ReportError(w, r, err, fmt.Sprintf("Error when copying response from %s", storageURL))
	}
}
