...
func repeatedTasksScheduler() {
	...
	for range time.Tick(*tasksSchedulerWaitTime) {
		...
		for _, prototype := range task_types.Prototypes() {
			...
			it := task_common.DatastoreTaskQuery(prototype,
				//query, args := task_common.DatastoreTaskQuery(prototype,
			...
			for _, task := range tasks {
				...
				if scheduledTime.Before(cutOffTime) {
					...
					if _, err := task_common.AddTask(context.Background(), task.GetDatastoreKind(), addTaskVars); err != nil {
						sklog.Errorf("Failed to add task %v: %v", task, err)
						continue
					}
					...
					if err := task_common.UpdateTask(taskVars, task); err != nil {
