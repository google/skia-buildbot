...
func (gs *GcsUtil) DownloadChromiumBuild(chromiumBuild string) (string, error) {
	localDir := filepath.Join(ChromiumBuildsDir, chromiumBuild)
	gsDir := path.Join(CHROMIUM_BUILDS_DIR_NAME, chromiumBuild)
	return gs.DownloadChromiumBuildFromTo(localDir, GCSBucketName, gsDir, CHROMIUM_BUILD_ZIP_NAME)
}
...
// binary's dir.
func (gs *GcsUtil) DownloadChromiumBuildFromTo(localDir, bucket, gsDir, zipName string) (string, error) {
	...
	pathToBinary := filepath.Join(localDir, "chrome")
	if _, err := os.Stat(pathToBinary); os.IsNotExist(err) {
		sklog.Infof("Could not find chrome binary at %s. Going to try one level up.", pathToBinary)
		pathToBinary = filepath.Join(localDir, strings.TrimSuffix(zipName, filepath.Ext(zipName)), "chrome")
		if _, err := os.Stat(pathToBinary); os.IsNotExist(err) {
			return "", fmt.Errorf("Could not find chrome binary at %s", pathToBinary)
		}
	}
