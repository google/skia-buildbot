...
//    Populates the results struct using an output CSV file.
...
func AddCTRunDataToPerf(ctx context.Context, groupName, runID, pathToCSVResults string, gs *GcsUtil) error {
	...
	hash, err := commitToSyntheticRepo(ctx, groupName, uniqueID, workdir, CT_PERF_REPO)
	if err != nil {
		return fmt.Errorf("Could not commit to %s: %s", CT_PERF_REPO, err)
	}
	...
	if err != nil {
		return fmt.Errorf("Could not convert %v to JSON: %s", ctPerfData, err)
	}
	jsonFile := path.Join(workdir, fmt.Sprintf("%s.json", uniqueID))
	if err := ioutil.WriteFile(jsonFile, perfJson, 0644); err != nil {
		return fmt.Errorf("Could not write to %s: %s", jsonFile, err)
	}
	...
	if err := gs.UploadFileToBucket(filepath.Base(jsonFile), workdir, gsDir, CT_PERF_BUCKET); err != nil {
		return fmt.Errorf("Could not upload %s to gs://%s/%s: %s", jsonFile, CT_PERF_BUCKET, gsDir, err)
	}
...
// it to the specified repo. It returns the commit hash.
func commitToSyntheticRepo(ctx context.Context, groupName, uniqueID, workdir, repo string) (string, error) {
	...
	checkout, err := git.NewCheckout(ctx, repo, tmpDir)
	if err != nil {
		return "", err
	}
...
func convertCSVToBenchData(hash, groupName, runID, pathToCSVResults string) (*ingestcommon.BenchData, error) {
	...
	// Below regexwill be used to extract page name (without rank) and rank.
		...
		for i := range headers {
			...
			if headers[i] == "traceUrls" {
				...
				if len(matches) == 3 {
					...
					if err != nil {
						sklog.Error("Could not get rank out of %s: %s")
						continue
					}
				} else {
