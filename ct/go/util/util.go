...
const (
	...
	MAX_SIMULTANEOUS_SWARMING_TASKS_PER_RUN = 2
...
func TriggerSwarmingTask(ctx context.Context, pagesetType, taskPrefix, isolateName, runID string, hardTimeout, ioTimeout time.Duration, priority, maxPagesPerBot, numPages int, isolateExtraArgs map[string]string, runOnGCE, local bool, repeatValue int, isolateDeps []string) (int, error) {
	...
	for taskMap := range chTasks {
		...
		for _, task := range tasks {
			...
			go func() {
				defer wg.Done()
				// TODO(rmistry): REMOVE ALL fmt.Println.
				fmt.Println("------------------")
				fmt.Printf("ABOUT TO COLLECT %s", task.Title)
				if _, _, err := task.Collect(ctx, s); err != nil {
					fmt.Printf("FAILED TO COLLECT %s", task.Title)
					fmt.Printf("RETRYING %s", task.Title)
					...
					if _, _, err := t[0].Collect(ctx, s); err != nil {
						fmt.Println("FAILED TO COLLECT RETRY OF %s. What is %s", t[0].Title, task.Title)
						sklog.Errorf("task %s failed inspite of a retry: %s", t[0].Title, err)
						return
					} else {
						fmt.Printf("SUCCEEDED IN COLLECTING RETRY %s", t[0].Title)
					}
				} else {
					fmt.Printf("SUCCEEDED IN COLLECTING %s", task.Title)
				}
			}()
		}
		wg.Wait()
