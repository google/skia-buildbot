...
func GetSwarmingIsolateClients(ctx context.Context, serviceAccountJSON string) (*swarming.SwarmingClient, *isolate.Client, error) {
	workDir, err := ioutil.TempDir(StorageDir, "swarming_work_")
	if err != nil {
		return nil, nil, fmt.Errorf("Could not get temp dir: %s", err)
	}
	swarmingClient, err := swarming.NewSwarmingClient(ctx, workDir, swarming.SWARMING_SERVER_PRIVATE, isolate.ISOLATE_SERVER_URL_PRIVATE, serviceAccountJSON)
	if err != nil {
		// Cleanup workdir.
		if err := os.RemoveAll(workDir); err != nil {
			sklog.Errorf("Could not cleanup swarming work dir: %s", err)
		}
		return nil, nil, fmt.Errorf("Could not instantiate swarming client: %s", err)
	}
	isolateClient, err := isolate.NewClientWithServiceAccount(workDir, isolate.ISOLATE_SERVER_URL_PRIVATE, serviceAccountJSON)
	if err != nil {
		return nil, nil, fmt.Errorf("Could not instantiate isolate client: %s", err)
	}
	return swarmingClient, isolateClient, nil
}
...
func TriggerSwarmingTask(ctx context.Context, pagesetType, taskPrefix, isolateName, runID, serviceAccountJSON, targetPlatform string, hardTimeout, ioTimeout time.Duration, priority, maxPagesPerBot, numPages int, runOnGCE, local bool, repeatValue int, baseCmd, isolateDeps []string, swarmingClient swarming.ApiClient) (int, error) {
	// Instantiate swarming and isolate clients.
	_, isolateClient, err := GetSwarmingIsolateClients(ctx, serviceAccountJSON)
	if err != nil {
		return 0, fmt.Errorf("Could not instantiate swarming and isolate clients: %s", err)
	}
	// defer swarmingClient.Cleanup()
	...
	// countOfTasks := 0
	// for i := 0; i < len(taskCmds); i += 1000 {
	// 	startRange := i
	// 	endRange := util.MinInt(len(taskCmds), i+1000)
	// 	isolateHashes, _, err := isolateClient.IsolateTasks(ctx, isolateTasks[startRange:endRange])
	// 	if err != nil {
	// 		return numTasks, fmt.Errorf("Could not isolate targets: %s", err)
	// 	}

	// 	// Add the above hashes to tasksToHashes.
	// 	for _, h := range isolateHashes {
	// 		countOfTasks++
	...
	// swarming.apiClient has a RetryTask.

	// Trigger and collect swarming tasks.
	for tasksMap := range chTasks {
		// // Trigger swarming using the isolate hashes.
		// tasks, err := swarmingClient.TriggerSwarmingTasks(ctx, taskMap, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, priority, expirationTime, hardTimeout, ioTimeout, false, true, CT_SERVICE_ACCOUNT, swarmingClient)
		// if err != nil {
		...
		for taskName, cmd := range tasksMap {
			...
			go func() {
				defer wg.Done()
				// Trigger swarming.
				req := MakeSwarmingTaskRequest(ctx, taskName, isolateHash, cipdPkgs, cmd, []string{"name:" + taskName, "runid:" + runID}, dimensions, map[string]string{"PATH": "cipd_bin_packages"}, swarming.RECOMMENDED_PRIORITY, ioTimeout)
				...
				if err != nil {
					// CHECK THE STATE HERE AND THEN RETRY IT!!!!!
					...
					resp.Request.Priority = TASKS_PRIORITY_HIGH
					retryResp, err := swarmingClient.RetryTask(resp)
					if err != nil {
					...
					// req := MakeSwarmingTaskRequest(ctx, taskName, isolateHash, cipdPkgs, cmd, []string{"name:" + taskName, "runid:" + runID}, dimensions, map[string]string{"PATH": "cipd_bin_packages"}, TASKS_PRIORITY_HIGH, ioTimeout)
					// resp, err := swarmingClient.TriggerTask(req)
					// if err != nil {
					// 	sklog.Errorf("Could not trigger swarming task %s: %s", taskName, err)
					// 	return
					// }
					// return 0, fmt.Errorf("Could not collect task ID %s: %s", resp.TaskId, err)
				}

				// if _, _, state, err := task.Collect(ctx, swarmingClient, false, false); err != nil {
				...
				// 	retryTask, err := swarmingClient.TriggerSwarmingTasks(ctx, map[string]string{task.Title: tasksToHashes[task.Title]}, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, TASKS_PRIORITY_HIGH, expirationTime, hardTimeout, ioTimeout, false, true, CT_SERVICE_ACCOUNT, swarmingClient)
				// 	if err != nil {
				// 		sklog.Errorf("Could not trigger retry of task %s: %s", task.Title, err)
				// 		return
				// 	}
				// 	// Collect the retried task.
				// 	if _, _, _, err := retryTask[0].Collect(ctx, swarmingClient, false, false); err != nil {
...
// FIGURE OUT HOW TO GET ISOlATED OUTPUT.
// HERE HERE HERE
// Return isolated output here if it exists, let isoaltetelemetry do the outputDir stuff! (assumign that it works)
func pollSwarmingTaskToCompletion(ctx context.Context, taskId string, swarmingClient swarming.ApiClient, isolateClient *isolate.Client) (string, string, error) {
	for range time.Tick(2 * time.Minute) {
		...
		case swarming.TASK_STATE_PENDING:
			...
			// sklog.Infof("The task %s is in pending state", taskId)
		case swarming.TASK_STATE_RUNNING:
			// The task is in runnig state.
			// sklog.Infof("The task %s is in running state", taskId)
...
func TriggerIsolateTelemetrySwarmingTask(ctx context.Context, taskName, runID, chromiumHash, serviceAccountJSON, targetPlatform string, patches []string, hardTimeout, ioTimeout time.Duration, local bool, swarmingClient swarming.ApiClient) (string, error) {
	...
	_, isolateClient, err := GetSwarmingIsolateClients(ctx, serviceAccountJSON)
	if err != nil {
		return "", fmt.Errorf("Could not instantiate swarming and isolate clients: %s", err)
	}
	// defer swarmingClient.Cleanup()

	// newSwarmingClient, err := swarming.NewApiClient(httpClient, swarming.SWARMING_SERVER_PRIVATE)
	// if err != nil {
	// 	return "", fmt.Errorf("Could not instantiate NEW swarming client: %s", err)
	// }
	...
	// tasksToHashes := map[string]string{taskName: isolateHashes[0]}
	...
	fmt.Println("HERE HERE HERE HERE HERE HEREE")
	...
	// if err := isolateClient.DownloadIsolate(ctx, isolateOutputsRef, outputDir); err != nil {
	// 	return "", fmt.Errorf("Could not download %s: %s", isolateOutputsRef, err)
	// }
	...
	// }
	// fmt.Println("GOT THESE CONTENTS")
...
func MakeSwarmingTaskRequest(ctx context.Context, taskName, isolatedHash string, cipdPkgs, cmd, tags []string, dims, envPrefixes map[string]string, priority int64, ioTimeoutSecs time.Duration) *swarming_api.SwarmingRpcsNewTaskRequest {
	...
	// var swarmingEnv []*swarming_api.SwarmingRpcsStringPair
	// if len(env) > 0 {
	// 	swarmingEnv = make([]*swarming_api.SwarmingRpcsStringPair, 0, len(env))
	// 	for k, v := range env {
	// 		swarmingEnv = append(swarmingEnv, &swarming_api.SwarmingRpcsStringPair{
	// 			Key:   k,
	// 			Value: v,
	// 		})
	// 	}
...
func TriggerMasterScriptSwarmingTask(ctx context.Context, runID, taskName, isolateFileName, serviceAccountJSON, targetPlatform string, local bool, isolateArgs map[string]string, cmd []string, swarmingClient swarming.ApiClient) (string, error) {
	// Instantiate swarming and isolate clients.
	_, isolateClient, err := GetSwarmingIsolateClients(ctx, serviceAccountJSON)
	if err != nil {
		return "", fmt.Errorf("Could not instantiate swarming and isolate clients: %s", err)
	...
	isolateTask := &isolate.Task{
		...
		Deps:        []string{},
		// ExtraVars:   isolateArgs, // NO THIS
		OsType: osType, // NO THIS
	}
	fmt.Println("XXXXXXX RUNNING ISOLATE TASKS!!!!!!")
	isolateHashes, _, err := isolateClient.IsolateTasks(ctx, []*isolate.Task{isolateTask})
	if err != nil {
...
func TriggerBuildRepoSwarmingTask(ctx context.Context, taskName, runID, repoAndTarget, targetPlatform, serviceAccountJSON string, hashes, patches, cipdPkgs []string, singleBuild, local bool, hardTimeout, ioTimeout time.Duration, swarmingClient swarming.ApiClient) ([]string, error) {
	...
	_, isolateClient, err := GetSwarmingIsolateClients(ctx, serviceAccountJSON)
	if err != nil {
		return nil, fmt.Errorf("Could not instantiate swarming and isolate clients: %s", err)
	}
	// defer swarmingClient.Cleanup()
	...
	// tasks, err := swarmingClient.TriggerSwarmingTasks(ctx, tasksToHashes, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, swarming.RECOMMENDED_PRIORITY, expirationTime, hardTimeout, ioTimeout, false, true, CT_SERVICE_ACCOUNT)
	// if err != nil {
	...
	// task := tasks[0]
	// _, outputDir, _, err := task.Collect(ctx, swarmingClient, false, false)
	// if err != nil {
	...
	defer util.RemoveAll(outputDir)
	if err := isolateClient.DownloadIsolate(ctx, isolateHash, outputDir); err != nil {
	...
	fmt.Println("DEBUGGING ISOLATE PROBLEM")
	fmt.Println(isolateHash)
	fmt.Println(string(contents))
	fmt.Println(ioutil.ReadDir(outputDir))
	return strings.Split(string(contents), ","), nil
}
