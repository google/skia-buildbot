...
func TriggerSwarmingTask(ctx context.Context, pagesetType, taskPrefix, isolateName, runID, serviceAccountJSON, targetPlatform string, hardTimeout, ioTimeout time.Duration, priority, maxPagesPerBot, numPages int, isolateExtraArgs map[string]string, runOnGCE, local bool, repeatValue int, isolateDeps []string) (int, error) {
	...
	isolateClient, err := isolate.NewClientWithServiceAccount(workDir, isolate.ISOLATE_SERVER_URL_PRIVATE, os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"))
	if err != nil {
...
func TriggerIsolateTelemetrySwarmingTask(ctx context.Context, taskName, runID, chromiumHash, serviceAccountJSON, targetPlatform string, patches []string, hardTimeout, ioTimeout time.Duration, local bool) (string, error) {
	...
	if len(hashes) != 1 {
		return "", fmt.Errorf("Expected one hash when isolating telemetry isolate task. Not: %+v", hashes)
	}
	...
	tasks, err := s.TriggerSwarmingTasks(ctx, map[string]string{taskName: hashes[0]}, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, swarming.RECOMMENDED_PRIORITY, expirationTime, hardTimeout, ioTimeout, false, true, CT_SERVICE_ACCOUNT)
	if err != nil {
...
func TriggerMasterScriptSwarmingTask(ctx context.Context, runID, taskName, isolateFileName, serviceAccountJSON, targetPlatform string, local bool, isolateArgs map[string]string) (string, error) {
	...
	tasks, err := s.TriggerSwarmingTasks(ctx, map[string]string{taskName: hashes[0]}, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, swarming.RECOMMENDED_PRIORITY, expirationTime, hardTimeout, 3*24*time.Hour, false, true, CT_SERVICE_ACCOUNT)
	if err != nil {
		return "", fmt.Errorf("Could not trigger swarming task: %s", err)
	}
	if len(tasks) != 1 {
...
func TriggerBuildRepoSwarmingTask(ctx context.Context, taskName, runID, repoAndTarget, targetPlatform, serviceAccountJSON string, hashes, patches, cipdPkgs []string, singleBuild, local bool, hardTimeout, ioTimeout time.Duration) ([]string, error) {
	...
	tasks, err := s.TriggerSwarmingTasks(ctx, map[string]string{taskName: isolateHashes[0]}, dimensions, map[string]string{"runid": runID}, map[string]string{"PATH": "cipd_bin_packages"}, cipdPkgs, swarming.RECOMMENDED_PRIORITY, expirationTime, hardTimeout, ioTimeout, false, true, CT_SERVICE_ACCOUNT)
	if err != nil {
