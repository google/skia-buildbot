...
func TriggerIsolateLayoutTestsSwarmingTask(ctx context.Context, taskName, runID, chromiumHash string, hardTimeout, ioTimeout time.Duration) (string, error) {
	...
	_, currentFile, _, _ := runtime.Caller(0)
	pathToIsolates := filepath.Join(filepath.Dir(filepath.Dir(filepath.Dir(currentFile))), "isolates")
	isolateFile := path.Join(pathToIsolates, ISOLATE_LAYOUT_TESTS_ISOLATE)

	isolateArgs := map[string]string{
		"RUN_ID":        runID,
		"CHROMIUM_HASH": chromiumHash,
	}
	genJSON, err := s.CreateIsolatedGenJSON(isolateFile, s.WorkDir, "linux", taskName, isolateArgs, []string{})
	if err != nil {
	...
	// REMOVE THIS ID HACK
	dims := GCE_LINUX_BUILDER_DIMENSIONS
	dims["id"] = "ct-linux-builder-007"
	tasks, err := s.TriggerSwarmingTasks(ctx, tasksToHashes, dims, map[string]string{"runid": runID}, []string{}, swarming.RECOMMENDED_PRIORITY, 2*24*time.Hour, hardTimeout, ioTimeout, false, true, getServiceAccount(GCE_LINUX_BUILDER_DIMENSIONS))
	if err != nil {
