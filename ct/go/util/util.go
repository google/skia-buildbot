...
func TriggerSwarmingTask(pagesetType, taskPrefix, isolateName, runID string, hardTimeout, ioTimeout time.Duration, priority, maxPagesPerBot, numPages int, isolateExtraArgs map[string]string, runOnGCE bool, repeatValue int) (int, error) {
	...
	// Kick off one goroutine to trigger and collect swarming tasks.
	for taskMap := range chTasks {
		// Collect all failed tasks so that we can retrigger and collect them.
		...
		if len(failedTasksToHashes) > 0 {
			...
			for _, task := range retryTasks {
				if _, _, err := task.Collect(s); err != nil {
					return numTasks, fmt.Errorf("task %s failed inspite of a retry: %s", task.Title, err)
				}
			}
		}
