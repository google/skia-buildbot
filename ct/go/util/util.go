...
func UploadToCAS(ctx context.Context, casClient cas.CAS, casSpec *CasSpec, local, runOnMaster bool) (string, error) {
	b, err := json.MarshalIndent(casSpec, "", "  ")
	if err != nil {
		sklog.Fatalf("Failed to marshal JSON: %s", err)
	}
	sklog.Infof("CAS spec: %s", string(b))
	...
	// Upload.
	sklog.Infof("Uploading to CAS: %+v", paths)
	...
	sklog.Infof("Merging digests: %+v", digests)
