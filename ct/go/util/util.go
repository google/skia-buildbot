...
func pollSwarmingTaskToCompletion(ctx context.Context, taskId string, swarmingClient swarming.ApiClient) error {
	for range time.Tick(2 * time.Minute) {
		...
		case swarming.TASK_STATE_BOT_DIED, swarming.TASK_STATE_CANCELED, swarming.TASK_STATE_EXPIRED, swarming.TASK_STATE_NO_RESOURCE, swarming.TASK_STATE_TIMED_OUT, swarming.TASK_STATE_KILLED:
			sklog.Errorf("The task %s exited early with state %v", taskId, swarmingTask.State)
			taskCompleted = true
			failure = true
		...
		case swarming.TASK_STATE_COMPLETED:
			...
			if swarmingTask.Failure {
				sklog.Infof("The task %s failed", taskId)
				failure = true
			} else {
