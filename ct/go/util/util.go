...
func CheckLengths(checks []LengthCheck) error {
	for _, check := range checks {
		fmt.Println("CHECK CEHCKING")
		fmt.Println(len(check.Value))
		fmt.Println(check.Limit)
...
func SavePatchToStorage(patch string) (string, error) {
	...
	fmt.Println("PATCH HASH IS")
	fmt.Println(patchHash)
	fmt.Println(string(patchHash))
	fmt.Println(patchHashHex)
