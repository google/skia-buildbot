...
var (
	...
	PagesetTypeToInfo = map[string]*PagesetTypeInfo{
		...
		PAGESET_TYPE_DUMMY_1k: {
			...
			CaptureSKPsTimeoutSecs:     300,
			RunChromiumPerfTimeoutSecs: 300,
			Description:                "Top 1K (with desktop user-agent, used for testing, hidden from Runs History by default)",
		},
		PAGESET_TYPE_MOBILE_DUMMY_1k: {
			...
			CaptureSKPsTimeoutSecs:     300,
			RunChromiumPerfTimeoutSecs: 300,
			Description:                "Top 1K (with mobile user-agent, used for testing, hidden from Runs History by default)",
		},
	}
