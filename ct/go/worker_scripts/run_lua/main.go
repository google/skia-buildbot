...
func runLua() error {
	...
	outLuaPictures, err := os.Create(luaPicturesLocalPath)
	if err != nil {
		return fmt.Errorf("Unable to create file %s: %s", luaPicturesLocalPath, err)
	}
	defer skutil.Remove(luaPicturesLocalPath)
	if _, err = io.Copy(outLuaPictures, luaPicturesRespBody); err != nil {
		return err
	}
	skutil.Close(outLuaPictures)
	// Downloaded lua_pictures binary needs to be set as an executable.
	skutil.LogErr(os.Chmod(luaPicturesLocalPath, 0777))
