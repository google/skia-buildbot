...
func runChromiumAnalysis() error {
	...
	if *apkGsPath == "" {
		if *chromeBuildGsPath == "" {
			pathToBinaryDir, err = gs.DownloadChromiumBuild(*chromiumBuild)
			if err != nil {
				return err
			}
		} else {
			...
			fmt.Println(gsDir)
			fmt.Println(zipName)
			fmt.Println(customChromeBuildDir)
			pathToBinaryDir, err = gs.DownloadChromiumBuildFromTo(customChromeBuildDir, bucket, gsDir, zipName)
			if err != nil {
				fmt.Println(err)
				fmt.Println("SLEEPING BEFORE ERRORING OUT")
				fmt.Println(pathToBinaryDir)
