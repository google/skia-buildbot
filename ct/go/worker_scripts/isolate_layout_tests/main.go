...
func buildRepo() error {
	...
	// Hack: the isolate file does not contain out/Release/args.gn. This fails the
	// run_web_tests.py script. Add it to the isolate file.
	isolateFileContents, err := ioutil.ReadFile(isolateFile)
	if err != nil {
		return err
	}
	isolateFileContentsStr := string(isolateFileContents)
	newFileContents := []string{}
	for _, l := range strings.Split(isolateFileContentsStr, "\n") {
		newFileContents = append(newFileContents, l)
		if strings.Contains(l, "../../net/data/") {
			newFileContents = append(newFileContents, strings.Replace(l, "../../net/data/", "args.gn", 1))
		}
	}
	if err := ioutil.WriteFile(isolateFile, []byte(strings.Join(newFileContents, "\n")), 0644); err != nil {
		return err
	}
