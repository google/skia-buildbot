	...
	for i := 0; i < numWorkers; i++ {
		...
		go func() {
			...
			for pagesetName := range pagesetRequests {
				...
				mutex.RLock()
				noPatchErr := util.RunBenchmark(pagesetName, pathToPagesets, pathToPyFiles, localOutputDirNoPatch, *chromiumBuildNoPatch, chromiumBinaryNoPatch, runIDNoPatch, *browserExtraArgsNoPatch, *benchmarkName, *targetPlatform, *benchmarkExtraArgs, *pagesetType, *repeatBenchmark, true /* retWebpageFailErr */)
				if noPatchErr != nil && strings.Contains(noPatchErr.Error(), exec.TIMEOUT_ERROR_PREFIX) {
					timeoutTracker.Increment()
				} else {
					timeoutTracker.Reset()
				}
				withPatchErr := util.RunBenchmark(pagesetName, pathToPagesets, pathToPyFiles, localOutputDirWithPatch, *chromiumBuildWithPatch, chromiumBinaryWithPatch, runIDWithPatch, *browserExtraArgsWithPatch, *benchmarkName, *targetPlatform, *benchmarkExtraArgs, *pagesetType, *repeatBenchmark, true /* retWebpageFailErr */)
				if withPatchErr != nil && strings.Contains(withPatchErr.Error(), exec.TIMEOUT_ERROR_PREFIX) {
					timeoutTracker.Increment()
				} else {
					timeoutTracker.Reset()
				}
