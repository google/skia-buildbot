...
const (
	...
	RUN_WEB_TESTS_BINARY_TIMEOUT = 3 * time.Hour
)
...
func captureArchives() error {
	...
	gsFailingDir := path.Join(util.SWARMING_DIR_NAME, util.LayoutTestRunsDir, outputDirName+"-failing")
	...
	failingFileName := fmt.Sprintf("worker%d-failing.json", *workerNum)
	failingFilePath := path.Join(localOutputDir, failingFileName)
	runWebTestsBinary := filepath.Join(util.GetPathToLayoutTestBinaries(!*worker_common.Local), RUN_WEB_TESTS_BINARY)
	args := []string{
		"--release",
		"-t", "layout_tests_isolates",
		"--repeat", "2",
		"--seed", "1234", // Added for https://bugs.chromium.org/p/skia/issues/detail?id=8219#c30
		"--json-test-results", outputFilePath,
		"--json-failing-test-results", failingFilePath,
	}
	env := os.Environ()
	env = append(env, "DISPLAY=:0", fmt.Sprintf("PATH=%s:$PATH", util.DepotToolsDir))
	if err := util.ExecuteCmd(ctx, runWebTestsBinary, args, env, RUN_WEB_TESTS_BINARY_TIMEOUT, nil, nil); err != nil {
		// Do not throw error, the script returns error code when there are failures.
		sklog.Errorf("Error when running %s: %s", runWebTestsBinary, err)
	}
	...
	if err := gs.UploadFile(outputFileName, localOutputDir, gsOutputDir); err != nil {
		return err
	}
	// Upload the failing file as well.
	if err := gs.UploadFile(failingFileName, localOutputDir, gsFailingDir); err != nil {
		return err
	}
