	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range traceRequests {
				traceBucket, tracePath, err := parseTraceURL(t)
				if err != nil {
				...
				downloadedTrace, err := downloadTrace(traceBucket, tracePath, traceDownloadDir, gs)
				if err != nil {
...
// destDir.
func downloadTrace(traceBucket, tracePath, destDir string, gs *util.GcsUtil) (string, error) {
