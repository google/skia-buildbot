...
// $ go run go/worker_scripts/metrics_analysis/main.go --start_range=1 --num=3 --run_id=rmistry-test1 --benchmark_extra_args="--output-format=csv" --metric_name="loadingMetric" --logtostderr=true --local --chromium_hash=41a2a781016029cf45dab096cd9ae6d94942b25f
//
package main
...
func metricsAnalysis() error {
	...
	// TODO(rmistry): Need to sync the chromium dor? because will have to apply patch?????
	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range traceRequests {
				...
				downloadedTrace, err := downloadTrace(t, traceDownloadDir, httpTimeoutClient, gs)
				if err != nil {
				...
				if err := runMetricsAnalysisBenchmark(ctx, localOutputDir, downloadedTrace); err != nil {
...
func runMetricsAnalysisBenchmark(ctx context.Context, outputPath, downloadedTrace string) error {
	args := []string{
		filepath.Join(util.TelemetryBinariesDir, util.BINARY_RUN_BENCHMARK),
		util.BENCHMARK_METRICS_ANALYSIS,
		"--urls-list=" + fmt.Sprintf("file:/%s", downloadedTrace),
	...
	//return util.ExecuteCmd(ctx, "python", args, env, METRICS_BENCHMARK_TIMEOUT_SECS*time.Second, nil, nil)
}

func downloadTrace(traceURL, destDir string, httpTimeoutClient *http.Client, gs *util.GcsUtil) (string, error) {
