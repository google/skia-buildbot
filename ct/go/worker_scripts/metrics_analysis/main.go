	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range traceRequests {
				...
				downloadedTrace, err := downloadTrace(t, traceDownloadDir, httpTimeoutClient)
				if err != nil {
				...
				runMetricsAnalysisBenchmark(ctx, localOutputDir, downloadedTrace)
			}
		}()
	}
...
func runMetricsAnalysisBenchmark(ctx context.Context, outputPath, downloadedTrace string) {
	args := []string{
...
func downloadTrace(traceURL, destDir string, httpTimeoutClient *http.Client) (string, error) {
	traceTokens := strings.Split(traceURL, "/")
	traceDest := filepath.Join(destDir, traceTokens[len(traceTokens)-1])
	resp, err := httpTimeoutClient.Get(traceURL)
	if err != nil {
		return "", fmt.Errorf("Could not GET %s: %s", traceURL, err)
	}
	defer skutil.Close(resp.Body)
	out, err := os.Create(traceDest)
	defer skutil.Close(out)
	if err != nil {
		return "", fmt.Errorf("Unable to create file %s: %s", traceDest, err)
	}
	if _, err = io.Copy(out, resp.Body); err != nil {
		return "", fmt.Errorf("Unable to write to file %s: %s", traceDest, err)
	}
