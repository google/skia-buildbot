	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range traceRequests {
				...
				if err := runMetricsAnalysisBenchmark(ctx, localOutputDir, downloadedTrace, t, traceID); err != nil {
...
func runMetricsAnalysisBenchmark(ctx context.Context, outputPath, downloadedTrace, cloudTraceLink string, traceID int) error {
	outputCSVDir := filepath.Join(outputPath, fmt.Sprintf("%d", traceID))
