...
const (
	...
	MAX_CT_INSTANCE_NUM = 100 // rmistry: Only playing with first 100 instances for now. Later make it 550.
)
...
func NewCTAutoscaler(ctx context.Context, local bool, getGCETasksCount func(ctx context.Context) (int, error)) (*CTAutoscaler, error) {
	...
	upGauge.Update(int64(runningGCETasksCount))

	if runningGCETasksCount == 0 {
		// Start from a clean slate by bringing down all CT instances since
		// there are no running GCE tasks. Also delete them from swarming.
	...
	go func() {
		for range time.Tick(2 * time.Minute) {
			if err := c.watchRunningGCETasks(); err != nil {
				sklog.Error(err)
			}
		}
	}()
...
func (c *CTAutoscaler) watchRunningGCETasks() error {
	c.mtx.Lock()
	defer c.mtx.Unlock()
