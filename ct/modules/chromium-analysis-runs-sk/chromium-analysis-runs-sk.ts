...
export class ChromiunAnalysisRunsSk extends ElementSk {
  ...
  constructor() {
    super(ChromiunAnalysisRunsSk.template);
    this._resetPagination();
  }

  private static template = (el: ChromiunAnalysisRunsSk) => html`
<div>
  ...
  <table class="surface-themes-sk secondary-links runssummary" id=runssummary>
    ...
    ${el._tasks.map((task, index) => ChromiunAnalysisRunsSk.taskRowTemplate(el, task, index))}
  </table>
</div>

${el._tasks.map((task, index) => ChromiunAnalysisRunsSk.taskDialogTemplate(task, index))}
<toast-sk id=confirm_toast class=primary-variant-container-themes-sk duration=5000></toast-sk>
`;

private static taskRowTemplate = (el: ChromiunAnalysisRunsSk, task: ChromiumAnalysisDatastoreTask, index: number) => html`
<tr>
  <!-- Id col -->
  <td class=nowrap>
    ${task.raw_output
...
define('chromium-analysis-runs-sk', ChromiunAnalysisRunsSk);
