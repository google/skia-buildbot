...
describe('chromium-analysis-sk', () => {
  ...
  const setDescription = (d: string) => {
    ($$('#description', chromiumAnalysis) as HTMLInputElement).value = d;
  };

  const setBenchmark = (b: string) => {
    ($$('#benchmark_name', chromiumAnalysis) as HTMLInputElement).value = b;
  };
  ...
  const clickSubmit = () => {
    ($$('#submit', chromiumAnalysis) as HTMLInputElement).click();
  };
