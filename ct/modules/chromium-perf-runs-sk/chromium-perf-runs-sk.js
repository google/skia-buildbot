...
<div>
  ...
  <pagination-sk></pagination-sk>
  <br/>
  <button @click=${() => el._constrainRunsByUser()}>
    ${el._constrainByUser ? 'View Everyone\'s Runs' : 'View Only My Runs'}
  </button>
  <button @click=${() => el._constrainRunsByTest()}>
...
<tr>
  ...
  <td class=nowrap>
    ...
    ${task.Results
    ? html`<a href="${task.Results}" target=_blank rel="noopener noreferrer">Overall Result</a>
      <br/>
      <a href="${task.NoPatchRawOutput}" target=_blank rel="noopener noreferrer">NoPatch Raw Output</a>
      <br/>
      <a href="${task.WithPatchRawOutput}" target=_blank rel="noopener noreferrer">WithPatch Raw Output</a>`
    : ''}
    ${task.SwarmingLogs
    ? html`<br/>
...
define('chromium-perf-runs-sk', class extends ElementSk {
  ...
  _redoTask(index) {
    ...
    fetch('/_/redo_chromium_perf_task', { method: 'POST', body: JSON.stringify(params) })
      .then((res) => {
        if (res.ok) {
          $$('#confirm_toast').innerText = `Resubmitted ${pendingTask.TaskType} task ${pendingTask.Id}`;
          $$('#confirm_toast').show();
          return;
        }
        // Non-OK status. Read the response and punt it to the catch.
