...
<collapse-sk ?closed=${!ele.open}>
  <textarea rows=1 placeholder=${ele.placeholderText} @input=${ele._computeResize}></textarea>
</collapse-sk>
`;
...
define('expandable-textarea-sk', class extends ElementSk {
  ...
  connectedCallback() {
    ...
    this._textarea = $$('textarea', this._collapser);
    this._textarea.rows = parseInt('5', 10);
  }
  ...
  _computeResize() {
    // We reduce number of rows any time we lack a scroll bar, this covers the case of deleting line(s) in a textarea that has grown.
    // When the input/value change has not reduced the number of lines, the  below 'grow' code reverts this change before the next render.
    if (this._textarea.rows > 5 && this._textarea.scrollHeight === this._textarea.clientHeight) {
      this._textarea.rows--;
    }
