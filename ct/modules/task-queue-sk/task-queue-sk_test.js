...
describe('task-queue-sk', () => {
  ...
  it('delete flow works', async () => {
    ...
    // TODO(weston): Update common-sk/confirm-dialog-sk to make this let
    // brittle.
  ...
  it('task details works', async () => {
    ...
    //expect($$('.dialog-background', table)).to.have.nested.property('style.display', '');
    expect($$('.dialog-background', table)).to.have.class('hidden');
    $$('.details', table).click();

    expect($$('.dialog-background', table)).to.not.have.class('hidden');
    //expect($$('.dialog-background', table)).to.have.nested.property('style.display', 'block');
  });
});
