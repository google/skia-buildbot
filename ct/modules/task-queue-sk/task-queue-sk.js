...
import { getFormattedTimestamp, taskDescriptors, getTimestamp, getCtDbTimestamp } from '../ctfe_utils';
...
${el._pendingTasks.map((task, index) => taskDetailDialogTemplate(el, task, index))}
...
<tr>
  ...
  <td class=nowrap>
    <a href=# rel=noopener class=details
      @click=${() => el.showDetailsDialog(index)}>Task Details</a>
  </td>
</tr>`;

const taskDetailDialogTemplate = (el, task, index) => html`
<div id=${`detailsDialog${index}`} class="dialog-background hidden"
  @click=${(e) => el.hideDialog(e)}>
  <div class=dialog-content>
    <pre>${el.formatTask(task)}</pre>
  </div>
</div>
`;
