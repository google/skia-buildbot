...
define('task-queue-sk', class extends ElementSk {
  ...
  connectedCallback() {
    ...
    if (this._running) {
      console.log('choosing not to run');
      return;
    }
    this._running = true;
    DomReady.then(() => {
    ...
    this.loadTaskQueue().then(() => {
      console.log('Tasks loaded, rendering.')
      this._render();
      this._running = false;
  ...
  loadTaskQueue() {
    ...
    var queryStr = '?' + fromObject(queryParams);
    var allPromises = [];
    ctfe_utils.taskDescriptors.forEach(function(obj) {
  ...
  updatePendingTasks(json, taskDescriptor) {
    ...
    for (var i = 0; i < tasks.length; i++) {
      ...
      if (task['RepeatAfterDays'] != 0 && task['TaskDone']) {
        ...
        var timestamp = ctfe_utils.getTimestamp(task['TsAdded']);
  ...
  deleteTask(index) {
    ...
    var params = {};
    params['id'] = 9999999; //pendingTask.Id;
        // TODO(must check response for none-200. that doesn't catch.
    fetch(pendingTask.DeleteURL, {method: 'POST', body: JSON.stringify(params)})
        .then(function(res) {
          if (!res.ok) {
// this is wrong, dont' want erro and confirm toast. perhaps just use the confirm.
	  return res.text().then((text) => {errorMessage('Failed to delete the task:' + text);})
           errorMessage('Failed to delete the task: ' + res.text()) 
          }
