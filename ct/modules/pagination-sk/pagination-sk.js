...
<div>
  <button class=action data-page=0 ?disabled=${el._onFirstPage()} @click=${el._update}><<</button>
  <button class=action data-page=${el._page - 1} ?disabled=${el._onFirstPage()} @click=${el._update}><</button>
  ${el._pageButtons.map((page) => html`
    <button data-page=${page}
     @click=${el._update}
     ?disabled=${page === el._page}>${page + 1}</button>`)}
  <button class=action data-page=${el._page + 1} ?disabled=${el._onLastPage()} @click=${el._update}>></button>
  <button class=action data-page=${el._allPages - 1} ?disabled=${el._onLastPage()} @click=${el._update}>>></button>
</div>
`;
