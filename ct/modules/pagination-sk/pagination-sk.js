...
<div>
  <button data-page=0 ?disabled=${el._onFirstPage()} @click=${el._update}><<</button>
  <button data-page=${el._page - 1} ?disabled=${el._onFirstPage()} @click=${el._update}><</button>
  ${el._pageButtons.map((page) => html`
    <button data-page=${page}
     @click=${el._update}
     ?disabled=${page === el._page}>${page + 1}</button>`)}
  <button data-page=${el._page + 1} ?disabled=${el._onLastPage()} @click=${el._update}>></button>
  <button data-page=${el._allPages - 1} ?disabled=${el._onLastPage()} @click=${el._update}>>></button>
</div>
`;
...
define('pagination-sk', class extends ElementSk {
  constructor() {
    super(template);
    this._pagination = { size: 10, offset: 0, total: 100 };
  ...
  _computePageButtons() {
    ...
    const start = Math.max(Math.min(this._page - this._showPagesOffset, this._allPages - this._showPages), 0);
  ...
  _onLastPage() {
    return this._page === (this._allPages - 1);
  }
