...
import { getFormattedTimestamp, formatRepeatAfterDays } from '../ctfe_utils';
import {
  ResponsePagination,
  AdminDatastoreTask,
  RedoTaskRequest,
  ...
  private static template = (el: AdminTaskRunsSk) => html`
    <div>
      ...
      <table
        ...
        ${el._tasks.map((task: AdminDatastoreTask, index: number) =>
          AdminTaskRunsSk.taskRowTemplate(el, task, index)
        )}
      </table>
    </div>
  `;
