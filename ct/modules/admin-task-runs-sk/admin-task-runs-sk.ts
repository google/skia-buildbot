...
export class AdminTaskRunsSk extends ElementSk {
  ...
  private constrainByUser = false;

  private constrainByTest = true;
...
<div>
  <h2>${el.contrainByUser ? 'My ' : ''}${el.taskType}</h2>
  <pagination-sk @page-changed=${(e: Event) => el._pageChanged(e)}></pagination-sk>
  <br/>
  <button id=userFilter @click=${() => el.constrainRunsByUser()}>
    ${el.constrainByUser ? 'View Everyone\'s Runs' : 'View Only My Runs'}
  </button>
  <button id=testFilter @click=${() => el.constrainRunsByTest()}>
    ${el.constrainByTest ? 'Include Test Run' : 'Exclude Test Runs'}
  </button>
  ...
  <table class="surface-themes-sk secondary-links runssummary" id=runssummary>
    ...
    ${el.tasks.map((task, index) => taskRowTemplate(el, task, index))}
  </table>
</div>
...
private static taskRowTemplate = (el: AdminTaskRunsSk, task, index) => html`
<tr>
  <!-- Id col -->
  <td class=nowrap>
    <span>${task.Id}</span>
