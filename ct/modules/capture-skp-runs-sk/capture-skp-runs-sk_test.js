...
describe('capture-skp-runs-sk', () => {
  const newInstance = setUpElementUnderTest('capture-skp-runs-sk');
  ...
  const expectReload = async (trigger, result) => {
    ...
    fetchMock.postOnce('begin:/_/get_capture_skp_tasks', result);
    trigger();
    await event;
  };
  ...
  it('shows table entries', async () => {
    expect($('table.runssummary>tbody>tr', captureSkpRuns)).to.have.length(6);
  ...
  it('deletes tasks', async () => {
    ...
    expect(fetchMock.lastOptions('begin:/_/delete').body).to.contain('"id":105');
  });
  ...
  it('reschedules tasks', async () => {
    ...
    await expectReload(confirmDialog);
    expect(fetchMock.lastOptions('begin:/_/redo').body).to.contain('"id":105');
  });
});
