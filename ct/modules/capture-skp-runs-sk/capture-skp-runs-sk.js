/**
 * @fileoverview The bulk of the Capture SKPs Runs History page.
 */
...
<div>
  <h2>${el._contrainByUser ? 'My ' : ''}Capture SkPicture Runs</h2>
...
define('capture-skp-runs-sk', class extends ElementSk {
  constructor() {
    super(template);
    this._tasks = [];
  ...
  _reload() {
    ...
    return fetch(`/_/get_capture_skp_tasks?${fromObject(queryParams)}`,
      { method: 'POST' })
      .then(jsonOrThrow)
      .then((json) => {
  ...
  _confirmDeleteTask(index) {
    const note = index >= 0
      ...
      ? ' Note: This SKP repository will no longer be available.' : '';
  ...
  _deleteTask(index) {
    ...
    fetch('/_/delete_capture_skps_task', { method: 'POST', body: JSON.stringify(params) })
      .then((res) => {
        if (res.ok) {
  ...
  _redoTask(index) {
    const params = {};
    params.id = this._tasks[index].Id;
    fetch('/_/redo_capture_skps_task', { method: 'POST', body: JSON.stringify(params) })
      .then((res) => {
        if (res.ok) {
