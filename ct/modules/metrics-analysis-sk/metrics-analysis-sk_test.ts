...
describe('metrics-analysis-sk', () => {
  ...
  const setDescription = (d: string) => {
    ($$('#description', metricsAnalysis) as HTMLInputElement).value = d;
  };

  const setMetricName = (m: string) => {
    ($$('#metric_name', metricsAnalysis) as HTMLInputElement).value = m;
  };

  const setAnalysisTaskId = (a: string) => {
    ($$('#analysis_task_id', metricsAnalysis) as HTMLInputElement).value = a;
  };

  const setPatch = async (patchtype: string, value: string, response: any) => {
    fetchMock.postOnce('begin:/_/cl_data', response);
    const input = $$(`#${patchtype}_patch input-sk`) as HTMLInputElement;
    input.value = value;
  ...
  const clickSubmit = () => {
    ($$('#submit', metricsAnalysis) as HTMLInputElement).click();
  };
