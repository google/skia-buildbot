...
describe('chromium-perf-sk', () => {
  ...
  const setDescription = (d: string) => {
    ($$('#description', chromiumPerf) as HTMLInputElement).value = d;
  };

  const setBenchmark = (b: string) => {
    ($$('#benchmark_name', chromiumPerf) as HTMLInputElement).value = b;
  };
  ...
  const clickSubmit = () => {
    ($$('#submit', chromiumPerf) as HTMLInputElement).click();
  };
