...
<select-sk
  @selection-changed=${() => console.log('yay')}>
...
define('pageset-selector-sk', class extends ElementSk {
  constructor() {
    ...
    this._hideIfContains = this._hideIfContains || [];
  }
  ...
  /**
   * @prop {Array<string>} hideIfContains - Entries containing these substrings
   * are removed from the pageSet listing.
   */
  get hideIfContains() {
    return this._hideIfContains;
  }

  set hideIfContains(val) {
    this._hideIfContains = val;
    this._filterPageSets();
  }

  _filterPageSets() {
    const blacklist = this._hideIfContains;
    const psHasSubstring = (ps) => blacklist.some((s) => ps.includes(s));
