...
<style>
    ...
    /* avoid clickable area of <ul> and <div>*/ 
    ul {
      padding: 0;
    }
    li {
...
define('suggest-input-sk', class extends ElementSk {
  constructor() {
    super(template);
    this._options = ['golang', 'c++', 'JS', 'Python', 'Python2.7', 'Python3', 'IronPython'];
  ...
  // TODO bug clicking/selecting the same entry twice in a row is the issue.....
  _refresh(e) {
    console.log('REFRESH');
    debugger;
    ...
    // Always begin selection at the top suggestion.
    this._suggestion_selected = -1;
    this._render();
  }
  ...
  _keyup(e) {
    ...
    if (e.keyCode === DOWN_ARROW && len > 0) {
      ...
      this._render();
    } else if (e.keyCode === ENTER && this._suggestion_selected >= 0) {
      this._commit();
    }
    // TODO enter custom value here??
  ...
  _commit() {
    const selection = this._suggestions[this._suggestion_selected];
    this.value = selection;
    this._suggestions = [];
    this._suggestion_selected = -1;
    console.log('committed: ' + selection);
    debugger;
    this._render();
  }

  _blur(e) {
    // Ignore if we already did the work via clicking a suggestion.
    ...
    if (selection > -1) {
      ...
      this.value = this._suggestions[selection];
      this._commit();
