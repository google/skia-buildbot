...
_IGNORE_MERGE_VALUES = {
    'reftest_type', 'has_stderr', 'bugs', 'is_flaky', 'is_unexpected',
    'is_testharness_test', 'is_missing_image', 'is_regression',
    'is_missing_text', 'has_repaint_overlay'}
...
def merge_tries(source, dest):
  ...
  while pending_nodes:
    ...
    for k, v in curr_node.iteritems():
      if k in dest_node:
        if k == 'expected':
          assert v == dest_node[k], (
              'Cannot merge tests with different expectations. Node: %s' %
              (curr_node))
        # Ignore merging these values
        elif k in _IGNORE_MERGE_VALUES:
          pass
          ...
          if not isinstance(dest_node[k], list):
            dest_node[k] = [dest_node[k]]
          dest_node[k].append(v)
        else:
          raise ValueError('Unsupported merge key: %s.\nNode: %s' % (
            repr(k), curr_node))
      else:
        dest_node[k] = v
  return dest
