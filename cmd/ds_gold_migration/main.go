...
func migrateExpectationStore(vdb *database.VersionedDB, dsClient *datastore.Client) {
	...
	// Copy over all the expectation changes
	...
	for p := nPages - 1; p >= 0; p-- {
		...
		sklog.Infof("%d records loaded.", len(logEntries))
		...
		for i := len(logEntries) - 1; i >= 0; i-- {
			entry := logEntries[i]
			if entry.TS <= lastTS {
				sklog.Fatal("TS not increasing monotonically. Change %d has time stamp %d following %d", entry.ID, entry.TS, lastTS)
			}
			lastTS = entry.TS
	...
	for p := nPages - 1; p >= 0; p-- {
		...
		for idx, sqlLogEntry := range logEntries {
