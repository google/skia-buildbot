...
func migrateExpectationStore(vdb *database.VersionedDB, dsClient *datastore.Client) {
	...
	cloudExpStore.DisableTransactionalAdds()

	printCount(dsClient, ds.MASTER_EXP_CHANGE)
	printCount(dsClient, ds.MASTER_TEST_DIGEST_EXP)
	printCount(dsClient, ds.HELPER_RECENT_KEYS)
	...
	if err := ds.DeleteAll(dsClient, ds.MASTER_TEST_DIGEST_EXP, true); err != nil {
		sklog.Fatalf("Error deleting entities: %s", err)
	}
	if err := ds.DeleteAll(dsClient, ds.HELPER_RECENT_KEYS, true); err != nil {
		sklog.Fatalf("Error deleting entities: %s", err)
	}
	...
	for p := nPages - 1; p >= 0; p-- {
		...
		for i := len(logEntries) - 1; i >= 0; i-- {
			...
			wg.Add(1)
			go func(entry *expstorage.TriageLogEntry) {
				defer wg.Done()

				changes := entry.GetChanges()
				if err := cloudExpStore.ImportChange(changes, entry.Name, entry.TS); err != nil {
		...
		sklog.Infof("Migrated %d/%d records. %.2f ms average", totalChangeRecs, total, avgTime)
	}
	...
	sklog.Infof("Database consistent now.")
	...
	sklog.Infof("Retrieved SQL expectations")
	...
	sklog.Infof("Retrieved Cloud expectations")
