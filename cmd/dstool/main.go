	...
	processKeysForOneParent := func(procSlice []*datastore.Key) {
		...
		go func() {
			...
			for len(procSlice) > 0 {
				...
				// Process the slice in a transaction. We ignore error handling here
				// since that is left to the function.
				_, _ = client.RunInTransaction(context.TODO(), func(tx *datastore.Transaction) error {
