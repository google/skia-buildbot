...
func readWriteChromiumBuilds(ctx context.Context) {

	table := db.TABLE_CHROMIUM_BUILD_TASKS
	kind := ds.CHROMIUM_BUILD_TASKS
	// 3 places below.
	...
	var wg sync.WaitGroup
	tasksChannel := make(chan chromium_builds.DBTask, len(tasks))
	for _, t := range tasks {
		tasksChannel <- t
	}
	close(tasksChannel)
	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range tasksChannel {
				...
				// Task specific cols.
				dsTask.ChromiumRev = t.ChromiumRev
				if t.ChromiumRevTs.Valid {
					dsTask.ChromiumRevTs = t.ChromiumRevTs.Int64
				}
				dsTask.SkiaRev = t.SkiaRev
...
func readWriteChromiumPerf(ctx context.Context) {

	table := db.TABLE_CHROMIUM_PERF_TASKS
	...
	var wg sync.WaitGroup
	tasksChannel := make(chan chromium_perf.DBTask, len(tasks))
	for _, t := range tasks {
		tasksChannel <- t
	}
	close(tasksChannel)
	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range tasksChannel {
				...
				dsTask.PageSets = t.PageSets
				dsTask.IsTestPageSet = t.PageSets == ctutil.PAGESET_TYPE_DUMMY_1k
				dsTask.RepeatRuns = t.RepeatRuns
				dsTask.RunInParallel = t.RunInParallel
				dsTask.BenchmarkArgs = t.BenchmarkArgs
				dsTask.BrowserArgsNoPatch = t.BrowserArgsNoPatch
				dsTask.BrowserArgsWithPatch = t.BrowserArgsWithPatch
				dsTask.Description = t.Description
				if t.Results.Valid {
					dsTask.Results = t.Results.String
				}
				if t.NoPatchRawOutput.Valid {
					dsTask.NoPatchRawOutput = t.NoPatchRawOutput.String
				}
				if t.WithPatchRawOutput.Valid {
					dsTask.WithPatchRawOutput = t.WithPatchRawOutput.String
				}
				...
				blinkPatchGSPath, err := ctutil.SavePatchToStorage(t.BlinkPatch)
				if err != nil {
					sklog.Fatalf("Could not save blink patch to storage: %s", err)
				}
				skiaPatchGSPath, err := ctutil.SavePatchToStorage(t.SkiaPatch)
				if err != nil {
					sklog.Fatalf("Could not save skia patch to storage: %s", err)
				...
				dsTask.CustomWebpagesGSPath = customWebpagesGSPath
				dsTask.ChromiumPatchGSPath = chromiumPatchGSPath
				dsTask.BlinkPatchGSPath = blinkPatchGSPath
				dsTask.SkiaPatchGSPath = skiaPatchGSPath
				dsTask.CatapultPatchGSPath = catapultPatchGSPath
				dsTask.BenchmarkPatchGSPath = benchmarkPatchGSPath
				dsTask.V8PatchGSPath = v8PatchGSPath
...
func readWriteMetricsAnalysis(ctx context.Context) {

	table := db.TABLE_METRICS_ANALYSIS_TASKS
	...
	var wg sync.WaitGroup
	tasksChannel := make(chan metrics_analysis.DBTask, len(tasks))
	for _, t := range tasks {
		tasksChannel <- t
	}
	close(tasksChannel)
	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range tasksChannel {
				...
				// Task specific cols.
				dsTask.MetricName = t.MetricName
				dsTask.AnalysisTaskId = t.AnalysisTaskId
				dsTask.AnalysisOutputLink = t.AnalysisOutputLink
				dsTask.BenchmarkArgs = t.BenchmarkArgs
				dsTask.Description = t.Description
				if t.RawOutput.Valid {
					dsTask.RawOutput = t.RawOutput.String
				...
				// Patches and custom webpages/traces.
				customTracesGSPath, err := ctutil.SavePatchToStorage(t.CustomTraces)
				if err != nil {
					sklog.Fatalf("Could not save custom webpages to storage: %s", err)
				}
				chromiumPatchGSPath, err := ctutil.SavePatchToStorage(t.ChromiumPatch)
				...
				dsTask.CustomTracesGSPath = customTracesGSPath
				dsTask.ChromiumPatchGSPath = chromiumPatchGSPath
				dsTask.CatapultPatchGSPath = catapultPatchGSPath
...
func readWritePixelDiff(ctx context.Context) {

	table := db.TABLE_PIXEL_DIFF_TASKS
	kind := ds.PIXEL_DIFF_TASKS
	// 4 places below.
	...
	var wg sync.WaitGroup
	tasksChannel := make(chan pixel_diff.DBTask, len(tasks))
	for _, t := range tasks {
		tasksChannel <- t
	}
	close(tasksChannel)
	...
	for i := 0; i < WORKER_POOL_SIZE; i++ {
		...
		go func() {
			...
			for t := range tasksChannel {
				...
				skiaPatchGSPath, err := ctutil.SavePatchToStorage(t.SkiaPatch)
				if err != nil {
					sklog.Fatalf("Could not save ski patch to storage: %s", err)
				}
				dsTask.CustomWebpagesGSPath = customWebpagesGSPath
				dsTask.ChromiumPatchGSPath = chromiumPatchGSPath
				dsTask.SkiaPatchGSPath = skiaPatchGSPath
...
func readWriteLuaScripts(ctx context.Context) {
	table := db.TABLE_LUA_SCRIPT_TASKS
	...
	for _, t := range tasks {
		...
		dsTask.ChromiumRev = t.ChromiumRev
		dsTask.SkiaRev = t.SkiaRev
		dsTask.LuaScript = t.LuaScript
		dsTask.LuaAggregatorScript = t.LuaAggregatorScript
		dsTask.Description = t.Description
		if t.ScriptOutput.Valid {
			dsTask.ScriptOutput = t.ScriptOutput.String
		}
		if t.AggregatedOutput.Valid {
			dsTask.AggregatedOutput = t.AggregatedOutput.String
		}
