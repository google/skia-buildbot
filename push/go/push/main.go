...
func (s *Server) stateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("refresh") == "true" {
		if err := s.packageInfo.ForceRefresh(); err != nil {
			httputils.ReportError(w, err, "Failed to refresh.")
		}
	}
	...
	if r.Method == "POST" {
		if !login.IsAdmin(r) {
			httputils.ReportError(w, nil, "You must be logged on as an admin to push.")
			return
		}
		push := PushNewPackage{}
		...
		if err := dec.Decode(&push); err != nil {
			httputils.ReportError(w, fmt.Errorf("Failed to decode push request"), "Failed to decode push request")
			return
		}
		if installedPackages, ok := allInstalled[push.Server]; !ok {
			httputils.ReportError(w, fmt.Errorf("Unknown server name"), "Unknown server name")
			return
		} else {
			...
			if err := s.packageInfo.PutInstalled(push.Server, newInstalled, installedPackages.Generation); err != nil {
				httputils.ReportError(w, err, "Failed to update server.")
				return
			}
...
func (s *Server) changeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseForm(); err != nil {
		httputils.ReportError(w, err, "Failed to parse form.")
		return
	}
	...
	if err != nil {
		httputils.ReportError(w, err, fmt.Sprintf("Failed to reach %s: %v %s", machine, resp, err))
		return
	}
	defer util.Close(resp.Body)
	if resp.StatusCode != 200 {
