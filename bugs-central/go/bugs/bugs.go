...
type BugFramework interface {
	...
	// Search returns issues that match the provided parameters. Also returns how many of those issues are unassigned.
	Search(ctx context.Context, config interface{}) ([]*Issue, int, error)

	// SearchClientAndPersist queries issues that match the provided client parameters.
	// The results are then put into DB and into the specified object for open issues.
	SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB) error
}
