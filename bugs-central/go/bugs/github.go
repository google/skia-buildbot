...
func (gh *Github) Search(ctx context.Context, config interface{}) ([]*Issue, int, error) {
	githubQueryConfig, ok := config.(GithubQueryConfig)
	if !ok {
		return nil, -1, errors.New("config must be GithubQueryConfig")
	}

	githubIssues, err := gh.githubClient.GetIssues(githubQueryConfig.Open, githubQueryConfig.Labels, MAX_GITHUB_RESULTS)
	if err != nil {
		return nil, -1, skerr.Wrapf(err, "could not get github issues with %s labels", githubQueryConfig.Labels)
	}
...
func (gh *Github) SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB) error {
	qc, ok := config.(GithubQueryConfig)
	if !ok {
		return errors.New("config must be GithubQueryConfig")
	}

	issues, unassignedCount, err := gh.Search(ctx, qc)
	if err != nil {
		return skerr.Wrapf(err, "error when searching github", err)
	}
	sklog.Infof("%s Github issues open:%d unassigned:%d", qc.Client, len(issues), unassignedCount)
	...
	// Put in DB
	if err := dbClient.PutInDB(ctx, client, GithubSource, queryDesc, queryLink, len(issues), unassignedCount); err != nil {
		return skerr.Wrapf(err, "error putting github results in DB")
	}
	// Put in memory.
	putOpenIssues(client, GithubSource, queryDesc, issues)
	return nil
}
