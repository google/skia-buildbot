...
func (gh *Github) PutInDB(ctx context.Context, config interface{}, openCount, unassignedCount int, dbClient *db.FirestoreDB) error {
	githubQueryConfig, ok := config.(GithubQueryConfig)
	if !ok {
	...
	if err := dbClient.PutInDB(ctx, githubQueryConfig.Client, GithubSource, queryDesc, queryLink, openCount, unassignedCount); err != nil {
