...
func (it *IssueTracker) Search(ctx context.Context, config interface{}) ([]*Issue, int, error) {
	itQueryConfig, ok := config.(IssueTrackerQueryConfig)
	if !ok {
		return nil, -1, errors.New("config must be IssueTrackerQueryConfig")
	}

	obj := it.storageClient.Bucket(IssueTrackerBucket).Object(ResultsFileName)
	reader, err := obj.NewReader(ctx)
	if err != nil {
		return nil, -1, skerr.Wrapf(err, "accessing gs://%s/%s failed", IssueTrackerBucket, ResultsFileName)
	}
	defer util.Close(reader)

	var results map[string][]IssueTrackerIssue
	if err := json.NewDecoder(reader).Decode(&results); err != nil {
		return nil, -1, skerr.Wrapf(err, "invalid JSON from %s", ResultsFileName)
	}

	if _, ok := results[itQueryConfig.Query]; !ok {
		return nil, -1, skerr.Fmt("could not find %s in %s", itQueryConfig.Query, ResultsFileName)
	}
...
func (it *IssueTracker) SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB) error {
	qc, ok := config.(IssueTrackerQueryConfig)
	if !ok {
		return errors.New("config must be IssueTrackerQueryConfig")
	}

	issues, unassignedCount, err := it.Search(ctx, qc)
	if err != nil {
		return skerr.Wrapf(err, "error when searching issuetracker")
	}
	sklog.Infof("%s issuetracker issues open:%d unassigned:%d", qc.Client, len(issues), unassignedCount)

	queryDesc := qc.Query
	queryLink := fmt.Sprintf("http://b/issues?q=%s", qc.Query)
	client := qc.Client

	// Put in DB.
	if err := dbClient.PutInDB(ctx, client, IssueTrackerSource, queryDesc, queryLink, len(issues), unassignedCount); err != nil {
