...
func (it *IssueTracker) Search(ctx context.Context, config interface{}) ([]*Issue, *types.IssueCountsData, error) {
	...
	for _, i := range trackerIssues {
		...
		countsData.IncPriority(types.StandardizedPriority(i.Priority))
...
func (it *IssueTracker) SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB) error {
	...
	// Put in DB.
	if err := dbClient.PutInDB(ctx, client, IssueTrackerSource, queryDesc, queryLink, countsData); err != nil {
