...
func (it *IssueTracker) PutInDB(ctx context.Context, config interface{}, openCount, unassignedCount int, dbClient *db.FirestoreDB) error {
	itQueryConfig, ok := config.(IssueTrackerQueryConfig)
	if !ok {
	...
	if err := dbClient.PutInDB(ctx, itQueryConfig.Client, IssueTrackerSource, itQueryConfig.Query, queryLink, openCount, unassignedCount); err != nil {
