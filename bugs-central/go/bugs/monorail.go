...
func (m *Monorail) PutInDB(ctx context.Context, config interface{}, count int, dbClient *db.FirestoreDB) error {
	mQueryConfig, ok := config.(MonorailQueryConfig)
	if !ok {
	...
	if err := dbClient.PutInDB(ctx, ChromiumClient, MonorailSource, mQueryConfig.Query, queryLink, count); err != nil {
