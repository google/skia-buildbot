...
func (m *Monorail) SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB) error {
	...
	issues, unassignedCount, err := m.Search(ctx, qc)
	if err != nil {
		return skerr.Wrapf(err, "error when searching monorail", err)
	}
	sklog.Infof("%s Monorail issues open:%d unassigned:%d", qc.Client, len(issues), unassignedCount)
	...
	// Put in DB
	if err := dbClient.PutInDB(ctx, client, MonorailSource, queryDesc, queryLink, len(issues), unassignedCount); err != nil {
