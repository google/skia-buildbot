...
func (m *Monorail) Search(ctx context.Context, config interface{}) ([]*Issue, *types.IssueCountsData, error) {
	mQueryConfig, ok := config.(MonorailQueryConfig)
	if !ok {
		return nil, nil, errors.New("config must be MonorailQueryConfig")
	}
	...
	for _, mi := range monorailIssues {
		...
		issues = append(issues, &Issue{
			...
			Priority: priority,
			Owner:    owner,
			Link:     m.GetIssueLink(mQueryConfig.Instance, id),
...
func (m *Monorail) SearchClientAndPersist(ctx context.Context, config interface{}, dbClient *db.FirestoreDB, runId string) error {
	qc, ok := config.(MonorailQueryConfig)
	if !ok {
		return errors.New("config must be MonorailQueryConfig")
	}

	issues, countsData, err := m.Search(ctx, qc)
	if err != nil {
	...
	queryDesc := qc.Query
	queryLink := fmt.Sprintf("https://bugs.chromium.org/p/%s/issues/list?can=2&q=%s", qc.Instance, qc.Query)
	client := qc.Client

	// Put in DB.
	if err := dbClient.PutInDB(ctx, client, MonorailSource, queryDesc, queryLink, runId, countsData); err != nil {
		return skerr.Wrapf(err, "error putting monorail results in DB")
	}
	// Put in memory.
	putOpenIssues(client, MonorailSource, queryDesc, issues)
	return nil
}
