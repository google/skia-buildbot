...
func (p *IssuesPoller) GetCounts(ctx context.Context, client types.RecognizedClient, source types.IssueSource, query string) (*types.IssueCountsData, string, error) {
...
func (p *IssuesPoller) StartPoll(pollInterval time.Duration) {
	...
	qcd, _, err := p.dbClient.GetCountsFromDB(context.Background(), "", "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(qcd)
	qcd, _, err = p.dbClient.GetCountsFromDB(context.Background(), AndroidClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(qcd)
	qcd, _, err = p.dbClient.GetCountsFromDB(context.Background(), ChromiumClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(qcd)
	qcd, _, err = p.dbClient.GetCountsFromDB(context.Background(), FlutterNativeClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(qcd)
	qcd, _, err = p.dbClient.GetCountsFromDB(context.Background(), FlutterOnWebClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	...
	dateToPriorityData := map[string]PriorityData{}
	for _, qd := range qds {
		d := qd.Created.UTC().Format("2006-01-02")
		if _, ok := dateToPriorityData[d]; !ok {
			dateToPriorityData[d] = PriorityData{}
		}
		pd := dateToPriorityData[d]
		fmt.Println(pd)

		data = append(data, []interface{}{
			qd.Created.UTC().Format("2006-01-02T15:04:05.000Z"),
			// "2020-10-01", 1, 9, 10, 12
		})
	}
