...
func (p *IssuesPoller) StartPoll(pollInterval time.Duration) {
	...
	cleanup.Repeat(pollInterval, func(_ context.Context) {
		...
		issueTrackerIssues, err := p.issueTrackerClient.Search(ctx, androidQueryConfig)
		if err != nil {
		...
		if err := p.issueTrackerClient.PutInDB(ctx, androidQueryConfig, len(issueTrackerIssues), p.dbClient); err != nil {
		...
		flutterOnWebGithubIssues, err := p.githubClient.Search(ctx, flutterOnWebQueryConfig)
		if err != nil {
		...
		if err := p.githubClient.PutInDB(ctx, flutterOnWebQueryConfig, len(flutterOnWebGithubIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
		...
		flutterNativeGithubIssues, err := p.githubClient.Search(ctx, flutterNativeQueryConfig)
		if err != nil {
			sklog.Errorf("Error when polling github: %s", err)
			return
		}
		...
		if err := p.githubClient.PutInDB(ctx, flutterNativeQueryConfig, len(flutterNativeGithubIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
		...
		skiaCrMonorailIssues, err := p.monorailClient.Search(ctx, qc1)
		if err != nil {
		...
		if err := p.monorailClient.PutInDB(ctx, qc1, len(skiaCrMonorailIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
		...
		skiaCompositingCrMonorailIssues, err := p.monorailClient.Search(ctx, qc2)
		if err != nil {
			sklog.Errorf("Error when polling monorail: %s", err)
			return
		}
		...
		if err := p.monorailClient.PutInDB(ctx, qc2, len(skiaCompositingCrMonorailIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
		...
		skiaPdfCrMonorailIssues, err := p.monorailClient.Search(ctx, qc3)
		if err != nil {
			sklog.Errorf("Error when polling monorail: %s", err)
			return
		}
		...
		if err := p.monorailClient.PutInDB(ctx, qc3, len(skiaPdfCrMonorailIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
		...
		skiaSkMonorailIssues, err := p.monorailClient.Search(ctx, qc4)
		if err != nil {
			sklog.Errorf("Error when polling monorail: %s", err)
			return
		}
		...
		if err := p.monorailClient.PutInDB(ctx, qc4, len(skiaSkMonorailIssues), p.dbClient); err != nil {
			sklog.Errorf("Error putting into DB: %s", err)
			return
		}
