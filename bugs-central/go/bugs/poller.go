...
func (p *IssuesPoller) GetCounts(ctx context.Context, client types.RecognizedClient, source types.IssueSource, query string) (int, int, string, error) {
...
func (p *IssuesPoller) StartPoll(pollInterval time.Duration) {
	...
	openCounts, untriagedCounts, _, err := p.dbClient.GetCountsFromDB(context.Background(), "", "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	...
	openCounts, untriagedCounts, _, err = p.dbClient.GetCountsFromDB(context.Background(), AndroidClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(openCounts)
	fmt.Println(untriagedCounts)
	openCounts, untriagedCounts, _, err = p.dbClient.GetCountsFromDB(context.Background(), ChromiumClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(openCounts)
	fmt.Println(untriagedCounts)
	openCounts, untriagedCounts, _, err = p.dbClient.GetCountsFromDB(context.Background(), FlutterNativeClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
	fmt.Println(openCounts)
	fmt.Println(untriagedCounts)
	openCounts, untriagedCounts, _, err = p.dbClient.GetCountsFromDB(context.Background(), FlutterOnWebClient, "", "")
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println("GOT THIS:")
