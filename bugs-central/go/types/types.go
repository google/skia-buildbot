...
func (qcd *IssueCountsData) CalculateSLOViolations(created, modified time.Time, priority StandardizedPriority) {
	now := time.Now()
	switch priority {
	case PriorityP0:
...
func (qcd *IssueCountsData) MergeInto(from ...*IssueCountsData) {
	for _, f := range from {
...
// 		qcd.P4Count == compareWith.P4Count &&
// 		qcd.P5Count == compareWith.P5Count &&
// 		qcd.P6Count == compareWith.P6Count
// }
