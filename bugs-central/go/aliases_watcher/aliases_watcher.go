...
// Watches different clients+issue frameworks for open issues assigned to a rotation alias.
...
// AliasWatcher will be used to watch for specific issues in the different issue frameworks.
type AliasWatcher struct {
	httpClient               *http.Client
	pathToServiceAccountFile string
}

// New returns an instance of AliasWatcher.
func New(ctx context.Context, ts oauth2.TokenSource, pathToServiceAccountFile string) (*AliasWatcher, error) {
...
func (p *AliasWatcher) Start(ctx context.Context, pollInterval time.Duration) error {
