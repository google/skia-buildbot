...
func (f *FirestoreDB) getAllSourcesCountsFromDB(ctx context.Context, client types.RecognizedClient) (int, int, error) {
	return -1, -1, nil
}

func (f *FirestoreDB) getAllQueriesCountsFromDB(ctx context.Context, client types.RecognizedClient, source types.IssueSource) (int, int, error) {
	return -1, -1, nil
}
	...
	if client == "" {
		...
		for {
			...
			for {
				...
				itQueries := s.Ref.Collections(ctx)
				for {
