...
func (f *FirestoreDB) getAllQueriesCountsFromDB(ctx context.Context, sourceDoc *firestore_api.DocumentRef) (int, int, error) {
	...
	for {
		...
		o, u, _, err := f.getLatestCountsFromQuery(ctx, query)
		openCount += o
		unassignedCount += u
	}
...
func (f *FirestoreDB) getLatestCountsFromQuery(ctx context.Context, queryCol *firestore_api.CollectionRef) (int, int, string, error) {
	var qd *QueryData
	...
	return qd.OpenCount, qd.UnassignedCount, qd.QueryLink, nil
}

func (f *FirestoreDB) GetCountsFromDB(ctx context.Context, client types.RecognizedClient, source types.IssueSource, query string) (int, int, string, error) {
	mtx.RLock()
	defer mtx.RUnlock()

	if client == "" {
		// Client has not been specified. Return the total count of all clients.
		o, u, err := f.getAllClientsCountsFromDB(ctx)
		return o, u, "", err
	}
	...
	if source == "" {
		...
		o, u, err := f.getAllSourcesCountsFromDB(ctx, clientCol)
		return o, u, "", err
	}
	...
	if query == "" {
		...
		o, u, err := f.getAllQueriesCountsFromDB(ctx, sourceDoc)
		return o, u, "", err
	}
...
func (f *FirestoreDB) PutInDB(ctx context.Context, client types.RecognizedClient, source types.IssueSource, query, queryLink string, openCount, unassignedCount int) error {
	...
	o, u, _, err := f.GetCountsFromDB(ctx, client, source, query)
