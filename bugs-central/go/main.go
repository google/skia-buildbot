...
var (
	...
	openIssuesAccrossFrameworks = []bugs.Issue{}
...
func New() (baseapp.App, error) {
	...
	cleanup.Repeat(*pollInterval, func(_ context.Context) {
		...
		// uniqueOpenIssues will be converted to openIssuesAccrossFrameworks at the end.
		...
		// Exclude any issues from flutterNativeGithubIssues that are already in flutterOnWebGithubIssues.
		// Doing this manually because there is no exclude label ability in Github API.
		// TODO(rmistry)
...
func addToOpenIssues(client bugs.RecognizedClient, source bugs.IssueSource, issues []*bugs.Issue) {
	...
	if frameworkToIssues, ok := openIssuesClientToSource[client]; ok {
		...
		frameworkIssues := frameworkToIssues[source]
		frameworkIssues = append(frameworkIssues, issues...)
	} else {
		// Create new slice and add issues.
		fmt.Printn("NOT FOUND")
