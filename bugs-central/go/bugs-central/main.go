...
// TODO(rmistry): Need some init flag or this will return 0.
// TODO(rmistry): Better: Do not use this for inmemory object, read directly from DB.
//                All endpoints should only read from DB...
...
func (srv *Server) getIssueCountsHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("DEBUGGINT THIS!!!")
	fmt.Println(q.Client == "")
	fmt.Println(q.Source == "")
	fmt.Println(q.Query == "")

	o, u, err := srv.pollerClient.GetCounts(r.Context(), q.Client, q.Source, q.Query)
	if err != nil {
