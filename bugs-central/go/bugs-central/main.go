...
var (
	...
	aliasWatcherInterval = flag.Duration("alias_watcher_interval", 2*time.Minute, "How often the server will look for issues assigned to rotation aliases.")
)
...
func New() (baseapp.App, error) {
	...
	aliasesWatcher, err := watcher.New(ctx, ts, *serviceAccountFile)
	if err != nil {
		sklog.Fatal("Could not init watcher: %s", err)
	}
	if err := aliasesWatcher.Start(ctx, *aliasWatcherInterval); err != nil {
