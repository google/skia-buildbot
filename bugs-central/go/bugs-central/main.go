...
func (srv *Server) getIssuesData(w http.ResponseWriter, r *http.Request) {
	...
	qds, err := srv.pollerClient.GetQueryData(context.Background(), types.RecognizedClient(client), types.IssueSource(source), query)
	if err != nil {
		sklog.Fatal(err)
	}
	...
	validRunIds, err := srv.pollerClient.GetAllRecognizedRunIds(r.Context())
	if err != nil {
		httputils.ReportError(w, err, "Failed to get valid runIds from DB", http.StatusInternalServerError)
		return
	}
	/*
		Thu, 15 Oct 2020 14:08:01 UTC
		PUTTING THIS IN
		Thu, 15 Oct 2020 14:24:59 UTC
		PUTTING THIS IN
		Thu, 15 Oct 2020 14:19:09 UTC
		PUTTING THIS IN
		Thu, 15 Oct 2020 14:14:08 UTC
	*/
	for _, qd := range qds {

		// Is the runId valid for this qd? if not then ignore it. Cache runIds.
		...
		// Calculate the date from the
		// t, err := time.Parse(qd.RunId, util.SAFE_TIMESTAMP_FORMAT)
		// Might as well try using the runId as the date to see what happens!!!

		// d := qd.Created.UTC().Format("2006-01-02")
		// d := qd.RunId
		// Convert the timestamp into something more user consumable from
		// https://stackoverflow.com/questions/36582789/javascript-toisostring-time-in-golang
		// fmt.Println("HERE HERE")
		// d1, err := time.Parse(time.RFC3339, qd.RunId)
		// if err != nil {
		// 	httputils.ReportError(w, err, fmt.Sprintf("Failed to time.Parse the string %s", qd.RunId), http.StatusInternalServerError)
		// 	return
		// }
		fmt.Println("HERE HERE")
...
func (srv *Server) getIssueCountsHandler(w http.ResponseWriter, r *http.Request) {
	...
	countsData, err := srv.pollerClient.GetCounts(r.Context(), q.Client, q.Source, q.Query)
	if err != nil {
		httputils.ReportError(w, err, "Failed to get issue counts", http.StatusInternalServerError)
		return
	}
