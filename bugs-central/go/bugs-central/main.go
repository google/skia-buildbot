...
func (srv *Server) getIssuesData(w http.ResponseWriter, r *http.Request) {
	...
	for _, qd := range qds {
		...
		if _, ok := validRunIds[qd.RunId]; !ok {
			valid, err := srv.pollerClient.IsRunIdValid(r.Context(), qd.RunId)
			if err != nil {
				httputils.ReportError(w, err, "Failed to validate runId from DB", http.StatusInternalServerError)
				return
			}
			if !valid {
			...
			// Cache the runId in memory.
			validRunIds[qd.RunId] = true
		}
