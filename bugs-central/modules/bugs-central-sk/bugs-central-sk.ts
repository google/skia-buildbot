...
export class BugsCentralSk extends ElementSk {
  ...
  <div class="charts-container">
    ...
    <div class="chart-div">
      ...
      <div class="chart-div">
      <bugs-chart-sk chart_type='${ChartType.UNTRIAGED}'
  ...
  private async _doImpl(url: string, detail: any, action: (json: any)=> void): Promise<void> {
    await fetch(url, {
  ...
  private async _getCounts(client: string, source: string, query: string) {
    const detail = {
      client: client,
      source: source,
      query: query,
  ...
  private async _populateCountsAndRender() {
    this._clients_to_counts = {};
    ...
    if (!c) {
      await Promise.all(Object.keys(this._clients_map).map(async (client) => this._clients_to_counts[getClientKey(client, '', '')] = await this._getCounts(client, '', '')));
    } else if (!s) {
      await Promise.all(Object.keys(this._clients_map[c]).map(async (source) => this._clients_to_counts[getClientKey(c, source, '')] = await this._getCounts(c, source, '')));
    } else if (!q) {
      await Promise.all(Object.keys(this._clients_map[c][s]).map(async (query) => this._clients_to_counts[getClientKey(c, s, query)] = await this._getCounts(c, s, query)));
    } else {
