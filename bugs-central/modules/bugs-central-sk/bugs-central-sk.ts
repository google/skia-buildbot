...
declare interface IssueCountsResp {
...
`;
    ...
    for (let i = 0; i < clientKeys.length; i++) {
      ...
      rowsHTML.push(html`
        <tr>
          ...
          <td>${clientCounts.p0_count}</td>
          <td>${clientCounts.p1_count}</td>
  ...
  async _getCounts(client: string, source: string, query: string) {
    ...
    let p2_count = 0;
    let query_link = '';
    const countsData = {} as CountsData;
    await this._doImpl('/_/get_issue_counts', detail, (json: CountsData) => {
      countsData.open_count = json.CountsData.open_count;
      countsData.unassigned_count = json.unassigned_count;
      countsData.p0_count = json.p0_count;
      countsData.p1_count = json.p1_count;
      countsData.p2_count = json.p2_count;
      countsData =
