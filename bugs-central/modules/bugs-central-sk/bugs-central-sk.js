...
function displayClientsRows(ele) {
  ...
  for (let i=0; i<clientKeys.length; i++) {
    ...
    const clientCounts = ele._clients_to_counts[clientKey];
    rowsHTML.push(html`
      <tr>
        <td @click=${() => ele._click(clientKey)}>
          <span class=client-link>${clientKey}</span>
          ${clientCounts.query_link ? html`<span class=query-link>[<a href="${clientCounts.query_link}" target=_blank>issues query</a>]</span>` : ''}
        </td>
