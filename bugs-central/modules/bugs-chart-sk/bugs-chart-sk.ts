...
function getChartOptions(type: string) {
    const displayType = type == ChartType.OPEN ? 'area' : 'line';
    const isStacked = type == ChartType.OPEN ? true : false;
...
export class BugsChartSk extends ElementSk {
    ...
    private static template = (el: BugsChartSk) => html`
      ...
      <google-chart
        ...
        data="/_/get_chart_data?client=${el.client}&source=${el.source}&query=${el.query}&type=${el.chart_type}">
      </google-chart>
`;
