...
func ingestBuild(build *buildbot.Build, commitHash, target string) error {
	...
	if existingBuild == nil {
		...
		// Copy Task ID if necessary.
		if id, err := existingBuild.GetStringProperty("taskId"); err == nil {
			build.Properties = append(build.Properties, []interface{}{"taskId", id, "datahopper_internal"})
...
// taskRequest adds/updates task using a POST/PUT request to Task Scheduler.
func taskRequest(method string, task *db.Task) error {
...
// addOrUpdateTask updates the Task Scheduler with the current state of build.
func addOrUpdateTask(build *buildbot.Build) error {
	if id, err := build.GetStringProperty("taskId"); err != nil {
		return addTaskAndUpdateBuild(build)
	} else {
		return updateTask(id, build)
	}
}

// addTaskAndUpdateBuild adds a task corresponding to build to the Task Scheduler DB, and saves the
// task ID as a build property.
func addTaskAndUpdateBuild(build *buildbot.Build) error {
	task := &db.Task{
		Created:  build.Started,
	...
	if err := taskRequest(http.MethodPost, task); err != nil {
		return err
	}

	// Save task ID in build.Properties.
	build.Properties = append(build.Properties, []interface{}{"taskId", task.Id, "datahopper_internal"})
	props, err := json.Marshal(build.Properties)
	if err == nil {
		build.PropertiesStr = string(props)
	} else {
	...
	sklog.Infof("Updating build %s %d with task ID %s", build.Builder, build.Number, task.Id)
	if err := buildbot.IngestBuild(buildbotDB, build, repos); err != nil {
		return fmt.Errorf("Failed to update build: %s", err)
	}
	return nil
}
...
func updateTask(id string, build *buildbot.Build) error {
	...
	return taskRequest(http.MethodPut, task)
}
