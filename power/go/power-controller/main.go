...
func main() {
	...
	authedClient, err := setupAuthentication()
	if err != nil {
...
func setupAuthentication() (*http.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Could not set up autheticated HTTP client: %s", err)
	}
	...
	if err := login.Init(useRedirectURL, *authWhiteList); err != nil {
		return nil, fmt.Errorf("Problem setting up server OAuth: %s", err)
	}
