...
func setupMockRecorder(mr *recorder.MockRecorder) {
	mr.On("NewlyFixedBots", mock.AnythingOfType("[]string")).Return()
	mr.On("NewlyDownBots", mock.AnythingOfType("[]string")).Return()
}
...
func testOneMissingBot(t *testing.T, mi, me *skswarming.MockApiClient, ma *promalertsclient.MockAPIClient, md *decider.MockDecider, mr *recorder.MockRecorder) {
	...
	g := NewPollingGatherer(me, mi, ma, md, mr, hostMap, 0).(*gatherer)
	g.update()
...
func testRecentlyDownBots(t *testing.T, mi, me *skswarming.MockApiClient, ma *promalertsclient.MockAPIClient, md *decider.MockDecider, mr *recorder.MockRecorder) {
	...
	mi.On("ListDownBots", mock.AnythingOfType("string")).Return([]*swarming.SwarmingRpcsBotInfo{}, nil)
	...
	for i, pool := range skswarming.POOLS_PUBLIC {
		...
		me.On("ListDownBots", pool).Return([]*swarming.SwarmingRpcsBotInfo{}, nil)
	}
	me.On("ListDownBots", skswarming.POOLS_PUBLIC[0]).Return([]*swarming.SwarmingRpcsBotInfo{}, nil).Once()
