...
const stepProperties = (ele, s) => html`
  <div class="table properties">
    ...
    ${s.data ? s.data.map((d) => tr(stepData(s, d))) : ""}
...
function stepData(s, d) {
  switch(d.type) {
    case "command":
      return html`${td("Command")}${td(d.data.command.join(" "))}`;
    case "httpRequest":
      return html`${td("HTTP Request")}${td(d.data.url)}`;
    case "httpResponse":
      return html`${td("HTTP Response")}${td(d.data.status)}`;
    case "log":
      return html`${td("Log (" + d.data.name + ")")}${td(html`
