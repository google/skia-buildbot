...
// Attach the given error as an exception to the step. The step will have the
// EXCEPTION result, indicating that there was a non-test-related failure.
func Exception(ctx context.Context, err error) error {
	props := getStep(ctx)
	getRun(ctx).Exception(props.Id, err)
	return err
}
...
func finishStep(ctx context.Context, recovered interface{}) {
	if recovered != nil {
		Exception(ctx, fmt.Errorf("Caught panic: %s", recovered))
		defer panic(recovered)
	}
	getRun(ctx).Finish(getStep(ctx).Id)
}
