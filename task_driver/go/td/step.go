...
func newStep(ctx context.Context, id string, parent *StepProperties, props *StepProperties) context.Context {
	...
	env := deriveEnv(ctx)
	props.Environ = MergeEnv(env, props.Environ)

	// Unset the environment. This prevents us from inheriting
	// environments in the wrong order; we don't want children
	// of this step inheriting the environment from SetEnv because
	// those variables are already passed down through this step.
	ctx = SetEnv(ctx, nil)
...
func execCtx(ctx context.Context) context.Context {
	return exec.NewContext(ctx, func(cmd *exec.Command) error {
		...
		cmd.Env = MergeEnv(deriveEnv(ctx), cmd.Env)
