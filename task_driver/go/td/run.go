...
const (
	...
	ENVVAR_SWARMING_TASK   = "SWARMING_TASK"
)
...
func StartRunWithErr(projectId, taskId, taskName, output *string, local *bool) (context.Context, error) {
	...
	ctx = newRun(ctx, receiver, *taskId, props)
	return ctx, nil
}
...
func newRun(ctx context.Context, rec Receiver, taskId string, props *RunProperties) context.Context {
	r := &run{
		receiver: rec,
		taskId:   taskId,
	}
	...
	ctx = newStep(ctx, STEP_ID_ROOT, nil, Props(STEP_ID_ROOT))
	return ctx
}
