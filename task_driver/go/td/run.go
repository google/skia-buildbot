...
// occurs.
func StartRunWithErr(projectId *string, rs db.RepoState, taskName, output *string, local *bool) (context.Context, error) {
	common.Init()
	...
	// Generate a random "sequence" number. This is really dumb, but until
	// we can use IDs generated by a central server, there's no good way to
	// to it. For now we'll just live with any collisions which occur.
	now := time.Now()
	rand.Seed(now.UnixNano())
	seq := rand.Uint64()
	taskId := CreateID(rs, time.Now(), seq)
	if *local {
		...
		taskId = fmt.Sprintf("%s_%s", uuid.New(), hostname)
	}
	if *projectId == "" {
...
func StartRun(projectId *string, rs db.RepoState, taskName, output *string, local *bool) context.Context {
	ctx, err := StartRunWithErr(projectId, rs, taskName, output, local)
	if err != nil {
