...
type DB interface {
	// InsertTaskDriver inserts the TaskDriver instance into the DB.
	InsertTaskDriver(*TaskDriver) error
	...
	GetTaskDriver(string) (*TaskDriver, error)
	...
	UpdateTaskDriver(string, func(*TaskDriver) error) error
}
...
func (s *Step) Copy() *Step {
	...
	var data []interface{}
	if s.Data != nil {
		data = append(make([]interface{}, 0, len(s.Data)), s.Data...)
	}
	return &Step{
...
// Copy returns a deep copy of the TaskDriver.
func (t *TaskDriver) Copy() *TaskDriver {
	if t == nil {
		return nil
	}
...
// Update a TaskDriver from the given Message.
func UpdateFromMessage(db DB, m *td.Message) error {
	// TODO(borenet): This is not thread-safe!
	return db.UpdateTaskDriver(m.TaskId, func(t *TaskDriver) error {
		if t.Steps == nil {
			t.Steps = map[string]*Step{}
		}
		step, ok := t.Steps[m.StepId]
		if !ok {
			// Create a near-empty step with the minimal amount of
			// information for us to use it.
			step = &Step{
				Properties: &td.StepProperties{
					Id: m.StepId,
				},
		...
		case td.MSG_TYPE_STEP_DATA:
			step.Data = append(step.Data, m.Data)
