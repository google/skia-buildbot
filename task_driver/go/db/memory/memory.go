...
type memoryDB struct {
	mtx         sync.Mutex
	taskDrivers map[string]*db.TaskDriver
}

// See documentation for db.DB interface.
func (d *memoryDB) InsertTaskDriver(t *db.TaskDriver) error {
	d.mtx.Lock()
	defer d.mtx.Unlock()
...
// See documentation for db.DB interface.
func (d *memoryDB) GetTaskDriver(id string) (*db.TaskDriver, error) {
	d.mtx.Lock()
	defer d.mtx.Unlock()
...
// See documentation for db.DB interface.
func (d *memoryDB) UpdateTaskDriver(id string, fn func(*db.TaskDriver) error) error {
	d.mtx.Lock()
	defer d.mtx.Unlock()
	t := d.taskDrivers[id]
	if t == nil {
