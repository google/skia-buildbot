...
func NewBigTableDB(ctx context.Context, project string, ts oauth2.TokenSource) (db.DB, error) {
	client, err := bigtable.NewClient(ctx, project, BT_INSTANCE, option.WithTokenSource(ts))
	if err != nil {
...
func (d *btDB) GetTaskDriver(id string) (*db.TaskDriverRun, error) {
	...
	sklog.Infof("Retrieved %d messages", len(msgs))
	t := &db.TaskDriverRun{}
	...
	sklog.Infof("Got task driver: %+v", t)
	return t, nil
}
...
func (d *btDB) UpdateTaskDriver(id string, msg *td.Message) error {
	...
	rk := rowKey(id, msg)
	sklog.Infof("Writing row with key %s", rk)
