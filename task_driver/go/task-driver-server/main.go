...
func handleMessage(msg *pubsub.Message) error {
	...
	sklog.Infof("Got message: %+v", e)
	if e.JsonPayload != nil {
		sklog.Infof("got JSON payload: %+v", e.JsonPayload)
		...
		sklog.Infof("Successfully inserted task driver update.")
	} else if e.TextPayload != "" {
		if err := lm.Insert(&e); err != nil {
			msg.Nack()
			return fmt.Errorf("Failed to insert log entry: %s", err)
		}
		sklog.Infof("Successfully inserted text payload.")
	} else {
		msg.Ack()
