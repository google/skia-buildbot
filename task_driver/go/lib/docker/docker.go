...
func (d *Docker) Extract(ctx context.Context, image, src, dest string) error {
	return td.Do(ctx, td.Props(fmt.Sprintf("Extract %s %s:%s", image, src, dest)), func(ctx context.Context) (rv error) {
		...
		out, err := sk_exec.RunCwd(ctx, ".", dockerCmd, "--config", d.configDir, "create", image, "dummy-cmd")
		if err != nil {
			return err
		}
		...
		defer func() {
			_, err := sk_exec.RunCwd(ctx, ".", dockerCmd, "rm", "-v", id)
			if err != nil {
				rv = err
			}
		}()
		...
		// Perform the copy.
		_, err = sk_exec.RunCwd(ctx, ".", dockerCmd, "cp", "-L", fmt.Sprintf("%s:%s", id, src), dest)
		return err
	})
}
...
func Push(ctx context.Context, tag, configDir string) (string, error) {
	...
	out, err := sk_exec.RunSimple(ctx, pushCmd)
	if err != nil {
		return "", err
	}
...
func BuildPushImageFromInfraImage(ctx context.Context, appName, image, tag, repo, configDir, workDir, infraImageTag string, topic *pubsub.Topic, cmd, volumes []string, env, buildArgs map[string]string) error {
