...
// tagged with 'tag'.
func Build(ctx context.Context, directory string, tag string, env map[string]string) error {
	ctx = td.StartStep(ctx, td.Props(fmt.Sprintf("docker %s", directory)))
	defer td.EndStep(ctx)

	// Confirm a Dockerfile exists in dir.

	// Run "docker build -t <some tag name> ." in 'directory' and stream the
	// output.
	...
	cmd := exec.CommandContext(ctx, "docker", "build", "-t", tag, directory)
	...
	// Spin up a Go routine to handle the steps of the Docker build.
