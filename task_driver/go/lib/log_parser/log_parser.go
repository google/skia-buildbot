...
func newStep(ctx context.Context, name string, parent, root *Step) *Step {
	...
	stdout := io.MultiWriter(logBuf, td.NewLogStream(ctx, "stdout", td.Info))
	stderr := io.MultiWriter(logBuf, td.NewLogStream(ctx, "stderr", td.Error))
...
// its ancestors except for the root step, which automatically receives all output.
