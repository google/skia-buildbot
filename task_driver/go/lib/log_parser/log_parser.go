...
func Run(ctx context.Context, cwd string, cmdLine []string, split bufio.SplitFunc, handleToken func(*StepManager, string) error) error {
	...
	// TODO(borenet): Are stdout and stderr going to be interleaved
	// correctly? If not, log output may be attributed to the wrong step. If
	// interleaved, we may encounter problems if stdout and stderr are not
	// buffered so that we see full lines from each at a time. An
	// alternative might be to use a separate cmd.StderrPipe(), spin up a
	// goroutine to read from it, and pass its output along to any active
	// step.
	...
	go func() {
		defer wg.Done()
		log := td.NewLogStream(ctx, "stderr", td.Error)
		scanner := bufio.NewScanner(stderr)
		for scanner.Scan() {
			_, err := log.Write([]byte(scanner.Text()))
			if err != nil {
				sklog.Errorf("Failed to write logs: %s", err)
			}
		}
	}()
