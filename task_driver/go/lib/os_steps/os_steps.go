...
func Stat(ctx context.Context, path string) (os.FileInfo, error) {
	var rv os.FileInfo
	return rv, td.Do(ctx, td.Name(fmt.Sprintf("Stat %s", path)).Infra(), func(context.Context) error {
		fi, err := os.Stat(path)
		rv = fi
		return err
	})
...
func MkdirAll(ctx context.Context, path string) (err error) {
	return td.Do(ctx, td.Name(fmt.Sprintf("MkdirAll %s", path)).Infra(), func(context.Context) error {
...
func RemoveAll(ctx context.Context, path string) (err error) {
	return td.Do(ctx, td.Name(fmt.Sprintf("RemoveAll %s", path)).Infra(), func(context.Context) error {
