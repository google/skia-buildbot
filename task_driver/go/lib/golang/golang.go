...
import (
	...
	"go.skia.org/infra/task_driver/go/lib/env"
)

var (
	goEnv env.Env // Filled in by Init().
)
...
func Init(workdir string) env.Env {
	goEnv = getEnv(workdir)
	return goEnv
}

// getEnv returns the full set of environment variables which should be used
// when running Go commands.
func getEnv(workdir string) env.Env {
	...
	PATH := strings.Join([]string{
		...
		env.PATH_PLACEHOLDER,
	}, string(os.PathListSeparator))
	return env.Base(workdir).Merge([]string{
