...
func EnsureGitCheckout(ctx context.Context, dest string, rs db.RepoState) (co *git.Checkout, err error) {
	ctx = td.StartStep(ctx, td.Infra(), td.Name("Ensure Git Checkout"))
	defer td.EndStep(ctx, &err)
...
func EnsureGitCheckoutWithDEPS(ctx context.Context, workdir string, rs db.RepoState) (co *git.Checkout, err error) {
	ctx = td.StartStep(ctx, td.Infra(), td.Name("Ensure Git Checkout (with DEPS)"))
	defer td.EndStep(ctx, &err)
