...
//
// Using a ramdisk as the Bazel cache prevents "disk stall detected" errors on GCE VMs due to slow
// I/O.
func NewWithRamdisk(ctx context.Context, workspace string, rbeCredentialFile string) (*Bazel, func(), error) {
	...
	cleanup := func() {
		...
		// OK because the Swarming bot will reboot shortly after the task finishes.
		//
