...
func (e Env) Merge(other Env) Env {
	...
	for _, kv := range other {
		...
		if existing, ok := m[k]; ok && k == PATH_VAR {
			sklog.Infof("Old PATH: %s", existing)
			sklog.Infof("New PATH: %s", v)
...
// Base returns the base Env used by all steps.
func Base() Env {
	return util.CopyStringSlice(td.BASE_ENV)
}
