...
func main() {
	...
	props := td.Name("named infra step with env").Env(env).Infra()
...
func RunStepFunc(ctx context.Context) error {
	...
	if err := td.Do(ctx, td.Name("parent step"), func(ctx context.Context) error {
		if err := td.Do(ctx, td.Name("sub-step 1"), func(ctx context.Context) error {
			// Perform some work.
			return doSomething()
		}); err != nil {
		...
		return td.Do(ctx, td.Name("sub-step 2"), func(ctx context.Context) error {
