const init = () => {
  ...
  document.querySelector('#thunder-button').addEventListener('click', () => {
    resumeIfNecessary();
    setFadeout(sources.thunder.element, sources.thunder.gainNode);
  ...
  const formula = 'sum (rate (container_cpu_usage_seconds_total[10m]))';
    ...
    setTimeout(async () => {
      try {
        const resp = await fetch(
          `http://localhost:9090/api/v1/query?query=${encodeURIComponent(
            formula
          )}`,
          {
            headers: {
        ...
        sources.crickets.gainNode.gain.value = value / 1000;
        console.log(sources.crickets.gainNode.gain.value);
      } catch (error) {
        console.log(error);
      } finally {
        loop();
