...
Google Cloud Build for Skia. The tracking bug was [skbug/9514](https://bugs.chromium.org/p/skia/issues/detail?id=9514).
...
Using cloud build "skia-release" and "skia-wasm-release" images were created and pushed per commit in the Skia
repository and the "infra" image was created and pushed per commit in the Buildbot repository.
...
Because of this work all failures when building apps cause the corresponding bots to turn red on status.skia.org for the
...
* Deploys apps to k8s using pushk for image the app's whitelist when they correspond to the latest commit.
