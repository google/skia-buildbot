...
// * "docker login ..." This is done everytime this function is run instead of once at startup because
...
//
func addDockerProdTag(ctx context.Context, ts oauth2.TokenSource, buildInfo docker_util.BuildInfo) error {
	// Doing docker login every time because it seems to expire after sometime, maybe when something
	// changes in the node host.
	token, err := ts.Token()
	if err != nil {
...
func tagProdToImage(ctx context.Context, fsClient *firestore.Client, gitRepo *gitiles.Repo, ts oauth2.TokenSource, buildInfo docker_util.BuildInfo) error {
	taggingMtx.Lock()
