...
func addDockerProdTag(ctx context.Context, buildInfo docker_util.BuildInfo) error {
	...
	pushCmd := fmt.Sprintf("docker push %s:%s", buildInfo.ImageName, buildInfo.Tag)
...
func tagProdToImage(ctx context.Context, fsClient *firestore.Client, gitRepo *gitiles.Repo, buildInfo docker_util.BuildInfo) error {
	...
	if len(docs) > 1 {
		...
		sklog.Infof("%s is newer than %s for %s. Going to apply the tag", buildInfo.Tag, fromDB.Tag, buildInfo.ImageName)
		if err := addDockerProdTag(ctx, buildInfo); err != nil {
			return skerr.Wrap(err)
		}
	}
...
func main() {
	...
	// Create token source.
	ts, err := auth.NewDefaultTokenSource(*local, auth.SCOPE_USERINFO_EMAIL, auth.SCOPE_FULL_CONTROL, pubsub.ScopePubSub, datastore.ScopeDatastore)
	if err != nil {
		sklog.Fatal(err)
	}

	// TODO(rmistry): What is this needed for?
	// Start gitauth.
	if !*local {
	...
	//hostURL, err := ParseHostURL("https://gcr.io")
	//if err != nil {
	//	sklog.Fatal(err)
	//}
	...
	//in the http headers.....
	//do everything else default I thin

	// httpClient2 := httputils.DefaultClientConfig().WithTokenSource(ts).Client()
	//httpClient2 := httputils.DefaultClientConfig().Client()
	httpClient2 := httputils.ClientConfig{Metrics: false}.Client()
	if err != nil {
		sklog.Fatal(err)
	}
	headers := map[string]string{"Authorization": fmt.Sprintf("Bearer %s", token.AccessToken)}
	dockerClient, err := docker_client.NewClient("https://gcr.io", "v1.24", httpClient2, headers)
	if err != nil {
		sklog.Fatalf("Could not instantiate docker client: %s", err)
	}
	containers, err := dockerClient.ContainerList(ctx, docker_types.ContainerListOptions{})
	if err != nil {
		sklog.Fatal(err)
	}
	fmt.Println(containers)
	if _, err := dockerClient.ImagePull(ctx, "gcr.io/skia-public/infra-v2:test", docker_types.ImagePullOptions{}); err != nil {
		sklog.Fatalf("Could not pull: %s", err)
	}
