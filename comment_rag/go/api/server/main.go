...
func main() {
	...
	cliApp := &cli.App{
		Name:  "comment_rag",
		Usage: "Command line tool that runs the Comment RAG api service.",
		...
		Commands: []*cli.Command{
			{
				Name:        "api",
				Usage:       "The comment rag api service",
				Description: "Runs the process that hosts the Comment RAG api service.",
				Flags:       (&flags).AsCliFlags(),
				Action: func(c *cli.Context) error {
					urfavecli.LogFlags(c)
					err := tracing.Init(flags.Local, "commentrag-api", 0.5)
					if err != nil {
						return err
					}
