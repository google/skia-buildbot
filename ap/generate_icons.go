...
const OUT_DIR = "./elements-sk/icon-sk/"
...
var SVG_CONTENT = regexp.MustCompile(`<svg .+?>(.+)</svg>`)
...
%s`, d, err)
	...
	// TODO(kjlubick) Maybe search for non-generated icons (i.e. custom ones not
	// given to us by Material Design) and also include those in the demos.
...
{{range .Names}}import 'elements-sk/icon-sk/{{.}}-icon-sk';
{{end}}`))
