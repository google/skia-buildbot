...
var (
	...
	MirrorUpdateRunningMutex sync.Mutex
	...
	for _, checkout := range checkouts {
		...
		group.Go(c, func() error {
			...
			if err := updateCheckout(ctx, c, false); err != nil {
				ac_util.UpdateCheckoutSyncFailureMetric(true, path.Base(c), hostname)
				return fmt.Errorf("Error when updating checkout in %s: %s", c, err)
			}
			ac_util.UpdateCheckoutSyncFailureMetric(false, path.Base(c), hostname)
			return nil
		})
	}
...
func setMirrorUpdateRunning(isRunning bool) {
	MirrorUpdateRunningMutex.Lock()
	defer MirrorUpdateRunningMutex.Unlock()
	MirrorUpdateRunning = isRunning
}

func getMirrorUpdateRunning() bool {
	MirrorUpdateRunningMutex.Lock()
	defer MirrorUpdateRunningMutex.Unlock()
	return MirrorUpdateRunning
}
...
func updateCheckout(ctx context.Context, checkoutPath string, isMirror bool) error {
	if isMirror {
		...
		setMirrorUpdateRunning(true)
		defer setMirrorUpdateRunning(false)
	} else {
		checkoutsMutex.RLock()
...
func RunCompileTask(ctx context.Context, g *gsFileLocation, task *ac_util.CompileTask, pathToCompileScript string) error {
	...
	if err := updateCheckout(ctx, checkoutPath, false); err != nil {
		ac_util.UpdateCheckoutSyncFailureMetric(true, path.Base(checkoutPath), hostname)
		return fmt.Errorf("Error when updating checkout in %s: %s", checkoutPath, err)
	}
	ac_util.UpdateCheckoutSyncFailureMetric(false, path.Base(checkoutPath), hostname)
