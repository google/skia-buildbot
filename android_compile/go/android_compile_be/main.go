...
// Run in a Goroutine to keep watching datastore for something.
...
func UpdateCompileTask(ctx context.Context, g *gsFileLocation, task *ac_util.CompileTask) error {
	...
	if _, err := ac_util.UpdateDSTask(ctx, task); err != nil {
...
func updateTaskInGoogleStorage(ctx context.Context, g *gsFileLocation, task ac_util.CompileTask) error {
	...
	b, err := json.Marshal(struct {
		...
		CompileTask: task,
		TaskID:      ac_util.GetDSKey(task.LunchTarget, task.Issue, task.PatchSet).Name,
	})
	if err != nil {
		...
		for {
			...
			if err := sub.Receive(ctx, func(ctx context.Context, m *pubsub.Message) {
				...
				if len(AvailableCheckoutsChan) == 0 {
					...
					sklog.Infof("Mirror is being updated right now. Nack'ing %s.", message.Name)
					m.Nack()
					return
				}
...
func main() {
	...
	if err := ac_util.UpdateInstanceInDS(ctx, hostname+"2", time.Now().Format("Mon Jan 2 15:04:05 MST"), *repoUpdateDuration, false); err != nil {
	...
	cleanup.Repeat(time.Minute, func() {
		...
		if err != nil {
			...
			fmt.Println("UPDATING THE MIRRO!!!!!!!!!!!!!!!")
			UpdateMirror(ctx)
		}
	}, nil)
	...
	go func() {
		...
		for _, t := range ownedPendingTasks {
			taskKey := ac_util.GetDSKey(t.LunchTarget, t.Issue, t.PatchSet)
	...
	go func() {
		for true {
			...
			taskKey := ac_util.GetDSKey(compileTask.LunchTarget, compileTask.Issue, compileTask.PatchSet)
			fileName := fmt.Sprintf("%s.json", taskKey.Name)
