...
func indexHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputils.ReportError(w, err, "Failed to get unowned/owned compile tasks")
		return
	}
	androidCompileInstances, err := util.GetAllCompileInstances(context.Background())
	if err != nil {
		httputils.ReportError(w, err, "Failed to get android compile instances")
		return
	}
...
func forceSyncHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := util.SetForceMirrorUpdateOnAllInstances(context.Background()); err != nil {
		httputils.ReportError(w, err, "Failed to set force mirror update on all instances")
		return
	}
