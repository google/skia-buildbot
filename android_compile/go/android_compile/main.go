...
var (
	...
	// Pubsub stuff?
	channels       = flag.String("channels", "expstorage:changed", "Comma separated list of event channels.")
...
func readGSAndTriggerCompileTask(ctx context.Context, g *gsFileLocation) error {
	...
	for _, existingTaskAndKey := range append(waitingTasksAndKeys, runningTasksAndKeys...) {
		if (task.Hash != "" && task.Hash == existingTaskAndKey.task.Hash) ||
			(task.Hash == "" && task.Issue == existingTaskAndKey.task.Issue && task.PatchSet == existingTaskAndKey.task.PatchSet) {
			// TODO(rmistry): Do nothing????????
			//if err := json.NewEncoder(w).Encode(map[string]interface{}{"taskID": existingTaskAndKey.key.ID}); err != nil {
			//	return fmt.Errorf("Failed to encode JSON", err)
			//}
...
// TODO(rmistry): I don't think you need to give taskID to the google storage object!!!!
...
func monitorForCloudPubSubEvents() {
	for range time.Tick(time.Duration(1 * time.Minute)) {
		fmt.Println("MONITORING HERE")
	}
}
...
type gsFileLocation struct {
	bucket string
	name   string
	// anything else then look in ingestion/helpers.go
...
func main() {
	...
	eventBus.SubscribeAsync(eventType, func(evt interface{}) {
		...
		if file.OverwroteGeneration != "" {
			fmt.Println("IT IS AN OVERIGHT")
			return
		} else {
			fmt.Println("IT IS NOT AN OVERIGHT")
		}
		sklog.Infof("Received storage event: %s / %s\n", file.BucketID, file.ObjectID)

		// Fetch the object attributes.
		objAttr, err := storageClient.Bucket(file.BucketID).
	...
	for _, taskAndKey := range runningTasksAndKeys {
		// rmistry: Create yoiur own objects here!!
		sklog.Infof("Found orphaned task %d. Retriggering it...", taskAndKey.key.ID)
		// Fetch the object attributes.
		fileName := fmt.Sprintf("%s-%s.json", taskAndKey.task.Issue, taskAndKey.task.PatchSet)
		objAttr, err := storageClient.Bucket(*storageBucket).
			Object(fileName).
			Attrs(context.TODO())
		if err != nil {
		...
		// TODO(rmistry): Make sure this works.
		resultCh <- newGCSFileLocation(objAttr, storageClient)

		//triggerCompileTask(ctx, taskAndKey.task, taskAndKey.key)
	}

	// Will have to create a client for Google storage here somewhere.

	// Wait for pubsub stuff.
	monitorForCloudPubSubEvents()
