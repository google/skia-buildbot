...
func triggerAndUpdateCompileTask(ctx context.Context, t *CompileTask) error {
	// About to trigger the task. Populate the Created timestamp.
	t.Created = time.Now()
	datastoreKey, err := PutDSTask(ctx, t)
	if err != nil {
...
func updateTaskInGoogleStorage(ctx context.Context, g *gsFileLocation, task CompileTask) error {
	b, err := json.Marshal(CompileTask{})
	if err != nil {
...
func main() {
	...
	go func() {
		for true {
			...
			if err = triggerAndUpdateCompileTask(ctx, compileTask); err != nil {
