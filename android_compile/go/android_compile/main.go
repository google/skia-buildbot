...
func registerRunHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, runningTaskAndKey := range runningTasksAndKeys {
		if (task.Hash != "" && (task.Hash == runningTaskAndKey.task.Hash)) ||
			...
			sklog.Infof("Got request for already running task [hash: %s, issue: %d, patchset: %d]. Returning already existing ID: %d", task.Hash, task.Issue, task.PatchSet, runningTaskAndKey.key.ID)
			return
		}
	}
