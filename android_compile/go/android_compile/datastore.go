...
func GetCompileTaskAndKeys() ([]*CompileTaskAndKey, []*CompileTaskAndKey, error) {
	waitingTaskAndKeys := []*CompileTaskAndKey{}
	runningTaskAndKeys := []*CompileTaskAndKey{}
	...
	for {
		...
		if t.Checkout == "" {
			waitingTaskAndKeys = append(waitingTaskAndKeys, taskAndKey)
		} else {
			runningTaskAndKeys = append(runningTaskAndKeys, taskAndKey)
		}
	}
	sort.Sort(sortTasks(waitingTaskAndKeys))
	sort.Sort(sortTasks(runningTaskAndKeys))

	return waitingTaskAndKeys, runningTaskAndKeys, nil
}

func GetCompileTasks() ([]*CompileTask, []*CompileTask, error) {
	waitingTaskAndKeys, runningTaskAndKeys, err := GetCompileTaskAndKeys()
	if err != nil {
		return nil, nil, err
	}
	waitingTasks := []*CompileTask{}
	for _, taskAndKey := range waitingTaskAndKeys {
