...
func updateCheckout(ctx context.Context, checkoutPath string, isMirror bool) error {
	...
	updateFunc := func() error {
		...
		defer func() {
			duration := timer.NewWithMetric(fmt.Sprintf("Time taken to update %s:", checkoutBase), syncTimesMetric).Stop()
			if isMirror {
				recreateMirror = duration > MAX_MIRROR_SYNC_TIME_BEFORE_RECREATION
				if recreateMirror {
					sklog.Infof("Mirror sync time %s was greater than %s", duration, MAX_MIRROR_SYNC_TIME_BEFORE_RECREATION)
