...
func CheckoutsInit(numCheckouts int, workdir string, repoUpdateDuration time.Duration, storageClient *storage.Client) error {
	...
	restoreOutput, err := os.Stat(pathToCcacheBackup)
	if err == nil {
		...
		sklog.Info("Output: %s", restoreOutput)
	}
...
func compileCheckout(ctx context.Context, checkoutPath, logFilePrefix, pathToCompileScript string) (bool, string, error) {
	...
	backupOutput, err := sk_exec.RunCwd(ctx, checkoutPath, rsyncCmd)
	if err != nil {
		return compileSuccess, "", fmt.Errorf("Failed to backup ccache: %s", err)
	}
	sklog.Info("Output: %s", backupOutput)
