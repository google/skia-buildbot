...
func ApplyAndCompilePatch(issue, patchset int, task *CompileTask, taskId int64) error {
	...
	withPatchSuccess, gsWithPatchLink, err := CompileCheckout(ctx, checkoutPath, fmt.Sprintf("%d_withpatch_", taskId))
	if err != nil {
	...
	if !withPatchSuccess {
		...
		noPatchSuccess, gsNoPatchLink, err := CompileCheckout(ctx, checkoutPath, fmt.Sprintf("%d_nopatch_", taskId))
		if err != nil {
