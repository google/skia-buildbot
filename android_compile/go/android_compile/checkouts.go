...
// goBackToOriginalSkiaState undoes all local changes to make the Skia checkout
// go back to the state it was in in the Androd repo.
func goBackToOriginalSkiaState(ctx context.Context, skiaCheckout *git.Checkout, androidSkiaHash string) {
	...
	if err := resetSkiaCheckout(ctx, skiaCheckout, "origin/master"); err != nil {
		sklog.Errorf("Error when resetting Skia checkout: %s", err)
		return
	}
	if _, err := skiaCheckout.Git(ctx, "checkout", androidSkiaHash); err != nil {
		sklog.Errorf("Failed to checkout %s: %s", androidSkiaHash, err)
		return
	}
	if err := deleteTryBranch(ctx, skiaCheckout); err != nil {
		sklog.Errorf("Failed to delete branch %s: %s", TRY_BRANCH_NAME, err)
		return
	}
}
...
// and compiling.
//
// Step 5: Get contents of SkUserConfigManual.h. We will use this after
// updating Skia to master/origin and before compiling.
//
// Step 6: Create a branch and have it track origin/master.
//
// Step 7: If it is a trybot run then apply the patch else apply the hash.
//
// Step 8: Prepare the Skia checkout for compilation: Run gn_to_bp.py and
// create SkUserConfigManual.h from Step 5.
//
// Step 9: Do the with patch or with hash compilation and update CompileTask
// with link to logs and whether it was successful.
//
// Step 10: If the compilation failed and if it is a trybot run then verify
...
func RunCompileTask(ctx context.Context, task *CompileTask, datastoreKey *datastore.Key) error {
	...
	// If anything fails from this point then go back to the original state.
	defer goBackToOriginalSkiaState(ctx, skiaCheckout, androidSkiaHash)
