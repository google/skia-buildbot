...
func removeLeftoverArtifacts(ctx context.Context, skiaCheckout *git.Checkout) error {
	if _, err := skiaCheckout.Git(ctx, "clean", "-d", "-f"); err != nil {
		return err
	}
	if _, err := skiaCheckout.Git(ctx, "reset", "--hard"); err != nil {
		return err
	}
...
func RunCompileTask(task *CompileTask, datastoreKey *datastore.Key) error {
	...
	// Remove any leftover artifacts and update the checkout.
	if err := removeLeftoverArtifacts(ctx, skiaCheckout); err != nil {
		return fmt.Errorf("Error when resetting Skia checkout: %s", err)
	}
	...
	if trybotRun {
		// Apply Patch
		if err := ApplyPatch(checkoutPath, task.Issue, task.PatchSet); err != nil {
			util.LogErr(cleanCheckout(ctx, skiaCheckout, androidSkiaHash))
