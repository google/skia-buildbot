...
func TestDynamicContentDiff(t *testing.T) {
	...
	left.SetNRGBA(0, 0, color.NRGBA{0, 255, 255, 0})
	left.SetNRGBA(0, 1, color.NRGBA{7, 7, 7, 0})

	right := image.NewNRGBA(image.Rect(0, 0, 2, 2))
	right.SetNRGBA(0, 1, color.NRGBA{7, 7, 7, 0})
	right.SetNRGBA(1, 0, color.NRGBA{128, 128, 128, 0})
	right.SetNRGBA(1, 1, color.NRGBA{0, 255, 255, 0})
