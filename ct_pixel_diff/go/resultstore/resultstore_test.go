...
func TestGetRunIDs(t *testing.T) {
	...
	// Specifying zero time for both start and end should return all the runIDs.
	runIDs, err = resultStore.GetRunIDs(time.Time{}, time.Time{})
	assert.NoError(t, err)
	...
	runIDs, err = resultStore.GetRunIDs(time.Time{}, splitTime)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(runIDs))
	assert.Equal(t, TEST_RUN_ID, runIDs[0])

	runIDs, err = resultStore.GetRunIDs(splitTime, time.Time{})
	assert.NoError(t, err)
	assert.Equal(t, 1, len(runIDs))
