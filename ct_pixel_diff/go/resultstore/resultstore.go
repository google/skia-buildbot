...
type ResultStore interface {
	...
	// GetFiltered returns cached results over the given threshold for the given runID.
	GetFiltered(runID string, startIdx int, threshold float32) ([]*ResultRec, int, error)
...
// GetFiltered returns CHUNK_SIZE ResultRecs whose PixelDiffPercent is greater
// than or equal to the given threshold. The filtering starts at the given start
// index for the runID's cached results. Also returns the next index so that the
// frontend knows where to start its next filter in the cache. Returns an error
// if there is no data cached for the runID.
func (b *BoltResultStore) GetFiltered(runID string, startIdx int, threshold float32) ([]*ResultRec, int, error) {
	if results, ok := b.cache[runID]; ok {
		ret := []*ResultRec{}
		i := 0
		for i < CHUNK_SIZE {
			...
			if results[startIdx].DiffMetrics.PixelDiffPercent >= threshold {
				ret = append(ret, results[startIdx])
				i++
			}
			startIdx++
		}
		return ret, startIdx + 1, nil
	} else {
