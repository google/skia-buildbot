...
type ResultStore interface {
	...
	// GetCached returns cached results in the given range for the given runID.
	GetCached(runID string, startIdx, endIdx int) ([]*ResultRec, error)

	// SortCache sorts the cached results for the given runID using the sort
	// parameters.
	SortCache(runID, sortField, sortOrder string) error
}
...
// GetCached returns all the ResultRecs in the specified range [startIdx,endIdx)
...
func (b *BoltResultStore) GetCached(runID string, startIdx, endIdx int) ([]*ResultRec, error) {
	if results, ok := b.cache[runID]; ok {
...
// SortCache sorts the ResultRecs for the given runID using the given sort
// parameter and sort order (ascending/descending). Returns an error if there
// is no data cached for the runID.
func (b *BoltResultStore) SortCache(runID, sortField, sortOrder string) error {
	if results, ok := b.cache[runID]; ok {
		switch sortField {
		...
		case GREEN_DIFF:
			{
				if sortOrder == ASC {
					sort.Slice(results, func(i, j int) bool {
						if results[i].DiffMetrics.MaxRGBADiffs[1] == results[j].DiffMetrics.MaxRGBADiffs[1] {
							return results[i].URL < results[j].URL
						}
