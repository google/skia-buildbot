...
var (
	...
	statusDir          = flag.String("status_dir", "ct_diff_status_dir", "Directory that stores the status for the ingester")
)
...
func main() {
	...
	if (*diffServerGRPCPort != "") || (*diffServerAddr != "") {
		...
		diffStore, err = diffstore.NewNetDiffStore(conn, *diffServerAddr, codec)
		if err != nil {
			sklog.Fatalf("Unable to initialize NetDiffStore: %s", err)
		}
	} else {
		diffStore, err = diffstore.NewMemDiffStore(client, nil, *imageDir, strings.Split(*gsBucketNames, ","), *gsBaseDirs, *cacheSize, diffstore.PixelDiffIDPathMapper{})
		if err != nil {
	...
	ingesterConfig := &sharedconfig.IngesterConfig{
		RunEvery:   sharedconfig.ConfDuration{time.Minute},
		MinDays:    30,
		StatusDir:  *statusDir,
	...
	source, err := ingestion.NewGoogleStorageSource(INGESTER_ID, *gsBucketNames, *gsBaseDirs, client)
	if err != nil {
	...
	// Initialize the cache for rendering and sorting results.
